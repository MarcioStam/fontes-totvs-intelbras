/******************************************************************************
  ...
  01/09/04  G Campbell      Fixed issue with hardcoding of temp\content.txt.
...
FUNCTION pdf_text_widthdec RETURNS DECIMAL ( INPUT pdfStream   AS CHARACTER,
  ...
  FIND FIRST TT_pdf_font WHERE TT_pdf_font.obj_stream = pdfStream
                           AND TT_pdf_font.font_name  = L_Font
                           AND TT_pdf_font.font_width <> "" NO-LOCK NO-ERROR.
  IF AVAIL TT_pdf_font THEN DO:
    IF TT_pdf_Font.font_type = "FIXED" THEN DO:
...
PROCEDURE pdf_close :
  ...
  OUTPUT STREAM S_pdf_inc TO VALUE( TT_pdf_stream.obj_file ) UNBUFFERED NO-CONVERT.
    ...
    IF CAN-FIND (FIRST TT_pdf_error WHERE TT_pdf_error.obj_stream = pdfStream NO-LOCK) THEN DO:
    ...
    ELSE DO:
      ...
      /* Load External Pages */
      IF CAN-FIND( FIRST TT_pdf_external WHERE TT_pdf_external.obj_stream = pdfStream NO-LOCK) THEN  
         RUN pdf_Load_External (pdfStream).                                                          
...
PROCEDURE pdf_set_parameter:
  ...
  IF NOT pdfParameter BEGINS "tmp" THEN DO:
    CASE pdfParameter:
...
PROCEDURE OutputTextContent: /* PRIVATE */
  ...
  IF pdf_Page(pdfStream) > 0 THEN DO:
    ...
    IF pdfType BEGINS "TEXT" AND TT_pdf_Stream.obj_DoingText THEN DO:
      RUN PutStreamContent(pdfStream, pdfContent).
      RETURN.
    END.
...
PROCEDURE PutStreamContent:  /* PRIVATE */
  ...
  PUT STREAM S_pdf_out UNFORMATTED pdfContent {&pdfSKIP}.
