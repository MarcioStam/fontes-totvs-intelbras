...
DEFINE FRAME d-vapara
     c_cod-usuario AT ROW 1.29 COL 19 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 2.71 COL 2.29
     bt-cancela AT ROW 2.71 COL 13.14
     bt-ajuda AT ROW 2.71 COL 32.43
     rt-button AT ROW 2.46 COL 1
...
DO:
FIND supervisor NO-LOCK
    WHERE supervisor.cod-usuario EQ INPUT FRAME {&FRAME-NAME} c_cod-usuario NO-ERROR.

IF NOT AVAILABLE supervisor THEN DO:
    {utp/ut-table.i mgesp supervisor 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(supervisor).
END.
