...
&Scoped-Define ENABLED-OBJECTS rt-button c_cnpj c_nr-filial bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cnpj c_nr-filial 
...
DEFINE VARIABLE c_cnpj AS CHARACTER FORMAT "X(15)":U 
     LABEL "CNPJ Filial" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c_nr-filial AS CHARACTER FORMAT "X(5)":U 
     LABEL "N£mero Filial" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_cnpj AT ROW 1 COL 23 COLON-ALIGNED WIDGET-ID 2
     c_nr-filial AT ROW 2 COL 23 COLON-ALIGNED WIDGET-ID 4
     bt-ok AT ROW 3.46 COL 7.29
     bt-cancela AT ROW 3.46 COL 19.43
     bt-ajuda AT ROW 3.5 COL 39.86
...
DO:
FIND filial-cliente NO-LOCK
    WHERE filial-cliente.cnpj EQ INPUT FRAME {&FRAME-NAME} c_cnpj
      AND filial-cliente.nr-filial EQ INPUT FRAME {&FRAME-NAME} c_nr-filial NO-ERROR.

IF NOT AVAILABLE filial-cliente THEN DO:
    {utp/ut-table.i mgesp filial-cliente 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(filial-cliente).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_cnpj c_nr-filial 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cnpj c_nr-filial bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
