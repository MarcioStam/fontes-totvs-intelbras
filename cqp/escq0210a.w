...
/********************************************************************************
*******************************************************************************/
{include/i-prgvrs.i ESCQ0210A 2.00.00.001}  /*** 010001 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
{include/i-license-manager.i ESCQ0210A MCQ}
&ENDIF
...
def input parameter r-exam-ficha as rowid no-undo.
...
DEFINE VARIABLE h_B04IN050 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 12.25 COL 3.86
     bt-cancela AT ROW 12.25 COL 15.86
     bt-ajuda AT ROW 12.25 COL 77
     rt-button AT ROW 12 COL 2.72
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Digita‡Æo dos Resultados dos Exames"
         HEIGHT             = 12.63
         WIDTH              = 90
         MAX-HEIGHT         = 36.67
         MAX-WIDTH          = 182.86
         VIRTUAL-HEIGHT     = 36.67
...
ON END-ERROR OF w-cadsim /* Digita‡Æo dos Resultados dos Exames */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Digita‡Æo dos Resultados dos Exames */
DO:
...
DO:
    DEF VAR c-aux-return AS CHAR NO-UNDO.

    IF c-nom-prog-upc-mg97 <> "" THEN DO:                                                     
        RUN VALUE(c-nom-prog-upc-mg97) (INPUT "Salvar",                                  
                                        INPUT "FRAME",                                    
                                        INPUT THIS-PROCEDURE,                               
                                        INPUT FRAME f-cad:HANDLE,                       
                                        INPUT string("exam-ficha"), 
                                        INPUT ROWID(exam-ficha)). 
        ASSIGN c-aux-return = RETURN-VALUE.
    END.
    IF c-aux-return <> "erro-upc-bt-ok":U THEN DO:
        find first exam-ficha exclusive-lock
            where rowid(exam-ficha) = r-exam-ficha no-error.
        if avail exam-ficha then
            assign exam-ficha.dt-laudo = today.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Componentes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 2.72 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.71 , 86.86 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'cqp/ESCQ0210A-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_B04IN050 ).
       RUN set-position IN h_B04IN050 ( 2.63 , 4.43 ) NO-ERROR.
       /* Size in UIB:  ( 8.88 , 83.14 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_B04IN050 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-enable w-cadsim 
...
------------------------------------------------------------------------------*/
  ...
  run pi-reabre in h_B04IN050 (input r-exam-ficha).
...
{utp/ut9000.i "ESCQ0210A" "2.00.00.006"}
    
  ASSIGN h-browse = h_B04IN050.
  /* Dispatch standard ADM method.                             */
...
-------------------------------------------------------------*/
  ...
  RUN "cqp/ESCQ0210A-b01.w *RTB-SmObj* ".
