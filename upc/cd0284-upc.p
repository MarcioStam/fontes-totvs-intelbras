...
DEFINE NEW GLOBAL SHARED VARIABLE c-estab-ini       AS CHARACTER   NO-UNDO INITIAL "":U.
DEFINE NEW GLOBAL SHARED VARIABLE c-estab-fim       AS CHARACTER   NO-UNDO.
...
DEFINE VARIABLE wh-motivo-sit-item AS WIDGET-HANDLE NO-UNDO.
...
/*--[ DESVIO NECESSµRIO POR CAUSA DE PONTO NOVO CRIADO PELA TOTVS, ONDE O PARAMETRO 
      P-WGH-OBJECT ESTµ PASSANDO ?, ESTOURANDO ERRO NO FILE-NAME ]-------------------------------*/
/*IF  p-ind-event = "rg-desc-ordem-cc" THEN RETURN "OK".*/
/*-----------------------------------------------------------------------------------------------*/

IF p-ind-event <> "rg-desc-pedido":U THEN
ASSIGN c-objeto = ENTRY(NUM-ENTRIES(p-wgh-object:FILE-NAME, "/":U), p-wgh-object:FILE-NAME, "/":U) NO-ERROR.
...
ASSIGN h-frame = p-wgh-frame:FIRST-CHILD
       h-frame = h-frame:FIRST-CHILD NO-ERROR.
...
IF p-ind-event  = "BEFORE-INITIALIZE":U AND
   p-ind-object = "VIEWER":U            THEN DO:
    ...
    CREATE FILL-IN wh-motivo-sit-item
    ASSIGN NAME              = "wh-motivo-sit-item":U
           FRAME             = p-wgh-frame
           DATA-TYPE         = "CHARACTER":U
           FORMAT            = "x(30)":U
           WIDTH             = 15
...
IF p-ind-event  = "DISPLAY":U AND
   p-ind-object = "VIEWER":U  THEN DO:
    ...
    DO WHILE VALID-HANDLE(h-frame):
        IF h-frame:TYPE <> "field-group":U THEN DO:
            CASE h-frame:NAME:
                ...
                WHEN "wh-motivo-sit-item" THEN ASSIGN wh-motivo-sit-item = h-frame.
            END CASE.
    ...
    ASSIGN wh-motivo-sit-item:FGCOLOR  = 12.
    ...
    ASSIGN wh-txt-fornec:SCREEN-VALUE    = "Fornecedor:":U
           ...
           wh-motivo-sit-item:SCREEN-VALUE = "":U.
    ...
    IF AVAILABLE item THEN DO:
        FIND FIRST int-item-uni-estab
            WHERE int-item-uni-estab.it-codigo   = item.it-codigo
              AND int-item-uni-estab.cod-estabel = c-estab-ini NO-LOCK NO-ERROR.
        ...
        FIND FIRST item-fornec-estab
            WHERE item-fornec-estab.it-codigo   = item.it-codigo
              AND item-fornec-estab.cod-estabel = c-estab-ini
        ...
        ASSIGN wh-motivo-sit-item:SCREEN-VALUE = ''.
        ...
        FIND FIRST item-uni-estab
             WHERE item-uni-estab.it-codigo   = item.it-codigo
               AND item-uni-estab.cod-estabel = c-estab-ini NO-LOCK NO-ERROR.
                ...
                IF NOT l-sit-dif-estab THEN DO:
                   ...
                   IF AVAIL int-item THEN DO:
                      CASE int-item.motivo-situacao:
                         WHEN 1 THEN
                            ASSIGN wh-motivo-sit-item:SCREEN-VALUE = "Alteraá∆o de estrutura".
                         WHEN 2 THEN
                            ASSIGN wh-motivo-sit-item:SCREEN-VALUE = "Phase out produto".
                         WHEN 3 THEN
                            ASSIGN wh-motivo-sit-item:SCREEN-VALUE = "Item EOL".
                         WHEN 4 THEN
                            ASSIGN wh-motivo-sit-item:SCREEN-VALUE = "Bloqueado compra".
                      END CASE.
                   END.
                END.
                ELSE DO:
       ...
       valid-handle(wh-neces) THEN DO:
        ...
        IF h-tipo:SCREEN-VALUE = "O C":U THEN DO:
            FOR FIRST item NO-LOCK
                ...
                WHERE ordem-compra.cod-estabel  = c-estab-ini
                ...
                FOR EACH ordens-embarque NO-LOCK
                    ...
                    CASE tt-emb.situacao:
                        ...
                        WHEN 98 THEN ASSIGN c-depos = "DI".
...
IF p-ind-event  = "DESTROY":U   AND
   p-ind-object = "CONTAINER":U THEN DO:
    ...
    IF VALID-HANDLE (wh-motivo-sit-item) THEN
        DELETE WIDGET wh-motivo-sit-item.
