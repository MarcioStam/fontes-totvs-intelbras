...
def new global shared temp-table tt-pedido-cm0201
...
if  p-ind-event  = "INITIALIZE" and 
    ...
    assign frame     = p-wgh-frame 
           ...
           tooltip   = "UPC"
           triggers:
             on choose persistent run upc/cm0201a-upc.w.
           end triggers.
...
IF p-ind-object    = "browser":U     AND
   p-ind-event = "before-display" THEN DO:    
    for each tt-pedido-cm0201:
        delete tt-pedido-cm0201.
    end.    
end.    
IF p-ind-object    = "browser":U     AND
   p-ind-event = "value-changed" THEN DO:
    ...
    DO i-linha = 1 TO wh-browse:NUM-SELECTED-ROWS :
        if wh-browse:fetch-selected-row(i-linha) then do:       
            find first tt-pedido-cm0201
                 where tt-pedido-cm0201.nr-pedcli = hped-venda-nr-pedcli:BUFFER-VALUE 
                   and tt-pedido-cm0201.nome-abrev = hped-venda-nome-abrev:BUFFER-VALUE 
                 no-lock no-error.
            if not avail tt-pedido-cm0201 then do:
               create tt-pedido-cm0201.
               assign tt-pedido-cm0201.nr-pedcli = hped-venda-nr-pedcli:BUFFER-VALUE 
                      tt-pedido-cm0201.nome-abrev = hped-venda-nome-abrev:BUFFER-VALUE.
            end.               
        end.
    END.
    /* for each tt-pedido: */
