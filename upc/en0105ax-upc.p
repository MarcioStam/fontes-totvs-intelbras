...
**       Objetivo: Exportacao de locais de montagem da estrutura.
**
*****************************************************************************/
def input param r-estrutura as rowid   no-undo.
...
/*
find estrutura where rowid(estrutura) = r-estrutura no-lock.
*/
FOR FIRST estrutura 
    WHERE ROWID(estrutura) = vRowEstrutura.
END.

/*IF NOT AVAIL estrutura THEN RETURN "OK".
R(2,5,13-14,17-18)
*/
...
assign c-item-estrut  = estrutura.es-codigo
       c-local-mont   = cLocalMontagem /*estrutura.local-mont*/.

run pi-sobe-estrut (estrutura.es-codigo, c-item-estrut).
...
procedure pi-sobe-estrut:
   ...
   find first b-estrutura where b-estrutura.es-codigo 
                              = c-item 
                          no-lock no-error.
   if not avail b-estrutura then do:
      ...
      for each b-estrutura where b-estrutura.es-codigo = c-item 
                           no-lock:
/*          message b-estrutura.it-codigo b-estrutura.es-codigo c-parte view-as alert-box. */
          if b-estrutura.it-codigo begins "7" or 
             b-estrutura.it-codigo begins "8" then do:
             find first tt-modelo where tt-modelo.modelo = b-estrutura.es-codigo
                                    and tt-modelo.parte  = c-parte
                                  no-error.
             if not avail tt-modelo then do:                     
                create tt-modelo.
                assign tt-modelo.modelo = b-estrutura.es-codigo
                       tt-modelo.parte  = c-parte.
             end.
          end.
          else do:
             /* NÆo ser  mais utilizado segundo claudiney - Garantia foi desativado
             if b-estrutura.u-dec-1 = 0 then */
                assign c-parte = b-estrutura.it-codigo.
             run pi-sobe-estrut(b-estrutura.it-codigo, c-parte).
          end.
      end.
   end.
end.
