...
{include/i-prgvrs.i cd0355-upca 2.06.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        cd0355-upca
&GLOBAL-DEFINE Version        2.06.00.001

&GLOBAL-DEFINE WindowType     Detail

&GLOBAL-DEFINE Folder         YES
&GLOBAL-DEFINE InitialPage    4
&GLOBAL-DEFINE FolderLabels   Layout,Parƒmetro,ImpressÆo
...
&GLOBAL-DEFINE page4Widgets   textArquivo cArquivo btPesquisar
...
DEFINE VARIABLE c-imp-old       AS CHAR.
DEFINE VARIABLE c-arq-old       AS CHAR.
DEFINE VARIABLE c-arq-old-batch AS CHAR.
...
DEFINE VARIABLE cEndField AS CHARACTER FORMAT "X(15)":U INITIAL "ZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE cIniField AS CHARACTER FORMAT "X(15)":U 
     LABEL "Campo" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88
     FONT 1 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brDigita wWindow _FREEFORM
...
DEFINE FRAME fPage2
     cIniField AT ROW 1.5 COL 15.43
     cEndField AT ROW 1.5 COL 54.14 NO-LABEL
...
/* SETTINGS FOR FRAME fPage2
                                                                        */
/* SETTINGS FOR FILL-IN cEndField IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN cIniField IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FRAME fPage3
                                                                        */
/* SETTINGS FOR FRAME fPage4
                                                                        */
/* SETTINGS FOR FILL-IN textArquivo IN FRAME fPage4
...
&Scoped-define SELF-NAME brDigita
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL brDigita wWindow
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btConfigImprDest wWindow
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btModelRtf wWindow
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btOpen wWindow
...
ON CHOOSE OF btPesquisar IN FRAME fPage4 /* Pesquisar */
DO:
    DEFINE VARIABLE cConvFile AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE lOk       AS LOGICAL     NO-UNDO.

    ASSIGN cConvFile = REPLACE(INPUT FRAME fPage4 cArquivo, "/":U, "~\":U).
...
ON / OF cArquivo IN FRAME fPage4
DO:
    APPLY "~\":U TO cArquivo IN FRAME fPage4.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cArquivo wWindow
ON LEAVE OF cArquivo IN FRAME fPage4
DO:
    ASSIGN cArquivo = REPLACE(INPUT FRAME fPage4 cArquivo, "/":U, "~\":U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL l-habilitaRtf wWindow
    ...
    case self:screen-value:
        when "1":U then do:
            assign cFile:sensitive       = no
                   ...
                   l-habilitaRtf:SCREEN-VALUE IN FRAME fPage3 = "No"
                   l-habilitaRtf = NO
                   &endif
                   .
                   /*Fim alteracao 15/02/2005*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rsExecution wWindow
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN edLayout = "+----------------------------------------------------------------------------+":U + CHR(10) +
                      ...
                      "|----------+-----------------------------------------------------------------|":U + CHR(10) +
                      "|      01  |  Codigo Tributo                                                 |":U + CHR(10) +
                      "|      02  |  E/S - Entrada ou Saida                                         |":U + CHR(10) +
                      "|      03  |  Data Inic Validade                                             |":U + CHR(10) +
                      "|      04  |  Estabelecimento                                                |":U + CHR(10) +
                      "|      05  |  Natureza de Opera‡Æo                                           |":U + CHR(10) +
                      "|      06  |  Classifica‡Æo Fiscal                                           |":U + CHR(10) +
                      "|      07  |  Codigo do Item                                                 |":U + CHR(10) +
                      "|      08  |  Grupo Emitente                                                 |":U + CHR(10) +
                      "|      09  |  C¢digo Emitente                                                |":U + CHR(10) +
                      "+----------------------------------------------------------------------------+":U.
...
do on error undo, return error on stop  undo, return error:
    ...
    {report/rprun.i upc/cd0355-upca-rp.p}
