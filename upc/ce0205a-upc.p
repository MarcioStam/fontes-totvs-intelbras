...
DEFINE NEW GLOBAL SHARED VARIABLE wh-cod-estabel-ce0205a-upc AS WIDGET-HANDLE NO-UNDO.
DEFINE NEW GLOBAL SHARED VARIABLE wh-cod-unid-negoc-ce0205a-upc AS WIDGET-HANDLE NO-UNDO.
DEFINE NEW GLOBAL SHARED VARIABLE wh-sc-codigo-ce0205a-upc AS WIDGET-HANDLE NO-UNDO.
...
IF  p-ind-event  = "initialize" AND 
    ...
    RUN busca-handle(INPUT p-wgh-frame,
                     INPUT "cod-estabel",
                     OUTPUT wh-cod-estabel-ce0205a-upc).

    RUN busca-handle(INPUT p-wgh-frame,
                     INPUT "c-ccusto",
                     OUTPUT wh-sc-codigo-ce0205a-upc).

    RUN busca-handle(INPUT p-wgh-frame,
                     INPUT "cod-unid-negoc",
                     OUTPUT wh-cod-unid-negoc-ce0205a-upc).
...
IF  p-ind-event  = "VALIDATE" AND 
   ...
   END.
    ...
    IF wh-sc-codigo-ce0205a-upc:SCREEN-VALUE <> "" THEN DO:
    
        FOR FIRST cc_uni_estab NO-LOCK
            WHERE cc_uni_estab.cod_estab  = wh-cod-estabel-ce0205a-upc:SCREEN-VALUE
            AND   cc_uni_estab.cod_ccusto = wh-sc-codigo-ce0205a-upc:SCREEN-VALUE:
        
            if wh-cod-unid-negoc-ce0205a-upc:SCREEN-VALUE <> cc_uni_estab.cod_unid_negoc THEN DO:
        
                RUN utp/ut-msgs.p (INPUT "show",
                                   INPUT 17006,
                                   INPUT "Unidade de Neg¢cio inv†lida para CC: " + wh-sc-codigo-ce0205a-upc:SCREEN-VALUE + " - Estab: " + wh-cod-estabel-ce0205a-upc:SCREEN-VALUE + ". Informe a Unidade do CC: " + cc_uni_estab.cod_unid_negoc + ". ~~ Para este Centro de Custo, utilizar unidade " + cc_uni_estab.cod_unid_negoc + ", conforme Planilha de Centros de Custo dispon°vel na INTRANET.").
        ...
        IF NOT AVAIL cc_uni_estab THEN DO:
            ...
            RUN utp/ut-msgs.p (INPUT "show",
                               INPUT 17006,
                               INPUT "CC: " + wh-sc-codigo-ce0205a-upc:SCREEN-VALUE + " - Estab: " + wh-cod-estabel-ce0205a-upc:SCREEN-VALUE + " n∆o possui Unidade de Neg¢cio relacionada (ESFGL001). Solicitar o cadastro Ö Controladoria.").
...
IF  p-ind-event  = "ASSIGN" AND 
    ...
    IF AVAIL movto-estoq THEN DO:
        ...
        ASSIGN c-mensagem = "Ol†," + "~n" +
                            "O usu†rio " + movto-estoq.usuario + "-" + (IF AVAIL usuar_mestre THEN usuar_mestre.nom_usuario ELSE "") + " fez uma devoluá∆o de material para: " + "~n" +
       ...
       IF movto-estoq.cod-estabel = "101" AND movto-estoq.ct-codigo BEGINS "4" THEN DO:
           ...
           IF AVAIL int-centro-custo THEN DO:
               ...
               IF NOT AVAIL usuar_mestre OR usuar_mestre.cod_e_mail_local = "" THEN DO:
                   RUN enviaMail (INPUT "ems@intelbras.com.br",
                                  ...
                                  INPUT "CE0205A-Devoluá∆o Materiais",
                                  INPUT "O usu†rio " + movto-estoq.usuario + "-" + (IF AVAIL usuar_mestre THEN usuar_mestre.nom_usuario ELSE "") + " est† tentando fazer devoluá∆o de material e n∆o existe e-mail cadastrado para usu†rio de matr°cula: " + STRING(int-centro-custo.cod_usuario) + " sendo que est† cadastrado como supervisor do Centro de custo " + movto-estoq.sc-codigo + ". Favor verificar!", 
                                  INPUT "").
                   MESSAGE "N∆o Ç poss°vel fazer devoluá∆o, n∆o existe e-mail cadastrado para usu†rio de matr°cula: " + STRING(int-centro-custo.cod_usuario) + " sendo que est† cadastrado como supervisor do Centro de custo " + movto-estoq.sc-codigo + ". Favor entrar em contato com Setor Controladoria!"
                       VIEW-AS ALERT-BOX INFO BUTTONS OK.
                   RETURN "NOK".
               END.
               ELSE DO:
           ...
           ELSE DO:
               RUN enviaMail (INPUT "ems@intelbras.com.br",
                              INPUT "grupo.contabil@intelbras.com.br;grupo.custos@intelbras.com.br",
                              INPUT "CE0205A-Devoluá∆o Materiais",
                              INPUT "O usu†rio " + movto-estoq.usuario + "-" + (IF AVAIL usuar_mestre THEN usuar_mestre.nom_usuario ELSE "") + " est† tentando fazer devoluá∆o de material e n∆o existe supervisor cadastrado para Centro de custo " + movto-estoq.sc-codigo + ". Favor verificar!",
                              INPUT "").
               MESSAGE "N∆o Ç poss°vel fazer devoluá∆o, n∆o existe supervisor cadastrado para Centro de custo " + movto-estoq.sc-codigo + ". Favor entrar em contato com a †rea cont†bil atravÇs do e-mail grupo.contabil@intelbras.com.br"
                   VIEW-AS ALERT-BOX INFO BUTTONS OK.
               RETURN "NOK".
           END.
       END.
