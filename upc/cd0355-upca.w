...
*******************************************************************************/
{include/i-prgvrs.i CD0355-UPCA 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i CD0355-UPCA esp}
&ENDIF
...
&GLOBAL-DEFINE FolderLabels   Layout,ParÉmetro,Impress∆o
...
DEFINE VARIABLE text-entrada AS CHARACTER FORMAT "X(256)":U INITIAL "Arquivo de Importaá∆o" 
      VIEW-AS TEXT 
     SIZE 16.14 BY .63
     FONT 1 NO-UNDO.
...
DEFINE FRAME fPage4
     btInputFile AT ROW 4.21 COL 79.14 HELP
          "Escolha do nome do arquivo"
     cInputFile AT ROW 4.29 COL 5 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     text-entrada AT ROW 3.25 COL 3.86 NO-LABEL
     RECT-12 AT ROW 3.5 COL 3
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE usuar_mestre             AND
       usuar_mestre.nom_dir_spool <> "":U THEN DO:
        ASSIGN cInputFile = REPLACE(usuar_mestre.nom_dir_spool, "/":U, "~\":U).

        IF usuar_mestre.nom_subdir_spool <> "":U THEN DO:
            IF SUBSTRING(cInputFile, LENGTH(cInputFile), 1) <> "~\":U THEN
                ASSIGN cInputFile = cInputFile + "~\":U.

            ASSIGN cInputFile = cInputFile + usuar_mestre.nom_subdir_spool.
    ...
    IF cInputFile = "":U THEN DO:
        ASSIGN cInputFile = "C:\temp":U.
    ...
    IF cInputFile = "":U THEN DO:
        ASSIGN cInputFile = REPLACE(SESSION:TEMP-DIRECTORY, "/":U, "~\":U).
    ...
    ASSIGN cInputFile = cInputFile + LC(c-programa-mg97) + ".csv":U.
...
do on error undo, return error on stop  undo, return error:
    ...
    if  tt-param.destino = 2 then 
        assign tt-param.arq-destino = input frame fPage6 cFile.
    else
        assign tt-param.arq-destino = session:temp-directory + c-programa-mg97 + ".tmp":U.    
        
        
    if       tt-param.destino = 1  then assign tt-param.arquivo = "":U.
    else if  tt-param.destino = 2  then assign tt-param.arquivo = input frame fPage6 cFile.
