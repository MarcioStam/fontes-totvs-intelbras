...
DEFINE NEW GLOBAL SHARED VARIABLE wh-btUpdate-of0147 AS WIDGET-HANDLE NO-UNDO.
...
if p-ind-event  = "AFTER-INITIALIZE" then do:
    ...
    DO WHILE VALID-HANDLE(h-object):
        IF h-object:TYPE <> "field-group" THEN DO:
            ...
            IF h-object:NAME = "btUpdate":U THEN DO:
                ASSIGN wh-btUpdate-of0147 = h-object.
                LEAVE.
            END.
    ...
    if valid-handle(wh-btUpdate-of0147) then do:
    
        create button wh-button  
        assign flat-button   = YES
               frame         = wh-btUpdate-of0147:frame 
               width         = wh-btUpdate-of0147:WIDTH
               height        = wh-btUpdate-of0147:HEIGHT
               row           = wh-btUpdate-of0147:ROW
               col           = wh-btUpdate-of0147:COLUMN + 25
               visible       = yes
               sensitive     = yes
               tooltip       = "Importar Planilha"
               triggers:
                    on choose persistent run upc/of0147a-upc.w.
               end triggers.
...
IF p-ind-event  = "DESTROY":U   AND
   p-ind-object = "CONTAINER":U THEN DO:
    ...
    IF VALID-HANDLE(wh-btUpdate-of0147) THEN
        DELETE WIDGET wh-btUpdate-of0147.

    ASSIGN wh-btUpdate-of0147 = ?.
END.
