/* -------------------------------------------------------------------------------------------------------------
Programa : upc/mod_portador-upc.p
Funcao   : UPC criada para permitir modificar o campo "Considera Comiss∆o Portador" no cadastro do portador do 
...
IF p-ind-event = "DISPLAY" AND 
   p-ind-object = "VIEWER" THEN DO:
    ...
    FOR FIRST int-portador OF emscad.portador:
        ASSIGN wh-toggle-mod:CHECKED = int-portador.log_considera_comissao.
    END.
END.
...
IF p-ind-event = "ASSIGN" AND 
   p-ind-object = "VIEWER" THEN DO:
    ...
    FIND FIRST emscad.portador WHERE RECID(portador) = p-row-table.
    FIND FIRST int-portador OF emscad.portador EXCLUSIVE-LOCK NO-ERROR.
    IF NOT AVAIL int-portador THEN DO:
        CREATE int-portador.
    END.
    ASSIGN int-portador.cod_portador           = emscad.portador.cod_portador
           int-portador.log_considera_comissao = wh-toggle-mod:CHECKED.
