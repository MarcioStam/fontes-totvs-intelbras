...
*******************************************************************************/
{include/i-prgvrs.i en0507-upc01 1.00.00.002}
...
DO:
    assign des-cor:screen-value in frame fPage1 = "".

    for first int-cor
        where int-cor.cod-cor = input frame fPage1 tt-int-item.cod-cor
              no-lock: end.

    if avail int-cor
    then assign tt-int-item.cod-cor:screen-value in frame fPage1 = trim(int-cor.cod-cor)
                des-cor:screen-value             in frame fPage1 = trim(int-cor.desc-cor).
END.
...
/*                          &EnableImplant="NO"}                */
  ...
  {include/zoomvar.i &prog-zoom  = esp/cdp/escdp103-z01.w
                     &campo      = tt-int-item.cod-cor
                     &campozoom  = cod-cor}
END.
...
DO:
    assign des-mod-pl:screen-value in frame fPage1 = "".

    for first int-modelo-placa
        where int-modelo-placa.cod-modelo-placa = input frame fPage1 tt-int-item.cod-modelo-placa
              no-lock: end.

    if avail int-modelo-placa
    then assign tt-int-item.cod-modelo-placa:screen-value in frame fPage1 = trim(int-modelo-placa.cod-modelo-placa)
                des-mod-pl:screen-value                   in frame fPage1 = trim(int-modelo-placa.desc-modelo-placa).
END.
...
ON LEAVE OF tt-int-item.cod-potencia IN FRAME fPage1 /* Potància */
DO:
    assign des-pot:screen-value in frame fPage1 = "".

    for first int-potencia
        where int-potencia.cod-potencia = input frame fPage1 tt-int-item.cod-potencia
              no-lock: end.

    if avail int-potencia
    then assign tt-int-item.cod-potencia:screen-value in frame fPage1 = trim(int-potencia.cod-potencia)
                des-pot:screen-value                  in frame fPage1 = trim(int-potencia.desc-potencia).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-int-item.cod-potencia wWindow
ON MOUSE-SELECT-DBLCLICK OF tt-int-item.cod-potencia IN FRAME fPage1 /* Potància */
DO:
...
DO:
    assign des-tam-pl:screen-value in frame fPage1 = "".

    for first int-tam-placa
        where int-tam-placa.cod-tam-placa = input frame fPage1 tt-int-item.cod-tam-placa
              no-lock: end.

    if avail int-tam-placa
    then assign tt-int-item.cod-tam-placa:screen-value in frame fPage1 = trim(int-tam-placa.cod-tam-placa)
                des-tam-pl:screen-value                in frame fPage1 = trim(int-tam-placa.desc-tam-placa).
END.
...
DO:
    assign des-tip-mp:screen-value in frame fPage1 = "".

    for first int-tipo-mp
        where int-tipo-mp.cod-tipo-mp = input frame fPage1 tt-int-item.cod-tipo-mp
              no-lock: end.

    if avail int-tipo-mp
    then assign tt-int-item.cod-tipo-mp:screen-value in frame fPage1 = trim(int-tipo-mp.cod-tipo-mp)
                des-tip-mp:screen-value              in frame fPage1 = trim(int-tipo-mp.desc-tipo-mp).
END.
...
------------------------------------------------------------------------------*/
    ...
    apply "entry":U to tt-int-item.cod-cor in frame fPage1.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
apply 'leave' to tt-int-item.cod-cor          in frame fPage1.
apply 'leave' to tt-int-item.cod-tipo-mp      in frame fPage1.
apply 'leave' to tt-int-item.cod-modelo-placa in frame fPage1.
apply 'leave' to tt-int-item.cod-tam-placa    in frame fPage1.
apply 'leave' to tt-int-item.cod-potencia     in frame fPage1.

if  input frame fPage1 tt-int-item.cod-cor <> ""
and not avail int-cor
then do:
     run utp/ut-msgs.p (input "show":U, input 17567, input "Cor n∆o cadastrada!").
     apply 'entry' to tt-int-item.cod-cor in frame fPage1.
     return 'ADM-ERROR':U.
end.

if  input frame fPage1 tt-int-item.cod-tipo-mp <> ""
and not avail int-tipo-mp
then do:
     run utp/ut-msgs.p (input "show":U, input 17567, input "Tipo MP n∆o cadastrado!").
     apply 'entry' to tt-int-item.cod-tipo-mp in frame fPage1.
     return 'ADM-ERROR':U.
end.

if  input frame fPage1 tt-int-item.cod-modelo-placa <> ""
and not avail int-modelo-placa
then do:
     run utp/ut-msgs.p (input "show":U, input 17567, input "Modelo Placa n∆o cadastrado!").
     apply 'entry' to tt-int-item.cod-modelo-placa in frame fPage1.
     return 'ADM-ERROR':U.
end.

if  input frame fPage1 tt-int-item.cod-tam-placa <> ""
and not avail int-tam-placa
then do:
     run utp/ut-msgs.p (input "show":U, input 17567, input "Tamanho Placa n∆o cadastrado!").
     apply 'entry' to tt-int-item.cod-tam-placa in frame fPage1.
     return 'ADM-ERROR':U.
end.

if  input frame fPage1 tt-int-item.cod-potencia <> ""
and not avail int-potencia
then do:
     run utp/ut-msgs.p (input "show":U, input 17567, input "Potància n∆o cadastrada!").
     apply 'entry' to tt-int-item.cod-potencia in frame fPage1.
     return 'ADM-ERROR':U.
end.
