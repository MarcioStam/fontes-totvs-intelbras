...
IF p-ind-event = "BEFORE-DISPLAY" AND p-ind-object = "VIEWER" THEN DO:

    FIND FIRST ctrab
        WHERE ROWID(ctrab) = p-row-table NO-LOCK NO-ERROR.

    IF AVAIL ctrab THEN DO:
        IF  VALID-HANDLE(wh-tg-consid-aps) THEN
            ASSIGN wh-tg-consid-aps:SENSITIVE = NO
                   wh-tg-consid-aps:CHECKED = ctrab.log-1.
    END.
END.
