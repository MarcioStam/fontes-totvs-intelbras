...
DEF NEW GLOBAL SHARED VAR wgh-query         AS widget-handle no-undo.
DEF NEW GLOBAL SHARED VAR wgh-buffer        AS widget-handle no-undo.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "bas_fornecedor_fin_epc_04" "2.00.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF  VALID-HANDLE(wh_fornec_bas_fin)
  AND wh_fornec_bas_fin:SCREEN-VALUE <> "" THEN
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH compl_movto_pagto NO-LOCK
        ...
        AND   compl_movto_pagto.cdn_fornecedor = int(wh_fornec_bas_fin:SCREEN-VALUE)
        BY dat_pagto DESC:
        ...
        IF  movto_tit_ap.log_movto_estordo THEN 
            NEXT.
