/*****************************************************************************
** Programa..............: fas705ca_epc.p
** Descricao.............: EPC do programa isl_bem_pat_ctbz
...
DEF VAR h_fas705ca_epc_1 AS HANDLE NO-UNDO.
...
DEF NEW GLOBAL SHARED VAR h_zoo_cenar        AS WIDGET-HANDLE  NO-UNDO.
...
IF  p_ind_event = "INITIALIZE" THEN DO:
    ...
    RUN epc/fas705ca_epc_01.p PERSISTENT SET h_fas705ca_epc_1(INPUT p_ind_event,
                                                              INPUT p_ind_object,
                                                              INPUT p_wgh_object,
                                                              INPUT p_wgh_frame,
                                                              INPUT p_cod_table,
    ...
    DO WHILE VALID-HANDLE(h_object):
        IF  h_object:TYPE <> "field_group" THEN DO:
            IF  h_object:NAME = "ind_dwb_set_type" THEN 
                ASSIGN h_ind_dwb_set_type = h_object.
        ...
        IF  VALID-HANDLE(h_ind_dwb_set_type) THEN 
            LEAVE.
    END.

    IF  VALID-HANDLE(h_ind_dwb_set_type) THEN
        ON "VALUE-CHANGED" OF h_ind_dwb_set_type PERSISTENT RUN pi_trata_cenario IN h_fas705ca_epc_1 .
...
IF  p_ind_event = 'DISPLAY' THEN DO:
    ...
    IF  NOT AVAIL tt-prog-ponto THEN DO:
        ...
        DO WHILE VALID-HANDLE(h_object):
            IF  h_object:TYPE <> "field_group" THEN DO:
                IF  h_object:NAME = "v_cod_cenar_ctbl" THEN 
                    ASSIGN h_cenar = h_object.
        ...
        ASSIGN h_cenar:SCREEN-VALUE = ""
               h_cenar:SENSITIVE    = NO.
        ...
        DO WHILE VALID-HANDLE(h_object):
            IF  h_object:TYPE <> "field_group" THEN DO:
                IF  h_object:NAME = "bt_zoo_225772" THEN 
                    ASSIGN h_zoo_cenar = h_object.
            ...
            IF  VALID-HANDLE(h_zoo_cenar) THEN 
                LEAVE.
        END.
    
        ASSIGN h_zoo_cenar:SENSITIVE = NO.
    END.
END.
RETURN "OK".
