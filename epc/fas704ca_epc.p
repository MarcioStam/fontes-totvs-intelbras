...
DEF VAR h_fas704ca_epc_1 AS WIDGET-HANDLE NO-UNDO.
DEF VAR h_object         AS WIDGET-HANDLE NO-UNDO.
DEF VAR v_cod_cta_dest   AS CHAR          NO-UNDO.

{esinc\es0000.i}

DEF NEW GLOBAL SHARED VAR h_ind_dwb_fas704ca    AS WIDGET-HANDLE  NO-UNDO.
DEF NEW GLOBAL SHARED VAR h_cenar_fas704ca      AS WIDGET-HANDLE  NO-UNDO.
DEF NEW GLOBAL SHARED VAR h_desc_cenar_fas704ca AS WIDGET-HANDLE  NO-UNDO.
DEF NEW GLOBAL SHARED VAR h_zoo_cenar_fas704ca  AS WIDGET-HANDLE  NO-UNDO.
...
IF  p_ind_event = "INITIALIZE" THEN DO:
    RUN epc/fas704ca_epc_01.p PERSISTENT SET h_fas704ca_epc_1(INPUT p_ind_event,
                                                              INPUT p_ind_object,
                                                              INPUT p_wgh_object,
                                                              INPUT p_wgh_frame,
                                                              INPUT p_cod_table,
    ...
    DO WHILE VALID-HANDLE(h_object):
        IF  h_object:TYPE <> "field_group" THEN DO:
            IF  h_object:NAME = "ind_dwb_set_type" THEN 
                ASSIGN h_ind_dwb_fas704ca = h_object.
        ...
        IF  VALID-HANDLE(h_ind_dwb_fas704ca) THEN 
            LEAVE.
    END.

    IF  VALID-HANDLE(h_ind_dwb_fas704ca) THEN
        ON "VALUE-CHANGED" OF h_ind_dwb_fas704ca PERSISTENT RUN pi_trata_cenario IN h_fas704ca_epc_1 .
         ...
         DO WHILE VALID-HANDLE(h_object):
            IF h_object:TYPE <> "field_group" THEN DO:
                 IF  h_object:NAME = "cod_cenar_ctbl" THEN 
                     ASSIGN h_cenar_fas704ca = h_object.
             ...
             IF  VALID-HANDLE(h_cenar_fas704ca) THEN 
                 LEAVE.
         END.
         ASSIGN h_cenar_fas704ca:SCREEN-VALUE = "".
         ASSIGN h_cenar_fas704ca:SENSITIVE = NO.
    
         /* Localizar a descri‡Æo do cen rio */
         ASSIGN h_object               = p_wgh_frame:FIRST-CHILD
                h_object               = h_object:FIRST-CHILD 
                h_desc_cenar_fas704ca  = ?.
         
         DO WHILE VALID-HANDLE(h_object):
            IF h_object:TYPE <> "field_group" THEN DO:
                 IF  h_object:NAME = "des_cenar_ctbl" THEN 
                     ASSIGN h_desc_cenar_fas704ca = h_object.
             ...
             IF  VALID-HANDLE(h_desc_cenar_fas704ca) THEN 
                 LEAVE.
         END.
         ASSIGN h_desc_cenar_fas704ca:SCREEN-VALUE = "".
    
         /* Localizar o zoom do cen rio */
         ASSIGN h_object              = p_wgh_frame:FIRST-CHILD
                h_object              = h_object:FIRST-CHILD 
                h_zoo_cenar_fas704ca  = ?.

         DO WHILE VALID-HANDLE(h_object):
            IF h_object:TYPE <> "field_group" THEN DO:
                 IF  h_object:NAME = "bt_zoo_54620" THEN 
                     ASSIGN h_zoo_cenar_fas704ca = h_object.
             ...
             IF  VALID-HANDLE(h_zoo_cenar_fas704ca) THEN 
                 LEAVE.
         END.
         ASSIGN h_zoo_cenar_fas704ca:SENSITIVE = NO.
    END.
