...
def query qr_sea_grp_clien 
    for emscad.ccusto, ccusto_unid_negoc, unid_negoc, emscad.empresa
    scrolling. 
...
def browse br_sea_grp_clien query qr_sea_grp_clien display  
    ...
    width-chars 03.5 
        column-label "Emp" 
    unid_negoc.cdn_unid_negoc format '999'
    width-chars 03 
        column-label "UN" 
...
def var rs_sea_grp_clien 
    ...
    radio-buttons "Por Empresa", "Por Empresa","Por CCusto", "Por CCusto","Por Descri‡Æo", "Por Descri‡Æo" 
    bgcolor 15  
    no-undo. 
...
def frame f_sea_01_grp_clien 
    ...
    with 1 down side-labels no-validate keep-tab-order three-d 
         ...
         title "Pesquisa CCusto - ES0512KB-2 - 1.00.00.001". 
    ...
    case input frame f_sea_01_grp_clien rs_sea_grp_clien: 
        when "Por Empresa" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    ...
                    where emscad.ccusto.cod_empresa >= v_cod_initial
                      and emscad.ccusto.cod_empresa <= v_cod_final,
                    each ccusto_unid_negoc no-lock of emscad.ccusto,
                    first unid_negoc of ccusto_unid_negoc no-lock,
                    first emscad.empresa of emscad.ccusto no-lock. 
            end. 
        when "Por CCusto" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    each emscad.ccusto no-lock 
                    where emscad.ccusto.cod_ccusto >= v_cod_initial
                      and emscad.ccusto.cod_ccusto <= v_cod_final,
                    each ccusto_unid_negoc no-lock of emscad.ccusto,
                    first unid_negoc of ccusto_unid_negoc no-lock,
                    first emscad.empresa of emscad.ccusto
                    by emscad.ccusto.cod_ccusto by unid_negoc.cdn_unid_negoc. 
            end. 
        when "Por Descri‡Æo" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    each emscad.ccusto no-lock 
                    where emscad.ccusto.des_tit_ctbl >= v_cod_initial
