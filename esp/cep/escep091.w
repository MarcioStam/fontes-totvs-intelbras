...
*******************************************************************************/
{include/i-prgvrs.i ESCEP091 2.00.00.003}
/*------------------------------------------------------------------------
    File        : ESCEP091.W
    Purpose     : Importaá∆o Dados para APS
    Syntax      : <none>
    Description : <none>
...
&GLOBAL-DEFINE Program        ESCEP091
&GLOBAL-DEFINE Version        2.00.00.003
&GLOBAL-DEFINE VersionLayout  002
...
&GLOBAL-DEFINE page4Fields    cInputFile v-alerta
...
DEFINE VARIABLE v-alerta AS LOGICAL INITIAL no 
     LABEL "Mostra Log Aviso" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME fPage4
     btInputFile AT ROW 9.42 COL 43.14 HELP
          "Escolha do nome do arquivo"
     cInputFile AT ROW 9.5 COL 3.14 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     v-alerta AT ROW 11.25 COL 2 WIDGET-ID 2
     text-entrada AT ROW 8.42 COL 3.14 NO-LABEL
     RECT-12 AT ROW 8.71 COL 2
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
    ...
    DO ON ERROR UNDO, RETURN ERROR
       ON STOP  UNDO, RETURN ERROR:
        ...
        ASSIGN tt-param.arq-csv = INPUT FRAME fPage4 cInputFile
               tt-param.log-alerta = INPUT FRAME fPage4 v-alerta.

        {report/imexb.i}

        IF SESSION:SET-WAIT-STATE("GENERAL":U) THEN.

        {report/imrun.i esp/cep/escep091rp.p}
