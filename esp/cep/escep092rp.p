...
DEFINE TEMP-TABLE tt-item-tot NO-UNDO
    FIELD nr-ord-prod AS CHARACTER
...
IF NOT tt-param.log-gera-txt THEN
    RUN esapi\esapi036a.p.
        ...
        FOR EACH movto-estoq NO-LOCK
           WHERE movto-estoq.nr-ord-prod = ord-prod.nr-ord-prod
             ...
             AND (movto-estoq.esp-docto  = 28  /*REQ*/
              OR  movto-estoq.esp-docto  = 31) /*RRQ*/ :

            RUN pi-inicializar IN h-acomp (INPUT "Integrando item " + movto-estoq.it-codigo).

            CREATE tt-item-tot.
            ASSIGN tt-item-tot.nr-ord-prod = STRING(ord-prod.nr-ord-prod)
                   tt-item-tot.cod-estabel = INT(ord-prod.cod-estabel)
                   tt-item-tot.it-codigo   = movto-estoq.it-codigo.

            IF movto-estoq.esp-docto = 28 THEN
                ASSIGN tt-item-tot.qtd-saida = tt-item-tot.qtd-saida + movto-estoq.quantidade.
            ELSE
                ASSIGN tt-item-tot.qtd-entrada = tt-item-tot.qtd-entrada + movto-estoq.quantidade.

        END.
    END.

    FOR EACH tt-item-tot.
        IF (tt-item-tot.qtd-saida - tt-item-tot.qtd-entrada) > 0 THEN DO:
            IF tt-param.log-gera-txt THEN DO:
                ASSIGN tt-item-tot.quantidade = tt-item-tot.qtd-saida - tt-item-tot.qtd-entrada.
                EXPORT DELIMITER ";" 
                    tt-item-tot.nr-ord-prod
                    tt-item-tot.it-codigo
                    tt-item-tot.quantidade
                    tt-item-tot.cod-estabel SKIP.
            END.
            ELSE DO:
                RUN esapi\esapi036.p (INPUT tt-item-tot.nr-ord-prod,
                                      INPUT tt-item-tot.it-codigo,
                                      INPUT (tt-item-tot.qtd-saida - tt-item-tot.qtd-entrada),
                                      INPUT tt-item-tot.cod-estabel,
                                      OUTPUT c-result).
                IF SUBSTRING(c-result,1,2) = '20' THEN 
                    PUT 'Item ' tt-item-tot.it-codigo 'integrado ao APS com Sucesso.' SKIP(1).
                ELSE
