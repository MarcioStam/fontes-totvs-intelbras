...
*******************************************************************************/
{include/i-prgvrs.i escep047 1.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        escep047
...
DEFINE VARIABLE EDITOR-1 AS CHARACTER 
     VIEW-AS EDITOR
     SIZE 78 BY 5.75 NO-UNDO.

DEFINE VARIABLE cEndField AS CHARACTER FORMAT "X(16)":U INITIAL "ZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE cIniField AS CHARACTER FORMAT "X(16)":U 
     LABEL "Itens" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88
     FONT 1 NO-UNDO.
...
DEFINE FRAME fPage2
     EDITOR-1 AT ROW 1.83 COL 3.86 NO-LABEL WIDGET-ID 2 NO-TAB-STOP 
     cIniField AT ROW 8.5 COL 20.29
     cEndField AT ROW 8.5 COL 51.43 NO-LABEL
     IMAGE-1 AT ROW 8.5 COL 39.29
     IMAGE-2 AT ROW 8.5 COL 48.43
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
/* SETTINGS FOR FILL-IN cIniField IN FRAME fPage2
   ALIGN-L                                                              */
ASSIGN 
       EDITOR-1:READ-ONLY IN FRAME fPage2        = TRUE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    assign EDITOR-1:screen-value in frame fPage2 = 
        'Este programa ir† atualizar os saldos de itens dispon°veis para venda no site da Ikeda.' + chr(10) + chr(10) +
        'Para agendamento peri¢digo e manutená∆o dos agendamentos, utilize o Programa de Agenda Autom†tica do EMS (CD8600). ' +
        'A execuá∆o online s¢ poder† ser feita se houver acesso ao WebService da Ikeda a partir da sess∆o corrente.' + chr(10) + chr(10) +
        'A sa°da do relat¢rio Ç feita com o envio de e-mail para o endereáo configurado nos parÉmetros do B2C.'.
END PROCEDURE.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.it-codigo-ini = input frame fPage2 cIniField
           tt-param.it-codigo-end = input frame fPage2 cEndField.
    ...
    {report/rprun.i esp/cep/escep047rp.p}
