...
*******************************************************************************/
{include/i-prgvrs.i ESCCP015A 1.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           ESCCP015A
...
DEFINE BUTTON bt-pedido 
     ...
     LABEL "Pedido" 
     SIZE 4 BY .88 TOOLTIP "Localizar pedido conforme nœmero informado".
...
DEFINE FRAME fPage1
     cb-frete AT ROW 1.17 COL 16 COLON-ALIGNED HELP
          "Informe se o Frete ý Pago ou a Pagar" WIDGET-ID 10
...
DO:
    ...
    ELSE DO:
        ...
        FIND FIRST ordem-compra 
             WHERE ordem-compra.num-pedido = int(ttpedido-compr.num-pedido:SCREEN-VALUE in frame fPage0)  
        NO-LOCK NO-ERROR.
    
        IF AVAIL ordem-compra THEN
           ASSIGN fi-mo-codigo:SCREEN-VALUE in frame fPage1 = STRING(ordem-compra.mo-codigo).
   ...
   IF INPUT FRAME fpage0 ttpedido-compr.num-pedido <> "" THEN DO:
      ...
      IF  c-tipo-evento = "AlterarPedido" THEN DO:
            ...
            IF  NOT AVAIL pedido-compr THEN DO:
                ...
                RUN utp/ut-msgs.p (INPUT "show",
                                   INPUT 17006,
                                   INPUT "Pedido de compras nÊo encontrado com o nßmero informado.").

                RETURN NO-APPLY.
            END.
            ELSE DO:

                IF pedido-compr.situacao  = 3 THEN DO: /* eliminado */
      ...
      IF  c-tipo-evento = "GerarPedido" THEN DO:
          ...
          IF  v-dat-entrega = ? OR 
              ...
              RUN utp\ut-msgs.p (INPUT "show",
                                 INPUT 17006,
                                 INPUT "Data de entrega inv˜lida.").
          ...
          IF RETURN-VALUE = "OK":U THEN DO:
              ...
              THEN DO:
                  ...
                  THEN
                      RUN utp/ut-msgs.p (INPUT "show",
                                         INPUT 15825,
                                         INPUT "Execu»’o Finalizada.~~Foi gerado o pedido " + STRING(ttpedido-compr.num-pedido)).
                  ...
                  THEN
                      RUN utp/ut-msgs.p (INPUT "show",
                                         INPUT 15825,
                                         INPUT "Execu»’o Finalizada.~~Foi gerado o pedido e processo de importa»’o " + STRING(ttpedido-compr.num-pedido)).
                  ...
                  THEN
                      RUN utp/ut-msgs.p (INPUT "show",
                                         INPUT 15825,
                                         INPUT "Execu»’o Finalizada.~~Foi gerado o pedido, processo de importa»’o e embarque " + STRING(ttpedido-compr.num-pedido)).
              ...
              ELSE DO:
                  RUN utp/ut-msgs.p (INPUT "show",
                                     INPUT 17006,
                                     INPUT "Execu»’o Interrompida.~~Os erros ocorridos impediram a gera»’o do pedido.").
                  ASSIGN s-l-ok = no.
              END.
          end.    
    
      END. 
      ELSE DO: /* alterarpedido */ 
          ...
          if RETURN-VALUE = "OK":U THEN DO:
              ...
              RUN utp/ut-msgs.p (INPUT "show",
                                 INPUT 15825,
                                 INPUT "Execu»’o Finalizada.~~Ordens de compra criadas no pedido " + STRING(ttpedido-compr.num-pedido)).
              APPLY "CLOSE":U TO THIS-PROCEDURE.
          END.
      END.
   END.
...
ON F5 OF ttpedido-compr.cod-cond-pag IN FRAME fPage1 /* Condi»’o Pagto */
DO:
    /*--- Zoom Smart Object ---*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttpedido-compr.cod-cond-pag wMaintenanceNoNavigation
ON LEAVE OF ttpedido-compr.cod-cond-pag IN FRAME fPage1 /* Condi»’o Pagto */
DO:
    if  input frame fPage1 ttpedido-compr.cod-cond-pag = 0 then do:
        assign c-cond-pagto:screen-value in frame fPage1 = "Condi»’o Pagto Espec­fica".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttpedido-compr.cod-cond-pag wMaintenanceNoNavigation
ON MOUSE-SELECT-DBLCLICK OF ttpedido-compr.cod-cond-pag IN FRAME fPage1 /* Condi»’o Pagto */
DO:
  apply "f5":U to self.
END.
...
DO: 
    ...
    if  i-cod-cond-pag > 0 then
        ...
        else assign c-cond-pagto:screen-value in frame fPage1 = "Condi»’o Pagto Espec­fica".
    end.    
    ...
    /*
    ** Habilita c«digo do itiner˜rio quando parümetros estiver para informa Data do Embarque
    */
...
ON F5 OF v-cod-itiner-pad IN FRAME fPage1 /* Itinerÿrio */
DO:
    {method/zoomfields.i 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v-cod-itiner-pad wMaintenanceNoNavigation
ON MOUSE-SELECT-DBLCLICK OF v-cod-itiner-pad IN FRAME fPage1 /* Itinerÿrio */
DO:
    apply "f5":U to self in frame fpage1.  
END.
...
------------------------------------------------------------------------------*/
    ...
    IF  AVAIL param-global AND
        ...
        AVAIL pedido-compr
    THEN DO:
        IF  param-global.modulo-07 AND      /* M½dulo de importa»’o estÿ implantado */
...
DEF var i-num-ordem as int  no-undo.
    ...
    run pi-inicializar in h-acomp (input "Criando Ordem Compra").
    ...
    DO TRANS ON ERROR UNDO, LEAVE:
        ...
        for each  ttcomponente
            ...
            assign tt-ordem-compra.ind-tipo-movto = 1
                   ...
                   tt-ordem-compra.mo-codigo      = int(fi-mo-codigo:screen-value in frame fpage1) 
            ...
            if not avail item-uni-estab or item-uni-estab.cod-unid-negoc = '' then do:
                ...
                RUN _insertError IN {&hDBOTable} (INPUT 17006,
                                                  INPUT "EMS":U,
                                                  INPUT "ERROR":U,
                                                  INPUT 'Unidade de Neg½cio N’o informada').    
            end.       
            ...
            ASSIGN ttcotacao-item.aliquota-icm = 0
                   ...
                   ttcotacao-item.mo-codigo    = int(fi-mo-codigo:screen-value in frame fpage1) 
        ...
        IF  c-tipo-evento = "GerarPedido" THEN DO:
            ...
            IF  tg-proc-imp THEN DO:
                ...
                IF  VALID-HANDLE(h-acomp)
                THEN
                    RUN pi-acompanhar in h-acomp (input "Gerando processo de importa»’o").
...
------------------------------------------------------------------------------*/
    ...
    if  ttpedido-compr.cod-cond-pag = 0 then do:
        ...
        run emptyRowErrors in {&hDBOTable}.
        run validaCondicaoEspecifica in {&hDBOTable} (input  no, /* parümetro usado na WEB */
...
/*:T------------------------------------------------------------------------------
  ...
  Parameters:  
  Notes:       Este mýtodo somente ý executado quando a vari˜vel pcAction 
