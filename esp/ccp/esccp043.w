...
*******************************************************************************/
{include/i-prgvrs.i esccp043 1.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          esccp043
...
&GLOBAL-DEFINE hDBOParent       HDBOttitem
...
&GLOBAL-DEFINE page0Fields      tt-item.desc-item tt-item.it-codigo
...
DEFINE VARIABLE c-desc-projeto LIKE int-projeto.desc-projeto NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-int-projeto-item.cod-projeto fnDescricao(tt-int-projeto-item.cod-projeto) @ c-desc-projeto   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brSon1   
&Scoped-define SELF-NAME brSon1
&Scoped-define QUERY-STRING-brSon1 FOR EACH  tt-int-projeto-item                             WHERE tt-int-projeto-item.it-codigo = tt-item.it-codigo NO-LOCK
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY {&SELF-NAME} FOR EACH  tt-int-projeto-item                             WHERE tt-int-projeto-item.it-codigo = tt-item.it-codigo NO-LOCK.
...
FUNCTION fnDescricao RETURNS CHARACTER
  ( i-projeto AS CHARACTER )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _FREEFORM
  QUERY brSon1 NO-LOCK DISPLAY
      tt-int-projeto-item.cod-projeto
      fnDescricao(tt-int-projeto-item.cod-projeto) @ c-desc-projeto COLUMN-LABEL "Descri‡Æo" FORMAT "x(40)":U WIDTH 42
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 10.13
...
DEFINE FRAME fPage0
     ...
     btHelp AT ROW 1.13 COL 86.86 HELP
          "Ajuda"
     tt-item.it-codigo AT ROW 2.83 COL 23.57 COLON-ALIGNED WIDGET-ID 8
          VIEW-AS FILL-IN 
          SIZE 18 BY .88
     tt-item.desc-item AT ROW 3.83 COL 23.57 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 62 BY .88
...
OPEN QUERY {&SELF-NAME} FOR EACH  tt-int-projeto-item
                            WHERE tt-int-projeto-item.it-codigo = tt-item.it-codigo NO-LOCK.
     _END_FREEFORM
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE c-it-codigo LIKE {&ttParent}.it-codigo  VIEW-AS FILL-IN SIZE 18 BY .88 NO-UNDO.
    
    DEFINE FRAME fGoToRecord
        c-it-codigo    AT ROW 1.21 COL 19.72 COLON-ALIGNED
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH  int-projeto-item NO-LOCK
        WHERE int-projeto-item.it-codigo = INPUT FRAME fPage0 tt-item.it-codigo:
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST int-projeto NO-LOCK
        WHERE  int-projeto.cod-projeto = i-projeto NO-ERROR.
    IF  AVAIL int-projeto
    THEN RETURN int-projeto.desc-projeto.
    ELSE RETURN "".
