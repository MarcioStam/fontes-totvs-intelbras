...
*******************************************************************************/
{include/i-prgvrs.i esccp042 1.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          esccp042
...
&GLOBAL-DEFINE ttParent         tt-int-projeto
&GLOBAL-DEFINE hDBOParent       HDBOttintprojeto
&GLOBAL-DEFINE DBOParentTable   int-projeto
&GLOBAL-DEFINE DBOParentDestroy NO

&GLOBAL-DEFINE ttSon1           tt-int-projeto-item
&GLOBAL-DEFINE hDBOSon1         HDBOtt-int-projeto-item
&GLOBAL-DEFINE DBOSon1Table     int-projeto-item
&GLOBAL-DEFINE DBOSon1Destroy   NO

&GLOBAL-DEFINE page0Fields      tt-int-projeto.cod-projeto tt-int-projeto.desc-projeto
...
DEFINE VARIABLE p-cod-projeto-ini LIKE int-projeto-item.cod-projeto NO-UNDO.
DEFINE VARIABLE p-cod-projeto-fim LIKE int-projeto-item.cod-projeto NO-UNDO.
DEFINE VARIABLE p-it-codigo-ini   LIKE int-projeto-item.it-codigo   NO-UNDO.
DEFINE VARIABLE p-it-codigo-fim   LIKE int-projeto-item.it-codigo   NO-UNDO.
...
&Scoped-define INTERNAL-TABLES tt-int-projeto-item

/* Definitions for BROWSE brSon1                                        */
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-int-projeto-item.it-codigo fnDescricao(tt-int-projeto-item.it-codigo) @ c-desc-item   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brSon1   
&Scoped-define SELF-NAME brSon1
&Scoped-define QUERY-STRING-brSon1 FOR EACH  tt-int-projeto-item                             WHERE tt-int-projeto-item.cod-projeto = tt-int-projeto.cod-projeto NO-LOCK
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY {&SELF-NAME} FOR EACH  tt-int-projeto-item                             WHERE tt-int-projeto-item.cod-projeto = tt-int-projeto.cod-projeto NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brSon1 tt-int-projeto-item
&Scoped-define FIRST-TABLE-IN-QUERY-brSon1 tt-int-projeto-item
...
tt-int-projeto.desc-projeto 
&Scoped-define DISPLAYED-TABLES tt-int-projeto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnDescricao wMasterDetail 
FUNCTION fnDescricao RETURNS CHARACTER
  ( c-it-codigo AS CHARACTER )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _FREEFORM
  QUERY brSon1 NO-LOCK DISPLAY
      tt-int-projeto-item.it-codigo FORMAT "x(16)":U
      fnDescricao(tt-int-projeto-item.it-codigo) @ c-desc-item COLUMN-LABEL "Descri‡Æo" FORMAT "x(40)":U WIDTH 42
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage0
     ...
     btHelp AT ROW 1.13 COL 86.86 HELP
          "Ajuda"
     tt-int-projeto.cod-projeto AT ROW 2.83 COL 29 COLON-ALIGNED WIDGET-ID 8
          VIEW-AS FILL-IN 
          SIZE 12 BY .88
     tt-int-projeto.desc-projeto AT ROW 3.83 COL 29 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 42 BY .88
...
DEFINE FRAME fPage1
     ...
     btDeleteSon1 AT ROW 10.33 COL 14
     btFaixa AT ROW 10.33 COL 26 WIDGET-ID 2
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 5.21
         SIZE 84.43 BY 10.63
         FONT 1 WIDGET-ID 100.
...
/* SETTINGS FOR FILL-IN tt-int-projeto.cod-projeto IN FRAME fPage0
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN tt-int-projeto.desc-projeto IN FRAME fPage0
   NO-ENABLE                                                            */
...
/* Query rebuild information for BROWSE brSon1
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH  tt-int-projeto-item
                            WHERE tt-int-projeto-item.cod-projeto = tt-int-projeto.cod-projeto NO-LOCK.
     _END_FREEFORM
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _Where[1]         = "Temp-Tables.tt-int-projeto-item.cod-projeto = Temp-Tables.tt-int-projeto.cod-projeto"
     _Query            is OPENED
*/  /* BROWSE brSon1 */
&ANALYZE-RESUME
...
DO:
    ...
    ASSIGN INPUT FRAME fPage0 tt-int-projeto.cod-projeto.

    RUN esp/ccp/esccp042a.w (INPUT "Create":U,
                             INPUT tt-int-projeto.cod-projeto,
...
DO:
    ...
    IF  RETURN-VALUE = "YES" THEN DO:
        ...
        if  brSon1:num-selected-rows > 0 then do on error undo, return no-apply:
            get current brSon1.

            FIND FIRST int-projeto-item EXCLUSIVE-LOCK
                WHERE  ROWID(int-projeto-item) = tt-int-projeto-item.r-rowid NO-ERROR.
            IF  AVAIL int-projeto-item THEN DELETE int-projeto-item.

            delete tt-int-projeto-item.
            if  brSon1:delete-current-row() in frame fPage1 then.
        end.
    END.
...
DO:
    EMPTY TEMP-TABLE tt-int-projeto-item NO-ERROR.

    ASSIGN p-cod-projeto-ini = tt-int-projeto.cod-projeto:SCREEN-VALUE IN FRAME fPage0
           p-cod-projeto-fim = tt-int-projeto.cod-projeto:SCREEN-VALUE IN FRAME fPage0.

    run esp/ccp/esccp042b.w (input-output p-cod-projeto-ini,
                             input-output p-cod-projeto-fim,
                             input-output p-it-codigo-ini,
                             input-output p-it-codigo-fim).
    
    run setConstraintFaixaItem IN {&hDBOSon1} (input p-cod-projeto-ini,
                                             input p-cod-projeto-fim,
                                             input p-it-codigo-ini,
                                             input p-it-codigo-fim) no-error.                                                                                               
                                                
    RUN openQueryStatic IN {&hDBOSon1} (INPUT "FaixaItem":U) NO-ERROR.    
    
    RUN getBatchRecords IN {&hDBOSon1} (INPUT ?,
                                      INPUT ?,
                                      INPUT 40,
                                      OUTPUT iqtd,
                                      OUTPUT TABLE tt-int-projeto-item).
...
OR CHOOSE OF MENU-ITEM miSearch IN MENU mbMain DO:
    {method/zoomreposition.i &ProgramZoom="eszoom/z01es702.w"}
END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE i-cod-projeto LIKE {&ttParent}.cod-projeto NO-UNDO.
    
    DEFINE FRAME fGoToRecord
        i-cod-projeto AT ROW 1.21 COL 19.72 COLON-ALIGNED
        btGoToOK          AT ROW 2.63 COL 2.14
        btGoToCancel      AT ROW 2.63 COL 13
        rtGoToButton      AT ROW 2.38 COL 1.25
        SPACE(0.28)
        WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
             THREE-D SCROLLABLE TITLE "V  Para Projeto" FONT 1
             DEFAULT-BUTTON btGoToOK CANCEL-BUTTON btGoToCancel.
    
/*tech1139 - FO 1338.917 - 10/07/2006  */
    RUN utp/ut-trfrrp.p (input Frame fGoToRecord:Handle).
    {utp/ut-liter.i "V _Para_Projeto"}
    ASSIGN FRAME fGoToRecord:TITLE = RETURN-VALUE.
/*tech1139 - FO 1338.917 - 10/07/2006  */

    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToRecord DO:
        ASSIGN i-cod-projeto.
        
        RUN goToKey IN {&hDBOParent} (INPUT i-cod-projeto).
        IF RETURN-VALUE = "NOK":U THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U, INPUT 2, INPUT "Projeto":U).
    ...
    ENABLE i-cod-projeto btGoToOK btGoToCancel 
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOParent}) OR
       ...
       {&hDBOParent}:FILE-NAME <> "esbo/boes702.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes702.p YES}
        {btb/btb008za.i2 esbo/boes702.p '' {&hDBOParent}} 
    END.
    ...
    IF NOT VALID-HANDLE({&hDBOSon1}) OR 
       ...
       {&hDBOSon1}:FILE-NAME <> "esbo/boes703.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes703.p YES}
        {btb/btb008za.i2 esbo/boes703.p '' {&hDBOSon1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {masterdetail/openqueriesson.i &Parent="int-projeto"
                                   &Query="int-projeto"
                                   &PageNumber="1"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-atualizar-browse wMasterDetail 
...
------------------------------------------------------------------------------*/
    ...
    EMPTY TEMP-TABLE tt-int-projeto-item.
    FOR EACH  int-projeto-item NO-LOCK
        WHERE int-projeto-item.cod-projeto = INPUT FRAME fPage0 tt-int-projeto.cod-projeto:
        CREATE tt-int-projeto-item.
        BUFFER-COPY int-projeto-item TO tt-int-projeto-item.
        ASSIGN tt-int-projeto-item.r-rowid = ROWID(int-projeto-item).

        /* Necess rio pois a tabela do browse ‚ a Temp-Table,
           e o rowid da TT ‚ diferente do rowid da tabela */
        IF  r-row-table = ROWID(int-projeto-item) 
        THEN ASSIGN r-row-table = ROWID(tt-int-projeto-item).
    END.
    ...
    IF  CAN-FIND(FIRST tt-int-projeto-item) THEN DO:
        ENABLE btDeleteSon1 btFaixa WITH FRAME fPage1.
    END.
    ELSE DO:
        DISABLE btDeleteSon1 btFaixa WITH FRAME fPage1.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnDescricao wMasterDetail 
FUNCTION fnDescricao RETURNS CHARACTER
  ( c-it-codigo AS CHARACTER ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
