...
*******************************************************************************/
{include/i-prgvrs.i ESCCP054 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCCP054 ESP}
&ENDIF
...
DEFINE VARIABLE tg-csv AS LOGICAL INITIAL yes 
     LABEL "Gera CSV" 
     VIEW-AS TOGGLE-BOX
...
DEFINE VARIABLE fi-data-pedido-fim AS DATE FORMAT "99/99/9999":U INITIAL 12/31/2099 
     VIEW-AS FILL-IN 
     SIZE 11 BY .88 NO-UNDO.

DEFINE VARIABLE fi-data-pedido-ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/001 
     LABEL "Data Pedido" 
     VIEW-AS FILL-IN 
     SIZE 11 BY .88 NO-UNDO.
...
DEFINE VARIABLE fi-num-pedido-fim AS INTEGER FORMAT ">>>>>,>>9":U INITIAL 99999999 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-pg-cla
     rs-classif AT ROW 1.75 COL 5 HELP
...
DEFINE FRAME f-pg-par
     tg-csv AT ROW 1.5 COL 3 WIDGET-ID 52
     bt-dir-2 AT ROW 3.54 COL 68.72 HELP
          "Escolha o diret¢rio" WIDGET-ID 26
     fi-diretorio AT ROW 3.63 COL 4 HELP
          "Diret¢rio em que os arquivos serÆo salvos" NO-LABEL WIDGET-ID 28
     "Diret¢rio" VIEW-AS TEXT
          SIZE 6.86 BY .54 AT ROW 2.75 COL 7.14 WIDGET-ID 50
     RECT-11 AT ROW 3 COL 3 WIDGET-ID 48
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 75 BY 10
...
DEFINE FRAME f-pg-sel
     fi-num-pedido-ini AT ROW 1.5 COL 15.86 COLON-ALIGNED HELP
          "Pedido Origem inicial"
     fi-num-pedido-fim AT ROW 1.5 COL 45 COLON-ALIGNED HELP
          "Pedido Origem final" NO-LABEL
     fi-it-codigo-ini AT ROW 2.5 COL 15.86 COLON-ALIGNED HELP
          "Item inicial" WIDGET-ID 4
     fi-it-codigo-fim AT ROW 2.5 COL 45 COLON-ALIGNED HELP
          "Item final" NO-LABEL WIDGET-ID 2
     fi-cod-emitente-ini AT ROW 3.5 COL 15.86 COLON-ALIGNED HELP
          "Pedido Origem inicial" WIDGET-ID 12
     fi-cod-emitente-fim AT ROW 3.5 COL 45 COLON-ALIGNED HELP
          "Fornecedor final" NO-LABEL WIDGET-ID 10
     fi-data-pedido-ini AT ROW 4.5 COL 15.86 COLON-ALIGNED HELP
          "Data Pedido inicial" WIDGET-ID 20
     fi-data-pedido-fim AT ROW 4.5 COL 45 COLON-ALIGNED HELP
          "Data Pedido final" NO-LABEL WIDGET-ID 18
     IMAGE-1 AT ROW 1.5 COL 35
     IMAGE-2 AT ROW 1.5 COL 44
     IMAGE-3 AT ROW 2.5 COL 35 WIDGET-ID 6
     IMAGE-4 AT ROW 2.5 COL 44 WIDGET-ID 8
     IMAGE-5 AT ROW 3.5 COL 35 WIDGET-ID 14
     IMAGE-6 AT ROW 3.5 COL 44 WIDGET-ID 16
     IMAGE-7 AT ROW 4.5 COL 35 WIDGET-ID 22
     IMAGE-8 AT ROW 4.5 COL 44 WIDGET-ID 24
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio de Troca de Pedidos DE-PARA"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 41.33
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF w-relat /* Relat¢rio de Troca de Pedidos DE-PARA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Relat¢rio de Troca de Pedidos DE-PARA */
DO:
...
DO:
    ...
    then do:
         ...
         disp fi-diretorio with frame f-pg-par.
    end.
END.
...
ON VALUE-CHANGED OF tg-csv IN FRAME f-pg-par /* Gera CSV */
DO:
  assign fi-diretorio:sensitive in frame f-pg-par = self:checked
         bt-dir-2:sensitive     in frame f-pg-par = self:checked.
END.
...
{utp/ut9000.i "ESCCP054" "1.00.00.000"}
...
assign wh-label-dig:screen-value = ""
...
/*     assign fi-data-pedido-ini:screen-value in frame f-pg-sel = string(date(month(today),1,year(today))) */
/*            fi-data-pedido-fim:screen-value in frame f-pg-sel = string(today).                           */
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-num-pedido-ini fi-num-pedido-fim fi-it-codigo-ini fi-it-codigo-fim 
          fi-cod-emitente-ini fi-cod-emitente-fim fi-data-pedido-ini 
          fi-data-pedido-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 IMAGE-7 IMAGE-8 
         fi-num-pedido-ini fi-num-pedido-fim fi-it-codigo-ini fi-it-codigo-fim 
         fi-cod-emitente-ini fi-cod-emitente-fim fi-data-pedido-ini 
         fi-data-pedido-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY tg-csv fi-diretorio 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-11 tg-csv bt-dir-2 fi-diretorio 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    if input frame f-pg-sel fi-num-pedido-ini > input frame f-pg-sel fi-num-pedido-fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Pedido Origem inicial maior que final!").
         apply 'entry' to fi-num-pedido-ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end. 
    ...
    if input frame f-pg-sel fi-cod-emitente-ini > input frame f-pg-sel fi-cod-emitente-fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Fornecedor inicial maior que final!").
         apply 'entry' to fi-cod-emitente-ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.   

    if input frame f-pg-sel fi-data-pedido-ini > input frame f-pg-sel fi-data-pedido-fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Data Pedido inicial maior que final!").
         apply 'entry' to fi-data-pedido-ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.
    ...
    assign tt-param.num-pedido-ini   = input frame f-pg-sel fi-num-pedido-ini  
           tt-param.num-pedido-fim   = input frame f-pg-sel fi-num-pedido-fim  
           ...
           tt-param.data-pedido-ini  = input frame f-pg-sel fi-data-pedido-ini 
           tt-param.data-pedido-fim  = input frame f-pg-sel fi-data-pedido-fim
           tt-param.csv              = tg-csv:checked in frame f-pg-par
    ...
    {include/i-rprun.i esp/ccp/esccp054rp.p}
    
    {include/i-rpexc.i}
    
    SESSION:SET-WAIT-STATE("":U).
    
    if not tg-csv:checked in frame f-pg-par
    then do:
        {include/i-rptrm.i}
    end.
end.
