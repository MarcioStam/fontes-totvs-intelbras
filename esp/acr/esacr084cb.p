/*****************************************************************************
** Programa..............: esacr084cb.p
** Autor.................: Andrey Mauricio de Oliveira
** Criado em.............: 13/12/2021
*****************************************************************************/

DEF INPUT PARAM p_num_mes AS INT  NO-UNDO.
DEF INPUT PARAM p_num_dia AS DATE NO-UNDO.
...
DEFINE FRAME fPage0
    ...
    v_mes             AT ROW 02.00 COL 24.00 LABEL "Màs"
    v_dt_negoc        AT ROW 03.00 COL 15.00 LABEL "Data Negociaá∆o"
...
ON  CHOOSE OF bt_ok IN FRAME fPage0 DO:
    ...
    IF  AVAIL ponto-programa THEN DO:    
        FIND FIRST conteudo-programa EXCLUSIVE-LOCK
            ...
            AND   entry(1,conteudo-programa.conteudo,";") = string(p_num_mes) NO-ERROR.
        
        IF  AVAIL conteudo-programa THEN DO:
            ASSIGN entry(2,conteudo-programa.conteudo,";") = string(INPUT FRAME fPage0 v_dt_negoc,"99/99/9999").
        END.
        ELSE DO:
...
DO  ON ENDKEY UNDO bem_nf_block, LEAVE bem_nf_block:
    ...
    DISABLE v_mes WITH FRAME fPage0.
...
PROCEDURE pi_carrega_dias:
    ...
    IF  AVAIL ponto-programa THEN DO:    
        ...
        IF  AVAIL conteudo-programa THEN DO:
            ASSIGN v_mes:SCREEN-VALUE IN FRAME fPage0 = entry(1,conteudo-programa.conteudo,";")
                   v_dt_negoc:SCREEN-VALUE IN FRAME fPage0 = entry(2,conteudo-programa.conteudo,";").
        END.
        ELSE DO:
            MESSAGE "Conte£do n∆o localizado !" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
            RETURN "NOK".
