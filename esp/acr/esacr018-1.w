...
&Scoped-Define ENABLED-OBJECTS c-dp-ini l_baixa rs-tipo rs-destino ~
c-arquivo bt-arquivo bt-imprime rs-execucao bt-cfimp bt-salva for_ini ~
for_fim dt_ini dt_fim IMAGE-13 IMAGE-2 IMAGE-4 IMAGE-9 RECT-10 RECT-2 ~
RECT-29 RECT-7 RECT-9 IMAGE-14 IMAGE-15 IMAGE-16 IMAGE-17 c-dp-fim ~
c-parc-ini c-parc-fim 
&Scoped-Define DISPLAYED-OBJECTS c-dp-ini l_baixa rs-tipo rs-destino ~
c-arquivo rs-execucao for_ini for_fim dt_ini dt_fim c-dp-fim c-parc-ini ~
c-parc-fim 
...
DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "esacr018.lst" 
     VIEW-AS FILL-IN 
     SIZE 44.29 BY .88 TOOLTIP "Destino"
     BGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE c-dp-fim AS CHARACTER FORMAT "X(10)":U INITIAL "ZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .79 NO-UNDO.

DEFINE VARIABLE c-dp-ini AS CHARACTER FORMAT "X(10)":U 
     LABEL "Duplicata" 
     VIEW-AS FILL-IN 
     SIZE 9.57 BY .79 NO-UNDO.

DEFINE VARIABLE c-parc-fim AS CHARACTER FORMAT "X(2)":U INITIAL "ZZ" 
     VIEW-AS FILL-IN 
     SIZE 3.72 BY .79 NO-UNDO.

DEFINE VARIABLE c-parc-ini AS CHARACTER FORMAT "X(2)":U 
     LABEL "Parcela" 
     VIEW-AS FILL-IN 
     SIZE 3.57 BY .79 NO-UNDO.

DEFINE VARIABLE dt_fim AS DATE FORMAT "99/99/9999":U INITIAL 12/31/06 
     VIEW-AS FILL-IN 
     SIZE 9.57 BY .88 NO-UNDO.

DEFINE VARIABLE dt_ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/05 
     LABEL "Data emiss∆o" 
     VIEW-AS FILL-IN 
     SIZE 9.57 BY .88 NO-UNDO.

DEFINE VARIABLE for_fim AS INTEGER FORMAT ">>>>,>>9":U INITIAL 9999999 
     VIEW-AS FILL-IN 
     SIZE 7.57 BY .88 NO-UNDO.

DEFINE VARIABLE for_ini AS INTEGER FORMAT ">>>>,>>9":U INITIAL 0 
     LABEL "Fornecedor" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE l_baixa AS LOGICAL INITIAL no 
     LABEL "Baixar previs‰es de representantes que bloqueia pagamento?" 
     VIEW-AS TOGGLE-BOX
     SIZE 45.57 BY .83 TOOLTIP "Considera o campo bloqueia pagto do cadastro repres financeiro" NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME f-relat
     c-dp-ini AT ROW 8.17 COL 11.43 COLON-ALIGNED
     l_baixa AT ROW 3.5 COL 10.43
     rs-tipo AT ROW 2.25 COL 10 NO-LABEL
     rs-destino AT ROW 12.75 COL 4.43 HELP
          "Destino da Impress∆o" NO-LABEL
     c-arquivo AT ROW 13.75 COL 3.86 HELP
          "Destino" NO-LABEL
     bt-arquivo AT ROW 13.54 COL 49 HELP
          "Localiza Arquivo"
     bt-imprime AT ROW 15.5 COL 2.29
     rs-execucao AT ROW 13.42 COL 56.14 HELP
          "Execuá∆o On Line ou Batch" NO-LABEL
     bt-cfimp AT ROW 15.5 COL 49 HELP
          "Layout Impress∆o"
     bt-salva AT ROW 15.5 COL 14
     for_ini AT ROW 6.13 COL 11.43 COLON-ALIGNED
     for_fim AT ROW 6.13 COL 29.86 COLON-ALIGNED NO-LABEL
     dt_ini AT ROW 7.13 COL 11.43 COLON-ALIGNED
     dt_fim AT ROW 7.13 COL 29.86 COLON-ALIGNED NO-LABEL
     c-dp-fim AT ROW 8.17 COL 30.29 COLON-ALIGNED NO-LABEL
     c-parc-ini AT ROW 9.13 COL 11.43 COLON-ALIGNED
     c-parc-fim AT ROW 9.13 COL 30.29 COLON-ALIGNED NO-LABEL
     "  Impress∆o" VIEW-AS TEXT
          SIZE 10.57 BY .54 AT ROW 11.92 COL 2
          FONT 6
     " Execuá∆o" VIEW-AS TEXT
          SIZE 9.43 BY .54 AT ROW 11.92 COL 56.14
          FONT 6
     " ParÉmetros" VIEW-AS TEXT
          SIZE 10 BY .54 AT ROW 1.08 COL 3
          FONT 6
     "Seleá∆o" VIEW-AS TEXT
          SIZE 7 BY .54 AT ROW 5 COL 3
          FONT 6
     IMAGE-13 AT ROW 7.13 COL 23.86
     IMAGE-2 AT ROW 6.13 COL 28.43
     IMAGE-4 AT ROW 7.13 COL 28.43
     IMAGE-9 AT ROW 6.13 COL 23.86
     RECT-10 AT ROW 5.25 COL 1
     RECT-2 AT ROW 15.17 COL 1
     RECT-29 AT ROW 12.08 COL 55
     RECT-7 AT ROW 12.08 COL 1
     RECT-9 AT ROW 1.33 COL 1
     IMAGE-14 AT ROW 8.13 COL 23.86
     IMAGE-15 AT ROW 9.08 COL 23.86
     IMAGE-16 AT ROW 8.13 COL 28.43
     IMAGE-17 AT ROW 9.17 COL 28.43
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Elimina previs‰es de comiss‰es(CPE) do APB - ESACR018"
         COLUMN             = 35.72
         ROW                = 9.42
         HEIGHT             = 15.83
         WIDTH              = 73.29
...
ON END-ERROR OF C-Win /* Elimina previs‰es de comiss‰es(CPE) do APB - ESACR018 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Elimina previs‰es de comiss‰es(CPE) do APB - ESACR018 */
DO:
...
DO:
  ...
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "esacr018",
                               INPUT  "1.00.000",
                               INPUT  0,
...
DO:
  ...
  if input frame f-relat rs-destino = 2 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          ...
          assign c-arquivo:screen-value in frame f-relat = "esacr018.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
...
------------------------------------------------------------------------------*/
  DISPLAY c-dp-ini l_baixa rs-tipo rs-destino c-arquivo rs-execucao for_ini 
          for_fim dt_ini dt_fim c-dp-fim c-parc-ini c-parc-fim 
      WITH FRAME f-relat IN WINDOW C-Win.
  ENABLE c-dp-ini l_baixa rs-tipo rs-destino c-arquivo bt-arquivo bt-imprime 
         rs-execucao bt-cfimp bt-salva for_ini for_fim dt_ini dt_fim IMAGE-13 
         IMAGE-2 IMAGE-4 IMAGE-9 RECT-10 RECT-2 RECT-29 RECT-7 RECT-9 IMAGE-14 
         IMAGE-15 IMAGE-16 IMAGE-17 c-dp-fim c-parc-ini c-parc-fim 
      WITH FRAME f-relat IN WINDOW C-Win.
...
/* */
    ...
    IF AVAILABLE dwb_set_list_param THEN DO:
        ...
        ASSIGN rs-destino   = LOOKUP(dwb_set_list_param.Cod_dwb_output, 'Impressora,Arquivo,Terminal')
               ...
               /* rs-tipo      = INTEGER(entry(2,dwb_set_list_param.cod_dwb_parameters,chr(10))) */
               l_baixa      = LOGICAL(entry(3,dwb_set_list_param.cod_dwb_parameters,chr(10)),"no/yes")
               FOR_ini      = INTEGER(entry(4,dwb_set_list_param.cod_dwb_parameters,chr(10))) 
               FOR_fim      = INTEGER(entry(5,dwb_set_list_param.cod_dwb_parameters,chr(10))) 
               ...
               c-dp-ini     = entry(8,dwb_set_list_param.cod_dwb_parameters,chr(10))
               c-dp-fim     = entry(9,dwb_set_list_param.cod_dwb_parameters,chr(10))
               c-parc-ini   = entry(10,dwb_set_list_param.cod_dwb_parameters,chr(10))
               c-parc-fim   = entry(11,dwb_set_list_param.cod_dwb_parameters,chr(10))
            NO-ERROR.
    END.
...
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "esacr018"
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "esacr018" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "esacr018"
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN dwb_set_list_param.Cod_dwb_program          = "esacr018"
    ...
    ASSIGN dwb_set_list_param.Cod_dwb_parameters       = STRING(rs-execucao) + chr(10) + STRING(rs-tipo)     + CHR(10) +
                                                                STRING(l_baixa)     + CHR(10) + STRING(FOR_ini)     + CHR(10) + 
                                                                STRING(FOR_fim)     + CHR(10) + STRING(dt_ini)      + CHR(10) + 
                                                                STRING(dt_fim)      + CHR(10) + c-dp-ini            + CHR(10) +
                                                                c-dp-fim            + CHR(10) + c-parc-ini          + CHR(10) +
                                                                c-parc-fim          + CHR(10).
