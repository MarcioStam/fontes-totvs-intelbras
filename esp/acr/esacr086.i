/************************************************************************************************************
*      Programa .....: esp/acr/esacr086.i                                                                   *
*      Data .........: 26 de Maio de 2023                                                                   *
*      Empresa ......: IDBA                                                                                 *
*      Cliente ......: Intelbras                                                                            *
*      Programador ..: Bruno Joaquim                                                                        *
*      Objetivo .....: Include para defini‡Æo das temp-tables do EMS 5, utilizadas no programa              *
*************************************************************************************************************
*  VERSAO        DATA        RESPONSAVEL       MOTIVO                                                       *
*  1.00.000.00  15/05/2023   IDBA-Bruno        Desenvolvimento                                              *
************************************************************************************************************/
...
def new shared temp-table tt_converter_finalid_econ no-undo
    ...
    field ttv_val_multa                    as decimal format "->>>,>>>,>>9.99" decimals 2
    field ttv_val_desc                     as decimal format "->>>,>>>,>>9.99" decimals 2
    ...
    field ttv_val_cm                       as decimal format "->>>>>,>>>,>>9.99" decimals 4 label "Corre‡Æo Monet ria" column-label "Corre‡Æo Monet ria"
...
FUNCTION fGera_ID_DL RETURNS CHAR (INPUT pAuthorization_number AS CHAR, 
                                   INPUT pAcquirer_nsu         AS CHAR):

    DEFINE VAR aText    AS CHAR.
    DEFINE VAR bText    AS CHAR.
    DEFINE VAR cText    AS CHAR.
    DEFINE VAR cRetorno AS CHAR.
    DEFINE VAR aInt     AS INT .

    ASSIGN aText = pAcquirer_nsu .

    ASSIGN aInt = int(aText) .

    ASSIGN bText = STRING(aInt ,"999999999").

    ASSIGN cText = SUBSTRING(bText,(LENGTH(bText) - 3 ),4) .

    ASSIGN CRetorno = pAuthorization_number + cText .
