...
DEFINE SHARED VARIABLE cCod_empresa    AS CHARACTER     NO-UNDO.
DEFINE SHARED VARIABLE iCdn_cliente    AS INTEGER       NO-UNDO.
DEFINE SHARED VARIABLE dtBaixaini      AS DATE          NO-UNDO FORMAT "99/99/9999" LABEL "Data Baixa"
    VIEW-AS FILL-IN SIZE 10 BY .88.
DEFINE SHARED VARIABLE dtBaixafim      AS DATE          NO-UNDO FORMAT "99/99/9999"
    VIEW-AS FILL-IN SIZE 10 BY .88. 
DEFINE SHARED VARIABLE cEspecieini     AS CHARACTER     NO-UNDO FORMAT "XX"         LABEL "EspÇcie"
    VIEW-AS FILL-IN SIZE 4 BY .88 . 
DEFINE SHARED VARIABLE cEspeciefim     AS CHARACTER     NO-UNDO FORMAT "XX"
    VIEW-AS FILL-IN SIZE 4 BY .88 . 
DEFINE SHARED VARIABLE cPortadorini    AS CHARACTER     NO-UNDO FORMAT "XXX"        LABEL "Portador" 
    VIEW-AS FILL-IN SIZE 6 BY .88 . 
DEFINE SHARED VARIABLE cPortadorfim    AS CHARACTER     NO-UNDO FORMAT "XXX"
    VIEW-AS FILL-IN SIZE 6 BY .88 . 
...
DEFINE BUTTON bt-imprime 
     LABEL "Imprimir" 
     SIZE 10 BY 1 TOOLTIP "Imprimir"
     FONT 1.

DEFINE BUTTON bt-salva 
     LABEL "Fechar" 
     SIZE 10 BY 1 TOOLTIP "Fechar/Salvar"
     FONT 1.

DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "es0796a.lst" 
     VIEW-AS FILL-IN 
...
"Arquivo", 2,
"Terminal", 3
     SIZE 43.43 BY .83 TOOLTIP "Destino da Impress∆o"
     FONT 1 NO-UNDO.
...
DEFINE FRAME f-relat
     rs-destino AT ROW 1.83 COL 2.14 HELP
          "Destino da Impress∆o" NO-LABEL
     rs-execucao AT ROW 1.83 COL 47.86 HELP
          "Execuá∆o On Line ou Batch" NO-LABEL
     c-arquivo AT ROW 2.83 COL 2.14 HELP
          "Destino" NO-LABEL
     bt-salva AT ROW 4.58 COL 12.43
     bt-imprime AT ROW 4.58 COL 2.14
     bt-cfimp AT ROW 2.83 COL 41.57 HELP
          "Layout Impress∆o"
     bt-arquivo AT ROW 2.83 COL 41.57 HELP
          "Localiza Arquivo"
     RECT-2 AT ROW 4.33 COL 1
     RECT-29 AT ROW 1.33 COL 46.72
     RECT-7 AT ROW 1.33 COL 1
     "  Impress∆o" VIEW-AS TEXT
          SIZE 10.57 BY .54 AT ROW 1.17 COL 3.29
          FONT 6
     " Execuá∆o" VIEW-AS TEXT
          SIZE 9.43 BY .54 AT ROW 1.17 COL 47.29
          FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio Pagamentos Efetuados - ES0796A"
         HEIGHT             = 5.04
         WIDTH              = 57.14
         MAX-HEIGHT         = 29.13
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF C-Win /* Relat¢rio Pagamentos Efetuados - ES0796A */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Relat¢rio Pagamentos Efetuados - ES0796A */
DO:
...
DO:
  ...
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "es0796a",
                               INPUT  "1.00.000",
                               INPUT  0,
...
DO:
  ...
  if input frame f-relat rs-destino = 2 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          assign c-arquivo:screen-value in frame f-relat = session:temp-directory + "es0796a.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
      else
          assign c-arquivo:screen-value in frame f-relat = "es0796a.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
...
/* */
  ...
  DO WITH FRAME f-relat:
    ASSIGN rs-execucao = INTEGER(ENTRY(1, dwb_set_list_param.cod_dwb_parameters, CHR(10)))
           rs-destino  = IF dwb_set_list_param.cod_dwb_output = "impressora" THEN 1
                         ELSE IF dwb_set_list_param.cod_dwb_output = "arquivo" THEN 2
                         ELSE 3.
    DISPLAY rs-execucao rs-destino WITH FRAME f-relat.
    APPLY "value-changed" TO rs-destino      IN FRAME f-relat.  
  END.   
END PROCEDURE.
...
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "es0796a"
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "es0796a" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "es0796a"
...
/* */
  ...
  ASSIGN dwb_set_list_param.Cod_dwb_program          = "es0796a"
         ...
         dwb_set_list_param.Cod_dwb_parameters       = STRING(rs-execucao)   + CHR(10) +
                                                              cCod_empresa          + CHR(10) +
                                                              STRING(iCdn_cliente)  + CHR(10) +
                                                              STRING(dtBaixaini)    + CHR(10) +
                                                              STRING(dtBaixafim)    + CHR(10) +
                                                              cEspecieini           + CHR(10) +
                                                              cEspeciefim           + CHR(10) +
                                                              STRING(cPortadorini)  + CHR(10) +
                                                              STRING(cPortadorfim)
