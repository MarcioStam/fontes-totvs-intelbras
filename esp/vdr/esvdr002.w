...
/*****************************************************************************
**     Programa.........: esp\vdr\esvdr002.w
**     Descricao .......: Emissao de Planilha Vendor - Convers∆o ES0375
**     Versao...........: 1.00.000
...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-7 RECT-8 RECT-9 data-base-ini ~
data-base-fim cod-portador-ini cod-portador-fim cod-emitente-ini ~
cod-emitente-fim nr-planilha-ini nr-planilha-fim rs-selecao rs-destino ~
bt-cfimp bt-arquivo c-arquivo bt-imprime bt-salva rs-execucao 
&Scoped-Define DISPLAYED-OBJECTS data-base-ini data-base-fim ~
cod-portador-ini cod-portador-fim cod-emitente-ini cod-emitente-fim ~
nr-planilha-ini nr-planilha-fim rs-selecao rs-destino c-arquivo rs-execucao 
...
DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "esvdr013.lst" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE data-base-fim AS DATE FORMAT "99/99/9999" INITIAL 05/11/05 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88.

DEFINE VARIABLE data-base-ini AS DATE FORMAT "99/99/9999" INITIAL 05/11/05 
     LABEL "Data Fechamento" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88.

DEFINE VARIABLE nr-planilha-fim AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 999999999 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88.

DEFINE VARIABLE nr-planilha-ini AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 0 
...
DEFINE FRAME f-relat
     data-base-ini AT ROW 1.75 COL 17 COLON-ALIGNED HELP
          "Data de Fechamento da operaá∆o de Vendor"
     data-base-fim AT ROW 1.75 COL 32 COLON-ALIGNED HELP
          "Data de Fechamento da operaá∆o de Vendor" NO-LABEL
     cod-portador-ini AT ROW 2.75 COL 17 COLON-ALIGNED HELP
          "C¢digo Portador"
     cod-portador-fim AT ROW 2.75 COL 32 COLON-ALIGNED HELP
          "C¢digo Portador" NO-LABEL
     cod-emitente-ini AT ROW 3.75 COL 17 COLON-ALIGNED HELP
          "C¢digo Cliente"
     cod-emitente-fim AT ROW 3.75 COL 32 COLON-ALIGNED HELP
          "C¢digo Cliente" NO-LABEL
     nr-planilha-ini AT ROW 4.75 COL 17 COLON-ALIGNED HELP
          "N£mero Planilha Vendor"
     nr-planilha-fim AT ROW 4.75 COL 32 COLON-ALIGNED HELP
          "N£mero Planilha Vendor" NO-LABEL
     rs-selecao AT ROW 6.71 COL 13.86 NO-LABEL
     ...
     " ParÉmetro" VIEW-AS TEXT
          SIZE 9.14 BY .54 AT ROW 6.08 COL 2.86
          FONT 6
     ...
     RECT-8 AT ROW 1.17 COL 1
     RECT-9 AT ROW 6.21 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Planilha de Vendor - ESVDR002"
         COLUMN             = 37.14
         ROW                = 15.42
         HEIGHT             = 12.04
         WIDTH              = 57.43
...
ON END-ERROR OF C-Win /* Planilha de Vendor - ESVDR002 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Planilha de Vendor - ESVDR002 */
DO:
...
DO:
  ...
  ASSIGN cod-emitente-fim 
         ...
         rs-selecao.
  /*
  RUN piValidaImpressao(OUTPUT vLogErro).
  IF vLogErro THEN RETURN NO-APPLY.
  */
  
  RUN pi_salva_param.

  RUN esp/vdr/esvdr002a.w (INPUT cod-emitente-ini,
                           INPUT cod-emitente-fim,
                           INPUT cod-portador-ini,
                           INPUT cod-portador-fim,
                           INPUT data-base-ini,
                           INPUT data-base-fim,
                           INPUT nr-planilha-ini,
                           INPUT nr-planilha-fim,
                           INPUT rs-selecao,
                           OUTPUT TABLE tt-plan).

  IF rs-execucao:SCREEN-VALUE IN FRAME f-relat = "2" THEN
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "esvdr002",
                               INPUT  "1.00.000",
                               INPUT  0,
  ...
  DO.
    ...
    RUN esp/vdr/esvdr002rp.p (INPUT TABLE tt-plan).
...
DO:
  ...
  if input frame f-relat rs-destino = 2 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          assign c-arquivo:screen-value in frame f-relat = session:temp-directory + "esvdr002.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
      else
          assign c-arquivo:screen-value in frame f-relat = "esvdr002.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
...
------------------------------------------------------------------------------*/
  DISPLAY data-base-ini data-base-fim cod-portador-ini cod-portador-fim 
          cod-emitente-ini cod-emitente-fim nr-planilha-ini nr-planilha-fim 
          rs-selecao rs-destino c-arquivo rs-execucao 
      WITH FRAME f-relat IN WINDOW C-Win.
  ENABLE RECT-2 RECT-7 RECT-8 RECT-9 data-base-ini data-base-fim 
         cod-portador-ini cod-portador-fim cod-emitente-ini cod-emitente-fim 
         nr-planilha-ini nr-planilha-fim rs-selecao rs-destino bt-cfimp 
         bt-arquivo c-arquivo bt-imprime bt-salva rs-execucao 
      WITH FRAME f-relat IN WINDOW C-Win.
...
/* */
  ...
  DO WITH FRAME f-relat:
    ...
    ASSIGN rs-execucao:SCREEN-VALUE       IN FRAME f-relat = entry(1,dwb_set_list_param.cod_dwb_parameters,chr(10))
           data-base-ini:screen-value     in frame f-relat = entry(2,dwb_set_list_param.cod_dwb_parameters,chr(10))
           data-base-fim:screen-value     in frame f-relat = entry(3,dwb_set_list_param.cod_dwb_parameters,chr(10))
           cod-portador-ini:screen-value  in frame f-relat = entry(4,dwb_set_list_param.cod_dwb_parameters,chr(10))
           cod-portador-fim:screen-value  in frame f-relat = entry(5,dwb_set_list_param.cod_dwb_parameters,chr(10))
           cod-emitente-ini:screen-value  in frame f-relat = entry(6,dwb_set_list_param.cod_dwb_parameters,chr(10))
           cod-emitente-fim:screen-value  in frame f-relat = entry(7,dwb_set_list_param.cod_dwb_parameters,chr(10))
           nr-planilha-ini:screen-value   in frame f-relat = entry(8,dwb_set_list_param.cod_dwb_parameters,chr(10))
           nr-planilha-fim:screen-value   in frame f-relat = entry(9,dwb_set_list_param.cod_dwb_parameters,chr(10))
...
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "esvdr002"
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "esvdr002" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "esvdr002"
...
/* */
  ...
  ASSIGN dwb_set_list_param.Cod_dwb_program          = "esvdr002"
         ...
         dwb_set_list_param.Cod_dwb_parameters       = STRING(rs-execucao) + chr(10) + 
                                                              ...
                                                              string(nr-planilha-ini) + chr(10) +
                                                              string(nr-planilha-fim) + chr(10) 
