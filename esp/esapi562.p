    ...
    FOR FIRST embarque-imp NO-LOCK
       ...
       FOR EACH prazo-compra NO-LOCK
          ...
          ASSIGN
             cJson = cJson
                   + '~{'
                   + '"numeroEmbarquePedido":"'
                      + STRING(embarque-imp.embarque    ) 
                      + "-"
                      + STRING(ordem-compra.num-pedido  )  + '",'
                   + '"id":"'                 + cIDItem + '",'
...
/*                 + '"valorUnitario":'       + REPLACE(STRING(ordem-compra.pre-unit-for,">>>>>>>>>9.99999"),",",".") + ',' */
                   ...
                   + '"modalidadeCambio": "Financiamento do Fornecedor (Supplier~'s Credit) Î Outros"'
                   + '}'
                   .
    ...
    THEN RUN pi-acompanhar IN h-acomp ("Cadastro de Item do Pedido").

    IF l-proc = NO
    THEN DO:
       IF es-api-log.aux > ""
       and not l-er-via-trans
       then RUN piErro ("Nao foi encontrado item do pedido " + es-api-log.aux,"").
    END.
...
/*
{
  "numeroEmbarquePedido": "666666-654987",
  "id": "666666-654987-456-1-1888475-14566",
  "codigoProduto": "1888475",
  "quantidade": 100,
  "valorUnitario": 100.5,
  "pesoLiquido": 50,
