...
*******************************************************************************/
{include/i-prgvrs.i ESFTP006 9.99.99.999}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESFTP006
...
&GLOBAL-DEFINE page2Fields    FidtEntregaIni fidtEntregaFim fidtNffIni fidtNffFim ~
                              FiCodRepIni FiCodRepFim FiNomeAbRegIni FiNomeAbRegFim 
&GLOBAL-DEFINE page4Fields    fiPeriodo tgCarteira tgCotas tgDetalhes tgDevolucoes tgRealizado
...
DEFINE VARIABLE fidtEntregaFim AS DATE FORMAT "99/99/9999":U 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE FidtEntregaIni AS DATE FORMAT "99/99/9999":U 
     LABEL "Entrega Pedido" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE fidtNffFim AS DATE FORMAT "99/99/9999":U 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE fidtNffIni AS DATE FORMAT "99/99/9999":U 
     LABEL "Emissao Nota" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE FiNomeAbRegFim AS CHARACTER FORMAT "x(12)" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE FiNomeAbRegIni AS CHARACTER FORMAT "x(12)" 
     LABEL "Superintendente":R20 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88
     FONT 1 NO-UNDO.
...
DEFINE VARIABLE fiPeriodo AS CHARACTER FORMAT "99/9999":U INITIAL "000000" 
     LABEL "Periodo Cota" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE tgCarteira AS LOGICAL INITIAL yes 
     LABEL "Carteira" 
     VIEW-AS TOGGLE-BOX
     SIZE 30.29 BY .83 NO-UNDO.

DEFINE VARIABLE tgCotas AS LOGICAL INITIAL yes 
     LABEL "Cotas" 
     VIEW-AS TOGGLE-BOX
     SIZE 30.29 BY .83 NO-UNDO.

DEFINE VARIABLE tgDetalhes AS LOGICAL INITIAL no 
     LABEL "Imprime detalhes" 
     VIEW-AS TOGGLE-BOX
     SIZE 30.29 BY .83 NO-UNDO.

DEFINE VARIABLE tgDevolucoes AS LOGICAL INITIAL yes 
     LABEL "Devoluá‰es" 
     VIEW-AS TOGGLE-BOX
     SIZE 30.29 BY .83 NO-UNDO.

DEFINE VARIABLE tgRealizado AS LOGICAL INITIAL yes 
     LABEL "Realizado" 
     VIEW-AS TOGGLE-BOX
     SIZE 30.29 BY .83 NO-UNDO.
...
DEFINE FRAME fPage4
     fiPeriodo AT ROW 1.83 COL 21.86 COLON-ALIGNED
     tgCotas AT ROW 4.5 COL 24.43
     tgCarteira AT ROW 5.5 COL 24.43
     tgDevolucoes AT ROW 6.5 COL 24.43
     tgRealizado AT ROW 7.5 COL 24.43
     tgDetalhes AT ROW 8.83 COL 24.43
     RECT-10 AT ROW 1.5 COL 3.86
     RECT-11 AT ROW 3.67 COL 3.86
     "Acrescentar nos resultados:" VIEW-AS TEXT
          SIZE 21.14 BY .54 AT ROW 3.5 COL 5.57
...
DEFINE FRAME fPage2
     FidtEntregaIni AT ROW 1.5 COL 11.71
     fidtEntregaFim AT ROW 1.5 COL 49 NO-LABEL
     fidtNffIni AT ROW 2.5 COL 12.86
     fidtNffFim AT ROW 2.5 COL 49 NO-LABEL
     FiNomeAbRegIni AT ROW 3.5 COL 8.86
     FiNomeAbRegFim AT ROW 3.5 COL 49 NO-LABEL
     FiCodRepIni AT ROW 4.5 COL 10.86 HELP
          "C¢digo do representante direto"
     FiCodRepFim AT ROW 4.5 COL 49 HELP
          "C¢digo do representante direto" NO-LABEL
     IMAGE-1 AT ROW 1.5 COL 36
     IMAGE-2 AT ROW 1.5 COL 45
     IMAGE-3 AT ROW 2.5 COL 36
     IMAGE-4 AT ROW 2.5 COL 45
     IMAGE-5 AT ROW 3.5 COL 36
     IMAGE-6 AT ROW 3.5 COL 45
     IMAGE-7 AT ROW 4.5 COL 36
     IMAGE-8 AT ROW 4.5 COL 45
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
/* SETTINGS FOR FILL-IN FiCodRepIni IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN fidtEntregaFim IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FidtEntregaIni IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN fidtNffFim IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN fidtNffIni IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FiNomeAbRegFim IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FiNomeAbRegIni IN FRAME fPage2
   ALIGN-L                                                              */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    ASSIGN FiCodRepFim    = 99999
           FiCodRepIni    = 0
           fidtEntregaFim = TODAY
           FidtEntregaIni = TODAY
           fidtNffFim     = TODAY
           fidtNffIni     = TODAY
           FiNomeAbRegFim = "ZZZZZZZZZZZZ"
           FiNomeAbRegIni = ""
           fiPeriodo      = STRING(MONTH(TODAY),'99') + STRING(YEAR(TODAY),'9999').
...
do on error undo, return error on stop  undo, return error:
    ...
    */
    ...
    DO  WITH FRAME fPage2:
        ASSIGN tt-param.dt-ini-ent    = INPUT FidtEntregaIni  
               tt-param.dt-fim-ent    = INPUT fidtEntregaFim  
               tt-param.dt-ini-nff    = INPUT fidtNffIni      
               tt-param.dt-fim-nff    = INPUT fidtNffFim      
               tt-param.i-cod-rep-ini = INPUT FiCodRepIni  
               tt-param.i-cod-rep-fim = INPUT FiCodRepFim  
               tt-param.c-sup-ini     = INPUT FiNomeAbRegIni  
               tt-param.c-sup-fim     = INPUT FiNomeAbRegFim.
    END.

    DO WITH FRAME fPage4:
        ASSIGN tt-param.c-periodo       = INPUT fiPeriodo
               tt-param.detalhes        = INPUT tgDetalhes
               tt-param.cotas           = INPUT tgCotas
               tt-param.carteira        = INPUT tgCarteira
               tt-param.devolucoes      = INPUT tgDevolucoes
               tt-param.realizado       = INPUT tgRealizado.
    END.
    ...
    */
    ...
    {report/rprun.i esp/ftp/esftp006rp.p}
