...
{include/i-prgvrs.i ESFTP023 2.04.00.001}

/****************************  Definitions  ****************************/
{esp/ftp/esftp023tt.i}
...
def temp-table tt-nf
    ...
    index tt-nf is primary unique i-tipo dt-emis-nota nat-operacao nr-nota-fis
...
DEF TEMP-TABLE tt-itens-servico
    FIELD it-codigo LIKE ITEM.it-codigo
    INDEX ch_ind it-codigo.
...
/* Seleciona classe fiscal */
    RUN esp/es0018p.p (INPUT "esftp023",
                       ...
                       INPUT 0,
                       INPUT "",
                       OUTPUT TABLE tt-prog-ponto8) NO-ERROR.
/* Fim sele‡Æo */
...
PROCEDURE piMontaRelat:
    FOR EACH tt-prog-ponto8 NO-LOCK:
        CREATE tt-itens-servico.
        ASSIGN tt-itens-servico.it-codigo = trim(tt-prog-ponto8.conteudo).
       
    END.
    FOR EACH tt-prog-ponto1:
        FOR EACH natur-oper NO-LOCK WHERE 
            natur-oper.nat-operacao BEGINS entry(1,tt-prog-ponto1.conteudo):
    ...
    FOR EACH tt-prog-ponto4:
        FOR EACH natur-oper NO-LOCK WHERE 
            natur-oper.nat-operacao BEGINS entry(1,tt-prog-ponto4.conteudo):
            CREATE tt-natur-oper.
            ASSIGN tt-natur-oper.nat-operacao = natur-oper.nat-operacao.
    ...
    FOR EACH tt-prog-ponto5:
        FOR EACH natur-oper NO-LOCK WHERE 
            natur-oper.nat-operacao BEGINS entry(1,tt-prog-ponto5.conteudo):
            CREATE tt-natur-oper.
            ASSIGN tt-natur-oper.nat-operacao = natur-oper.nat-operacao.
...
*/
    ...
    DO da-data = tt-param.dt-emis-ini TO tt-param.dt-emis-fim:
       ...
       FOR EACH nota-fiscal NO-LOCK                WHERE 
           ...
           FIND first tt-itens-servico
               WHERE tt-itens-servico.it-codigo = it-nota-fisc.it-codigo NO-LOCK NO-ERROR.

           IF  AVAIL tt-itens-servico THEN DO:
    ...
    /*               IF NOT AVAIL tt-prog-ponto2 THEN NEXT.                                  */
                  ...
                  IF AVAIL int-familia AND
                     int-familia.ind-lei-informatica = YES THEN .
                  ELSE
                      NEXT.
               ...
               FOR EACH tt-prog-ponto1:
                   IF it-nota-fisc.aliquota-ipi = de-ali-1 OR
                      ...
                      it-nota-fisc.aliquota-ipi = de-ali-6 OR
                      it-nota-fisc.nat-operacao BEGINS string(entry(1,tt-prog-ponto1.conteudo)) THEN DO:   

                      IF  it-nota-fisc.nat-operacao BEGINS string(entry(2,tt-prog-ponto1.conteudo)) THEN NEXT. /* Desconsidera */
               ...
               IF l-achou = YES THEN NEXT.

               FOR EACH tt-prog-ponto4:
                   IF it-nota-fisc.nat-operacao BEGINS tt-prog-ponto4.conteudo THEN DO:
               ...
               FOR EACH tt-prog-ponto5:
                   IF it-nota-fisc.nat-operacao begins tt-prog-ponto5.conteudo THEN DO:
...
PROCEDURE piImprimeRelat:
    ...
    if tt-param.excel = yes then       
        put "T;Item   ;Descricao                           ;Class Fisc"
            ...
            "IPI Devol       ;ICMS Devol   ;PIS/COFINS Dev;Qtde Saldo;Valr Saldo ;IPI Saldo;ICMS Saldo;"
            "PIS/COFINS    ;Valor Base        ;Conv I  ;Conv II ;Conv IIb  ;Conv SC ;Deposito  ;Interno;UF     ;Cliente  ;Taxa exportacao" skip.
    else
        put "T Item    Descricao                            Class Fisc"
            " Nat Op NF                  Emissao Familia    Quant Fatur    Vl Mercad Liq   "
            "Valor IPI Item     Vl ICMS Item       PIS/COFINS       Quant Devol      Vl Devolucao         "
    ...
    for each tt-nf where: /* tt-nf.i-tipo = 1: */
        put tt-nf.i-tipo c-separador
            tt-nf.it-codigo c-separador
            tt-nf.descricao c-separador
            tt-nf.class-fiscal c-separador
            tt-nf.nat-operacao c-separador
            tt-nf.nr-nota-fis c-separador
            tt-nf.dt-emis-nota c-separador
            tt-nf.fm-codigo c-separador
            tt-nf.qt-fatura c-separador
            tt-nf.vl-merc-liq c-separador  
            tt-nf.vl-ipi-it c-separador    
            tt-nf.vl-icms-it c-separador   
            tt-nf.vl-pis-cofins c-separador
            tt-nf.qt-devolvida c-separador 
            tt-nf.vl-devol format "->>,>>>,>>9.99" c-separador    
            tt-nf.vl-ipi-devol c-separador
            tt-nf.vl-icms-devol c-separador
            tt-nf.vl-pis-cofins-devol c-separador
            tt-nf.qt-fatura - tt-nf.qt-devolvida  format "->>,>>>,>>9.99" c-separador
            tt-nf.vl-merc-liq - tt-nf.vl-devol format "->>,>>>,>>9.99" c-separador
            tt-nf.vl-ipi-it - tt-nf.vl-ipi-devol format "->>,>>>,>>9.99" c-separador
            tt-nf.vl-icms-it - tt-nf.vl-icms-devol format "->>,>>>,>>9.99" c-separador
            tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol format "->>,>>>,>>9.99" c-separador

            (tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol) format "->>,>>>,>>9.99" c-separador
            
            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) * 
            tt-param.de-convenio-1 / 100 format "->>,>>>,>>9.99" c-separador
            
            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) * 
            tt-param.de-convenio-2a / 100  format "->>,>>>,>>9.99" c-separador

            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) * 
            tt-param.de-convenio-2b / 100 format "->>,>>>,>>9.99" c-separador

            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) * 
            tt-param.de-convenio-sc / 100 format "->>,>>>,>>9.99" c-separador

            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) * 
            tt-param.de-deposito / 100 format "->>,>>>,>>9.99" c-separador

            ((tt-nf.vl-merc-liq - tt-nf.vl-devol) -
            (tt-nf.vl-icms-it - tt-nf.vl-icms-devol) -
            (tt-nf.vl-pis-cofins - tt-nf.vl-pis-cofins-devol)) *            
            tt-param.de-perc-int / 100 format "->>,>>>,>>9.99" c-separador

            tt-nf.estado c-separador
            tt-nf.cod-emitente c-separador
            tt-nf.vl-taxa-exp 
            skip. 
    end.
END.
...
procedure pi-sem-zona-franca:               
    ...
    assign tt-nf.qt-fatura     = tt-nf.qt-fatura   + it-nota-fisc.qt-faturada[1]
           tt-nf.vl-merc-liq   = tt-nf.vl-merc-liq + it-nota-fisc.vl-merc-liq
           tt-nf.vl-ipi-it     = tt-nf.vl-ipi-it   + it-nota-fisc.vl-ipi-it
           tt-nf.vl-icms-it    = tt-nf.vl-icms-it  + it-nota-fisc.vl-icms-it.
    ...
    if it-nota-fisc.nat-operacao begins "51" or
       ...
       assign tt-nf.vl-pis-cofins = tt-nf.vl-pis-cofins + 
                                    if SUBSTRING(it-nota-fisc.nat-operacao,1,4) <> "6109" then
...
procedure pi-ver-devolucao.
   for each docum-est no-lock
       ...
       if avail it-nota-fisc then DO:
           ...
           FIND first tt-itens-servico
               WHERE tt-itens-servico.it-codigo = item-doc-est.it-codigo NO-LOCK NO-ERROR.

           IF  AVAIL tt-itens-servico THEN DO:
                RUN pi-grava-devolucao.
           END.
           ELSE DO:
        ...
        /*        IF NOT AVAIL tt-prog-ponto2 THEN NEXT.                                  */
               ...
               if it-nota-fisc.aliquota-ipi = tt-param.de-ali-1 or
                  ...
                  it-nota-fisc.aliquota-ipi = tt-param.de-ali-5 or
                  it-nota-fisc.aliquota-ipi = tt-param.de-ali-6 then do:
...
PROCEDURE pi-grava-devolucao:
       ...
       if it-nota-fisc.nat-operacao begins "61" or
          ...
          it-nota-fisc.nat-operacao begins "693301" THEN DO:
      ...
      where docum-est.dt-trans = da-data
       ...
       find first it-nota-fisc of nota-fiscal no-lock
            where it-nota-fisc.it-codigo = item-doc-est.it-codigo 
              ...
              and it-nota-fisc.nat-operacao BEGINS "6109" no-error.
   ...
   for each docum-est no-lock
      where docum-est.dt-trans = da-data
       ...
       find first it-nota-fisc of nota-fiscal no-lock
            where it-nota-fisc.it-codigo = item-doc-est.it-codigo 
              and it-nota-fisc.nr-seq-fat = item-doc-est.seq-comp 
              and it-nota-fisc.nat-operacao begins "7" no-error.
