...
*******************************************************************************/
{include/i-prgvrs.i esftp210 2.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-gera-pdf 
...
DEFINE VARIABLE h_esftp210-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp210-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp210-v01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp210-v02 AS HANDLE NO-UNDO.
...
DEFINE BUTTON bt-gera-pdf 
     IMAGE-UP FILE "image\im-calc2.bmp":U
     LABEL "" 
     SIZE 4 BY 1.
...
DEFINE FRAME f-cad
     bt-gera-pdf AT ROW 1.25 COL 58 WIDGET-ID 2
...
&Scoped-define SELF-NAME bt-gera-pdf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-gera-pdf w-cadpaifilho-ambos
ON CHOOSE OF bt-gera-pdf IN FRAME f-cad
DO:

  RUN pi-posicao-query IN h_esftp210-q01 (OUTPUT r-rowid).
  RUN esp/ftp/esftp210c.p (INPUT r-rowid,
                           INPUT 1 /*Somente gera o pdf*/).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp210-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp210-v01 ).
       RUN set-position IN h_esftp210-v01 ( 2.75 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Itens Devol|Observa‡äes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 7.63 , 1.43 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp210-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/ftp/z01esftp210.w,
                     ProgVaPara = esp/ftp/g01esftp210.w,
                     ProgIncMod = esp/ftp/esftp210a.w,
                     Implantar = no':U ,
             OUTPUT h_esftp210-q01 ).
       RUN set-position IN h_esftp210-q01 ( 1.25 , 47.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_esftp210-v01. */
       RUN add-link IN adm-broker-hdl ( h_esftp210-q01 , 'Record':U , h_esftp210-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_esftp210-v01 ).
       RUN add-link IN adm-broker-hdl ( h_esftp210-v01 , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_esftp210-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_esftp210-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esftp210-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esftp210-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esftp210-q01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-gera-pdf:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadpai ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp210-v01 ,
             bt-gera-pdf:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_esftp210-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp210-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/ftp/esftp210b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_esftp210-b01 ).
       RUN set-position IN h_esftp210-b01 ( 9.50 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 86.00 ) */

       /* Links to BrowserCadastro2 h_esftp210-b01. */
       RUN add-link IN adm-broker-hdl ( h_esftp210-q01 , 'Record':U , h_esftp210-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp210-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp210-v02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp210-v02 ).
       RUN set-position IN h_esftp210-v02 ( 9.50 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.50 , 79.00 ) */

       /* Links to SmartViewer h_esftp210-v02. */
       RUN add-link IN adm-broker-hdl ( h_esftp210-q01 , 'Record':U , h_esftp210-v02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp210-v02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFTP210" "2.00.00.000"}
...
-------------------------------------------------------------*/
  ...
  IF p-state = "desabilita" THEN DO:
      RUN enable-modifica IN h_p-cadpai (INPUT NO).  
      RUN enable-elimina  IN h_p-cadpai (INPUT NO).  
      RUN local-display-fields IN h_esftp210-b01.
  END.

  IF  p-state = "habilita" THEN DO:
      RUN enable-modifica IN h_p-cadpai (INPUT YES).  
      RUN enable-elimina  IN h_p-cadpai (INPUT YES).  
      RUN local-display-fields IN h_esftp210-b01.
  END.
END PROCEDURE.
