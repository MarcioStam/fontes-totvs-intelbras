...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-sup-exec i_periodo-mes ~
i_periodo-ano bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-sup-exec i_periodo-mes i_periodo-ano ~
i_idi-tipo 
...
DEFINE VARIABLE i_cod-sup-exec AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Executivo" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE i_idi-tipo AS INTEGER FORMAT ">9":U INITIAL 1 
     VIEW-AS FILL-IN 
     SIZE .14 BY .21 NO-UNDO.

DEFINE VARIABLE i_periodo-ano AS INTEGER FORMAT "9999":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE i_periodo-mes AS INTEGER FORMAT "99":U INITIAL 0 
     LABEL "Per¡odo" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-sup-exec AT ROW 1.25 COL 30 COLON-ALIGNED WIDGET-ID 6
     i_periodo-mes AT ROW 2.25 COL 30 COLON-ALIGNED WIDGET-ID 2
     i_periodo-ano AT ROW 2.25 COL 34 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     i_idi-tipo AT ROW 3.17 COL 64 COLON-ALIGNED NO-LABEL WIDGET-ID 8
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     "/" VIEW-AS TEXT
          SIZE 1 BY .54 AT ROW 2.42 COL 35 WIDGET-ID 10
...
/* SETTINGS FOR FILL-IN i_idi-tipo IN FRAME d-vapara
   NO-ENABLE                                                            */
ASSIGN 
       i_idi-tipo:HIDDEN IN FRAME d-vapara           = TRUE.
...
DO:
FIND int-execsuperv-calc NO-LOCK
    WHERE int-execsuperv-calc.periodo-mes EQ INPUT FRAME {&FRAME-NAME} i_periodo-mes
      AND int-execsuperv-calc.periodo-ano EQ INPUT FRAME {&FRAME-NAME} i_periodo-ano
      AND int-execsuperv-calc.cod-sup-exec EQ INPUT FRAME {&FRAME-NAME} i_cod-sup-exec
      AND int-execsuperv-calc.idi-tipo EQ INPUT FRAME {&FRAME-NAME} i_idi-tipo NO-ERROR.

IF NOT AVAILABLE int-execsuperv-calc THEN DO:
    {utp/ut-table.i mgesp int-execsuperv-calc 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-execsuperv-calc).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-sup-exec i_periodo-mes i_periodo-ano i_idi-tipo 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-sup-exec i_periodo-mes i_periodo-ano bt-ok bt-cancela 
         bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "g01esftp205" "2.00.00.000"}
