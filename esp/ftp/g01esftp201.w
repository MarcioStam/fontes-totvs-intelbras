...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-executivo bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-executivo 
...
DEFINE VARIABLE i_cod-executivo AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-executivo AT ROW 2 COL 18 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 40.14
...
DO:
FIND int-executivo NO-LOCK
    WHERE int-executivo.cod-executivo EQ INPUT FRAME {&FRAME-NAME} i_cod-executivo NO-ERROR.

IF NOT AVAILABLE int-executivo THEN DO:
    {utp/ut-table.i mgesp int-executivo 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-executivo).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-executivo 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-executivo bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01esftp201" "2.00.00.000"}
