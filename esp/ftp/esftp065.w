...
&GLOBAL-DEFINE Program        ESFTP065
...
&GLOBAL-DEFINE page5Widgets   brDigita bt-marca bt-marca-todos bt-desmarca-todos bt-marca-sel
...
&GLOBAL-DEFINE page2Fields    c-cod-estabel c-serie dt-ini-emiss c-atendente-ini c-atendente-fim dt-fim-emiss c-estado-ini c-estado-fim ~
                              fi-uf-origem  iNome-transp  
...
DEFINE VARIABLE i-nome-programa AS CHAR.
DEFINE VARIABLE i-ponto         AS INT.
DEFINE VARIABLE i-sequencia     AS INT.
DEFINE VARIABLE i-conteudo      AS CHAR.
DEFINE VARIABLE l-volta         AS LOG.
...
&Scoped-define FIELDS-IN-QUERY-brDigita tt-digita.selecionado tt-digita.cod-estab tt-digita.serie tt-digita.nr-nota-fis tt-digita.estado tt-digita.cod-dep tt-digita.nr-pedcli tt-digita.nome-ab-cli tt-digita.dt-emissao tt-digita.cd-atendente tt-digita.nome-transp tt-digita.vl-total-nota tt-digita.nr-volume   
...
DEFINE VARIABLE c-atendente-ini AS CHARACTER FORMAT "X(2)":U 
     LABEL "Atendente" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.
...
DEFINE VARIABLE c-serie AS CHARACTER FORMAT "X(256)" 
     LABEL "Serie" 
     VIEW-AS FILL-IN 
     SIZE 8 BY .88 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brDigita wReport _FREEFORM
  QUERY brDigita DISPLAY
    ...
    tt-digita.dt-emissao  COLUMN-LABEL "Dt Emis" WIDTH 11
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 82 BY 8.75
...
DEFINE FRAME fPage2
     l-FatComercial AT ROW 2.21 COL 33.72 WIDGET-ID 6
     c-cod-estabel AT ROW 2.25 COL 16 COLON-ALIGNED HELP
          "Estabelecimento emissor do documento fiscal" WIDGET-ID 8
     c-serie AT ROW 3.25 COL 16 COLON-ALIGNED HELP
          "Estabelecimento emissor do documento fiscal" WIDGET-ID 56
     dt-ini-emiss AT ROW 4.25 COL 14 COLON-ALIGNED
     Dt-fim-Emiss AT ROW 4.25 COL 33 COLON-ALIGNED NO-LABEL
     c-atendente-ini AT ROW 5.25 COL 20 COLON-ALIGNED WIDGET-ID 42
     c-atendente-fim AT ROW 5.25 COL 33 COLON-ALIGNED NO-LABEL WIDGET-ID 40
     c-estado-ini AT ROW 6.25 COL 20 COLON-ALIGNED WIDGET-ID 48
     c-estado-fim AT ROW 6.25 COL 33 COLON-ALIGNED NO-LABEL WIDGET-ID 54
     fi-uf-origem AT ROW 7.63 COL 14 COLON-ALIGNED HELP
          "Unidade da Federaá∆o" WIDGET-ID 34
     iNome-transp AT ROW 7.63 COL 38 COLON-ALIGNED WIDGET-ID 36
     bt-buscar AT ROW 9 COL 65.43
     IMAGE-1 AT ROW 4.25 COL 26.57
     IMAGE-2 AT ROW 4.25 COL 31.57
     RECT-12 AT ROW 7.29 COL 2.72 WIDGET-ID 38
     IMAGE-3 AT ROW 5.25 COL 26.57 WIDGET-ID 44
     IMAGE-4 AT ROW 5.25 COL 31.57 WIDGET-ID 46
     IMAGE-5 AT ROW 6.25 COL 26.57 WIDGET-ID 50
     IMAGE-6 AT ROW 6.25 COL 31.57 WIDGET-ID 52
...
DEFINE FRAME fPage5
     ...
     bt-marca-sel AT ROW 10 COL 48.72 WIDGET-ID 2
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-marca-sel wReport
ON CHOOSE OF bt-marca-sel IN FRAME fPage5 /* Marca Selecionados */
DO:
    ASSIGN i-num-select-row = browse brDigita:num-selected-rows.
    
    do_blk:
    do  i-num-row-a = 1 to i-num-select-row ON ERROR UNDO do_blk, LEAVE do_blk:
        BROWSE brDigita:FETCH-SELECTED-ROW(i-num-row-a).
...
DO:
  if self:private-data ne self:screen-value then do:
    ...
    ASSIGN c-lista = SUBSTRING(c-lista,1,LENGTH(c-lista) - 1)
           iNome-transp:LIST-ITEM-PAIRS IN FRAME fPage2 = c-lista. 
...
------------------------------------------------------------------------------*/
    ...
    END.
     ...
     FOR EACH b-ponto-programa WHERE 
             b-ponto-programa.nome-programa = "ESFTP065":
    ...
    DO da-data = INPUT FRAME fPage2 Dt-ini-emiss TO INPUT FRAME fPage2 Dt-fim-Emiss:
        ...
        FOR EACH nota-fiscal USE-INDEX ch-distancia NO-LOCK         
           WHERE nota-fiscal.cod-estabel    = estabelec.cod-estabel 
             AND nota-fiscal.serie          = c-serie:SCREEN-VALUE IN FRAME fPage2       
             AND nota-fiscal.dt-emis-nota   = da-data               
             AND nota-fiscal.dt-cancel      = ?                     
             AND nota-fiscal.cdd-embarq     = 0
             AND nota-fiscal.estado         >= c-estado-ini:SCREEN-VALUE IN FRAME fPage2 
             AND nota-fiscal.estado         <= c-estado-fim:SCREEN-VALUE IN FRAME fPage2,
           FIRST natur-oper NO-LOCK 
           WHERE natur-oper.nat-operacao = nota-fiscal.nat-operacao:

            IF nota-fiscal.nome-transp <> iNome-transp:SCREEN-VALUE IN FRAME fPage2 THEN NEXT.
            /***
            ...
            ***/
            ...
            IF INPUT FRAME fPage2 l-FatComercial = YES THEN DO: /* Faturamento Comercial Autom†tico */
                ...
                IF ped-venda.tp-pedido < c-atendente-ini:SCREEN-VALUE IN FRAME fPage2 
                OR ped-venda.tp-pedido > c-atendente-fim:SCREEN-VALUE IN FRAME fPage2 THEN
                    NEXT.
                    ...
                    FOR EACH it-nota-fisc OF nota-fiscal NO-LOCK:
                        ...
                        IF NOT AVAIL pre-fatur THEN DO:
                            ...
                            IF NOT AVAIL tt-digita THEN DO:
                                ...
                                FIND FIRST fat-ser-lote USE-INDEX ch-lote NO-LOCK
                                ...
                                ASSIGN tt-digita.cod-dep = IF AVAIL fat-ser-lote THEN fat-ser-lote.cod-dep ELSE "".
...
------------------------------------------------------------------------------*/
    ...
    DO  WITH FRAME fPage2:
        ASSIGN  tt-param.cod-estabel   = INPUT c-cod-estabel
                tt-param.serie         = INPUT c-serie
                tt-param.dt-ini-emiss  = INPUT dt-ini-emiss  
                tt-param.dt-fim-emiss  = INPUT dt-fim-emiss
                tt-param.atendente-ini = INPUT c-atendente-ini
                tt-param.atendente-fim = INPUT c-atendente-fim
                tt-param.estado-ini    = INPUT c-estado-ini
                tt-param.estado-fim    = INPUT c-estado-fim
                .
    END.
    ...
    {report/rprun.i esp/ftp/ESFTP065rp.p}
