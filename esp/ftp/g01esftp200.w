...
*******************************************************************************/
{include/i-prgvrs.i g01esftp200 2.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_periodo-mes i_periodo-ano bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_periodo-mes i_periodo-ano 
...
DEFINE VARIABLE i_periodo-ano AS INTEGER FORMAT "9999":U 
     VIEW-AS FILL-IN 
     SIZE 5.14 BY .88 NO-UNDO.

DEFINE VARIABLE i_periodo-mes AS INTEGER FORMAT "99":U 
     LABEL "Mˆs" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_periodo-mes AT ROW 2 COL 20 COLON-ALIGNED WIDGET-ID 2
     i_periodo-ano AT ROW 2 COL 24.43 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     "/" VIEW-AS TEXT
          SIZE 1 BY .54 AT ROW 2.17 COL 25.14 WIDGET-ID 6
...
DO:
FIND int-param-comis NO-LOCK
    WHERE int-param-comis.periodo-mes EQ INPUT FRAME {&FRAME-NAME} i_periodo-mes
      AND int-param-comis.periodo-ano EQ INPUT FRAME {&FRAME-NAME} i_periodo-ano NO-ERROR.

IF NOT AVAILABLE int-param-comis THEN DO:
    {utp/ut-table.i mgesp int-param-comis 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-param-comis).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_periodo-mes i_periodo-ano 
      WITH FRAME d-vapara.
  ENABLE rt-button i_periodo-mes i_periodo-ano bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "g01esftp200" "2.00.00.000"}
