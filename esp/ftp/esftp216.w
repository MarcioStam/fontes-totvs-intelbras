...
*******************************************************************************/
{include/i-prgvrs.i esftp216 2.00.00.001}
...
&Scoped-define FIELDS-IN-QUERY-br-digita tt-digita.it-codigo tt-digita.desc-item tt-digita.qtde   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-digita tt-digita.it-codigo tt-digita.qtde   
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnDescItem w-relat 
...
DEFINE VARIABLE c-nat-operacao AS CHARACTER FORMAT "X(06)":U 
...
DEFINE FRAME f-pg-sel
     c-cod-estabel AT ROW 2 COL 26 COLON-ALIGNED
     c-nome-abrev AT ROW 3 COL 26 COLON-ALIGNED WIDGET-ID 2
     c-nat-operacao AT ROW 4 COL 26 COLON-ALIGNED WIDGET-ID 4
     c-nome-transp AT ROW 5 COL 26 COLON-ALIGNED WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
...
DO:
    if  br-digita:new-row in frame f-pg-dig then do:
        ...
        display tt-digita.it-codigo
                tt-digita.qtde with browse br-digita. 
    end.
    return no-apply.
END.
...
DO:
    ...
    do transaction on error undo, return no-apply:
        ...
        assign input browse br-digita tt-digita.it-codigo
               input browse br-digita tt-digita.qtde.
...
{utp/ut9000.i "esftp216" "2.00.00.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ON "LEAVE":U OF tt-digita.it-codigo IN BROWSE br-digita DO:
        FIND FIRST ITEM NO-LOCK
             WHERE ITEM.it-codigo = tt-digita.it-codigo:SCREEN-VALUE IN BROWSE br-digita  NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-cod-estabel c-nome-abrev c-nat-operacao c-nome-transp 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE c-cod-estabel c-nome-abrev c-nat-operacao c-nome-transp 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF NOT CAN-FIND (FIRST estabelec
                     WHERE estabelec.cod-estabel = INPUT FRAME f-pg-sel c-cod-estabel) THEN DO:
    ...
    IF NOT CAN-FIND (FIRST emitente
                     WHERE emitente.nome-abrev = INPUT FRAME f-pg-sel c-nome-abrev) THEN DO:

        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Cliente n∆o encontrado.").
        RETURN ERROR.
                           
    END.

    IF NOT CAN-FIND (FIRST natur-oper
                     WHERE natur-oper.nat-operacao = INPUT FRAME f-pg-sel c-nat-operacao) THEN DO:
    ...
    IF NOT CAN-FIND (FIRST transporte
                     WHERE transporte.nome-abrev = INPUT FRAME f-pg-sel c-nome-transp) THEN DO:

        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Transportadora n∆o encontrada.").
        RETURN ERROR.
    ...
    IF NOT CAN-FIND (FIRST tt-digita) THEN DO:
        ...
        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Digite um item na aba de digitaá∆o.").
        RETURN ERROR.
    ...
    for each tt-digita no-lock:
        ...
        IF  NOT CAN-FIND (FIRST ITEM
            ...
            run utp/ut-msgs.p (input "show":U, input 17006, input "Item n∆o encontrado").
            apply "ENTRY":U to tt-digita.it-codigo in browse br-digita.
            
            return error.
        end.

        IF  tt-digita.qtde <= 0 then do:
            assign browse br-digita:CURRENT-COLUMN = tt-digita.qtde:HANDLE in browse br-digita.
            
            apply "MOUSE-SELECT-CLICK":U to im-pg-dig in frame f-relat.
            reposition br-digita to rowid r-tt-digita.
            
            run utp/ut-msgs.p (input "show":U, input 17006, input "Quantidade deve ser meior que 0").
            apply "ENTRY":U to tt-digita.qtde in browse br-digita.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.nome-abrev      = input frame f-pg-sel c-nome-abrev
           tt-param.nat-operacao    = input frame f-pg-sel c-nat-operacao
           tt-param.nome-transp     = input frame f-pg-sel c-nome-transp
           .
    ...
    {include/i-rprun.i esp/ftp/esftp216rp.p}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnDescItem w-relat 
FUNCTION fnDescItem RETURNS CHARACTER
  ( p-it-codigo AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
