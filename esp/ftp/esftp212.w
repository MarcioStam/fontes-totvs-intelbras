...
*******************************************************************************/
{include/i-prgvrs.i esftp212 2.00.00.001}
...
DEFINE VARIABLE c-destino-fim AS CHARACTER FORMAT "!!":U INITIAL "ZZ" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE c-destino-ini AS CHARACTER FORMAT "!!":U 
     LABEL "UF Destino" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE c-estabelec AS CHARACTER FORMAT "X(100)":U INITIAL "103,104,105,110,111" 
...
DEFINE VARIABLE c-origem-fim AS CHARACTER FORMAT "!!":U INITIAL "ZZ" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE c-origem-ini AS CHARACTER FORMAT "!!":U 
     LABEL "UF Origem" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.
...
DEFINE VARIABLE i-item-fat AS INTEGER INITIAL 3 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Produtos Fatur†veis", 1,
"Produtos n∆o Fatur†veis", 2,
"AMBOS", 3
     SIZE 54 BY 1.63 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     c-estabelec AT ROW 2.5 COL 16 COLON-ALIGNED WIDGET-ID 32
     c-it-codigo-ini AT ROW 3.5 COL 16 COLON-ALIGNED
     c-it-codigo-fim AT ROW 3.5 COL 46 COLON-ALIGNED NO-LABEL
     c-origem-ini AT ROW 4.5 COL 26 COLON-ALIGNED WIDGET-ID 12
     c-origem-fim AT ROW 4.5 COL 46 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     c-destino-ini AT ROW 5.5 COL 26 COLON-ALIGNED WIDGET-ID 20
     c-destino-fim AT ROW 5.5 COL 46 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     i-item-fat AT ROW 8 COL 12.72 NO-LABEL WIDGET-ID 22
     IMAGE-1 AT ROW 3.5 COL 33
     IMAGE-2 AT ROW 3.5 COL 45.14
     IMAGE-13 AT ROW 5.54 COL 33 WIDGET-ID 6
     IMAGE-14 AT ROW 5.54 COL 45.14 WIDGET-ID 8
     IMAGE-15 AT ROW 4.5 COL 33 WIDGET-ID 14
     IMAGE-16 AT ROW 4.5 COL 45.14 WIDGET-ID 16
     RECT-10 AT ROW 7.5 COL 2 WIDGET-ID 26
     RECT-12 AT ROW 1.75 COL 2 WIDGET-ID 34
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
DO:
    if  br-digita:new-row in frame f-pg-dig then do:
        ...
        display tt-digita.it-codigo tt-digita.desc-item with browse br-digita. 
    end.
    return no-apply.
END.
...
DO:
    ...
    FIND FIRST ITEM 
         WHERE ITEM.it-codigo = tt-digita.it-codigo:SCREEN-VALUE IN BROWSE br-digita 
    NO-LOCK NO-ERROR.

    IF NOT AVAIL ITEM THEN
    DO:
        MESSAGE 'Item informado nao localizado'
...
DO:
    ...
    do transaction on error undo, return no-apply:
        create tt-digita.
        assign input browse br-digita tt-digita.it-codigo tt-digita.desc-item. 
        ...
        ELSE
        DO: 
           ASSIGN tt-digita.desc-item:screen-value in browse br-digita = ''.
...
ON F5 OF c-estabelec IN FRAME f-pg-sel /* Estabelecimentos */
DO:
   {include/zoomvar.i &prog-zoom = inzoom/z01in172.w
                      &campo=c-it-codigo-ini
                      &campozoom=it-codigo}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-estabelec w-relat
ON MOUSE-SELECT-DBLCLICK OF c-estabelec IN FRAME f-pg-sel /* Estabelecimentos */
DO:
   APPLY 'f5' TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-it-codigo-ini w-relat
ON F5 OF c-it-codigo-ini IN FRAME f-pg-sel /* Item */
DO:
   {include/zoomvar.i &prog-zoom = inzoom/z01in172.w
                      &campo=c-it-codigo-ini
                      &campozoom=it-codigo}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-it-codigo-ini w-relat
ON MOUSE-SELECT-DBLCLICK OF c-it-codigo-ini IN FRAME f-pg-sel /* Item */
DO:
   APPLY 'f5' TO SELF.
END.
...
{utp/ut9000.i "esftp212" "2.00.00.001"}

/*:T inicializaá‰es do template de relat¢rio */
{include/i-rpini.i}

c-it-codigo-ini:load-mouse-pointer ("image/lupa.cur") IN FRAME f-pg-sel.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-estabelec c-it-codigo-ini c-it-codigo-fim c-origem-ini c-origem-fim 
          c-destino-ini c-destino-fim i-item-fat 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-13 IMAGE-14 IMAGE-15 IMAGE-16 RECT-10 RECT-12 
         c-estabelec c-it-codigo-ini c-it-codigo-fim c-origem-ini c-origem-fim 
         c-destino-ini c-destino-fim i-item-fat 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    for each tt-digita no-lock:
        ...
        IF NOT CAN-FIND (FIRST ITEM
            ...
            run utp/ut-msgs.p (input "show":U, input 17006, input "N∆o encontrado item " + tt-digita.it-codigo).
            apply "ENTRY":U to tt-digita.it-codigo in browse br-digita.
    ...
    IF SUBSTRING(c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel,LENGTH(c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel),1) <> ',' THEN
       ASSIGN c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel = c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel + ','.

    ASSIGN c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel = REPLACE(c-estabelec:SCREEN-VALUE IN FRAME f-pg-sel,';',',').
    ...
    assign tt-param.usuario          = c-seg-usuario
           ...
           tt-param.uf-origem-ini    = input frame f-pg-sel c-origem-ini 
           tt-param.uf-origem-fim    = input frame f-pg-sel c-origem-fim 
           tt-param.uf-destino-ini   = input frame f-pg-sel c-destino-ini
           tt-param.uf-destino-fim   = input frame f-pg-sel c-destino-fim
           tt-param.it-codigo-ini    = input frame f-pg-sel c-it-codigo-ini
           tt-param.it-codigo-fim    = input frame f-pg-sel c-it-codigo-fim
           tt-param.item-fat         = input frame f-pg-sel i-item-fat
           tt-param.estabelecimentos = input frame f-pg-sel c-estabelec.
    ...
    {include/i-rprun.i esp/ftp/esftp212rp.p}
