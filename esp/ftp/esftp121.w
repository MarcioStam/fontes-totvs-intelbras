...
DEFINE TEMP-TABLE tt-int-rateio-kit NO-UNDO LIKE int-rateio-kit
...
DEFINE BUFFER bf-int-rateio-kit FOR int-rateio-kit.
...
&Scoped-define FIELDS-IN-QUERY-brFilhos tt-int-rateio-kit.it-componente fn-descricao(tt-int-rateio-kit.it-componente) @ c-descricao tt-int-rateio-kit.perc-custo tt-int-rateio-kit.perc-fatur   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brFilhos   
&Scoped-define SELF-NAME brFilhos
&Scoped-define QUERY-STRING-brFilhos FOR EACH tt-int-rateio-kit     WHERE tt-int-rateio-kit.it-codigo = ttitem.it-codigo        BY tt-int-rateio-kit.it-codigo
&Scoped-define OPEN-QUERY-brFilhos OPEN QUERY {&SELF-NAME} FOR EACH tt-int-rateio-kit     WHERE tt-int-rateio-kit.it-codigo = ttitem.it-codigo        BY tt-int-rateio-kit.it-codigo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brFilhos wMaintenance _FREEFORM
  QUERY brFilhos DISPLAY
      ...
      fn-descricao(tt-int-rateio-kit.it-componente) @ c-descricao
      tt-int-rateio-kit.perc-custo
      tt-int-rateio-kit.perc-fatur
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 93 BY 7.5
...
DEFINE FRAME fpage0
     ...
     btSearch AT ROW 1.13 COL 21.57 HELP
          "Pesquisa"
     btQueryJoins AT ROW 1.13 COL 81.72 HELP
          "Consultas relacionadas"
     btReportsJoins AT ROW 1.13 COL 85.72 HELP
          "Relat¢rios relacionados"
     btExit AT ROW 1.13 COL 89.72 HELP
          "Sair"
     btHelp AT ROW 1.13 COL 93.72 HELP
          "Ajuda"
     ttitem.it-codigo AT ROW 3.25 COL 22.86 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     ttitem.desc-item AT ROW 3.25 COL 35.86 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 44 BY .88
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-int-rateio-kit T "?" NO-UNDO mgesp int-rateio-kit
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-int-rateio-kit
    WHERE tt-int-rateio-kit.it-codigo = ttitem.it-codigo
...
&Scoped-define SELF-NAME brFilhos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL brFilhos wMaintenance
ON ENTRY OF brFilhos IN FRAME fPage1 /* Componentes Rateio */
DO:
    /*
     FIND int-rateio-kit NO-LOCK
         WHERE ROWID(int-rateio-kit) = tt-int-rateio-kit.r-rowid NO-ERROR.
     MESSAGE "entry: " AVAIL int-rateio-kit
             int-rateio-kit.perc-custo
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL brFilhos wMaintenance
ON TAB OF brFilhos IN FRAME fPage1 /* Componentes Rateio */
DO:
  /*RETURN NO-APPLY.*/
END.
...
ON CHOOSE OF bt-incluir IN FRAME fPage1 /* Modificar Rateio */
DO:
     
     
     RUN esp/ftp/esftp121a.w (INPUT ttitem.it-codigo).
    ...
    DEFINE VARIABLE de-perc-custo  LIKE int-rateio-kit.perc-custo NO-UNDO.
    DEFINE VARIABLE de-perc-fatur  LIKE int-rateio-kit.perc-fatur NO-UNDO.
    DEFINE VARIABLE i-nr-sequencia AS INTEGER                NO-UNDO.    


    DEFINE FRAME fItemFilho
        c-it-codigo       AT ROW 1.21 COL 14.72 
        c-desc-item       AT ROW 2.21 COL 14.72 NO-LABEL
        de-perc-custo     AT ROW 3.21 COL 14.72 
        de-perc-fatur     AT ROW 4.21 COL 24.72 
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fItemFilho DO:
        ...
        IF CAN-FIND(FIRST int-rateio-kit 
                    WHERE int-rateio-kit.it-codigo   = ttitem.it-codigo
                      AND int-rateio-kit.it-componente = c-it-codigo) THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U, INPUT 17006, INPUT "Item j  cadastrado.":U).
            RETURN NO-APPLY.
        END.
                
        CREATE int-rateio-kit.
        ASSIGN int-rateio-kit.it-codigo   = ttitem.it-codigo
               int-rateio-kit.it-componente = c-it-codigo.
    ...
    ENABLE c-it-codigo de-perc-custo de-perc-fatur btGoToOK btGoToCancel 
        WITH FRAME fItemFilho. 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    ENABLE bt-incluir brFilhos WITH FRAME fPage1.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-abre-browser wMaintenance 
PROCEDURE pi-abre-browser :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
