...
*******************************************************************************/
{include/i-prgvrs.i ESFTP106 2.00.00.000}
...
DEFINE VARIABLE h_q01in106 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01in106 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE IMAGE bt-importar
     FILENAME "adeicon/cntrlhry.ico":U
     SIZE 4.86 BY 1.17 TOOLTIP "Importar Filial".
...
DEFINE FRAME f-cad
     ...
     bt-importar AT ROW 1.25 COL 58 WIDGET-ID 4
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Cadastro de Lojas do Cliente"
         HEIGHT             = 7.25
         WIDTH              = 90
         MAX-HEIGHT         = 27.04
         MAX-WIDTH          = 195.14
...
ON MOUSE-SELECT-CLICK OF bt-importar IN FRAME f-cad
DO:
  RUN esp/ftp/esftp106a.w.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'invwr/v01in106.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01in106 ).
       RUN set-position IN h_v01in106 ( 2.75 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 5.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'inqry/q01in106.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = inzoom\z01es106.w,
                     ProgVaPara = ingo\g01in106.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01in106 ).
       RUN set-position IN h_q01in106 ( 1.25 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01in106. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01in106 ).
       RUN add-link IN adm-broker-hdl ( h_q01in106 , 'Record':U , h_v01in106 ).

       /* Links to SmartQuery h_q01in106. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01in106 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01in106 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01in106 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01in106 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "EMS 2.0.6" "ESFTP106"}
