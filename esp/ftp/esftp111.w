...
*******************************************************************************/
{include/i-prgvrs.i esftp111 2.04.00.000}
...
DEFINE VARIABLE v-periodo-ini  AS CHARACTER NO-UNDO VIEW-AS FILL-IN SIZE 7 BY 0.88 FORMAT "9999/99".
DEFINE VARIABLE v-periodo-fim  AS CHARACTER NO-UNDO VIEW-AS FILL-IN SIZE 7 BY 0.88 FORMAT "9999/99".
...
&Scoped-define FIELDS-IN-QUERY-brMain tt-faturamento.mercado tt-faturamento.periodo tt-faturamento.segmento tt-faturamento.unid-neg tt-faturamento.vl-cart tt-faturamento.vl-fat-orc tt-faturamento.vl-fat-real tt-faturamento.vl-pendente   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brMain   
&Scoped-define SELF-NAME brMain
&Scoped-define OPEN-QUERY-brMain RUN pi-ordena. IF c-ordena-campo <> '' THEN DO:      CASE c-ordena-campo:         WHEN "periodo"      THEN DO:             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.periodo.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.periodo DESC.         END.         WHEN "segmento"     THEN             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.segmento.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.segmento DESC.         WHEN "unid-neg"     THEN             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.unid-neg.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.unid-neg DESC.         WHEN "vl-cart"      THEN             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-cart.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-cart DESC.         WHEN "vl-fat-orc"   THEN             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-fat-orc.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-fat-orc DESC.         WHEN "vl-fat-real"  THEN             IF l-parametro  THEN                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-fat-real.             ELSE                 OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK                     BY tt-faturamento.vl-fat-real DESC.     END CASE.   END. ELSE DO:     OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK         BY tt-faturamento.periodo. END.
&Scoped-define TABLES-IN-QUERY-brMain tt-faturamento
&Scoped-define FIRST-TABLE-IN-QUERY-brMain tt-faturamento
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brMain wMaintenance _FREEFORM
  QUERY brMain NO-LOCK DISPLAY
      tt-faturamento.mercado     COLUMN-LABEL "Mercado"             FORMAT "x(12)":U
      tt-faturamento.periodo     COLUMN-LABEL "Periodo"             FORMAT "9999/99":U 
      tt-faturamento.segmento    COLUMN-LABEL "Segmento"            FORMAT "x(30)":U
      tt-faturamento.unid-neg    COLUMN-LABEL "Unid Neg"            FORMAT "X(20)":U 
      tt-faturamento.vl-cart     COLUMN-LABEL "Carteira"            FORMAT "->>>,>>>,>>9.99":U 
      tt-faturamento.vl-fat-orc  COLUMN-LABEL "Faturamento Or‡ado"  FORMAT "->>>,>>>,>>9.99":U
      tt-faturamento.vl-fat-real COLUMN-LABEL "Faturamento Real"    FORMAT "->>>,>>>,>>9.99":U
      tt-faturamento.vl-pendente COLUMN-LABEL "Valor Pendente"      FORMAT "->>>,>>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 102.43 BY 8.83
         FONT 1 ROW-HEIGHT-CHARS .54 TOOLTIP "Duplo-clique para alterar linha".
...
DEFINE FRAME fpage0
     ...
     btQueryJoins AT ROW 1.13 COL 87 HELP
          "Consultas relacionadas"
     btReportsJoins AT ROW 1.13 COL 91 HELP
          "Relat¢rios relacionados"
     btExit AT ROW 1.13 COL 95 HELP
          "Sair"
     btHelp AT ROW 1.13 COL 99 HELP
          "Ajuda"
     bt-importar AT ROW 1.25 COL 65 WIDGET-ID 10
     bt-layout AT ROW 1.25 COL 75 WIDGET-ID 12
...
ASSIGN 
       brMain:ALLOW-COLUMN-SEARCHING IN FRAME fpage0 = TRUE
...
IF c-ordena-campo <> '' THEN DO:
    ...
    CASE c-ordena-campo:
        WHEN "periodo"      THEN DO:
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.periodo.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.periodo DESC.
        END.
        WHEN "segmento"     THEN
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.segmento.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.segmento DESC.
        WHEN "unid-neg"     THEN
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.unid-neg.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.unid-neg DESC.
        WHEN "vl-cart"      THEN
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-cart.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-cart DESC.
        WHEN "vl-fat-orc"   THEN
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-fat-orc.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-fat-orc DESC.
        WHEN "vl-fat-real"  THEN
            IF l-parametro  THEN
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-fat-real.
            ELSE
                OPEN QUERY {&SELF-NAME} FOR EACH tt-faturamento NO-LOCK
                    BY tt-faturamento.vl-fat-real DESC.
    END CASE.
...
END.
     _END_FREEFORM
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-faturamento.periodo|yes"
     _Query            is OPENED
*/  /* BROWSE brMain */
&ANALYZE-RESUME
...
ON MOUSE-SELECT-CLICK OF brMain IN FRAME fpage0
DO:
...
DO:
    ...
    REPEAT:
        ...
        FIND FIRST faturamento EXCLUSIVE-LOCK
            WHERE faturamento.mercado     =  ENTRY(2, c-linha, ";")
              AND faturamento.periodo     =  ENTRY(3, c-linha, ";")
              AND faturamento.segmento    =  ENTRY(4, c-linha, ";")
              AND faturamento.unid-neg    =  ENTRY(5, c-linha, ";")
              AND faturamento.vl-cart     =  DEC(ENTRY(6, c-linha, ";")) 
              AND faturamento.vl-fat-orc  =  DEC(ENTRY(7, c-linha, ";")) 
              AND faturamento.vl-fat-real =  DEC(ENTRY(8, c-linha, ";")) 
              AND faturamento.vl-pendente =  DEC(ENTRY(9, c-linha, ";")) NO-ERROR.
        ...
        IF c-acao BEGINS "i" THEN DO:
            ...
            IF AVAIL faturamento THEN DO:
                ...
                ASSIGN faturamento.mercado     =  ENTRY(2, c-linha, ";")      
                       faturamento.periodo     =  ENTRY(3, c-linha, ";")           
                       faturamento.segmento    =  ENTRY(4, c-linha, ";")           
                       faturamento.unid-neg    =  ENTRY(5, c-linha, ";")           
                       faturamento.vl-cart     =  DEC(ENTRY(6, c-linha, ";"))           
                       faturamento.vl-fat-orc  =  DEC(ENTRY(7, c-linha, ";"))           
                       faturamento.vl-fat-real =  DEC(ENTRY(8, c-linha, ";"))           
                       faturamento.vl-pendente =  DEC(ENTRY(9, c-linha, ";")).
            END.
...
DO:
    ...
    ASSIGN c-editor = FILL("-",124)                         + CHR(13) +
                      ...
                      "A‡Æo;Mercado;Periodo;Segmento;Unidade de negocio;Vl Cart;Vl.Fatu.Orc;Vl.Fatu.REAL;Vl.Pendente" + CHR(13) + CHR(13) +
                      "i;interno;2011/01;ICOMP;ICO;0,00;0,00;17.804,76;0,00"  + CHR(13) +
                      "e;interno;2011/01;ICOMP;ICO;0,00;0,00;17.804,76;0,00"  + CHR(13).
...
DO:
    ...
    AND BROWSE brMain:NUM-SELECTED-ROWS > 0 THEN DO:
        ...
        RUN esp/ftp/esftp111a.w (INPUT THIS-PROCEDURE, 
                                 INPUT {&hDBOTable}, 
                                 INPUT "update",
                                 INPUT tt-faturamento.r-rowid).
...
DO:
    ...
    AND BROWSE brMain:NUM-SELECTED-ROWS > 0    
        ...
        IF RETURN-VALUE = "yes" THEN DO:
            RUN repositionRecord IN {&hDBOTable} (INPUT tt-faturamento.r-rowid).
...
------------------------------------------------------------------------------*/
    IF NOT VALID-HANDLE({&hDBOTable}) OR
       ...
       {&hDBOTable}:FILE-NAME <> "esbo/boes915.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes915.p YES}
        {btb/btb008za.i2 esbo/boes915.p '' {&hDBOTable}}
    END.
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-ordena wMaintenance 
PROCEDURE pi-ordena :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  IF NOT ERROR-STATUS:ERROR AND
     ...
     h-coluna <> ?          THEN DO:
      ...
      IF  c-ordena-campo = "periodo"      OR 
          c-ordena-campo = "segmento"     OR 
          c-ordena-campo = "unid-neg"     OR 
          c-ordena-campo = "vl-cart"      OR 
          c-ordena-campo = "vl-fat-orc"   OR 
          c-ordena-campo = "vl-fat-real"  
          THEN DO:
          
      END.
      ELSE DO:
          MESSAGE "Nao ‚ poss¡vel ordenar por esta coluna"
...
------------------------------------------------------------------------------*/
    ...
    DEFINE FRAME fBusca
        v-periodo-ini  AT ROW 1.21 COL 15.0 COLON-ALIGNED LABEL "Periodo"
        v-periodo-fim  AT ROW 1.21 COL 36.0 COLON-ALIGNED NO-LABEL
        v-unid-neg-ini AT ROW 2.21 COL 15.0 COLON-ALIGNED LABEL "Unidade de Negocio"
        v-unid-neg-fim AT ROW 2.21 COL 36.0 COLON-ALIGNED NO-LABEL
        v-segmento-ini AT ROW 3.21 COL 15.0 COLON-ALIGNED LABEL "Segmento"
        v-segmento-fim AT ROW 3.21 COL 36.0 COLON-ALIGNED NO-LABEL  
    ...
    ON "CHOOSE":U OF btBuscaOK IN FRAME fBusca DO:
        ASSIGN INPUT FRAME fBusca v-periodo-ini      
               INPUT FRAME fBusca v-periodo-fim
               INPUT FRAME fBusca v-unid-neg-ini      
               INPUT FRAME fBusca v-unid-neg-fim
               INPUT FRAME fBusca v-segmento-ini 
               INPUT FRAME fBusca v-segmento-fim. 

        RUN setConstraintSequencia IN {&hDBOTable} (INPUT v-periodo-ini,
                                                    INPUT v-periodo-fim,
                                                    INPUT v-unid-neg-ini,
        ...
        IF  v-periodo-ini = ""
        AND v-periodo-fim = "ZZZZZZZZZZZZZZZ"
        AND v-unid-neg-ini = "" 
    ...
    ASSIGN  v-periodo-ini:SCREEN-VALUE IN FRAME fBusca   = STRING(v-periodo-ini)        
            v-periodo-fim:SCREEN-VALUE IN FRAME fBusca   = STRING(v-periodo-fim)
            v-unid-neg-ini:SCREEN-VALUE IN FRAME fBusca  = STRING(v-unid-neg-ini)        
