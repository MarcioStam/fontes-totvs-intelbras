...
*******************************************************************************/
{include/i-prgvrs.i ESFTP039 1.00.00.001}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESFTP039 MFP}
&ENDIF
...
DEFINE VARIABLE h_esftp039-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp039-v01 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Usuario Lib Item Fat"
         HEIGHT             = 5.88
         WIDTH              = 90
         MAX-HEIGHT         = 27.5
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Usuario Lib Item Fat */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Usuario Lib Item Fat */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp039-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp039-v01 ).
       RUN set-position IN h_esftp039-v01 ( 2.83 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 3.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp039-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/ftp/esftp039-z01.w,
                     ProgVaPara = esp/ftp/esftp039-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_esftp039-q01 ).
       RUN set-position IN h_esftp039-q01 ( 1.29 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_esftp039-v01. */
       RUN add-link IN adm-broker-hdl ( h_esftp039-q01 , 'Record':U , h_esftp039-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_esftp039-v01 ).

       /* Links to SmartQuery h_esftp039-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esftp039-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esftp039-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esftp039-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp039-v01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFTP039" "1.00.00.001"}
