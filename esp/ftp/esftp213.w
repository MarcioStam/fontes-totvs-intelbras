...
*******************************************************************************/
{include/i-prgvrs.i esftp213 2.00.00.000}
...
DEFINE VARIABLE EDITOR-1 AS CHARACTER INITIAL "Importaá∆o das metas de vendas por canal / item" 
     VIEW-AS EDITOR
     SIZE 58 BY 1.25 NO-UNDO.

DEFINE VARIABLE c-cod-canal-fim AS INTEGER FORMAT ">>9":U INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 NO-UNDO.

DEFINE VARIABLE c-cod-canal-ini AS INTEGER FORMAT ">>9":U INITIAL 0 
     LABEL "Canal" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 NO-UNDO.
...
DEFINE VARIABLE fi-dt-fim-meta AS DATE FORMAT "99/99/9999":U INITIAL 12/31/2999 
     VIEW-AS FILL-IN 
     SIZE 16 BY .88 NO-UNDO.

DEFINE VARIABLE fi-dt-ini-meta AS DATE FORMAT "99/99/9999":U INITIAL 01/01/00 
     LABEL "Data Inicio" 
     VIEW-AS FILL-IN 
     SIZE 16 BY .88 NO-UNDO.
...
DEFINE VARIABLE rs-import-del AS INTEGER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Importar Metas", 1,
"Excluir Metas", 2
...
DEFINE FRAME f-pg-sel
     EDITOR-1 AT ROW 1.5 COL 4 NO-LABEL WIDGET-ID 4
     c-cod-canal-ini AT ROW 3.75 COL 22 COLON-ALIGNED WIDGET-ID 8
     c-cod-canal-fim AT ROW 3.75 COL 47.14 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     c-it-codigo-ini AT ROW 4.75 COL 12.86 COLON-ALIGNED WIDGET-ID 16
     c-it-codigo-fim AT ROW 4.75 COL 47.14 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     fi-dt-ini-meta AT ROW 5.83 COL 12.86 COLON-ALIGNED WIDGET-ID 44
     fi-dt-fim-meta AT ROW 5.83 COL 47.14 COLON-ALIGNED NO-LABEL WIDGET-ID 42
     rs-import-del AT ROW 7.58 COL 22 NO-LABEL WIDGET-ID 30
     bt-arquivo-entrada AT ROW 9.83 COL 43.14 HELP
          "Escolha do nome do arquivo" WIDGET-ID 34
     c-arquivo-entrada AT ROW 9.88 COL 3.29 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL WIDGET-ID 36
     text-entrada AT ROW 8.83 COL 4.14 NO-LABEL WIDGET-ID 40
     "Data Fim:" VIEW-AS TEXT
          SIZE 7 BY .54 AT ROW 6.04 COL 38.86 WIDGET-ID 50
     IMAGE-1 AT ROW 3.75 COL 30.86 WIDGET-ID 10
     IMAGE-2 AT ROW 3.75 COL 46.29 WIDGET-ID 12
     IMAGE-3 AT ROW 4.75 COL 30.86 WIDGET-ID 18
     IMAGE-4 AT ROW 4.75 COL 46.29 WIDGET-ID 20
     RECT-8 AT ROW 9.17 COL 2 WIDGET-ID 38
     IMAGE-5 AT ROW 5.83 COL 30.86 WIDGET-ID 46
     IMAGE-6 AT ROW 5.83 COL 46.29 WIDGET-ID 48
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
DO:
    ...
    ASSIGN cConvFile = REPLACE(INPUT FRAME f-pg-sel c-arquivo-entrada, "/":U, "~\":U).
           
    SYSTEM-DIALOG GET-FILE cConvFile
       FILTERS "*.csv":U "*.csv":U,
               "*.*":U "*.*":U
       &IF 'c-arquivo-entrada' <> 'cInputFile' &THEN
           ASK-OVERWRITE
...
{utp/ut9000.i "esftp213" "2.00.00.000"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    /*
    ASSIGN c-periodo-ini:SCREEN-VALUE IN FRAME f-pg-sel = string(MONTH(TODAY),"99") +  string(YEAR(TODAY))
           c-periodo-fim:SCREEN-VALUE IN FRAME f-pg-sel = string(MONTH(TODAY),"99") +  string(YEAR(TODAY)).*/
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY EDITOR-1 c-cod-canal-ini c-cod-canal-fim c-it-codigo-ini 
          c-it-codigo-fim fi-dt-ini-meta fi-dt-fim-meta rs-import-del 
          c-arquivo-entrada 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 RECT-8 IMAGE-5 IMAGE-6 c-cod-canal-ini 
         c-cod-canal-fim c-it-codigo-ini c-it-codigo-fim fi-dt-ini-meta 
         fi-dt-fim-meta rs-import-del bt-arquivo-entrada c-arquivo-entrada 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF  input frame f-pg-sel rs-import-del = 1
    AND input frame f-pg-sel c-arquivo-entrada = "" THEN DO:
        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Arquivo de importaá∆o n∆o informado.").
        RETURN ERROR.
    END.
    /*Fim teste Modelo*/
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.cod-canal-ini   = input frame f-pg-sel c-cod-canal-ini   
           tt-param.cod-canal-fim   = input frame f-pg-sel c-cod-canal-fim   
           tt-param.it-codigo-ini   = input frame f-pg-sel c-it-codigo-ini   
           tt-param.it-codigo-fim   = input frame f-pg-sel c-it-codigo-fim   
          /* tt-param.periodo-ini     = input frame f-pg-sel c-periodo-ini     
           tt-param.periodo-fim     = input frame f-pg-sel c-periodo-fim*/     
           tt-param.import-del      = input frame f-pg-sel rs-import-del      
           tt-param.arquivo-entrada = input frame f-pg-sel c-arquivo-entrada 
           tt-param.dt-ini-meta     = input frame f-pg-sel fi-dt-ini-meta 
           tt-param.dt-fim-meta     = input frame f-pg-sel fi-dt-fim-meta .
    ...
    {include/i-rprun.i esp/ftp/esftp213rp.p}
