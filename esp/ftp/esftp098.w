...
/*****************************************************************************
** Programa: esp/ftp/esftp098.w
** VersÆo..: 1.00
** Data....: 22/11/2013
** Autor...: Estevan Krger - Sensus
** Obs.....: Cadastro de D‚bitos/Cr‚ditos no sal rio final
*****************************************************************************/
{include/i-prgvrs.i ESFTP098 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          ESFTP098
...
&GLOBAL-DEFINE hDBOParent       h-esbofn017
&GLOBAL-DEFINE DBOParentTable   esbofn017
&GLOBAL-DEFINE DBOParentDestroy YES

&GLOBAL-DEFINE ttSon1           tt-colab-deb-cred-final
&GLOBAL-DEFINE hDBOSon1         h-boes662
&GLOBAL-DEFINE DBOSon1Table     boes662
&GLOBAL-DEFINE DBOSon1Destroy   YES

&GLOBAL-DEFINE page0Fields      tt_usuar_mestre.cod_usuario ~
...
&Scoped-define INTERNAL-TABLES tt-colab-deb-cred-final

/* Definitions for BROWSE brSon1                                        */
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-colab-deb-cred-final.cod-mov ~
fnDescMov(tt-colab-deb-cred-final.cod-mov) @ c-desc-mov ~
...
&Scoped-define QUERY-STRING-brSon1 FOR EACH tt-colab-deb-cred-final NO-LOCK ~
    BY tt-colab-deb-cred-final.cod-colab ~
       BY tt-colab-deb-cred-final.dt-movto ~
        BY tt-colab-deb-cred-final.cod-mov
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY brSon1 FOR EACH tt-colab-deb-cred-final NO-LOCK ~
    BY tt-colab-deb-cred-final.cod-colab ~
       BY tt-colab-deb-cred-final.dt-movto ~
        BY tt-colab-deb-cred-final.cod-mov.
&Scoped-define TABLES-IN-QUERY-brSon1 tt-colab-deb-cred-final
&Scoped-define FIRST-TABLE-IN-QUERY-brSon1 tt-colab-deb-cred-final
...
tt_usuar_mestre.nom_usuario 
&Scoped-define ENABLED-TABLES tt_usuar_mestre
...
tt_usuar_mestre.nom_usuario 
&Scoped-define DISPLAYED-TABLES tt_usuar_mestre
...
DEFINE QUERY brSon1 FOR 
      tt-colab-deb-cred-final SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _STRUCTURED
  QUERY brSon1 NO-LOCK DISPLAY
      tt-colab-deb-cred-final.cod-mov COLUMN-LABEL "Cod" FORMAT ">>9":U
            WIDTH 3.56
      fnDescMov(tt-colab-deb-cred-final.cod-mov) @ c-desc-mov COLUMN-LABEL "Movimento" FORMAT "x(50)":U
            WIDTH 18.56
      tt-colab-deb-cred-final.dt-movto FORMAT "99/99/9999":U WIDTH 10.56
      tt-colab-deb-cred-final.deb-cred COLUMN-LABEL "D/C" FORMAT "D/C":U
      tt-colab-deb-cred-final.valor FORMAT ">>>,>>9.99":U WIDTH 10.67
      tt-colab-deb-cred-final.historico FORMAT "x(250)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage0
     ...
     btHelp AT ROW 1.13 COL 86.89 HELP
          "Ajuda"
     tt_usuar_mestre.cod_usuario AT ROW 3.37 COL 18.33 COLON-ALIGNED WIDGET-ID 8
          LABEL "Colaborador"
          VIEW-AS FILL-IN 
          SIZE 10.45 BY .87
     tt_usuar_mestre.nom_usuario AT ROW 3.37 COL 29.11 COLON-ALIGNED NO-LABEL WIDGET-ID 10
          VIEW-AS FILL-IN 
          SIZE 49.89 BY .87
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wMasterDetail ASSIGN
         HIDDEN             = YES
         TITLE              = "Cadastro D‚bito/Cr‚dito Final"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 31.7
         MAX-WIDTH          = 213.33
...
/* SETTINGS FOR FILL-IN tt_usuar_mestre.cod_usuario IN FRAME fPage0
   EXP-LABEL                                                            */
...
/* Query rebuild information for BROWSE brSon1
     _TblList          = "Temp-Tables.tt-colab-deb-cred-final"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-colab-deb-cred-final.cod-colab|yes,Temp-Tables.tt-colab-deb-cred-final.dt-movto|yes,Temp-Tables.tt-colab-deb-cred-final.cod-mov|yes"
     _FldNameList[1]   > Temp-Tables.tt-colab-deb-cred-final.cod-mov
"tt-colab-deb-cred-final.cod-mov" "Cod" ? "integer" ? ? ? ? ? ? no ? no no "3.56" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > "_<CALC>"
"fnDescMov(tt-colab-deb-cred-final.cod-mov) @ c-desc-mov" "Movimento" "x(50)" ? ? ? ? ? ? ? no ? no no "18.56" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-colab-deb-cred-final.dt-movto
"tt-colab-deb-cred-final.dt-movto" ? ? "date" ? ? ? ? ? ? no ? no no "10.56" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-colab-deb-cred-final.deb-cred
"tt-colab-deb-cred-final.deb-cred" "D/C" "D/C" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-colab-deb-cred-final.valor
"tt-colab-deb-cred-final.valor" ? ? "decimal" ? ? ? ? ? ? no ? no no "10.67" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = Temp-Tables.tt-colab-deb-cred-final.historico
     _Query            is OPENED
*/  /* BROWSE brSon1 */
&ANALYZE-RESUME
...
ON END-ERROR OF wMasterDetail /* Cadastro D‚bito/Cr‚dito Final */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wMasterDetail wMasterDetail
ON WINDOW-CLOSE OF wMasterDetail /* Cadastro D‚bito/Cr‚dito Final */
DO:
...
DO:
    {masterdetail/addson.i &ProgramSon="esp/ftp/esftp098a.w"
                           &PageNumber="1"}
END.
...
DO:
    {masterdetail/copyson.i &ProgramSon="esp/ftp/esftp098a.w"
                            &PageNumber="1"}
END.
...
DO:
    {masterdetail/updateson.i &ProgramSon="esp/ftp/esftp098a.w"
                              &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE c-cod-colab LIKE usuar_mestre.cod_usuario NO-UNDO.
    
    DEFINE FRAME fGoToRecord
        c-cod-colab  AT ROW 1.21 COL 17.72 COLON-ALIGNED
        btGoToOK     AT ROW 2.63 COL 2.14
        btGoToCancel AT ROW 2.63 COL 13
        rtGoToButton AT ROW 2.38 COL 1
        SPACE(0.28)
        WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
             THREE-D SCROLLABLE TITLE "V  Para Colaborador" FONT 1
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToRecord DO:
        ...
        RUN goToKey IN {&hDBOParent} (INPUT c-cod-colab).
        IF RETURN-VALUE = "NOK":U THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U, INPUT 2, INPUT "Colaborador":U).
    ...
    ENABLE c-cod-colab btGoToOK btGoToCancel 
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOParent}) OR
       ...
       {&hDBOParent}:FILE-NAME <> "esbo/esbofn017.p":U THEN DO:
        {btb/btb008za.i1 esbo/esbofn017.p YES}
        {btb/btb008za.i2 esbo/esbofn017.p '' {&hDBOParent}} 
    END.
    
    RUN openQueryStatic IN {&hDBOParent} (INPUT "SalVariavel":U) NO-ERROR.
    
    /*:T--- Verifica se o DBO j  est  inicializado ---*/
    IF NOT VALID-HANDLE({&hDBOSon1}) OR 
       {&hDBOSon1}:TYPE <> "PROCEDURE":U OR
       {&hDBOSon1}:FILE-NAME <> "esbo/boes662.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes662.p YES}
        {btb/btb008za.i2 esbo/boes662.p '' {&hDBOSon1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {masterdetail/openqueriesson.i &Parent="colab"
                                   &Query="colab"
                                   &PageNumber="1"}
