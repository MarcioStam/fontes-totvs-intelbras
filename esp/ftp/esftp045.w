...
*******************************************************************************/
{include/i-prgvrs.i esftp045 2.01.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        esftp045
&GLOBAL-DEFINE Version        2.01.00.001

&GLOBAL-DEFINE Folder         YES
&GLOBAL-DEFINE InitialPage    1

&GLOBAL-DEFINE FolderLabels   Nota Fiscal
...
&GLOBAL-DEFINE page1Fields    c-conhecimento dt-dt-saida /* c-nr-volumes c-peso c-pedido c-cliente c-transportadora*/
...
btLast btGoTo btSearch btUpdate btUndo btCancel btSave btExit BtHelp 
...
DEFINE VARIABLE c-conhecimento AS CHARACTER FORMAT "X(256)":U 
     LABEL "Nro.Conhecimento" 
     VIEW-AS FILL-IN 
     SIZE 17 BY .79 NO-UNDO.

DEFINE VARIABLE c-embarque AS CHARACTER FORMAT "X(256)":U 
     LABEL "Nr.Embarque" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .79 NO-UNDO.

DEFINE VARIABLE c-nr-volumes AS CHARACTER FORMAT "X(256)":U 
     LABEL "Volumes" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .79 NO-UNDO.

DEFINE VARIABLE c-pedido AS CHARACTER FORMAT "X(256)":U 
     LABEL "Pedido" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .79 NO-UNDO.

DEFINE VARIABLE c-peso AS DECIMAL FORMAT "->>,>>9.99":U INITIAL 0 
     LABEL "Peso" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .79 NO-UNDO.
...
DEFINE VARIABLE dt-dt-saida AS DATE FORMAT "99/99/9999":U 
     LABEL "Data Saida Mercadoria" 
     VIEW-AS FILL-IN 
     SIZE 11.72 BY .79 NO-UNDO.
...
DEFINE FRAME fpage0
     ...
     ttint-nota-fiscal.cod-estabel AT ROW 3 COL 30 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 6 BY .79
     ttint-nota-fiscal.serie AT ROW 4 COL 30 COLON-ALIGNED WIDGET-ID 4 FORMAT "X(3)"
          VIEW-AS FILL-IN 
          SIZE 6 BY .79
...
DEFINE FRAME fPage1
     c-cliente AT ROW 1.25 COL 26 COLON-ALIGNED WIDGET-ID 10
     c-pedido AT ROW 2.25 COL 26 COLON-ALIGNED WIDGET-ID 12
     c-transportadora AT ROW 3.25 COL 26 COLON-ALIGNED WIDGET-ID 14
     c-nr-volumes AT ROW 4.25 COL 26 COLON-ALIGNED WIDGET-ID 16
     c-peso AT ROW 5.25 COL 26 COLON-ALIGNED WIDGET-ID 18
     c-embarque AT ROW 6.25 COL 26 COLON-ALIGNED WIDGET-ID 20
     dt-dt-saida AT ROW 7.25 COL 26 COLON-ALIGNED WIDGET-ID 8
     c-conhecimento AT ROW 8.25 COL 26 COLON-ALIGNED WIDGET-ID 6
...
/* SETTINGS FOR FRAME fpage0
   FRAME-NAME                                                           */
/* SETTINGS FOR FILL-IN ttint-nota-fiscal.serie IN FRAME fpage0
   EXP-FORMAT                                                           */
...
------------------------------------------------------------------------------*/
    ...
    find nota-fiscal
     where nota-fiscal.cod-estabel = input frame fPage0 ttint-nota-fiscal.cod-estabel 
       ...
       and nota-fiscal.nr-nota-fis = input frame fPage0 ttint-nota-fiscal.nr-nota-fis no-lock no-error.
    if avail nota-fiscal then do:
        find emitente
             where emitente.nome-abrev = nota-fiscal.nome-ab-cli no-lock no-error.
        assign  dt-dt-saida:screen-value in frame fpage1 = string(nota-fiscal.dt-saida) 
                c-pedido:screen-value in frame fpage1 = nota-fiscal.nr-pedcli
                c-transportadora:screen-value in frame fpage1 = nota-fiscal.nome-transp
                c-peso:screen-value in frame fpage1 = string(nota-fiscal.peso-bru-tot)
                c-nr-volumes:screen-value in frame fpage1 = nota-fiscal.nr-volume  
                c-cliente:screen-value in frame fpage1 = emitente.nome-emit
                c-embarque:screen-value in frame fpage1 = string(nota-fiscal.cdd-embarq).
        

    end.
    assign c-conhecimento:screen-value in frame fpage1 = substring(ttint-nota-fiscal.char-1,1,20).
END PROCEDURE.
...
------------------------------------------------------------------------------*/
    FIND FIRST nota-fiscal
         where nota-fiscal.cod-estabel = input frame fPage0 ttint-nota-fiscal.cod-estabel 
           and nota-fiscal.serie       = input frame fPage0 ttint-nota-fiscal.serie 
           and nota-fiscal.nr-nota-fis = input frame fPage0 ttint-nota-fiscal.nr-nota-fis EXCLUSIVE-LOCK NO-ERROR.
    assign overlay(ttint-nota-fiscal.char-1,1,20)     = input frame fPage1 c-conhecimento
           nota-fiscal.dt-saida                     = date(input frame fPage1 dt-dt-saida).
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToRecord DO:
        ...
        IF RETURN-VALUE = "NOK":U THEN DO:
            ...
            if avail nota-fiscal then do:
               ...
               RUN utp/ut-msgs.p (INPUT "SHOW":U, INPUT 2, INPUT "int-nota-fiscal":U).            
               RETURN NO-APPLY.
            end.
        END.
