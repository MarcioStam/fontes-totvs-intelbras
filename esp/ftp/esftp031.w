...
*******************************************************************************/
{include/i-prgvrs.i ESFTP031 1.00.00.001}
...
DEFINE VARIABLE h_esftp031-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp031-b02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp031-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp031-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 156.57 BY 23.42 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Solicita‡Æo - Item Fatur vel"
         HEIGHT             = 23.42
         WIDTH              = 156.57
         MAX-HEIGHT         = 27.5
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Solicita‡Æo - Item Fatur vel */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Solicita‡Æo - Item Fatur vel */
DO:
...
{src/adm/template/windowmn.i}
    ...
    RUN enable-Cancela        in h_p-cadsim (Input NO).
    RUN enable-Desfaz         in h_p-cadsim (Input NO).
    RUN enable-Salva          in h_p-cadsim (Input NO).
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 140.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp031-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp031-v01 ).
       RUN set-position IN h_esftp031-v01 ( 2.83 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 6.71 , 155.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp031-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp031-b01 ).
       RUN set-position IN h_esftp031-b01 ( 9.75 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 14.54 , 72.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp031-b02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp031-b02 ).
       RUN set-position IN h_esftp031-b02 ( 9.75 , 75.29 ) NO-ERROR.
       /* Size in UIB:  ( 14.54 , 81.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp031-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/ftp/esftp031-z01.w,
                     ProgVaPara = esp/ftp/esftp031-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_esftp031-q01 ).
       RUN set-position IN h_esftp031-q01 ( 1.25 , 126.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_esftp031-v01. */
       RUN add-link IN adm-broker-hdl ( h_esftp031-q01 , 'Record':U , h_esftp031-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_esftp031-v01 ).

       /* Links to SmartBrowser h_esftp031-b01. */
       RUN add-link IN adm-broker-hdl ( h_esftp031-v01 , 'Record':U , h_esftp031-b01 ).

       /* Links to SmartBrowser h_esftp031-b02. */
       RUN add-link IN adm-broker-hdl ( h_esftp031-v01 , 'Record':U , h_esftp031-b02 ).

       /* Links to SmartQuery h_esftp031-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esftp031-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esftp031-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esftp031-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp031-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp031-b01 ,
             h_esftp031-v01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp031-b02 ,
             h_esftp031-b01 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFTP031" "1.00.00.001"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN h-browserCamposLib = h_esftp031-b01
         h-browserEstabsLib = h_esftp031-b02
         h-ViewerEstabLib   = h_esftp031-v01.


  run pi-after-initialize.

  RUN pi-alteracoluna IN h-browserEstabsLib.
