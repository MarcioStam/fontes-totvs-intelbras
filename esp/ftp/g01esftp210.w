...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-emitente d_dt-simula ~
i_nr-sequencia bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-emitente d_dt-simula i_nr-sequencia 
...
DEFINE VARIABLE d_dt-simula AS DATE FORMAT "99/99/9999":U 
...
DEFINE VARIABLE i_nr-sequencia AS INTEGER FORMAT "99999":U 
     LABEL "Sequencia" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.
...
DEFINE FRAME d-vapara
     i_cod-emitente AT ROW 1 COL 30 COLON-ALIGNED WIDGET-ID 2
     d_dt-simula AT ROW 2 COL 30 COLON-ALIGNED WIDGET-ID 4
     i_nr-sequencia AT ROW 3 COL 30 COLON-ALIGNED WIDGET-ID 6
...
DO:
FIND int-simula-dev NO-LOCK
    WHERE int-simula-dev.cod-emitente EQ INPUT FRAME {&FRAME-NAME} i_cod-emitente
      AND int-simula-dev.dt-simula EQ INPUT FRAME {&FRAME-NAME} d_dt-simula
      AND int-simula-dev.nr-sequencia EQ INPUT FRAME {&FRAME-NAME} i_nr-sequencia NO-ERROR.

IF NOT AVAILABLE int-simula-dev THEN DO:
    {utp/ut-table.i mgesp int-simula-dev 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-simula-dev).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-emitente d_dt-simula i_nr-sequencia 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-emitente d_dt-simula i_nr-sequencia bt-ok bt-cancela 
         bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ESFTP210" "2.00.00.000"}
