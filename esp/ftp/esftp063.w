...
*******************************************************************************/
{include/i-prgvrs.i ESPFT063 2.04.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESFTP063
...
&GLOBAL-DEFINE page2Fields    fi-estab-ini fi-estab-fim fi-cod-repres-ini fi-cod-repres-fim fi-periodo rs-previa-oficial
...
DEFINE VARIABLE fi-periodo AS CHARACTER FORMAT "X(256)":U INITIAL "200901" 
     LABEL "Periodo" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE rs-previa-oficial AS INTEGER 
     ...
     RADIO-BUTTONS 
          "Pr‚via", 1,
"Oficial - Gera Integra‡Æo com RH", 2
...
DEFINE VARIABLE cFile AS CHARACTER INITIAL "c:~\temp~\comissoes~\esftp063.lst" 
...
DEFINE FRAME fPage2
     fi-estab-ini AT ROW 2.42 COL 22 COLON-ALIGNED WIDGET-ID 2
     fi-estab-fim AT ROW 2.42 COL 46 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     fi-cod-repres-ini AT ROW 3.5 COL 22 COLON-ALIGNED
     fi-cod-repres-fim AT ROW 3.5 COL 46 COLON-ALIGNED NO-LABEL
     fi-periodo AT ROW 4.5 COL 22 COLON-ALIGNED WIDGET-ID 52
     rs-previa-oficial AT ROW 7.75 COL 20 NO-LABEL WIDGET-ID 66
     IMAGE-3 AT ROW 3.5 COL 43 WIDGET-ID 8
     IMAGE-4 AT ROW 3.5 COL 39 WIDGET-ID 10
     IMAGE-15 AT ROW 2.25 COL 43 WIDGET-ID 46
     IMAGE-16 AT ROW 2.25 COL 39 WIDGET-ID 48
...
DO ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    DO WITH FRAME fPage2:
        assign tt-param.fi-estab-ini            = string(INPUT fi-estab-ini:SCREEN-VALUE IN FRAME fpage2)
               ...
               tt-param.fi-periodo              = INPUT fi-periodo:SCREEN-VALUE IN FRAME fpage2
               tt-param.rs-previa-oficial       = int(INPUT rs-previa-oficial:SCREEN-VALUE IN FRAME fpage2).
              
    END.

/*     find first comissao-fat                                                                                                     */
/*          where comissao-fat.periodo = string(year(tt-param.fi-dt-emissao-ini)) + string(month(tt-param.fi-dt-emissao-ini),"99") */
/*            and comissao-fat.cod-estabel >= tt-param.fi-estab-ini                                                                */
/*            and comissao-fat.cod-estabel <= tt-param.fi-estab-fim                                                                */
/*          no-lock no-error.                                                                                                      */
/*     if avail comissao-fat and                                                                                                   */
/*        tt-param.rs-previa-oficial = 2 then do:                                                                                  */
/*           message "Periodo ja finalizado, nÆo ‚ permitido executar com op‡Æo Oficial":U view-as alert-box.                      */
/*                                                                                                                                 */
/*          RETURN error.                                                                                                          */
/*     end.                                                                                                                        */
    ...
    {report/rprun.i esp\ftp\esftp063rp.p}
