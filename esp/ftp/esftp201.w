...
&Scoped-Define ENABLED-OBJECTS rt-button BUTTON-2 
...
DEFINE VARIABLE h_esftp201-b01-3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp201-q01-3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esftp201-v01 AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-2 
     IMAGE-UP FILE "adeicon/fields.bmp":U
     LABEL "Button 2" 
     SIZE 5 BY 1.25.
...
DEFINE FRAME f-cad
     BUTTON-2 AT ROW 1.17 COL 65.86 WIDGET-ID 2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-2 w-cadpaifilho-ambos
ON CHOOSE OF BUTTON-2 IN FRAME f-cad /* Button 2 */
DO:
    RUN esp/ftp/esftp202.w.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp201-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esftp201-v01 ).
       RUN set-position IN h_esftp201-v01 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Metas' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp201-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/ftp/z01esftp201.w,
                     ProgVaPara = esp/ftp/g01esftp201.w,
                     ProgIncMod = esp/ftp/esftp201a.w,
                     Implantar = no':U ,
             OUTPUT h_esftp201-q01-3 ).
       RUN set-position IN h_esftp201-q01-3 ( 1.25 , 53.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.75 , 7.00 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_esftp201-v01. */
       RUN add-link IN adm-broker-hdl ( h_esftp201-q01-3 , 'Record':U , h_esftp201-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_esftp201-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_esftp201-q01-3. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_esftp201-q01-3 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esftp201-q01-3 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esftp201-q01-3 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esftp201-q01-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             BUTTON-2:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadpai ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             BUTTON-2:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp201-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_esftp201-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ftp/esftp201-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/ftp/esftp201b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_esftp201-b01-3 ).
       RUN set-position IN h_esftp201-b01-3 ( 9.25 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 84.00 ) */

       /* Links to BrowserCadastro2 h_esftp201-b01-3. */
       RUN add-link IN adm-broker-hdl ( h_esftp201-q01-3 , 'Record':U , h_esftp201-b01-3 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'State':U , h_esftp201-b01-3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esftp201-b01-3 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esftp201" "2.00.00.000"}
  
  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  RUN New-State('initialize':U).
  /* Code placed here will execute AFTER standard behavior.    */
