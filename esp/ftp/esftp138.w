...
*******************************************************************************/
{include/i-prgvrs.i esftp138 2.06.00.001}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i esftp138 mft}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        esftp138
...
&GLOBAL-DEFINE page2Fields    dt-emis-ini dt-emis-fim c-serie-ini c-serie-fim c-nota-ini c-nota-fim ~
                              c-estab-ini c-estab-fim c-cod-depos
...
&Scoped-define FIELDS-IN-QUERY-brDigita tt-digita.deposito   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brDigita tt-digita.deposito   
...
DEFINE VARIABLE c-cod-depos AS CHARACTER FORMAT "X(5)":U INITIAL "WEX" 
     LABEL "Deposito" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE c-nota-fim AS CHARACTER FORMAT "X(16)":U INITIAL "ZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 18 BY .88 NO-UNDO.

DEFINE VARIABLE c-nota-ini AS CHARACTER FORMAT "X(16)":U 
     LABEL "Nota Fiscal" 
     VIEW-AS FILL-IN 
     SIZE 18 BY .88 NO-UNDO.
...
DEFINE VARIABLE dt-emis-ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/01 
     LABEL "Dt.EmissÆo" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88
     FONT 1 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brDigita wReport _FREEFORM
  QUERY brDigita DISPLAY
      tt-digita.deposito COLUMN-LABEL "Deposito"
ENABLE
tt-digita.deposito
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage2
     dt-emis-ini AT ROW 1.5 COL 12.57
     dt-emis-fim AT ROW 1.5 COL 54.14 NO-LABEL
     c-estab-ini AT ROW 2.5 COL 19 COLON-ALIGNED WIDGET-ID 2
     c-estab-fim AT ROW 2.5 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     c-serie-ini AT ROW 3.5 COL 19 COLON-ALIGNED WIDGET-ID 8
     c-serie-fim AT ROW 3.5 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     c-nota-ini AT ROW 4.5 COL 19 COLON-ALIGNED WIDGET-ID 12
     c-nota-fim AT ROW 4.5 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     c-cod-depos AT ROW 5.5 COL 19 COLON-ALIGNED WIDGET-ID 26
     IMAGE-1 AT ROW 1.5 COL 39.86
     IMAGE-2 AT ROW 1.5 COL 51.14
     IMAGE-27 AT ROW 4.54 COL 39.86 WIDGET-ID 14
     IMAGE-28 AT ROW 4.54 COL 51.14 WIDGET-ID 16
     IMAGE-29 AT ROW 3.54 COL 39.86 WIDGET-ID 18
     IMAGE-30 AT ROW 3.54 COL 51.14 WIDGET-ID 20
     IMAGE-31 AT ROW 2.54 COL 39.86 WIDGET-ID 22
     IMAGE-32 AT ROW 2.54 COL 51.14 WIDGET-ID 24
...
/* SETTINGS FOR FRAME fPage2
                                                                        */
/* SETTINGS FOR FILL-IN dt-emis-fim IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN dt-emis-ini IN FRAME fPage2
   ALIGN-L                                                              */
...
DO:
    if  brDigita:new-row in frame fPage5 then do:
        ...
        display tt-digita.deposito with browse brDigita. 
    end.
    return no-apply.
END.
...
DO:
    ...
    do transaction on error undo, return no-apply:
        create tt-digita.
        assign input browse brDigita tt-digita.deposito.
...
DO:
    ...
    if num-results("brDigita":U) > 0 then
        ...
        apply "entry":U to tt-digita.deposito in browse brDigita. 
    end.
END.
...
DO:
   apply 'entry' to tt-digita.deposito in browse brDigita. 
END.
...
/*CREATE tt-digita.
ASSIGN tt-digita.deposito = "WEX".

{&OPEN-QUERY-brDigita}*/

ASSIGN dt-emis-ini:SCREEN-VALUE IN FRAME fpage2 = string("10/02/2023")
       dt-emis-fim:SCREEN-VALUE IN FRAME fpage2 = string(TODAY).
...
do on error undo, return error on stop  undo, return error:
    ...
    for each tt-digita no-lock:
        ...
        find first b-tt-digita 
            where b-tt-digita.deposito = tt-digita.deposito
        ...
        if  tt-digita.deposito = "" then do:
            assign browse brDigita:CURRENT-COLUMN = tt-digita.deposito:HANDLE in browse brDigita.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           &endif
           tt-param.dt-emis-ini     = INPUT FRAME fpage2 dt-emis-ini
           tt-param.dt-emis-fim     = INPUT FRAME fpage2 dt-emis-fim
           ...
           tt-param.c-nota-ini      = INPUT FRAME fpage2 c-nota-ini
           tt-param.c-nota-fim      = INPUT FRAME fpage2 c-nota-fim
           tt-param.c-cod-depos     = INPUT FRAME fpage2 c-cod-depos
           .
    ...
    {report/rprun.i esp/ftp/esftp138rp.p}
