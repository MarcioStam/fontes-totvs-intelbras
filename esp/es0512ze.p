...
def temp-table tt_unid-negoc 
    field cod-unid-negoc as char format "x(08)" label "Centro Custo" column-label "Centro Custo"
    field descricao as char format "x(32)" label "Descriá∆o" column-label "Descriá∆o"
    index tt_id is primary unique
          cod-unid-negoc ascending.
...
DEF BUFFER b_ccusto_enter          FOR emscad.ccusto.

def query qr_tt_unid-negoc
    for tt_unid-negoc
    scrolling.
...
def browse br_tt_unid-negoc query qr_tt_unid-negoc display 
    tt_unid-negoc.cod-unid-negoc
    width-chars 10.00
        column-label "Unidade"
    tt_unid-negoc.descricao
    width-chars 25.00
...
def browse br_usu-cc-un-orc query qr_usu-cc-un-orc display 
    tt-usu-cc-un-orc.cod-unid-negoc
    width-chars 10.00
        column-label "Unidade"
    tt_unid-negoc.descricao
    width-chars 25.00
...
DEF VAR v_cod-ccusto  LIKE emscad.ccusto.cod_ccusto         NO-UNDO.

def frame f_dlg_03_atualizar_grupos_seguranca
    rt_cxcf
         at row 12.17 col 02.00 bgcolor 7 
    v_cod-estab
         at row 01.10 col 18.00 colon-aligned label "Estabelecimento"
         view-as fill-in
         size-chars 04.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_30682
         at row 01.10 col 24.14
    estabelecimento.nom_abrev
         at row 01.10 col 28.57 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_cod-ccusto
         at row 02.10 col 18.00 colon-aligned label "Centro Custo"
         view-as fill-in
         size-chars 9.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_30686
         at row 02.10 col 29.14
    emscad.ccusto.des_tit_ctbl
         at row 02.10 col 33.57 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_cod-usuario
         at row 03.10 col 18.00 colon-aligned label "Usu†rio"
         view-as fill-in
         size-chars 13.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_30681
         at row 03.10 col 33.14
    bt_ent_30681
         at row 03.10 col 37.14
    usuar_mestre.nom_usuario
         at row 03.10 col 41.57 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    br_tt_unid-negoc
         at row 04.00 col 02.00
         help "CCustos Dispon°veis"
    br_usu-cc-un-orc
         at row 04.00 col 49.00
    ...
    with 1 down side-labels no-validate keep-tab-order three-d
         ...
         title "Relaciona Usu†rio a Unidade - ES0512ZE - 1.00.00.001".
    ...
    assign br_usu-cc-un-orc:private-data in frame f_dlg_03_atualizar_grupos_seguranca         = "HLP=000000000":U
           br_tt_unid-negoc:private-data    in frame f_dlg_03_atualizar_grupos_seguranca     = "HLP=000000000":U
...
ON CHOOSE OF bt_ent_30681 IN FRAME f_dlg_03_atualizar_grupos_seguranca DO:
    ...
    if not avail b_usuar_mestre_enter then do:
    ...
    if not avail b_estabelecimento_enter then do:
    ...
    find b_ccusto_enter 
        where b_ccusto_enter.cod_empresa =  b_estabelecimento_enter.cod_empres
        AND   b_ccusto_enter.cod_ccusto  = INPUT frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto no-lock no-error.

    if not avail b_ccusto_enter then do:
       message substitute("&1 inexistente." ,"Centro de Custos")
            view-as alert-box warning buttons ok.
       return no-apply.
    end.
...
ON CHOOSE OF bt_ins_frm IN FRAME f_dlg_03_atualizar_grupos_seguranca DO:
    ...
    retira_block:
    do v_num_row_a = 1 to browse br_tt_unid-negoc:num-selected-rows:

        assign v_log_method = browse br_tt_unid-negoc:fetch-selected-row(v_num_row_a).
        assign v_rec_table  = recid(tt_unid-negoc).

        find tt_unid-negoc where recid(tt_unid-negoc) = v_rec_table no-lock no-error.
        ...
        find usu-cc-un-orc no-lock
             where usu-cc-un-orc.cod-empresa    = estabelecimento.cod_empr
               AND usu-cc-un-orc.cod-estab      = estabelecimento.cod_estab
               AND usu-cc-un-orc.cod-ccusto     = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto
               AND usu-cc-un-orc.cod-usuario    = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-usuario
               and usu-cc-un-orc.cod-unid-negoc = tt_unid-negoc.cod-unid-negoc 
            no-error.

        if  not avail usu-cc-un-orc then do:
            create usu-cc-un-orc.
            assign usu-cc-un-orc.cod-empresa    = estabelecimento.cod_empr                                     
                   usu-cc-un-orc.cod-estab      = estabelecimento.cod_estab                                    
                   usu-cc-un-orc.cod-ccusto     = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto 
                   usu-cc-un-orc.cod-usuario    = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-usuario
                   usu-cc-un-orc.cod-unid-negoc = tt_unid-negoc.cod-unid-negoc                               .
        end.
...
DO:
    ...
    do v_num_row_a = 1 to browse br_usu-cc-un-orc:num-selected-rows:
        ...
        FIND usu-cc-un-orc OF tt-usu-cc-un-orc EXCLUSIVE-LOCK NO-ERROR.
        if  avail usu-cc-un-orc then do:
...
ON  CHOOSE OF bt_zoo_30686 IN FRAME f_dlg_03_atualizar_grupos_seguranca
...
ON LEAVE OF v_cod-ccusto IN FRAME f_dlg_03_atualizar_grupos_seguranca DO:

    find estabelecimento no-lock
         where estabelecimento.cod_estab = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-estab no-error.

    find emscad.ccusto no-lock
         where emscad.ccusto.cod_empresa      = estabelecimento.cod_empresa
           and emscad.ccusto.cod_plano_ccusto = 'Padrao'
           and emscad.ccusto.cod_ccusto       = INPUT FRAME f_dlg_03_atualizar_grupos_seguranca  v_cod-ccusto no-error.
...
run pi_cria_tt_unid-negoc.

open query qr_tt_unid-negoc for
   each tt_unid-negoc no-lock.

disable usuar_mestre.nom_usuario estabelecimento.nom_abrev bt_ins_frm bt_outdent ccusto.des_tit_ctbl with frame f_dlg_03_atualizar_grupos_seguranca.
...
PROCEDURE pi_open_atualizar_grupos_seguranca:
        ...
        FOR EACH usu-cc-un-orc NO-LOCK
            where usu-cc-un-orc.cod-empresa = estabelecimento.cod_empresa
            AND   usu-cc-un-orc.cod-estab   = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-estab
            AND   usu-cc-un-orc.cod-usuario = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-usuario
            AND   usu-cc-un-orc.cod-ccusto  = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto:
...
END PROCEDURE.
/*****************************************************************************
** Procedure Interna.....: pi_cria_tt_unid-negoc
** Descricao.............: pi_cria_tt_unid-negoc
*****************************************************************************/
PROCEDURE pi_cria_tt_unid-negoc:

    FOR EACH unid_negoc NO-LOCK:
        create tt_unid-negoc.
        assign tt_unid-negoc.cod-unid-negoc = unid_negoc.cod_unid_negoc
               tt_unid-negoc.descricao      = unid_negoc.des_unid_negoc.
    END.        
    create tt_unid-negoc.
    assign tt_unid-negoc.cod-unid-negoc = "*"
           tt_unid-negoc.descricao      = "Todos".
