...
def var v_rec_table
    ...
    initial ?
    no-undo.
def var v_cod-ccusto     like usu-cc-un-orc.cod-ccusto no-undo.
DEF VAR v_cod-unid-negoc LIKE usu-cc-un-orc.cod-unid-negoc NO-UNDO.
...
DEF TEMP-TABLE tt-usu-cc-un-orc LIKE usu-cc-un-orc
    FIELD nom_usuar LIKE usuar_mestre.nom_usuar.

DEF TEMP-TABLE tt-usuar_mestre LIKE usuar_mestre.
...
def browse br_usu-cc-un-orc query qr_usu-cc-un-orc display 
    tt-usu-cc-un-orc.cod-usuario
    width-chars 10.00
        column-label "Usu†rio"
    tt-usu-cc-un-orc.nom_usuar
    width-chars 25.00
    with separators multiple 
         size 39.00 by 08.00
         font 1
...
def frame f_dlg_03_atualizar_grupos_seguranca
    ...
    estabelecimento.nom_abrev
         ...
         fgcolor ? bgcolor 15 font 2
    v_cod-unid-negoc
         at row 03.67 col 18.00 colon-aligned label "UN"
         view-as fill-in
    ...
    unid_negoc.des_unid_negoc
         ...
         fgcolor ? bgcolor 15 font 2
    v_cod-ccusto
         at row 04.67 col 18.00 colon-aligned label "Centro Custo"
         view-as fill-in
         size-chars 9.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_30681
         at row 04.67 col 29.14
    bt_ent_30681
    ...
    emscad.ccusto.des_tit_ctbl
         ...
         fgcolor ? bgcolor 15 font 2
    br_usuar_mestre
         at row 06.00 col 02.00
         help "Usu†rios Dispon°veis"
    br_usu-cc-un-orc
         at row 06.00 col 49.00
    ...
    with 1 down side-labels no-validate keep-tab-order three-d
         ...
         title "Relaciona CCusto ao Usu†rio - ES0512ZC - 1.00.00.001".
...
ON CHOOSE OF bt_ins_frm IN FRAME f_dlg_03_atualizar_grupos_seguranca DO:
    ...
    do v_num_row_a = 1 to browse br_usuar_mestre:num-selected-rows:
        ...
        find tt-usuar_mestre where recid(tt-usuar_mestre) = v_rec_table no-lock no-error.

        find usu-cc-un-orc no-lock
             where usu-cc-un-orc.cod-empresa  = emscad.empresa.cod_empresa
               and usu-cc-un-orc.cod-estab    = estabelecimento.cod_estab
               and usu-cc-un-orc.cod-usuario  = tt-usuar_mestre.cod_usuar
               and usu-cc-un-orc.cod-ccusto   = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto 
               and usu-cc-un-orc.cod-unid-neg = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-unid-negoc no-error.
        if not avail usu-cc-un-orc then do:
            create usu-cc-un-orc.
            assign usu-cc-un-orc.cod-empresa = emscad.empresa.cod_empresa
                   usu-cc-un-orc.cod-estab   = estabelecimento.cod_estab
                   usu-cc-un-orc.cod-usuario = tt-usuar_mestre.cod_usuar
...
ON  CHOOSE OF bt_zoo_30685 IN FRAME f_dlg_03_atualizar_grupos_seguranca
...
OR ENTER OF v_cod-ccusto IN FRAME f_dlg_03_atualizar_grupos_seguranca DO:
    ...
    find unid_negoc no-lock
         where unid_negoc.cod_unid_negoc = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-unid-negoc no-error.
...
DO:
    ...
    find emscad.ccusto no-lock
        where emscad.ccusto.cod_empresa      = input frame f_dlg_03_atualizar_grupos_seguranca v_cod_empresa
          and emscad.ccusto.cod_plano_ccusto = 'Padrao'
          and emscad.ccusto.cod_ccusto       = frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto no-error.
    display emscad.ccusto.des_tit_ctbl when avail emscad.ccusto
            "" when not avail emscad.ccusto @ emscad.ccusto.des_tit_ctbl
            with frame f_dlg_03_atualizar_grupos_seguranca.
...
disable emscad.ccusto.des_tit_ctbl
        emscad.empresa.nom_razao_social 
        unid_negoc.des_unid_neg
        estabelecimento.nom_abrev 
        bt_ins_frm 
        bt_outdent 
        with frame f_dlg_03_atualizar_grupos_seguranca.

RUN pi_open_atualizar_usuar.
...
PROCEDURE pi_open_atualizar_usuar:           

    CLOSE QUERY qr_usuar_mestre.
    EMPTY TEMP-TABLE tt-usuar_mestre.

    FOR EACH usuar_mestre NO-LOCK:

        IF  usuar_mestre.dat_fim_valid  < TODAY
        OR  usuar_mestre.dat_inic_valid > TODAY THEN
            NEXT.
    ...
    END.
    CREATE tt-usuar_mestre.
    ASSIGN tt-usuar_mestre.cod_usuar = "*"
           tt-usuar_mestre.nom_usuar = "Todos"
           tt-usuar_mestre.idi_dtsul = NEXT-VALUE(seq_ped_exec).

    OPEN query qr_usuar_mestre FOR
        EACH tt-usuar_mestre NO-LOCK
        BY cod_usuar.
END.
...
PROCEDURE pi_open_atualizar_grupos_seguranca:
        ...
        FOR EACH usu-cc-un-orc NO-LOCK
            WHERE usu-cc-un-orc.cod-empresa  = input frame f_dlg_03_atualizar_grupos_seguranca v_cod_empresa
              AND usu-cc-un-orc.cod-estab    = input frame f_dlg_03_atualizar_grupos_seguranca v_cod_estab
              AND usu-cc-un-orc.cod-ccusto   = input frame f_dlg_03_atualizar_grupos_seguranca v_cod-ccusto
            ...
            IF tt-usu-cc-un-orc.cod-usuario <> "*" THEN DO:
                FIND first usuar_mestre 
                    where usuar_mestre.cod_usuario = tt-usu-cc-un-orc.cod-usuario
                    NO-LOCK NO-ERROR.
    
                IF  AVAIL usuar_mestre THEN DO:
                    ASSIGN tt-usu-cc-un-orc.nom_usuar = usuar_mestre.nom_usuar.
                END.
            END.
            ELSE DO:
                ASSIGN tt-usu-cc-un-orc.nom_usuar = "Todos".
            END.
        END.
        
        open query qr_usu-cc-un-orc for
            each tt-usu-cc-un-orc no-lock
            by tt-usu-cc-un-orc.cod-usuario.
