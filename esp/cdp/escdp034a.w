...
*******************************************************************************/
{include/i-prgvrs.i ESCDP034A 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCDP034A
...
c-ds-categoria c-fm-cod-com c-ds-fm-com dt-vigencia-ini dt-vigencia-fim ~
de-pc-desconto btOK btSalvar btCancel 
...
c-ds-fm-com dt-vigencia-ini dt-vigencia-fim de-pc-desconto 
...
DEFINE FRAME fPage0
     ...
     c-ds-fm-com AT ROW 4.88 COL 28.14 COLON-ALIGNED NO-LABEL WIDGET-ID 52
     ...
     de-pc-desconto AT ROW 6.88 COL 18 COLON-ALIGNED HELP
          "Percentual de Desconto" WIDGET-ID 42
     btOK AT ROW 8.38 COL 2
     btSalvar AT ROW 8.38 COL 12.43 WIDGET-ID 40
     btCancel AT ROW 8.38 COL 22.86
     rtToolBar AT ROW 8.17 COL 1
     RECT-1 AT ROW 1.17 COL 1.43 WIDGET-ID 26
     RECT-5 AT ROW 2.67 COL 1.43 WIDGET-ID 32
     IMAGE-1 AT ROW 5.92 COL 31.86 WIDGET-ID 48
     IMAGE-2 AT ROW 5.92 COL 38.14 WIDGET-ID 50
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWindow ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Desconto para Pedido Programado"
         HEIGHT             = 8.58
         WIDTH              = 90
         MAX-HEIGHT         = 28.33
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF wWindow /* Inclui/Modifica Desconto para Pedido Programado */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWindow wWindow
ON WINDOW-CLOSE OF wWindow /* Inclui/Modifica Desconto para Pedido Programado */
DO:
    ...
    CASE pType:
        ...
        WHEN "Copy":U   THEN DO:
            FIND FIRST crm-desc-ped-prog NO-LOCK
                WHERE  ROWID(crm-desc-ped-prog) = pRow-table NO-ERROR.
            IF  AVAIL  crm-desc-ped-prog THEN
                ASSIGN i-cod-gr-cli    = crm-desc-ped-prog.cod-gr-cli
                       i-cd-categoria  = crm-desc-ped-prog.cd-categoria
                       c-fm-cod-com    = crm-desc-ped-prog.fm-cod-com
                       dt-vigencia-ini = crm-desc-ped-prog.dt-vigencia-ini
                       dt-vigencia-fim = crm-desc-ped-prog.dt-vigencia-fim
                       de-pc-desconto  = crm-desc-ped-prog.pc-desconto.
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
        WHEN "Create":U OR
        WHEN "Copy":U   THEN DO:
            CREATE crm-desc-ped-prog.
            ASSIGN crm-desc-ped-prog.cd-unid-negoc   = STRING(i-cd-unid-comerc)
                   crm-desc-ped-prog.cod-gr-cli      = i-cod-gr-cli
                   crm-desc-ped-prog.cd-categoria    = i-cd-categoria
                   crm-desc-ped-prog.fm-cod-com      = c-fm-cod-com
                   crm-desc-ped-prog.dt-vigencia-ini = dt-vigencia-ini.
        END.
        WHEN "Update":U THEN DO:
            FIND FIRST crm-desc-ped-prog EXCLUSIVE-LOCK
                WHERE  ROWID(crm-desc-ped-prog) = pRow-table NO-ERROR.
            IF  NOT AVAIL crm-desc-ped-prog THEN
                RETURN "NOK":U.
        END.
        OTHERWISE .
    END CASE.

    ASSIGN crm-desc-ped-prog.dt-vigencia-fim = dt-vigencia-fim
           crm-desc-ped-prog.pc-desconto     = de-pc-desconto.

    /* Grava a data inicial na data final da £ltima vigˆncia da UN x Cliente x Tab Pre‡o */
    FIND LAST crm-desc-ped-prog EXCLUSIVE-LOCK
        WHERE crm-desc-ped-prog.cd-unid-negoc   = STRING(i-cd-unid-comerc)
        AND   crm-desc-ped-prog.cod-gr-cli      = i-cod-gr-cli
        AND   crm-desc-ped-prog.cd-categoria    = i-cd-categoria
        AND   crm-desc-ped-prog.fm-cod-com      = c-fm-cod-com
        AND   crm-desc-ped-prog.dt-vigencia-ini < dt-vigencia-ini NO-ERROR.
    IF  AVAIL crm-desc-ped-prog AND crm-desc-ped-prog.dt-vigencia-fim = ? THEN
        ASSIGN crm-desc-ped-prog.dt-vigencia-fim = dt-vigencia-ini.
...
------------------------------------------------------------------------------*/
    ...
    IF  pType = "Create":U OR
        ...
        IF  CAN-FIND(FIRST crm-desc-ped-prog NO-LOCK
                     ...
                     AND   crm-desc-ped-prog.cd-categoria    = i-cd-categoria
                     AND   crm-desc-ped-prog.fm-cod-com      = c-fm-cod-com
                     AND   crm-desc-ped-prog.dt-vigencia-ini = dt-vigencia-ini) THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U,
                               INPUT 7,
                               INPUT "Desconto para Pedido Programado":U).
