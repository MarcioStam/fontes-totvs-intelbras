...
*******************************************************************************/
{include/i-prgvrs.i ESCDP122 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP122 ESP}
&ENDIF
...
/* ***************************  Definitions  ************************** */
def var lg-inc as logi no-undo.
def var lg-del as logi no-undo.
...
DEFINE VARIABLE h_escdp122-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp122-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp122-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 90 BY 18.21 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadco2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Parƒmetros UEP"
         HEIGHT             = 18.21
         WIDTH              = 90
         MAX-HEIGHT         = 40.5
         MAX-WIDTH          = 274.29
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _XFTR "SmartWindowCues" w-cadco2 _INLINE
...
ON END-ERROR OF w-cadco2 /* Manuten‡Æo Parƒmetros UEP */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadco2 w-cadco2
ON WINDOW-CLOSE OF w-cadco2 /* Manuten‡Æo Parƒmetros UEP */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp122-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp122-v01 ).
       RUN set-position IN h_escdp122-v01 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Contas Pat' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 7.75 , 3.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.33 , 88.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp122-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_escdp122-q01 ).
       RUN set-position IN h_escdp122-q01 ( 1.00 , 55.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_escdp122-v01. */
       RUN add-link IN adm-broker-hdl ( h_escdp122-q01 , 'Record':U , h_escdp122-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_escdp122-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_escdp122-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_escdp122-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_escdp122-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp122-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_escdp122-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp122-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/cdp/escdp122b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_escdp122-b01 ).
       RUN set-position IN h_escdp122-b01 ( 9.50 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to BrowserCadastro2 h_escdp122-b01. */
       RUN add-link IN adm-broker-hdl ( h_escdp122-q01 , 'Record':U , h_escdp122-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp122-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  def var c_cod_plano_ccusto as char init "PADRAO" no-undo.

  if not can-find(first plano_ccusto where
                        plano_ccusto.cod_empresa      = v_cod_empres_usuar
                    and plano_ccusto.cod_plano_ccusto = c_cod_plano_ccusto
                        no-lock)
  then assign c_cod_plano_ccusto = "".

  if not can-find(first int_param_uep where
                        int_param_uep.cod_empresa = v_cod_empres_usuar
                        no-lock)
  then do:
       create int_param_uep.
       assign int_param_uep.cod_empresa      = v_cod_empres_usuar
              int_param_uep.cod_plano_ccusto = c_cod_plano_ccusto.

       if v_cod_empres_usuar = "1"
       then assign int_param_uep.cod_ccusto_ini   = "44000"
                   int_param_uep.cod_ccusto_fim   = "46910".
       find current int_param_uep no-lock no-error.
  end.
  ...
  {utp/ut9000.i "ESCDP122" "1.00.00.000"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-controle w-cadco2 
...
case p-acao:
    when "enable"
    then do:
         run pi-get-status in h_escdp122-b01 (output lg-inc,
                                              output lg-del).

         run pi-set-status in h_escdp122-b01 (input no,
                                              input no).

    end.
    when "disable"
    then run pi-set-status in h_escdp122-b01 (input lg-inc,
                                              input lg-del).
end case.
