...
*******************************************************************************/
{include/i-prgvrs.i ESCDP093 1.00.00.001}
...
DEFINE VARIABLE h_escdp093-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp093-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 89.29 BY 5.25 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp093-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp093-v01 ).
       RUN set-position IN h_escdp093-v01 ( 2.71 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 3.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp093-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/cdp/escdp093-z01.w,
                     ProgVaPara = ,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_escdp093-q01 ).
       RUN set-position IN h_escdp093-q01 ( 1.00 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.75 , 6.00 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_escdp093-v01. */
       RUN add-link IN adm-broker-hdl ( h_escdp093-q01 , 'Record':U , h_escdp093-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_escdp093-v01 ).

       /* Links to SmartQuery h_escdp093-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_escdp093-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_escdp093-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_escdp093-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp093-v01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP093" "1.00.00.001"}
