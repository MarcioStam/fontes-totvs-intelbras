...
*******************************************************************************/
{include/i-prgvrs.i ESCDP032C 2.00.00.001}
...
DEF SHARED TEMP-TABLE tt-selecao-crm-desc-cli 
    FIELD r-rowid AS ROWID.
...
&Scoped-define FIELDS-IN-QUERY-br-digita tt-digita.ordem tt-digita.lista-rw   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-digita tt-digita.ordem tt-digita.lista-rw   
...
DEFINE VARIABLE cInputFile AS CHARACTER INITIAL "c:~\temp~\escdp032.csv" 
     VIEW-AS EDITOR MAX-CHARS 256
     SIZE 40 BY .88
     BGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE c-arquivo-batch AS CHARACTER FORMAT "X(256)":U 
     LABEL "Exemplo Batch" 
     VIEW-AS FILL-IN 
     SIZE 27 BY .88 NO-UNDO.

DEFINE VARIABLE c-arquivo-online AS CHARACTER FORMAT "X(256)":U INITIAL "c:~\temp~\arquivo.csv" 
     LABEL "Exemplo Online" 
     VIEW-AS FILL-IN 
     SIZE 27 BY .88 NO-UNDO.

DEFINE VARIABLE text-entrada AS CHARACTER FORMAT "X(256)":U INITIAL "Arquivo de Entrada / Sa°da" 
      VIEW-AS TEXT 
     SIZE 20.86 BY .63
     FONT 1 NO-UNDO.

DEFINE VARIABLE rs-imp-exp AS CHARACTER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Importa", "1",
"Exporta", "2",
...
DEFINE FRAME f-pg-par
     rs-imp-exp AT ROW 2.5 COL 29 NO-LABEL WIDGET-ID 2
     cInputFile AT ROW 5.04 COL 19.14 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL WIDGET-ID 24
     btInputFile AT ROW 5.04 COL 59.14 HELP
          "Escolha do nome do arquivo" WIDGET-ID 22
     c-arquivo-batch AT ROW 7.75 COL 29 COLON-ALIGNED WIDGET-ID 18
     c-arquivo-online AT ROW 8.75 COL 29 COLON-ALIGNED WIDGET-ID 20
     text-entrada AT ROW 4 COL 20.14 NO-LABEL WIDGET-ID 28
     "Os arquivos devem ser no formato CSV." VIEW-AS TEXT
          SIZE 36 BY .54 AT ROW 6.92 COL 20 WIDGET-ID 14
     RECT-12 AT ROW 4.25 COL 18 WIDGET-ID 26
     RECT-8 AT ROW 6.5 COL 18 WIDGET-ID 12
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
/* SETTINGS FOR FRAME f-pg-par
                                                                        */
ASSIGN 
       c-arquivo-batch:READ-ONLY IN FRAME f-pg-par        = TRUE.

ASSIGN 
       c-arquivo-online:READ-ONLY IN FRAME f-pg-par        = TRUE.

/* SETTINGS FOR FILL-IN text-entrada IN FRAME f-pg-par
   NO-DISPLAY NO-ENABLE ALIGN-L                                         */
ASSIGN 
       text-entrada:PRIVATE-DATA IN FRAME f-pg-par     = 
                "Arquivo de Entrada / Sa°da".
...
DO:
    if  br-digita:new-row in frame f-pg-dig then do:
        ...
        display tt-digita.ordem
                tt-digita.lista-rw with browse br-digita. 
    end.
    return no-apply.
END.
...
DO:
   ...
   if  br-digita:new-row in frame f-pg-dig then do:
       assign tt-digita.lista-rw:screen-value in browse br-digita = string(today, "99/99/9999").
   end.
END.
...
DO:
    ...
    do transaction on error undo, return no-apply:
        ...
        assign input browse br-digita tt-digita.ordem
               input browse br-digita tt-digita.lista-rw.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btInputFile w-relat
ON CHOOSE OF btInputFile IN FRAME f-pg-par
DO:
    {report/imarq.i cInputFile f-pg-par}
END.
...
{utp/ut9000.i "ESCDP032C" "2.00.00.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    IF AVAIL tt-prog-ponto THEN
        ASSIGN c-arquivo-batch:SCREEN-VALUE IN FRAME f-pg-par = tt-prog-ponto.conteudo + "/usuario/arquivo.csv".
  

    ASSIGN i-cont = 0.
    

    FOR EACH tt-selecao-crm-desc-cli NO-LOCK:

        IF  i-cont = 0 OR i-cont = 1000 THEN
        DO:
           ASSIGN i-cont = 0.

           CREATE tt-digita.

           i-cont-aux = i-cont-aux + 1.
        END.
    
        i-cont = i-cont + 1.

        IF tt-digita.lista-rw = '' THEN
           ASSIGN tt-digita.lista-rw = string(tt-selecao-crm-desc-cli.r-rowid) + ';'.
        ELSE
           ASSIGN tt-digita.lista-rw = tt-digita.lista-rw + string(tt-selecao-crm-desc-cli.r-rowid) + ';'.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY rs-imp-exp cInputFile c-arquivo-batch c-arquivo-online 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-12 RECT-8 rs-imp-exp cInputFile btInputFile c-arquivo-batch 
         c-arquivo-online 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF INPUT FRAME f-pg-par rs-imp-exp = '3' THEN 
    DO:
        IF NOT CAN-FIND(FIRST tt-selecao-crm-desc-cli ) THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U, 
                               INPUT 17006, 
                               INPUT "Nenhum registro selecionado para eliminar":U).
            RETURN NO-APPLY.
        END.
    END.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.rs-imp-exp      = input frame f-pg-par rs-imp-exp
           tt-param.arq-imp-exp     = input frame f-pg-par cInputFile
    ...
    {include/i-rprun.i esp/cdp/escdp032crp.p}
