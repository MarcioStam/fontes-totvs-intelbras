...
DEFINE VARIABLE h_escdp078-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp078-v01 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo - Categoria do Produto"
         HEIGHT             = 3.54
         WIDTH              = 90
         MAX-HEIGHT         = 27.5
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo - Categoria do Produto */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo - Categoria do Produto */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp078-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp078-v01 ).
       RUN set-position IN h_escdp078-v01 ( 2.88 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.38 , 89.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp078-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/cdp/escdp078-z01.w,
                     ProgVaPara = esp/cdp/escdp078-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_escdp078-q01 ).
       RUN set-position IN h_escdp078-q01 ( 1.25 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_escdp078-v01. */
       RUN add-link IN adm-broker-hdl ( h_escdp078-q01 , 'Record':U , h_escdp078-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_escdp078-v01 ).

       /* Links to SmartQuery h_escdp078-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_escdp078-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_escdp078-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_escdp078-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp078-v01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
