...
*******************************************************************************/
{include/i-prgvrs.i ESCDP029A 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCDP029A
...
c-ds-categoria dt-vigencia-ini dt-vigencia-fim de-pc-desconto btOK btSalvar ~
btCancel 
...
dt-vigencia-fim de-pc-desconto 
...
DEFINE FRAME fPage0
     ...
     btOK AT ROW 7.38 COL 2
     btSalvar AT ROW 7.38 COL 12.43 WIDGET-ID 40
     btCancel AT ROW 7.38 COL 22.86
     rtToolBar AT ROW 7.17 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWindow ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Desconto Unidade de Neg¢cio x Grupo de Cliente x Categoria"
         HEIGHT             = 7.67
         WIDTH              = 90
         MAX-HEIGHT         = 28.33
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF wWindow /* Inclui/Modifica Desconto Unidade de Neg¢cio x Grupo de Cliente x Categoria */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWindow wWindow
ON WINDOW-CLOSE OF wWindow /* Inclui/Modifica Desconto Unidade de Neg¢cio x Grupo de Cliente x Categoria */
DO:
    ...
    CASE pType:
        ...
        WHEN "Copy":U   THEN DO:
            FIND FIRST crm-desc-gc-cat NO-LOCK
                WHERE  ROWID(crm-desc-gc-cat) = pRow-table NO-ERROR.
            IF  AVAIL  crm-desc-gc-cat THEN
                ASSIGN i-cod-gr-cli    = crm-desc-gc-cat.cod-gr-cli
                       i-cd-categoria  = crm-desc-gc-cat.cd-categoria
                       dt-vigencia-ini = crm-desc-gc-cat.dt-vigencia-ini
                       dt-vigencia-fim = crm-desc-gc-cat.dt-vigencia-fim
                       de-pc-desconto  = crm-desc-gc-cat.pc-desconto.
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
        WHEN "Create":U OR
        WHEN "Copy":U   THEN DO:
            CREATE crm-desc-gc-cat.
            ASSIGN crm-desc-gc-cat.cd-unid-negoc   = STRING(i-cd-unid-comerc)
                   crm-desc-gc-cat.cod-gr-cli      = i-cod-gr-cli
                   crm-desc-gc-cat.cd-categoria    = i-cd-categoria
                   crm-desc-gc-cat.dt-vigencia-ini = dt-vigencia-ini.
        END.
        WHEN "Update":U THEN DO:
            FIND FIRST crm-desc-gc-cat EXCLUSIVE-LOCK
                WHERE  ROWID(crm-desc-gc-cat) = pRow-table NO-ERROR.
            IF  NOT AVAIL crm-desc-gc-cat THEN
                RETURN "NOK":U.
        END.
        OTHERWISE .
    END CASE.

    ASSIGN crm-desc-gc-cat.dt-vigencia-fim = dt-vigencia-fim
           crm-desc-gc-cat.pc-desconto     = de-pc-desconto.

    /* Grava a data inicial na data final da £ltima vigˆncia da UN x Cliente x Tab Pre‡o */
    FIND LAST crm-desc-gc-cat EXCLUSIVE-LOCK
        WHERE crm-desc-gc-cat.cd-unid-negoc   = STRING(i-cd-unid-comerc)
        AND   crm-desc-gc-cat.cod-gr-cli      = i-cod-gr-cli
        AND   crm-desc-gc-cat.cd-categoria    = i-cd-categoria
        AND   crm-desc-gc-cat.dt-vigencia-ini < dt-vigencia-ini NO-ERROR.
    IF  AVAIL crm-desc-gc-cat AND crm-desc-gc-cat.dt-vigencia-fim = ? THEN
        ASSIGN crm-desc-gc-cat.dt-vigencia-fim = dt-vigencia-ini.
...
------------------------------------------------------------------------------*/
    ...
    IF  pType = "Create":U OR
        ...
        IF  CAN-FIND(FIRST crm-desc-gc-cat NO-LOCK
                     WHERE crm-desc-gc-cat.cd-unid-negoc   = STRING(i-cd-unid-comerc)
                     AND   crm-desc-gc-cat.cod-gr-cli      = i-cod-gr-cli
                     AND   crm-desc-gc-cat.cd-categoria    = i-cd-categoria
                     AND   crm-desc-gc-cat.dt-vigencia-ini = dt-vigencia-ini) THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U,
                               INPUT 7,
                               INPUT "Desconto por Unidade Comercial x Grupo Cliente x Categoria":U).
