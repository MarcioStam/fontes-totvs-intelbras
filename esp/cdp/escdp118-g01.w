...
*******************************************************************************/
{include/i-prgvrs.i ESCDP118-G01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP118-G01 ESP}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button i_num_bem_pat i_num_seq_bem_pat ~
bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_num_bem_pat i_num_seq_bem_pat 
...
DEFINE VARIABLE i_num_bem_pat AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 0 
     LABEL "Bem Patrimonial" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE i_num_seq_bem_pat AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Sequància Bem" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_num_bem_pat AT ROW 1.17 COL 30 COLON-ALIGNED WIDGET-ID 2
     i_num_seq_bem_pat AT ROW 2.17 COL 30 COLON-ALIGNED WIDGET-ID 4
     bt-ok AT ROW 3.5 COL 2.14
     bt-cancela AT ROW 3.5 COL 13
     bt-ajuda AT ROW 3.5 COL 58
     rt-button AT ROW 3.25 COL 1
     SPACE(0.13) SKIP(0.07)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V† Para Extens∆o Bem Patrimonial"
...
ON GO OF FRAME d-vapara /* V† Para Extens∆o Bem Patrimonial */
DO:
FIND int_bem_pat NO-LOCK
    WHERE int_bem_pat.cod_empresa EQ v_cod_empres_usuar
      AND int_bem_pat.num_bem_pat EQ INPUT FRAME {&FRAME-NAME} i_num_bem_pat
      AND int_bem_pat.num_seq_bem_pat EQ INPUT FRAME {&FRAME-NAME} i_num_seq_bem_pat NO-ERROR.

IF NOT AVAILABLE int_bem_pat THEN DO:
    FIND first int_bem_pat NO-LOCK
        WHERE int_bem_pat.cod_empresa EQ v_cod_empres_usuar
          AND int_bem_pat.num_bem_pat EQ INPUT FRAME {&FRAME-NAME} i_num_bem_pat
          AND int_bem_pat.num_seq_bem_pat EQ INPUT FRAME {&FRAME-NAME} i_num_seq_bem_pat NO-ERROR.
    ...
    THEN do:
        {utp/ut-table.i mgesp int_bem_pat 1}
        RUN utp/ut-msgs.p(INPUT "show",
                          INPUT 2,
                          INPUT "Extens∆o Bem Patrimonial").
        RETURN NO-APPLY.
    END.
    else run utp/ut-msgs.p (input "show", input 19085, input "Mais de uma Conta para Extens∆o Bem Patrimonial. Posicionar-se-† na primeira encontrada.").
END.
ASSIGN p-row-tabela = ROWID(int_bem_pat).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Extens∆o Bem Patrimonial */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_num_bem_pat i_num_seq_bem_pat 
      WITH FRAME d-vapara.
  ENABLE rt-button i_num_bem_pat i_num_seq_bem_pat bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP118-G01" "1.00.00.000"}
