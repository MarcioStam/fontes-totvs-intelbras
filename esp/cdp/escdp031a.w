...
*******************************************************************************/
{include/i-prgvrs.i ESCDP031A 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCDP031A
...
c-ds-categoria c-it-codigo c-ds-item dt-vigencia-ini dt-vigencia-fim ~
de-pc-desconto btOK btSalvar btCancel 
&Scoped-Define DISPLAYED-OBJECTS i-cd-unid-comerc c-ds-unid-comerc ~
i-cod-gr-cli c-ds-gr-cli i-cd-categoria c-ds-categoria c-it-codigo ~
c-ds-item dt-vigencia-ini dt-vigencia-fim de-pc-desconto 
...
DEFINE VARIABLE c-ds-item AS CHARACTER FORMAT "X(30)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME fPage0
     ...
     c-it-codigo AT ROW 4.88 COL 18 COLON-ALIGNED HELP
          "C¢digo Item" WIDGET-ID 44
     c-ds-item AT ROW 4.88 COL 36.29 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     ...
     btOK AT ROW 8.33 COL 2
     btSalvar AT ROW 8.33 COL 12.43 WIDGET-ID 40
     btCancel AT ROW 8.33 COL 22.86
     rtToolBar AT ROW 8.13 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWindow ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Desconto por Produto por Unidade de Neg¢cio x Grupo de Cliente x Categoria"
         HEIGHT             = 8.58
         WIDTH              = 90
         MAX-HEIGHT         = 28.33
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF wWindow /* Inclui/Modifica Desconto por Produto por Unidade de Neg¢cio x Grupo de Cliente x Categoria */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWindow wWindow
ON WINDOW-CLOSE OF wWindow /* Inclui/Modifica Desconto por Produto por Unidade de Neg¢cio x Grupo de Cliente x Categoria */
DO:
...
DO:
    ...
    DISP c-ds-item WITH FRAME fPage0.
END.
    ...
    CASE pType:
        ...
        WHEN "Copy":U   THEN DO:
            FIND FIRST crm-desc-prod NO-LOCK
                WHERE  ROWID(crm-desc-prod) = pRow-table NO-ERROR.
            IF  AVAIL  crm-desc-prod THEN
                ASSIGN i-cod-gr-cli    = crm-desc-prod.cod-gr-cli
                       i-cd-categoria  = crm-desc-prod.cd-categoria
                       c-it-codigo     = crm-desc-prod.it-codigo
                       dt-vigencia-ini = crm-desc-prod.dt-vigencia-ini
                       dt-vigencia-fim = crm-desc-prod.dt-vigencia-fim
                       de-pc-desconto  = crm-desc-prod.pc-desconto.
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
        ...
        WHEN "Copy":U   THEN DO:
            ...
            ASSIGN crm-desc-prod.cd-unid-negoc   = STRING(i-cd-unid-comerc)
                   crm-desc-prod.cod-gr-cli      = i-cod-gr-cli
                   crm-desc-prod.cd-categoria    = i-cd-categoria
                   crm-desc-prod.it-codigo       = c-it-codigo
                   crm-desc-prod.dt-vigencia-ini = dt-vigencia-ini.
        END.
        WHEN "Update":U THEN DO:
            FIND FIRST crm-desc-prod EXCLUSIVE-LOCK
                WHERE  ROWID(crm-desc-prod) = pRow-table NO-ERROR.
            IF  NOT AVAIL crm-desc-prod THEN
                RETURN "NOK":U.
        END.
        OTHERWISE .
    END CASE.

    ASSIGN crm-desc-prod.dt-vigencia-fim = dt-vigencia-fim
           crm-desc-prod.pc-desconto     = de-pc-desconto.

    /* Retorna o Rowid do registro Criado/Alterado */
    ASSIGN pRow-table = ROWID(crm-desc-prod).
    ...
    FIND LAST crm-desc-prod EXCLUSIVE-LOCK
        ...
        AND   crm-desc-prod.cd-categoria    = i-cd-categoria
        AND   crm-desc-prod.it-codigo       = c-it-codigo
        AND   crm-desc-prod.dt-vigencia-ini < dt-vigencia-ini NO-ERROR.
    IF  AVAIL crm-desc-prod AND crm-desc-prod.dt-vigencia-fim = ? THEN
        ASSIGN crm-desc-prod.dt-vigencia-fim = dt-vigencia-ini.
...
------------------------------------------------------------------------------*/
    ...
    IF  pType = "Create":U OR
        ...
        IF  CAN-FIND(FIRST crm-desc-prod NO-LOCK
                     ...
                     AND   crm-desc-prod.cd-categoria    = i-cd-categoria
                     AND   crm-desc-prod.it-codigo       = c-it-codigo
                     AND   crm-desc-prod.dt-vigencia-ini = dt-vigencia-ini) THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U,
                               INPUT 7,
                               INPUT "Desconto por Produto por Unidade Comercial x Grupo Cliente x Categoria":U).
