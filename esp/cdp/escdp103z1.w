...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DECLARATIONS wZoom 
...
DEFINE TEMP-TABLE tt-int-cor NO-UNDO LIKE int-cor
       field r-rowid as rowid.
DEFINE TEMP-TABLE tt-int-cor-2 NO-UNDO LIKE int-cor
       field r-rowid as rowid.
...
*******************************************************************************/
{include/i-prgvrs.i escdp103z1 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i escdp103z1 ESP}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           escdp103z1
&GLOBAL-DEFINE Version           1.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Cor, Descri‡Æo

&GLOBAL-DEFINE Range             NO

&GLOBAL-DEFINE ttTable1          tt-int-cor
&GLOBAL-DEFINE hDBOTable1        hDBOIc
&GLOBAL-DEFINE DBOTable1         int-cor

&GLOBAL-DEFINE ttTable2          tt-int-cor-2
&GLOBAL-DEFINE hDBOTable2        hDBOIc2
&GLOBAL-DEFINE DBOTable2         int-cor
...
&Scoped-define INTERNAL-TABLES tt-int-cor tt-int-cor-2

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-int-cor.cod-cor ~
tt-int-cor.desc-cor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-int-cor NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-int-cor NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-int-cor
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-int-cor


/* Definitions for BROWSE brTable2                                      */
&Scoped-define FIELDS-IN-QUERY-brTable2 tt-int-cor-2.cod-cor ~
tt-int-cor-2.desc-cor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable2 
&Scoped-define QUERY-STRING-brTable2 FOR EACH tt-int-cor-2 NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable2 OPEN QUERY brTable2 FOR EACH tt-int-cor-2 NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable2 tt-int-cor-2
&Scoped-define FIRST-TABLE-IN-QUERY-brTable2 tt-int-cor-2
...
DEFINE VARIABLE v-cor-fim AS CHARACTER FORMAT "X(20)" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .79 NO-UNDO.

DEFINE VARIABLE v-cor-ini AS CHARACTER FORMAT "X(20)" 
     LABEL "Cor" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .79 NO-UNDO.
...
DEFINE QUERY brTable1 FOR 
      tt-int-cor SCROLLING.

DEFINE QUERY brTable2 FOR 
      tt-int-cor-2 SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-int-cor.cod-cor FORMAT "x(20)":U
      tt-int-cor.desc-cor FORMAT "x(60)":U WIDTH 69
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable2 wZoom _STRUCTURED
  QUERY brTable2 NO-LOCK DISPLAY
      tt-int-cor-2.cod-cor FORMAT "x(20)":U
      tt-int-cor-2.desc-cor FORMAT "x(60)":U WIDTH 69
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 9.5
         FONT 2 FIT-LAST-COLUMN.
...
DEFINE FRAME fPage1
     ...
     v-cor-ini AT ROW 1.75 COL 16.43 COLON-ALIGNED WIDGET-ID 2
     v-cor-fim AT ROW 1.75 COL 45 COLON-ALIGNED NO-LABEL WIDGET-ID 8
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-int-cor T "?" NO-UNDO mgesp int-cor
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
      TABLE: tt-int-cor-2 T "?" NO-UNDO mgesp int-cor
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* SETTINGS FOR FRAME fPage1
                                                                        */
/* BROWSE-TAB brTable1 v-cor-fim fPage1 */
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-int-cor"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.tt-int-cor.cod-cor
     _FldNameList[2]   > Temp-Tables.tt-int-cor.desc-cor
"tt-int-cor.desc-cor" ? ? "character" ? ? ? ? ? ? no ? no no "69" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable2
     _TblList          = "Temp-Tables.tt-int-cor-2"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.tt-int-cor-2.cod-cor
     _FldNameList[2]   > Temp-Tables.tt-int-cor-2.desc-cor
"tt-int-cor-2.desc-cor" ? ? "character" ? ? ? ? ? ? no ? no no "69" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable2 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fPage1 v-cor-ini v-cor-fim.   
...
DO:
    {zoom/Implant.i &ProgramImplant="esp/cdp/escdp103.w"
                    &PageNumber="1"}  
END.
...
DO WITH FRAME fPage1:
    ...
    ASSIGN v-cor-ini:SCREEN-VALUE = "":U
           v-cor-fim:SCREEN-VALUE = "ZZZZZZZZZZZZZZZZZZZZ":U
           v-cor-ini:SENSITIVE    = YES
           v-cor-fim:SENSITIVE    = YES
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esp\cdp\escdp103bo.p":U THEN DO:
       
        {btb/btb008za.i1 esp\cdp\escdp103bo.p YES}
        {btb/btb008za.i2 esp\cdp\escdp103bo.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintCor IN {&hDBOTable1} (INPUT "":U , INPUT "ZZZZZZZZZZZZZZZZZZZZ":U) NO-ERROR.

    IF NOT VALID-HANDLE({&hDBOTable2}) OR
       {&hDBOTable2}:TYPE <> "PROCEDURE":U OR
       {&hDBOTable2}:FILE-NAME <> "esp\cdp\escdp103bo.p":U THEN DO:
       
        {btb/btb008za.i1 esp\cdp\escdp103bo.p YES}
        {btb/btb008za.i2 esp\cdp\escdp103bo.p '' {&hDBOTable2}} 
    END.
    
    RUN setConstraintDes IN {&hDBOTable2} (INPUT "":U , INPUT "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ":U) NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-cor":U  THEN ASSIGN pcFieldValue = STRING({&ttTable1}.cod-cor).
            WHEN "desc-cor":U THEN ASSIGN pcFieldValue = STRING({&ttTable1}.desc-cor).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable2} THEN DO:
        CASE pcField:
            WHEN "cod-cor":U  THEN ASSIGN pcFieldValue = STRING({&ttTable2}.cod-cor).
            WHEN "desc-cor":U THEN ASSIGN pcFieldValue = STRING({&ttTable2}.desc-cor).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintCor IN {&hDBOTable1} (INPUT v-cor-ini, INPUT v-cor-fim).
