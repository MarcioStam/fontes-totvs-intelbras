...
*******************************************************************************/
{include/i-prgvrs.i ESCDP120 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP120 ESP}
&ENDIF
...
DEFINE VARIABLE tg-salva-log AS LOGICAL INITIAL yes 
     LABEL "Salva log" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .88 NO-UNDO.

DEFINE VARIABLE tg-simulacao AS LOGICAL INITIAL no 
     LABEL "Apenas simulaá∆o" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-par
     tg-simulacao AT ROW 1.17 COL 3 WIDGET-ID 6
     tg-salva-log AT ROW 2.17 COL 3 WIDGET-ID 4
     c-arquivo-entrada AT ROW 9.88 COL 3.29 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     bt-arquivo-entrada AT ROW 9.88 COL 70.86 HELP
          "Escolha do nome do arquivo"
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Importaá∆o Extens‰es Bens Patrimoniais"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF C-Win /* Importaá∆o Extens‰es Bens Patrimoniais */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Importaá∆o Extens‰es Bens Patrimoniais */
DO:
...
&Scoped-define FRAME-NAME f-pg-par
&Scoped-define SELF-NAME tg-salva-log
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-salva-log C-Win
ON VALUE-CHANGED OF tg-salva-log IN FRAME f-pg-par /* Salva log */
DO:
    assign tg-salva-log = input frame f-pg-par tg-salva-log.  
END.
...
&Scoped-define SELF-NAME tg-simulacao
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-simulacao C-Win
ON VALUE-CHANGED OF tg-simulacao IN FRAME f-pg-par /* Apenas simulaá∆o */
DO:
  do with frame f-pg-par:
      if not self:checked
      then assign tg-salva-log:checked   = tg-salva-log
                  tg-salva-log:sensitive = yes.
      else assign tg-salva-log:checked   = no
                  tg-salva-log:sensitive = no.
  end.
END.
...
{utp/ut9000.i "ESCDP120" "1.00.00.000"}
...
/*     {include/i-immbl.i} */
    ...
    assign c-arq-layout = search("layout/loescdp120.001")
           bt-editar:sensitive in frame f-pg-lay = no.

    if  search(c-arq-layout) <> ? 
    then do:
         if ed-layout:read-file(c-arq-layout) in frame f-pg-lay then.
         assign bt-editar:sensitive in frame f-pg-lay = yes.
    end.

    apply 'value-changed' to tg-salva-log in frame f-pg-par.

    {include/i-immbl.i im-pg-par}

/*     {include/i-imvrf.i &escdp120 &versao-layout=001} */
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY tg-simulacao tg-salva-log c-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
  ENABLE RECT-8 tg-simulacao tg-salva-log c-arquivo-entrada bt-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
do  on error undo, return error
    ...
    if not can-find(first int_param_uep where
                          ...
                          no-lock)
    then do:
         run utp/ut-msgs.p (input "show", input 17567, input "ParÉmetros UEP n∆o informados (escdp122). Importaá∆o n∆o ser† realizada!").
         return error.
    end.
    ...
    /*:T Coloque aqui a l¢gica de gravaá∆o dos parÉmtros e seleá∆o na temp-table
       tt-param */ 
    assign tt-param.simulacao = tg-simulacao:checked in frame f-pg-par
           tt-param.salva-log = tg-salva-log:checked in frame f-pg-par.

    {include/i-imexb.i}

    if  session:set-wait-state("general":U) then.

    {include/i-imrun.i esp/cdp/escdp120rp.p}
