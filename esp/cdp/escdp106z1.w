...
DEFINE TEMP-TABLE tt-int-potencia NO-UNDO LIKE int-potencia
       field r-rowid as rowid.
DEFINE TEMP-TABLE tt-int-potencia-2 NO-UNDO LIKE int-potencia
       field r-rowid as rowid.
...
*******************************************************************************/
{include/i-prgvrs.i escdp106z1 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i escdp106z1 ESP}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           escdp106z1
&GLOBAL-DEFINE Version           1.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Potˆncia, Descri‡Æo

&GLOBAL-DEFINE Range             NO

&GLOBAL-DEFINE FieldsRangePage1  v-pot-ini,v-pot-fim
&GLOBAL-DEFINE FieldsRangePage2  v-des-ini,v-des-fim

&GLOBAL-DEFINE ttTable1          tt-int-potencia
&GLOBAL-DEFINE hDBOTable1        hDBOIp
&GLOBAL-DEFINE DBOTable1         int-potencia

&GLOBAL-DEFINE ttTable2          tt-int-potencia-2
&GLOBAL-DEFINE hDBOTable2        hDBOIp2
&GLOBAL-DEFINE DBOTable2         int-potencia
...
&Scoped-define INTERNAL-TABLES tt-int-potencia tt-int-potencia-2

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-int-potencia.cod-potencia ~
tt-int-potencia.desc-potencia 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-int-potencia NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-int-potencia NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-int-potencia
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-int-potencia


/* Definitions for BROWSE brTable2                                      */
&Scoped-define FIELDS-IN-QUERY-brTable2 tt-int-potencia-2.cod-potencia ~
tt-int-potencia-2.desc-potencia 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable2 
&Scoped-define QUERY-STRING-brTable2 FOR EACH tt-int-potencia-2 NO-LOCK
&Scoped-define OPEN-QUERY-brTable2 OPEN QUERY brTable2 FOR EACH tt-int-potencia-2 NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable2 tt-int-potencia-2
&Scoped-define FIRST-TABLE-IN-QUERY-brTable2 tt-int-potencia-2
...
DEFINE VARIABLE v-pot-fim AS CHARACTER FORMAT "X(20)" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .79 NO-UNDO.

DEFINE VARIABLE v-pot-ini AS CHARACTER FORMAT "X(20)" 
     LABEL "Potˆncia" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .79 NO-UNDO.
...
DEFINE QUERY brTable1 FOR 
      tt-int-potencia SCROLLING.

DEFINE QUERY brTable2 FOR 
      tt-int-potencia-2 SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-int-potencia.cod-potencia FORMAT "x(20)":U
      tt-int-potencia.desc-potencia FORMAT "x(60)":U WIDTH 69
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable2 wZoom _STRUCTURED
  QUERY brTable2 NO-LOCK DISPLAY
      tt-int-potencia-2.cod-potencia FORMAT "x(20)":U
      tt-int-potencia-2.desc-potencia FORMAT "x(60)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 9.5
         FONT 2 FIT-LAST-COLUMN.
...
DEFINE FRAME fPage1
     ...
     v-pot-ini AT ROW 1.75 COL 16.43 COLON-ALIGNED WIDGET-ID 2
     v-pot-fim AT ROW 1.75 COL 45 COLON-ALIGNED NO-LABEL WIDGET-ID 8
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-int-potencia T "?" NO-UNDO mgesp int-potencia
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
      TABLE: tt-int-potencia-2 T "?" NO-UNDO mgesp int-potencia
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* SETTINGS FOR FRAME fPage1
                                                                        */
/* BROWSE-TAB brTable1 v-pot-fim fPage1 */
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-int-potencia"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.tt-int-potencia.cod-potencia
     _FldNameList[2]   > Temp-Tables.tt-int-potencia.desc-potencia
"tt-int-potencia.desc-potencia" ? ? "character" ? ? ? ? ? ? no ? no no "69" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable2
     _TblList          = "Temp-Tables.tt-int-potencia-2"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.tt-int-potencia-2.cod-potencia
     _FldNameList[2]   = Temp-Tables.tt-int-potencia-2.desc-potencia
     _Query            is OPENED
*/  /* BROWSE brTable2 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fPage1 v-pot-ini v-pot-fim.
...
DO:
    {zoom/Implant.i &ProgramImplant="esp/cdp/escdp106.w"
                    &PageNumber="1"}  
END.
...
DO WITH FRAME fPage1:
    ...
    ASSIGN v-pot-ini:SCREEN-VALUE = "":U
           v-pot-fim:SCREEN-VALUE = "ZZZZZZZZZZZZZZZZZZZ":U
           v-pot-ini:SENSITIVE    = YES
           v-pot-fim:SENSITIVE    = YES
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esp\cdp\escdp106bo.p":U THEN DO:
       
        {btb/btb008za.i1 esp\cdp\escdp106bo.p YES}
        {btb/btb008za.i2 esp\cdp\escdp106bo.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintPot IN {&hDBOTable1} (INPUT "":U , INPUT "ZZZZZZZZZZZZZZZZZZZ":U) NO-ERROR.

    IF NOT VALID-HANDLE({&hDBOTable2}) OR
       {&hDBOTable2}:TYPE <> "PROCEDURE":U OR
       {&hDBOTable2}:FILE-NAME <> "esp\cdp\escdp106bo.p":U THEN DO:
       
        {btb/btb008za.i1 esp\cdp\escdp106bo.p YES}
        {btb/btb008za.i2 esp\cdp\escdp106bo.p '' {&hDBOTable2}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-potencia":U  THEN ASSIGN pcFieldValue = STRING({&ttTable1}.cod-potencia).
            WHEN "desc-potencia":U THEN ASSIGN pcFieldValue = STRING({&ttTable1}.desc-potencia).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable2} THEN DO:
        CASE pcField:
            WHEN "cod-potencia":U  THEN ASSIGN pcFieldValue = STRING({&ttTable2}.cod-potencia).
            WHEN "desc-potencia":U THEN ASSIGN pcFieldValue = STRING({&ttTable2}.desc-potencia).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintPot IN {&hDBOTable1} (INPUT v-pot-ini, INPUT v-pot-fim).
