...
*******************************************************************************/
{include/i-prgvrs.i ESCDP101 1.00.00.001}
...
&Scoped-Define ENABLED-OBJECTS bt-importa rt-button 
...
DEFINE VARIABLE h_escdp101-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp101-v01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q03ad098 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-importa 
     IMAGE-UP FILE "image/im-carga.bmp":U
     IMAGE-DOWN FILE "image/ii-carga.bmp":U NO-FOCUS FLAT-BUTTON
     LABEL "Importa" 
     SIZE 4 BY 1.25 TOOLTIP "Importa registros".
...
DEFINE FRAME f-cad
     bt-importa AT ROW 1.13 COL 31 WIDGET-ID 166
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-filho ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Desconto-Acr‚scimo Cliente"
         HEIGHT             = 13.08
         WIDTH              = 90
         MAX-HEIGHT         = 40.5
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF w-cadpaifilho-filho /* Manuten‡Æo Desconto-Acr‚scimo Cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Manuten‡Æo Desconto-Acr‚scimo Cliente */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-importa w-cadpaifilho-filho
ON CHOOSE OF bt-importa IN FRAME f-cad /* Importa */
DO:
    run esp/cdp/escdp101b.w.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp101-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp101-v01 ).
       RUN set-position IN h_escdp101-v01 ( 2.58 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Desc-Acr‚sc' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 4.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.83 , 90.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adqry/q03ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = adzoom/z02ad098.r,
                     ProgVaPara = adgo/g02ad098.r,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q03ad098 ).
       RUN set-position IN h_q03ad098 ( 1.00 , 49.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_escdp101-v01. */
       RUN add-link IN adm-broker-hdl ( h_q03ad098 , 'Record':U , h_escdp101-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q03ad098. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q03ad098 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q03ad098 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q03ad098 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp101-v01 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_escdp101-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp101-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/cdp/escdp101a.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_escdp101-b01 ).
       RUN set-position IN h_escdp101-b01 ( 5.50 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 69.00 ) */

       /* Links to BrowserCadastro2 h_escdp101-b01. */
       RUN add-link IN adm-broker-hdl ( h_q03ad098 , 'Record':U , h_escdp101-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp101-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP101" "1.00.00.001"}
