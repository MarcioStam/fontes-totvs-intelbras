...
*******************************************************************************/
{include/i-prgvrs.i ESCDP071 9.99.99.999}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-importar 
...
DEFINE VARIABLE h_query-supervisor AS HANDLE NO-UNDO.
DEFINE VARIABLE h_viewer-supervisor AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-importar AT ROW 1.33 COL 58.29 WIDGET-ID 2
     rt-button AT ROW 1.21 COL 1.43
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 89.57 BY 6 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Supervisor - ESCDP071"
         HEIGHT             = 5.71
         WIDTH              = 90
         MAX-HEIGHT         = 27.04
         MAX-WIDTH          = 195.14
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Supervisor - ESCDP071 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Supervisor - ESCDP071 */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-importar w-cadsim
ON CHOOSE OF bt-importar IN FRAME f-cad /* Button 1 */
DO:
  RUN esp\cdp\escdp071a.w.
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navega ).
       RUN set-position IN h_p-navega ( 1.33 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-cadsim ).
       RUN set-position IN h_p-cadsim ( 1.33 , 27.14 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 71.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp071-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_viewer-supervisor ).
       RUN set-position IN h_viewer-supervisor ( 2.75 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp071-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/cdp/escdp071-z01.w,
                     ProgVaPara = esp/cdp/escdp071-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_query-supervisor ).
       RUN set-position IN h_query-supervisor ( 1.25 , 64.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 5.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_viewer-supervisor. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_viewer-supervisor ).
       RUN add-link IN adm-broker-hdl ( h_query-supervisor , 'Record':U , h_viewer-supervisor ).

       /* Links to SmartQuery h_query-supervisor. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_query-supervisor ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_query-supervisor ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_query-supervisor ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-importar:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsim ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             bt-importar:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_viewer-supervisor ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP071" "9.99.99.999"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-importar w-cadsim 
