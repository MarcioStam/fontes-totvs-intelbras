...
*******************************************************************************/
{include/i-prgvrs.i ESCDP119 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP119 ESP}
&ENDIF
...
def var lg-aux             as logi    no-undo.
...
DEFINE VARIABLE tg-cad-compl AS LOGICAL INITIAL no 
     LABEL "Cadastro completo" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .88 NO-UNDO.

DEFINE VARIABLE tg-cad-incompl AS LOGICAL INITIAL yes 
     LABEL "Cadastro incompleto" 
     VIEW-AS TOGGLE-BOX
     SIZE 17 BY .88 NO-UNDO.

DEFINE VARIABLE tg-exporta AS LOGICAL INITIAL no 
     LABEL "Exporta escdp119.csv" 
     VIEW-AS TOGGLE-BOX
     SIZE 19 BY .88 NO-UNDO.

DEFINE VARIABLE tg-sem-cad AS LOGICAL INITIAL yes 
     LABEL "Sem cadastro" 
     VIEW-AS TOGGLE-BOX
     SIZE 13 BY .88 NO-UNDO.

DEFINE VARIABLE tg-somente-incons AS LOGICAL INITIAL no 
     LABEL "Somente inconsistentes" 
     VIEW-AS TOGGLE-BOX
     SIZE 19 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_ccusto_fim AS CHARACTER FORMAT "X(11)":U INITIAL "99999999999" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_ccusto_ini AS CHARACTER FORMAT "X(11)":U 
     LABEL "Centro de Custo" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_cta_fim AS CHARACTER FORMAT "X(18)":U INITIAL "ZZZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 19 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_cta_ini AS CHARACTER FORMAT "X(18)":U 
     LABEL "Conta Patrimonial" 
     VIEW-AS FILL-IN 
     SIZE 19 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_empresa AS CHARACTER FORMAT "X(3)":U INITIAL "0" 
     LABEL "Empresa" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_estab_fim AS CHARACTER FORMAT "X(5)":U INITIAL "104" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_estab_ini AS CHARACTER FORMAT "X(5)":U INITIAL "104" 
     LABEL "Estabelecimento" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE fi_cod_plano_ccusto AS CHARACTER FORMAT "X(8)":U INITIAL "PADRAO" 
     LABEL "Plano Centros Custo" 
     VIEW-AS FILL-IN 
     SIZE 9 BY .88 NO-UNDO.

DEFINE VARIABLE fi_dat_aquis_fim AS DATE FORMAT "99/99/9999":U INITIAL 12/31/2199 
     VIEW-AS FILL-IN 
     SIZE 11 BY .88 NO-UNDO.

DEFINE VARIABLE fi_dat_aquis_ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/001 
     LABEL "Data Aquisiá∆o" 
     VIEW-AS FILL-IN 
     SIZE 11 BY .88 NO-UNDO.

DEFINE VARIABLE fi_num_bem_fim AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 999999999 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi_num_bem_ini AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 0 
     LABEL "Bem Patrimonial" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi_num_seq_bem_fim AS INTEGER FORMAT ">>>>9":U INITIAL 99999 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE fi_num_seq_bem_ini AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Sequància Bem" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-par
     tg-somente-incons AT ROW 6.5 COL 4 WIDGET-ID 18
     tg-exporta AT ROW 7.5 COL 4 WIDGET-ID 20
     tg-sem-cad AT ROW 2.25 COL 8 WIDGET-ID 8
     tg-cad-incompl AT ROW 3.25 COL 8 WIDGET-ID 10
     tg-cad-compl AT ROW 4.25 COL 8 WIDGET-ID 12
     fi-diretorio AT ROW 8.83 COL 8 COLON-ALIGNED HELP
          "Diret¢rio em que os arquivos ser∆o salvos" WIDGET-ID 28
     bt-dir-2 AT ROW 8.75 COL 68.72 HELP
          "Escolha o diret¢rio" WIDGET-ID 26
     "Cadastro" VIEW-AS TEXT
          SIZE 8 BY .54 AT ROW 1.25 COL 5.14 WIDGET-ID 16
     RECT-10 AT ROW 1.5 COL 3 WIDGET-ID 14
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 75 BY 10
...
DEFINE FRAME f-pg-sel
     fi_cod_empresa AT ROW 1.5 COL 17 COLON-ALIGNED WIDGET-ID 42
     fi_dat_aquis_ini AT ROW 2.5 COL 17 COLON-ALIGNED
     fi_dat_aquis_fim AT ROW 2.5 COL 45.86 COLON-ALIGNED NO-LABEL
     fi_cod_cta_ini AT ROW 3.5 COL 17 COLON-ALIGNED WIDGET-ID 4
     fi_cod_cta_fim AT ROW 3.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     fi_num_bem_ini AT ROW 4.5 COL 17 COLON-ALIGNED WIDGET-ID 12
     fi_num_bem_fim AT ROW 4.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 10
     fi_num_seq_bem_ini AT ROW 5.5 COL 17 COLON-ALIGNED WIDGET-ID 20
     fi_num_seq_bem_fim AT ROW 5.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     fi_cod_estab_ini AT ROW 6.5 COL 17 COLON-ALIGNED WIDGET-ID 28
     fi_cod_estab_fim AT ROW 6.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     fi_cod_ccusto_ini AT ROW 7.5 COL 17 COLON-ALIGNED WIDGET-ID 36
     fi_cod_ccusto_fim AT ROW 7.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 34
     fi_cod_plano_ccusto AT ROW 8.5 COL 17 COLON-ALIGNED WIDGET-ID 44
     IMAGE-1 AT ROW 2.5 COL 38
     IMAGE-2 AT ROW 2.5 COL 45
     IMAGE-3 AT ROW 3.5 COL 38 WIDGET-ID 6
     IMAGE-4 AT ROW 3.5 COL 45 WIDGET-ID 8
     IMAGE-5 AT ROW 4.5 COL 38 WIDGET-ID 14
     IMAGE-6 AT ROW 4.5 COL 45 WIDGET-ID 16
     IMAGE-7 AT ROW 5.5 COL 38 WIDGET-ID 22
     IMAGE-8 AT ROW 5.5 COL 45 WIDGET-ID 24
     IMAGE-9 AT ROW 6.5 COL 38 WIDGET-ID 30
     IMAGE-10 AT ROW 6.5 COL 45 WIDGET-ID 32
     IMAGE-11 AT ROW 7.5 COL 38 WIDGET-ID 38
     IMAGE-12 AT ROW 7.5 COL 45 WIDGET-ID 40
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Consistància Extens‰es Bens"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 41.33
         MAX-WIDTH          = 274.29
...
/* SETTINGS FOR FRAME f-pg-par
   L-To-R,COLUMNS                                                       */
/* SETTINGS FOR BUTTON bt-dir-2 IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-diretorio IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR TOGGLE-BOX tg-exporta IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR FRAME f-pg-sel
   L-To-R                                                               */
/* SETTINGS FOR FILL-IN fi_cod_empresa IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi_cod_plano_ccusto IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-relat /* Consistància Extens‰es Bens */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Consistància Extens‰es Bens */
DO:
...
ON CHOOSE OF bt-dir-2 IN FRAME f-pg-par
DO:
    ...
    if not l-cancel
    then do:
         assign fi-diretorio:screen-value in frame f-pg-par = c-dir-aux.
         assign input frame f-pg-par fi-diretorio.
    end.
END.
...
ON VALUE-CHANGED OF tg-exporta IN FRAME f-pg-par /* Exporta escdp119.csv */
DO:
  assign bt-dir-2:sensitive in frame f-pg-par = self:checked.

  if self:checked
  then do:
       assign fi-diretorio:sensitive    in frame f-pg-par = yes               
              fi-diretorio:screen-value in frame f-pg-par = fi-diretorio.

       if  fi-diretorio = ""
       and not lg-aux
       then assign fi-diretorio:screen-value in frame f-pg-par = session:temp-directory.

       assign lg-aux = yes.
  end.
  else assign fi-diretorio                                = input frame f-pg-par fi-diretorio
              fi-diretorio:sensitive                      = no
              fi-diretorio:screen-value in frame f-pg-par = "".
END.
...
&Scoped-define SELF-NAME tg-somente-incons
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-somente-incons w-relat
ON VALUE-CHANGED OF tg-somente-incons IN FRAME f-pg-par /* Somente inconsistentes */
DO:
  if self:checked
  then assign tg-sem-cad                                 = tg-sem-cad:checked     in frame f-pg-par
              tg-cad-incompl                             = tg-cad-incompl:checked in frame f-pg-par
              tg-cad-compl                               = tg-cad-compl:checked   in frame f-pg-par
              tg-sem-cad:checked       in frame f-pg-par = yes
              tg-cad-incompl:checked   in frame f-pg-par = yes
              tg-cad-compl:checked     in frame f-pg-par = yes
              tg-exporta:checked       in frame f-pg-par = tg-exporta
              tg-sem-cad:sensitive     in frame f-pg-par = no
              tg-cad-incompl:sensitive in frame f-pg-par = no
              tg-cad-compl:sensitive   in frame f-pg-par = no
              tg-exporta:sensitive     in frame f-pg-par = yes.
  else assign tg-exporta                                 = tg-exporta:checked in frame f-pg-par
              tg-sem-cad:checked       in frame f-pg-par = tg-sem-cad
              tg-cad-incompl:checked   in frame f-pg-par = tg-cad-incompl
              tg-cad-compl:checked     in frame f-pg-par = tg-cad-compl
              tg-exporta:checked       in frame f-pg-par = no
              tg-sem-cad:sensitive     in frame f-pg-par = yes
              tg-cad-incompl:sensitive in frame f-pg-par = yes
              tg-cad-compl:sensitive   in frame f-pg-par = yes
              tg-exporta:sensitive     in frame f-pg-par = no.

  apply 'value-changed' to tg-exporta in frame f-pg-par.
END.
...
{utp/ut9000.i "ESCDP119" "1.00.00.000"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    assign dt-aux = date(month(today),1,year(today)).

    assign fi_cod_empresa:screen-value   in frame f-pg-sel = v_cod_empres_usuar
           fi_dat_aquis_ini:screen-value in frame f-pg-sel = string(dt-aux)
           fi_dat_aquis_fim:screen-value in frame f-pg-sel = string(today).

    for first int_param_uep no-lock
        where int_param_uep.cod_empres = v_cod_empres_usuar:
        assign 
/*                fi_cod_plano_ccusto:screen-value in frame f-pg-sel = int_param_uep.cod_plano_ccusto */
/*                fi_cod_ccusto_ini:screen-value   in frame f-pg-sel = int_param_uep.cod_ccusto_ini   */
               fi_cod_ccusto_fim:screen-value   in frame f-pg-sel = int_param_uep.cod_ccusto_fim.
    end.

    apply 'value-changed' to tg-somente-incons in frame f-pg-par.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi_cod_empresa fi_dat_aquis_ini fi_dat_aquis_fim fi_cod_cta_ini 
          fi_cod_cta_fim fi_num_bem_ini fi_num_bem_fim fi_num_seq_bem_ini 
          fi_num_seq_bem_fim fi_cod_estab_ini fi_cod_estab_fim fi_cod_ccusto_ini 
          fi_cod_ccusto_fim fi_cod_plano_ccusto 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 IMAGE-7 IMAGE-8 
         IMAGE-9 IMAGE-10 IMAGE-11 IMAGE-12 fi_dat_aquis_ini fi_dat_aquis_fim 
         fi_cod_cta_ini fi_cod_cta_fim fi_num_bem_ini fi_num_bem_fim 
         fi_num_seq_bem_ini fi_num_seq_bem_fim fi_cod_estab_ini 
         fi_cod_estab_fim fi_cod_ccusto_ini fi_cod_ccusto_fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY tg-somente-incons tg-exporta tg-sem-cad tg-cad-incompl tg-cad-compl 
          fi-diretorio 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-10 tg-somente-incons tg-sem-cad tg-cad-incompl tg-cad-compl 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    if not can-find(first int_param_uep where
                          int_param_uep.cod_empresa = v_cod_empres_usuar
                          no-lock)
    then do:
         run utp/ut-msgs.p (input "show", input 17567, input "ParÉmetros UEP n∆o informados (escdp122). Programa n∆o ser† executado!").
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_dat_aquis_ini > input frame f-pg-sel fi_dat_aquis_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Data de Aquisiá∆o inicial maior que final!").
         apply 'entry' to fi_dat_aquis_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_cod_cta_ini > input frame f-pg-sel fi_cod_cta_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Conta Patrimonial inicial maior que final!").
         apply 'entry' to fi_cod_cta_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_num_bem_ini > input frame f-pg-sel fi_num_bem_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Bem Patrimonial inicial maior que final!").
         apply 'entry' to fi_num_bem_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_num_seq_bem_ini > input frame f-pg-sel fi_num_seq_bem_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Sequància Bem inicial maior que final!").
         apply 'entry' to fi_num_seq_bem_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_cod_estab_ini > input frame f-pg-sel fi_cod_estab_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Estabelecimento inicial maior que final!").
         apply 'entry' to fi_cod_estab_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if input frame f-pg-sel fi_cod_ccusto_ini > input frame f-pg-sel fi_cod_ccusto_fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Centro de Custo inicial maior que final!").
         apply 'entry' to fi_cod_ccusto_ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if not can-find(first plano_ccusto where
                          plano_ccusto.cod_empresa      = v_cod_empres_usuar
                      and plano_ccusto.cod_plano_ccusto = input frame f-pg-sel fi_cod_plano_ccusto
                          no-lock)
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Plano Centro de Custo n∆o cadastrado!").
         apply 'entry' to fi_cod_plano_ccusto in frame f-pg-sel.
         return 'ADM-ERROR'.
    end.

    if  not tg-sem-cad:checked     in frame f-pg-par
    and not tg-cad-incompl:checked in frame f-pg-par
    and not tg-cad-compl:checked   in frame f-pg-par
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-par in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Ao menos uma opá∆o deve ser marcada!").
         apply 'entry' to tg-sem-cad in frame f-pg-par.
         return 'ADM-ERROR'.
    end.

    if tg-exporta:checked in frame f-pg-par
    then do:
         if input frame f-pg-par fi-diretorio = ""
         then do:
              apply "MOUSE-SELECT-CLICK":U to im-pg-par in frame f-relat.
              run utp/ut-msgs.p (input "show", input 17567, input "Diret¢rio deve ser informado!").
              apply 'entry' to fi-diretorio in frame f-pg-par.
              return 'ADM-ERROR'.
         end.
    ...
    end. /* if tg-exporta:checked in frame f-pg-par */
    ...
    assign tt-param.dat_aquis_ini    = input frame f-pg-sel fi_dat_aquis_ini  
           tt-param.dat_aquis_fim    = input frame f-pg-sel fi_dat_aquis_fim  
           tt-param.cod_cta_ini      = input frame f-pg-sel fi_cod_cta_ini    
           tt-param.cod_cta_fim      = input frame f-pg-sel fi_cod_cta_fim    
           tt-param.num_bem_ini      = input frame f-pg-sel fi_num_bem_ini    
           tt-param.num_bem_fim      = input frame f-pg-sel fi_num_bem_fim    
           tt-param.num_seq_bem_ini  = input frame f-pg-sel fi_num_seq_bem_ini
           tt-param.num_seq_bem_fim  = input frame f-pg-sel fi_num_seq_bem_fim
           tt-param.cod_estab_ini    = input frame f-pg-sel fi_cod_estab_ini  
           tt-param.cod_estab_fim    = input frame f-pg-sel fi_cod_estab_fim  
           tt-param.cod_ccusto_ini   = input frame f-pg-sel fi_cod_ccusto_ini 
           tt-param.cod_ccusto_fim   = input frame f-pg-sel fi_cod_ccusto_fim
           tt-param.cod_plano_ccusto = input frame f-pg-sel fi_cod_plano_ccusto
           tt-param.sem-cad          = tg-sem-cad:checked        in frame f-pg-par
           tt-param.cad-incompl      = tg-cad-incompl:checked    in frame f-pg-par
           tt-param.cad-compl        = tg-cad-compl:checked      in frame f-pg-par
           tt-param.so-inconsist     = tg-somente-incons:checked in frame f-pg-par
           tt-param.exporta          = tg-exporta:checked        in frame f-pg-par
           tt-param.diretorio        = input frame f-pg-par fi-diretorio + "escdp119.csv".

    if tg-somente-incons:checked in frame f-pg-par
    then assign tt-param.cod_ccusto_ini = ""
                tt-param.cod_ccusto_fim = "99999999999".
    ...
    {include/i-rprun.i esp/cdp/escdp119rp.p}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
assign fi-diretorio = trim(input frame f-pg-par fi-diretorio).
...
if  substr(fi-diretorio,length(fi-diretorio),1) <> "/"
and substr(fi-diretorio,length(fi-diretorio),1) <> "\"
then assign fi-diretorio = fi-diretorio + "/".

if fi-diretorio begins "\\"
then assign fi-diretorio = replace(fi-diretorio,"/","\").
else assign fi-diretorio = replace(fi-diretorio,"\","/").

assign fi-diretorio:screen-value in frame f-pg-par = fi-diretorio.

assign file-info:file-name = fi-diretorio.
