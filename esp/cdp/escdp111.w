...
*******************************************************************************/
{include/i-prgvrs.i ESCDP111 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP111 ESP}
&ENDIF
...
def var lg-mod as logi no-undo.
...
DEFINE VARIABLE h_escdp111-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp111-v01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadsi2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01in144 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadco2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Relacto GM x Ferramenta"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 20.71
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadco2 /* Manuten‡Æo Relacto GM x Ferramenta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadco2 w-cadco2
ON WINDOW-CLOSE OF w-cadco2 /* Manuten‡Æo Relacto GM x Ferramenta */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp111-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp111-v01 ).
       RUN set-position IN h_escdp111-v01 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Recurso' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 6.67 , 2.72 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.33 , 88.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'inqry/q01in144.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Key-Name = ,
                     ProgPesquisa = inzoom/z01in144.r,
                     ProgVaPara = ingo/g01in144.r,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01in144 ).
       RUN set-position IN h_q01in144 ( 1.25 , 56.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_escdp111-v01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_escdp111-v01 ).
       RUN add-link IN adm-broker-hdl ( h_q01in144 , 'Record':U , h_escdp111-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01in144. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_q01in144 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01in144 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01in144 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01in144 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp111-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_escdp111-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp111-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/cdp/escdp111b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_escdp111-b01 ).
       RUN set-position IN h_escdp111-b01 ( 8.50 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 77.00 ) */

       /* Links to BrowserCadastro2 h_escdp111-b01. */
       RUN add-link IN adm-broker-hdl ( h_q01in144 , 'Record':U , h_escdp111-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp111-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP111" "1.00.00.000"}
...
case p-acao:
    when "enable"
    then do:
         run pi-get-status in h_escdp111-b01 (output lg-inc,
                                              output lg-mod,
                                              output lg-del).

         run pi-set-status in h_escdp111-b01 (input no,
                                              input no,
                                              input no).

    end.
    when "disable"
    then run pi-set-status in h_escdp111-b01 (input lg-inc,
                                              input lg-mod,
                                              input lg-del).
end case.
