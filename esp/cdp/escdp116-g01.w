...
&Scoped-Define ENABLED-OBJECTS rt-button RECT-1 i_cod-segmento bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-segmento 
...
DEFINE VARIABLE i_cod-segmento AS INTEGER FORMAT ">>9":U INITIAL 0 
     LABEL "Cod Segmento" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-segmento AT ROW 1.75 COL 17.86 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 25
     rt-button AT ROW 4.25 COL 1
     RECT-1 AT ROW 1.08 COL 1.57 WIDGET-ID 4
     SPACE(1.28) SKIP(1.66)
...
DO:
FIND int-segmento-portifolio NO-LOCK
    WHERE int-segmento-portifolio.cod-segmento EQ INPUT FRAME {&FRAME-NAME} i_cod-segmento NO-ERROR.

IF NOT AVAILABLE int-segmento-portifolio THEN DO:
    {utp/ut-table.i TMGESP int-segmento-portifolio 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-segmento-portifolio).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-segmento 
      WITH FRAME d-vapara.
  ENABLE rt-button RECT-1 i_cod-segmento bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
