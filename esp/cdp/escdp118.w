...
*******************************************************************************/
{include/i-prgvrs.i ESCDP118 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP118 ESP}
&ENDIF
...
def var rw_int_bem_pat as rowid no-undo.
...
DEFINE VARIABLE h_escdp118-b03 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp118-b04 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp118-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_escdp118-v01 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadcom ASSIGN
         HIDDEN             = YES
         TITLE              = "Manutená∆o Extens∆o Bem Patrimonial"
         HEIGHT             = 21.21
         WIDTH              = 90
         MAX-HEIGHT         = 21.92
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadcom /* Manutená∆o Extens∆o Bem Patrimonial */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadcom w-cadcom
ON WINDOW-CLOSE OF w-cadcom /* Manutená∆o Extens∆o Bem Patrimonial */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp118-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp118-v01 ).
       RUN set-position IN h_escdp118-v01 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Gr M†quina|Centros Custo' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 10.67 , 2.14 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.33 , 88.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp118-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/cdp/escdp118-z02.w,
                     ProgVaPara = esp/cdp/escdp118-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_escdp118-q01 ).
       RUN set-position IN h_escdp118-q01 ( 1.25 , 62.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_escdp118-v01. */
       RUN add-link IN adm-broker-hdl ( h_escdp118-q01 , 'Record':U , h_escdp118-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_escdp118-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_escdp118-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_escdp118-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_escdp118-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_escdp118-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_escdp118-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp118-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_escdp118-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp118-b03.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Layout = ,
                     Create-On-Add = ?,
                     ProgAtributo = esp/cdp/escdp118a.w,
                     ProgIncMod = ':U ,
             OUTPUT h_escdp118-b03 ).
       RUN set-position IN h_escdp118-b03 ( 12.50 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.50 , 66.00 ) */

       /* Links to BrowserFormaFilho h_escdp118-b03. */
       RUN add-link IN adm-broker-hdl ( h_escdp118-q01 , 'Record':U , h_escdp118-b03 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp118-b03 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/cdp/escdp118-b04.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_escdp118-b04 ).
       RUN set-position IN h_escdp118-b04 ( 12.50 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to SmartBrowser h_escdp118-b04. */
       RUN add-link IN adm-broker-hdl ( h_escdp118-q01 , 'Record':U , h_escdp118-b04 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_escdp118-b04 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP118" "1.00.00.000"}

  assign rw_int_bem_pat = gr_int_bem_pat.
  ...
  assign gr_int_bem_pat = rw_int_bem_pat.

  for first int_bem_pat
      where rowid(int_bem_pat) = gr_int_bem_pat
            no-lock: end.

  if avail int_bem_pat
  then run pi-reposiciona-query in h_escdp118-q01 (input rowid(int_bem_pat)).
END PROCEDURE.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-controle w-cadcom 
PROCEDURE pi-controle :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
def input parameter p-acao as char no-undo.
def input parameter p-new  as logi no-undo.

case p-acao:
    when "enable"
    then run pi-controle in h_escdp118-b03 (input no,
                                            input p-new).
    when "disable"
    then run pi-controle in h_escdp118-b03 (input yes,
                                            input p-new).
end case.
