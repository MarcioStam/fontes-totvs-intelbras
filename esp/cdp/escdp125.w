...
*******************************************************************************/
{include/i-prgvrs.i ESCDP125 2.12.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP125 ESP}
&ENDIF
...
&Scoped-define FIELDS-IN-QUERY-br-digita tt-digita.gm-codigo tt-digita.descricao   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-digita tt-digita.gm-codigo   
...
DEFINE VARIABLE rs-classif AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Grupo de M†quina / Bem Patrimonial / Sequància Bem / Conta Patrimonial", 1,
"Bem Patrimonial / Sequància Bem / Conta Patrimonial / Grupo de M†quina", 2
...
DEFINE VARIABLE fi-gm-codigo-fim AS CHARACTER FORMAT "X(9)":U INITIAL "ZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi-gm-codigo-ini AS CHARACTER FORMAT "X(9)":U 
     LABEL "Grupo M†quina" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-digita w-relat _FREEFORM
  QUERY br-digita DISPLAY
      tt-digita.gm-codigo format "x(9)"  column-label "Grupo Maq" width 10
tt-digita.descricao format "x(32)" column-label "Descriá∆o" width 33
ENABLE
tt-digita.gm-codigo
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME f-pg-sel
     ...
     fi_dat_aquis_ini AT ROW 2.5 COL 17 COLON-ALIGNED WIDGET-ID 48
     fi_dat_aquis_fim AT ROW 2.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     ...
     fi-gm-codigo-ini AT ROW 6.5 COL 17 COLON-ALIGNED WIDGET-ID 56
     fi-gm-codigo-fim AT ROW 6.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 54
     fi_cod_estab_ini AT ROW 7.5 COL 17 COLON-ALIGNED WIDGET-ID 28
     fi_cod_estab_fim AT ROW 7.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     fi_cod_ccusto_ini AT ROW 8.5 COL 17 COLON-ALIGNED WIDGET-ID 36
     fi_cod_ccusto_fim AT ROW 8.5 COL 45.86 COLON-ALIGNED NO-LABEL WIDGET-ID 34
     fi_cod_plano_ccusto AT ROW 9.5 COL 17 COLON-ALIGNED WIDGET-ID 44
     IMAGE-1 AT ROW 2.5 COL 38 WIDGET-ID 50
     IMAGE-2 AT ROW 2.5 COL 45 WIDGET-ID 52
     ...
     IMAGE-9 AT ROW 7.5 COL 38 WIDGET-ID 30
     IMAGE-10 AT ROW 7.5 COL 45 WIDGET-ID 32
     IMAGE-11 AT ROW 8.5 COL 38 WIDGET-ID 38
     IMAGE-12 AT ROW 8.5 COL 45 WIDGET-ID 40
     IMAGE-13 AT ROW 6.5 COL 38 WIDGET-ID 60
     IMAGE-14 AT ROW 6.5 COL 45 WIDGET-ID 58
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio Gr Maq Patrimìnio x UEP"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 36
         MAX-WIDTH          = 219.43
...
ON END-ERROR OF w-relat /* Relat¢rio Gr Maq Patrimìnio x UEP */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Relat¢rio Gr Maq Patrimìnio x UEP */
DO:
...
DO:
    ...
    do transaction on error undo, return no-apply:
        create tt-digita.
        assign input browse br-digita tt-digita.gm-codigo
...
DO:
   apply 'entry':U to tt-digita.gm-codigo in browse br-digita. 
END.
...
/* ***************************  Main Block  *************************** */
tt-digita.gm-codigo:load-mouse-pointer("image/lupa.cur") in browse br-digita.
/* Set CURRENT-WINDOW: this will parent dialog-boxes and frames.        */
ASSIGN CURRENT-WINDOW                = {&WINDOW-NAME} 
       THIS-PROCEDURE:CURRENT-WINDOW = {&WINDOW-NAME}.

{utp/ut9000.i "ESCDP125" "2.12.00.000"}
...
on 'F5':U, 
   'mouse-select-dblclick':U OF tt-digita.gm-codigo, 
                                tt-digita.descricao in browse br-digita
do:
    if num-results("br-digita":U) = 0
    then return.

   {include/zoomvar.i &browse     = br-digita
                      &prog-zoom  = inzoom/z01in144.w
                      &campo      = tt-digita.descricao
                      &campozoom  = descricao
                      &campo2     = tt-digita.gm-codigo
                      &campozoom2 = gm-codigo} 

    return.
end.

on 'leave':U of tt-digita.gm-codigo in browse br-digita
do:
    if not avail tt-digita
    then return.

    assign tt-digita.descricao = "".
    assign input browse br-digita tt-digita.gm-codigo.

    for first grup-maquina fields (gm-codigo descricao) no-lock
        where grup-maquina.gm-codigo = tt-digita.gm-codigo:
        assign tt-digita.descricao = trim(grup-maquina.descricao).
    end. /* for first grup-maquina */

    disp tt-digita.descricao with browse br-digita.
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    for first int_param_uep no-lock
        ...
        assign fi_cod_ccusto_fim:screen-value in frame f-pg-sel = int_param_uep.cod_ccusto_fim.
    end.
...
------------------------------------------------------------------------------*/
  ENABLE im-pg-cla im-pg-dig im-pg-imp im-pg-sel bt-executar bt-cancelar 
         bt-ajuda 
  ...
  DISPLAY fi_cod_empresa fi_dat_aquis_ini fi_dat_aquis_fim fi_cod_cta_ini 
          ...
          fi_num_seq_bem_fim fi-gm-codigo-ini fi-gm-codigo-fim fi_cod_estab_ini 
  ...
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 IMAGE-7 IMAGE-8 
         IMAGE-9 IMAGE-10 IMAGE-11 IMAGE-12 IMAGE-13 IMAGE-14 fi_dat_aquis_ini 
         fi_dat_aquis_fim fi_cod_cta_ini fi_cod_cta_fim fi_num_bem_ini 
         fi_num_bem_fim fi_num_seq_bem_ini fi_num_seq_bem_fim fi-gm-codigo-ini 
         fi-gm-codigo-fim fi_cod_estab_ini fi_cod_estab_fim fi_cod_ccusto_ini 
         fi_cod_ccusto_fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    for each tt-digita no-lock:
        ...
        find first b-tt-digita where b-tt-digita.gm-codigo = tt-digita.gm-codigo and 
        ...
        if  not can-find(first grup-maquina where
                               grup-maquina.gm-codigo = tt-digita.gm-codigo
                               no-lock)
        and not can-find(first int_bem_pat_gm use-index ch-gm where
                               int_bem_pat_gm.gm-codigo = tt-digita.gm-codigo
                               no-lock)
        then.
        else next.

        assign browse br-digita:CURRENT-COLUMN = tt-digita.gm-codigo:HANDLE in browse br-digita.
        
        apply "MOUSE-SELECT-CLICK":U to im-pg-dig in frame f-relat.
        reposition br-digita to rowid r-tt-digita.
        
        run utp/ut-msgs.p (input "show", input 17567, input "Grupo de M†quina n∆o cadastrado").
        apply "ENTRY":U to tt-digita.gm-codigo in browse br-digita.
    ...
    and input frame f-pg-sel fi-gm-codigo-ini > input frame f-pg-sel fi-gm-codigo-fim
    then do:
         apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
         run utp/ut-msgs.p (input "show", input 17567, input "Grupo de M†quina inicial maior que final!").
         apply 'entry' to fi-gm-codigo-ini in frame f-pg-sel.
         return 'ADM-ERROR'.
    end. 
    ...
    assign tt-param.dat_aquis_ini    = input frame f-pg-sel fi_dat_aquis_ini  
           ...
           tt-param.gm-codigo-ini    = input frame f-pg-sel fi-gm-codigo-ini
           tt-param.gm-codigo-fim    = input frame f-pg-sel fi-gm-codigo-fim
    ...
    {include/i-rprun.i esp/cdp/escdp125rp.p}
