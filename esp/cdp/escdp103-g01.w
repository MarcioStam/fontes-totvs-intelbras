...
*******************************************************************************/
{include/i-prgvrs.i ESCDP103-G01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCDP103-G01 ESP}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod-cor bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod-cor 
...
DEFINE VARIABLE c_cod-cor AS CHARACTER FORMAT "x(20)":U 
...
DEFINE FRAME d-vapara
     c_cod-cor AT ROW 1.17 COL 23.43 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Cor */
DO:
FIND int-cor NO-LOCK
    WHERE int-cor.cod-cor EQ INPUT FRAME {&FRAME-NAME} c_cod-cor NO-ERROR.

IF NOT AVAILABLE int-cor THEN DO:
    {utp/ut-table.i mgesp int-cor 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT "Cor").
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-cor).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Cor */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod-cor 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod-cor bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCDP103-G01" "1.00.00.000"}
