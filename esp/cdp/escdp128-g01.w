...
&Scoped-Define ENABLED-OBJECTS rt-button RECT-1 c_nat-operacao bt-ajuda ~
bt-ok bt-cancela 
...
DEFINE VARIABLE c_nat-operacao AS CHARACTER FORMAT "x(06)":U 
     LABEL "Nat Opera‡Æo" 
     VIEW-AS FILL-IN 
...
DO:
FIND int-natur-gener-cte NO-LOCK
    WHERE int-natur-gener-cte.nat-operacao EQ INPUT FRAME {&FRAME-NAME} c_nat-operacao NO-ERROR.

IF NOT AVAILABLE int-natur-gener-cte THEN DO:
    {utp/ut-table.i mgesp int-natur-gener-cte 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-natur-gener-cte).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_nat-operacao 
      WITH FRAME d-vapara.
  ENABLE rt-button RECT-1 c_nat-operacao bt-ajuda bt-ok bt-cancela 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
