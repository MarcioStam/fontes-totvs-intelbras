...
{include/i-prgvrs.i ESCDP085ARP 2.00.00.000}
...
PROCEDURE pi-executar:
    FOR EACH tt-digita WHERE
        ...
        FOR FIRST int-def-sigla-transp 
             WHERE int-def-sigla-transp.cod-transp     = tt-digita.cod-transp
               AND int-def-sigla-transp.cod-estabel    = tt-digita.cod-estabel   
               AND int-def-sigla-transp.estado         = tt-digita.estado        
               AND int-def-sigla-transp.cep-inicial    = tt-digita.cep-inicial   
               AND int-def-sigla-transp.cep-final      = tt-digita.cep-final 
               AND int-def-sigla-transp.cidade         = tt-digita.cidade EXCLUSIVE-LOCK:

            RUN pi-acompanhar IN h-acomp (INPUT "Transportadora: " + STRING(int-def-sigla-transp.cod-transp)).

            DELETE int-def-sigla-transp.
