...
DEFINE TEMP-TABLE tt-emitente-rast NO-UNDO LIKE cliente-rast
       field r-rowid as rowid.
...
*******************************************************************************/
{include/i-prgvrs.i ESCPP098 2.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          ESCPP098
&GLOBAL-DEFINE Version          2.00.00.000

&GLOBAL-DEFINE Folder           YES
&GLOBAL-DEFINE InitialPage      1
&GLOBAL-DEFINE FolderLabels     Per¡odos
...
&GLOBAL-DEFINE hDBOParent       h-esboad098
&GLOBAL-DEFINE DBOParentTable   emitente
&GLOBAL-DEFINE DBOParentDestroy NO

&GLOBAL-DEFINE ttSon1           tt-emitente-rast
&GLOBAL-DEFINE hDBOSon1         h-boes921
&GLOBAL-DEFINE DBOSon1Table     cliente-rast
...
&Scoped-define INTERNAL-TABLES tt-emitente-rast

/* Definitions for BROWSE brSon1                                        */
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-emitente-rast.data-ini ~
tt-emitente-rast.data-fim tt-emitente-rast.usuario ~
tt-emitente-rast.data-inclusao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brSon1 
&Scoped-define QUERY-STRING-brSon1 FOR EACH tt-emitente-rast NO-LOCK
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY brSon1 FOR EACH tt-emitente-rast NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brSon1 tt-emitente-rast
&Scoped-define FIRST-TABLE-IN-QUERY-brSon1 tt-emitente-rast
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _STRUCTURED
  QUERY brSon1 NO-LOCK DISPLAY
      tt-emitente-rast.data-ini FORMAT "99/99/9999":U WIDTH 18
      tt-emitente-rast.data-fim FORMAT "99/99/9999":U WIDTH 18
      tt-emitente-rast.usuario FORMAT "x(12)":U WIDTH 16
      tt-emitente-rast.data-inclusao FORMAT "99/99/9999 HH:MM":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage0
     ...
     btHelp AT ROW 1.13 COL 86.86 HELP
          "Ajuda"
     tt-emitente.cod-emitente AT ROW 3 COL 26.14 COLON-ALIGNED WIDGET-ID 4
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     tt-emitente.nome-emit AT ROW 4 COL 26.14 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 44 BY .88
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
      TABLE: tt-emitente-rast T "?" NO-UNDO mgesp cliente-rast
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brSon1
     _TblList          = "Temp-Tables.tt-emitente-rast"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-emitente-rast.data-ini
"data-ini" ? ? "date" ? ? ? ? ? ? no ? no no "18" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-emitente-rast.data-fim
"data-fim" ? ? "date" ? ? ? ? ? ? no ? no no "18" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-emitente-rast.usuario
"usuario" ? ? "character" ? ? ? ? ? ? no ? no no "16" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-emitente-rast.data-inclusao
"data-inclusao" ? "99/99/9999 HH:MM" "datetime" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brSon1 */
&ANALYZE-RESUME
...
DO:
    {masterdetail/addson.i &ProgramSon="esp/cpp/escpp098a.w"
                           &PageNumber="1"}
END.
...
DO:
    {masterdetail/copyson.i &ProgramSon="esp/cpp/escpp098a.w"
                            &PageNumber="1"}
END.
...
DO:
    {masterdetail/updateson.i &ProgramSon="esp/cpp/escpp098a.w"
                              &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR i-cod-emitente AS INTEGER LABEL "Cliente" NO-UNDO.

    DEFINE FRAME fGoToRecord
        i-cod-emitente view-as fill-in size 10 by 0.88 AT ROW 1.21 COL 17.72 COLON-ALIGNED 
...
/*tech1139 - FO 1338.917 - 10/07/2006  */
    ...
    {utp/ut-liter.i "V _Para_Cliente"}
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOParent}) OR
       ...
       {&hDBOParent}:FILE-NAME <> "esbo/esboad098.p":U THEN DO:
        {btb/btb008za.i1 esbo/esboad098.p YES}
        {btb/btb008za.i2 esbo/esboad098.p '' {&hDBOParent}} 
    END.
    ...
    IF NOT VALID-HANDLE({&hDBOSon1}) OR 
       ...
       {&hDBOSon1}:FILE-NAME <> "esbo/boes921.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes921.p YES}
        {btb/btb008za.i2 esbo/boes921.p '' {&hDBOSon1}} 
    END.
