...
*******************************************************************************/
{include/i-prgvrs.i ESCPP105D 2.00.00.000}
{include/i-license-manager.i ESCPP105D FGL}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCPP105D
...
&GLOBAL-DEFINE page0Widgets   btQueryJoins btReportsJoins btExit btHelp ~
                              ...
                              c-sigla btConfigImpr btReimpressao c-ns
...
DEFINE VARIABLE fc-ns         AS CHARACTER NO-UNDO.
DEFINE VARIABLE i-cont-ns     AS INTEGER   NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS c-it-codigo c-sigla c-ns btReimpressao ~
btQueryJoins btReportsJoins btExit btHelp c-desc-item c-desc-modelo ~
c-desc-sigla c-cod-estabel c-desc-estabelec cb-modelo btConfigImpr btCancel ~
btHelp2 fiPrinter rtToolBar-2 rtToolBar RECT-1 RECT-2 
&Scoped-Define DISPLAYED-OBJECTS c-it-codigo c-sigla c-ns c-desc-item ~
...
DEFINE FRAME fpage0
     ...
     c-ns AT ROW 7.13 COL 18.72 COLON-ALIGNED WIDGET-ID 44
...
ON RETURN OF c-ns IN FRAME fpage0 /* Numero Serie */
DO:

    ASSIGN fc-ns = INPUT FRAME fPage0 c-ns.

    assign input frame fPage0 c-ns:screen-value = "". 
    ...
    THEN DO:
        RUN utp/ut-msgs.p (INPUT 'show':U,
                           INPUT 17006,
                           INPUT 'Numero de serie n∆o encontrado.'
                                 + '~~' + 'Verifique se o NS digitado esta correto').
        RETURN "NOK":U.
    END.
    ELSE DO:
        ...
        THEN DO:
           RUN utp/ut-msgs.p (INPUT 'show':U,
                              INPUT 17006,
                              INPUT 'Item do NS (' + STRING(num-serie.it-codigo) + ') diferente do item selecionado'
                                    + '~~' + 'O Item selecionado Ç diferente do item do NS').
           RETURN "NOK":U.
        END.
        ...
        THEN DO:        
           RUN utp/ut-msgs.p (INPUT 'show':U,
                              INPUT 17006,
                              INPUT 'Este numero de serie ' + num-serie.n-serie + ' foi vinculado ou impresso em outra DUO.'
                                    + '~~' + 
                                    'Esse numero de serie foi vinculado com o NS: ' + num-serie.char-2 ).
           RETURN "NOK":U.
        END.

        FIND FIRST tt-lista-ns WHERE
                   tt-lista-ns.num-serie = fc-ns
                   NO-LOCK NO-ERROR.

        IF AVAIL tt-lista-ns 
        THEN DO:
            RUN utp/ut-msgs.p (INPUT 'show':U,
                               INPUT 17006,
                               INPUT 'Numero de serie j† foi bipado.'
                                     + '~~' + 'O numero de serie foi bipado').
            RETURN "NOK":U.
        END.


        IF NOT AVAIL tt-lista-ns 
        THEN DO:

            CREATE tt-lista-ns.
            ASSIGN tt-lista-ns.num-serie = num-serie.n-serie.

            ASSIGN i-cont-ns = i-cont-ns + 1.
        END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-ns wWindow
ON VALUE-CHANGED OF c-ns IN FRAME fpage0 /* Numero Serie */
DO:
/*
   //    assign input frame fPage0 c-ns:SCREEN-VALUE = SUBSTRING(INPUT FRAME fPage0 c-ns,1,12).

    IF LENGTH(INPUT FRAME fPage0 c-ns) = 12  
    THEN DO:
            ...
            THEN DO:
                RUN utp/ut-msgs.p (INPUT 'show':U,
                                   ...
                                   INPUT 'Mac Address j† vinculado a um Numero de Serie.'
                                         + '~~' + 'O Mac ' + string(fc-ns) + ' j† vinculado ao NS ' 
...
ASSIGN c-it-codigo:SCREEN-VALUE IN FRAME fPage0 = p-it-codigo.
...
*/
    ...
    FOR EACH tt-lista-ns:
        FIND FIRST b-tt-lista-ns WHERE
                   b-tt-lista-ns.num-serie <> tt-lista-ns.num-serie
                   NO-ERROR.

        IF AVAIL b-tt-lista-ns 
        THEN DO:
            FIND FIRST num-serie WHERE
                       num-serie.n-serie = b-tt-lista-ns.num-serie
                       EXCLUSIVE-LOCK NO-ERROR.

            IF AVAIL num-serie 
            THEN ASSIGN num-serie.char-2 = tt-lista-ns.num-serie.
        END.                 
    END.
