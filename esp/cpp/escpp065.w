...
*******************************************************************************/
{include/i-prgvrs.i ESCPP065 2.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          ESCPP065
&GLOBAL-DEFINE Version          2.00.00.000

&GLOBAL-DEFINE Folder           YES
&GLOBAL-DEFINE InitialPage      1
&GLOBAL-DEFINE FolderLabels     Modelos
...
&GLOBAL-DEFINE ttSon1           tt-item-mod-etiq
&GLOBAL-DEFINE hDBOSon1         h-boes613
&GLOBAL-DEFINE DBOSon1Table     item-mod-etiq
&GLOBAL-DEFINE DBOSon1Destroy   YES

&GLOBAL-DEFINE page0Fields      tt-item.it-codigo tt-item.desc-item
...
DEFINE VARIABLE c-desc-modelo AS CHARACTER   NO-UNDO.
...
&Scoped-define INTERNAL-TABLES tt-item-mod-etiq

/* Definitions for BROWSE brSon1                                        */
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-item-mod-etiq.cod-modelo ~
f-desc-modelo(tt-item-mod-etiq.cod-modelo) @ c-desc-modelo ~
tt-item-mod-etiq.padrao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brSon1 
&Scoped-define QUERY-STRING-brSon1 FOR EACH tt-item-mod-etiq NO-LOCK
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY brSon1 FOR EACH tt-item-mod-etiq NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brSon1 tt-item-mod-etiq
&Scoped-define FIRST-TABLE-IN-QUERY-brSon1 tt-item-mod-etiq
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD f-desc-modelo wMasterDetail 
FUNCTION f-desc-modelo RETURNS CHARACTER
  ( INPUT p-cod-modelo AS INT )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _STRUCTURED
  QUERY brSon1 NO-LOCK DISPLAY
      tt-item-mod-etiq.cod-modelo FORMAT "999":U WIDTH 8
      f-desc-modelo(tt-item-mod-etiq.cod-modelo) @ c-desc-modelo COLUMN-LABEL "Descri‡Æo" FORMAT "X(80)":U
            WIDTH 61.86
      tt-item-mod-etiq.padrao FORMAT "Sim/NÆo":U WIDTH 7
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage0
     ...
     btHelp AT ROW 1.13 COL 86.86 HELP
          "Ajuda"
     tt-item.it-codigo AT ROW 3 COL 26 COLON-ALIGNED WIDGET-ID 4
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     tt-item.desc-item AT ROW 4 COL 26 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 44 BY .88
...
DEFINE FRAME fPage1
     ...
     bt-atribuir AT ROW 10.33 COL 42 WIDGET-ID 26
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 6.71
         SIZE 84.43 BY 10.63
         FONT 1 WIDGET-ID 100.
...
/* Query rebuild information for BROWSE brSon1
     _TblList          = "Temp-Tables.tt-item-mod-etiq"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-item-mod-etiq.cod-modelo
"tt-item-mod-etiq.cod-modelo" ? ? "integer" ? ? ? ? ? ? no ? no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > "_<CALC>"
"f-desc-modelo(tt-item-mod-etiq.cod-modelo) @ c-desc-modelo" "Descri‡Æo" "X(80)" ? ? ? ? ? ? ? no ? no no "61.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-item-mod-etiq.padrao
"tt-item-mod-etiq.padrao" ? ? "logical" ? ? ? ? ? ? no ? no no "7" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brSon1 */
&ANALYZE-RESUME
...
ON MOUSE-SELECT-DOWN OF brSon1 IN FRAME fPage1
DO:
  IF AVAIL tt-item-mod-etiq THEN DO:
    IF DYNAMIC-FUNCTION("fnRetornaQtdeSelo" IN h-boes794,
                        INPUT tt-item-mod-etiq.cod-modelo) > 0 THEN
        ASSIGN bt-atribuir:SENSITIVE IN FRAME fPage1 = YES.
    ELSE
        ASSIGN bt-atribuir:SENSITIVE = NO.

    FIND modelo-etiq NO-LOCK
        WHERE modelo-etiq.cod-modelo = tt-item-mod-etiq.cod-modelo NO-ERROR.
    IF  AVAIL modelo-etiq
    AND modelo-etiq.tipo = 6 /*Outras*/ THEN
        ASSIGN bt-atribuir:SENSITIVE IN FRAME fPage1 = YES.
END.
ELSE
    ASSIGN bt-atribuir:SENSITIVE = NO.
...
DO:
    IF NOT DYNAMIC-FUNCTION("fnVerificaImgQtde" IN h-boes794,
                            INPUT tt-item-mod-etiq.it-codigo,
                            INPUT tt-item-mod-etiq.cod-modelo) THEN
        ASSIGN tt-item-mod-etiq.cod-modelo:BGCOLOR IN BROWSE {&page1Browse} = 12
               c-desc-modelo              :BGCOLOR IN BROWSE {&page1Browse} = 12
               tt-item-mod-etiq.padrao    :BGCOLOR IN BROWSE {&page1Browse} = 12.
...
&Scoped-define SELF-NAME bt-atribuir
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-atribuir wMasterDetail
ON CHOOSE OF bt-atribuir IN FRAME fPage1 /* Imagem(s) */
DO:
    DEF VAR l-habilita-image AS LOG INIT NO.
        
    IF  AVAIL tt-item-mod-etiq THEN DO:

        IF DYNAMIC-FUNCTION("fnRetornaQtdeSelo" IN h-boes794,
                            INPUT tt-item-mod-etiq.cod-modelo) > 0 THEN
            l-habilita-image = YES.
        ELSE
            l-habilita-image = NO.
        ...
        IF  NOT l-habilita-image THEN DO:
            RUN utp/ut-msgs.p ("show",
                               17006,
                               "Modelo nÆo permite v¡nculo de imagem.").
            RETURN NO-APPLY.
        END.


        RUN esp/cpp/escpp065b.w PERSISTENT SET h-escpp065b (INPUT tt-item.it-codigo:SCREEN-VALUE IN FRAME fPage0,
                                                            INPUT tt-item-mod-etiq.cod-modelo).
        
        RUN initializeInterface IN h-escpp065b.
        
        WAIT-FOR CLOSE OF h-escpp065b.
        IF VALID-HANDLE(h-escpp065b) THEN
            DELETE PROCEDURE h-escpp065b.
...
DO:
    {masterdetail/addson.i &ProgramSon="esp/cpp/escpp065a.w"
                           &PageNumber="1"}
...
DO:
    {masterdetail/copyson.i &ProgramSon="esp/cpp/escpp065a.w"
                            &PageNumber="1"}
END.
...
DO:
    ASSIGN i-qtde = DYNAMIC-FUNCTION("fnRetornaQtdeImg" IN h-boes794,
                                     INPUT tt-item-mod-etiq.it-codigo,
                                     INPUT tt-item-mod-etiq.cod-modelo).
    IF i-qtde <> 0 THEN DO:
        RUN utp/ut-msgs.p (INPUT "SHOW",
                           INPUT 17006,
                           INPUT "Elimina‡Æo inv lida~~Existe(m) " + STRING(i-qtde) +
                                 " imagem(s) vinculada(s) a esse Item/Modelo.  necess rio desvincular a(s) imagem(s) antes da elimina‡Æo.").
        RETURN "NOK".
    END.
    ELSE DO:
...
OR CHOOSE OF MENU-ITEM miSearch IN MENU mbMain DO:
    {method/zoomreposition.i &ProgramZoom="inzoom/z20in172.w"}
END.
...
DO:
    {masterdetail/updateson.i &ProgramSon="esp/cpp/escpp065a.w"
                              &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE c-it-codigo LIKE {&ttParent}.it-codigo NO-UNDO.
...
/*tech1139 - FO 1338.917 - 10/07/2006  */
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToRecord DO:
        ...
        RUN goToKey IN {&hDBOParent} (INPUT c-it-codigo).
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOParent}) OR
       ...
       {&hDBOParent}:FILE-NAME <> "inbo/boin172.p":U THEN DO:
        {btb/btb008za.i1 inbo/boin172.p YES}
        {btb/btb008za.i2 inbo/boin172.p '' {&hDBOParent}} 
    END.
    ...
    IF NOT VALID-HANDLE({&hDBOSon1}) OR 
       ...
       {&hDBOSon1}:FILE-NAME <> "esbo/boes613.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes613.p YES}
        {btb/btb008za.i2 esbo/boes613.p '' {&hDBOSon1}} 
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION f-desc-modelo wMasterDetail 
FUNCTION f-desc-modelo RETURNS CHARACTER
  ( INPUT p-cod-modelo AS INT ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
