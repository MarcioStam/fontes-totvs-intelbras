...
*******************************************************************************/
{include/i-prgvrs.i ESCPP166A 2.00.00.000}
{include/i-license-manager.i ESCPP166A FGL}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCPP166A
...
DEFINE INPUT PARAM p-escpp105 AS LOGICAL     NO-UNDO.
...
c-it-codigo c-desc-item c-desc-modelo cb-modelo fiPrinter i-nr-ord-produ 
...
DEFINE VARIABLE fi-ns-fim AS CHARACTER FORMAT "X(20)":U 
     VIEW-AS FILL-IN 
     SIZE 20 BY .88 NO-UNDO.

DEFINE VARIABLE fi-ns-ini AS CHARACTER FORMAT "X(20)":U 
...
DEFINE FRAME fpage0
     ...
     cb-reimp AT ROW 8.83 COL 15 COLON-ALIGNED WIDGET-ID 44
     ...
     btHelp AT ROW 1.13 COL 86.72 HELP
          "Ajuda"
     fi-ns-ini AT ROW 3.83 COL 21.14 COLON-ALIGNED WIDGET-ID 2
     fi-ns-fim AT ROW 3.83 COL 56.14 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     ...
     cb-modelo AT ROW 6.29 COL 15 COLON-ALIGNED WIDGET-ID 32
     btOK AT ROW 20.79 COL 2
     btCancel AT ROW 20.79 COL 13
     btHelp2 AT ROW 20.79 COL 88.43
     fiPrinter AT ROW 7.21 COL 17 NO-LABEL WIDGET-ID 24 NO-TAB-STOP 
     i-nr-ord-produ AT ROW 8.83 COL 68 COLON-ALIGNED WIDGET-ID 52
...
DO:
    ...
    DO WITH FRAME fPage0:
        ...
        IF CAN-FIND (FIRST tt-prog-ponto
            ...
            ASSIGN i-nr-ord-produ:VISIBLE   IN FRAME fpage0 = YES
                   i-nr-ord-produ:SENSITIVE IN FRAME fpage0 = YES.
        END.
        ELSE DO:
            ASSIGN i-nr-ord-produ:VISIBLE   IN FRAME fpage0 = NO
                   i-nr-ord-produ:SENSITIVE IN FRAME fpage0 = NO.
...
ON MOUSE-SELECT-DBLCLICK OF i-nr-ord-produ IN FRAME fpage0 /* Ordem Prod */
DO:
    APPLY "F5" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-ord-produ wWindow
ON RETURN OF i-nr-ord-produ IN FRAME fpage0 /* Ordem Prod */
DO:
    APPLY "LEAVE" TO SELF.
END.
...
END.
ELSE DO:
    FOR EACH etiq-coletiva NO-LOCK
        WHERE etiq-coletiva.cod-etiqueta >= INPUT FRAME fPage0 fi-ns-ini
        AND   etiq-coletiva.cod-etiqueta <= INPUT FRAME fPage0 fi-ns-fim:
    
        CREATE tt-lista-ns.
        ASSIGN tt-lista-ns.num-serie = etiq-coletiva.cod-etiqueta.
    END.
END.

RUN pi-acompanhar IN h-acomp (INPUT "Imprimindo Etiquetas").

IF p-escpp105 THEN DO:
    /* Banda KU escpp105 para n∆o imprimir a data */
    RUN piAlteraEscpp105 IN h-esapi016 (INPUT YES).    
END.
