...
END.  
    ...
    FOR EACH tt-prog-ponto:
        FOR EACH ord-prod NO-LOCK
            WHERE ord-prod.cod-estabel = ENTRY(1,tt-prog-ponto.conteudo)
              AND ord-prod.dt-inicio  >= TODAY
              AND ord-prod.dt-inicio  <= TODAY + INT(ENTRY(2,tt-prog-ponto.conteudo))
              AND ord-prod.estado < 7:

            RUN pi-acompanhar in h-acomp (input "Integracao Ord.Prod: " + STRING(ord-prod.nr-ord-prod)).
