...
*******************************************************************************/
{include/i-prgvrs.i ESCPP079 1.00.00.000}
...
&GLOBAL-DEFINE Program          ESCPP079
...
&GLOBAL-DEFINE hDBOParent       h-boes659
&GLOBAL-DEFINE DBOParentTable   origem-mqa
&GLOBAL-DEFINE DBOParentDestroy YES

&GLOBAL-DEFINE ttSon1           tt-item-falha
&GLOBAL-DEFINE hDBOSon1         h-boes655
&GLOBAL-DEFINE DBOSon1Table     item-falha
&GLOBAL-DEFINE DBOSon1Destroy   YES

&GLOBAL-DEFINE page0Fields      tt-origem-mqa.origem-falha tt-origem-mqa.descricao
...
DEFINE VARIABLE c-desc-falha AS CHARACTER   NO-UNDO.
...
&Scoped-define INTERNAL-TABLES tt-item-falha

/* Definitions for BROWSE brSon1                                        */
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-item-falha.cod-falha ~
f-desc-falha(tt-item-falha.cod-falha) @ c-desc-falha 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brSon1 
&Scoped-define QUERY-STRING-brSon1 FOR EACH tt-item-falha NO-LOCK
&Scoped-define OPEN-QUERY-brSon1 OPEN QUERY brSon1 FOR EACH tt-item-falha NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brSon1 tt-item-falha
&Scoped-define FIRST-TABLE-IN-QUERY-brSon1 tt-item-falha
...
tt-origem-mqa.descricao 
&Scoped-define ENABLED-TABLES tt-origem-mqa
&Scoped-define FIRST-ENABLED-TABLE tt-origem-mqa
&Scoped-Define ENABLED-OBJECTS rtToolBar rtParent btFirst btPrev btNext ~
btLast btGoTo btSearch bt-faixa btQueryJoins btReportsJoins btExit btHelp 
&Scoped-Define DISPLAYED-FIELDS tt-origem-mqa.origem-falha ~
tt-origem-mqa.descricao 
&Scoped-define DISPLAYED-TABLES tt-origem-mqa
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD f-desc-falha wMasterDetail 
FUNCTION f-desc-falha RETURNS CHARACTER
  ( INPUT p-cod-falha AS INT )  FORWARD.
...
DEFINE BUTTON bt-faixa 
     IMAGE-UP FILE "image/im-ran5.bmp":U
     LABEL "Query Joins" 
     SIZE 4 BY 1.25
     FONT 4.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _STRUCTURED
  QUERY brSon1 NO-LOCK DISPLAY
      tt-item-falha.cod-falha FORMAT ">>9":U WIDTH 7.43
      f-desc-falha(tt-item-falha.cod-falha) @ c-desc-falha COLUMN-LABEL "Descri‡Æo" FORMAT "x(60)":U
            WIDTH 20
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage0
     ...
     btSearch AT ROW 1.13 COL 21.57 HELP
          "Pesquisa"
     bt-faixa AT ROW 1.13 COL 69.14 HELP
     ...
     btHelp AT ROW 1.13 COL 86.86 HELP
          "Ajuda"
     tt-origem-mqa.origem-falha AT ROW 3 COL 16 COLON-ALIGNED WIDGET-ID 14
          VIEW-AS FILL-IN 
          SIZE 4 BY .88
     tt-origem-mqa.descricao AT ROW 3 COL 20.57 COLON-ALIGNED NO-LABEL WIDGET-ID 12
          VIEW-AS FILL-IN 
          SIZE 62 BY .88
...
/* Query rebuild information for BROWSE brSon1
     _TblList          = "Temp-Tables.tt-item-falha"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-item-falha.cod-falha
"tt-item-falha.cod-falha" ? ? "integer" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > "_<CALC>"
"f-desc-falha(tt-item-falha.cod-falha) @ c-desc-falha" "Descri‡Æo" "x(60)" ? ? ? ? ? ? ? no ? no no "20" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brSon1 */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-faixa wMasterDetail
ON CHOOSE OF bt-faixa IN FRAME fPage0 /* Query Joins */
DO:
    RUN esp/cpp/escpp079b.w.
END.
...
DO:
    {masterdetail/addson.i &ProgramSon="esp/cpp/escpp079a.w"
                           &PageNumber="1"}
END.
...
DO:
    ...
    ASSIGN i-origem = INPUT FRAME fpage0 tt-origem-mqa.origem-falha.

    RUN esp/cpp/escpp079c.w (INPUT i-origem).
END.
...
OR CHOOSE OF MENU-ITEM miSearch IN MENU mbMain DO:    
    {method/zoomreposition.i &ProgramZoom="eszoom/z01es659.w"}
END.
...
DO:
    {masterdetail/updateson.i &ProgramSon="esp/cpp/escpp079a.w"
                              &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN bt-faixa:SENSITIVE IN FRAME fPage0 = TRUE.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE i-origem LIKE {&DBOParentTable}.origem-falha NO-UNDO.
    
    DEFINE FRAME fGoToRecord
        i-origem          AT ROW 1.21 COL 17.72 COLON-ALIGNED VIEW-AS FILL-IN SIZE 4 BY .88
        btGoToOK          AT ROW 3.63 COL 2.14
        btGoToCancel      AT ROW 3.63 COL 13
        rtGoToButton      AT ROW 3.38 COL 1
        SPACE(0.28)
        WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
             THREE-D SCROLLABLE TITLE "V  Para Origem Falha" FONT 1
...
/*tech1139 - FO 1338.917 - 10/07/2006  */
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToRecord DO:
        ...
        RUN goToKey IN {&hDBOParent} (INPUT i-origem ).
        IF RETURN-VALUE = "NOK":U THEN DO:
            RUN utp/ut-msgs.p (INPUT "SHOW":U, INPUT 2, INPUT "Origem falha":U).
    ...
    ENABLE i-origem btGoToOK btGoToCancel 
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOParent}) OR
       ...
       {&hDBOParent}:FILE-NAME <> "esbo/boes659.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes659.p YES}
        {btb/btb008za.i2 esbo/boes659.p '' {&hDBOParent}}
    END.
    ...
    IF NOT VALID-HANDLE({&hDBOSon1}) OR 
       ...
       {&hDBOSon1}:FILE-NAME <> "esbo/boes655.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes655.p YES}
        {btb/btb008za.i2 esbo/boes655.p '' {&hDBOSon1}} 
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION f-desc-falha wMasterDetail 
FUNCTION f-desc-falha RETURNS CHARACTER
  ( INPUT p-cod-falha AS INT ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
