...
*******************************************************************************/
{include/i-prgvrs.i ESCPP101 2.00.00.000}
{include/i-license-manager.i ESCPP101 FGL}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCPP101
...
&GLOBAL-DEFINE page0Widgets   btQueryJoins btReportsJoins btExit btHelp ~
                              ...
                              c-sigla i-qtd btConfigImpr bt-imp-teste
...
DEFINE VARIABLE h-esapi024  AS HANDLE      NO-UNDO.
DEF STREAM s-import.
...
&Scoped-Define ENABLED-OBJECTS c-width cb-dpi tg-web btQueryJoins ~
btReportsJoins btExit btHelp c-it-codigo c-desc-item c-desc-modelo c-sigla ~
c-desc-sigla c-cod-estabel c-desc-estabelec i-qtd cb-modelo btConfigImpr ~
btCancel fiPrinter btHelp2 bt-imp-teste rtToolBar-2 rtToolBar RECT-1 RECT-2 ~
RECT-3 rt-cor c-height 
&Scoped-Define DISPLAYED-OBJECTS c-width cb-dpi tg-web c-it-codigo ~
c-desc-item c-desc-modelo c-sigla c-desc-sigla c-cod-estabel ~
c-desc-estabelec i-qtd cb-modelo fiPrinter c-height 
...
DEFINE VARIABLE cb-dpi AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 6 
     LABEL "Dpi" 
     VIEW-AS COMBO-BOX INNER-LINES 4
     LIST-ITEM-PAIRS "152",6,
                     "203",8,
                     "300",12,
                     "600",24
     DROP-DOWN-LIST
...
DEFINE VARIABLE c-height AS INTEGER FORMAT ">>9":U INITIAL 4 
     LABEL "Altura Etiqueta cm" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE c-width AS INTEGER FORMAT ">>9":U INITIAL 6 
     LABEL "Largura Etiqueta cm" 
     VIEW-AS FILL-IN 
     SIZE 4.43 BY .88 NO-UNDO.
...
DEFINE VARIABLE tg-web AS LOGICAL INITIAL no 
     LABEL "Visualizar no navegador web?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME fpage0
     c-width AT ROW 9.13 COL 40 COLON-ALIGNED WIDGET-ID 48
     cb-dpi AT ROW 9.13 COL 18.72 COLON-ALIGNED WIDGET-ID 46
     tg-web AT ROW 8.13 COL 70 WIDGET-ID 44
     ...
     btConfigImpr AT ROW 8.08 COL 65.29 HELP
          "Configuraá∆o da impressora" WIDGET-ID 22
     btCancel AT ROW 22.54 COL 1.86
     fiPrinter AT ROW 8.13 COL 20.72 NO-LABEL WIDGET-ID 24 NO-TAB-STOP 
     btHelp2 AT ROW 22.54 COL 88.43
     bt-imp-teste AT ROW 22.54 COL 45.14 WIDGET-ID 34
     c-height AT ROW 9.13 COL 58.72 COLON-ALIGNED WIDGET-ID 50
     "Impressora:" VIEW-AS TEXT
          SIZE 8 BY .63 AT ROW 8.17 COL 12.57 WIDGET-ID 26
          FONT 1
     rtToolBar-2 AT ROW 1 COL 1
     rtToolBar AT ROW 22.33 COL 1
     RECT-1 AT ROW 2.75 COL 1 WIDGET-ID 2
     RECT-2 AT ROW 6.92 COL 2 WIDGET-ID 18
     RECT-3 AT ROW 10.5 COL 2 WIDGET-ID 20
     im-imagem AT ROW 10.79 COL 3 WIDGET-ID 28
     rt-cor AT ROW 20.5 COL 3 WIDGET-ID 38
...
&Scoped-define SELF-NAME c-height
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-height wWindow
ON LEAVE OF c-height IN FRAME fpage0 /* Altura Etiqueta cm */
DO:
  IF INPUT FRAME fPage0 c-height = 0 
  THEN DO:
      RUN utp/ut-msgs.p (INPUT 'show':U,
                         INPUT 17006,
                         INPUT 'Valor informado errado.'
                               + '~~' + 'Obrigat¢rio informar um valor em CM diferente de 0').
      return no-apply.
  END.  
END.
...
&Scoped-define SELF-NAME c-width
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-width wWindow
ON LEAVE OF c-width IN FRAME fpage0 /* Largura Etiqueta cm */
DO:
  IF INPUT FRAME fPage0 c-width = 0 
  THEN DO:
      RUN utp/ut-msgs.p (INPUT 'show':U,
                         INPUT 17006,
                         INPUT 'Valor informado errado.'
...
&Scoped-define SELF-NAME tg-web
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-web wWindow
ON VALUE-CHANGED OF tg-web IN FRAME fpage0 /* Visualizar no navegador web? */
DO:
  
    IF INPUT FRAME fPage0 tg-web = YES 
    THEN ASSIGN cb-dpi:SENSITIVE     IN FRAME fPage0 = YES
                cb-dpi:SCREEN-VALUE  IN FRAME fPage0 = string(6)
                c-width:SENSITIVE    IN FRAME fPage0 = YES
                c-height:SENSITIVE   IN FRAME fPage0 = YES.
    ELSE ASSIGN cb-dpi:SENSITIVE     IN FRAME fPage0 = NO
                c-width:SENSITIVE    IN FRAME fPage0 = NO
                c-height:SENSITIVE   IN FRAME fPage0 = NO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
ASSIGN tg-web:CHECKED        IN FRAME fPage0 = NO
       tg-web:SENSITIVE      IN FRAME fPage0 = NO
       c-width:SCREEN-VALUE  IN FRAME fPage0 = "6"
       c-width:SENSITIVE     IN FRAME fPage0 = NO
       c-height:SCREEN-VALUE IN FRAME fPage0 = "4"
       c-height:SENSITIVE    IN FRAME fPage0 = NO.
...
RUN esapi/esapi024.p PERSISTENT SET h-esapi024.
...
RUN piImpressao IN h-esapi024 (INPUT INPUT FRAME fPage0 c-it-codigo,
                               INPUT INPUT FRAME fPage0 cb-modelo,
                               INPUT INPUT FRAME fPage0 c-sigla,
                               INPUT INPUT FRAME fPage0 i-qtd,
                               INPUT i-qtd-embalag,
...
IF RETURN-VALUE <> "OK":U THEN DO:
    ...
    RUN piRetornaErros IN h-esapi024 (OUTPUT TABLE tt-erro).

    RUN pi-finalizar IN h-acomp.

    RUN cdp/cd0666.w (INPUT TABLE tt-erro).

    DELETE PROCEDURE h-esapi024.
...
IF l-arquivo = YES AND
   INPUT frame fPage0 tg-web = YES
THEN DO:
   RUN piweb.
END.
...
------------------------------------------------------------------------------*/
    ...
    IF cAuxFile = "":U THEN
        ASSIGN fiPrinter = cPrinter + ":":U + cLayout
               l-arquivo = NO
               tg-web:CHECKED   IN FRAME fPage0 = NO
               tg-web:SENSITIVE IN FRAME fPage0 = NO.
    ELSE
        ASSIGN fiPrinter = cAuxFile
               l-arquivo = YES
               tg-web:SENSITIVE IN FRAME fPage0 = YES.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piweb wWindow 
PROCEDURE piweb :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
   ...
   DEF VAR c-arquivo AS LONGCHAR NO-UNDO.

   COPY-LOB FROM FILE STRING(INPUT frame fPage0 fiPrinter:SCREEN-VALUE) TO c-arquivo NO-ERROR.
   ...
   chWebAppl:Navigate("http://labelary.com/viewer.html?density=" 
                     + STRING(INPUT frame fPage0 cb-dpi) 
                     + "&width="
                     + STRING(INPUT frame fPage0 c-width:SCREEN-VALUE)
                     + "&height="
                     + STRING(INPUT frame fPage0 c-height:SCREEN-VALUE)
                     + "&units=inches&index=0&zpl=" + string(c-arquivo)).
