...
*******************************************************************************/
{include/i-prgvrs.i ESCPP127 2.00.00.000}
{include/i-license-manager.i ESCPP124 FGL}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESCPP127
...
&GLOBAL-DEFINE page0Widgets    btQueryJoins btReportsJoins btExit btHelp ~
                              btOK btCancel btHelp2 c-it-codigo cb-modelo i-num-po ~
                              br-dun fi-cod-ean i-qtd btConfigImpr fi-lei-info
...
DEF TEMP-TABLE tt-item-dun LIKE item-dun.
...
&Scoped-define BROWSE-NAME br-dun

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES tt-item-dun

/* Definitions for BROWSE br-dun                                        */
&Scoped-define FIELDS-IN-QUERY-br-dun tt-item-dun.qtd-emb tt-item-dun.cod-dun   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-dun   
&Scoped-define SELF-NAME br-dun
&Scoped-define QUERY-STRING-br-dun FOR EACH tt-item-dun NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-dun OPEN QUERY {&SELF-NAME} FOR EACH tt-item-dun NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-dun tt-item-dun
&Scoped-define FIRST-TABLE-IN-QUERY-br-dun tt-item-dun
...
&Scoped-Define ENABLED-OBJECTS br-dun fi-lei-info btQueryJoins ~
btReportsJoins btExit btHelp c-it-codigo c-desc-item c-desc-modelo i-qtd ~
i-num-po cb-modelo btConfigImpr btOK btCancel btHelp2 fiPrinter rtToolBar-2 ~
rtToolBar RECT-1 RECT-2 RECT-3 rt-cor 
&Scoped-Define DISPLAYED-OBJECTS fi-lei-info fi-cod-ean c-it-codigo ~
c-desc-item c-desc-modelo i-qtd i-num-po cb-modelo fiPrinter 
...
DEFINE VARIABLE fi-cod-ean AS CHARACTER FORMAT "X(256)":U 
     LABEL "Cod.EAN" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE i-num-po AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "N¯ PO" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-dun FOR 
      tt-item-dun SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE br-dun
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-dun wWindow _FREEFORM
  QUERY br-dun NO-LOCK DISPLAY
      tt-item-dun.qtd-emb FORMAT "->,>>>,>>9":U
tt-item-dun.cod-dun FORMAT "x(14)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 58 BY 7.25
...
DEFINE FRAME fpage0
     br-dun AT ROW 8.5 COL 20.86 WIDGET-ID 200
     fi-lei-info AT ROW 16.13 COL 27.14 COLON-ALIGNED NO-LABEL WIDGET-ID 46
     ...
     btHelp AT ROW 1.13 COL 94.57 HELP
          "Ajuda"
     fi-cod-ean AT ROW 2.75 COL 18.72 COLON-ALIGNED WIDGET-ID 44
     c-it-codigo AT ROW 3.67 COL 18.72 COLON-ALIGNED WIDGET-ID 4
     c-desc-item AT ROW 3.67 COL 32.72 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     c-desc-modelo AT ROW 4.58 COL 32.72 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     i-qtd AT ROW 6.13 COL 18.72 COLON-ALIGNED WIDGET-ID 10
     i-num-po AT ROW 6.13 COL 48.72 COLON-ALIGNED WIDGET-ID 48
     cb-modelo AT ROW 4.58 COL 18.72 COLON-ALIGNED WIDGET-ID 32 NO-TAB-STOP 
     btConfigImpr AT ROW 6.96 COL 65.29 HELP
          "Configuraá∆o da impressora" WIDGET-ID 22
     btOK AT ROW 17.83 COL 2
     btCancel AT ROW 17.83 COL 12
     btHelp2 AT ROW 17.83 COL 88.43
     fiPrinter AT ROW 7.04 COL 20.72 NO-LABEL WIDGET-ID 24 NO-TAB-STOP 
     "Impressora:" VIEW-AS TEXT
          SIZE 8 BY .63 AT ROW 7.21 COL 12.57 WIDGET-ID 26
          FONT 1
     rtToolBar-2 AT ROW 1 COL 1
     rtToolBar AT ROW 17.63 COL 1
     RECT-1 AT ROW 2.54 COL 2 WIDGET-ID 2
     RECT-2 AT ROW 5.92 COL 2 WIDGET-ID 18
     RECT-3 AT ROW 8.33 COL 2 WIDGET-ID 20
     rt-cor AT ROW 16 COL 3 WIDGET-ID 38
...
/* SETTINGS FOR FRAME fpage0
   FRAME-NAME Custom                                                    */
/* BROWSE-TAB br-dun 1 fpage0 */
ASSIGN 
       c-it-codigo:READ-ONLY IN FRAME fpage0        = TRUE.

/* SETTINGS FOR FILL-IN fi-cod-ean IN FRAME fpage0
   NO-ENABLE                                                            */
ASSIGN 
...
ASSIGN 
       i-num-po:HIDDEN IN FRAME fpage0           = TRUE.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-dun
/* Query rebuild information for BROWSE br-dun
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt-item-dun NO-LOCK INDEXED-REPOSITION.
     _END_FREEFORM
...
&Scoped-define SELF-NAME fi-cod-ean
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-ean wWindow
ON RETURN OF fi-cod-ean IN FRAME fpage0 /* Cod.EAN */
DO:
    FIND FIRST item-mat WHERE item-mat.cod-ean = INPUT FRAME fPage0 fi-cod-ean NO-LOCK NO-ERROR.

    IF NOT AVAIL item-mat THEN DO:
       RUN utp/ut-msgs.p (INPUT 'show':U,
                          INPUT 17006,
                          INPUT 'Cod.EAN nao encontrado'). 
       
       APPLY 'entry' TO fi-cod-ean IN FRAME fpage0.

       RETURN "NOK":U.
   END.

   ASSIGN c-it-codigo:SCREEN-VALUE IN FRAME fPage0 = item-mat.it-codigo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-ean wWindow
ON VALUE-CHANGED OF fi-cod-ean IN FRAME fpage0 /* Cod.EAN */
DO:
/*
...
ASSIGN i-num-po:VISIBLE IN FRAME fPage0 = NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piCarregaDUN wWindow 
PROCEDURE piCarregaDUN :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
EMPTY TEMP-TABLE tt-item-dun.

FOR EACH item-mat 
    WHERE item-mat.cod-ean = fi-cod-ean:SCREEN-VALUE IN FRAME fpage0,
    EACH item-dun
    WHERE item-dun.it-codigo = item-mat.it-codigo:

    CREATE tt-item-dun.
    BUFFER-COPY item-dun TO tt-item-dun.
END.
...
------------------------------------------------------------------------------*/
    DO WITH FRAME fPage0:
       ...
       RUN esp/es0018p.p (INPUT "escpp127":U,
                          INPUT 1,
                          INPUT 0,
                          INPUT "":U,
                          OUTPUT TABLE tt-prog-ponto).
       ...
       ASSIGN cb-modelo:LIST-ITEM-PAIRS = ",0". 
        ...
        FOR EACH tt-prog-ponto: 
            cb-modelo:ADD-LAST(STRING(tt-prog-ponto.conteudo),INT(tt-prog-ponto.conteudo)) IN FRAME fPage0.
        END.

        IF LENGTH(cb-modelo:LIST-ITEM-PAIRS) > 0 THEN DO:
            FOR EACH tt-prog-ponto: 
                ASSIGN cb-modelo:SCREEN-VALUE IN FRAME fPage0 = string(tt-prog-ponto.conteudo) . 
            END.
        END.
        ELSE DO:

            ASSIGN cb-modelo:LIST-ITEM-PAIRS IN FRAME fPage0 = ",0".

            ASSIGN cb-modelo:SCREEN-VALUE IN FRAME fPage0 = "0".

        END.

        APPLY "LEAVE" TO cb-modelo IN FRAME fPage0.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piCarregaPO wWindow 
PROCEDURE piCarregaPO :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
ASSIGN i-num-po:VISIBLE      IN FRAME fPage0 = NO
       i-num-po:SCREEN-VALUE IN FRAME fPage0 = '0'.

       
IF cb-modelo:SCREEN-VALUE IN FRAME fPage0 = '510' AND v_cod_estab_usuar = '104' THEN DO:

   FIND FIRST item-ean WHERE item-ean.it-codigo = c-it-codigo:SCREEN-VALUE NO-LOCK NO-ERROR.
   
   IF AVAIL item-ean THEN DO:
       IF item-ean.tp-fabric = 1 THEN /*CKD*/
          ASSIGN i-num-po:VISIBLE IN FRAME fPage0 = YES.
   END.                                                 
END.
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    RUN piImpressao IN h-esapi016 (INPUT INPUT FRAME fPage0 i-num-po,
...
FOR FIRST modelo-etiq NO-LOCK
    ...
    IF cb-modelo:SCREEN-VALUE IN FRAME fPage0 = '510' AND v_cod_estab_usuar = '104' THEN DO:
       ...
       IF AVAIL item-ean THEN DO:
          IF item-ean.tp-fabric = 1 THEN DO: /*CKD*/
             IF INT(i-num-po:SCREEN-VALUE IN FRAME fpage0) = 0 THEN DO:
                RUN utp/ut-msgs.p (INPUT 'show':U,
                                   INPUT 17006,
                                   INPUT 'Pedido invalido.' + '~~' + 'Pedido deve ser informado').
                APPLY 'entry':U TO i-num-po IN FRAME fpage0.
                RETURN "NOK":U.
               
             END.
             ELSE DO:
                 FIND FIRST pedido-compr 
                      WHERE pedido-compr.num-pedido = INT(i-num-po:SCREEN-VALUE IN FRAME fpage0)
                 NO-LOCK NO-ERROR.
            
                 IF NOT AVAIL pedido-compr THEN DO:
                    RUN utp/ut-msgs.p (INPUT 'show':U,
                                       INPUT 17006,
                                       INPUT 'Pedido nao encontrado.' + '~~' + 'Pedido informado invalido').
                    APPLY 'entry':U TO i-num-po IN FRAME fpage0.
                    RETURN "NOK":U.
                 END.    
             END.
    ...
    END. /* INPUT FRAME fPage0 cb-modelo = 510 */ 
