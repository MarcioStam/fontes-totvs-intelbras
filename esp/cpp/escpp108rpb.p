...
/*:T*******************************************************************************
**
**  Programa.: esp/rep/escpp108rpb.p
...
{include/i-prgvrs.i escpp108rpb 2.00.00.002}
...
DEFINE OUTPUT PARAMETER TABLE FOR tt-erro-aux.
DEFINE OUTPUT PARAMETER TABLE FOR tt-docum-est-aux.
...
blk_nota:
DO TRANSACTION
ON ERROR UNDO blk_nota, LEAVE blk_nota
ON STOP  UNDO blk_nota, LEAVE blk_nota: 
    ...
    IF NOT CAN-FIND (FIRST tt-erro) THEN DO:
        ...
        IF RETURN-VALUE = "OK" THEN DO:
            ...
            ASSIGN tt-item-doc-est.it-codigo      = tt-param.it-codigo
                   ...
                   tt-item-doc-est.desconto[1]    = 0                   
                   tt-item-doc-est.despesas[1]    = 0
...
PROCEDURE validateDados:
    ...
    IF NOT CAN-FIND (FIRST RowErrors) THEN DO:
        ...
        /*Grava na tabela do monitor as notas geradas*/
        FIND FIRST int-ord-prod-monitor EXCLUSIVE-LOCK
             WHERE int-ord-prod-monitor.nr-ord-produ = tt-param.nr-ord-prod NO-ERROR.

        IF AVAIL int-ord-prod-monitor THEN DO:
            /**Retorno**/
            IF p-tipo = 1 THEN
                ASSIGN int-ord-prod-monitor.r-docum-est-ret  = STRING(r-rowid-docum). 
            ELSE 
                ASSIGN int-ord-prod-monitor.r-docum-est-serv  = STRING(r-rowid-docum). 

            ASSIGN int-ord-prod-monitor.ind-status-industr = 1 /*Faturada*/.
        END.
    END.
