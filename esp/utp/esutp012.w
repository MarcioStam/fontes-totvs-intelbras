...
*******************************************************************************/
{include/i-prgvrs.i esutp012 2.04.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        esutp012
...
&GLOBAL-DEFINE page2Widgets   fi-periodo fi-diretorio tg-email fi-cc-ini fi-cc-fim fi-responsavel
...
DEFINE VARIABLE fi-periodo AS CHARACTER FORMAT "9999/99" 
     LABEL "Per°odo":R15 
     VIEW-AS FILL-IN 
     SIZE 8 BY .88 NO-UNDO.

DEFINE VARIABLE fi-responsavel AS CHARACTER FORMAT "X(12)" 
     LABEL "Respons†vel" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88.
...
DEFINE VARIABLE tg-email AS LOGICAL INITIAL no 
     LABEL "Envia Email Supervisores?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME fPage2
     fi-periodo AT ROW 2.75 COL 16 COLON-ALIGNED HELP
          "Per°odo" WIDGET-ID 60
     fi-diretorio AT ROW 3.88 COL 16 COLON-ALIGNED HELP
          "Diret¢rio Geraá∆o dos Arquivos" WIDGET-ID 18
     tg-email AT ROW 5.08 COL 18.14 WIDGET-ID 56
     fi-cc-ini AT ROW 7 COL 16 COLON-ALIGNED HELP
          "Centro Custo Inicial" WIDGET-ID 70
     fi-cc-fim AT ROW 7 COL 42 COLON-ALIGNED HELP
          "Centro Custo Final" NO-LABEL WIDGET-ID 68
     fi-responsavel AT ROW 8 COL 16 COLON-ALIGNED WIDGET-ID 124
     fi-nome-usuario AT ROW 8 COL 23.29 COLON-ALIGNED NO-LABEL WIDGET-ID 126
     IMAGE-17 AT ROW 7 COL 28.57 WIDGET-ID 80
     IMAGE-18 AT ROW 7 COL 40.86 WIDGET-ID 82
     RECT-14 AT ROW 2.42 COL 6 WIDGET-ID 128
     RECT-15 AT ROW 6.63 COL 6 WIDGET-ID 130
...
DO:
   do  on error undo, return no-apply:
       IF input frame fPage2 tg-email THEN DO:
           MESSAGE "Ser† enviado email para todos os supervisores, confirma?" UPDATE l-envia AS LOGICAL
...
ON F5 OF fi-responsavel IN FRAME fPage2 /* Respons†vel */
DO:
    ...
    IF  v_rec_usuar_mestre <> ? THEN DO:
        ...
        DISP usuar_mestre.cod_usuar @ fi-responsavel WITH FRAME fpage2.
        APPLY "leave" TO fi-responsavel IN FRAME fpage2.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-responsavel wReport
ON LEAVE OF fi-responsavel IN FRAME fPage2 /* Respons†vel */
DO:
    assign input frame fPage2 fi-responsavel.

    IF fi-responsavel <> "" THEN DO:
        FIND FIRST usuar_mestre NO-LOCK
             WHERE usuar_mestre.cod_usuario = fi-responsavel NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-responsavel wReport
ON LEFT-MOUSE-DBLCLICK OF fi-responsavel IN FRAME fPage2 /* Respons†vel */
DO:
  APPLY "F5" TO SELF.
END.
...
fi-responsavel:LOAD-MOUSE-POINTER("image/lupa.cur":U) IN FRAME fPage2.
{report/MainBlock.i}
...
FIND LAST integra-equipamentos WHERE integra-equipamento.tipo = 0 NO-LOCK NO-ERROR.
IF NOT AVAIL integra-equipamentos THEN
    ASSIGN fi-periodo:SCREEN-VALUE IN FRAME fPage2 = STRING(YEAR(TODAY),"9999") + STRING(MONTH(TODAY),"99").
ELSE
    ASSIGN fi-periodo:SCREEN-VALUE IN FRAME fPage2 = integra-equipamentos.mes-ref.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario          = c-seg-usuario
           ...
           tt-param.responsavel      = input frame fPage2 fi-responsavel.
    ...
    {report/rprun.i esp/utp/esutp012rp.p}
