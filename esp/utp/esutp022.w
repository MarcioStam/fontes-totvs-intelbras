...
*******************************************************************************/
{include/i-prgvrs.i ESUTP022 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program          ESUTP022
...
&GLOBAL-DEFINE page1Fields      fi-unidade fi-centro-cuscto fi-valor bt-confirma bt-volta br-rateio
...
DEF VAR r-vpc-rateio AS ROWID NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-brSon1 tt-vpc.nr-vpc tt-vpc.valor ~
fn-saldo(tt-vpc.nr-vpc) @ de-saldo tt-vpc.cod-estabel ~
...
DEFINE BUTTON btLiberacao 
     LABEL "Liberaá∆o" 
     SIZE 9 BY 1.

DEFINE BUTTON btPagtoProduto 
     LABEL "Pagto Prod." 
     SIZE 9 BY 1.

DEFINE BUTTON btTitulos 
     LABEL "T°tulos" 
     SIZE 8 BY 1.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-rateio wMasterDetail _STRUCTURED
  QUERY br-rateio NO-LOCK DISPLAY
      ...
      vpc-rateio.cod_ccusto COLUMN-LABEL "Centro de Custo" FORMAT "x(5)":U
            WIDTH 13
      vpc-rateio.valor FORMAT "->>>,>>9.99":U WIDTH 16.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 46 BY 4.75
         FONT 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brSon1 wMasterDetail _STRUCTURED
  QUERY brSon1 NO-LOCK DISPLAY
      ...
      tt-vpc.valor FORMAT "->>>,>>9.99":U WIDTH 10
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 85 BY 8.46
         FONT 2.
...
DEFINE FRAME fPage1
     brSon1 AT ROW 1.33 COL 2
     btAddSon1 AT ROW 10.08 COL 2
     btUpdateSon1 AT ROW 10.08 COL 10.86
     btDeleteSon1 AT ROW 10.08 COL 19.86
     btCopySon1 AT ROW 10.08 COL 28.86 WIDGET-ID 14
     btTitulos AT ROW 10.08 COL 42.14 HELP
          "Zera Saldo do T°tulo" WIDGET-ID 10
     btPagtoProduto AT ROW 10.08 COL 50.14 HELP
          "Zera Saldo do T°tulo" WIDGET-ID 12
     btAlterVencto AT ROW 10.08 COL 59 HELP
          "Zera Saldo do T°tulo" WIDGET-ID 8
     btZeraSaldo AT ROW 10.08 COL 69 HELP
          "Zera Saldo do T°tulo" WIDGET-ID 4
     btLiberacao AT ROW 10.08 COL 78 HELP
          "Gera T°tulo da VPC" WIDGET-ID 2
...
/* BROWSE-TAB br-rateio btLiberacao fPage1 */
ASSIGN 
       brSon1:MAX-DATA-GUESS IN FRAME fPage1         = 900.

ASSIGN 
       btCopySon1:HIDDEN IN FRAME fPage1           = FALSE.
...
"vpc-rateio.valor" ? ? "decimal" ? ? ? ? ? ? no ? no no "16.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE br-rateio */
&ANALYZE-RESUME
...
"tt-vpc.nr-vpc" ? ? "integer" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-vpc.valor
"tt-vpc.valor" ? ? "decimal" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"fn-situacao(tt-vpc.situacao) @ c-situacao" "Situaá∆o" "x(10)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    IF AVAIL tt-vpc THEN DO:
     ...
     IF  AVAIL tt-vpc AND tt-vpc.nr-vpc <> 0 THEN
         RUN pi-refresh.
...
DO:
    IF  AVAIL tt-vpc AND tt-vpc.nr-vpc <> 0 THEN DO:
...
DO:
    ...
    h-esutp022-brson1 = THIS-PROCEDURE.
END.
...
&Scoped-define SELF-NAME btAlterVencto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btAlterVencto wMasterDetail
ON CHOOSE OF btAlterVencto IN FRAME fPage1 /* Altera Vencto */
DO:
    DEFINE BUTTON btVenctoCancel AUTO-END-KEY 
         LABEL "&Cancelar" 
         SIZE 10 BY 1
         BGCOLOR 8.
    
    DEFINE BUTTON btVenctoOK AUTO-GO 
         LABEL "&OK" 
         SIZE 10 BY 1
         BGCOLOR 8.
    
    DEFINE RECTANGLE rtVenctoButton
         EDGE-PIXELS 2 GRAPHIC-EDGE  
         SIZE 78 BY 1.42
         BGCOLOR 7.
    
    DEFINE VARIABLE rVencto AS ROWID NO-UNDO.
    
    DEFINE VARIABLE dt-vencto-novo AS DATE FORMAT "99/99/9999" LABEL "Vencimento" NO-UNDO.
    DEFINE VARIABLE c-motivo       AS CHAR FORMAT "x(2000)" LABEL "Motivo"
        VIEW-AS EDITOR SCROLLBAR-VERTICAL  SIZE 65 BY 3 NO-UNDO FONT 1.

    DEFINE FRAME fAlterVencto
        dt-vencto-novo      AT ROW 1.21 COL 10.0 COLON-ALIGNED VIEW-AS FILL-IN SIZE 11  BY .88
        c-motivo            AT ROW 2.21 COL 10.0 COLON-ALIGNED
        btVenctoOK          AT ROW 5.63 COL 2.14
        btVenctoCancel      AT ROW 5.63 COL 13
        rtVenctoButton      AT ROW 5.38 COL 1
        SPACE(0.28)
        WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
             THREE-D SCROLLABLE TITLE "Altera Vencimento VPC" FONT 1
             DEFAULT-BUTTON btVenctoOK CANCEL-BUTTON btVenctoCancel.
    
/*tech1139 - FO 1338.917 - 10/07/2006  */
    RUN utp/ut-trfrrp.p (input Frame fAlterVencto:Handle).
    {utp/ut-liter.i "Altera_Vencimento_VPC"}
    ASSIGN FRAME fAlterVencto:TITLE = RETURN-VALUE.
/*tech1139 - FO 1338.917 - 10/07/2006  */

    ON "CHOOSE":U OF btVenctoOK IN FRAME fAlterVencto DO:
        ASSIGN dt-vencto-novo c-motivo.
        
        DEFINE VARIABLE h-esapi015 AS HANDLE NO-UNDO.
        DEFINE VARIABLE rRepos     AS ROWID  NO-UNDO.
        ...
        IF AVAIL tt-vpc THEN DO:
            ...
            RUN pi-altera-vencto IN h-esapi015 (INPUT tt-vpc.nr-vpc,
                                                INPUT dt-vencto-novo,
                                                INPUT c-motivo,
            ...
            IF CAN-FIND(FIRST tt_log_erros_tit_ap_alteracao) THEN DO:
                ...
                {method/showmessage.i2}
                 ...
                 APPLY "choose":U TO btVenctoCancel.

                WAIT-FOR CLOSE OF hShowMsg.
            END.
            ELSE DO:
                MESSAGE "Alteraá∆o do Vencimento efetuado com sucesso!"
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.

                /*:T Retorna rowid do registro corrente do DBO */
                RUN getRowid IN {&hDBOParent} (OUTPUT rRepos).

                /*:T Reposiciona registro com base em um rowid */
                RUN repositionRecord IN THIS-PROCEDURE (INPUT rRepos).
        ...
        APPLY "GO":U TO FRAME fAlterVencto.
    END.
    
    ENABLE dt-vencto-novo c-motivo btVenctoOK btVenctoCancel 
        WITH FRAME fAlterVencto. 
    
    WAIT-FOR "GO":U OF FRAME fAlterVencto.
END.
...
DO:
   {masterdetail/copyson.i &ProgramSon="esp/utp/esutp022a.w"
                               &PageNumber="1"}
...
&Scoped-define FRAME-NAME fPage1
&Scoped-define SELF-NAME btLiberacao
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btLiberacao wMasterDetail
ON CHOOSE OF btLiberacao IN FRAME fPage1 /* Liberaá∆o */
DO:
    DEFINE VARIABLE h-esapi015 AS HANDLE NO-UNDO.
    DEFINE VARIABLE rRepos     AS ROWID  NO-UNDO.
    ...
    IF AVAIL tt-vpc THEN DO:
        CREATE tt-aux.
        BUFFER-COPY tt-vpc TO tt-aux.

        RUN esapi/esapi015.p PERSISTENT SET h-esapi015.
        RUN pi-libera-vpc IN h-esapi015 (INPUT  TABLE tt-aux,
        ...
        IF CAN-FIND(FIRST tt_log_erros_atualiz) THEN DO:
            FOR EACH tt_log_erros_atualiz:
                CREATE RowErrors.
                ASSIGN RowErrors.errorNumber      = tt_log_erros_atualiz.ttv_num_mensagem
                       RowErrors.errorDescription = tt_log_erros_atualiz.ttv_des_msg_erro
                       rowerrors.errorhelp        = tt_log_erros_atualiz.ttv_des_msg_ajuda.
            END.
            {method/showmessage.i1}
        ...
        END.
        ELSE DO:
            MESSAGE "Liberaá∆o efetuada com sucesso!"
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
...
&Scoped-define FRAME-NAME fPage1
&Scoped-define SELF-NAME btPagtoProduto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btPagtoProduto wMasterDetail
ON CHOOSE OF btPagtoProduto IN FRAME fPage1 /* Pagto Prod. */
DO:
    IF AVAIL tt-vpc THEN DO:
        ASSIGN CURRENT-WINDOW:SENSITIVE = NO.
        RUN esp/utp/esutp022a1.w (INPUT tt-vpc.nr-vpc).
...
&Scoped-define FRAME-NAME fPage1
&Scoped-define SELF-NAME btTitulos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btTitulos wMasterDetail
ON CHOOSE OF btTitulos IN FRAME fPage1 /* T°tulos */
DO:
    DEFINE VARIABLE h-esapi015 AS HANDLE      NO-UNDO.
    ...
    IF AVAIL tt-vpc THEN DO:
        RUN esapi/esapi015.p PERSISTENT SET h-esapi015.
        RUN pi-verifica-titulo IN h-esapi015 (INPUT  tt-vpc.nr-vpc,
                                              OUTPUT v_rec_tit_ap).
...
DO:
    {masterdetail/updateson.i &ProgramSon="esp/utp/esutp022a.w"
                              &PageNumber="1"}
     h-esutp022-brson1 = THIS-PROCEDURE.                         
END.
...
&Scoped-define FRAME-NAME fPage1
&Scoped-define SELF-NAME btZeraSaldo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btZeraSaldo wMasterDetail
ON CHOOSE OF btZeraSaldo IN FRAME fPage1 /* Zerar Saldo */
DO:

    DEFINE BUTTON btZeraCancel AUTO-END-KEY 
         LABEL "&Cancelar" 
         SIZE 10 BY 1
         BGCOLOR 8.
    
    DEFINE BUTTON btZeraOK AUTO-GO 
         LABEL "&OK" 
         SIZE 10 BY 1
         BGCOLOR 8.
    ...
    DEFINE FRAME fZeraSaldo
        c-motivo            AT ROW 1.21 COL 10.0 COLON-ALIGNED
        btZeraOK            AT ROW 4.63 COL 2.14
        btZeraCancel        AT ROW 4.63 COL 13
        rtVenctoButton      AT ROW 4.38 COL 1
        SPACE(0.28)
        WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
             THREE-D SCROLLABLE TITLE "Zera Saldo Titulo VPC" FONT 1
             DEFAULT-BUTTON btZeraOK CANCEL-BUTTON btZeraCancel.
    
/*tech1139 - FO 1338.917 - 10/07/2006  */
    RUN utp/ut-trfrrp.p (input Frame fZeraSaldo:Handle).
    {utp/ut-liter.i "Zera_Saldo_Titulo_VPC"}
    ASSIGN FRAME fZeraSaldo:TITLE = RETURN-VALUE.
/*tech1139 - FO 1338.917 - 10/07/2006  */

    ON "CHOOSE":U OF btZeraOK IN FRAME fZeraSaldo DO:
        ASSIGN c-motivo.
        ...
        IF AVAIL tt-vpc THEN DO:
            RUN esapi/esapi015.p PERSISTENT SET h-esapi015.
            RUN pi-zera-saldo IN h-esapi015 (INPUT tt-vpc.nr-vpc,
                                             INPUT c-motivo,
                                             OUTPUT TABLE tt_log_erros_tit_ap_alteracao).
            ...
            IF CAN-FIND(FIRST tt_log_erros_tit_ap_alteracao) THEN DO:
                ...
                {method/showmessage.i2}
                 ...
                 APPLY "choose":U TO btZeraCancel.

                WAIT-FOR CLOSE OF hShowMsg.
            END.
            ELSE DO:
                MESSAGE "T°tulo zerado. Acerto de Valor efetuado com sucesso!"
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
        ...
        APPLY "GO":U TO FRAME fZeraSaldo.
    END.
    
    ENABLE c-motivo btZeraOK btZeraCancel 
        WITH FRAME fZeraSaldo. 
    
    WAIT-FOR "GO":U OF FRAME fZeraSaldo.
END.
...
/*tech1139 - FO 1338.917 - 10/07/2006  */
    ...
    ON "CHOOSE":U OF btGoToOK IN FRAME fGoToVpc DO:
       ...
       RUN openQueryStatic IN {&hDBOSon1} (INPUT "VpcEmitente":U) NO-ERROR.
        ...
        FIND FIRST tt-vpc
            WHERE tt-vpc.nr-vpc >= i-nr-vpc NO-LOCK NO-ERROR.

        IF  AVAIL tt-vpc THEN
            QUERY brson1:REPOSITION-TO-ROWID(ROWID(tt-vpc)).   
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-refresh wMasterDetail 
...
------------------------------------------------------------------------------*/
    ...
    IF  AVAIL tt-vpc
    AND tt-vpc.situacao = 0 THEN
        ASSIGN btCopySon1:SENSITIVE IN FRAME fPage1 = YES.
    ELSE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-valida-campos wMasterDetail 
...
------------------------------------------------------------------------------*/
    ASSIGN btDeleteSon1:SENSITIVE   IN FRAME fPage1 = NO
           btTitulos:SENSITIVE      IN FRAME fPage1 = NO
           btAlterVencto:SENSITIVE  IN FRAME fPage1 = NO
           btZeraSaldo:SENSITIVE    IN FRAME fPage1 = NO
           btLiberacao:SENSITIVE    IN FRAME fPage1 = NO
           btPagtoProduto:SENSITIVE IN FRAME fPage1 = NO.
