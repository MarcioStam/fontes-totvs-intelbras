...
def var v_tx_fornec      as char.
def var v_tx_mut         as char.
def var v_result         as char format "X(1)".
...
if  v_cod_arq <> '' and v_cod_arq <> ? then do:
    run pi_version_extract ('fnc_exporta‡Æo_safra_cheq', 'esp/edf/edf479ve.py', '1.00.00.001', 'pro').
end /* if */.
/* End_Include: i_version_extract */

/*************************************************************************
  MARIO FRANCISCO FLEITH JR. - PLANEER CONSULTORIA E TREINAMENTO LTDA.
  MAIL fleith@planner.inf.br
*************************************************************************/


if p_cdn_element_edi = 24 then do: /*Flag Fim de Arquivo*/

        FOR EACH reg_proces_entr_edi NO-LOCK 
            WHERE reg_proces_entr_edi.cdn_proces_edi = int(v_des_flag_public_geral[1]) 
              AND reg_proces_entr_edi.des_id_reg_bloco_modul_edi BEGINS "planilha":
            ASSIGN de-soma = de-soma + dec(entry(11,reg_proces_entr_edi.dsl_dados_entr_edi,CHR(24))).
        END.
        
        RETURN STRING(de-soma).
  
end.

if p_cdn_element_edi = 51 then do: /* GAP */
   FIND FIRST tt_param_program_formul where tta_cdn_element_edi = 16 NO-LOCK NO-ERROR.
   assign v_tx_fornec = substr(ttv_des_contdo,1,5).
   FIND FIRST tt_param_program_formul where tta_cdn_element_edi = 17 NO-LOCK NO-ERROR.
   assign v_tx_mut = substr(ttv_des_contdo,1,5).
   
/**** COMENTADO POR CLAUDINEY EM 09/02/2005, APOS CONTATO COM O SR. GALVÇO  */
  /* if int(v_tx_fornec) = int(v_tx_mut) then
      assign v_result = "0".
   if int(v_tx_fornec) > int(v_tx_mut) then 
      assign v_result = "1".
   if int(v_tx_fornec) < int(v_tx_mut) then
