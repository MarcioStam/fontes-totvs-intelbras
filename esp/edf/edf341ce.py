...
def var v_cdn_agenc
    ...
    format ">>>>>>>>>>>>9":U
    no-undo.
def var v_cod_inst_bcia1
    as character
    format "x(3)":U
    no-undo.
def var v_cod_inst_bcia2
    as character
    format "x(3)":U
    no-undo.
def var v_num_dia_inst
    as character
    format "x(3)":U
    no-undo.    
...
then do:
    run pi_version_extract ('fnc_cob_bradesc', 'esp/edf/edf237ce.py', '1.00.00.006', 'pro').
end /* if */.
...
find tt_param_program_formul
    ...
    and   tt_param_program_formul.tta_cdn_element_edi = 3814 no-error. /* INSTRUCAO 1*/
assign v_cod_inst_bcia1 = tt_param_program_formul.ttv_des_contdo.

find tt_param_program_formul
    where tt_param_program_formul.tta_cdn_segment_edi = 292
    and   tt_param_program_formul.tta_cdn_element_edi = 3815 no-error. /* INSTRUCAO 2*/
assign v_cod_inst_bcia2 = tt_param_program_formul.ttv_des_contdo.

if v_cod_inst_bcia1 = '81' or
   v_cod_inst_bcia2 = '81' then do:
   find tt_param_program_formul
      where tt_param_program_formul.tta_cdn_segment_edi = 292
      and   tt_param_program_formul.tta_cdn_element_edi = 81 no-error. /* NR DE DIAS COMPLEMENTARES*/
    assign v_num_dia_inst = tt_param_program_formul.ttv_des_contdo.
end.

IF p_cdn_element_edi = 3814 THEN DO: /* INSTRUCAO 1*/
   RETURN v_cod_inst_bcia1.
END.
IF p_cdn_element_edi = 3815 THEN DO: /* INSTRUCAO 2*/
  IF v_cod_inst_bcia2  <> v_cod_inst_bcia1 THEN
     RETURN v_cod_inst_bcia2.
  ELSE 
     RETURN '0'.
END.

IF p_cdn_element_edi = 4144 THEN DO: /* INSTRUCAO 2*/
  IF v_cod_inst_bcia1 = '81' or
     v_cod_inst_bcia2 = '81' THEN
     RETURN v_num_dia_inst.
  ELSE 
     RETURN '0'.
END.
