/*********************************************************************************
** Programa: esp/crm/escrm009.p
** VersÆo..: 1.00
** Data....: 30/09/2010
** Autor...: Estevan Krger - Exponencial TI
** Obs.....: API para buscar o % de Desconto M ximo que pode ser dado ao cliente,
...
DEFINE INPUT  PARAMETER p-cod-rep        AS INTEGER   FORMAT ">>>>9"         NO-UNDO.
...
IF  l-log THEN DO:
    ...
    PUT UNFORMATTED "--------------------------------------------------------------------------------" SKIP
                    "Inicio API BuscarDescManga -- " + STRING(TODAY, "99/99/9999") + " - " + STRING(TIME, "HH:MM:SS") SKIP.
    PUT UNFORMATTED "Parametros Recebidos:" SKIP
                    " - Representante.: " p-cod-rep SKIP
...
IF  NOT CAN-FIND(FIRST repres NO-LOCK
                 WHERE repres.cod-rep = p-cod-rep) THEN DO:
    ...
    IF  l-log THEN DO:
        ...
        ELSE DO:
            OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
        END.
        PUT UNFORMATTED "NÆo encontrou o Representante (" + STRING(p-cod-rep) + ")!" SKIP.
        OUTPUT CLOSE.
    END.
    /*****************************************************************/
    RETURN "NOK":U.
...
/* Busca o % de desconto m ximo para o Cliente, Fam¡lia Comercial, UN e Categoria */
FIND FIRST crm-desc-manga NO-LOCK
    ...
    AND    crm-desc-manga.cd-categoria     = p-cd-categoria
    AND    crm-desc-manga.cod-rep          = p-cod-rep
    AND    crm-desc-manga.fm-cod-com       = p-fm-cod-com
    AND    crm-desc-manga.dt-vigencia-ini <= TODAY
    AND   (IF crm-desc-manga.dt-vigencia-fim <> ? THEN crm-desc-manga.dt-vigencia-fim > TODAY ELSE YES) NO-ERROR.
/******************** Log de Execu‡Æo da API *********************/
IF  l-log THEN DO:
    IF  OPSYS = "WIN32":U THEN DO:
        OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
    ...
    ELSE DO:
        OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
    END.
    PUT UNFORMATTED "Encontrou Desconto Manga (crm-desc-manga)? " AVAIL crm-desc-manga SKIP
...
END.
/*****************************************************************/
IF  AVAIL  crm-desc-manga THEN DO:
    /******************** Log de Execu‡Æo da API *********************/
    IF  l-log THEN DO:
        IF  OPSYS = "WIN32":U THEN DO:
            OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
        ...
        ELSE DO:
            OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
        END.
        PUT UNFORMATTED "Desconto M ximo encontrado: " + STRING(crm-desc-manga.pc-desconto) SKIP.
        OUTPUT CLOSE.
    END.
    /*****************************************************************/
    ASSIGN p-desc-maximo = crm-desc-manga.pc-desconto.
END.
