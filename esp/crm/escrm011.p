/*********************************************************************************
** Programa: esp/crm/escrm011.p
** VersÆo..: 1.00
** Data....: 30/09/2010
** Autor...: Estevan Krger - Exponencial TI
** Obs.....: API para buscar o Valor M¡nimo para o Pedido, e retornar ao Portal B2B
*********************************************************************************/
...
DEFINE OUTPUT PARAMETER p-vl-minimo      AS DECIMAL   FORMAT ">,>>>,>>>,>>9.99" NO-UNDO.
...
IF  l-log THEN DO:
    ...
    PUT UNFORMATTED "--------------------------------------------------------------------------------" SKIP
                    "Inicio API BuscarPedidoMinimo -- " + STRING(TODAY, "99/99/9999") + " - " + STRING(TIME, "HH:MM:SS") SKIP.
...
/* Busca o valor m¡nimo para o grupo de cliente e categoria */
FIND FIRST crm-desc-ped-minimo NO-LOCK
    WHERE  crm-desc-ped-minimo.cd-unid-negoc    = STRING(unid-comerc.cd-unid-comerc)
    AND    crm-desc-ped-minimo.cod-gr-cli       = i-cod-gr-cli
    AND    crm-desc-ped-minimo.cd-categoria     = p-cd-categoria
    AND    crm-desc-ped-minimo.dt-vigencia-ini <= TODAY
    AND   (IF crm-desc-ped-minimo.dt-vigencia-fim <> ? THEN crm-desc-ped-minimo.dt-vigencia-fim > TODAY ELSE YES) NO-ERROR.
/******************** Log de Execu‡Æo da API *********************/
IF  l-log THEN DO:
    IF  OPSYS = "WIN32":U THEN DO:
        OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
    ...
    ELSE DO:
        OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
    END.
    PUT UNFORMATTED "Encontrou o Valor M¡nimo do Pedido (crm-desc-ped-minimo)?" AVAIL crm-desc-ped-minimo SKIP.
    OUTPUT CLOSE.
END.
/*****************************************************************/
IF  AVAIL  crm-desc-ped-minimo THEN DO:
    /******************** Log de Execu‡Æo da API *********************/
    IF  l-log THEN DO:
        IF  OPSYS = "WIN32":U THEN DO:
            OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
        ...
        ELSE DO:
            OUTPUT TO value(session:temp-directory + "/log-apis-crm.txt") NO-CONVERT APPEND.
        END.
        PUT UNFORMATTED " - Valor M¡nimo: " crm-desc-ped-minimo.vl-minimo SKIP.
        OUTPUT CLOSE.
    END.
    /*****************************************************************/

    ASSIGN p-vl-minimo = crm-desc-ped-minimo.vl-minimo.
END.
