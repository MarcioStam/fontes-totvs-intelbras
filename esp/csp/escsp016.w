...
*******************************************************************************/
{include/i-prgvrs.i ESCSP016 12.01.34.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCSP016 MCS}
&ENDIF
...
def var da-iniper-fech     as date    no-undo.
def var da-fimper-fech     as date    no-undo.
...
DEFINE VARIABLE rs-opcao AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Lista Horas Recalculadas", 1,
"Atualiza com Horas Recalculadas", 2,
"Retorna Horas Originais", 3
     SIZE 29.14 BY 2.71 NO-UNDO.
...
DEFINE VARIABLE tg-detalha AS LOGICAL INITIAL no 
     LABEL "Detalha Movimentos do Calculo" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-par
     fi-data-ini AT ROW 3.04 COL 15.57 COLON-ALIGNED WIDGET-ID 92
     fi-data-fim AT ROW 3.04 COL 50.14 COLON-ALIGNED NO-LABEL WIDGET-ID 90
     rs-opcao AT ROW 4.75 COL 17.57 NO-LABEL WIDGET-ID 104
     tg-detalha AT ROW 8.21 COL 17.57 WIDGET-ID 110
     "Horas Reportadas" VIEW-AS TEXT
          SIZE 12.57 BY .54 AT ROW 4.25 COL 3.43 WIDGET-ID 108
     RECT-13 AT ROW 2.75 COL 2.72 WIDGET-ID 86
     RECT-14 AT ROW 4.46 COL 2.72 WIDGET-ID 88
     IMAGE-3 AT ROW 3.04 COL 28 WIDGET-ID 94
     IMAGE-4 AT ROW 3.04 COL 48.86 WIDGET-ID 96
     RECT-15 AT ROW 1 COL 1 WIDGET-ID 98
     RECT-16 AT ROW 7.92 COL 2.72 WIDGET-ID 112
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 77 BY 10.46
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Rec†lculo Horas Reportadas"
         HEIGHT             = 15.04
         WIDTH              = 81.14
         MAX-HEIGHT         = 41.33
         MAX-WIDTH          = 365.72
...
/* SETTINGS FOR FRAME f-pg-par
                                                                        */
/* SETTINGS FOR FILL-IN fi-data-fim IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-data-ini IN FRAME f-pg-par
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-relat /* Rec†lculo Horas Reportadas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Rec†lculo Horas Reportadas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rs-opcao w-relat
ON VALUE-CHANGED OF rs-opcao IN FRAME f-pg-par
DO:

    assign tg-detalha:sensitive in frame f-pg-par = int(self:screen-value in frame f-pg-par) <> 3. 

    if not tg-detalha:sensitive in frame f-pg-par  
    then tg-detalha:checked in frame f-pg-par = no.
...
{utp/ut9000.i "ESCSP016" "12.01.34.000}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    assign fi-data-ini:screen-value in frame f-pg-par = string(fi-data-ini)
           fi-data-fim:screen-value in frame f-pg-par = string(fi-data-fim).
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-data-ini fi-data-fim rs-opcao tg-detalha 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-13 RECT-14 IMAGE-3 IMAGE-4 RECT-15 RECT-16 rs-opcao tg-detalha 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
define var dt-per-aux       as date     no-undo.
define var dt-mens-aux      as date     no-undo.
...
do on error undo, return error on stop  undo, return error:
    ...
    if input frame f-pg-par fi-data-ini > input frame f-pg-par fi-data-fim
    then do:
        run utp/ut-msgs.p (input "show":U, input 5360, input "").
        apply "ENTRY":U to fi-data-ini in frame f-pg-par.
        return error.
    end.
    ...
    then do:
        assign dt-mens-aux = date(month(param-estoq.mensal-ate),01,year(param-estoq.mensal-ate))
               dt-mens-aux = dt-mens-aux + 31
               dt-mens-aux = dt-mens-aux - day(dt-mens-aux)
               dt-per-aux  = date(int(substring(input frame f-pg-par fi-periodo,1,2)),01,int(substring(input frame f-pg-par fi-periodo,3,4)))
               dt-per-aux  = dt-per-aux + 31
               dt-per-aux  = dt-per-aux - day(dt-per-aux).

        if dt-per-aux <= dt-mens-aux
        then do:
            run utp/ut-msgs.p (input "show":U, input 17567, input "Per°odo informado Ç menor ou igual que o £ltimo mÇdio calculado").
            apply "ENTRY":U to fi-periodo in frame f-pg-par.
            return error.
        end.
    end.*/
    ...
    assign tt-param.dt-per-ini        = input frame f-pg-par fi-data-ini
           tt-param.dt-per-fim        = input frame f-pg-par fi-data-fim
           tt-param.opcao             = input frame f-pg-par rs-opcao
           tt-param.lg-detalha-calc   = input frame f-pg-par tg-detalha.
    ...
    {include/i-rprun.i esp/csp/escsp016rp.p}
