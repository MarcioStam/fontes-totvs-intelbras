/******************************************************************************
** Programa..............: gera_capital_giro_por_estab
** Versao................:  1.00.00.000
** Nome Externo..........: esp/es0030rp.p
** Criado por............: Fabiano
** Criado em.............: 07/05/2008
...
if not avail param-global
   then next.
   
/* ** Efetuar as leituras nos estabelecimentos da empresa ****/
for each estabelecimento no-lock
    where estabelecimento.cod_empresa = string(param-global.empresa-prin):

    /* ** Efetua leitura em tràs meses ***/
    repeat v_num_mes = 3 to 1 by -1:
      ...
      for each sit_movimen_modul no-lock
            where sit_movimen_modul.cod_modul_dtsul       = 'FGL'
              and sit_movimen_modul.cod_unid_organ        = string(param-global.empresa-prin)
      ...
      then do:
            ...
            find tt_capital
               where tt_capital.ttv_cod_estab = estabelecimento.cod_estab
                 and tt_capital.ttv_des_grupo = 'Saldo de Caixa' no-error.
            if not avail tt_capital
            then do:
                 create tt_capital.
                 assign tt_capital.ttv_cod_estab = estabelecimento.cod_estab
            ...
            run pi_cria_leitura (input estabelecimento.cod_estab,
                                 input '11100000',
                                 input '11200000',
                                 input v_dat_refer).
      ...
      /* ** Caso o per°odo n∆o esteja fechado ou congelado, efetua a leitura nas movimentaá‰es ***/
...
for each tt_capital:
    ...
    find cap_giro exclusive-lock
       where cap_giro.cod_empresa = tt_capital.ttv_cod_estab
    ...
    then do:
         create cap_giro.
         assign cap_giro.cod_empresa = tt_capital.ttv_cod_estab
...
assign v_cod_arq = session:temp-directory + 'es0030' + ".tmp":U.
...
for each tt_capital use-index tt_seq
    where tt_capital.ttv_cod_estab = '1':
...
PROCEDURE pi_cria_leitura:
    ...
    ASSIGN tt_input_leitura_sdo.ttv_cod_label    = "Empresa"
           tt_input_leitura_sdo.ttv_des_conteudo = string(param-global.empresa-prin)
    ...
    ASSIGN tt_input_leitura_sdo.ttv_cod_label    = "Estabelecimento Inicial"
           tt_input_leitura_sdo.ttv_des_conteudo = p_cod_estab
           tt_input_leitura_sdo.ttv_num_seq_1    = v_num_seq
    ...
    for each cta_ctbl no-lock
        ...
        repeat v_dat = date("01" + string(month(p_dat_refer),'99') + string(year(today), '9999')) to p_dat_refer:
            ...
            then do:
                ...
                FOR EACH movto-estoq FIELDS (dt-trans valor-mob-m valor-ggf-m tipo-trans valor-mat-m cod-estabel it-codigo contabilizado quantidade conta-contab) NO-LOCK
