...
*******************************************************************************/
{include/i-prgvrs.i ESAQP012 1.00.00.000}
...
&GLOBAL-DEFINE Program        ESAQP012
...
&GLOBAL-DEFINE page0Widgets   btOk ~
                              btCancel ~
                              btHelp2
&GLOBAL-DEFINE page2Widgets   cbTp-teste cbTp-teste-fim 
...
&GLOBAL-DEFINE page2Fields    datIniAdicional datFimAdicional c-cod-estabel iNr-linha c-it-codigo fi-auditor      
...
def var v_codigo_teste     as int     no-undo.
...
DEFINE VARIABLE cbTp-Teste AS CHARACTER INITIAL "1" 
     LABEL "Teste" 
     VIEW-AS COMBO-BOX INNER-LINES 20
     DROP-DOWN
     SIZE 33 BY 1 NO-UNDO.

DEFINE VARIABLE cbTp-Teste-fim AS CHARACTER INITIAL "1" 
     VIEW-AS COMBO-BOX INNER-LINES 20
     DROP-DOWN
     SIZE 33 BY 1 NO-UNDO.
...
DEFINE VARIABLE cDesc-auditor AS CHARACTER FORMAT "x(60)" 
     VIEW-AS FILL-IN 
     SIZE 43.57 BY .88.
...
DEFINE FRAME fPage2
     datIniAdicional AT ROW 2 COL 20.86 HELP
          "Data de amostragem adicional inicial"
     datFimAdicional AT ROW 2 COL 50 HELP
          "Data final de amostragem adicional" NO-LABEL
     c-cod-estabel AT ROW 4.25 COL 23 COLON-ALIGNED WIDGET-ID 46
     c-desc-estabel AT ROW 4.25 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 48
     iNr-linha AT ROW 5.25 COL 24 COLON-ALIGNED HELP
          "Numero da Linha de Produ‡Æo" WIDGET-ID 8
     cDescricaoLinProd AT ROW 5.25 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     c-it-codigo AT ROW 6.25 COL 11 COLON-ALIGNED HELP
          "C¢digo do Item" WIDGET-ID 2
     cDesc-item AT ROW 6.25 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     fi-auditor AT ROW 7.25 COL 11 COLON-ALIGNED WIDGET-ID 56
     cDesc-auditor AT ROW 7.25 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 58
     cbTp-Teste AT ROW 9 COL 5.43 COLON-ALIGNED WIDGET-ID 10
     cbTp-Teste-fim AT ROW 9 COL 46.43 COLON-ALIGNED NO-LABEL WIDGET-ID 54
     IMAGE-1 AT ROW 2 COL 41.57
     IMAGE-2 AT ROW 2 COL 45.57
     RECT-13 AT ROW 1.04 COL 1 WIDGET-ID 12
     RECT-14 AT ROW 3.79 COL 1 WIDGET-ID 14
     RECT-15 AT ROW 8.5 COL 1 WIDGET-ID 16
     IMAGE-3 AT ROW 9 COL 41.43 WIDGET-ID 50
     IMAGE-4 AT ROW 9 COL 44.57 WIDGET-ID 52
...
/* SETTINGS FOR FRAME fPage2
                                                                        */
/* SETTINGS FOR FILL-IN cDesc-auditor IN FRAME fPage2
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME fi-auditor
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-auditor wReport
ON LEAVE OF fi-auditor IN FRAME fPage2 /* Auditor */
DO:

  FIND FIRST usuar_mestre NO-LOCK
       WHERE usuar_mestre.cod_usuario = INPUT FRAME fPage2 fi-auditor NO-ERROR.
  IF AVAIL usuar_mestre THEN
      ASSIGN cDesc-auditor:SCREEN-VALUE IN FRAME fPage2 = usuar_mestre.nom_usuario.
  ELSE
      ASSIGN cDesc-auditor:SCREEN-VALUE IN FRAME fPage2 = "".  
END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE v_codigo_teste_fim AS INTEGER NO-UNDO.
    DEFINE VARIABLE l-primeiro AS LOG NO-UNDO.

    /* POLULA CAMPO NÖCIO FAIXA */
    FOR EACH aq-teste NO-LOCK
        BY aq-teste.nr-seq-teste:
        IF  l-primeiro = NO then
            assign v_codigo_teste = aq-teste.nr-seq-teste.
        l-primeiro = YES.

        assign v_popula_cb = v_popula_cb + string(aq-teste.nr-seq-teste) + " - " + aq-teste.des-teste + "," + string(aq-teste.nr-seq-teste) + ",".

        assign v_codigo_teste_fim = aq-teste.nr-seq-teste.
    end.
 
    assign cbTp-teste:list-item-pairs in frame fPage2 = substring(v_popula_cb,1,LENGTH(v_popula_cb) - 1)
           cbTp-teste:screen-value in frame fPage2 = string(v_codigo_teste).
    
    /* POLULA CAPO FIM FAIXA */
    assign cbTp-teste-fim:list-item-pairs in frame fPage2 = substring(v_popula_cb,1,LENGTH(v_popula_cb) - 1)
           cbTp-teste-fim:screen-value in frame fPage2 = string(v_codigo_teste_fim).
...
do on error undo, return error on stop  undo, return error:
    ...
    if input frame fPage2 cbTp-Teste > input frame fPage2 cbTp-Teste-fim  then do:
        run utp/ut-msgs.p (input "show":U, input 17006, input "Faixa de Teste Inicial deve ser Menor ou igual a Faixa Final":U).
        apply "ENTRY":U to cbTp-Teste-fim in frame fPage2.
        return error.
    end.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.sit-teste-ini   = int(INPUT FRAME fPage2 cbTp-Teste)
           tt-param.sit-teste-fim   = int(INPUT FRAME fPage2 cbTp-Teste-fim)
    ...
    {report/rprun.i esp/aqp/esaqp012rp.p}
