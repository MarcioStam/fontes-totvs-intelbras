...
{include/i-prgvrs.i ESAQP020RP 1.00.00.000}
...
if tt-param.destino = 3 or
   tt-param.destino = 1 then
    assign v_des_destino = string(session:temp-directory + "ESAQP020" + string (time) + ".xlsx").
else
...
break by auditoria-geral.dt-amostragem:
    ...
    find first tt-registros exclusive-lock
         where tt-registros.it-codigo = auditoria-geral.it-codigo no-error.
    if not avail tt-registros then do:
        create tt-registros.
        assign tt-registros.it-codigo = auditoria-geral.it-codigo. 
    end.
...
procedure pi-cria-planilha:
    ...
    chExcel:selection:value = "TOTAL DE LOTES REVISADOS POR PRODUTO".
...
procedure pi-imprime-registros:
    ...
    for each tt-registros:
        ...
        find first item no-lock
            where item.it-codigo = tt-registros.it-codigo no-error.
        if avail item then
          assign v_des_produto = item.desc-item.
...
procedure pi-cria-grafico:
    ...
    assign v_des_produto = chPlanilha:Range("A6"):value.
    
    chExcel:ActiveChart:SeriesCollection(1):XValues = "Plan1!$A$6:$A$" + string(i-linha).
    chExcel:ActiveChart:SeriesCollection(1):name    = v_des_produto.
