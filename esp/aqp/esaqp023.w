...
*******************************************************************************/
{include/i-prgvrs.i ESAQP023 1.00.00.000}
...
&GLOBAL-DEFINE Program        ESAQP023
...
&GLOBAL-DEFINE page0Widgets   btOk ~
                              btCancel ~
                              btHelp2
&GLOBAL-DEFINE page2Widgets   rs-tip-proced rs-tip-grafico
...
&GLOBAL-DEFINE page2Fields    datIniAdicional datFimAdicional c-des-auditor i-nr-seq-procedimento i-nr-seq-setor      
...
DEFINE VARIABLE rs-tip-grafico AS INTEGER 
...
DEFINE VARIABLE rs-tip-proced AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Por Unidade de Neg¢cio", 1,
"Por Setor", 2,
"Por NÆo conformidade", 3
...
DEFINE FRAME fPage2
     ...
     c-des-auditor AT ROW 4.5 COL 17 COLON-ALIGNED WIDGET-ID 8
     i-nr-seq-procedimento AT ROW 5.5 COL 17 COLON-ALIGNED WIDGET-ID 2
     v_des_procedimento AT ROW 5.5 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     i-nr-seq-setor AT ROW 6.5 COL 17 COLON-ALIGNED WIDGET-ID 28
     v_des_setor AT ROW 6.5 COL 29.43 COLON-ALIGNED NO-LABEL WIDGET-ID 30
     rs-tip-proced AT ROW 8.25 COL 22 NO-LABEL WIDGET-ID 34
     rs-tip-grafico AT ROW 8.25 COL 58 NO-LABEL WIDGET-ID 18
     "Procedimento" VIEW-AS TEXT
          SIZE 10 BY .5 AT ROW 7.5 COL 20 WIDGET-ID 38
     "Tipo Gr fico" VIEW-AS TEXT
          SIZE 9 BY .5 AT ROW 7.5 COL 56 WIDGET-ID 22
     ...
     RECT-15 AT ROW 7.75 COL 55 WIDGET-ID 16
     RECT-16 AT ROW 7.75 COL 19 WIDGET-ID 32
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
...
/* SETTINGS FOR FILL-IN datIniAdicional IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN v_des_procedimento IN FRAME fPage2
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN v_des_setor IN FRAME fPage2
   NO-ENABLE                                                            */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-procedimento wReport
ON F5 OF i-nr-seq-procedimento IN FRAME fPage2 /* Procedimento */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-procedimento wReport
ON LEAVE OF i-nr-seq-procedimento IN FRAME fPage2 /* Procedimento */
DO:
  FIND FIRST aq-procedimento NO-LOCK
       WHERE aq-procedimento.nr-seq-procedimento = INPUT FRAME fPage2 i-nr-seq-procedimento NO-ERROR.
  IF AVAIL aq-procedimento THEN
      ASSIGN v_des_procedimento:SCREEN-VALUE IN FRAME fPage2 = aq-procedimento.des-procedimento.
  ELSE
      ASSIGN v_des_procedimento:SCREEN-VALUE IN FRAME fPage2 = "".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-procedimento wReport
ON MOUSE-SELECT-DBLCLICK OF i-nr-seq-procedimento IN FRAME fPage2 /* Procedimento */
DO:
  APPLY 'F5' TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-setor wReport
ON F5 OF i-nr-seq-setor IN FRAME fPage2 /* Setor */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-setor wReport
ON LEAVE OF i-nr-seq-setor IN FRAME fPage2 /* Setor */
DO:
  find first aq-setor no-lock
       where aq-setor.nr-seq-setor = input frame fPage2 i-nr-seq-setor no-error.
  if avail aq-setor then
      assign v_des_setor:screen-value in frame fPage2 = aq-setor.des-setor.
  else
      assign v_des_setor:screen-value in frame fPage2 = "".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL i-nr-seq-setor wReport
ON MOUSE-SELECT-DBLCLICK OF i-nr-seq-setor IN FRAME fPage2 /* Setor */
DO:
  apply 'F5' to self.
END.
...
i-nr-seq-procedimento:load-mouse-pointer("image/lupa.cur") IN FRAME fPage2.
i-nr-seq-setor:load-mouse-pointer("image/lupa.cur") IN FRAME fPage2.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.des-auditor     = input frame fPage2 c-des-auditor
           tt-param.nr-procedimento = input frame fPage2 i-nr-seq-procedimento
           tt-param.nr-setor        = input frame fPage2 i-nr-seq-setor
           tt-param.tip-proced      = int(input frame fPage2 rs-tip-proced)
           tt-param.tip-grafico     = int(input frame fPage2 rs-tip-grafico)
           &if "{&RTF}":U = "YES":U &then
    ...
    {report/rprun.i esp/aqp/esaqp023rp.p}
