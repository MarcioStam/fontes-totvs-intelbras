...
{include/i-prgvrs.i ESAQP021RP 1.00.00.000}
...
if tt-param.destino = 3 or
   tt-param.destino = 1 then
    assign v_des_destino = string(session:temp-directory + "ESAQP021" + string (time) + ".xlsx").
else
    ...
    for each auditoria-visual
       where auditoria-visual.nr-seq-auditoria = auditoria-geral.nr-seq-auditoria:
        find first tt-registros exclusive-lock
             where tt-registros.nr-seq-orig-prob = auditoria-visual.nr-seq-orig-prob no-error.
        if not avail tt-registros then do:
            create tt-registros.
            assign tt-registros.nr-seq-orig-prob = auditoria-visual.nr-seq-orig-prob. 
        end.
...
procedure pi-cria-planilha:
    ...
    chExcel:selection:value = "LOTES REVISADOS POR ORIGEM".
...
procedure pi-imprime-registros:
    ...
    chExcel:selection:value = "Total Lotes".
    chExcel:selection:Font:Bold           = True.
    chExcel:selection:Borders({&xlEdgeLeftTopBottomRight}):LineStyle = 1.
    chExcel:selection:Interior:ColorIndex = 15.
    ...
    for each tt-registros:
        ...
        find first aq-origem-prob no-lock
             where aq-origem-prob.nr-seq-orig-prob = tt-registros.nr-seq-orig-prob no-error.
...
procedure pi-cria-grafico:
    ...
    chExcel:ActiveChart:ChartTitle:Characters:Text = "LOTES REVISADOS POR PRODUTO".
    ...
    assign v_des_orig_prob = chPlanilha:Range("A6"):value.
    
    chExcel:ActiveChart:SeriesCollection(1):XValues = "Plan1!$A$6:$A$" + string(i-linha).
    chExcel:ActiveChart:SeriesCollection(1):name    = v_des_orig_prob.
