...
*******************************************************************************/
{include/i-prgvrs.i ESIMP006 1.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESIMP006
...
&GLOBAL-DEFINE page4Widgets   rs-moeda tb-despesas tb-fator-interna tb-invoices 
...
&GLOBAL-DEFINE page4Text      text-email text-imprime text-moeda
&GLOBAL-DEFINE page6Text      text-destino text-modo text-rtf text-ModelRtf

&GLOBAL-DEFINE page2Fields    fi-fim-data fi-ini-data
&GLOBAL-DEFINE page4Fields    fi-email-emb-1 fi-email-emb-2 fi-email-emb-3 fi-email-emb-4~
                              fi-cod-estabel 
...
DEFINE VARIABLE fi-email-emb-1 AS CHARACTER FORMAT "X(60)":U 
     LABEL "E-Mail" 
     VIEW-AS FILL-IN 
     SIZE 61 BY .88 NO-UNDO.

DEFINE VARIABLE fi-email-emb-2 AS CHARACTER FORMAT "X(60)":U 
     LABEL "E-Mail" 
     VIEW-AS FILL-IN 
     SIZE 61 BY .88 NO-UNDO.

DEFINE VARIABLE fi-email-emb-3 AS CHARACTER FORMAT "X(60)":U 
     LABEL "E-Mail" 
     VIEW-AS FILL-IN 
     SIZE 61 BY .88 NO-UNDO.

DEFINE VARIABLE fi-email-emb-4 AS CHARACTER FORMAT "X(60)":U 
     LABEL "E-Mail" 
     VIEW-AS FILL-IN 
     SIZE 61 BY .88 NO-UNDO.
...
DEFINE VARIABLE text-email AS CHARACTER FORMAT "X(256)":U INITIAL "E-Mails" 
      VIEW-AS TEXT 
     SIZE 14.86 BY .63
     FONT 1 NO-UNDO.
...
DEFINE VARIABLE text-moeda AS CHARACTER FORMAT "X(256)":U INITIAL "FOB/Frete/Seguro" 
      VIEW-AS TEXT 
     SIZE 14.86 BY .63
     FONT 1 NO-UNDO.
...
DEFINE VARIABLE tb-despesas AS LOGICAL INITIAL no 
     LABEL "Despesas" 
     VIEW-AS TOGGLE-BOX
     SIZE 11.29 BY .83 NO-UNDO.

DEFINE VARIABLE tb-fator-interna AS LOGICAL INITIAL no 
     LABEL "Fator Internaá∆o" 
     VIEW-AS TOGGLE-BOX
     SIZE 14 BY .83 NO-UNDO.

DEFINE VARIABLE tb-invoices AS LOGICAL INITIAL no 
     LABEL "Invoices" 
     VIEW-AS TOGGLE-BOX
     SIZE 11.29 BY .83 NO-UNDO.
...
DEFINE FRAME fPage2
     fi-ini-data AT ROW 5.25 COL 26 COLON-ALIGNED WIDGET-ID 70
     fi-fim-data AT ROW 5.25 COL 52.14 COLON-ALIGNED NO-LABEL WIDGET-ID 72
...
DEFINE FRAME fPage4
     rs-moeda AT ROW 2.25 COL 7 NO-LABEL WIDGET-ID 20
     tb-despesas AT ROW 2.25 COL 45 WIDGET-ID 60
     tb-fator-interna AT ROW 2.25 COL 61 WIDGET-ID 66
     tb-invoices AT ROW 3.25 COL 45 WIDGET-ID 62
     fi-email-emb-1 AT ROW 5.5 COL 15 COLON-ALIGNED WIDGET-ID 68
     fi-email-emb-2 AT ROW 6.5 COL 15 COLON-ALIGNED WIDGET-ID 72
     fi-email-emb-3 AT ROW 7.5 COL 15 COLON-ALIGNED WIDGET-ID 74
     fi-email-emb-4 AT ROW 8.5 COL 15 COLON-ALIGNED WIDGET-ID 76
     fi-cod-estabel AT ROW 10 COL 15 COLON-ALIGNED HELP
          "C¢digo do estabelecimento" WIDGET-ID 56
     fi-nome AT ROW 10 COL 20.57 COLON-ALIGNED HELP
          "Nome Estabelecimento" NO-LABEL WIDGET-ID 58 NO-TAB-STOP 
     text-moeda AT ROW 1.25 COL 6.14 NO-LABEL WIDGET-ID 32
     text-imprime AT ROW 1.25 COL 44 NO-LABEL WIDGET-ID 64
     text-email AT ROW 4.75 COL 6 NO-LABEL WIDGET-ID 78
     RECT-12 AT ROW 1.5 COL 4 WIDGET-ID 34
     RECT-15 AT ROW 1.5 COL 42 WIDGET-ID 48
     RECT-13 AT ROW 5 COL 4 WIDGET-ID 70
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
/* SETTINGS FOR FILL-IN fi-nome IN FRAME fPage4
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN text-email IN FRAME fPage4
   NO-DISPLAY NO-ENABLE ALIGN-L                                         */
ASSIGN 
       text-email:PRIVATE-DATA IN FRAME fPage4     = 
                "E-Mails".

/* SETTINGS FOR FILL-IN text-imprime IN FRAME fPage4
   NO-DISPLAY NO-ENABLE ALIGN-L                                         */
ASSIGN 
       text-imprime:PRIVATE-DATA IN FRAME fPage4     = 
                "Imprime".

/* SETTINGS FOR FILL-IN text-moeda IN FRAME fPage4
   NO-DISPLAY NO-ENABLE ALIGN-L                                         */
ASSIGN 
       text-moeda:PRIVATE-DATA IN FRAME fPage4     = 
                "FOB/Frete/Seguro".
...
if avail usuar_mestre THEN do:
  assign fi-email-emb-1 = usuar_mestre.cod_e_mail_local.
end.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.ind-moeda       = input frame fPage4 rs-moeda
           tt-param.log-despesas    = input frame fPage4 tb-despesas
           tt-param.log-invoices    = input frame fPage4 tb-invoices
           tt-param.log-fator-interna = input frame fPage4 tb-fator-interna
           tt-param.data-ini          = input frame fPage2 fi-ini-data
           tt-param.data-fim          = input frame fPage2 fi-fim-data
           tt-param.e-mail[1]         = input frame fPage4 fi-email-emb-1
           tt-param.e-mail[2]         = input frame fPage4 fi-email-emb-2
           tt-param.e-mail[3]         = input frame fPage4 fi-email-emb-3
           tt-param.e-mail[4]         = input frame fPage4 fi-email-emb-4.
    ...
    {report/rprun.i esp/imp/esimp006rp.p}
