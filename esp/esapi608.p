...
def var jContaCtbl      as JsonObject no-undo.
def var jArrayContaCtbl as JsonArray  no-undo.
...
for first es-api-log
  ...
  if valid-handle(h-acomp) then 
    run pi-acompanhar in h-acomp ("Ariba Cta Ctbl").
  ...
  run pi-consulta-cta-ctbl.

  assign es-api-log.retorno-content-type = "application/json".
  if i-aux > 0 
  then do:
       jsonObjectOutput:add("account", jArrayContaCtbl).
       ...
       oError:add("errorInfo", "Nenhuma Conta Cont bil processada"). 
       oError:add("errorDescription", "Nenhuma Conta Cont bil processada").
       oErrors:add(oError).
  ...
  then assign es-api-log.cod-retorno = "200"
              es-api-log.aux         = "Contas Cont beis lidas com sucesso".
...
procedure pi-consulta-cta-ctbl:
    empty temp-table RowErrors.

    assign i-aux = 0.
    assign jArrayContaCtbl = new JsonArray().

    for each int_cta_ctbl use-index ch-ariba no-lock
       where int_cta_ctbl.ind_ariba          = yes
         and int_cta_ctbl.cod_plano_cta_ctbl = "PADRAO",
       first cta_ctbl no-lock
       where cta_ctbl.cod_plano_cta_ctbl = int_cta_ctbl.cod_plano_cta_ctbl
         and cta_ctbl.cod_cta_ctbl       = int_cta_ctbl.cod_cta_ctbl
         and cta_ctbl.dat_inic_valid    <= today
         and cta_ctbl.dat_fim_valid     >= today:
        assign i-aux = i-aux + 1.

        assign jContaCtbl = new JsonObject().
        jContaCtbl:add("ledgerAccount",cta_ctbl.cod_cta_ctbl).
        jContaCtbl:add("name",trim(replace(cta_ctbl.des_tit_ctbl,",","."))).
    
        jArrayContaCtbl:add(jContaCtbl).
    end. /* for each int_cta_ctbl */

    return "OK".
end procedure. /* procedure pi-consulta-cta-ctbl */
