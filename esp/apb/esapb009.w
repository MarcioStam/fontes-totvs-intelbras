...
/*****************************************************************************
**     Programa.........: esp/apb/esapb009.p
**     Descricao .......: Relat¢rio de Pagamentos por Transaá∆o
**     Versao...........: 1.00.001
**     Autor............: Giovane Alves
**     Criado...........: 21/02/2008
**     Desc. Atualizaá∆o: 
...
CREATE WIDGET-POOL.
&SCOPED-DEFINE NomProg   ESAPB009
&SCOPED-DEFINE DescProg  Relat¢rio de Pagamentos por Transaá∆o
...
&Scoped-Define ENABLED-OBJECTS bt_selec_estab IMAGE-1 IMAGE-2 IMAGE-4 IMAGE-9 RECT-2 ~
RECT-29 RECT-7 RECT-8 RECT-9 fi_ini_cod_tip_fluxo_financ ~
fi_fim_cod_tip_fluxo_financ fi_ini_data fi_fim_data c_cod_estab_selec ~
rs-execucao rs-destino bt-arquivo bt-cfimp c-arquivo ~
bt-imprime bt-salva 
&Scoped-Define DISPLAYED-OBJECTS bt_selec_estab fi_ini_cod_tip_fluxo_financ ~
fi_fim_cod_tip_fluxo_financ fi_ini_data fi_fim_data c_cod_estab_selec ~
rs-execucao rs-destino c-arquivo 
...
DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "esapb009.csv" 
     VIEW-AS FILL-IN 
     SIZE 44.29 BY .88 TOOLTIP "Destino"
     BGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE fi_fim_cod_tip_fluxo_financ AS CHARACTER FORMAT "x(12)" INITIAL "999" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fi_ini_cod_tip_fluxo_financ AS CHARACTER FORMAT "x(12)" INITIAL "200" 
     LABEL "Tipo Fluxo" 
     VIEW-AS FILL-IN 
     SIZE 14.86 BY .88 NO-UNDO.
...
DEFINE FRAME f-relat
     fi_ini_cod_tip_fluxo_financ AT ROW 1.5 COL 16 COLON-ALIGNED HELP
          "C¢digo Tipo Fluxo Financeiro" WIDGET-ID 2
     fi_fim_cod_tip_fluxo_financ AT ROW 1.5 COL 39 COLON-ALIGNED HELP
          "C¢digo Tipo Fluxo Financeiro" NO-LABEL WIDGET-ID 6
     fi_ini_data AT ROW 2.5 COL 16 COLON-ALIGNED HELP
          "Data" WIDGET-ID 4
     fi_fim_data AT ROW 2.5 COL 39 COLON-ALIGNED HELP
          "Data" NO-LABEL WIDGET-ID 8
     c_cod_estab_selec AT ROW 4.75 COL 14 COLON-ALIGNED HELP
          "C¢digo Estabelecimento" 
     bt_selec_estab     AT ROW 4.65 COL 43 FONT ?
     ...
     " Execuá∆o" VIEW-AS TEXT
          ...
          FONT 6
     IMAGE-1 AT ROW 2.5 COL 34
     IMAGE-2 AT ROW 1.5 COL 37
     IMAGE-4 AT ROW 2.5 COL 37
     IMAGE-9 AT ROW 1.5 COL 34
...
DO:
  ...
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "esapb009rp",
                               INPUT  "1.00.000",
                               INPUT  0,
...
DO:
  ...
  if input frame f-relat rs-destino = 1 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          assign c-arquivo:screen-value in frame f-relat = session:temp-directory + "esapb009.csv"
                 c-impressora                          = ""
                 c-layout                              = "".
      else
          assign c-arquivo:screen-value in frame f-relat = "esapb009.csv"
                 c-impressora                          = ""
                 c-layout                              = "".
...
------------------------------------------------------------------------------*/
  DISPLAY bt_selec_estab fi_ini_cod_tip_fluxo_financ fi_fim_cod_tip_fluxo_financ fi_ini_data 
          fi_fim_data c_cod_estab_selec rs-execucao rs-destino 
          c-arquivo 
      WITH FRAME f-relat IN WINDOW C-Win.
  ENABLE bt_selec_estab IMAGE-1 IMAGE-2 IMAGE-4 IMAGE-9 RECT-2 RECT-29 RECT-7 RECT-8 RECT-9 
         fi_ini_cod_tip_fluxo_financ fi_fim_cod_tip_fluxo_financ fi_ini_data 
         fi_fim_data rs-execucao rs-destino
...
/* */
  ...
  DO WITH FRAME f-relat:
    ASSIGN rs-execucao:SCREEN-VALUE       IN FRAME f-relat = entry(1,dwb_set_list_param.cod_dwb_parameters,chr(10))
           rs-destino:SCREEN-VALUE        IN FRAME f-relat = IF dwb_set_list_param.cod_dwb_output = "arquivo" THEN "1"
                                                             ELSE "2".
    ASSIGN fi_ini_cod_tip_fluxo_financ       =     ENTRY(02,dwb_set_list_param.cod_dwb_parameters,CHR(10))
           fi_fim_cod_tip_fluxo_financ       =     ENTRY(03,dwb_set_list_param.cod_dwb_parameters,CHR(10)) NO-ERROR.
...
/* */
if input frame f-relat rs-destino = 1 then 
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "esapb009"
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "esapb009" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "esapb009"
...
/* */
  ASSIGN INPUT FRAME f-relat c-arquivo
                             rs-destino
                             rs-execucao
                             fi_ini_cod_tip_fluxo_financ 
                             fi_fim_cod_tip_fluxo_financ 
                             fi_ini_data 
                             fi_fim_data 
                             c_cod_estab_selec.
  ...
  ASSIGN dwb_set_list_param.Cod_dwb_program          = "esapb009"
         ...
         dwb_set_list_param.Cod_dwb_output           = IF rs-destino = 1 THEN "Arquivo"
                                                       ELSE "Terminal".
                                                                        
                                                                            
  ASSIGN dwb_set_list_param.Cod_dwb_parameters       = 
                 STRING(rs-execucao)                  + CHR(10) + 
                 STRING(fi_ini_cod_tip_fluxo_financ)  + CHR(10) + 
                 STRING(fi_fim_cod_tip_fluxo_financ)  + CHR(10) + 
                 STRING(fi_ini_data)                  + CHR(10) + 
                 STRING(fi_fim_data)                  + CHR(10) + 
                 STRING(c_cod_estab_selec)            no-error.
