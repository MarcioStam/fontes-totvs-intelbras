...
** Versao................: 5.00.00.000
** Nome Externo..........: esp/apb/esapb031rp.p
...
def new shared var v_rpt_s_1_name    as character initial "Logs Importaá∆o Despesa Cart∆o Amex".
...
def frame f_rnl_31_histor_fornec_importar_ems
    ...
    with 1 down side-labels no-validate keep-tab-order three-d
         ...
         view-as dialog-box
         font 1 fgcolor ? bgcolor 8
         title "Executa Importaá∆o Despesa Cart‰es Amex".
    /* adjust size of objects in this frame */
...
FOR FIRST tt-prog-ponto:
    ...
    IF SUBSTRING(tt-prog-ponto.conteudo, LENGTH(tt-prog-ponto.conteudo), 1) <> "~\" THEN
        ASSIGN tt-prog-ponto.conteudo = tt-prog-ponto.conteudo + "~\".
    
    ASSIGN ed_1x40 = tt-prog-ponto.conteudo + 'spool\esapb031.txt'.
END.
...
do on endkey undo main_block, leave main_block on error undo main_block, leave main_block:
    ...
    if  rs_ind_message_output:screen-value = "Em Arquivo" /*l_on_file*/ then do:
        assign v_nom_prog_ext  = caps("ESAPB031":U)
...
PROCEDURE pi_rnl_histor_fornec_importar_ems:
    ...
    DEFINE VARIABLE v_log_avail_for AS LOG INIT NO NO-UNDO.
    ...
    RUN pi_wait_processing (INPUT "Importando Despesas Cart∆o Amex",
                            INPUT "Importaá∆o Despesas Cart∆o Amex").
    ...
    REPEAT TRANSACTION:
        ...
        ASSIGN v_cod_tip_reg = SUBSTRING(v_des_reg_import,1,1)
               v_num_line    = v_num_line + 1.


        /*** S¢ utiliza o registro 2 - Resumo de Transaá‰es do Portador ***/
        IF  v_cod_tip_reg <> "2" THEN
            NEXT.


        /*** Resumo de Transaá‰es do Portador ***/
        IF  v_cod_tip_reg = "2" THEN DO:
            ASSIGN v_cdn_fornec = INT(SUBSTRING(v_des_reg_import,151,10)) /* Matr°cula */
                   v_val_tit_ap = TRUNCATE(DEC(STRING(SUBSTRING(v_des_reg_import,41,15),"9999999999999,99")),2) /* DÇbitos do portador */.

            /* Cria somente os movimentos se tiverem valor */
            IF  v_val_tit_ap = 0 THEN
                NEXT.

            FIND FIRST fornec_financ NO-LOCK
                WHERE  fornec_financ.cod_empresa = v_cod_empres_usuar
                AND    fornec_financ.cdn_fornec  = v_cdn_fornec NO-ERROR.
            IF  NOT AVAIL fornec_financ THEN DO:
                ASSIGN v_des_mensagem = SUBSTITUTE("Fornecedor &1 n∆o localizado!", v_cdn_fornec).
                RUN pi_print_editor ("s_1", v_des_mensagem, "     050", "", "     ", "", "     ").
                PUT STREAM s_1 UNFORMATTED  v_num_line TO 27 FORMAT  ">>,>>9"
            ...
            REPEAT WHILE v_log_refer_uni = NO:
               RUN pi_retorna_sugestao_referencia (INPUT  "C",
                                                   INPUT  TODAY,
                                                   OUTPUT v_cod_refer).
               RUN pi_verifica_refer_unica_apb (INPUT  v_cod_estab_usuar,
                                                INPUT  v_cod_refer,
            ...
            ASSIGN tt_integr_apb_antecip_pef_pend.tta_cod_empresa           = v_cod_empres_usuar
                   tt_integr_apb_antecip_pef_pend.tta_cod_estab             = v_cod_estab_usuar
                   tt_integr_apb_antecip_pef_pend.tta_cod_refer             = v_cod_refer
                   tt_integr_apb_antecip_pef_pend.tta_cod_espec_docto       = "GV"
                   tt_integr_apb_antecip_pef_pend.tta_cod_ser_docto         = "CC"
                   tt_integr_apb_antecip_pef_pend.tta_cdn_fornecedor        = v_cdn_fornec
                   tt_integr_apb_antecip_pef_pend.tta_cod_parcela           = "1"
                   tt_integr_apb_antecip_pef_pend.tta_cod_indic_econ        = "Real"
                   tt_integr_apb_antecip_pef_pend.tta_cod_portador          = ""
                   tt_integr_apb_antecip_pef_pend.tta_ind_tip_refer         = "Antecipaá∆o"
                   ...
                   tt_integr_apb_antecip_pef_pend.tta_cod_tit_ap            = STRING(TODAY, "99999999")
                   tt_integr_apb_antecip_pef_pend.tta_val_tit_ap            = v_val_tit_ap
                   tt_integr_apb_antecip_pef_pend.tta_dat_emis_docto        = v_dat_transacao
                   tt_integr_apb_antecip_pef_pend.tta_dat_vencto_tit_ap     = tt_integr_apb_antecip_pef_pend.tta_dat_emis_docto.
            ...
            ASSIGN tt_integr_apb_aprop_ctbl_pend.ttv_rec_antecip_pef_pend   = tt_integr_apb_antecip_pef_pend.ttv_rec_antecip_pef_pend
                   ...
                   tt_integr_apb_aprop_ctbl_pend.tta_cod_cta_ctbl           = "11910100"
    ...
    IF  CAN-FIND(FIRST tt_integr_apb_antecip_pef_pend) AND 
        ...
        DO TRANSACTION:
            ...
            IF  CAN-FIND(FIRST tt_log_erros_atualiz) THEN DO:
                ...
                MESSAGE "Erro ao incluir a Antecipaá∆o!"
            ...
            FIND LAST bord_ap NO-LOCK
                WHERE bord_ap.cod_estab_bord = v_cod_estab_usuar
                AND   bord_ap.cod_portador   = IF v_cod_estab_usuar = "102" THEN "3411" ELSE "341" NO-ERROR.
            ...
            ASSIGN tt_integr_apb_pagto.tta_cod_empresa               = v_cod_empres_usuar
                   tt_integr_apb_pagto.tta_cod_estab_bord            = v_cod_estab_usuar
                   tt_integr_apb_pagto.tta_cod_portador              = IF v_cod_estab_usuar = "102" THEN "3411" ELSE "341"
            ...
            MESSAGE "Importaá∆o conclu°da, borderì n£mero: " v_num_bord 
                VIEW-AS ALERT-BOX.
            *****************/
    ...
    IF  v_ind_message_output = "Em Arquivo" THEN DO:
