...
** Nome Externo .....: esp/apb/esapb032.p
...
def new global shared var v_arq_import_esapb032    as CHARACTER format "x(80)":U view-as editor max-chars 250 NO-WORD-WRAP size 40 by .88 bgcolor 15 font 2 label "Nome Arquivo" column-label "Arquivo" no-undo.
...
def button bt_zoo_portador label "Zoom" tooltip "Zoom" image-up file "image/im-zoo" image-insensitive file "image/ii-zoo" size 4 by 0.95.
def button bt_zoo_negoc    label "Zoom" tooltip "Zoom" image-up file "image/im-zoo" image-insensitive file "image/ii-zoo" size 4 by 0.95.

def new global shared var v_rec_portador as RECID format ">>>>>>9":U initial ? no-undo.
...
def new global shared var v_cod_estab_usuar as CHARACTER format "x(3)":U label "Estabelecimento" column-label "Estab" no-undo.
def new global shared var v_rec_unid_negoc as RECID format ">>>>>>9":U initial ? no-undo.
...
def frame f_bas_importa_APB
    ...
    v_portador_esapb032 at row 02.80 col 18.00 colon-aligned help "Portador do arquivo de importaá∆o" 
    bt_zoo_portador   at row 02.75 col 25 colon-aligned HELP 'Busca Portador'
    v_unid_negoc_esapb032 at row 03.80 col 18.00 colon-aligned help "Unidade de Negocio do arquivo de importaá∆o" 
    bt_zoo_negoc     at row 03.75 col 25 colon-aligned HELP 'Busca Unidade Negocio'
    v_dat_transacao_esapb032 at row 04.80 col 18.00 colon-aligned help "Data de transaá∆o utilizada para geraá∆o dos movimentos"
    v_arq_import_esapb032 at row 05.80 col 18.00 colon-aligned label "Nome Arquivo" help "Arquivo com os dados para importaá∆o" view-as editor max-chars 250 no-word-wrap size 46 by .88 bgcolor 15 font 2
    bt_get_file2_264936 at row 05.75 col 66.10 help "Buscar Arquivo"
    

    //v_val_total_esapb032 at row 04.80 col 18.00 colon-aligned help "Valor total do arquivo de importaá∆o"
    
    with 1 down side-labels no-validate keep-tab-order three-d
         size-char 80.72 by 07.29
         at row 02.25 col 01.00
         font 1 fgcolor ? bgcolor 8
         title "Geraá∆o T°tulos Contas a Pagar - ESAPB032 - ".
    ...
    assign bt_exi:width-chars     in frame f_bas_importa_APB = 04.00
           ...
           rt_mold:height-chars   in frame f_bas_importa_APB = 04.50
    ...
    assign v_arq_import_esapb032:return-inserted in frame f_bas_importa_APB = yes.
    ...
    assign bt_rnl1:private-data                  in frame f_bas_importa_APB = "HLP=000008794":U
           ...
           v_arq_import_esapb032:private-data    in frame f_bas_importa_APB = "HLP=000017147":U
...
OR  CHOOSE OF MENU-ITEM mi_run IN MENU m_10 DO:
    assign v_arq_import_esapb032   
           v_dat_transacao_esapb032
           v_portador_esapb032
           v_unid_negoc_esapb032.   

    FIND emscad.portador WHERE portador.cod_portador = v_portador_esapb032 NO-LOCK NO-ERROR.

    IF NOT AVAIL emscad.portador THEN
    DO:
        RUN utp/ut-msgs.p (INPUT "show":U,
                           INPUT 17006,
                           INPUT "Portador informado nao localizado").

        APPLY "ENTRY":U TO v_portador_esapb032 IN FRAME f_bas_importa_APB.
        RETURN NO-APPLY.
    END.


    find unid_negoc WHERE unid_negoc.cod_unid_negoc = v_unid_negoc_esapb032 NO-LOCK NO-ERROR.

    IF NOT AVAIL unid_negoc THEN
    DO:
        RUN utp/ut-msgs.p (INPUT "show":U,
                           INPUT 17006,
                           INPUT "Unid.Negocio informado nao localizado").

        APPLY "ENTRY":U TO v_unid_negoc_esapb032 IN FRAME f_bas_importa_APB.
        RETURN NO-APPLY.
    END.


    if  v_arq_import_esapb032    = "" THEN DO:
        RUN utp/ut-msgs.p (INPUT "show":U,
                           INPUT 17006,
                           INPUT "Arquivo de importaá∆o n∆o pode ficar em branco.":U +
                                 "~~":U +
                                 "Informe um <caminho>/<arquivo> v†lido para importaá∆o.":U).
        APPLY "ENTRY":U TO v_arq_import_esapb032 IN FRAME f_bas_importa_APB.
        RETURN NO-APPLY.
    END.
    
    IF  v_dat_transacao_esapb032 = ? THEN DO:
        RUN utp/ut-msgs.p (INPUT "show":U,
                           INPUT 17006,
                           INPUT "Data de Transaá∆o n∆o pode ficar em branco.":U +
                                 "~~":U +
                                 "Informe uma data v†lida para criaá∆o dos movimentos.":U).
        APPLY "ENTRY":U TO v_dat_transacao_esapb032 IN FRAME f_bas_importa_APB.
        RETURN NO-APPLY.
    END.
    ...
    /*
    IF  v_val_total_esapb032 = 0 THEN DO:
        RUN utp/ut-msgs.p (INPUT "show":U,
                           INPUT 17006,
                           INPUT "Valor Total n∆o pode ficar zerado.":U +
                                 "~~":U +
                                 "Informe um valor v†lido para importaá∆o. Este total ser† validado contra o total dos registros no arquivo de importaá∆o.":U).
        APPLY "ENTRY":U TO v_val_total_esapb032 IN FRAME f_bas_importa_APB.
        RETURN NO-APPLY.
    END.*/
    
    run esp/apb/esapb032rp.p.
END.
...
OR F5 OF v_arq_import_esapb032 IN FRAME f_bas_importa_APB 
OR MOUSE-SELECT-DBLCLICK OF v_arq_import_esapb032 IN FRAME f_bas_importa_APB DO:
    /************************* Variable Definition Begin ************************/
    ...
    if  v_log_pressed = yes then do:
        assign v_arq_import_esapb032:screen-value in frame f_bas_importa_APB = v_cod_get_file.
        apply "entry" to v_arq_import_esapb032 in frame f_bas_importa_APB.
    end /* if */.
...
ASSIGN v_portador_esapb032      = '0'
       v_unid_negoc_esapb032    = ''
       v_dat_transacao_esapb032 = TODAY.

disp v_arq_import_esapb032
     v_dat_transacao_esapb032
     v_portador_esapb032
     v_unid_negoc_esapb032
...
ON  CHOOSE OF bt_zoo_portador IN FRAME f_bas_importa_APB
OR F5 OF v_portador_esapb032  IN FRAME f_bas_importa_APB DO:
    ...
    do:
       find emscad.portador where recid(portador) = v_rec_portador no-lock no-error.

       IF AVAIL portador  THEN
          assign v_portador_esapb032:screen-value in frame f_bas_importa_APB = string(portador.cod_portador).

       /*
       display v_portador_esapb032
               with frame f_bas_importa_APB.*/

    end /* if */.
    apply "entry" to v_portador_esapb032 in frame f_bas_importa_APB.
end. /* ON  CHOOSE OF bt_zoo_136126 IN FRAME f_bap_01_portad_edi */


ON  CHOOSE OF bt_zoo_negoc IN FRAME f_bas_importa_APB
OR F5 OF v_unid_negoc_esapb032  IN FRAME f_bas_importa_APB DO:

    ASSIGN v_rec_unid_negoc = ?.

    run prgint/utb/utb011ka.p /*prg_sea_unid_negoc*/.

    if v_rec_unid_negoc <> ? then 
    do:
       find unid_negoc where recid(unid_negoc) =  v_rec_unid_negoc no-lock no-error.

       IF AVAIL unid_negoc THEN
          assign v_unid_negoc_esapb032:screen-value in frame f_bas_importa_APB = string(unid_negoc.cod_unid_negoc).
    end /* if */.

    apply "entry" to v_unid_negoc_esapb032 in frame f_bas_importa_APB.
end. /* ON  CHOOSE OF bt_zoo_136126 IN FRAME f_bap_01_portad_edi */
...
do on endkey undo main_block, leave main_block on error undo main_block, leave main_block:
    ...
    assign v_arq_import_esapb032:read-only    in frame f_bas_importa_APB = no
           v_dat_transacao_esapb032:read-only in frame f_bas_importa_APB = no.
           //v_portador_esapb032:READ-ONLY      in frame f_bas_importa_APB = no.
           //v_val_total_esapb032:read-only     in frame f_bas_importa_APB = no.
