/* ******************************************************************* 
*** Programa: esapb030
*** Descri‡Æo: Gera‡Æo controle cessÆo de cr‚dito fornecedor - Ita£
*** Data: 25/09/2013
*********************************************************************/
...
def temp-table tt_log_erros_atualiz_fat no-undo
    field tta_cod_estab                    as character format "x(3)" label "Estabelecimento" column-label "Estab"
...
DEF TEMP-TABLE tt_titulos NO-UNDO
    ...
    FIELD v_cod_tit_ap         AS CHAR FORMAT "x(16)"      LABEL "Titulo"
    ...
    FIELD v_prazo              AS INT                      LABEL "Prazo".

DEF TEMP-TABLE tt_item_bord_ap NO-UNDO LIKE item_bord_ap
    FIELD rec_tt_integr_apb_pagto AS RECID.
...
DEF BUTTON bt_cessao     LABEL "CessÆo" TOOLTIP "CessÆo"    SIZE 1 BY 1 AUTO-GO.
DEF BUTTON bt_can_cessao LABEL "Sair"   TOOLTIP "Sair"      SIZE 1 BY 1 AUTO-ENDKEY.
 
DEF BUTTON bt_elimina_nao_selec LABEL "Elimina nÆo Selecionados"   TOOLTIP "Elimina nÆo Selecionados" SIZE 1 BY 1.
...
def var v_hdl_apb902ze as HANDLE format ">>>>>>9":U no-undo.
DEF VAR c-destinatario AS CHAR FORMAT "x(200)"      NO-UNDO.
DEF VAR c-tabela       AS LONGCHAR                  NO-UNDO.
DEF VAR c-corpo-email  AS LONGCHAR                  NO-UNDO.
DEF VAR de-total       LIKE tit_ap.val_sdo_tit_ap   NO-UNDO.
...
DEF BUFFER b_bord_ap FOR bord_ap.

{esp/es0018.i}
DEFINE VARIABLE c-dir-envio AS CHARACTER   NO-UNDO.
...
def new shared var v_rpt_s_1_name    as character initial "Logs Importa‡Æo Ita£".
...
def frame f_concil
    ...
    ed_1x40
         ...
         help "" no-label
    bt_get_file
         at row 1.85 col 106.2 font ?
         help "Pesquisa Arquivo"
    rt_cxcf
         at row 23.00 col 02.00 bgcolor 7
    rt_001
         at row 3.5 col 90.00 bgcolor 8
    rt_002
         at ROW 12.5 col 90.00 bgcolor 8
    v_cod_estab_ini
         AT ROW 05 COL 92 LABEL "Estabelec"
         view-as fill-in
         size-chars 04.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_cod_estab_fim
         AT ROW 05 COL 103.7 LABEL "at‚"
         view-as fill-in
         size-chars 04.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_cdn_fornec
         AT ROW 06.5 COL 91.22 LABEL "Fornecedor"
         view-as fill-in
         size-chars 11.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_dat_vencto  
         AT ROW 08 COL 91 LABEL "Vencimento"
         view-as fill-in
         size-chars 11.14 by .88
         fgcolor ? bgcolor 15 font 2
    v_cod_estab_imp
         AT ROW 6.5 COL 91 LABEL "Estab Pagto"
         view-as fill-in
         size-chars 4.14 by .88
         fgcolor ? bgcolor 15 font 2 
    bt_pesquisa_titulos
         at row 10.21 col 91.00 font ?
    bt_elimina_nao_selec
         at row 15 col 91.00 font ?
    bt_elimina_todos
         at row 17 col 91.00 font ?
    bt_seleciona_todos
         at row 19 col 91.00 font ?
    br_tt_titulos
         AT ROW 3.5 COL 2
    bt_cessao
    ...
    with 1 down side-labels no-validate keep-tab-order three-d
         size-char 112.5 by 25 default-button bt_can_cessao
         view-as dialog-box
         font 1 fgcolor ? bgcolor 8
         title "CessÆo de Cr‚dito Ita£ - esapb030".
...
assign bt_cessao:width-chars       in frame f_concil = 10.00
       ...
       rt_cxcf:width-chars         in frame f_concil = 110
       ...
       rt_target:width-chars       in frame f_concil = 110
    ...
    FOR EACH estabelecimento NO-LOCK
        ...
        FOR EACH tit_ap NO-LOCK
            ...
            AND   tit_ap.dat_vencto     >= v_dat_vencto
               BY tit_ap.dat_vencto
               BY tit_ap.cod_tit_ap:
            
            FIND FIRST proces_pagto OF tit_ap 
                WHERE (proces_pagto.cod_portador         = "34101"
                OR     proces_pagto.cod_portador         = "341")
                AND    proces_pagto.ind_sit_proces_pagto = "Em Pagamento" NO-LOCK NO-ERROR.
            ...
            ASSIGN tt_titulos.v_log_selec     = NO
                   ...
                   tt_titulos.v_dat_emis      = tit_ap.dat_emis_docto
                   ...
                   tt_titulos.v_prazo         = int(tit_ap.dat_vencto - tit_ap.dat_emis_docto).
        END.
    END.
...
ON CHOOSE OF bt_elimina_nao_selec IN FRAME f_concil DO:
...
DO:
    ...
    RUN esp/es0018p.p (INPUT "esapb030":U,
                       INPUT 1,
                       INPUT 0,
                       INPUT "":U,
    ...
    FOR FIRST tt-prog-ponto:
        ASSIGN c-dir-envio = replace(tt-prog-ponto.conteudo, "/", "~\").
    
        IF SUBSTRING(c-dir-envio, LENGTH(c-dir-envio), 1) <> "~\" THEN
            ASSIGN c-dir-envio = c-dir-envio + "~\".
    END.
        ...
        case self:screen-value:
            when "Exportar" /*l_file*/ then fil:
             do:
                ...
                ASSIGN v_cod_dwb_file = c-dir-envio + "itau" + STRING(YEAR(TODAY), "9999") + STRING(MONTH(TODAY), "99") + STRING(DAY(TODAY), "99") + STRING(TIME) + ".csv"
            ...
            when "Importar" /*l_file*/ then fil:
             do:
                assign ed_1x40:SCREEN-VALUE    = "S:/Financeiro/cessao/itau/retorno/"
                       ed_1x40:SENSITIVE       = YES
...
ASSIGN v_des_contdo_prog_valid_dtsul = 'esapb030'.
...
ASSIGN rs_cod_dwb_output = "Exportar".
...
/*selecionar titulos conforme faixa e sele‡Æo com portador diferente de 745*/
    ...
    DO TRANSACTION on error undo trans_block, leave trans_block:
        ...
        FOR EACH tt_titulos
            ...
            ASSIGN tt_tit_ap_alteracao_base_aux_1.ttv_cod_usuar_corren              = v_cod_usuar_corren          
                   ...
                   tt_tit_ap_alteracao_base_aux_1.tta_cod_portador                  = "34101"
                   tt_tit_ap_alteracao_base_aux_1.tta_des_histor_padr               = "Altera‡Æo do portador - esapb030 - CessÆo Cr‚dito Disponibilizado.".
        ...
        FOR EACH tt_log_erros_tit_ap_alteracao NO-LOCK
            WHERE tt_log_erros_tit_ap_alteracao.ttv_num_mensagem <> 6542
              AND tt_log_erros_tit_ap_alteracao.ttv_num_mensagem <> 6788:
            MESSAGE "API Altera‡Æo" SKIP
        ...
        THEN DO:
             ...
             MESSAGE "Opera‡Æo Cancelada !"
                 VIEW-AS ALERT-BOX INFO BUTTONS OK.
             UNDO trans_block, RETURN ERROR.
        END.

        /*---[ In¡cio do trecho border“ ]----------------------------------------------------------------------*/
        assign v_nom_prog_ext   = caps("esapb030":U)
               v_cod_release    = " 5.01.00.000":U
               v_dat_execution  = today
               v_hra_execution  = replace(string(time,"hh:mm:ss"),":","")
               v_nom_filename   = SESSION:TEMP-DIRECTORY + "esapb030-bordero.txt"
        ...
        EMPTY TEMP-TABLE tt_item_bord_ap.
        ...
        FOR EACH tt_titulos
            ...
            IF  FIRST-OF(tt_titulos.v_dat_vcto) THEN DO:
                ...
                IF  v_num_bord = 0 THEN DO:
                    FIND LAST bord_ap
                        ...
                        AND   bord_ap.cod_portador   = "34101" NO-LOCK NO-ERROR.
                ...
                ASSIGN tt_integr_apb_pagto.tta_cod_empresa               = v_cod_empres_usuar
                       ...
                       tt_integr_apb_pagto.tta_cod_portador              = "34101"
                ...
                FOR EACH b_bord_ap
                    ...
                    AND   b_bord_ap.cod_portador  = "34101"
                    AND   b_bord_ap.dat_transacao = tt_titulos.v_dat_vcto NO-LOCK:

                    IF  b_bord_ap.ind_sit_bord_ap = "Ja Impresso"
                    OR  b_bord_ap.ind_sit_bord_ap = "Em Digita‡Æo"
                    OR  b_bord_ap.ind_sit_bord_ap = "Transmitir ao Banco" THEN DO:

                        FOR EACH item_bord_ap NO-LOCK
                            WHERE item_bord_ap.cod_estab_bord = b_bord_ap.cod_estab_bord
                            AND   item_bord_ap.cod_portador   = b_bord_ap.cod_portador
                            AND   item_bord_ap.num_bord_ap    = b_bord_ap.num_bord_ap:

                            CREATE tt_item_bord_ap.
                            BUFFER-COPY item_bord_ap TO tt_item_bord_ap.

                            ASSIGN tt_item_bord_ap.rec_tt_integr_apb_pagto = RECID(tt_integr_apb_pagto).
                        END.
                    END.
                END.
            END.
            ...
            ASSIGN tt_integr_apb_bord_lote_pagto.tta_cod_empresa           = v_cod_empres_usuar
                   ...
                   tt_integr_apb_bord_lote_pagto.tta_cod_portador          = "34101"
                   ...
                   tt_integr_apb_bord_lote_pagto.tta_cod_forma_pagto       = "200"
        ...
        FOR EACH tt_item_bord_ap:
            FIND FIRST item_bord_ap
                WHERE item_bord_ap.cod_estab_bord = tt_item_bord_ap.cod_estab_bord
                AND   item_bord_ap.cod_portador   = tt_item_bord_ap.cod_portador
                AND   item_bord_ap.num_bord_ap    = tt_item_bord_ap.num_bord_ap
                AND   item_bord_ap.num_seq_bord   = tt_item_bord_ap.num_seq_bord EXCLUSIVE-LOCK NO-ERROR.
            ...
            IF  AVAIL item_bord_ap THEN DO:
                ...
                ASSIGN tt_integr_apb_bord_lote_pagto.tta_cod_empresa           = v_cod_empres_usuar
                       tt_integr_apb_bord_lote_pagto.ttv_cod_estab_bord_refer  = v_cod_estab_imp
                       tt_integr_apb_bord_lote_pagto.tta_cod_portador          = "34101"
                       tt_integr_apb_bord_lote_pagto.tta_cod_estab             = item_bord_ap.cod_estab
                       tt_integr_apb_bord_lote_pagto.tta_cod_espec_docto       = item_bord_ap.cod_espec_docto
                       tt_integr_apb_bord_lote_pagto.tta_cod_ser_docto         = item_bord_ap.cod_ser_docto
                       tt_integr_apb_bord_lote_pagto.tta_cdn_fornecedor        = item_bord_ap.cdn_fornecedor
                       tt_integr_apb_bord_lote_pagto.tta_cod_tit_ap            = item_bord_ap.cod_tit_ap
                       tt_integr_apb_bord_lote_pagto.tta_cod_parcela           = item_bord_ap.cod_parcela
                       tt_integr_apb_bord_lote_pagto.tta_val_pagto             = item_bord_ap.val_pagto
                       tt_integr_apb_bord_lote_pagto.tta_val_cotac_indic_econ  = 1
                       tt_integr_apb_bord_lote_pagto.tta_cod_forma_pagto       = "200"
                       tt_integr_apb_bord_lote_pagto.tta_cod_indic_econ        = "Real"
                       tt_integr_apb_bord_lote_pagto.tta_ind_sit_item_bord_ap  = "Enviado ao Banco"
                       tt_integr_apb_bord_lote_pagto.tta_log_critic_atualiz_ok = NO
                       tt_integr_apb_bord_lote_pagto.ttv_ind_forma_pagto       = "Informada"
                       tt_integr_apb_bord_lote_pagto.ttv_rec_table_parent      = tt_item_bord_ap.rec_tt_integr_apb_pagto
                       tt_integr_apb_bord_lote_pagto.ttv_rec_table_child       = RECID(tt_integr_apb_bord_lote_pagto).

                FIND FIRST bord_ap
                     WHERE bord_ap.cod_estab_bord = tt_item_bord_ap.cod_estab_bord
                     AND   bord_ap.cod_portador   = tt_item_bord_ap.cod_portador
                     AND   bord_ap.num_bord_ap    = tt_item_bord_ap.num_bord_ap EXCLUSIVE-LOCK NO-ERROR.
                ...
                IF  AVAIL tit_ap THEN DO:
                    FIND LAST proces_pagto OF tit_ap
                        WHERE proces_pagto.ind_sit_proces_pagto = "Em Pagamento"
                        AND   proces_pagto.ind_modo_pagto       = "Border“" EXCLUSIVE-LOCK NO-ERROR.
        ...
        FOR EACH tt_log_erros_atualiz:
            ...
            ASSIGN v_des_mensagem = "Erro API Border“: " + tt_log_erros_atualiz.ttv_des_msg_erro + tt_log_erros_atualiz.ttv_des_msg_ajuda.
        ...
        ELSE DO:
            FOR EACH tt_integr_apb_pagto:
                ASSIGN v_des_mensagem = "Border“ Gerado: " + tt_integr_apb_pagto.tta_cod_estab_bord + "-" + tt_integr_apb_pagto.tta_cod_portador + "-" + string(tt_integr_apb_pagto.tta_num_bord_ap) + "-" + string(tt_integr_apb_pagto.tta_dat_transacao).
                ...
                FOR EACH tt_integr_apb_bord_lote_pagto OF tt_integr_apb_pagto:
                    ASSIGN v_des_mensagem = "Item Border“: " + tt_integr_apb_bord_lote_pagto.tta_cod_estab      + "/" +
...
PROCEDURE pi_gera_arquivo:
    ...
    ASSIGN c-tabela = c-tabela + "<TH>" + "Dt Envio" + "</TH>" 
           c-tabela = c-tabela + "<TH>" + "T¡tulo" + "</TH>" 
           c-tabela = c-tabela + "<TH>" + "Parcela" + "</TH>"
           c-tabela = c-tabela + "<TH>" + "Valor" + "</TH>"
           c-tabela = c-tabela + "<TH>" + "Dt Vencimento" + "</TH>" + "</TR>".

    ASSIGN c-corpo-email = "Prezado(a)," + "<BR>" + "<BR>" + "Informamos que foram encaminhados ao banco Ita£, via Risco Sacado, os t¡tulos abaixo:".

    FOR EACH tt_titulos
        WHERE tt_titulos.v_log_selec = YES:
    
        ASSIGN de-total = de-total + tt_titulos.v_val_sdo.

        ASSIGN c-tabela = c-tabela + "<TR>" + "<TD ALIGN=" + "'" + "center" + "'" + ">" + STRING(TODAY)                                  +  "</TD>"  
               c-tabela = c-tabela +          "<TD ALIGN=" + "'" + "center" + "'" + ">" + tt_titulos.v_cod_tit_ap                        +  "</TD>"  
               c-tabela = c-tabela +          "<TD ALIGN=" + "'" + "center" + "'" + ">" + tt_titulos.v_cod_parc                          +  "</TD>"  
               c-tabela = c-tabela +          "<TD ALIGN=" + "'" + "center" + "'" + ">" + string(tt_titulos.v_val_sdo, ">>>,>>>,>>9.99") +  "</TD>"
               c-tabela = c-tabela +          "<TD ALIGN=" + "'" + "center" + "'" + ">" + string(tt_titulos.v_dat_vcto)                  +  "</TD>" + "</TR>".

        FIND emitente NO-LOCK
            WHERE emitente.cod-emitente = tt_titulos.v_cdn_fornec NO-ERROR.
    
        ASSIGN c-destinatario = emitente.e-mail WHEN AVAIL emitente.

        PUT STREAM s_1 UNFORMATTED
                "2;82901000000127;1570;055556;INTELBRAS SA IND TELEC ELETR BRASILEIRA;"
                STRING(emitente.nome-emit, "x(30)")         ";"
                STRING(emitente.cgc, "99999999999999")      ";"            
                STRING(tt_titulos.v_dat_emis, "99/99/9999") ";"
                tt_titulos.v_cod_tit_ap                     ";0;"
                TRIM(STRING(tt_titulos.v_val_sdo * 100, ">>>>>>>>>>>>>9")) ";"
                STRING(tt_titulos.v_dat_vcto, "99/99/9999") SKIP.
    ...
    ASSIGN c-corpo-email = c-corpo-email +  c-tabela + "</TABLE></p>".

    ASSIGN c-tabela = ""
           c-tabela = c-tabela + "<p><TABLE BORDER ALIGN='CENTER' BGCOLOR='#FFFFFF' WIDTH='" + string(900) + "'>".
           c-tabela = c-tabela + "<TR>".

    ASSIGN c-tabela = c-tabela + "<TR>" + "<TH ALIGN=" +  "'" + "right"  + "'" + ">"  + "TOTAL: " + TRIM(STRING(de-total,">>>,>>>,>>9.99")) + "</TH>"  + "</TR>".

    ASSIGN c-corpo-email = c-corpo-email + c-tabela + "</TABLE></p>" + "<BR>" + "<BR>".

    ASSIGN c-corpo-email = c-corpo-email + "Em caso de d£vidas ou divergˆncias, favor entrar em contato atrav‚s do e-mailÿtesouraria@intelbras.com.brÿou (48) 3281-9594." 
                                         + "<BR>" + "<BR>" + "Atenciosamente," + "<BR>" + "Financeiro Intelbras".
...
PROCEDURE pi_retorna_titulo:
    ...
    assign v_nom_prog_ext  = caps("esapb030":U)
           ...
           v_dat_execution = today
           v_hra_execution = replace(string(time,"hh:mm:ss"),":","")
           v_nom_filename  = SESSION:TEMP-DIRECTORY + "esapb030.txt".
    ...
    REPEAT:
        ...
        IF SUBSTRING(v_des_reg_import, 49, 1) = "N" THEN ASSIGN v_des_ocor = "Pendente de Autoriza‡Æo".
        IF SUBSTRING(v_des_reg_import, 49, 1) = "F" THEN ASSIGN v_des_ocor = "Pendente da éltima Autoriza‡Æo".
        ...
        ASSIGN v_des_mensagem = "T¡tulo Importado: " + tit_ap.cod_estab + "/" + STRING(tit_ap.cdn_fornec) + "/" + tit_ap.cod_espec + "/" + tit_ap.cod_ser + "/" + tit_ap.cod_tit_ap + "/" + tit_ap.cod_parcela + " / Ocorrˆncia: " + v_des_ocor.
        RUN pi_print_editor ("s_1", v_des_mensagem, "     072", "", "     ", "", "     ").
        PUT STREAM s_1 UNFORMATTED ENTRY(1, RETURN-VALUE, CHR(255)) AT 4 FORMAT "x(72)" SKIP.
        RUN pi_print_editor ("s_1", v_des_mensagem, "at004072", "", "", "", "").
    ...
    do on error undo main_block, leave main_block on endkey undo main_block, leave main_block transaction:
        ...
        FOR EACH tt_titulos
            ...
            ASSIGN tt_tit_ap_alteracao_base_aux_1.ttv_cod_usuar_corren              = v_cod_usuar_corren          
                   ...
                   tt_tit_ap_alteracao_base_aux_1.tta_des_histor_padr               = "Altera‡Æo do portador - esapb030 - CessÆo Cr‚dito Rejeitado.".
            VALIDATE tt_tit_ap_alteracao_base_aux_1.
        ...
        FOR EACH tt_log_erros_tit_ap_alteracao NO-LOCK
            ...
            ASSIGN v_des_mensagem = "Erro API Altera‡Æo: " + tt_log_erros_tit_ap_alteracao.tta_cod_estab        + "/"         
        ...
        FOR EACH tt_titulos
            ...
            RUN pi_wait_processing (INPUT "Gerando Fatura...",
                                    INPUT "CessÆo Ita£").

            ASSIGN v_log_refer_uni = NO.
  
            REPEAT WHILE v_log_refer_uni = NO:
               RUN pi_retorna_sugestao_referencia (INPUT "F",
                                                   INPUT TODAY,
                                                   OUTPUT v_cod_refer).
            ...
            EMPTY TEMP-TABLE tt_log_erros_atualiz_fat.

            /* ** Instancia a API ***/
            RUN prgfin\apb\apb925za.py PERSISTENT SET v_hdl_apb925za.
  
            /* ** Popula Temp-Table Lote ***/
            CREATE tt_integr_apb_lote_fatura.
            ASSIGN tt_integr_apb_lote_fatura.tta_cod_estab                = v_cod_estab_imp
                   tt_integr_apb_lote_fatura.tta_cod_refer                = v_cod_refer
                   tt_integr_apb_lote_fatura.tta_cod_espec_docto          = 'CE'
                   tt_integr_apb_lote_fatura.tta_dat_transacao            = TODAY
                   tt_integr_apb_lote_fatura.tta_ind_origin_tit_ap        = 'APB'
                   tt_integr_apb_lote_fatura.tta_cod_empresa              = v_cod_empres_usuar
                   ...
                   tt_integr_apb_lote_fatura.tta_cdn_fornecedor           = tt_titulos.v_cdn_fornec
                   tt_integr_apb_lote_fatura.tta_num_fatur_ap             = INT(tt_titulos.v_cod_tit_ap)
                   tt_integr_apb_lote_fatura.tta_qtd_parcela              = 1
                   tt_integr_apb_lote_fatura.tta_cod_histor_padr          = 'Fatura Importada - esapb030'
                   tt_integr_apb_lote_fatura.tta_cod_histor_padr_dupl     = 'Fatura Importada - esapb030'
                   tt_integr_apb_lote_fatura.ttv_ind_matriz_fornec        = 'Matriz'
                   tt_integr_apb_lote_fatura.ttv_rec_integr_apb_lote_impl = RECID(tt_integr_apb_lote_fatura).
  
            /* ** Popula Temp-Table Origem ***/
            CREATE tt_integr_apb_relacto_fatura.
            ASSIGN tt_integr_apb_relacto_fatura.ttv_rec_integr_apb_lote_impl = tt_integr_apb_lote_fatura.ttv_rec_integr_apb_lote_impl
                   tt_integr_apb_relacto_fatura.tta_cod_estab_tit_ap_pai     = tt_titulos.v_cod_estab
                   tt_integr_apb_relacto_fatura.tta_cdn_fornec_pai           = tt_titulos.v_cdn_fornec
                   tt_integr_apb_relacto_fatura.tta_cod_espec_docto_nf       = tt_titulos.v_cod_espec
                   tt_integr_apb_relacto_fatura.tta_cod_ser_docto_nf         = tt_titulos.v_cod_ser
                   tt_integr_apb_relacto_fatura.tta_cod_tit_ap               = tt_titulos.v_cod_tit_ap
                   tt_integr_apb_relacto_fatura.tta_cod_parc_nf              = tt_titulos.v_cod_parc
                   tt_integr_apb_relacto_fatura.tta_ind_motiv_acerto_val     = 'Fatura'
                   tt_integr_apb_relacto_fatura.ttv_log_bxo_estab_tit        = NO.

            ASSIGN tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap = tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap + tit_ap.val_sdo_tit_ap.

            /* ** Popula Temp-Table Destino ***/
            CREATE tt_integr_apb_item_lote_fatura.
            ASSIGN tt_integr_apb_item_lote_fatura.ttv_rec_integr_apb_lote_impl  = tt_integr_apb_lote_fatura.ttv_rec_integr_apb_lote_impl  
                   tt_integr_apb_item_lote_fatura.tta_num_seq_refer             = 1
                   tt_integr_apb_item_lote_fatura.tta_cdn_fornecedor            = tt_titulos.v_cdn_fornec
                   tt_integr_apb_item_lote_fatura.tta_cod_ser_docto             = tt_titulos.v_cod_ser
                   tt_integr_apb_item_lote_fatura.tta_cod_tit_ap                = tt_titulos.v_cod_tit_ap
                   tt_integr_apb_item_lote_fatura.tta_cod_parcela               = tt_titulos.v_cod_parc 
                   tt_integr_apb_item_lote_fatura.tta_dat_emis_docto            = tt_titulos.v_dat_emis
                   tt_integr_apb_item_lote_fatura.tta_dat_vencto_tit_ap         = tt_titulos.v_dat_vcto
                   tt_integr_apb_item_lote_fatura.tta_dat_prev_pagto            = tt_titulos.v_dat_vcto
                   tt_integr_apb_item_lote_fatura.tta_cod_indic_econ            = 'Real'
                   tt_integr_apb_item_lote_fatura.tta_val_tit_ap                = tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap
                   tt_integr_apb_item_lote_fatura.tta_cod_portador              = '341'
                   tt_integr_apb_item_lote_fatura.ttv_rec_integr_apb_item_lote  = RECID(tt_integr_apb_item_lote_fatura)
                   tt_integr_apb_item_lote_fatura.tta_des_text_histor           = 'Fatura Importada - esapb030'
                   tt_integr_apb_item_lote_fatura.tta_val_cotac_indic_econ      = 1.
            ...
            RUN pi_main_code_apb925za IN v_hdl_apb925za (INPUT 1,
                                                         ...
                                                         INPUT-OUTPUT TABLE tt_log_erros_atualiz_fat).
  
            /* ** Elimina Instanciamento ***/
            DELETE PROCEDURE v_hdl_apb925za NO-ERROR.
  
            /* ** Apresenta Erros ***/
            FOR EACH tt_log_erros_atualiz_fat:
    
                 ASSIGN v_des_mensagem = "Erro API Fatura: " + tt_log_erros_atualiz_fat.ttv_des_msg_erro.         
            ...
            END.
            IF CAN-FIND(FIRST tt_log_erros_atualiz_fat) 
            THEN DO: 
                 ASSIGN v_log_method = SESSION:SET-WAIT-STATE("").
                 MESSAGE "Erro ao incluir a Fatura !"
                     VIEW-AS ALERT-BOX INFO BUTTONS OK.
                 UNDO main_block, LEAVE main_block.
            END. 
        END.
...
PROCEDURE pi_gera_email:
    ...
    ASSIGN tt-envio2.versao-integracao = 1
           ...
           tt-envio2.copia             = "tesouraria@intelbras.com.br"
           tt-envio2.assunto           = "CessÆo de cr‚dito Ita£"
           tt-envio2.arq-anexo         = ""
