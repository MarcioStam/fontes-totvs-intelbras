...
**
** Nome Externo .....: esp/apb/esapb033rp.p
**
...
def temp-table tt_file_import no-undo
    ...
    field tta_unid_negoc           as character format "x(3)"                                 label "Unidade Neg¢cio"   column-label "Unid Neg"
    field tta_cod_ccusto           as character format "x(11)"                                label "Centro Custo"      column-label "C.Custo"
    ...
    field tta_historico            as character format "x(200)"                               label "Hist¢rico"         column-label "Hist"
    field tta_cod_indic_econ       as character format "x(8)"                                 label "Moeda"             column-label "Moeda".
     
def temp-table tt_aprop_file_import no-undo
    field tta_cod_estab            as character format "x(3)"                                 label "Estabelecimento"   column-label "Estab"
    field tta_cod_evento           as character format "x(40)"                                label "Evento"            column-label "Evento"
    field tta_cdn_fornecedor       as Integer   format ">>>,>>>,>>9" initial 0                label "Fornecedor"        column-label "Fornecedor"
    field tta_cod_cta_ctbl         as character format "x(20)"                                label "Conta Contabil"    column-label "Conta Contabil"
    ...
    field tta_cod_parcela          as character format "x(02)"                                label "Parcela"           column-label "Parc"
    field tta_cod_espec_docto      as character format "x(3)"                                 label "EspÇcie Documento" column-label "EspÇcie"
    field tta_val_aprop_tit_ap     as decimal   format "->>>,>>>,>>9.99" decimals 2 initial 0 label "Valor T°tulo"      column-label "Valor T°tulo".
...
DEF TEMP-TABLE tt_antecip NO-UNDO
    field cod_estab_dp        as character format "x(3)"                                 label "Estab DP"       column-label "Estab DP"    
    field cod_espec_docto_dp  as character format "x(3)"                                 label "EspÇcie DP"     column-label "EspÇcie DP"  
    field cod_ser_docto_dp    as character format "x(3)"                                 label "SÇrie DP"       column-label "SÇrie DP"    
    field cdn_fornecedor_dp   as Integer   format ">>>,>>>,>>9" initial 0                label "Fornec DP"      column-label "Fornec DP"   
    field cod_tit_ap_dp       as character format "x(10)"                                label "T°tulo DP"      column-label "T°tulo DP"   
    field cod_parcela_dp      as character format "x(02)"                                label "Parcela DP"     column-label "Parcela DP"  
    field cod_estab_an        as character format "x(3)"                                 label "Estab AN"       column-label "Estab AN"    
    field cod_espec_docto_an  as character format "x(3)"                                 label "EspÇcie AN"     column-label "EspÇcie AN"  
    field cod_ser_docto_an    as character format "x(3)"                                 label "SÇrie AN"       column-label "SÇrie AN"    
    field cdn_fornecedor_an   as Integer   format ">>>,>>>,>>9" initial 0                label "Fornec AN"      column-label "Fornec AN"   
    field cod_tit_ap_an       as character format "x(10)"                                label "T°tulo AN"      column-label "T°tulo AN"   
    field cod_parcela_an      as character format "x(02)"                                label "Parcela AN"     column-label "Parcela AN"  
    field val_abat            as DECIMAL   format ">>>>>,>>>,>>9.99"                     LABEL "Val Abatimento" column-label "Vl Abat"
    field val_saldo           as DECIMAL   format ">>>>>,>>>,>>9.99"                     LABEL "Val Saldo" column-label "Vl Saldo".
...
def var v_val_saldo_abat       as DECIMAL format ">>>>>,>>>,>>9.99":U INITIAL 0 no-undo.
def var v_val_abat             as DECIMAL format ">>>>>,>>>,>>9.99":U INITIAL 0 no-undo.
def var v_val_sdo_tit_ap       as DECIMAL format ">>>>>,>>>,>>9.99":U INITIAL 0 no-undo.
def var v_val_tt_antecip       as DECIMAL format ">>>>>,>>>,>>9.99":U INITIAL 0 no-undo.
def var v_cod_estab            like tt_file_import.tta_cod_estab              no-undo.
def var v_cdn_fornecedor       like tt_file_import.tta_cdn_fornecedor         no-undo.
def var v_cod_ser_docto        like tt_file_import.tta_cod_ser_docto          no-undo.
def var v_cod_tit_ap           like tt_file_import.tta_cod_tit_ap             no-undo.
DEF VAR v_cod_parcela          like tt_file_import.tta_cod_parcela            no-undo.
def var v_cod_espec_docto      like tt_file_import.tta_cod_espec_docto        no-undo.
def var v_dat_vencto_tit_ap    like tt_file_import.tta_dat_vencto_tit_ap      no-undo.
def var v_historico            like tt_file_import.tta_historico              no-undo.
def var v_val_cotac            like emsuni.cotac_parid.val_cotac_indic_econ   no-undo.
...
def new global shared var v_arq_import_esapb033    as CHARACTER format "x(80)":U view-as editor max-chars 250 NO-WORD-WRAP size 40 by 1 bgcolor 15 font 2 label "Nome Arquivo" column-label "Arquivo" no-undo.
...
FOR FIRST tt-prog-ponto:
    ...
    ASSIGN ed_1x40 = IF rs_tipo_exec_esapb033 = 1 THEN tt-prog-ponto.conteudo + 'ESAPB033_Fundo_Fixo.txt' ELSE IF  rs_tipo_exec_esapb033 = 2 THEN tt-prog-ponto.conteudo + 'ESAPB033_Verbas.txt' ELSE tt-prog-ponto.conteudo + 'ESAPB033_Export.txt'.
END.
...
do on endkey undo main_block, leave main_block on error undo main_block, leave main_block:
    ...
    if  rs_ind_message_output:screen-value = "Em Arquivo" then do:
        assign v_nom_prog_ext  = caps("esapb033":U)
...
PROCEDURE pi_importa_APB:
    ...
    ASSIGN v_nom_report_title = IF rs_tipo_exec_esapb033 = 1 THEN "Importa Planilha Fundo Fixo" ELSE IF rs_tipo_exec_esapb033 = 2 THEN "Importa Planilha Verbas" ELSE "Importa Planilha de Exportaá‰es".
    ...
    EMPTY TEMP-TABLE tt_file_import NO-ERROR.
    INPUT FROM VALUE(v_arq_import_esapb033) NO-CONVERT.
    ...
    REPEAT:
        ...
        IF  TRIM(ENTRY(1, v_des_reg_import, ";"))  <> "" THEN DO:
            ...
            ASSIGN tt_file_import.tta_num_line             = v_num_line
                   ...
                   tt_file_import.tta_cdn_fornecedor       = INT(TRIM(ENTRY(3, v_des_reg_import, ";")))
                   tt_file_import.tta_cod_cta_ctbl         = TRIM(ENTRY(4, v_des_reg_import, ";"))
                   tt_file_import.tta_unid_negoc           = TRIM(ENTRY(5, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_ccusto           = TRIM(ENTRY(6, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_tip_fluxo_financ = TRIM(ENTRY(7, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_ser_docto        = TRIM(ENTRY(8, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_tit_ap           = TRIM(ENTRY(9, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_parcela          = TRIM(ENTRY(10, v_des_reg_import, ";"))
                   tt_file_import.tta_cod_espec_docto      = TRIM(ENTRY(11, v_des_reg_import, ";"))
               /*  tt_file_import.tta_val_tit_ap           = DEC(ENTRY(12, v_des_reg_import, ";"))  vai somar depois pelo valor das apropriaá‰es */
                   tt_file_import.tta_dat_vencto_tit_ap    = DATE(ENTRY(13, v_des_reg_import, ";"))
                   tt_file_import.tta_historico            = TRIM(ENTRY(14, v_des_reg_import, ";")).

            IF  rs_tipo_exec_esapb033 = 3 THEN
                ASSIGN tt_file_import.tta_cod_indic_econ   = TRIM(ENTRY(15, v_des_reg_import, ";")).

            ASSIGN v_cod_estab            = TRIM(ENTRY(1, v_des_reg_import, ";"))          
                   v_cdn_fornecedor       = INT(TRIM(ENTRY(3, v_des_reg_import, ";")))     
                   v_cod_ser_docto        = TRIM(ENTRY(8, v_des_reg_import, ";"))     
                   v_cod_tit_ap           = TRIM(ENTRY(9, v_des_reg_import, ";"))     
                   v_cod_parcela          = TRIM(ENTRY(10, v_des_reg_import, ";"))    
                   v_cod_espec_docto      = TRIM(ENTRY(11, v_des_reg_import, ";"))  
                   v_dat_vencto_tit_ap    = DATE(ENTRY(13, v_des_reg_import, ";"))
                   v_historico            = TRIM(ENTRY(14, v_des_reg_import, ";")).

            CREATE tt_aprop_file_import.
            ASSIGN tt_aprop_file_import.tta_cod_estab            = TRIM(ENTRY(1, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cdn_fornecedor       = INT(TRIM(ENTRY(3, v_des_reg_import, ";")))
                   tt_aprop_file_import.tta_cod_cta_ctbl         = TRIM(ENTRY(4, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_unid_negoc           = TRIM(ENTRY(5, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_ccusto           = TRIM(ENTRY(6, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_tip_fluxo_financ = TRIM(ENTRY(7, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_ser_docto        = TRIM(ENTRY(8, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_tit_ap           = TRIM(ENTRY(9, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_parcela          = TRIM(ENTRY(10, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_cod_espec_docto      = TRIM(ENTRY(11, v_des_reg_import, ";"))
                   tt_aprop_file_import.tta_val_aprop_tit_ap     = DEC(ENTRY(12, v_des_reg_import, ";")).
        ...
        ELSE DO:
            ...
            /* Rateios */
            IF  v_num_line > 1
            AND TRIM(ENTRY(1, v_des_reg_import, ";"))  = ""
            AND TRIM(ENTRY(6, v_des_reg_import, ";")) <> "" THEN DO:
                CREATE tt_aprop_file_import.
                ASSIGN tt_aprop_file_import.tta_cod_estab            = v_cod_estab     
                       tt_aprop_file_import.tta_cdn_fornecedor       = v_cdn_fornecedor
                       tt_aprop_file_import.tta_cod_cta_ctbl         = TRIM(ENTRY(4, v_des_reg_import, ";"))
                       tt_aprop_file_import.tta_unid_negoc           = TRIM(ENTRY(5, v_des_reg_import, ";"))
                       tt_aprop_file_import.tta_cod_ccusto           = TRIM(ENTRY(6, v_des_reg_import, ";"))
                       tt_aprop_file_import.tta_cod_tip_fluxo_financ = TRIM(ENTRY(7, v_des_reg_import, ";"))
                       tt_aprop_file_import.tta_cod_ser_docto        = v_cod_ser_docto  
                       tt_aprop_file_import.tta_cod_tit_ap           = v_cod_tit_ap     
                       tt_aprop_file_import.tta_cod_parcela          = v_cod_parcela    
                       tt_aprop_file_import.tta_cod_espec_docto      = v_cod_espec_docto
                       tt_aprop_file_import.tta_val_aprop_tit_ap     = DEC(ENTRY(12, v_des_reg_import, ";")).
            END.
        END.
    END.
    ...
    FOR EACH tt_file_import:
        ...
        FOR EACH tt_aprop_file_import
            WHERE tt_aprop_file_import.tta_cod_estab       = tt_file_import.tta_cod_estab       
            AND   tt_aprop_file_import.tta_cod_espec_docto = tt_file_import.tta_cod_espec_docto 
            AND   tt_aprop_file_import.tta_cod_ser_docto   = tt_file_import.tta_cod_ser_docto   
            AND   tt_aprop_file_import.tta_cdn_fornecedor  = tt_file_import.tta_cdn_fornecedor  
            AND   tt_aprop_file_import.tta_cod_tit_ap      = tt_file_import.tta_cod_tit_ap      
            AND   tt_aprop_file_import.tta_cod_parcela     = tt_file_import.tta_cod_parcela:
            ASSIGN tt_file_import.tta_val_tit_ap = tt_file_import.tta_val_tit_ap + tt_aprop_file_import.tta_val_aprop_tit_ap.
        END.
    ...
    if  v_log_integr then do:
        ...
        /* Abater antecipaá‰es apenas quando for Fundo Fixo */
        IF  rs_tipo_exec_esapb033 = 1 THEN DO:
            
            antecip:
            FOR EACH tt_file_import BREAK BY tt_file_import.tta_cod_estab:
    
                ASSIGN v_val_saldo_abat = 0.
    
                FIND FIRST estabelecimento
                    WHERE estabelecimento.cod_estab = tt_file_import.tta_cod_estab NO-LOCK NO-ERROR.
    
                IF  AVAIL estabelecimento THEN DO:    
                    ASSIGN v_val_saldo_abat = tt_file_import.tta_val_tit_ap.
    
                    FIND FIRST fornecedor NO-LOCK
                         WHERE fornecedor.cod_empresa    = estabelecimento.cod_empresa
                         AND   fornecedor.cdn_fornecedor = tt_file_import.tta_cdn_fornecedor NO-ERROR.
    
                    if  not can-find (first b_tit_ap
                                      where b_tit_ap.cod_estab           =  "101" /* Busca antecipaá‰es do estabelecimento 101 conforme chamado 112510 */
                                      and   b_tit_ap.ind_tip_espec_docto = "Antecipaá∆o"
                                      and   b_tit_ap.num_pessoa          =  fornecedor.num_pessoa
                                      and   b_tit_ap.dat_transacao       < (v_dat_transacao_esapb033 + 1)) THEN NEXT. 
                    ...
                    for each b_tit_ap no-lock
                        ...
                        and   b_tit_ap.ind_tip_espec_docto = "Antecipaá∆o"
                        and   b_tit_ap.num_pessoa          =  fornecedor.num_pessoa
                        and   b_tit_ap.dat_transacao       < (v_dat_transacao_esapb033 + 1)
                        use-index titap_tip_espec_pessoa_trans:
            
                        ASSIGN v_val_tt_antecip = 0.
    
                        FOR EACH tt_antecip
                            WHERE tt_antecip.cod_estab_an       = b_tit_ap.cod_estab        
                            AND   tt_antecip.cod_espec_docto_an = b_tit_ap.cod_espec_docto  
                            AND   tt_antecip.cod_ser_docto_an   = b_tit_ap.cod_ser_docto    
                            AND   tt_antecip.cdn_fornecedor_an  = b_tit_ap.cdn_fornecedor   
                            AND   tt_antecip.cod_tit_ap_an      = b_tit_ap.cod_tit_ap       
                            AND   tt_antecip.cod_parcela_an     = b_tit_ap.cod_parcela NO-LOCK:
                            ASSIGN v_val_tt_antecip = v_val_tt_antecip + tt_antecip.val_abat.
                        END.
    
                        FIND FIRST tt_antecip
                            WHERE tt_antecip.cod_estab_an       = b_tit_ap.cod_estab        
                            AND   tt_antecip.cod_espec_docto_an = b_tit_ap.cod_espec_docto  
                            AND   tt_antecip.cod_ser_docto_an   = b_tit_ap.cod_ser_docto    
                            AND   tt_antecip.cdn_fornecedor_an  = b_tit_ap.cdn_fornecedor   
                            AND   tt_antecip.cod_tit_ap_an      = b_tit_ap.cod_tit_ap       
                            AND   tt_antecip.cod_parcela_an     = b_tit_ap.cod_parcela NO-LOCK NO-ERROR.
    
                        IF  AVAIL tt_antecip 
                        AND tt_antecip.val_saldo <= v_val_tt_antecip THEN
                            next gera_abat.
    
                        if  b_tit_ap.log_tit_ap_estordo
                        or  not b_tit_ap.log_sdo_tit_ap then
                            next gera_abat.
            
                        assign v_val_sdo_tit_ap = b_tit_ap.val_sdo_tit_ap.
            
                        for each abat_antecip_vouch no-lock
                            where abat_antecip_vouch.cod_estab       = b_tit_ap.cod_estab
                            ...
                            and   abat_antecip_vouch.cdn_fornecedor  = b_tit_ap.cdn_fornecedor
                        ...
                        IF  v_val_sdo_tit_ap >= v_val_saldo_abat THEN DO:
                            CREATE tt_antecip.
                            ASSIGN tt_antecip.cod_estab_dp       = tt_file_import.tta_cod_estab       
                                   tt_antecip.cod_espec_docto_dp = tt_file_import.tta_cod_espec_docto 
                                   tt_antecip.cod_ser_docto_dp   = tt_file_import.tta_cod_ser_docto   
                                   tt_antecip.cdn_fornecedor_dp  = tt_file_import.tta_cdn_fornecedor  
                                   tt_antecip.cod_tit_ap_dp      = tt_file_import.tta_cod_tit_ap      
                                   tt_antecip.cod_parcela_dp     = tt_file_import.tta_cod_parcela     
                                   tt_antecip.cod_estab_an       = b_tit_ap.cod_estab      
                                   tt_antecip.cod_espec_docto_an = b_tit_ap.cod_espec_docto
                                   tt_antecip.cod_ser_docto_an   = b_tit_ap.cod_ser_docto  
                                   tt_antecip.cdn_fornecedor_an  = b_tit_ap.cdn_fornecedor 
                                   tt_antecip.cod_tit_ap_an      = b_tit_ap.cod_tit_ap     
                                   tt_antecip.cod_parcela_an     = b_tit_ap.cod_parcela
                                   tt_antecip.val_abat           = v_val_saldo_abat
                                   tt_antecip.val_saldo          = v_val_sdo_tit_ap.
    
                            NEXT antecip.
                        END.
                        ELSE DO:
                            ASSIGN v_val_abat = v_val_sdo_tit_ap.
                            ...
                            ASSIGN tt_antecip.cod_estab_dp       = tt_file_import.tta_cod_estab
                                   ...
                                   tt_antecip.cod_tit_ap_an      = b_tit_ap.cod_tit_ap     
                                   tt_antecip.cod_parcela_an     = b_tit_ap.cod_parcela
                                   tt_antecip.val_abat           = v_val_abat
                                   tt_antecip.val_saldo          = v_val_sdo_tit_ap.
                        END.         
    
                        ASSIGN v_val_saldo_abat = v_val_saldo_abat - v_val_abat.
                    END.    
                END.
            END.
        END.
        ...
        FOR EACH tt_file_import BREAK BY tt_file_import.tta_cod_estab:
            ...
            IF  FIRST-OF(tt_file_import.tta_cod_estab) THEN DO:
                ...
                REPEAT WHILE NOT v_log_refer_uni:
                    RUN pi_retorna_sugestao_referencia (INPUT "APB":U, INPUT  v_dat_transacao_esapb033, OUTPUT v_cod_refer).
                ...
                ASSIGN tt_integr_apb_lote_impl.tta_cod_estab         = tt_file_import.tta_cod_estab
                       ...
                       tt_integr_apb_lote_impl.tta_dat_transacao     = v_dat_transacao_esapb033
            ...
            IF  rs_tipo_exec_esapb033 = 3
            AND tt_file_import.tta_cod_indic_econ <> "Real" THEN DO:
                FIND emsuni.cotac_parid no-lock 
                     WHERE emsuni.cotac_parid.cod_indic_econ_base  = tt_file_import.tta_cod_indic_econ
                     AND   emsuni.cotac_parid.cod_indic_econ_idx   = "Real"
                     AND   emsuni.cotac_parid.dat_cotac_indic_econ = v_dat_transacao_esapb033
                     AND   emsuni.cotac_parid.ind_tip_cotac_parid  = "Real" NO-ERROR.
                
                IF  AVAIL emsuni.cotac_parid THEN
                    ASSIGN v_val_cotac = emsuni.cotac_parid.val_cotac_indic_econ.
                ELSE DO:
                    RUN pi_print_editor ("s_1", "Cotaá∆o n∆o encontrada. Documento: " + 
            ...
            ASSIGN tt_integr_apb_item_lote_impl_3.ttv_rec_integr_apb_lote_impl = RECID(tt_integr_apb_lote_impl)
                   ...
                   tt_integr_apb_item_lote_impl_3.tta_dat_emis_docto           = v_dat_transacao_esapb033
                   ...
                   tt_integr_apb_item_lote_impl_3.tta_cod_indic_econ           = IF rs_tipo_exec_esapb033 = 3 THEN tt_file_import.tta_cod_indic_econ ELSE "Real":U
                   tt_integr_apb_item_lote_impl_3.tta_val_tit_ap               = tt_file_import.tta_val_tit_ap
                   tt_integr_apb_item_lote_impl_3.tta_cod_portador             = IF rs_tipo_exec_esapb033 = 3 THEN "9991":U ELSE "341":U
                   tt_integr_apb_item_lote_impl_3.tta_val_cotac_indic_econ     = v_val_cotac
                   tt_integr_apb_item_lote_impl_3.tta_des_text_histor          = /* IF  rs_tipo_exec_esapb033 = 1 THEN "Fundo Fixo importado - Programa: ESAPB033 - Arquivo: " + v_arq_import_esapb033 + CHR(13) + tt_file_import.tta_historico ELSE 
                                                                                                                    "Verbas importadas - Programa: ESAPB033 - Arquivo: "    + v_arq_import_esapb033 + CHR(13) + */ tt_file_import.tta_historico.
            ...
            FOR EACH tt_aprop_file_import
                ...
                ASSIGN tt_integr_apb_aprop_ctbl_pend.ttv_rec_integr_apb_item_lote  = RECID(tt_integr_apb_item_lote_impl_3)
                       ...
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_unid_negoc            = IF tt_aprop_file_import.tta_unid_negoc = ""           THEN tt_file_import.tta_unid_negoc           ELSE tt_aprop_file_import.tta_unid_negoc
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_tip_fluxo_financ      = IF tt_aprop_file_import.tta_cod_tip_fluxo_financ = "" THEN tt_file_import.tta_cod_tip_fluxo_financ ELSE tt_aprop_file_import.tta_cod_tip_fluxo_financ
                       tt_integr_apb_aprop_ctbl_pend.tta_val_aprop_ctbl            = tt_aprop_file_import.tta_val_aprop_tit_ap
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_plano_cta_ctbl        = "PADRAO"
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_cta_ctbl              = IF tt_aprop_file_import.tta_cod_cta_ctbl = ""         THEN tt_file_import.tta_cod_cta_ctbl         ELSE tt_aprop_file_import.tta_cod_cta_ctbl
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_plano_ccusto          = "PADRAO"
                       tt_integr_apb_aprop_ctbl_pend.tta_cod_ccusto                = IF tt_aprop_file_import.tta_cod_ccusto = ""           THEN tt_file_import.tta_cod_ccusto           ELSE tt_aprop_file_import.tta_cod_ccusto.
            END.

            IF  rs_tipo_exec_esapb033 = 1 THEN DO:
                /* Gerar abat antecip */              
                FOR EACH tt_antecip
                    WHERE tt_antecip.cod_estab_dp       = tt_file_import.tta_cod_estab       
                    AND   tt_antecip.cod_espec_docto_dp = tt_file_import.tta_cod_espec_docto 
                    and   tt_antecip.cod_ser_docto_dp   = tt_file_import.tta_cod_ser_docto   
                    and   tt_antecip.cdn_fornecedor_dp  = tt_file_import.tta_cdn_fornecedor  
                    and   tt_antecip.cod_tit_ap_dp      = tt_file_import.tta_cod_tit_ap      
                    and   tt_antecip.cod_parcela_dp     = tt_file_import.tta_cod_parcela:
                    ...
                    ASSIGN tt_integr_apb_abat_antecip_vouc.ttv_rec_integr_apb_item_lote = RECID(tt_integr_apb_item_lote_impl_3)
                           tt_integr_apb_abat_antecip_vouc.tta_cod_estab                = tt_antecip.cod_estab_an      
                           tt_integr_apb_abat_antecip_vouc.tta_cod_espec_docto          = tt_antecip.cod_espec_docto_an
                           tt_integr_apb_abat_antecip_vouc.tta_cod_ser_docto            = tt_antecip.cod_ser_docto_an  
                           tt_integr_apb_abat_antecip_vouc.tta_cdn_fornecedor           = tt_antecip.cdn_fornecedor_an 
                           tt_integr_apb_abat_antecip_vouc.tta_cod_tit_ap               = tt_antecip.cod_tit_ap_an     
                           tt_integr_apb_abat_antecip_vouc.tta_cod_parcela              = tt_antecip.cod_parcela_an    
                           tt_integr_apb_abat_antecip_vouc.tta_val_abat_tit_ap          = tt_antecip.val_abat.
            ...
            DO TRANSACTION:
                ...
                IF  CAN-FIND (FIRST tt_log_erros_atualiz) THEN DO:
                    FOR EACH tt_log_erros_atualiz:
                        ...
                        IF  AVAIL tt_integr_apb_item_lote_impl_3 THEN DO:
...
PROCEDURE pi-zera-tabelas:
    ...
    EMPTY TEMP-TABLE tt_integr_apb_item_lote_impl_3  NO-ERROR.
    EMPTY TEMP-TABLE tt_file_import                  NO-ERROR.
    EMPTY TEMP-TABLE tt_antecip                      NO-ERROR.
    EMPTY TEMP-TABLE tt_aprop_file_import            NO-ERROR.
END PROCEDURE.
