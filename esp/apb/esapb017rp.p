/*****************************************************************************
** Descricao.............: Importaá∆o de Seguros - MAPFRE
** Versao................:  5.00.00.000
** Nome Externo..........: esp/apb/esapb017rp.p
** Criado por............: Fabiano Zarpe Henke
** Criado em.............: 15/06/2009
*****************************************************************************/
...
DEF TEMP-TABLE tt_tit_ap NO-UNDO
    ...
    FIELD tta_num_id_tit_ap AS INT
    FIELD tta_val_tit_ap    AS DEC
    INDEX tt_tit_ap_id      IS PRIMARY 
...
def temp-table tt_integr_apb_item_lote_fatura no-undo
    ...
    field tta_cdn_fornecedor               as Integer format ">>>,>>>,>>9" initial 0 label "Fornecedor" column-label "Fornecedor"
    field tta_cod_ser_docto                as character format "x(3)" label "SÈrie Documento" column-label "SÈrie"
    field tta_cod_tit_ap                   as character format "x(10)" label "TÌtulo" column-label "TÌtulo"
    ...
    field tta_cod_indic_econ               as character format "x(8)" label "Moeda" column-label "Moeda"
    field tta_val_tit_ap                   as decimal format "->>>,>>>,>>9.99" decimals 2 initial 0 label "Valor TÌtulo" column-label "Valor TÌtulo"
    ...
    field tta_cod_apol_seguro              as character format "x(12)" label "ApÛlice Seguro" column-label "Apolice Seguro"
    ...
    field tta_des_text_histor              as character format "x(2000)" label "HistÛrico" column-label "HistÛrico"
    ...
    field ttv_ind_vencto_previs            as character format "X(4)" initial "MÍs" label "C·lculo Vencimento" column-label "C·lculo Vencimento"
...
def temp-table tt_integr_apb_impto_impl_pend1 no-undo
    ...
    field tta_cod_espec_docto              as character format "x(3)" label "EspÈcie Documento" column-label "EspÈcie"
    field tta_cod_ser_docto                as character format "x(3)" label "SÈrie Documento" column-label "SÈrie"
    field tta_cod_tit_ap                   as character format "x(10)" label "TÌtulo" column-label "TÌtulo"
    field tta_cod_parcela                  as character format "x(02)" label "Parcela" column-label "Parc"
    ...
    field tta_val_outras_deduc_impto       as decimal format ">,>>>,>>>,>>9.99" decimals 2 initial 0 label "Outras DeduÁıes" column-label "Outras DeduÁıes"
    field tta_val_base_liq_impto           as decimal format ">,>>>,>>>,>>9.99" decimals 2 initial 0 label "Base LÌquida Imposto" column-label "Base LÌquida Imposto"
    ...
    field tta_num_id_movto_tit_ap          as integer format "9999999999" initial 0 label "Token Movto Tit AP" column-label "Id Tit AP"
    field tta_num_id_movto_cta_corren      as integer format "9999999999" initial 0 label "ID Movto Conta" column-label "ID Movto Conta"
    field tta_cod_pais_ext                 as character format "x(20)" label "PaÌs Externo" column-label "PaÌs Externo"
...
def temp-table tt_log_erros_atualiz_fat no-undo
    ...
    field ttv_des_msg_erro                 as character format "x(60)" label "Mensagem Erro" column-label "InconsistÍncia"
...
def new global shared var v_val_outros_imp
    ...
    no-undo.
def new global shared var v_cdn_fornec_imp
    as INTEGER
    FORMAT ">>>>>>,>>9"
    VIEW-AS FILL-IN
    size 11.14 by 1
...
def new shared var v_rpt_s_1_name as character initial "Logs Importaá∆o MAPFRE".
...
def frame f_rnl_31_histor_fornec_importar_ems
    ...
    with 1 down side-labels no-validate keep-tab-order three-d
         ...
         view-as dialog-box
         font 1 fgcolor ? bgcolor 8
         title "Executa Importaá∆o MAPFRE".
    /* adjust size of objects in this frame */
...
ASSIGN v_des_contdo_prog_valid_dtsul = 'esapb017rp'.
...
ASSIGN ed_1x40 = c-dir-saida + 'spool~\esapb017.txt'.
...
do on endkey undo main_block, leave main_block on error undo main_block, leave main_block:
    ...
    if  rs_ind_message_output:screen-value = "Em Arquivo" /*l_on_file*/ 
    then do:
        assign v_nom_prog_ext  = caps("ESAPB017":U)
...
PROCEDURE pi_rnl_histor_fornec_importar_ems:
    ...
    RUN pi_wait_processing (INPUT "Importando arquivo MAPFRE...",
                            INPUT "Importaá∆o MAPFRE").

    ASSIGN v_log_method = SESSION:SET-WAIT-STATE('general') 
           v_num_line = 0.

    /* ** Zarpe - Valores Fixos ***/
    ASSIGN v_cod_espec  = 'di'
           v_cod_ser    = '6'.
    ...
    REPEAT TRANSACTION:
        ...
        /* ** Importa soemnte os registros dos embarques e totais  ***
        96476;;;;;;Cond./Franquia:;1,00;;;;
        ;;;;Total:;20.368,47;;;;;;12,22
        TOTAIS:;Embarques:;308;;;22.376.920,05;;;;;;13.426,18      ***/

        IF NUM-ENTRIES(v_des_reg_import, ";") < 7
           THEN NEXT.

        IF TRIM(ENTRY(7, v_des_reg_import, ";")) = "Cond./Franquia:"
           THEN ASSIGN v_cod_tip_reg = "1".
        IF TRIM(ENTRY(5, v_des_reg_import, ";")) = "Total:"
           THEN ASSIGN v_cod_tip_reg = "2".
        IF TRIM(ENTRY(1, v_des_reg_import, ";")) = "TOTAIS:"
           THEN ASSIGN v_cod_tip_reg = "3".
        ...
        IF v_cod_tip_reg = "1" 
        THEN DO:
             /*pode haver mais de um embarque na mesma linha
             ENTRY(1, v_des_reg_import, ";")*/
             ASSIGN v_cod_embarque     = ENTRY(1, v_des_reg_import, ";")
                    v_cod_embarque_aux = ''
                    v_qtd_bloco        = v_qtd_bloco + 1.

             IF NUM-ENTRIES(v_cod_embarque, "-") > 1 
             THEN DO:

                  DO v_num_count = 1 TO NUM-ENTRIES(v_cod_embarque, "-"):

                     IF v_cod_embarque_aux = ""
                        THEN ASSIGN v_cod_embarque_aux = ENTRY(1, ENTRY(v_num_count, v_cod_embarque, "-"), " ").
                        ELSE ASSIGN v_cod_embarque_aux = v_cod_embarque_aux + "-" + (ENTRY(1, TRIM(ENTRY(v_num_count, v_cod_embarque, "-")), " ")).

                  END.

                  ASSIGN v_cod_embarque = v_cod_embarque_aux.

             END.
             ELSE ASSIGN v_cod_embarque = ENTRY(1, ENTRY(1, v_des_reg_import, ";"), " ").
             ...
             DO v_num_count = 1 TO NUM-ENTRIES(v_cod_embarque, "-"):
                 ...
                 FOR EACH estabelecimento NO-LOCK
                     ...
                     FIND tit_ap NO-LOCK
                         WHERE tit_ap.cod_estab          = estabelecimento.cod_estab
                           AND tit_ap.cdn_fornec         = v_cdn_fornec_imp
                           AND tit_ap.cod_espec          = v_cod_espec
                           AND tit_ap.cod_tit_ap         = ENTRY(v_num_count, v_cod_embarque, "-")
                 ...
                 IF NOT AVAIL tit_ap 
                 THEN DO:
                      ASSIGN v_des_mensagem = SUBSTITUTE("T°tulo &1 n∆o localizado !", ENTRY(v_num_count, v_cod_embarque, "-")).
                 ...
                 ASSIGN v_log_avail_tit = YES.
    
                 FIND tt_tit_ap NO-LOCK
                     WHERE tt_tit_ap.tta_cod_estab     = tit_ap.cod_estab    
                       AND tt_tit_ap.tta_num_id_tit_ap = tit_ap.num_id_tit_ap NO-ERROR.
                 IF NOT AVAIL tt_tit_ap 
                 THEN DO:
                      CREATE tt_relac_erro.
                      ASSIGN tt_relac_erro.tta_cod_refer = ""
                             tt_relac_erro.ttv_num_linha = v_num_line.

                      CREATE tt_tit_ap.
                      ASSIGN tt_tit_ap.tta_cod_estab     = tit_ap.cod_estab
                             tt_tit_ap.tta_num_id_tit_ap = tit_ap.num_id_tit_ap.
        ...
        /* ** Valor Embarque ***/
        IF  v_cod_tip_reg   = "2"  THEN

        IF  v_cod_tip_reg   = "2" 
        AND v_log_avail_tit = YES
        THEN DO:
             ASSIGN tt_tit_ap.tta_val_tit_ap = tt_tit_ap.tta_val_tit_ap + DEC(ENTRY(12, v_des_reg_import, ";"))
                    v_tot_bloco              = v_tot_bloco + tt_tit_ap.tta_val_tit_ap.
        END.

        /* ** Totalizadores ***/
        IF v_cod_tip_reg    = "3" 
        THEN DO:
             ASSIGN v_qtd_emb = INT(ENTRY(3, v_des_reg_import, ";"))
                    v_tot_emb = DEC(ENTRY(12, v_des_reg_import, ";")).
        END.

    END.
        
    /* ** Valida Totais ***/
    IF v_qtd_emb <> v_qtd_bloco
    THEN DO:
         ASSIGN v_des_mensagem = SUBSTITUTE("Total de Embarques (&1) difere da quantidade de Blocos (&2) !", v_qtd_emb, v_qtd_bloco).
         RUN pi_print_editor ("s_1", v_des_mensagem, "     050", "", "     ", "", "     ").
         PUT STREAM s_1 UNFORMATTED  
             v_num_line TO 27 FORMAT  ">>,>>9"
             ENTRY(1, RETURN-VALUE, CHR(255)) AT 31 FORMAT "x(50)" SKIP.
         ...
         END.
    END.
    IF v_tot_emb <> v_tot_bloco
    THEN DO:
         ASSIGN v_des_mensagem = SUBSTITUTE("Valor Total dos Embarques (&1) difere do Valor Total dos Blocos (&2) !", TRIM(STRING(v_tot_emb, ">>>,>>>,>>9.99")), TRIM(STRING(v_tot_bloco, ">>>,>>>,>>9.99"))).
         RUN pi_print_editor ("s_1", v_des_mensagem, "     050", "", "     ", "", "     ").
         PUT STREAM s_1 UNFORMATTED  
             v_num_line TO 27 FORMAT  ">>,>>9"
             ENTRY(1, RETURN-VALUE, CHR(255)) AT 31 FORMAT "x(50)" SKIP.
         ...
         END.
    END.
    IF v_tot_emb <> v_val_bordero_imp
    THEN DO:
         ASSIGN v_des_mensagem = SUBSTITUTE("Valor Total dos Embarques (&1) difere do Valor do Borderì (&2) !", TRIM(STRING(v_tot_emb, ">>>,>>>,>>9.99")), TRIM(STRING(v_val_bordero_imp, ">>>,>>>,>>9.99"))).
         RUN pi_print_editor ("s_1", v_des_mensagem, "     050", "", "     ", "", "     ").
         PUT STREAM s_1 UNFORMATTED  
             v_num_line TO 27 FORMAT  ">>,>>9"
             ENTRY(1, RETURN-VALUE, CHR(255)) AT 31 FORMAT "x(50)" SKIP.
    ...
    THEN DO:
        ...
        DO TRANSACTION:
            ...
            ASSIGN tt_integr_apb_lote_fatura.tta_cod_estab                = v_cod_estab_imp
                   ...
                   tt_integr_apb_lote_fatura.tta_cod_espec_docto          = 'DI'
                   tt_integr_apb_lote_fatura.tta_dat_transacao            = v_dat_fatura_imp
                   tt_integr_apb_lote_fatura.tta_ind_origin_tit_ap        = 'APB'
                   tt_integr_apb_lote_fatura.tta_cod_empresa              = v_cod_empres_usuar
                   tt_integr_apb_lote_fatura.tta_cod_indic_econ           = 'Dolar'
                   tt_integr_apb_lote_fatura.ttv_log_atualiza_refer_apb   = YES
                   tt_integr_apb_lote_fatura.ttv_log_elimina_lote         = NO 
                   tt_integr_apb_lote_fatura.tta_cdn_fornecedor           = v_cdn_fornec_imp
                   tt_integr_apb_lote_fatura.tta_num_fatur_ap             = v_num_fatura_imp
                   tt_integr_apb_lote_fatura.tta_qtd_parcela              = 1
                   tt_integr_apb_lote_fatura.tta_cod_histor_padr          = 'Fatura Importada - esapb017'
                   tt_integr_apb_lote_fatura.tta_cod_histor_padr_dupl     = 'Fatura Importada - esapb017'
            ...
            FOR EACH tt_tit_ap:
                ...
                FIND tit_ap NO-LOCK
                   WHERE tit_ap.cod_estab     = tt_tit_ap.tta_cod_estab
                     AND tit_ap.num_id_tit_ap = tt_tit_ap.tta_num_id_tit_ap NO-ERROR.
                ...
                ASSIGN tt_integr_apb_relacto_fatura.ttv_rec_integr_apb_lote_impl = tt_integr_apb_lote_fatura.ttv_rec_integr_apb_lote_impl
                       tt_integr_apb_relacto_fatura.tta_cod_estab_tit_ap_pai     = tit_ap.cod_estab
                       tt_integr_apb_relacto_fatura.tta_cdn_fornec_pai           = tit_ap.cdn_fornec
                       tt_integr_apb_relacto_fatura.tta_cod_espec_docto_nf       = tit_ap.cod_espec
                       tt_integr_apb_relacto_fatura.tta_cod_ser_docto_nf         = tit_ap.cod_ser
                ...
                ASSIGN tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap = tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap + tit_ap.val_sdo_tit_ap
                       v_cod_portador                                         = tit_ap.cod_portador.
            ...
            ASSIGN tt_integr_apb_item_lote_fatura.ttv_rec_integr_apb_lote_impl  = tt_integr_apb_lote_fatura.ttv_rec_integr_apb_lote_impl  
                   ...
                   tt_integr_apb_item_lote_fatura.tta_cdn_fornecedor            = v_cdn_fornec_imp
                   tt_integr_apb_item_lote_fatura.tta_cod_ser_docto             = "U"
                   tt_integr_apb_item_lote_fatura.tta_cod_tit_ap                = STRING(v_num_fatura_imp)
                   tt_integr_apb_item_lote_fatura.tta_cod_parcela               = '1' 
                   tt_integr_apb_item_lote_fatura.tta_dat_emis_docto            = v_dat_fatura_imp
                   tt_integr_apb_item_lote_fatura.tta_dat_vencto_tit_ap         = v_dat_bordero_imp
                   tt_integr_apb_item_lote_fatura.tta_dat_prev_pagto            = v_dat_bordero_imp
                   tt_integr_apb_item_lote_fatura.tta_cod_indic_econ            = 'Dolar'
                   tt_integr_apb_item_lote_fatura.tta_val_tit_ap                = tt_integr_apb_lote_fatura.tta_val_tot_lote_impl_tit_ap
                   tt_integr_apb_item_lote_fatura.tta_cod_portador              = v_cod_portador
                   tt_integr_apb_item_lote_fatura.ttv_rec_integr_apb_item_lote  = RECID(tt_integr_apb_item_lote_fatura)
                   tt_integr_apb_item_lote_fatura.tta_des_text_histor           = 'Fatura Importada - esapb017'
            ...
            FOR EACH tt_log_erros_atualiz_fat:
                FIND tt_relac_erro 
                    WHERE tt_relac_erro.tta_cod_refer = tt_log_erros_atualiz_fat.tta_cod_refer NO-ERROR.
            ...
            FIND tit_ap NO-LOCK
                WHERE tit_ap.cod_estab   = v_cod_estab_imp
                  AND tit_ap.cdn_fornec  = v_cdn_fornec_imp
                  AND tit_ap.cod_espec   = v_cod_espec
                  AND tit_ap.cod_ser     = "U"
                  AND tit_ap.cod_tit_ap  = STRING(v_num_fatura_imp)
                  AND tit_ap.cod_parcela = '1' NO-ERROR.

            CREATE tt_integr_apb_bord_lote_pagto.
            ASSIGN tt_integr_apb_bord_lote_pagto.tta_cod_empresa           = tit_ap.cod_empresa
                   tt_integr_apb_bord_lote_pagto.ttv_cod_estab_bord_refer  = tit_ap.cod_estab
                   tt_integr_apb_bord_lote_pagto.tta_cod_portador          = tit_ap.cod_portador
                   ...
                   tt_integr_apb_bord_lote_pagto.tta_cod_refer_antecip_pef = ""
                   tt_integr_apb_bord_lote_pagto.tta_val_pagto             = (v_val_bordero_imp + v_val_outros_imp) /* ** Valor em Real ***/
                   tt_integr_apb_bord_lote_pagto.tta_cod_forma_pagto       = "40"
                   ...
                   tt_integr_apb_bord_lote_pagto.tta_val_cotac_indic_econ  = (tit_ap.val_sdo_tit_ap / (v_val_bordero_imp + v_val_outros_imp))
                   tt_integr_apb_bord_lote_pagto.tta_cod_indic_econ        = "Dolar".
