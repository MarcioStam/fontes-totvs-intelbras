...
&Scoped-Define ENABLED-OBJECTS repres-ini repres-fim data-ini data-fim ~
gr-fornec-ini gr-fornec-fim tg-resumido rs-baixa tg-envio-email ~
tg-rel-final-mes rs-destino rs-execucao bt-cfimp bt-arquivo c-arquivo ~
bt-imprime bt-salva IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-7 IMAGE-8 RECT-2 ~
RECT-29 RECT-30 RECT-31 RECT-7 RECT-8 
&Scoped-Define DISPLAYED-OBJECTS repres-ini repres-fim data-ini data-fim ~
gr-fornec-ini gr-fornec-fim tg-resumido rs-baixa tg-envio-email ~
tg-rel-final-mes rs-destino rs-execucao c-arquivo 
...
DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "esapb004.lst" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE data-ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/1900 
     LABEL "Data Transaá∆o" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE gr-fornec-fim AS CHARACTER FORMAT "XXXX":U INITIAL "ZZZ" 
     VIEW-AS FILL-IN 
     SIZE 6.86 BY .88 NO-UNDO.

DEFINE VARIABLE gr-fornec-ini AS CHARACTER FORMAT "XXXX":U 
     LABEL "Gr. Fornecedor" 
     VIEW-AS FILL-IN 
     SIZE 6.86 BY .88 NO-UNDO.

DEFINE VARIABLE repres-fim AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 6.86 BY .88 NO-UNDO.

DEFINE VARIABLE repres-ini AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     LABEL "Representante" 
     VIEW-AS FILL-IN 
     SIZE 6.86 BY .88 NO-UNDO.
...
DEFINE VARIABLE rs-baixa AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "A Baixar(CPO)", 1,
"Liquidadas(CPO)", 2,
"A Vencer(CPE)", 3
     SIZE 14.43 BY 2.25 NO-UNDO.
...
DEFINE VARIABLE tg-envio-email AS LOGICAL INITIAL no 
     LABEL "Envio de e-mail" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .83 NO-UNDO.

DEFINE VARIABLE tg-rel-final-mes AS LOGICAL INITIAL no 
     LABEL "Relat¢rio de final de màs" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-relat
     repres-ini AT ROW 1.5 COL 17.43 COLON-ALIGNED
     repres-fim AT ROW 1.5 COL 32.29 COLON-ALIGNED NO-LABEL
     data-ini AT ROW 2.54 COL 14.43 COLON-ALIGNED
     data-fim AT ROW 2.54 COL 32.43 COLON-ALIGNED NO-LABEL
     gr-fornec-ini AT ROW 3.5 COL 17.57 COLON-ALIGNED
     gr-fornec-fim AT ROW 3.5 COL 32.57 COLON-ALIGNED NO-LABEL
     tg-resumido AT ROW 6.13 COL 30
     rs-baixa AT ROW 6.25 COL 5.29 NO-LABEL
     tg-envio-email AT ROW 7.13 COL 30
     tg-rel-final-mes AT ROW 8.13 COL 30
     rs-destino AT ROW 10.96 COL 2.14 HELP
          "Destino da Impress∆o" NO-LABEL
     rs-execucao AT ROW 11.04 COL 47.86 HELP
          "Execuá∆o On Line ou Batch" NO-LABEL
     bt-cfimp AT ROW 12.08 COL 41.14 HELP
          "Layout Impress∆o"
     bt-arquivo AT ROW 12.08 COL 41.14 HELP
          "Localiza Arquivo"
     c-arquivo AT ROW 12.13 COL 2.29 HELP
          "Destino" NO-LABEL
     bt-imprime AT ROW 13.83 COL 2.57
     bt-salva AT ROW 13.83 COL 14.29
     IMAGE-1 AT ROW 1.5 COL 26.43
     IMAGE-2 AT ROW 1.5 COL 31.57
     IMAGE-3 AT ROW 3.5 COL 26.72
     IMAGE-4 AT ROW 3.5 COL 31.86
     IMAGE-7 AT ROW 2.5 COL 26.57
     IMAGE-8 AT ROW 2.54 COL 31.72
     RECT-2 AT ROW 13.54 COL 1.57
     RECT-29 AT ROW 10.38 COL 47.14
     RECT-30 AT ROW 5.54 COL 3
     RECT-31 AT ROW 5.54 COL 26.14
     RECT-7 AT ROW 10.42 COL 1.43
     RECT-8 AT ROW 1 COL 1
     "  Impress∆o" VIEW-AS TEXT
          SIZE 10.57 BY .54 AT ROW 10.33 COL 3.14
          FONT 6
     " Execuá∆o" VIEW-AS TEXT
          SIZE 9.43 BY .54 AT ROW 10.25 COL 47.43
          FONT 6
     " Seleá∆o" VIEW-AS TEXT
          SIZE 8 BY .54 AT ROW 1 COL 3
          FONT 6
     " ParÉmetro" VIEW-AS TEXT
          SIZE 8 BY 1 AT ROW 5 COL 4
     " Tipo" VIEW-AS TEXT
          SIZE 5 BY 1 AT ROW 5 COL 27
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio de Comiss‰es - ESAPB004"
         COLUMN             = 37.43
         ROW                = 6.58
         HEIGHT             = 14.08
         WIDTH              = 57.57
...
ON END-ERROR OF C-Win /* Relat¢rio de Comiss‰es - ESAPB004 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Relat¢rio de Comiss‰es - ESAPB004 */
DO:
...
DO:
  ...
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "esapb004rp",
                               INPUT  "1.00.000",
                               INPUT  0,
...
&Scoped-define SELF-NAME rs-baixa
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rs-baixa C-Win
ON VALUE-CHANGED OF rs-baixa IN FRAME f-relat
DO:
  IF INPUT FRAME {&FRAME-NAME} rs-baixa = 2 OR       /*Liquidadas (CPO)*/
     INPUT FRAME {&FRAME-NAME} rs-baixa = 3 THEN DO: /*A Vencer(CPE)   */
     ASSIGN tg-rel-final-mes = NO.
     DISPLAY tg-rel-final-mes WITH FRAME {&FRAME-NAME}.
     DISABLE tg-rel-final-mes WITH FRAME {&FRAME-NAME}.
  END.
  ELSE 
     ENABLE tg-rel-final-mes WITH FRAME {&FRAME-NAME}.
END.
...
DO:
  ...
  if input frame f-relat rs-destino = 2 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          assign c-arquivo:screen-value in frame f-relat = session:temp-directory + "esapb004.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
      else
          assign c-arquivo:screen-value in frame f-relat = "esapb004.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
  ...
  if input frame f-relat rs-destino = 3 then
      assign bt-arquivo:visible  in frame f-relat = no
             ...
             rs-execucao:SCREEN-VALUE IN FRAME f-relat = '1'.
...
&Scoped-define SELF-NAME tg-envio-email
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-envio-email C-Win
ON VALUE-CHANGED OF tg-envio-email IN FRAME f-relat /* Envio de e-mail */
DO:

  IF INPUT FRAME f-relat tg-envio-email = YES THEN DO:
    ASSIGN rs-destino:SCREEN-VALUE IN FRAME f-relat = '2'.
    apply "value-changed" to rs-destino in frame f-relat.

  END.
  ELSE DO:
     ENABLE rs-destino WITH FRAME f-relat.
     apply "value-changed" to rs-destino in frame f-relat.
  END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  apply "value-changed" to tg-envio-email in FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  DISPLAY repres-ini repres-fim data-ini data-fim gr-fornec-ini gr-fornec-fim 
          tg-resumido rs-baixa tg-envio-email tg-rel-final-mes rs-destino 
          rs-execucao c-arquivo 
      WITH FRAME f-relat IN WINDOW C-Win.
  ENABLE repres-ini repres-fim data-ini data-fim gr-fornec-ini gr-fornec-fim 
         tg-resumido rs-baixa tg-envio-email tg-rel-final-mes rs-destino 
         rs-execucao bt-cfimp bt-arquivo c-arquivo bt-imprime bt-salva IMAGE-1 
         IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-7 IMAGE-8 RECT-2 RECT-29 RECT-30 RECT-31 
         RECT-7 RECT-8 
      WITH FRAME f-relat IN WINDOW C-Win.
...
/* */
  ...
  DO WITH FRAME f-relat:
    ASSIGN rs-execucao:SCREEN-VALUE       IN FRAME f-relat = entry(1,dwb_set_list_param.cod_dwb_parameters,chr(10))
           repres-ini:screen-value        in frame f-relat = entry(2,dwb_set_list_param.cod_dwb_parameters,chr(10))
           repres-fim:screen-value        in frame f-relat = entry(3,dwb_set_list_param.cod_dwb_parameters,chr(10))
           gr-fornec-ini:screen-value     in frame f-relat = entry(4,dwb_set_list_param.cod_dwb_parameters,chr(10))
           gr-fornec-fim:screen-value     in frame f-relat = entry(5,dwb_set_list_param.cod_dwb_parameters,chr(10))
           data-ini:screen-value          in frame f-relat = entry(6,dwb_set_list_param.cod_dwb_parameters,chr(10))
           data-fim:screen-value          in frame f-relat = entry(7,dwb_set_list_param.cod_dwb_parameters,chr(10))
           rs-baixa:screen-value          in frame f-relat = entry(8,dwb_set_list_param.cod_dwb_parameters,chr(10))
           tg-resumido:screen-value       in frame f-relat = entry(9,dwb_set_list_param.cod_dwb_parameters,chr(10))
           tg-envio-email:screen-value    in frame f-relat = entry(10,dwb_set_list_param.cod_dwb_parameters,chr(10))
           tg-rel-final-mes:screen-value  in frame f-relat = entry(11,dwb_set_list_param.cod_dwb_parameters,chr(10))
           rs-destino:SCREEN-VALUE        IN FRAME f-relat = IF dwb_set_list_param.cod_dwb_output = "impressora" 
                                                               ...
                                                               ELSE IF dwb_set_list_param.cod_dwb_output = "arquivo" 
                                                                    ...
                                                                    ELSE "3".
  IF INPUT FRAME {&FRAME-NAME} rs-baixa = 2 OR    /*Liquidadas (CPO)*/
     INPUT FRAME {&FRAME-NAME} rs-baixa = 3 THEN  /*A Vencer(CPE)   */
     DISABLE tg-rel-final-mes WITH FRAME {&FRAME-NAME}.
  ELSE 
     ENABLE tg-rel-final-mes WITH FRAME {&FRAME-NAME}.
...
DEF VAR v_log_msg AS LOGICAL NO-UNDO.

/* */
if input frame f-relat rs-destino = 2 then 
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "esapb004rp"
    ...
    do:
      ...
      do:
        MESSAGE "Nome do Arquivo encontrado em outro pedido,Deseja continuar ? " SKIP         
                "Foi encontrado um pedido com o mesmo nome a ser criado."       SKIP
                "Arquivo....: " ped_exec_param.cod_dwb_file                     SKIP                  
                "Usuario....: " ped_exec.cod_usuar                              SKIP              
                "Num Pedido.: " string(ped_exec_param.num_ped_exec)             SKIP                   
                 VIEW-AS ALERT-BOX WARNING BUTTONS OK-CANCEL UPDATE v_log_msg.

        RETURN STRING(v_log_msg).
      end.  
    end.
  end.    
end.
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "esapb004" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "esapb004"
...
/* */
  ...
  ASSIGN dwb_set_list_param.Cod_dwb_program          = "esapb004rp"
         ...
         dwb_set_list_param.Cod_dwb_parameters       = STRING(rs-execucao) + chr(10) + 
                                                              string(repres-ini) + chr(10) + 
                                                              string(repres-fim) + chr(10) +
                                                              string(gr-fornec-ini) + chr(10) +
                                                              string(gr-fornec-fim) + chr(10) +
                                                              string(data-ini) + chr(10) +
                                                              string(data-fim) + chr(10) +
                                                              string(rs-baixa) + chr(10) +
                                                              string(tg-resumido) + chr(10)+
                                                              STRING(tg-envio-email) + CHR(10) +
                                                              STRING(tg-rel-final-mes) + CHR(10)
