...
*******************************************************************************/
{include/i-prgvrs.i ES0101 2.06.00.000}
/*------------------------------------------------------------------------
    File        : XX9999.W
    Purpose     : <none>
    Syntax      : <none>
...
&GLOBAL-DEFINE page4Fields    tg-emails c-emails i-estoque i-ordem i-notas ed-descricao
...
DEFINE VARIABLE i-estoque AS INTEGER FORMAT ">>>9":U INITIAL 0 
     LABEL "Estoque" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE i-notas AS INTEGER FORMAT ">>>9":U INITIAL 0 
     LABEL "Notas Fiscais" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE i-ordem AS INTEGER FORMAT ">>>9":U INITIAL 0 
     LABEL "Ordem" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.
...
DEFINE FRAME fPage4
     ...
     i-estoque AT ROW 10.46 COL 21.14 COLON-ALIGNED WIDGET-ID 16
     i-ordem AT ROW 11.46 COL 21.14 COLON-ALIGNED WIDGET-ID 18
     i-notas AT ROW 12.46 COL 21.14 COLON-ALIGNED WIDGET-ID 6
     "E-mail" VIEW-AS TEXT
          SIZE 8 BY .54 AT ROW 6.46 COL 13 WIDGET-ID 12
     "Dias de movimentaá‰es" VIEW-AS TEXT
          SIZE 17 BY .54 AT ROW 9.71 COL 7.72 WIDGET-ID 14
     RECT-13 AT ROW 6.71 COL 6 WIDGET-ID 8
     RECT-14 AT ROW 9.96 COL 6 WIDGET-ID 10
...
------------------------------------------------------------------------------*/
    ...
    RUN esp/es0018p.p (INPUT "monitor-estq", /* Nome do programa */
    ...
    FOR EACH tt-prog-ponto:
        CASE tt-prog-ponto.sequencia:
            ...
            WHEN 1 THEN ASSIGN i-estoque = INT(tt-prog-ponto.conteudo).
            WHEN 2 THEN ASSIGN i-ordem   = INT(tt-prog-ponto.conteudo).
            WHEN 3 THEN ASSIGN i-notas   = INT(tt-prog-ponto.conteudo).
        END.
    END.

    ASSIGN ed-descricao = "Monitorando: " + CHR(10) +
                          " - Movimento de estoque duplicado" + CHR(10) +
                          " - Movimento de estoque de ACA sem REQ" + CHR(10) +
                          " - Movimento de estoque em deposito/item bloqueado" + CHR(10) +
                          " - Nota fiscal com situaá∆o diferente de Uso Autorizado e com movimento de estoque" + CHR(10) +
                          " - Nota fiscal Autorizada no TOTVS mas cancelada/inutilizada no SEFAZ" + CHR(10) + 
                          " - Nota fiscal cancelada no TOTVS mas autorizada na SEFAZ ".
    ...
    DO ON ERROR UNDO, RETURN ERROR
        ...
        */
        ...
        assign tt-param.usuario         = c-seg-usuario
               ...
               tt-param.estoque         = INPUT FRAME fPage4 i-estoque
               tt-param.ordem           = INPUT FRAME fPage4 i-ordem
               tt-param.notas           = INPUT FRAME fPage4 i-notas.
