...
*******************************************************************************/
{include/i-prgvrs.i BC9018H 2.00.00.041 } /*** "010041" ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
{include/i-license-manager.i bc9018h MBC}
&ENDIF
...
** autorizacao expressa.
*******************************************************************************/
/* versÆo das bases e bases instaladas */ {include/i_dbinst.i}  
/*******************************************************************************
** Copyright DATASUL S.A. (2003)
** Todos os Direitos Reservados.
**
...
{bcp/bc9018.i " "}
{bcp/bc9018.i1 }
{bcp/bc9018h.i} /*fk - definicao das procedures utilizadas*/
...
/*mk Variaveis utilizadas pela BOSC145 para conversÆo de qtd x emb~\un*/
...
DEFINE VARIABLE cTextoOpcao      AS CHARACTER   FORMAT 'x(20)':U     INIT ''  NO-UNDO.
...
/************************************************************/
                             //ttWork.cod-item                                   At Row 01 Col 04 No-label Format 'x(16)'       ~
/* Definicao da Frame01 ---                                 */
...
&global-define Frame01Defs   'Item:'                                           At Row 01 Col 01                               ~
                             ...
                             ttWork.qtd-item-digit                             At Row 04 Col 23 NO-LABELS FORMAT  '>>>>>9':U                      ~
                             cTextoOpcao                                       AT ROW 06 COL 01 NO-LABELS                     ~
                             ttWork.opcao                                      AT ROW 06 COL 21 NO-LABELS                     ~
...
&global-define Frame02Defs   'Item:'                                           At Row 01 Col 01                               ~
                             ...
                             'Qtd:'                                            At Row 04 Col 19                               ~
                             ttWork.qtd-item-digit                             At Row 04 Col 23 NO-LABELS FORMAT  '>>>>>9':U                      ~
                             cTextoOpcao                                       AT ROW 07 COL 01 NO-LABELS                     ~
                             ttWork.opcao                                      AT ROW 07 COL 21 NO-LABELS                     ~
                             'Lida'                                            At Row 07 Col 01                               ~
                             'Pend'                                            At Row 07 Col 17                               ~
                             ttWork.cod-livre-3                                At Row 08 Col 01 NO-LABEL FORMAT "x(16)"       ~
                             ttWork.cod-livre-4                                At Row 08 Col 17 NO-LABEL FORMAT "x(16)"       ~
...
&global-define Frame03Defs   'Picking WMS Manufat'                             At Row 01 Col 01                         ~
...
&if '{&mgscm_version}' >= '2.07' &then     /* FLAVIO-INI FO 1389.034 */
    ...
    &global-define Frame04Defs   'Picking WMS Manufat'                             At Row 01 Col 01                          ~
    ...
    &global-define Frame05Defs   'Picking WMS Manufat'                             At Row 01 Col 01                          ~
...
&global-define TriggerAfterFrame02  Run GravaCamposFrame02. If return-value = "ESC":U then return return-value. ~
...
&global-define UserTriggers ~
                            ON 'esc' of current-window anywhere do: ~
                                ...
                                return 'esc':u.                     ~
                            END. ~
                            ON 'leave':U  OF ttWork.num-serial In Frame {&Frame02Name}              ~
                            DO:                                                                     ~
                                ASSIGN ttWork.num-serial. ~
                                ...
                                IF AVAIL ttWm-etiqueta THEN DO: ~
                                    ASSIGN  ttWork.cod-lote = ttWm-etiqueta.cod-lote~
                                            ttWork.cod-lote :Screen-value In Frame {&Frame02Name} = ttWm-etiqueta.cod-lote.~
                                END.~
                            END.                                                                    ~
...
&global-define ActiveObject2  wgbosc145
...
Procedure InicializaCamposFrame01:
    ...
    If  vLogEmProcesso = No Then Do:
        ...
        If  Return-value <> 'OK' Then Do:
            Assign vLogErro = Yes.
            {bcp/bc9105.i "302" "Movimento Expirado ou Jÿ Alocado (WMS)"}
            Return Error.
        End.
    ...
    IF iIndTipoVisualiza = 4 THEN DO: /* Emb/Un */
        ...
        ASSIGN ttWork.cod-livre-4:SCREEN-VALUE IN FRAME {&Frame01Name} = STRING(p-qtd-caixas,">>>>>9") + "/" + STRING(p-qtd-unidades,">>>>>9").
        ...
        ASSIGN ttWork.cod-livre-3:SCREEN-VALUE IN FRAME {&Frame01Name} = STRING(p-qtd-caixas,">>>>>9") + "/" + STRING(p-qtd-unidades,">>>>>9").
    ...
    Assign ttWork.cod-item       = ttWm-box-movto-idx-picking.cod-item
           ...
           ttWork.cod-lote       :Screen-value In Frame {&Frame01Name} = ttWm-box-movto-idx-picking.cod-lote
...
Procedure InicializaCamposFrame02:
    ...
    IF iIndTipoVisualiza = 2 THEN DO: /* Quantidade */
        ASSIGN ttWork.cod-livre-4:SCREEN-VALUE IN FRAME {&Frame02Name} = String((ttWm-box-movto-idx-picking.qtd-item * ttWm-box-movto-idx-picking.qti-embalagem) - ttWm-box-movto-idx-picking.qtd-item-picking,">>>>>9") 
               ttWork.cod-livre-3:SCREEN-VALUE IN FRAME {&Frame02Name} = STRING(ttWm-box-movto-idx-picking.qtd-item-picking,">>>>>9").
    END.
    IF iIndTipoVisualiza = 4 THEN DO: /* Emb/Un */
        ...
        ASSIGN ttWork.cod-livre-4:SCREEN-VALUE IN FRAME {&Frame02Name} = STRING(p-qtd-caixas,">>>>>9") + "/" + STRING(p-qtd-unidades,">>>>>9").
        ...
        ASSIGN ttWork.cod-livre-3:SCREEN-VALUE IN FRAME {&Frame02Name} = STRING(p-qtd-caixas,">>>>>9") + "/" + STRING(p-qtd-unidades,">>>>>9").
    END.
    ...
    Assign //ttWork.cod-item       :Screen-value In Frame {&Frame01Name} = ttWm-box-movto-idx-picking.cod-item
           ...
           vdes-item             :Screen-value In Frame {&Frame02Name} = TRIM(ttWm-box-movto-idx-picking.cod-item) + "-" + SUBSTRING(c-desc-item,1,15)
            //ttWork.cod-item       :Screen-value In Frame {&Frame02Name} = ttwm-docto-itens.cod-item
           ...
           ttWork.cod-lote       :Screen-value In Frame {&Frame02Name} = ttWm-box-movto-idx-picking.cod-lote
    ...
    If  Return-value <> 'OK':U OR NOT VALID-HANDLE(wgbc9018f) Then Do:
        Assign vLogErro = Yes.
         {bcp/bc9105.i "104" "Problemas na inicializa‡Æo (DC)"}
    End.

    /* Verifica se h  alguma separa‡Æo com a transa‡Æo pendente de atuliza‡Æo */
    ...
    IF l-movto-pendente = YES THEN DO:
        ...
        END.
        ELSE 
            ASSIGN ttWork.cod-livre-3:SCREEN-VALUE IN FRAME {&Frame02Name} = STRING(ttWork.qtd-embal-lidas,">>>>>9"). 
    END.

    IF VALID-HANDLE(wgbosc145) THEN RUN destroy IN wgbosc145.

    /* mk - ConversÆo emb/un - Fim */
    Pause 0 No-message.
End Procedure.
...
Procedure GravaCamposFrame02:
    ...
    /* Atribuindo o valor direto para o n£mero de itens, pois ‚ sem serial. Toma como referˆncia o movimento gerado pelo WMS  */
    ...
    /* Se a quantidade for diferente de zero, ‚ obrigado que seja informada a quantidade */
    /* de retirada. Esta ‚ a mesma regra da etiqueta pai.                                */
    RUN getInfoEtiqueta IN wgbosc074 (INPUT ttWork.num-serial,
                                      OUTPUT TABLE ttwm-etiqueta).
    ...
    /* Verifica se abre embalagem. O m‚todo da procedure verifica somente se  */
    /* ‚ aberta a embalagem pai.                                              */
    ...
    /* Valida se a quantidade a ser retirada ‚ maior ou igual a quantidade de saldo da etiqueta.*/
    ...
    /* quantidade de embalagens a serem retiradas. Este processo ‚ feito no programa BC9018G.   */
    ...
    Else Do:
        ...
        If Avail tt-picking-wms-table Then Do:
            Assign vLogErro = Yes.
    ...
    /* Permite alterar a quantidade quando o serial abre embalgem ou via item nÆo sendo Box de Picking */
    ...
    If  v-log-abre-embalagem Then Do: 
        ...
        Hide All No-pause.
        Run esp/bcp/esbcp015g.p (Input Rowid(ttwm-box),        
    ...
    /* Caso encontre algum movimento nÆo atualizado, incrementa a quantidade deste, */
    /* fazendo a separa‡Æo com a quantidade dispon¡vel da etiqueta.                 */
    If Available tt-picking-wms-table Then Do:
    ...
    /* Se nÆo encontrar nenhum movimento pendente, faz a separa‡Æo retirando o saldo     */
    /* dispon¡vel da etiqueta. Esta ser  a mesma quantidade que incrementa a quantidade  */
    /* de itens j  retirados da wm-box-movto. A atualiza‡Æo da wm-box-movto somente ser  */
    ...
    Else Do:         
        ...
        RUN pi-cria-tt-picking-ems-table IN wgbc9018f (INPUT ttWm-box-movto-idx-picking.cod-estabel,
                                                       ...
                                                       INPUT ttWork.cod-coletor,
                                                       INPUT ttWork.cod-equipamento,
                                                       input "", /* Lote em branco, pois se nÆo encontrar ‚ porque ‚ uma transa‡Æo com serial que ir  pegar o lote pela etiqueta na API de efetiva‡Æo */
                                                       INPUT-OUTPUT TABLE tt-picking-wms-table,
    ...
    /* Verifica a tabela que foi criada no programa bc9018f pelo m‚todo pi-atualiza-qtd-digitada */
    /* ou pi-cria-tt-picking-ems-table. A qtd-embal-lidas ‚ o que o WMS efetivou na separa‡Æo do */
    /* movimento, seja esta conclu¡da ou nÆo.                                                    */
    ASSIGN ttWork.qtd-embal-lidas = 0.
    FOR EACH tt-picking-wms-table 
       WHERE tt-picking-wms-table.id-docto       = ttWm-box-movto-idx-picking.id-docto 
         AND tt-picking-wms-table.id-movto       = ttWm-box-movto-idx-picking.id-movto
    ...
    /* Esta verifica‡Æo pode ser parcial, nÆo necessariamente precisando ser igual a quantidade do movimento         */
    /* Se a quantidade do movimento ‚ maior ou igual a quantidade separada para a etiqueta, ‚ gravada a transa‡Æo    */
    /* e feito o processo de separa‡Æo parcial ou total no WMS, acrescentando a qtd-item-picking da wm-box-movto     */
    /* e baixando o saldo da etiqueta. Esta l¢gica somente ser  acessada se a quantidade da etiqueta for menor       */ 
    /* ou igual a quantidade do movimento, caso contr rio, est  rotina ‚ feita no programa BC9018G, onde ‚ informada */
    ...
    If  ((ttWm-box-movto-idx-picking.qtd-item * ttWm-box-movto-idx-picking.qti-embalagem) - ttWm-box-movto-idx-picking.qtd-item-picking) >= ttWork.qtd-embal-lidas Then Do:
        ...
        ASSIGN cTextoOpcao:SCREEN-VALUE IN Frame {&Frame02Name} = "1-Conf. 2-NConf.".
        DISPLAY ttWork.qtd-item @ ttWork.qtd-item-digit WITH Frame {&Frame02Name}.

        UPDATE  ttWork.opcao WITH Frame {&Frame02Name}.
        IF ttWork.opcao = 1
        THEN DO:
            Run gravaTransacao.       
        END.
...
PROCEDURE pi-enderecos-entrega:
    ...
    IF VALID-HANDLE(wgbc9018f) THEN RUN pi-destroy-dbo IN wgbc9018f.
...
PROCEDURE pi-ler-endereco-transito:
    ...
    REPEAT:
        ON END-ERROR OF de-id-box in frame {&Frame04Name}
        DO: 
            return no-apply.
        END.
...
PROCEDURE pi-getIndTipoLeitura:
    ...
    IF v-log-abre-embalagem = YES THEN DO:
        ...
        /* Se nÆo achar o parametro, assume o valor 2 */
...
/* mk - Verifica tipo de visualiza‡Æo - Inicio*/
...
END PROCEDURE.
/* mk - Verifica tipo de visualiza‡Æo - Fim */
