...
def new global shared var c_lista_cc
    as character 
    format "x(80)":U 
    no-undo. 
...
def query qr_sea_grp_clien 
    for emscad.ccusto, usu-cc-un-orc
    scrolling. 
...
def var rs_sea_grp_clien 
    ...
    radio-buttons "Por CCusto", "Por CCusto","Por Descri‡Æo", "Por Descri‡Æo" 
    bgcolor 15  
    no-undo. 
...
def frame f_sea_01_grp_clien 
    ...
    bt_can 
         ...
         help "Cancela" 
    bt_todos
         AT ROW 11.46 COL 25.00 FONT ? 
         HELP "Todos"
    bt_nenhum
         AT ROW 11.46 COL 36.00 FONT ? 
         HELP "Nenhum"
    with 1 down side-labels no-validate keep-tab-order three-d 
         size-char 66.72 by 13.04 default-button bt_ok 
         view-as dialog-box 
         font 1 fgcolor ? bgcolor 8 
         title "Pesquisa CCusto - ES5500ZG - 1.00.00.001". 
...
ON CHOOSE OF bt_ok IN FRAME f_sea_01_grp_clien DO: 
    ...
    DO i-linha = 1 TO br_sea_grp_clien:NUM-SELECTED-ROWS :
       if  br_sea_grp_clien:fetch-selected-row(i-linha) then do:       
           ASSIGN c_lista_cc = c_lista_cc + emscad.ccusto.cod_ccusto + IF  i-linha < br_sea_grp_clien:NUM-SELECTED-ROWS THEN "," ELSE "".                      
       END.
    END.      
END.
...
DO: 
    ...
    IF  input frame f_sea_01_grp_clien  rs_sea_grp_clien = "Por descri‡Æo" THEN
        ASSIGN v_cod_final = v_cod_initial.
        ...
        when "Por CCusto" then 
            ...
            do: 
                 open query qr_sea_grp_clien for 
                    ...
                    OR     emscad.ccusto.cod_empresa  = "5"
                    OR     emscad.ccusto.cod_empresa  = "6")
                    AND    emscad.ccusto.cod_plano_cc = "padrao"
                    AND    emscad.ccusto.cod_ccusto  >= v_cod_initial
                    AND    emscad.ccusto.cod_ccusto  <= v_cod_final,
                    FIRST  usu-cc-un-orc
                    WHERE usu-cc-un-orc.cod-empresa= emscad.ccusto.cod_empresa
                     AND  usu-cc-un-orc.cod-usuar  = v_cod_usuar_corren 
                     AND  (usu-cc-un-orc.cod-ccusto = emscad.ccusto.cod_ccusto                     
                     OR    usu-cc-un-orc.cod-ccusto = "*")                     
        ...
        when "Por Descri‡Æo" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    ...
                    OR     emscad.ccusto.cod_empresa   = "6")
                    AND    emscad.ccusto.cod_plano_cc  = "padrao"
                    AND    emscad.ccusto.des_tit_ctbl >= v_cod_initial
                    AND    emscad.ccusto.des_tit_ctbl <= v_cod_final,
                    FIRST usu-cc-un-orc
                    WHERE usu-cc-un-orc.cod-empresa= emscad.ccusto.cod_empresa
                     AND  usu-cc-un-orc.cod-usuar  = v_cod_usuar_corren 
                     AND (usu-cc-un-orc.cod-ccusto = emscad.ccusto.cod_ccusto
                     OR   usu-cc-un-orc.cod-ccusto = "*")
                     by emscad.ccusto.des_tit_ctbl. 
            end /* do name_block */. 
