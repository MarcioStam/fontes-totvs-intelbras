...
*******************************************************************************/
{include/i-prgvrs.i ESENP001 2.04.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESENP001
...
&GLOBAL-DEFINE page1Widgets   br-posicao BtInc BtDel bt-ok
&GLOBAL-DEFINE page2Widgets   FiPosIni FiLetra FiPosFim ~
                              BtOk BtCancel

/* Parameters Definitions ---                                           */
DEF NEW GLOBAL SHARED VAR vRowEstrutura     AS ROWID NO-UNDO.
...
DEFINE FRAME fPage1
     ...
     bt-ok AT ROW 14.75 COL 1.86 WIDGET-ID 2
...
DEFINE FRAME fPage2
     FiLetra AT ROW 2 COL 34 COLON-ALIGNED
     FiPosIni AT ROW 3 COL 34 COLON-ALIGNED
     FiPosFim AT ROW 4 COL 34 COLON-ALIGNED
     btOK AT ROW 14.71 COL 2
     btCancel AT ROW 14.71 COL 13
     RECT-1 AT ROW 1.5 COL 6
...
OR CHOOSE OF MENU-ITEM miExit IN MENU mbMain DO:
    ...
    IF LENGTH(c-letra) > 40 THEN DO:
      /*MESSAGE c-letra SKIP
              "A quantidade de posicoes estrapolou o tamanho do campo" 
              VIEW-AS ALERT-BOX.*/
    ...
    END.
    ELSE DO:
        FIND CURRENT estrutura EXCLUSIVE-LOCK NO-ERROR.
        IF l-alterou AND AVAIL estrutura THEN 
            RUN upc/en0105ax-upc.p (INPUT ROWID(estrutura), 
                                    INPUT "NO", 
                                    INPUT estrutura.local-montag).
         /*ASSIGN estrutura.local-montag = c-letra.*/
         FIND CURRENT estrutura NO-LOCK NO-ERROR.
         IF l-alterou AND AVAIL estrutura THEN DO:
             RUN upc/en0105ax-upc.p (INPUT ROWID(estrutura), 
                                     INPUT "YES", 
                                     INPUT "").
         END.
    END.
    ASSIGN wh-local-montag:SCREEN-VALUE = c-letra /*estrutura.local-montag*/ .
    APPLY "CLOSE":U TO THIS-PROCEDURE.
END.
...
DO:
    ...
    DEFINE VARIABLE l-ini-int AS LOGICAL     NO-UNDO.
    DEFINE VARIABLE l-fim-int AS LOGICAL     NO-UNDO.

    ASSIGN INPUT FiLetra
           INPUT fiPosIni
           INPUT fiPosFim.

    ASSIGN l-ini-int = TRUE
           l-fim-int = TRUE.
    ...
    IF l-ini-int <> l-fim-int THEN DO:

        MESSAGE "Os campos 'Posi‡Æo Inicial' e 'Posi‡Æo Final' devem ser do mesmo tipo, caracter ou inteiro."
    ...
    IF l-ini-int AND l-fim-int THEN DO:

        IF int(FiPosini) > int(FiPosfim) THEN DO:
           MESSAGE "Posicao inicial maior que posicao final" 
                   VIEW-AS ALERT-BOX.
           RETURN NO-APPLY.
        END.
...
------------------------------------------------------------------------------*/
    ...
    FOR FIRST estrutura 
        WHERE ROWID(estrutura) = vRowEstrutura.
        ASSIGN l-texto = NO.
    END.
    /*IF NOT AVAIL estrutura THEN RETURN "NOK".*/
    /*
    ...
    RUN piTestaLocalMont(INPUT  cLocalMontagem /*estrutura.local-montag*/ , 
                         OUTPUT l-erro).
    IF l-erro THEN DO:
       MESSAGE "Local de Montagem esta fora dos padroes permitidos."
               "Favor cadastrar Local de Montagem de acordo com os padräes." VIEW-AS ALERT-BOX.
       ASSIGN l-texto = YES.
       RUN upc/en0105ax-upc.p (INPUT vRowEstrutura, 
                               INPUT "YES", 
                               INPUT "").
    END.
