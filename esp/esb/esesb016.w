...
*******************************************************************************/
{include/i-prgvrs.i ESESB016 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        ESESB0016
...
DEFINE TEMP-TABLE tt-param-canal NO-UNDO LIKE int-param-canal-benef
...
&Scoped-define INTERNAL-TABLES tt-param-canal

/* Definitions for BROWSE brParam                                       */
&Scoped-define FIELDS-IN-QUERY-brParam tt-param-canal.cod-unid-negoc tt-param-canal.ds-tp-beneficio tt-param-canal.cod-canal-venda   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brParam   
&Scoped-define SELF-NAME brParam
&Scoped-define QUERY-STRING-brParam FOR EACH tt-param-canal
&Scoped-define OPEN-QUERY-brParam OPEN QUERY {&SELF-NAME} FOR EACH tt-param-canal.
&Scoped-define TABLES-IN-QUERY-brParam tt-param-canal
&Scoped-define FIRST-TABLE-IN-QUERY-brParam tt-param-canal
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brParam wWindow _FREEFORM
  QUERY brParam NO-LOCK DISPLAY
      tt-param-canal.cod-unid-negoc
   tt-param-canal.ds-tp-beneficio COLUMN-LABEL "Benef¡cio"
   tt-param-canal.cod-canal-venda
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brParam
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tt-param-canal.
     _END_FREEFORM
     _Options          = "NO-LOCK"
     _Query            is OPENED
*/  /* BROWSE brParam */
...
DO:
  IF brParam:NUM-SELECTED-ROWS > 0 THEN
    ...
    IF AVAIL tt-param-canal THEN
        ASSIGN r-rowid-tt-param = tt-param-canal.r-rowid.
    ELSE
...
DO:
   ...
   ASSIGN r-rowid-tt = ROWID(tt-param-canal).

   RUN esp\esb\esesb016a.w (INPUT IF AVAIL tt-param-canal THEN tt-param-canal.r-rowid ELSE ?,
                            INPUT "Update",
...
DO:
    ...
    IF r-rowid-tt-param <> ? THEN DO:
         ...
         FIND int-param-canal-benef EXCLUSIVE-LOCK
            WHERE rowid(int-param-canal-benef) = r-rowid-tt-param NO-ERROR.
...
DO:
    RUN esp\esb\esesb016a.w (INPUT ?,
                             INPUT "Create",
                             OUTPUT r-rowid-inclusao).

    RUN atualizarBrowse IN THIS-PROCEDURE.

    FIND FIRST tt-param-canal NO-LOCK 
        WHERE tt-param-canal.r-rowid = r-rowid-inclusao NO-ERROR.

    IF AVAIL tt-param-canal THEN DO:
        ASSIGN r-rowid-tt = ROWID(tt-param-canal).
        REPOSITION brParam TO ROWID r-rowid-tt.
    END.
...
------------------------------------------------------------------------------*/
    ...
    EMPTY TEMP-TABLE tt-param-canal.

    FOR EACH  int-param-canal-benef NO-LOCK:

        CREATE tt-param-canal.
        BUFFER-COPY int-param-canal-benef TO tt-param-canal.
        ASSIGN tt-param-canal.r-rowid = ROWID(int-param-canal-benef).

        CASE int-param-canal-benef.tp-beneficio:
            WHEN 21 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "VMC".
            WHEN 22 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Stock Rotation".
            WHEN 37 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Rebate".
            WHEN 66 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Rebate P¢s-Venda".
            WHEN 15 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Show Room".
            WHEN 08 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Price Protection".
            WHEN 04 THEN
                ASSIGN tt-param-canal.ds-tp-beneficio = "Stock Backup".
    ...
    IF  CAN-FIND(FIRST tt-param-canal) THEN
