...
{include/i-prgvrs.i esesb008 2.00.00.000}
...
&Scoped-define QUERY-STRING-br-movimentos FOR EACH int-solicitacao NO-LOCK         WHERE int-solicitacao.CodigoConta                  = tt-conta-corrente.guid-canal           AND int-solicitacao.CodigoUnidadeNegocio         = tt-conta-corrente.unid-neg           AND int-solicitacao.tipo-beneficio               = tt-conta-corrente.tipo-beneficio           AND int-solicitacao.dt-periodo-ini               = tt-conta-corrente.dt-periodo-ini           AND int-solicitacao.dt-periodo-fim               = tt-conta-corrente.dt-periodo-fim            BY int-solicitacao.DataCriacao            BY int-solicitacao.hora-trans.           /*AND int-solicitacao.SituacaoSolicitacaoBeneficio = 993520004:*/ /* Pago */
...
&Scoped-Define ENABLED-OBJECTS bt-relatorio-3 bt-memoria bt-relatorio-2 ~
bt-relatorio1 fi-canal bt-exit bt-goto-emitente bt-param-cc bt-ok ~
bt-ajustes bt-novo bt-expande bt-exit-3 bt-detalhes ~
...
DEFINE BUTTON bt-detalhes 
     IMAGE-UP FILE "adeicon/fntupgrd.bmp":U
     LABEL "" 
     SIZE 4 BY 1.17 TOOLTIP "Detalhes Faturamento/Devoluá‰es/SellOut".
...
DEFINE BUTTON bt-memoria DEFAULT 
     IMAGE-UP FILE "image\im-orct1":U NO-FOCUS FLAT-BUTTON
     LABEL "" 
     SIZE 4 BY 1.17 TOOLTIP "Mem¢ria de c†lculo".
...
DEFINE BUTTON bt-relatorio-3 DEFAULT 
     IMAGE-UP FILE "adeicon/rpt-u.bmp":U NO-FOCUS FLAT-BUTTON
     LABEL "" 
     SIZE 4 BY 1.17 TOOLTIP "Saldo Provisionamento Ativo".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-movimentos w-cadsim _FREEFORM
  QUERY br-movimentos DISPLAY
     ...
     (IF int-solicitacao.desc-forma-pagto = "PRODUTO" THEN 
         int-solicitacao.ValorPago                                     
     ELSE IF int-solicitacao.SituacaoSolicitacaoBeneficio = 993520004 THEN
      ...
      (IF  int-solicitacao.Ajuste THEN 
          0 
      ELSE IF int-solicitacao.desc-forma-pagto = "PRODUTO" THEN
          (int-solicitacao.ValorSolicitado - int-solicitacao.vl-empenho-pago - int-solicitacao.vl-abatido-apb)
           ELSE IF  int-solicitacao.SituacaoSolicitacaoBeneficio = 993520004 THEN
                    0 
                ELSE     
                    int-solicitacao.ValorSolicitado  
...
DEFINE FRAME f-cad
     bt-relatorio-3 AT ROW 1.13 COL 93 HELP
          "Saldo Provisionamento Ativo" WIDGET-ID 114
     bt-memoria AT ROW 1.13 COL 114 HELP
          "Detalha Geraá∆o Saldo Conta Corrente e envia e-mail" WIDGET-ID 108
     ...
     bt-exit-3 AT ROW 1.13 COL 104.86 HELP
          "Definir permiss∆o de acesso as funá‰es do programa" WIDGET-ID 24
     bt-detalhes AT ROW 1.13 COL 109.43 HELP
          "Detalhes da movimentaá∆o mensa/trimestral para o canal" WIDGET-ID 28
...
DEFINE FRAME fPage1
     ...
     "Provisionamento:" VIEW-AS TEXT
          SIZE 13 BY .54 AT ROW 1.21 COL 39.43 WIDGET-ID 96
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-detalhes w-cadsim
...
DO:
    ...
    ELSE
        IF  AVAIL tt-canal THEN
            ASSIGN r-row-canal = tt-canal.r-rowid
                   r-row-cc    = ?.
        ELSE
            ASSIGN r-row-canal = ?
                   r-row-cc    = ?.
...
&Scoped-define SELF-NAME bt-exit-3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-exit-3 w-cadsim
ON CHOOSE OF bt-exit-3 IN FRAME f-cad
DO:
...
&Scoped-define SELF-NAME bt-memoria
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-memoria w-cadsim
ON CHOOSE OF bt-memoria IN FRAME f-cad
DO:
    DEF VAR c-erro-permissao AS CHAR NO-UNDO.
      /* VERIFICAÄ«O DE PERMISSÂES DE USUµRIOS */
    RUN pi-valida-operacao-usuario (INPUT  c-seg-usuario,
                                    INPUT  7, /* Enviar log para de calculo de benef°cio para cliente*/
                                    OUTPUT c-erro-permissao) .

    IF  RETURN-VALUE <> "OK" THEN DO:
        RUN utp/ut-msgs.p(input "show":U, 
                          input 17006,
                          input "Usu†rio sem permiss∆o para executar esse programa" + "~~" + c-erro-permissao).
        RETURN NO-APPLY.
    END.
...
&Scoped-define SELF-NAME bt-relatorio-3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-relatorio-3 w-cadsim
ON CHOOSE OF bt-relatorio-3 IN FRAME f-cad
DO:
    DEF VAR c-arquivo AS CHAR NO-UNDO.
    DEF VAR i-canal           AS INTEGER NO-UNDO.
    DEF VAR c-nome-abrev      AS CHAR    NO-UNDO.
    ...
    DEF VAR de-base           AS DEC     NO-UNDO.
    DEF VAR de-meta           AS DEC     NO-UNDO.
    DEF VAR h-acomp           AS HANDLE  NO-UNDO.

    ASSIGN c-arquivo = STRING(SESSION:TEMP-DIRECTORY) + "Saldo_Provisao_" + STRING(TODAY, "99-99-9999") + "_"+ STRING(TIME) + ".csv".

    OUTPUT STREAM s-1 TO value(c-arquivo) CONVERT TARGET "iso8859-1".

    PUT STREAM s-1 "Canal;NomeAbrev;Nome;Tp Benef;Benef°cio;Unidade;Classif;Categoria;Base Fatur; % Ating Meta;% Custo; % Benef°cio;Provis∆o" SKIP.
    ...
    FOR EACH int-cc-benef NO-LOCK
        WHERE int-cc-benef.id-status = 1 /* ATIVAS */
          AND int-cc-benef.tp-movto  = 1 /* PROVISÂES */
        ,FIRST emitente NO-LOCK
            WHERE emitente.cod-emitente = int-cc-benef.canal:

        RUN pi-acompanhar IN h-acomp ("Gerando Relat¢rio de Provis‰es Ativas...").
        FIND FIRST int-class-canal NO-LOCK
             WHERE int-class-canal.codigo-classificacao = int-cc-benef.classificacao NO-ERROR.
        ...
        ASSIGN  i-canal       = int-cc-benef.canal
                ...
                c-class       = IF  NOT AVAIL int-class-canal THEN "N∆o encontrou" ELSE int-class-canal.nome            
                c-categoria   = int-cc-benef.categoria 
                de-base       = int-cc-benef.vl-base-calc
                de-meta       = int-cc-benef.perc-prov-meta
                de-custo      = int-cc-benef.perc-custo
                de-benef      = int-cc-benef.perc-benef
...
------------------------------------------------------------------------------*/
  ...
  ENABLE bt-relatorio-3 bt-memoria bt-relatorio-2 bt-relatorio1 fi-canal 
         bt-exit bt-goto-emitente bt-param-cc bt-ok bt-ajustes bt-novo 
         bt-expande bt-exit-3 bt-detalhes bt-exporta-ContaCorrente rt-button 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esesb008" "2.00.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH int-cc-benef NO-LOCK
        ...
        ASSIGN tt-conta-corrente.desc-tp-movto        = fnMovto(tt-conta-corrente.tp-movto)          
               ...
               tt-conta-corrente.nome-classificacao   = LOWER(int-class-canal.nome)
...
/*     /*-------------------------*/                                                                                                          */
/*                                                                                                                                            */
/*     IF  NOT CAN-FIND (FIRST tt-central                                                                                                     */
/*                         WHERE tt-central.canal-central = tt-conta-corrente.canal) THEN DO:                                                 */
/*         EMPTY TEMP-TABLE tt-erro.                                                                                                          */
/*         RUN esp/esb/esesbapi005.p (INPUT tt-conta-corrente.canal,                                                                          */
...
/*     EMPTY TEMP-TABLE tt-canal-crm.                                                                                                         */
/*                                                                                                                                            */
/*     FIND FIRST tt-central                                                                                                                  */
/*         WHERE tt-central.canal-central = tt-conta-corrente.canal NO-ERROR.                                                                 */
/*     IF  NOT AVAIL tt-central THEN DO:                                                                                                      */
/*         RUN utp/ut-msgs.p(INPUT "show",                                                                                                    */
/*                           INPUT 17006,                                                                                                     */
/*                           INPUT "Canal inv†lido. A apuraá∆o de benef°cios deve ser feita pela centralizada pela matriz").                  */
/*         RETURN "NOK".                                                                                                                      */
/*     END.                                                                                                                                   */
/*                                                                                                                                            */
/*     CREATE tt-canal-crm.                                                                                                                   */
/*     ASSIGN tt-canal-crm.canal       = tt-conta-corrente.canal                                                                              */
/*            tt-canal-crm.guid-canal  = tt-central.guid-canal-central                                                                        */
/*            tt-canal-crm.guid-class  = tt-central.guid-class-central.                                                                       */
/*                                                                                                                                            */
/*     RUN esp/esb/esesbapi004-benef.p (INPUT  NO, /*Indica que n∆o Ç provisionamento necessariamente, logo busca o stock rotation */         */
/*                                      INPUT  NO, /* Buscar msg0111 com o % global de cada benef°cio   */                                    */
/*                                      INPUT  NO, /* Buscar msg0142, parÉmetros financeiros e provis∆o */                                    */
/*                                      INPUT  tt-conta-corrente.unid-neg, /* (?) Totadas as Unidades ou informe a unidade desejada */        */
/*                                      INPUT  tt-conta-corrente.tipo-beneficio, /* (?) Totadas as Unidades ou informe a unidade desejada */  */
/*                                      INPUT  TABLE tt-canal-crm,                                                                            */
/*                                      OUTPUT TABLE tt-erro,                                                                                 */
...
/*         RETURN "NOK".                                                                                                                      */
/*                                                                                                                                            */
/*     /* ATRIBUI O STATUS DO BENEF÷CIO */                                                                                                    */
/*     FIND FIRST tt-beneficio                                                                                                                */
/*         WHERE tt-beneficio.unid-neg       = tt-conta-corrente.unid-neg                                                                     */
/*           AND tt-beneficio.tipo-beneficio = tt-conta-corrente.tipo-beneficio NO-ERROR.                                                     */
/*                                                                                                                                            */
/*     DEF VAR c-status AS CHAR NO-UNDO.                                                                                                      */
/*     IF  AVAIL tt-beneficio THEN DO WITH FRAME fpage1:                                                                                      */
...
/*         END.                                                                                                                               */
/*                                                                                                                                            */
/*         RUN pi-seta-status-crm-em-tela (INPUT c-status).                                                                                   */
/*                                                                                                                                            */
/*     END.                                                                                                                                   */
/*                                                                                                                                            */
/*     ASSIGN tt-conta-corrente.status-crm = fi-status-beneficio:SCREEN-VALUE.                                                                */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-display-nota-importante w-cadsim 
PROCEDURE pi-display-nota-importante :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    IF  AVAIL int-solicitacao THEN DO:
       ...
       IF  int-solicitacao.log-historica THEN 
           ASSIGN tx-nota:VISIBLE  IN FRAME fpage5 = YES
                  tx-nota:SCREEN-VALUE  IN FRAME fpage5 = "Nota Importante:"
                  ed-nota:VISIBLE IN FRAME fpage5 = YES
                  ed-nota:SCREEN-VALUE IN FRAME fpage5 = "Esta Solicitaá∆o existe como hist¢rico, foi transferida para o trimestre seguinte. ".
       ELSE DO:
           FIND b-hist-solicit NO-LOCK
               WHERE b-hist-solicit.CodigoSolicitacaoBeneficio = "HIST_" + int-solicitacao.CodigoSolicitacaoBeneficio NO-ERROR.
           IF  AVAIL b-hist-solicit THEN
               ASSIGN tx-nota:VISIBLE  IN FRAME fpage5 = YES
                      tx-nota:SCREEN-VALUE  IN FRAME fpage5 = "Nota Importante:"
                      ed-nota:VISIBLE IN FRAME fpage5 = YES
                      ed-nota:SCREEN-VALUE IN FRAME fpage5 = "Solicitaá∆o migrada de trimestre anterior".
            ELSE
               ASSIGN tx-nota:VISIBLE  IN FRAME fpage5 = NO
                      tx-nota:SCREEN-VALUE  IN FRAME fpage5 = ""
                      ed-nota:VISIBLE  IN FRAME fpage5 = NO
