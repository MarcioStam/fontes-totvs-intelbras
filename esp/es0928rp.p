...
/* run cralias2.p. */
{include/i-prgvrs.i es0928rp 2.00.00.000}
{esp/es0928.i}
{utp/ut-glob.i}
{include/i-rpvar.i}
...
def var l-cab     as log init no no-undo.
...
ASSIGN c-sistema      = "Espec¡ficos Intelbras"
       c-titulo-relat = "Saldo Terceiros Pendente"
...
run pi-inicializar in h-acomp (input "Saldo em Terceiros").
...
for each saldo-terc no-lock
    ...
    run pi-acompanhar in h-acomp (input "NF: " + saldo-terc.nro-docto).
    ...
    /* Gerar Relat¢rio - Valida Usu rio */
    IF tt-param.i-tipo = 2 THEN DO:
        IF AVAIL ped-fiscal THEN DO:
            IF NOT(ped-fiscal.usuario-magnus >= tt-param.usuario-ini  AND
                   ped-fiscal.usuario-magnus <= tt-param.usuario-fin) THEN NEXT.
        END.
        ELSE NEXT.
    END.
    ...
    if componente.dt-retorno <= today - 160 then do:
       ...
       assign tt-notas.nro-docto    = saldo-terc.nro-docto
              ...
              tt-notas.descricao    = REPLACE(REPLACE(REPLACE(c-desc,CHR(13),""),CHR(10),""),";","-")
...
IF tt-param.i-tipo = 1 THEN DO:
    ...
    for each tt-notas where
        ...
        if first-of(tt-notas.usuario-magnus) then do:
           ...
           /*ASSIGN c-Arquivo = SESSION:TEMP-DIRECTORY + "saldo-terc-1.htm"*/
           ASSIGN c-Arquivo = c-dir-arquivo-session + "saldo-terc-1.htm"
                  c-texto-html = "".
           ...
           assign c-texto-html[1] = "Prezado Colaborador, " + chr(13) + "                                                                                                                                                                                               "
                  ...
                  c-texto-html[4] = "Duvidas entre em contato com Controladoria " + chr(13) + "                                                                                                                                            "
        ...
        run pi-acompanhar in h-acomp (input "Gerando Arquivo/E-mail: " + tt-notas.nro-docto).
    ...
    /****** ENVIA RELATORIO COMPLETO PARA FABRICIO,LUCIANO E SUELEN ******/
    for each tt-notas where
        ...
        if first(tt-notas.enviados) then do:
           ...
           /*ASSIGN c-Arquivo = SESSION:TEMP-DIRECTORY + "saldo-terc-2.htm"*/
           ASSIGN c-Arquivo = c-dir-arquivo-session + "saldo-terc-2.htm"
                  c-texto-html = "".
...
ELSE DO:
    FOR EACH tt-notas        
        ...
        IF NOT(l-cab) THEN DO:
            ...
            PUT "Docto;Ser;Nat Op;Descricao;Item;Descricao;Fornec;Nome Abrev;Dt Envio;Qtde;Valor;Usuario;Observacao" SKIP.
        END.

        RUN pi-acompanhar IN h-acomp (INPUT "Gerando Arquivo Texto: " + tt-notas.nro-docto).
        
        PUT UNFORMATTED 
            tt-notas.nro-docto         ";"
            tt-notas.serie             ";"
            tt-notas.nat-operacao      ";"
            TRIM(tt-notas.denominacao) ";"
            tt-notas.it-codigo         ";"
            TRIM(tt-notas.descricao)   ";"
            tt-notas.cod-emitente      ";"
            tt-notas.nome-abrev        ";"
            tt-notas.dt-retorno        ";"
            tt-notas.quantidade        ";"
            tt-notas.valor             ";"
            tt-notas.usuario-magnus    ";".

        PUT UNFORMATTED 
            TRIM(REPLACE(REPLACE(REPLACE(tt-notas.observ-nota,CHR(10),""),CHR(13),""),";","-")) SKIP.        
    END.    
    {include/i-rpclo.i}
END.
