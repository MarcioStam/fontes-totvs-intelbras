...
PROCEDURE pi-evento:
    ...
    IF c-evento = "atualizaInicial" THEN DO:
        ...
        RUN esp/es0018p.p (INPUT "WSO0003-MIBO":U, /* Nome do programa */
        ...
        FOR FIRST tt-prog-ponto
            WHERE entry(1,tt-prog-ponto.conteudo,"|") = ttItem.codigoItem:
            ASSIGN ttItem.codigoItem = entry(2,tt-prog-ponto.conteudo,"|").
        END.

        FOR EACH ttItem:
            ASSIGN ttItem.estabelecimento = "101".

            ASSIGN ttItem.precoItem = ttItem.precoItem + ttItem.valorDesconto.
        ...
        IF AVAIL ttEndereco THEN DO:
            ...
            IF NOT CAN-FIND(first cep
                ...
                CREATE cep.
                ASSIGN cep.cep        = int(ttEndereco.CEP)
                       cep.nome-log   = REPLACE(ttEndereco.Logradouro,"-"," ")
                       cep.bairro-ini = ttEndereco.Bairro
                       cep.localidade = ttEndereco.municipio
                       cep.uf         = ttEndereco.siglaEstado.
    ...
    IF c-evento = "aposAtualizaPessoa" THEN DO:
        ...
        IF AVAIL emitente THEN DO:
            ...
            END.
            FIND CURRENT emitente EXCLUSIVE-LOCK NO-ERROR.
            ASSIGN emitente.agente-retencao = YES.
            FIND CURRENT emitente NO-LOCK NO-ERROR.
            RELEASE emitente.
        END.
    ...
    IF c-evento = "atualizaFinal" THEN DO:
        ...
        FOR FIRST ped-venda NO-LOCK
            ...
            FOR EACH ped-item OF ped-venda EXCLUSIVE-LOCK:
                ASSIGN ped-item.nat-operacao = IF emitente.natureza = 1 THEN "500012" ELSE "500014"
            ...
            ASSIGN ped-venda.nat-operacao = IF emitente.natureza = 1 THEN "500012" ELSE "500014"
        ...
        RUN pi-gerar-dados-extrato ("> ANTES atualizaFinal " + STRING(AVAIL tt-ped-venda)).
...
PROCEDURE pi-esftp016rp:
    ...
    IF AVAIL ped-venda THEN DO:
        ...
        IF ped-venda.cod-priori <> 7 THEN DO:
            ...
            EMPTY TEMP-TABLE tt-prog-ponto.
            RUN esp/es0018p.p (INPUT "wso0003":U, INPUT 15, INPUT 0, INPUT "":U, OUTPUT TABLE tt-prog-ponto).
            FIND FIRST tt-prog-ponto NO-ERROR.

            create tt-param.
            assign tt-param.usuario     = IF AVAIL tt-prog-ponto THEN tt-prog-ponto.conteudo ELSE "integra-lms"
                   tt-param.destino     = 2
