...
DEFINE VARIABLE r-int-pedido-vtex      AS ROWID       NO-UNDO.
...
DEFINE VARIABLE h-prx073               AS HANDLE       NO-UNDO.
...
IF TRIM(ttPedido.marketplace) = "" THEN ASSIGN ttPedido.marketplace = "VTEX".

IF ttPedido.numeroPedido = "977450880822-01" THEN DO:
...
RUN pi-gerar-dados-extrato ("> ANTES pi-cria-historico-wso2").
RUN pi-cria-historico-wso2.
RUN pi-gerar-dados-extrato ("> DEPOIS pi-cria-historico-wso2").
RUN pi-gerar-dados-extrato ("> ANTES pi-cria-resposta-xml-wso2").
RUN pi-cria-resposta-xml-wso2.
RUN pi-gerar-dados-extrato ("> DEPOIS pi-cria-resposta-xml-wso2").
RUN pi-inicio.
...
PROCEDURE pi-cria-historico-wso2:
    ...
    FOR FIRST ttPedido:
        ...
        IF NOT AVAIL int-pedido-vtex THEN DO:
            ...
            ASSIGN tt-int-pedido-vtex.nr-pedido      = ttPedido.numeroPedido
                   tt-int-pedido-vtex.seq-pedido     = ttPedido.sequenciaPedido
                   tt-int-pedido-vtex.dt-integracao  = d-data-integr
                   tt-int-pedido-vtex.hr-integracao  = c-hota-integr
                   tt-int-pedido-vtex.cod-loja       = ttPedido.codigoLoja
                   tt-int-pedido-vtex.marketplace    = ttPedido.marketplace
                   tt-int-pedido-vtex.dt-criacao     = ttPedido.dataCriacao
                   /*tt-int-pedido-vtex.hr-criacao     = SUBSTRING(c-data-criacao,12,8)*/
                   tt-int-pedido-vtex.vl-tot-itens   = ttPedido.totalItens
                   tt-int-pedido-vtex.vl-tot-pedido  = ttPedido.totalPedido
                   tt-int-pedido-vtex.vl-tot-frete   = ttPedido.totalFrete
                   tt-int-pedido-vtex.vl-tot-desc    = ttPedido.totalDesconto
                   tt-int-pedido-vtex.reference-number = IF AVAIL ttCondicaoPagamento THEN ttCondicaoPagamento.numeroReferencia ELSE ""
                   tt-int-pedido-vtex.tid              = IF AVAIL ttCondicaoPagamento THEN ttCondicaoPagamento.tid ELSE ""
                   tt-int-pedido-vtex.dt-pagto       = TODAY.
                   /*tt-int-pedido-vtex.mo-codigo      = int(ttPedido.moedaCorrente)*/.
        END.

        IF AVAIL ttCondicaoPagamento THEN
            ASSIGN tt-int-pedido-vtex.forma-pagto    = ttCondicaoPagamento.formaPagamento
                   tt-int-pedido-vtex.dt-pagto       = ttCondicaoPagamento.dataCaptura
                   /*tt-int-pedido-vtex.hr-pagto       = SUBSTRING(c-data-captura,12,8)*/
                   tt-int-pedido-vtex.qtd-parcelas   = ttCondicaoPagamento.quantidadeParcelas
                   tt-int-pedido-vtex.final-cartao   = string(ttCondicaoPagamento.finalCartao)
                   tt-int-pedido-vtex.id-autorizacao = ttCondicaoPagamento.idAutorizacao
                   tt-int-pedido-vtex.nsu-cartao     = ttCondicaoPagamento.nsu
                   tt-int-pedido-vtex.vl-tot-pagto   = ttCondicaoPagamento.valorTotal.

        IF AVAIL ttTelefone THEN
            ASSIGN tt-int-pedido-vtex.telefone      = ttTelefone.ddd + ttTelefone.telefone
                   tt-int-pedido-vtex.telefone-tipo = ttTelefone.tipo.

        IF AVAIL ttEmail THEN
            ASSIGN tt-int-pedido-vtex.email      = ttEmail.endereco.
                   tt-int-pedido-vtex.email-tipo = ttEmail.tipo.

        IF AVAIL ttEndereco THEN DO:
            ...
            ASSIGN tt-int-pedido-vtex.endereco    = REPLACE(ttEndereco.logradouro,"-"," ")
                   tt-int-pedido-vtex.cep         = ttEndereco.cep
                   tt-int-pedido-vtex.bairro      = ttEndereco.bairro
                   tt-int-pedido-vtex.cidade      = ttEndereco.municipio
                   tt-int-pedido-vtex.estado      = ttEndereco.siglaEstado
                   tt-int-pedido-vtex.complemento = ttEndereco.complemento
                   tt-int-pedido-vtex.numero      = ttEndereco.numero
                   tt-int-pedido-vtex.pais        = ttEndereco.siglaPais.
        END.

        IF l-pj THEN DO:
            FOR FIRST ttpessoaJuridica:
                ASSIGN tt-int-pedido-vtex.nome = ttpessoaJuridica.nomeFantasia.
            END.

            FOR FIRST ttDocumento
                WHERE ttDocumento.tipoDocumento = "CNPJ":
                ASSIGN tt-int-pedido-vtex.tipo-docto = ttDocumento.tipoDocumento
                       tt-int-pedido-vtex.num-docto  = ttDocumento.numeroDocumento.
            END.
            FOR FIRST ttDocumento
                WHERE ttDocumento.tipoDocumento = "inscricaoEstadual":
                ASSIGN tt-int-pedido-vtex.ins-estadual = ttDocumento.numeroDocumento.
            END.
        END.
        ELSE DO:
            FOR FIRST ttpessoaFisica:
                ASSIGN tt-int-pedido-vtex.nome = ttpessoaFisica.nome.
            END.
        ...
        IF NOT AVAIL int-pedido-vtex THEN DO:
            ...
            BUFFER-COPY tt-int-pedido-vtex TO int-pedido-vtex.
            ASSIGN r-int-pedido-vtex = ROWID(int-pedido-vtex).
        ...
        FOR EACH ttComissao:
            ...
            FIND FIRST int-pedido-comissao NO-LOCK
        ...
        ASSIGN int-ocorrencias-vtex.nr-pedido     = ttPedido.numeroPedido     
               ...
               int-ocorrencias-vtex.tipo-registro = "Pedido"
               int-ocorrencias-vtex.cod-erro      = 999
               int-ocorrencias-vtex.descricao     = "Registro integrado, falta criar o pedido.".
               
    END.
END PROCEDURE.

/* TRATAMENTO PARA RETORNAR SUCESSO OU ERROS */
PROCEDURE pi-cria-resposta-xml-wso2:
    FIND FIRST ttPedido NO-ERROR.
...
PROCEDURE pi-inicio:
    ...
    /** Limpa temp-tables de integraªío **/
...
PROCEDURE pi-atualiza-pedido:
    ...
    FIND FIRST repres-atend NO-LOCK
        WHERE  repres-atend.cod-estab = c-estabelec-xml
           AND repres-atend.cd-oper   = i-cod-atendente NO-ERROR.
    ...
    /*  Respacho  */
    /*------------*/
    IF ttPedido.marketplace = "MIBO" OR ttPedido.marketplace = "GLX" OR ttPedido.marketplace = "LMS" THEN
        ASSIGN c-cod-redespacho = "".
    ...
    FOR EACH ttItem:
        ...
        ASSIGN tt-ped-item.aliquota-ipi            = item.aliquota-ipi
               ...
               tt-ped-item.ct-codigo               = ITEM.ct-codigo.
    ...
    IF AVAILABLE ped-venda THEN DO:
        ...
        END.
        ELSE DO:
            IF int-ped-venda2.PedidoeCommerce <> ttPedido.numeroPedido THEN DO:
    ...
    */
    ...
    AND ttPedido.marketplace <> "LMS" THEN DO:
        ASSIGN tt-ped-venda.observacoes = "Pedido RETIRA - Estado do cliente difere do estado de entrega.".
        RUN UpdateSuspension.
    END.             
    ...
    AND ttPedido.codigoLoja <> "3" THEN DO:
        ASSIGN tt-ped-venda.tp-pedido = "22"
    ...
    FOR FIRST ped-venda NO-LOCK
        ...
        FIND CURRENT ped-venda EXCLUSIVE-LOCK NO-ERROR.
        ASSIGN ped-venda.observacoes = tt-ped-venda.observacoes
               ped-venda.tp-pedido   = tt-ped-venda.tp-pedido
               ped-venda.ind-fat-par = tt-ped-venda.ind-fat-par
               ped-venda.dt-suspensao = tt-ped-venda.dt-suspensao
               ped-venda.user-susp    = tt-ped-venda.user-susp   
               ped-venda.dt-usersusp  = tt-ped-venda.dt-usersusp 
               ped-venda.desc-suspend = tt-ped-venda.desc-suspend
               ped-venda.dt-useralt   = tt-ped-venda.dt-useralt  
               ped-venda.user-alt     = tt-ped-venda.user-alt    
               ped-venda.cod-sit-ped  = tt-ped-venda.cod-sit-ped 
               ped-venda.cod-priori   = tt-ped-venda.cod-priori.  
...
PROCEDURE pi-executar-bos:
   ...
   IF AVAIL ped-venda THEN DO:
       ...
       /* Cria a Cond-ped antes do Complete, onde o pedido jˇ estˇ criado e antes da validaªío de parcelamento */
       ...
       FOR FIRST int-pedido-vtex NO-LOCK
           WHERE int-pedido-vtex.nr-pedido     = ttPedido.numeroPedido
             ...
             AND int-pedido-vtex.dt-integracao = d-data-integr
             AND int-pedido-vtex.hr-integracao = c-hota-integr:
   ...
   IF (ped-venda.cod-sit-ped <= 2) AND (ped-venda.completo)  THEN DO:
      ...
      */
      ...
      FOR EACH ped-item OF ped-venda NO-LOCK
         ...
         FOR EACH int-vtex-estab-depos NO-LOCK
             ...
             FOR FIRST saldo-estoq NO-LOCK
                WHERE  saldo-estoq.it-codigo = ped-item.it-codigo
                  ...
                  AND  saldo-estoq.cod-depos = int-pedido-param.deposito-pad
             ...
             FOR EACH reservas-ast
                 WHERE reservas-ast.cod-depos   = int-pedido-param.deposito-pad
         ...
         END.
         ELSE DO:
             IF int-ped-item.log-transferido <> YES THEN DO:
...
PROCEDURE pi-cria-tt-emitente:
    ...
    IF (i-ind-tipo-movto = 1) THEN DO:
        ...
        ASSIGN tt-emitente.cod-emitente   = i-cod-emitente
               ...
               tt-emitente.identIFic      = 3 /*1 Cliente, 2 Fornecedore, 3 Ambos*/
               ...
               tt-emitente.tp-desp-padrao = 8
               tt-emitente.cod-gr-forn    = 3
               ...
               tt-emitente.cod-inscr-inss  = "ISENTO"
               tt-emitente.portador-ap     = 999
               tt-emitente.char-1          = "     N                                                                                               1                              N     10                    "
               tt-emitente.char-2          = "                                                                                                    N  Produtos                  ".
        ...
        ASSIGN tt-emitente.observacoes    = 'Cadastro inserido pelo ' + ttPedido.marketplace + ' em ' + string(today,"99/99/9999") + '.~r'.
     ...
     ELSE do:
        ...
        ASSIGN tt-emitente.observacoes     = 'Cadastro modificado pelo ' + ttPedido.marketplace + ' em ' + string(today,"99/99/9999") + '.~r'.
     ...
     FOR LAST  ttendereco NO-LOCK
         ...
         IF LENGTH(ttEndereco.complemento) < 2 THEN
             ASSIGN ttendereco.complemento = "Complemento " + ttendereco.complemento.
    ...
    RUN pi-regras-especificas (INPUT "aposCriaTTEmitente").
     ...
     ELSE do:
        ...
        for each tt-int-loc-entr NO-LOCK:
           ...
           END.
           ELSE DO:
               IF int-loc-entr.endereco-completo <> tt-int-loc-entr.endereco-completo THEN DO:
        ...
        FIND FIRST wm-cliente NO-LOCK
             WHERE wm-cliente.cod-cliente = tt-emitente.cod-emitente
               AND wm-cliente.nome-abrev  = tt-emitente.nome-abrev NO-ERROR.
        IF NOT AVAIL wm-cliente THEN DO:
            ...
            RUN replicaCliente IN h-prx073(INPUT tt-emitente.cod-emitente,
                                           INPUT tt-emitente.nome-abrev,
...
PROCEDURE UpdateSuspension:
    ...
    IF  tt-ped-venda.observacoes <> "" AND
        tt-ped-venda.cod-priori <> 44  AND 
        tt-ped-venda.cod-priori <> 3 THEN DO:
        FOR FIRST ped-item OF tt-ped-venda
            WHERE ped-item.cod-sit-item <= 2 NO-LOCK:
                FOR FIRST ped-ent OF ped-item
                    WHERE ped-ent.cod-sit-ent <= 2 NO-LOCK:

                    FIND CURRENT ped-ent EXCLUSIVE-LOCK NO-ERROR.
                    ASSIGN ped-ent.cod-sit-ent  = 5
                           ped-ent.dt-suspensao = TODAY
                           ped-ent.user-susp    = "Integra"
                           ped-ent.dt-usersusp  = TODAY.
                    FIND CURRENT ped-ent NO-LOCK NO-ERROR.
                    RELEASE ped-ent.
                END.
        ...
        ASSIGN tt-ped-venda.dt-suspensao = today
               tt-ped-venda.user-susp    = "Integra"
               tt-ped-venda.dt-usersusp  = today
               tt-ped-venda.desc-suspend = ped-venda.observacoes
               tt-ped-venda.dt-useralt   = TODAY 
               tt-ped-venda.user-alt     = "Integra"
               tt-ped-venda.cod-sit-ped  = 5.
        
        IF NOT tt-ped-venda.observacoes MATCHES "*composto*" THEN
