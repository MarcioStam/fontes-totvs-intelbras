/***********************************************************************************************
** Integra estados para condi‡Æo de pagamento 
***********************************************************************************************/
{esp/esapi505.i} 
{esp/wso/eswso0014.i} 
...
FIND FIRST tt-prog-ponto 
  WHERE ENTRY(1,tt-prog-ponto.conteudo,";") = 'eswso0014' NO-ERROR.
IF AVAILABLE tt-prog-ponto AND
...
IF l-log = YES THEN DO:
    ...
    IF OPSYS = 'UNIX' THEN
       ASSIGN c-arquivo-log1 = '/mnt/spool/totvs/UNIX_eswso0014'.
    ELSE
       ASSIGN c-arquivo-log1 = '\\erpapp\spool\totvs\WIN_eswso0014'.
...
IF SEARCH("esp/wso/out/wso0014.r") <> ?
OR SEARCH("esp/wso/out/wso0014.p") <> ? 
   AND evento = 'PUT' THEN DO:
    run esp/wso/out/wso0014.p  (INPUT  row-table,
...
FOR EACH tt-cond BREAK BY tt-cond.cond-pagto:
    ...
    RUN pi-gerar-dados-extrato (INPUT 'Exportando CondPag: ' + STRING(tt-cond.cond-pagto) + '/' + STRING(tt-cond.uf) + ' ' + evento).
    RUN pi-gera-json.
    ...
    IF evento = 'PUT' THEN
        FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'integraCPagtoUF' NO-LOCK NO-ERROR.
    ELSE FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'deleteCPagtoUF' NO-LOCK NO-ERROR.
    ...
    ASSIGN es-api-log.seqexec        = IF AVAIL bf-las-api-log THEN bf-las-api-log.seqexec  +  10 ELSE 10  
           ...
           es-api-log.id-codigo      = IF AVAIL tt-cond THEN string(tt-cond.cond-pagto) + '/' + STRING(tt-cond.uf) ELSE '0'
           es-api-log.id-URI         = IF evento = 'PUT' THEN 'integraCPagtoUF' ELSE 'deleteCPagtoUF'
           es-api-log.dh-request     = NOW
           es-api-log.end-envio      = c-endereco
           es-api-log.flg-processado = NO
           es-api-log.Origem         = 'TOTVS'
           es-api-log.aux            = IF AVAIL tt-cond THEN  string(tt-cond.cond-pagto) + '/' + STRING(tt-cond.uf) ELSE '0'
