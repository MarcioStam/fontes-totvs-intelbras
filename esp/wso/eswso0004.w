...
{include/i-prgvrs.i eswso0004 2.00.00.000}
...
DEF BUFFER bint-ped-item-vtex FOR int-ped-item-vtex.
...
&Scoped-define FIELDS-IN-QUERY-br-aprovados tt-int-pedido-vtex.dt-criacao tt-int-pedido-vtex.cod-estabel tt-int-pedido-vtex.nr-pedido tt-int-pedido-vtex.nr-pedcli tt-int-pedido-vtex.nr-nota-fis tt-int-pedido-vtex.cod-emitente tt-int-pedido-vtex.nome-emit tt-int-pedido-vtex.vl-tot-pagto tt-int-pedido-vtex.cod-loja fnSitPed(tt-int-pedido-vtex.cod-sit-ped) @ c-situacao tt-int-pedido-vtex.atendente tt-int-pedido-vtex.nome-transp tt-int-pedido-vtex.estado tt-int-pedido-vtex.cidade   
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "eswso0004" "2.00.00.000"}
  ...
  ASSIGN d-dt-implant-ini = TODAY - 30
         d-dt-integr-ini  = TODAY - 30.
...
RUN esp/es0018p.p (INPUT "eswso0004":U, INPUT 1, INPUT 0, INPUT "":U, OUTPUT TABLE tt-prog-ponto).

DEF VAR contMarketplace AS INT NO-UNDO.
DEFINE VAR l-elimina-registro AS LOG NO-UNDO.

ASSIGN c-marketplace = "".
...
run pi-inicializar in h-acomp (input 'Lendo pedidos.').
                                       
DO contMarketplace = 1 TO NUM-ENTRIES(c-marketplace, ","):
    FOR EACH int-pedido-vtex 
       WHERE int-pedido-vtex.marketplace = ENTRY(contMarketplace,c-marketplace,",")
         AND int-pedido-vtex.dt-integr >= d-dt-integr-ini 
        ...
        run pi-acompanhar in h-acomp (input 'Pedido: ' + int-pedido-vtex.nr-pedido).
        ...
        IF int-pedido-vtex.nr-pedcli <> "" THEN DO:
            ...
            IF AVAIL ped-venda THEN DO:
                ...
                IF CAN-FIND(FIRST bint-pedido-vtex NO-LOCK
                    ...
                    IF ped-venda.cod-sit-ped >= 3 THEN NEXT.
                ...
                /* PI-APROVA-PEDIDO */
                IF AVAIL ped-venda
                AND ped-venda.cod-sit-aval <> 3 THEN DO:
        ...
        ELSE DO:
            ...
            IF CAN-FIND(FIRST int-ocorrencias-vtex
                        WHERE int-ocorrencias-vtex.nr-pedido  = int-pedido-vtex.nr-pedido   
                          AND int-ocorrencias-vtex.seq-pedido = int-pedido-vtex.seq-pedido
                          AND int-ocorrencias-vtex.descricao  = "SPLIT DE PEDIDO") THEN DO:
        
                IF CAN-FIND(FIRST bint-pedido-vtex NO-LOCK
                            WHERE bint-pedido-vtex.seq-pedido = int-pedido-vtex.seq-pedido
                              AND bint-pedido-vtex.nr-pedido <> int-pedido-vtex.nr-pedido) THEN DO:
            
                    ASSIGN l-elimina-registro = YES.
                    ...
                    FOR EACH int-ped-item-vtex NO-LOCK
                       WHERE int-ped-item-vtex.nr-pedido                    = int-pedido-vtex.nr-pedido    
                        ...
                        IF NOT CAN-FIND(FIRST bint-ped-item-vtex
                                        WHERE bint-ped-item-vtex.seq-pedido = int-ped-item-vtex.seq-pedido
                                          AND bint-ped-item-vtex.nr-pedido <> int-ped-item-vtex.nr-pedido) THEN DO:
                            ASSIGN l-elimina-registro = NO.
                        END.
                    END.
                END.
                IF l-elimina-registro THEN DO:
                    FIND CURRENT int-pedido-vtex EXCLUSIVE-LOCK NO-ERROR.
                    DELETE int-pedido-vtex.
                    FIND CURRENT int-pedido-vtex EXCLUSIVE-LOCK NO-ERROR.
                END.
            END.
            IF l-elimina-registro = NO THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-carrega w-cadsim 
PROCEDURE pi-carrega-pedido-unico :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR FIRST int-pedido-vtex NO-LOCK
    ...
    DELETE tt-int-pedido-vtex-erro.
...
IF i-folder = 1 THEN DO:
    FOR EACH tt-int-pedido-vtex:
        PUT UNFORMATTED "Cria‡Æo;Estab;Pedido;Ped Cli;Nr NotaFiscal;Cliente;Loja;Atendente;Transp;Estado;Cidade;Valor;" SKIP.
