/***********************************************************************************************
** Integra condi‡Æo de pagamento do cliente 
***********************************************************************************************/
{esp/esapi505.i} 
{esp/wso/eswso0013.i} 
...
FIND FIRST tt-prog-ponto 
  WHERE ENTRY(1,tt-prog-ponto.conteudo,";") = 'eswso0013' NO-ERROR.
IF AVAILABLE tt-prog-ponto AND
...
IF l-log = YES THEN DO:
    ...
    IF OPSYS = 'UNIX' THEN
       ASSIGN c-arquivo-log1 = '/mnt/spool/totvs/UNIX_eswso0013'.
    ELSE
       ASSIGN c-arquivo-log1 = '\\erpapp\spool\totvs\WIN_eswso0013'.
...
IF SEARCH("esp/wso/out/wso0013.r") <> ?
OR SEARCH("esp/wso/out/wso0013.p") <> ? 
   AND evento = 'PUT' THEN DO:
    run esp/wso/out/wso0013.p  (INPUT  row-table,
...
FOR EACH tt-cond BREAK BY tt-cond.cond-pagto:
    ...
    RUN pi-gerar-dados-extrato (INPUT 'Exportando CondPag: ' + STRING(tt-cond.cond-pagto) + ' ' + STRING(tt-cond.externalID) + ' ' + evento).
    RUN pi-gera-json.
    ...
    IF evento = 'PUT' THEN
        FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'integraCPagtoCRM' NO-LOCK NO-ERROR.
    ELSE FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'deleteCPagtoCRM' NO-LOCK NO-ERROR.
    ...
    ASSIGN es-api-log.seqexec        = IF AVAIL bf-las-api-log THEN bf-las-api-log.seqexec  +  10 ELSE 10  
           ...
           es-api-log.id-codigo      = IF AVAIL tt-cond THEN string(tt-cond.externalID) ELSE '0'
           es-api-log.id-URI         = IF evento = 'PUT' THEN 'integraCPagtoCRM' ELSE 'deleteCPagtoCRM'
           es-api-log.dh-request     = NOW
           es-api-log.end-envio      = c-endereco
           es-api-log.flg-processado = NO
           es-api-log.Origem         = 'TOTVS'
           es-api-log.aux            = IF AVAIL tt-cond THEN string(tt-cond.externalId) ELSE '0'
