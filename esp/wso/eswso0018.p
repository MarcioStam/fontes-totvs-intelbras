...
DEFINE input-output PARAM table     for tt-benef.
...
FIND FIRST tt-prog-ponto 
  WHERE ENTRY(1,tt-prog-ponto.conteudo,";") = 'eswso0018' NO-ERROR.
IF AVAILABLE tt-prog-ponto AND
...
IF l-log = YES THEN DO:
    ...
    IF OPSYS = 'UNIX' THEN
       ASSIGN c-arquivo-log1 = '/mnt/spool/totvs/UNIX_eswso0018'.
    ELSE
       ASSIGN c-arquivo-log1 = '\\erpapp\spool\totvs\WIN_eswso0018'.
...
FOR EACH tt-benef :
    ...
    RUN pi-gerar-dados-extrato (INPUT 'Saldo Beneficio: ' + STRING(tt-benef.externalID) + ' ' + STRING(tt-benef.requestLimit) + ' ' + evento).
    RUN pi-gera-json.
    
    ASSIGN c-jason = JsonAPIUtils:getJsonArrayChar(arrayBenef).
    
    
    FIND LAST bf-las-api-log WHERE bf-las-api-log.id-api-log > 0        NO-LOCK NO-ERROR.
    
    IF evento = 'PUT' THEN
        FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'integraBeneCanal' NO-LOCK NO-ERROR.
    ELSE FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'deleteBeneCanal' NO-LOCK NO-ERROR.
    ...
    ASSIGN es-api-log.seqexec        = IF AVAIL bf-las-api-log THEN bf-las-api-log.seqexec  +  10 ELSE 10  
           ...
           es-api-log.id-codigo      = IF AVAIL tt-benef THEN string(tt-benef.externalID) ELSE '0'
           es-api-log.id-URI         = IF evento = 'PUT' THEN 'integraBeneCanal' ELSE 'deleteBeneCanal'
           es-api-log.dh-request     = NOW
           es-api-log.end-envio      = c-endereco
           es-api-log.flg-processado = NO
           es-api-log.Origem         = 'TOTVS'
           es-api-log.aux            = IF AVAIL tt-benef THEN string(tt-benef.externalId) ELSE '0'
