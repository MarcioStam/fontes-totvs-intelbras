/*****************************************************************************************
 INTEGRAۂO DE PORTIFOLIO COM O CRM
******************************************************************************************/ 
...
def input-output PARAM table     for tt-item.
...
FIND FIRST tt-prog-ponto 
  WHERE ENTRY(1,tt-prog-ponto.conteudo,";") = 'eswso0012' NO-ERROR.
IF AVAILABLE tt-prog-ponto AND
...
IF l-log = YES THEN DO:
    ...
    IF OPSYS = 'UNIX' THEN
       ASSIGN c-arquivo-log1 = '/mnt/spool/totvs/UNIX_eswso0012'.
    ELSE
       ASSIGN c-arquivo-log1 = '\\erpapp\spool\totvs\WIN_eswso0012'.
...
OR SEARCH("esp/wso/out/wso0012.p") <> ? 
   AND evento = 'POST'                  THEN DO:
    run esp/wso/out/wso0012.p  (INPUT  row-table,
                                input-output table tt-item).
END.

FOR EACH tt-item:

    RUN pi-gerar-dados-extrato (INPUT 'Exportando Portifolio Grcli: ' + STRING(tt-item.cod-gr-cli) +
                                 ',' + tt-item.it-codigo + ',' + tt-item.cod-unid-neg + ',' + tt-item.segmento + ',' + 
                                 tt-item.familia + ',' + evento).
    ...
    IF evento = 'POST' THEN
        FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'integraPortifCRM' NO-LOCK NO-ERROR.
    ELSE 
        FIND FIRST es-api-URI    WHERE es-api-URI.id-URI = 'deletePortifCRM' NO-LOCK NO-ERROR.
    ...
    ASSIGN es-api-log.seqexec        = IF AVAIL bf-las-api-log THEN bf-las-api-log.seqexec  +  10 ELSE 10  
           ...
           es-api-log.id-codigo      = IF AVAIL tt-item THEN string(tt-item.cod-gr-cli) ELSE '0'
           es-api-log.id-URI         = IF evento = 'POST' THEN 'integraPortifCRM' ELSE 'deletePortifCRM'
           es-api-log.dh-request     = NOW
           es-api-log.end-envio      = c-endereco
           es-api-log.flg-processado = NO
           es-api-log.Origem         = 'Totvs'
           es-api-log.aux            = IF AVAIL tt-item THEN string(tt-item.cod-gr-cli) ELSE '0'
