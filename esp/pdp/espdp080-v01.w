...
*******************************************************************************/
{include/i-prgvrs.i espdp080-V01 1.00.00.001}
{upc\btb910za-upc.i}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i espdp080-v01 MUT}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-key rt-mold bt-historico 
...
reservas-ast.qt-reserva reservas-ast.dt-reserva ~
...
reservas-ast.cd-usuario reservas-ast.dt-reserva ~
reservas-ast.dt-reserva-altera reservas-ast.cd-usuario-altera 
&Scoped-define ADM-MODIFY-FIELDS reservas-ast.cd-usuario ~
reservas-ast.dt-reserva reservas-ast.dt-reserva-altera ~
...
DEFINE FRAME f-main
     reservas-ast.cod-estabel AT ROW 1.17 COL 20.57 COLON-ALIGNED WIDGET-ID 8
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .88
     c-desc-estab AT ROW 1.17 COL 27.72 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     reservas-ast.cod-depos AT ROW 2.08 COL 20.57 COLON-ALIGNED WIDGET-ID 6
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .88
     c-desc-deposito AT ROW 2.08 COL 27.72 COLON-ALIGNED NO-LABEL WIDGET-ID 20
     reservas-ast.it-codigo AT ROW 3 COL 20.57 COLON-ALIGNED WIDGET-ID 14
          VIEW-AS FILL-IN 
          SIZE 17.14 BY .88
     c-desc-item AT ROW 3 COL 37.72 COLON-ALIGNED NO-LABEL WIDGET-ID 22
     reservas-ast.cd-usuario AT ROW 3.92 COL 20.57 COLON-ALIGNED WIDGET-ID 2
          LABEL "Usu†rio"
          VIEW-AS FILL-IN 
          SIZE 17.14 BY .88
     reservas-ast.qt-reserva AT ROW 5.71 COL 20.57 COLON-ALIGNED WIDGET-ID 16
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     reservas-ast.dt-reserva AT ROW 5.71 COL 62.86 COLON-ALIGNED WIDGET-ID 10
          LABEL "Dt Reserva"
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     reservas-ast.dt-reserva-altera AT ROW 6.63 COL 20.57 COLON-ALIGNED WIDGET-ID 12
          LABEL "Dt Res Alterada"
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     reservas-ast.cd-usuario-altera AT ROW 6.63 COL 62.86 COLON-ALIGNED WIDGET-ID 4
          LABEL "Usu†rio Altera"
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     reservas-ast.data-limite AT ROW 7.54 COL 20.57 COLON-ALIGNED WIDGET-ID 24
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     bt-historico AT ROW 7.54 COL 64.86 WIDGET-ID 26
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-historico V-table-Win
ON CHOOSE OF bt-historico IN FRAME f-main /* Hist¢rico */
DO:
    DO WITH FRAME f-main:
        RUN esp/pdp/espdp080b.w(INPUT reservas-ast.it-codigo:SCREEN-VALUE,
                                INPUT reservas-ast.cod-estabel:SCREEN-VALUE,
                                INPUT reservas-ast.cod-depos:SCREEN-VALUE,
                                INPUT reservas-ast.cd-usuario:SCREEN-VALUE).
    END.
END.
...
ON ENTRY OF reservas-ast.cod-estabel IN FRAME f-main /* Estabelecimento */
DO:
    RUN piAtribuiUsuar.  
END.
...
DO:
    ...
    THEN ASSIGN reservas-ast.dt-reserva       :SCREEN-VALUE IN FRAME f-main = string(TODAY)
    ...
    ELSE DO:
        IF  reservas-ast.cd-usuario:SCREEN-VALUE IN FRAME f-main NE ""
        AND reservas-ast.cd-usuario:SCREEN-VALUE IN FRAME f-main NE c-seg-usuario THEN DO:
            RUN utp/ut-msgs.p (INPUT "show":U,
                               INPUT 17006,
                               INPUT "Procedimento interrompido." + '~~' + "N∆o Ç permitida qualquer aá∆o em registro que n∆o seja de seu usu†rio.").
            return NO-APPLY.
        END.
...
------------------------------------------------------------------------------*/
DEFINE BUFFER b-reservas-ast FOR reservas-ast.
...
IF c-seg-usuario <> reservas-ast.cd-usuario:SCREEN-VALUE THEN DO:
    {include/i-vldprg.i}
    RUN utp/ut-msgs.p (INPUT "SHOW":U,
                       INPUT 17006,
                       INPUT "Usu†rio inv†lido~~N∆o Ç permitida qualquer aá∆o em registro que n∆o seja de seu usu†rio.").
    RETURN 'ADM-ERROR':U.
END.
...
IF adm-new-record THEN DO:
    IF CAN-FIND(FIRST b-reservas-ast
                WHERE b-reservas-ast.cod-estabel = reservas-ast.cod-estabel:SCREEN-VALUE
                AND   b-reservas-ast.cod-depos   = reservas-ast.cod-depos:SCREEN-VALUE
                AND   b-reservas-ast.it-codigo   = reservas-ast.it-codigo:SCREEN-VALUE
                AND   b-reservas-ast.cd-usuario  = reservas-ast.cd-usuario:SCREEN-VALUE) THEN DO:

        {include/i-vldprg.i}
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "Reserva inv†lida~~J† existe reserva cadastrada com os dados informados.").
        RETURN 'ADM-ERROR':U.
    END.

    IF  NOT CAN-FIND(FIRST item 
                     WHERE item.it-codigo = reservas-ast.it-codigo:SCREEN-VALUE) THEN DO:

        {include/i-vldprg.i}
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "Item inv†lido~~Item " + reservas-ast.it-codigo:SCREEN-VALUE + " n∆o encontrado.").
        RETURN 'ADM-ERROR':U.
    END.

    IF  NOT CAN-FIND(FIRST estabelec 
                     WHERE estabelec.cod-estabel = reservas-ast.cod-estabel:SCREEN-VALUE) THEN DO:

        {include/i-vldprg.i}
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "Estabelecimento inv†lido~~Estabelecimento " + reservas-ast.cod-estabel:SCREEN-VALUE + " n∆o encontrado.").
        RETURN 'ADM-ERROR':U.
    END.
    
    IF  NOT CAN-FIND(FIRST deposito
                     WHERE deposito.cod-depos = reservas-ast.cod-depos:SCREEN-VALUE) THEN DO:

        {include/i-vldprg.i}
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "Dep¢sito inv†lido~~Dep¢sito " + reservas-ast.cod-depos:SCREEN-VALUE + " n∆o encontrado.").
        RETURN 'ADM-ERROR':U.
    END.

    IF  NOT CAN-FIND(FIRST usuar_mestre
                     WHERE usuar_mestre.cod_usuario = reservas-ast.cd-usuario:SCREEN-VALUE) THEN DO:

        {include/i-vldprg.i}
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "Usu†rio inv†lido~~Usu†rio " + reservas-ast.cd-usuario:SCREEN-VALUE + " n∆o encontrado.").
        RETURN 'ADM-ERROR':U.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piAtribuiUsuar V-table-Win 
PROCEDURE piAtribuiUsuar :
/*------------------------------------------------------------------------------
  Purpose:     
  Notes:       
------------------------------------------------------------------------------*/
IF adm-new-record THEN
    ASSIGN reservas-ast.cd-usuario:SCREEN-VALUE IN FRAME f-main = c-seg-usuario
           reservas-ast.cd-usuario:SENSITIVE                    = NO
           reservas-ast.dt-reserva:SCREEN-VALUE                 = STRING(TODAY).

ASSIGN reservas-ast.dt-reserva-altera:SCREEN-VALUE = STRING(TODAY)
       reservas-ast.cd-usuario-altera:SCREEN-VALUE = c-seg-usuario.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piValidaSaldo V-table-Win 
...
IF l-mail THEN DO:
    ASSIGN cMensagem = cMensagem + '~nReserva Gerada: Estab: ' + reservas-ast.cod-estabel:SCREEN-VALUE IN FRAME f-main  + 
                                   ...
                                   ', ITEM: ' + reservas-ast.it-codigo:SCREEN-VALUE IN FRAME f-main                     +
                                   ', Usu†rio: ' + reservas-ast.cd-usuario:SCREEN-VALUE IN FRAME f-main                 + '~n'.
