...
**  Descricao.: Integra‡Æo WS Categoria Produto - Ikeda
-----------------------------------------------------------------------*/
...
define variable hProdutoCategoriaSoap  as handle   no-undo.
...
procedure conecta:
   ...
   hWebService:connect("-WSDL '" + param-b2c.url-webservices + "/ProdutoCategoria.asmx?WSDL'") no-error.
   if (hWebService:connected()) then do:
      run ProdutoCategoriaSoap set hProdutoCategoriaSoap on hWebService no-error.
    
      if (error-status:error) then
         assign iStatus = 98
                cStatus = "Erro ao carregar o PortType ProdutoCategoriaSoap no Web Service Ikeda.".
      else
         assign iStatus = 1.
   end.
   else
...
procedure incluirCodigoInterno:
   ...
   if (valid-handle(hWebService)) and (hWebService:connected()) and valid-handle(hProdutoCategoriaSoap) then do:
      run IncluirCodigoInterno in hProdutoCategoriaSoap(input 0 /** LojaCodigo **/,
      ...
      if (error-status:error) then
         assign iStatus = 97
                cStatus = "Erro ao carregar o Metodo IncluirCodigoInterno no Web Service Ikeda. " + error-status:get-message(1).
      else do:
