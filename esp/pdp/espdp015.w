...
&GLOBAL-DEFINE Program        ESPDP015
...
&GLOBAL-DEFINE page2Fields    fi-Cod-Emitente-ini fi-Cod-Emitente-fim radio-set-1 fi-cod-grc-ini fi-cod-grc-fim
...
DEFINE VARIABLE fi-Cod-grc-fim AS INTEGER FORMAT ">9":U INITIAL 99 
     VIEW-AS FILL-IN 
     SIZE 12 BY .79 NO-UNDO.

DEFINE VARIABLE fi-Cod-grc-ini AS INTEGER FORMAT ">9":U INITIAL 0 
...
DEFINE FRAME fPage2
     RADIO-SET-1 AT ROW 1.75 COL 3 NO-LABEL
     fi-Cod-Emitente-ini AT ROW 3.46 COL 17 COLON-ALIGNED
     fi-Cod-Emitente-fim AT ROW 3.46 COL 37 COLON-ALIGNED NO-LABEL
     fi-Cod-grc-ini AT ROW 4.46 COL 17 COLON-ALIGNED
     fi-Cod-grc-fim AT ROW 4.46 COL 37 COLON-ALIGNED NO-LABEL
     "At‚" VIEW-AS TEXT
          SIZE 4 BY .54 AT ROW 3.46 COL 33.86
     "At‚" VIEW-AS TEXT
          SIZE 4 BY .54 AT ROW 4.46 COL 34
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.75
         SIZE 80.43 BY 7
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-Cod-Emitente-ini wReport
ON MOUSE-SELECT-DBLCLICK OF fi-Cod-Emitente-ini IN FRAME fPage2 /* Emitente */
DO:
...
DO ON ERROR UNDO, RETURN ERROR ON STOP UNDO, RETURN ERROR:
    ...
    DO WITH FRAME fPage2:
       ASSIGN tt-param.Cod-Emitente-ini  = INT(INPUT fi-Cod-Emitente-ini:SCREEN-VALUE IN FRAME fpage2)
              tt-param.Cod-Emitente-fim  = INT(INPUT fi-Cod-Emitente-fim:SCREEN-VALUE IN FRAME fpage2)
              tt-param.Cod-Grc-Ini       = INT(INPUT fi-Cod-Grc-ini:SCREEN-VALUE IN FRAME fpage2)
              tt-param.Cod-Grc-Fim       = INT(INPUT fi-Cod-Grc-Fim:SCREEN-VALUE IN FRAME fpage2)
              tt-param.Ident             = INT(INPUT radio-set-1:SCREEN-VALUE IN FRAME fpage2).
    END.
    ...
    {report/rprun.i esp\pdp\espdp015rp.p}
