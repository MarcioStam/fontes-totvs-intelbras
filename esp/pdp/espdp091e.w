...
*******************************************************************************/
{include/i-prgvrs.i espdp091e 1.12.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i espdp091e mft}
&ENDIF

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program        espdp091e
...
&GLOBAL-DEFINE page1Widgets   fi-Estabel-ini          fi-Estabel-fim          fi-Atendente-ini        ~
                              fi-Atendente-fim        fi-Atendente-mestre-ini fi-Atendente-mestre-fim ~
                              fi-Cod-repres-ini       fi-Cod-Repres-fim       it-codigo               fi-dt-entrega-ini       ~
                              fi-dt-entrega-fim       fi-nr-pedcli-ini        fi-nr-pedcli-fim        ~
                              fi-dt-implant-ini       fi-dt-implant-fim       fi-cond-pagto-ini       fi-Cond-pagto-fim       ~
                              fi-prioridade-ini       fi-prioridade-fim       fi-cod-emitente-ini     fi-Cod-emitente-fim     tg-entregaFutura ~
                              fi-estado-ini           fi-estado-fim ~
...
DEFINE INPUT-OUTPUT PARAM pestado-ini     LIKE ped-venda.estado.
DEFINE INPUT-OUTPUT PARAM pestado-fim     LIKE ped-venda.estado.
...
DEFINE VARIABLE fi-Estado-fim AS CHARACTER FORMAT "X(4)":U INITIAL "ZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.

DEFINE VARIABLE fi-Estado-ini AS CHARACTER FORMAT "X(4)":U 
     LABEL "Estado" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.
...
DEFINE FRAME fPage1
     ...
     fi-Atendente-ini AT ROW 2.46 COL 17.43 COLON-ALIGNED WIDGET-ID 2
     fi-Atendente-fim AT ROW 2.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 44
     fi-Atendente-mestre-ini AT ROW 3.46 COL 17.43 COLON-ALIGNED WIDGET-ID 56
     fi-Atendente-mestre-fim AT ROW 3.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 54
     tg-entregaFutura AT ROW 3.75 COL 64 WIDGET-ID 94
     fi-Cod-repres-ini AT ROW 4.46 COL 17.43 COLON-ALIGNED WIDGET-ID 80
     fi-Cod-Repres-fim AT ROW 4.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 78
     it-codigo AT ROW 4.75 COL 62 COLON-ALIGNED WIDGET-ID 96
     fi-dt-entrega-ini AT ROW 5.46 COL 17.43 COLON-ALIGNED WIDGET-ID 84
     fi-dt-entrega-fim AT ROW 5.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 82
     fi-nr-pedcli-ini AT ROW 6.46 COL 17.43 COLON-ALIGNED WIDGET-ID 92
     fi-nr-pedcli-fim AT ROW 6.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 90
     fi-dt-implant-ini AT ROW 7.46 COL 17.43 COLON-ALIGNED WIDGET-ID 88
     fi-dt-implant-fim AT ROW 7.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 86
     fi-cond-pagto-ini AT ROW 8.46 COL 17.43 COLON-ALIGNED WIDGET-ID 22
     fi-Cond-pagto-fim AT ROW 8.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 20
     fi-prioridade-ini AT ROW 9.46 COL 17.43 COLON-ALIGNED WIDGET-ID 30
     fi-prioridade-fim AT ROW 9.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 28
     fi-cod-emitente-ini AT ROW 10.46 COL 17.43 COLON-ALIGNED HELP
          "C¢digo do emitente" WIDGET-ID 38
     fi-cod-emitente-fim AT ROW 10.46 COL 41.43 COLON-ALIGNED HELP
          "C¢digo do emitente" NO-LABEL WIDGET-ID 36
     fi-Estado-ini AT ROW 11.46 COL 17.43 COLON-ALIGNED WIDGET-ID 102
     fi-Estado-fim AT ROW 11.46 COL 41.43 COLON-ALIGNED NO-LABEL WIDGET-ID 100
     ...
     IMAGE-25 AT ROW 11.5 COL 38.43 WIDGET-ID 104
     IMAGE-26 AT ROW 11.5 COL 34.43 WIDGET-ID 106
...
/*            pCodSitAval    = INPUT FRAME fPage1 i-sit-cred */
           ...
           pestado-ini    = INPUT FRAME fPage1 fi-estado-ini
           pestado-fim    = INPUT FRAME fPage1 fi-estado-fim.
...
&Scoped-define SELF-NAME fi-Estado-fim
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-Estado-fim wWindow
ON MOUSE-SELECT-DBLCLICK OF fi-Estado-fim IN FRAME fPage1
DO:
...
DO:
    ...
    IF NOT AVAIL ITEM THEN DO:
        run utp/ut-msgs.p (input "show":U, input 17091, input "Item Inv lido!~~Item informado nÆo existe.":U).
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME fPage1:
        ...
        ASSIGN fi-estado-ini          :SCREEN-VALUE = STRING(pestado-ini   )
               fi-estado-fim          :SCREEN-VALUE = STRING(pestado-fim   ).
