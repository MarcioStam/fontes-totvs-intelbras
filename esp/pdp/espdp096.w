...
*******************************************************************************/
{include/i-prgvrs.i espdp096 2.00.00.000}
...
DEFINE VARIABLE EDITOR-1 AS CHARACTER INITIAL "Integra‡Æo tabelas de pre‡os WSO2" 
     VIEW-AS EDITOR
...
DEFINE VARIABLE fi-tab-fim AS CHARACTER FORMAT "x(8)" INITIAL "ZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 9.57 BY .88 NO-UNDO.

DEFINE VARIABLE fi-tab-ini AS CHARACTER FORMAT "x(8)" 
     LABEL "Tab. Pre‡o":R10 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-pg-sel
     ...
     fi-tab-ini AT ROW 4 COL 15.57 COLON-ALIGNED WIDGET-ID 18
     fi-tab-fim AT ROW 4 COL 39 COLON-ALIGNED NO-LABEL WIDGET-ID 20
     fi-item-ini AT ROW 5 COL 8 COLON-ALIGNED WIDGET-ID 14
     fi-item-fim AT ROW 5 COL 39 COLON-ALIGNED NO-LABEL WIDGET-ID 22
     IMAGE-1 AT ROW 5.08 COL 28 WIDGET-ID 6
     IMAGE-2 AT ROW 5.08 COL 37.72 WIDGET-ID 8
     IMAGE-5 AT ROW 4 COL 28 WIDGET-ID 10
     IMAGE-6 AT ROW 4 COL 37.72 WIDGET-ID 12
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
ON MOUSE-SELECT-DBLCLICK OF fi-item-fim IN FRAME f-pg-sel
DO:
  apply "F5":U to self.
END.
...
ON MOUSE-SELECT-DBLCLICK OF fi-item-ini IN FRAME f-pg-sel /* Item */
DO:
  apply "F5":U to self.
END.
...
&Scoped-define SELF-NAME fi-tab-fim
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-tab-fim w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-tab-fim IN FRAME f-pg-sel
DO:
  apply "F5":U to self.
END.
...
&Scoped-define SELF-NAME fi-tab-ini
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-tab-ini w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-tab-ini IN FRAME f-pg-sel /* Tab. Pre‡o */
DO:
  apply "F5":U to self.
END.
...
{utp/ut9000.i "espdp096" "2.00.00.000"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY EDITOR-1 fi-tab-ini fi-tab-fim fi-item-ini fi-item-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-5 IMAGE-6 fi-tab-ini fi-tab-fim fi-item-ini 
         fi-item-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF INPUT FRAME f-pg-sel fi-tab-ini > INPUT FRAME f-pg-sel fi-tab-fim THEN DO:
       RUN utp/ut-msgs.p (INPUT "show":U, 
                          INPUT 17006, 
                          INPUT "Tabela de pre‡o inicial deve ser menor ou igual a final!").
       APPLY "ENTRY" TO fi-tab-ini IN FRAME f-pg-sel.
       RETURN ERROR.
    END.

    IF INPUT FRAME f-pg-sel fi-item-ini > INPUT FRAME f-pg-sel fi-item-fim THEN DO:
       RUN utp/ut-msgs.p (INPUT "show":U, 
                          INPUT 17006, 
                          INPUT "Item inicial deve ser menor ou igual ao final!").
       APPLY "ENTRY" TO fi-item-ini IN FRAME f-pg-sel.
       RETURN ERROR.
    END.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.fi-tab-ini      = INPUT FRAME f-pg-sel fi-tab-ini
           tt-param.fi-tab-fim      = INPUT FRAME f-pg-sel fi-tab-fim
           tt-param.fi-item-ini     = INPUT FRAME f-pg-sel fi-item-ini
           tt-param.fi-item-fim     = INPUT FRAME f-pg-sel fi-item-fim
           .
    ...
    {include/i-rprun.i esp/pdp/espdp096rp.p}
