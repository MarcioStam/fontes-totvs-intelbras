    ...
    IF qtd-total-disponivel >= p-qtd-alocar THEN DO:
        ...
        FOR EACH saldo-estoq NO-LOCK
           WHERE saldo-estoq.cod-estabel = ped-venda.cod-estabel
            ...
            IF de-saldo = 0 THEN NEXT. /* so aloca lote com saldo*/
...
/*     END.                                                                                                                                                                                                                                 */
    ...
    */
    ...
    IF p-localizacao <> "" THEN DO:
        ...
        END.
        /* IDBA BRUNO Joaquim -> M2403-017 Trava faturamento dep¢sito EXP
        IF p-cod-depos = "EXP" 
        OR p-cod-depos = "WEX"
    ...
    AND p-qtd-alocar MOD ITEM.lote-mulven <> 0 THEN DO:
        ...
        IF LOOKUP(ped-venda.tp-pedido,c-atendente) = 0 THEN DO: //nao valida lote multiplo de venda nas lojas
            RUN pi-cria-erro (INPUT 17006,
