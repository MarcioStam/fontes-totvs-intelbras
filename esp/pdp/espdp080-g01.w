...
*******************************************************************************/
{include/i-prgvrs.i espdp080-G01 1.00.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod-estabel c_cod-depos ~
c_it-codigo c_cd-usuario bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod-estabel c_cod-depos c_it-codigo ~
c_cd-usuario 
...
DEFINE VARIABLE c_cd-usuario AS CHARACTER FORMAT "x(16)":U 
     LABEL "Usu rio" 
     VIEW-AS FILL-IN 
     SIZE 16 BY .88 NO-UNDO.

DEFINE VARIABLE c_cod-depos AS CHARACTER FORMAT "x(3)":U 
     LABEL "Dep¢sito" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_cod-estabel AT ROW 1.17 COL 30 COLON-ALIGNED WIDGET-ID 2
     c_cod-depos AT ROW 2.08 COL 30 COLON-ALIGNED WIDGET-ID 4
     c_it-codigo AT ROW 3 COL 30 COLON-ALIGNED WIDGET-ID 6
     c_cd-usuario AT ROW 3.92 COL 30 COLON-ALIGNED WIDGET-ID 8
     bt-ok AT ROW 5.13 COL 2.14
     bt-cancela AT ROW 5.13 COL 13
     bt-ajuda AT ROW 5.13 COL 58
     rt-button AT ROW 4.88 COL 1
...
DO:
FIND reservas-ast NO-LOCK
    WHERE reservas-ast.cod-estabel EQ INPUT FRAME {&FRAME-NAME} c_cod-estabel
      AND reservas-ast.cod-depos  EQ INPUT FRAME {&FRAME-NAME} c_cod-depos
      AND reservas-ast.it-codigo  EQ INPUT FRAME {&FRAME-NAME} c_it-codigo
      AND reservas-ast.cd-usuario EQ INPUT FRAME {&FRAME-NAME} c_cd-usuario NO-ERROR.

IF NOT AVAILABLE reservas-ast THEN DO:
    {utp/ut-table.i mgesp reservas-ast 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(reservas-ast).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod-estabel c_cod-depos c_it-codigo c_cd-usuario 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod-estabel c_cod-depos c_it-codigo c_cd-usuario bt-ok 
         bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "espdp080-G01" "1.00.00.001"}
