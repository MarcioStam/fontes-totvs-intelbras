...
/* ** Definiá‰es para conectar os bancos externos ***/
...
&Scoped-Define ENABLED-OBJECTS i-estab rs-destino bt-cfimp bt-arquivo ~
c-arquivo rs-execucao bt-imprime bt-salva RECT-2 RECT-29 RECT-7 RECT-8 
&Scoped-Define DISPLAYED-OBJECTS i-estab rs-destino c-arquivo rs-execucao 
...
DEFINE VARIABLE c-arquivo AS CHARACTER FORMAT "X(40)":U INITIAL "escfl001.lst" 
     VIEW-AS FILL-IN 
     SIZE 44.29 BY .88 TOOLTIP "Destino"
     BGCOLOR 15 FONT 1 NO-UNDO.

DEFINE VARIABLE i-estab AS INTEGER FORMAT "999":U INITIAL 0 
     LABEL "Estabelecimento" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-relat
     i-estab AT ROW 2 COL 17 COLON-ALIGNED WIDGET-ID 2
     v_des_warning
         at row 3.5 col 1.00 colon-aligned NO-LABEL
         help "Aviso"
         view-as editor max-chars 2000 scrollbar-vertical
         size 60 by 5
         bgcolor 15 font 2
     rs-destino AT ROW 10.71 COL 2 HELP
          "Destino da Impress∆o" NO-LABEL
     bt-cfimp AT ROW 11.71 COL 46.57 HELP
          "Layout Impress∆o"
     bt-arquivo AT ROW 11.71 COL 46.57 HELP
          "Localiza Arquivo"
     c-arquivo AT ROW 11.79 COL 2.14 HELP
          "Destino" NO-LABEL
     rs-execucao AT ROW 10.71 COL 54.14 HELP
          "Execuá∆o On Line ou Batch" NO-LABEL
     bt-imprime AT ROW 13.67 COL 2.29
     bt-salva AT ROW 13.67 COL 14
     "Impress∆o do Relat¢rio de Conferància" VIEW-AS TEXT
          SIZE 32.14 BY .54 AT ROW 9.75 COL 2.86
          FONT 6
     " Execuá∆o" VIEW-AS TEXT
          SIZE 9.43 BY .54 AT ROW 9.79 COL 53.86
          FONT 6
     "Seleá∆o para a Geraá∆o" VIEW-AS TEXT
          SIZE 20.57 BY .54 AT ROW 1.08 COL 3.43
          FONT 6
     RECT-2 AT ROW 13.38 COL 1.29
     RECT-29 AT ROW 10 COL 53
     RECT-7 AT ROW 10 COL 1
...
assign v_des_warning:return-inserted in frame f-relat = yes.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Geraá∆o Fluxo de Caixa - Impto/Desp/Antecip - CCP - escfl001"
         COLUMN             = 29.57
         ROW                = 7.92
         HEIGHT             = 14.08
         WIDTH              = 64.29
...
ON END-ERROR OF C-Win /* Geraá∆o Fluxo de Caixa - Impto/Desp/Antecip - CCP - escfl001 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Geraá∆o Fluxo de Caixa - Impto/Desp/Antecip - CCP - escfl001 */
DO:
...
DO:
  ...
  DO.
    RUN prgtec/btb/btb911za.p (INPUT  "escfl001rp",
                               INPUT  "1.00.000",
                               INPUT  0,
...
DO:
  ...
  if input frame f-relat rs-destino = 2 then do:
      ...
      if input frame f-relat rs-execucao = 1 then
          assign c-arquivo:screen-value in frame f-relat = session:temp-directory + "escfl001.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
      else
          assign c-arquivo:screen-value in frame f-relat = "escfl001.lst"
                 c-impressora                          = ""
                 c-layout                              = "".
...
assign i-estab = int(v_cod_estab_usuar).
...
assign v_des_warning:screen-value = "   Executar preferencialmente em modo BATCH (via RPW)." + chr(10) +
        "" + chr(10) +
        "   Ser† criado no diret¢rio " + c-dir-saida + " um arquivo .csv para ser importado no excel contendo a origem de todos os valores gerados." + chr(10) +
        "" + chr(10) +
        "   O nome do arquivo ser† fluxo+estab+-dia+mes+ano, exemplo, " + c-dir-saida + "fluxo101-01062008.csv.".
...
------------------------------------------------------------------------------*/
  DISPLAY i-estab rs-destino c-arquivo rs-execucao 
      WITH FRAME f-relat IN WINDOW C-Win.
  ENABLE i-estab v_des_warning rs-destino bt-cfimp bt-arquivo c-arquivo rs-execucao 
         bt-imprime bt-salva RECT-2 RECT-29 RECT-7 RECT-8 
      WITH FRAME f-relat IN WINDOW C-Win.
  assign v_des_warning:read-only in frame f-relat = yes.
    ...
    ASSIGN rs-execucao:SCREEN-VALUE   IN FRAME f-relat = entry(1,dwb_set_list_param.cod_dwb_parameters,chr(10))
           rs-destino:SCREEN-VALUE    IN FRAME f-relat = IF dwb_set_list_param.cod_dwb_output = "impressora" 
                                                               ...
                                                               ELSE IF dwb_set_list_param.cod_dwb_output = "arquivo" 
                                                                    THEN "2"
                                                                    ELSE "3".
    if num-entries(dwb_set_list_param.cod_dwb_parameters,chr(10)) >= 4
    then do:
         ASSIGN i-estab = int(entry(4,dwb_set_list_param.cod_dwb_parameters,chr(10))).
         DISP i-estab WITH FRAME f-relat.
    end.
...
do:
  for each ped_exec no-lock
      where ped_exec.cod_prog_dtsul = "escfl001rp"
...
FIND prog_dtsul NO-LOCK
    WHERE prog_dtsul.cod_prog_dtsul = "escfl001" NO-ERROR.
...
DO:
  FOR EACH usuar_grp_usuar NO-LOCK
    ...
    IF NOT CAN-FIND(FIRST prog_dtsul_segur NO-LOCK
                    WHERE  prog_dtsul_segur.cod_prog_dtsul = "escfl001"
...
/* */
  ...
  ASSIGN dwb_set_list_param.Cod_dwb_program          = "escfl001rp"
