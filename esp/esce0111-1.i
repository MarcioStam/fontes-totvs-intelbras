...
FOR EACH req-ord EXCLUSIVE-LOCK
   WHERE req-ord.it-codigo = ITEM.it-codigo:

    FIND FIRST breq-ord EXCLUSIVE-LOCK
         WHERE breq-ord.nr-ord-produ = req-ord.nr-ord-produ 
           AND breq-ord.item-pai     = req-ord.item-pai
           AND breq-ord.cod-roteiro  = req-ord.cod-roteiro
           AND breq-ord.op-codigo    = req-ord.op-codigo
           AND breq-ord.it-codigo    = req-ord.it-codigo
           AND breq-ord.cod-refer    = {2}
           AND breq-ord.cod-estabel  = req-ord.cod-estabel
           AND breq-ord.cod-depos    = req-ord.cod-depos 
           AND breq-ord.cod-localiz  = req-ord.cod-localiz
           AND breq-ord.lote-serie   = {1} NO-ERROR.

    IF NOT AVAILABLE breq-ord THEN DO:

        ASSIGN req-ord.lote-serie = {1}
               req-ord.cod-refer  = {2}.
    END.
    ELSE DO:

        IF ROWID(breq-ord) <> ROWID(req-ord) THEN DO:

            ASSIGN breq-ord.qt-requis          = breq-ord.qt-requis          + req-ord.qt-requis
                   breq-ord.qtd-requisitd-lote = breq-ord.qtd-requisitd-lote + req-ord.qtd-requisitd-lote.
...
/*Atualizar a pl-it-calc para replaneja sempre que houver alguma altera‡Æo na produ‡Æo*/
IF param-global.modulo-pl OR param-global.modulo-pi THEN
    RUN cdp/cd7000.p(INPUT 2,
...
{utp/ut-liter.i Atualizando_Saldos... mce R}

RUN pi-acompanhar in h-acomp (INPUT TRIM(RETURN-VALUE)).

FOR EACH saldo-estoq USE-INDEX item EXCLUSIVE-LOCK
   WHERE saldo-estoq.it-codigo = item.it-codigo:

    FOR EACH b-saldo-est USE-INDEX estabel-dep NO-LOCK
       WHERE b-saldo-est.cod-estabel = saldo-estoq.cod-estabel
         AND b-saldo-est.cod-depos   = saldo-estoq.cod-depos
         AND b-saldo-est.cod-localiz = saldo-estoq.cod-localiz
         AND b-saldo-est.it-codigo   = saldo-estoq.it-codigo
         AND ROWID(b-saldo-est)     <> ROWID(saldo-estoq):

        ASSIGN saldo-estoq.qtidade-atu = saldo-estoq.qtidade-atu
                                       + b-saldo-est.qtidade-atu
               saldo-estoq.qtidade-ini = saldo-estoq.qtidade-ini
                                       + b-saldo-est.qtidade-ini
               saldo-estoq.qtidade-fin = saldo-estoq.qtidade-fin
                                       + b-saldo-est.qtidade-fin
               saldo-estoq.qt-alocada = saldo-estoq.qt-alocada
                                      + b-saldo-est.qt-alocada
               saldo-estoq.qt-aloc-prod = saldo-estoq.qt-aloc-prod
                                        + b-saldo-est.qt-aloc-prod.

        IF b-saldo-est.data-ult-ent > saldo-estoq.data-ult-ent THEN
            ASSIGN saldo-estoq.data-ult-ent = b-saldo-est.data-ult-ent.

        IF b-saldo-est.dt-ul-contag > saldo-estoq.dt-ul-contag THEN
            ASSIGN saldo-estoq.dt-ul-contag = b-saldo-est.dt-ul-contag.

        FIND FIRST b2-saldo-est
             WHERE ROWID(b2-saldo-est) = ROWID(b-saldo-est) EXCLUSIVE-LOCK.

        DELETE b2-saldo-est VALIDATE(TRUE,"").
    END.
   
    ASSIGN saldo-estoq.lote         = {1}
           saldo-estoq.cod-refer    = {2}
           saldo-estoq.dt-vali-lote = {3}.
END.
