...
&Scoped-Define ENABLED-OBJECTS rt-button i_cd-servico bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cd-servico 
...
DEFINE VARIABLE i_cd-servico AS INTEGER FORMAT ">>>>>>>>9":U 
     LABEL "Cd. Servi‡o" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cd-servico AT ROW 2 COL 24 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND int-codigo-servico NO-LOCK
    WHERE int-codigo-servico.cd-servico EQ INPUT FRAME {&FRAME-NAME} i_cd-servico NO-ERROR.

IF NOT AVAILABLE int-codigo-servico THEN DO:
    {utp/ut-table.i mgesp int-codigo-servico 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-codigo-servico).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cd-servico 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cd-servico bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
