...
*******************************************************************************/
{include/i-prgvrs.i ESREP045-G01 1.00.00.001}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESREP045-G01 MUT}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-atividade bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-atividade 
...
DEFINE VARIABLE i_cod-atividade AS INTEGER FORMAT ">>9":U 
     LABEL "Cod.Atividade MEI" 
     VIEW-AS FILL-IN 
     SIZE 5 BY 1 NO-UNDO.
...
DEFINE FRAME d-vapara
     i_cod-atividade AT ROW 2.04 COL 26 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 43.72
...
DO:
FIND int-atividade-mei NO-LOCK
    WHERE int-atividade-mei.cod-atividade EQ INPUT FRAME {&FRAME-NAME} i_cod-atividade NO-ERROR.

IF NOT AVAILABLE int-atividade-mei THEN DO:
    {utp/ut-table.i mgesp int-atividade-mei 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(int-atividade-mei).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-atividade 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-atividade bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESREP045-G01" "1.00.00.001"}
