...
&GLOBAL-DEFINE Program        ESREP006
...
&GLOBAL-DEFINE page0Widgets   btOk ~
                              btCancel ~
                              btHelp2
&GLOBAL-DEFINE page2Widgets   fi-fim-data fi-ini-data fi-ini-estabel fi-fim-estabel ~
                              fi-cod-msg-devolucao-ini fi-cod-msg-devolucao-fim
...
DEFINE VARIABLE fi-fim-estabel AS INTEGER FORMAT ">>>" INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 5 BY .88 NO-UNDO.
...
DEFINE VARIABLE fi-ini-estabel AS INTEGER FORMAT ">>>" INITIAL 0 
...
DEFINE FRAME fPage4
     rsTipoImpressao AT ROW 3 COL 5 NO-LABEL WIDGET-ID 4
     "Tipo relat¢rio" VIEW-AS TEXT
          SIZE 10 BY .54 AT ROW 1.75 COL 5 WIDGET-ID 8
     RECT-10 AT ROW 2 COL 3 WIDGET-ID 2
...
DEFINE FRAME fPage2
     fi-ini-estabel AT ROW 2 COL 18 COLON-ALIGNED WIDGET-ID 4
     fi-fim-estabel AT ROW 2 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     fi-ini-data AT ROW 3.25 COL 18 COLON-ALIGNED
     fi-fim-data AT ROW 3.25 COL 52 COLON-ALIGNED NO-LABEL
     fi-cod-msg-devolucao-ini AT ROW 4.5 COL 18 COLON-ALIGNED WIDGET-ID 72
     fi-cod-msg-devolucao-fim AT ROW 4.5 COL 51.86 COLON-ALIGNED NO-LABEL WIDGET-ID 70
     IMAGE-1 AT ROW 3.25 COL 33
     IMAGE-2 AT ROW 3.25 COL 51.14
     IMAGE-3 AT ROW 2 COL 33 WIDGET-ID 6
     IMAGE-4 AT ROW 2 COL 51.14 WIDGET-ID 8
     IMAGE-25 AT ROW 4.5 COL 32.86 WIDGET-ID 74
     IMAGE-26 AT ROW 4.5 COL 51 WIDGET-ID 76
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.57 ROW 2.81
         SIZE 84.43 BY 10.15
...
assign fi-ini-estabel:screen-value in frame fPage2 = v_cod_estab_usuar
       fi-fim-estabel:screen-value in frame fPage2 = v_cod_estab_usuar.
...
do on error undo, return error on stop  undo, return error:
    ...
    find estabelec where estabel.cod-estabel = input frame fPage2 fi-ini-estabel no-lock no-error.
    if not avail estabel then do:
        run setFolder IN hFolder (INPUT 2).
        RUN UTP/UT-MSGS.P (INPUT "SHOW",
                   INPUT 17567,
                   INPUT "Estabelecimento inicial n∆o cadastrado.").
        apply "entry" to fi-ini-estabel IN FRAME fpage2.
        return error.    
    end.
    
    find bestabel where bestabel.cod-estabel = input frame fPage2 fi-fim-estabel no-lock no-error.
    if not avail bestabel then do:
        run setFolder IN hFolder (INPUT 2).
        RUN UTP/UT-MSGS.P (INPUT "SHOW",
                   INPUT 17567,
                   INPUT "Estabelecimento final n∆o cadastrado.").
        apply "entry" to fi-fim-estabel IN FRAME fpage2.
        return error.    
    end.
    
    if input frame fPage2 fi-fim-estabel < input frame fPage2 fi-ini-estabel then do:
        run setFolder IN hFolder (INPUT 2).
        RUN UTP/UT-MSGS.P (INPUT "SHOW",
                   INPUT 17567,
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.tipo            = input frame fPage4 rsTipoImpressao.
    ...
    assign tt-param.cod-ini-estabel  = input frame fPage2 fi-ini-estabel
           tt-param.cod-fim-estabel  = input frame fPage2 fi-fim-estabel
    ...
    {report/rprun.i esp/rep/esrep006rp.p}
