...
*******************************************************************************/
{include/i-prgvrs.i ESCQP010 1.00.00.000}
...
&GLOBAL-DEFINE Program        ESCQP010
...
btAssumir btQueryJoins btReportsJoins btExit btHelp c-desc-estab ~
c-desc-item fi-saldo-cq fi-nome-usuario 
...
DEFINE BUTTON btAssumir 
     IMAGE-UP FILE "adeicon/dog.bmp":U
     IMAGE-INSENSITIVE FILE "adeicon/dog.bmp":U NO-CONVERT-3D-COLORS
     LABEL "Assumir" 
     SIZE 5 BY 1.29 TOOLTIP "Assumir solicita‡Æo".
...
DEFINE FRAME fpage0
     ...
     btSave AT ROW 1.13 COL 55 HELP
          "Confirma altera‡äes"
     btAssumir AT ROW 1.13 COL 61 HELP
          "Assumir solicita‡Æo" WIDGET-ID 40
...
&Scoped-define SELF-NAME btAssumir
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btAssumir wMaintenance
ON CHOOSE OF btAssumir IN FRAME fpage0 /* Assumir */
DO:
    IF tt-it-critico-cq.usuar-atend <> "" THEN DO:
        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Solicita‡Æo j  possui atendente~~Solicita‡Æo j  possui atendente").
        RETURN "NOK".
    END.

    RUN utp/ut-msgs.p (INPUT "show",
                       INPUT 27100,
                       INPUT "Confirma‡Æo~~Deseja assumir a solicita‡Æo?").
    
    IF LOGICAL(RETURN-VALUE) THEN
        RUN piGravaAtendente.
END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL tt-it-critico-cq THEN DO:
        ...
        ASSIGN btAssumir:SENSITIVE = YES.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piGravaAtendente wMaintenance 
...
IF VALID-HANDLE({&hDBOTable}) THEN DO:
    RUN piGravaAtendente IN {&hDBOTable} (INPUT c-seg-usuario,
                                          INPUT tt-it-critico-cq.cod-estabel:SCREEN-VALUE IN FRAME fPage0,
                                          INPUT INTEGER(tt-it-critico-cq.cod-solic:SCREEN-VALUE)).

    IF RETURN-VALUE = "OK" THEN
        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 15825,
                           INPUT "Confirma‡Æo~~Solicita‡Æo assumida com sucesso.").
    ELSE
        RUN utp/ut-msgs.p (INPUT "show",
                           INPUT 17006,
                           INPUT "Erro~~Erro ao tentar assumir a solicita‡Æo.").
END.
