...
def var rs_sea_grp_clien 
    ...
    radio-buttons "Por Estab", "Por Estab","Por Usu rio", "Por Usu rio","Por CCusto", "Por CCusto","Por Un", "Por Un" 
    bgcolor 15  
    no-undo. 
...
def frame f_sea_01_grp_clien 
    ...
    with 1 down side-labels no-validate keep-tab-order three-d 
         ...
         title "Pesquisa Usu rio x CCusto - ES0512KC - 1.00.00.001". 
...
DO: 
    ...
    then do: 
        assign v_rec_usu-cc-un-orc = recid(usu-cc-un-orc). 
    end /* if */. 
END.
    ...
    case input frame f_sea_01_grp_clien rs_sea_grp_clien: 
        when "Por Estab" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    each usu-cc-un-orc no-lock 
                    where usu-cc-un-orc.cod-estab >= v_cod_initial 
                      and usu-cc-un-orc.cod-estab <= v_cod_final. 
            end. 
        when "Por Usu rio" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    each usu-cc-un-orc no-lock 
                    where usu-cc-un-orc.cod-usuario >= v_cod_initial 
                      and usu-cc-un-orc.cod-usuario <= v_cod_final 
                    by usu-cc-un-orc.cod-usuario. 
            end. 
        when "Por CCusto" then 
            ...
            do: 
                open query qr_sea_grp_clien for 
                    each usu-cc-un-orc no-lock 
                    where usu-cc-un-orc.cod-ccusto >= v_cod_initial 
                      and usu-cc-un-orc.cod-ccusto <= v_cod_final 
                    by usu-cc-un-orc.cod-ccusto. 
            end /* do name_block */. 
        when "Por Un" then 
            un_block: 
            do: 
                open query qr_sea_grp_clien for 
                    each usu-cc-un-orc no-lock 
                    where usu-cc-un-orc.cod-unid-neg >= v_cod_initial 
                      and usu-cc-un-orc.cod-unid-neg <= v_cod_final 
                    by usu-cc-un-orc.cod-unid-neg. 
            end /* do name_block */. 
    end /* case case_block */. 
END PROCEDURE.
