/******************************************************************************************
******* Programa: docto-saida-wms                                                   *******
******* Objetivo: Integrar documento de saida protheus x totvs                      *******
...
DEFINE INPUT PARAM pfilialPedido                 as char      no-undo.
DEFINE INPUT PARAM pnumeroPedido                 as char      no-undo.
DEFINE INPUT PARAM pcodigoCliente                as char      no-undo.
DEFINE INPUT PARAM pentregaEstado                as char      no-undo.
DEFINE INPUT PARAM pentregaCidade                as char      no-undo.
DEFINE INPUT PARAM pentregaRazaoSocial           as char      no-undo.
DEFINE INPUT PARAM pnomeTransportadora           AS CHAR      NO-UNDO.
DEFINE INPUT PARAM parmazem                      AS CHAR      NO-UNDO.
...
IF CAN-FIND(FIRST tt-prog-ponto) THEN DO:
    FIND FIRST tt-prog-ponto
         WHERE ENTRY(1,tt-prog-ponto.conteudo,";") = pfilialPedido NO-ERROR.
...
FOR EACH tt-item:
    FIND FIRST tt-item-aux
         WHERE tt-item-aux.codigoProduto = tt-item.codigoProduto NO-ERROR.
...
FOR EACH tt-item-aux:
    ...
    IF tt-item-aux.codigoProduto = "" THEN DO:
        RUN piCreateError (INPUT 17006,                           /* ErrorNumber     */
...
ASSIGN c-num-docto = pnumeroPedido.

FIND FIRST wm-docto 
     WHERE wm-docto.cod-estabel     = wm-local.cod-estabel
       AND wm-docto.cod-local       = wm-local.cod-local
       AND wm-docto.num-docto       = c-num-docto 
       AND wm-docto.ind-tipo-trans  = 2 NO-LOCK NO-ERROR.
IF AVAIL wm-docto THEN DO:
    RUN piCreateError (INPUT 17006,                             /* ErrorNumber     */
                       INPUT "Documento j† integrado ao WMS.",  /* ErrorParameters */  
                       INPUT "ERROR").                          /* ErrorSubType    */
    RETURN 'NOK'.
END.
...
ASSIGN ttWm-docto.cod-estabel           = wm-local.cod-estabel
       ...
       ttWm-docto.num-docto             = c-num-docto
       ttWm-docto.id-docto              = IF AVAIL wm-docto THEN wm-docto.id-docto ELSE 0
       ttWm-docto.num-docto-origem      = cCodEstabelOrigem + "-" + pnumeroPedido
       ttWm-docto.ind-tipo-trans        = 2
       ttWm-docto.ind-origem-docto      = 2
...
FOR EACH tt-item-aux:
    ...
    ASSIGN ttWm-docto-itens.cod-estabel           = wm-local.cod-estabel 
           ...
           ttWm-docto-itens.num-docto             = c-num-docto
...
DO TRANS:
    ...
    END.
    ELSE DO:
        /* Armazena em tabela dados do
           cliente, cod cliente, nome cliente,
           nome da transportadora,
           nome rota da transportadora,
           cidade, uf
        */
        FIND FIRST wm-docto EXCLUSIVE-LOCK
             WHERE wm-docto.cod-estabel       = wm-local.cod-estabel
               AND wm-docto.cod-local         = wm-local.cod-local  
               AND Wm-docto.ind-origem-docto  = 2 
               AND Wm-docto.num-docto-origem  = cCodEstabelOrigem + "-" + pnumeroPedido 
               AND Wm-docto.ind-tipo-trans    = 2 NO-ERROR.
        IF AVAIL Wm-docto THEN DO:
            MESSAGE pEntregaRazaoSocial
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
            ASSIGN overlay(Wm-docto.char-2,100,30) = STRING(pEntregaRazaoSocial)                            // Nome Cliente
                   overlay(Wm-docto.char-2,131,30) = STRING(pnomeTransportadora)                          // Nome Transportadora
                   overlay(Wm-docto.char-2,161,30) = STRING(pentregaCidade) + "-" + STRING(pentregaEstado)  // Cidade - Estado
                   overlay(Wm-docto.char-2,191,30) = pcodigoCliente .


            FOR EACH wm-docto-itens OF wm-docto EXCLUSIVE-LOCK,
               FIRST tt-item
               WHERE tt-item.codigoProduto = wm-docto-itens.cod-item:

                ASSIGN overlay(wm-docto-itens.char-2,100,10) = tt-item.numeroOrdemProducao
                       overlay(wm-docto-itens.char-2,111,20) = tt-item.codigoGerador
                       overlay(wm-docto-itens.char-2,131,50) = tt-item.descGerador.
