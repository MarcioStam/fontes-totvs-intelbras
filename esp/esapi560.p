    ...
    FOR FIRST embarque-imp NO-LOCK
       ...
       FOR EACH bf-ordens-embarque NO-LOCK
          WHERE /*bf-ordens-embarque.cod-estabel = embarque-imp.cod-estabel
           ...
           ROWID (bf-ordens-embarque) = ROWID(ordens-embarque),
          ...
          FIND FIRST mgcad.moeda NO-LOCK
               WHERE mgcad.moeda.mo-codigo = ordem-compra.mo-codigo //cotacao-item.mo-codigo
               NO-ERROR.
...
*/
          ...
          THEN DO:
              ...
              ASSIGN
                 cJson = '~{'
                   +     '"id":"'               + embarque-imp.embarque + '-' + REPLACE(REPLACE(STRING(ordem-compra.num-pedido),".","")," ","") + '",'
             ...
             ASSIGN
                cJson = cJson
                   ...
                   +           '"numeroInvoice":"'    + (IF AVAIL invoice-emb-imp 
                                                         ...
                                                         ELSE "") + '",'
                   +           '"cubagem":"'           + REPLACE(STRING(bf-ordens-embarque.val-cub-tot),",",".").
    ...
    THEN RUN pi-acompanhar IN h-acomp ("Altera‡Æo de Pedido").

    IF l-proc = NO
    THEN DO:
       IF es-api-log.aux > ""
       and not l-er-via-trans
       then RUN piErro ("Nao foi encontrado pedido " + es-api-log.aux,"").
    END.
