...
*******************************************************************************/
{include/i-prgvrs.i Z01ES900 3.00.00.000}
/**ATUALIZACAO: 20/03/2012**/

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           Z01ES900
&GLOBAL-DEFINE Version           3.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Ser x Est

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.esp-ext-ser-estab.serie,mgesp.esp-ext-ser-estab.cod-estabel
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO


&GLOBAL-DEFINE ttTable1          tt-esp-ext-ser-estab
&GLOBAL-DEFINE hDBOTable1        h-boes900
&GLOBAL-DEFINE DBOTable1         esp-ext-ser-estab
...
DEF VAR cDescEstabel AS CHAR FORMAT "x(40)" NO-UNDO.
...
&Scoped-define INTERNAL-TABLES tt-esp-ext-ser-estab

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-esp-ext-ser-estab.serie ~
tt-esp-ext-ser-estab.cod-estabel fnDescEstabel() @ cDescEstabel 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-esp-ext-ser-estab NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-esp-ext-ser-estab NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-esp-ext-ser-estab
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-esp-ext-ser-estab
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnDescEstabel wZoom 
FUNCTION fnDescEstabel RETURNS CHARACTER
...
DEFINE QUERY brTable1 FOR 
      tt-esp-ext-ser-estab SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-esp-ext-ser-estab.serie FORMAT "x(5)":U
      tt-esp-ext-ser-estab.cod-estabel FORMAT "x(3)":U
      fnDescEstabel() @ cDescEstabel COLUMN-LABEL "Descri‡Æo" FORMAT "x(40)":U
            WIDTH 40
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-esp-ext-ser-estab"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.tt-esp-ext-ser-estab.serie
     _FldNameList[2]   = Temp-Tables.tt-esp-ext-ser-estab.cod-estabel
     _FldNameList[3]   > "_<CALC>"
"fnDescEstabel() @ cDescEstabel" "Descri‡Æo" "x(40)" ? ? ? ? ? ? ? no ? no no "40" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes900.p":U THEN DO:
       
        RUN esbo\boes900.p PERSISTENT SET {&hDBOTable1} NO-ERROR. 
    END.
    
    RUN setConstraintSerEstab IN {&hDBOTable1} (INPUT "",
                                                INPUT "ZZZZZ",
                                                INPUT "",
                                                INPUT "ZZZ") NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="serEstab"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintSerEstab IN {&hDBOTable1} (INPUT fnIniRangeCharPage(1,1),
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnDescEstabel wZoom 
FUNCTION fnDescEstabel RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
    ...
    IF  AVAIL {&ttTable1} THEN DO:
        FOR FIRST estabelec NO-LOCK
            WHERE estabelec.cod-estabel = {&ttTable1}.cod-estabel:
            ASSIGN cDescEstabel = estabelec.nome.
        END.
    END.

    RETURN cDescEstabel.   /* Function return value. */
END FUNCTION.
