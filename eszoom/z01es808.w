...
DEFINE TEMP-TABLE tt-auditoria-lote NO-UNDO LIKE auditoria-lote
...
*******************************************************************************/
{include/i-prgvrs.i Z01es808 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES808
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Auditoria Lote

&GLOBAL-DEFINE Range             yes

&GLOBAL-DEFINE FieldsRangePage1  mgesp.auditoria-lote.it-codigo
&GLOBAL-DEFINE FieldsAnyKeyPage1 no

&GLOBAL-DEFINE ttTable1          tt-auditoria-lote
&GLOBAL-DEFINE hDBOTable1        h-boes808
&GLOBAL-DEFINE DBOTable1         auditoria-lote
...
&Scoped-define INTERNAL-TABLES tt-auditoria-lote

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-auditoria-lote.it-codigo ~
tt-auditoria-lote.qt-prod-lote tt-auditoria-lote.qt-apar-teste
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-auditoria-lote NO-LOCK ~
    BY tt-auditoria-lote.it-codigo
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-auditoria-lote NO-LOCK ~
    BY tt-auditoria-lote.it-codigo.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-auditoria-lote
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-auditoria-lote
...
DEFINE QUERY brTable1 FOR 
      tt-auditoria-lote SCROLLING.

/* Browse definitions                                                   */
DEFINE BROWSE brTable1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-auditoria-lote.it-codigo   
      tt-auditoria-lote.qt-prod-lote 
      tt-auditoria-lote.qt-apar-teste
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-auditoria-lote T "?" NO-UNDO mgesp auditoria-lote
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
      TABLE: tt-auditoria-lote-2 T "?" NO-UNDO mgesp auditoria-lote
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-auditoria-lote"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-auditoria-lote.it-codigo|yes"
     _FldNameList[1]   = Temp-Tables.tt-auditoria-lote.it-codigo
     _FldNameList[2]   = Temp-Tables.tt-auditoria-lote.nr-seq-comp
     _FldNameList[3]   > Temp-Tables.tt-auditoria-lote.log-ativo
"tt-auditoria-lote.log-ativo" ? "Sim/NÆo" "logical" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable2
     _TblList          = "Temp-Tables.tt-auditoria-lote-2"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-auditoria-lote-2.des-componente|yes"
     _FldNameList[1]   = Temp-Tables.tt-auditoria-lote-2.des-componente
     _FldNameList[2]   = Temp-Tables.tt-auditoria-lote-2.nr-seq-comp
     _FldNameList[3]   > Temp-Tables.tt-auditoria-lote-2.log-ativo
"tt-auditoria-lote-2.log-ativo" ? "Sim/NÆo" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable2 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes808.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes808.p YES}
        {btb/btb008za.i2 esbo/boes808.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintItem IN {&hDBOTable1} (INPUT FnIniRangeCharPage(1,1),
