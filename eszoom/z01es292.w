...
*******************************************************************************/
{include/i-prgvrs.i z01es292 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es292
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Layout Tabela Pre‡o

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          ttlayout-tabpreco
&GLOBAL-DEFINE hDBOTable1        hboes292
&GLOBAL-DEFINE DBOTable1         ttlayout-tabpreco
...
DEF VAR ini-codigo LIKE layout-tabpreco.lay-codigo.
DEF VAR fim-codigo LIKE layout-tabpreco.lay-codigo INIT 999.
...
&Scoped-define INTERNAL-TABLES ttlayout-tabpreco

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttlayout-tabpreco.lay-codigo ~
ttlayout-tabpreco.lay-nome 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttlayout-tabpreco NO-LOCK ~
    BY ttlayout-tabpreco.lay-codigo ~
       BY ttlayout-tabpreco.lay-nome
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttlayout-tabpreco NO-LOCK ~
    BY ttlayout-tabpreco.lay-codigo ~
       BY ttlayout-tabpreco.lay-nome.
&Scoped-define TABLES-IN-QUERY-brTable1 ttlayout-tabpreco
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttlayout-tabpreco
...
DEFINE VARIABLE fi-fim-codigo AS INTEGER FORMAT ">>9":U INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 8.86 BY .79 NO-UNDO.

DEFINE VARIABLE fi-ini-codigo AS INTEGER FORMAT ">>9":U INITIAL 0 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
     SIZE 8.86 BY .79 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttlayout-tabpreco.lay-codigo COLUMN-LABEL "C¢digo" FORMAT ">>9":U
      ttlayout-tabpreco.lay-nome FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-codigo AT ROW 1.25 COL 21 COLON-ALIGNED
     fi-fim-codigo AT ROW 1.25 COL 51 COLON-ALIGNED NO-LABEL
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttlayout-tabpreco"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.ttlayout-tabpreco.lay-codigo|yes,Temp-Tables.ttlayout-tabpreco.lay-nome|yes"
     _FldNameList[1]   > Temp-Tables.ttlayout-tabpreco.lay-codigo
"ttlayout-tabpreco.lay-codigo" "C¢digo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = Temp-Tables.ttlayout-tabpreco.lay-nome
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    ASSIGN INPUT FRAME fpage1
           fi-fim-codigo fi-ini-codigo.
    ASSIGN ini-codigo = fi-ini-codigo
           fim-codigo = fi-fim-codigo.
...
------------------------------------------------------------------------------*/
    ...
    ENABLE fi-fim-codigo
           fi-ini-codigo
           btCheck 
           WITH FRAME fPage1.

    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-ini-codigo IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes292.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes292.p YES}
        {btb/btb008za.i2 esbo/boes292.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "lay-codigo":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.lay-codigo).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintByCod IN {&hDBOTable1} (INPUT fi-ini-codigo,
                                                     INPUT fi-fim-codigo).
