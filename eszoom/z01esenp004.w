...
DEFINE TEMP-TABLE tt-centro-custo NO-UNDO LIKE centro-custo
...
*******************************************************************************/
{include/i-prgvrs.i z01esenp004 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01esenp004
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      CC

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          tt-centro-custo
&GLOBAL-DEFINE hDBOTable1        hboes916
&GLOBAL-DEFINE DBOTable1         centro-custo
...
DEF VAR cc-codigo-ini LIKE centro-custo.cc-codigo.
DEF VAR cc-codigo-fin LIKE centro-custo.cc-codigo INIT "ZZZZZZZZZZZZZZZZZZZZ".
...
&Scoped-define INTERNAL-TABLES tt-centro-custo

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-centro-custo.cc-codigo ~
tt-centro-custo.descricao tt-centro-custo.cod-estabel 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-centro-custo ~
      WHERE tt-centro-custo.cc-codigo >= fi-cc-codigo-ini ~
 AND tt-centro-custo.cc-codigo <= fi-cc-codigo-fin NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-centro-custo ~
      WHERE tt-centro-custo.cc-codigo >= fi-cc-codigo-ini ~
 AND tt-centro-custo.cc-codigo <= fi-cc-codigo-fin NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-centro-custo
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-centro-custo
...
DEFINE VARIABLE fi-cc-codigo-fin AS CHARACTER FORMAT "X(20)" INITIAL "ZZZZZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 21.14 BY .88.

DEFINE VARIABLE fi-cc-codigo-ini AS CHARACTER FORMAT "X(20)" 
     LABEL "Centro Custo" 
     VIEW-AS FILL-IN 
     SIZE 21.14 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-centro-custo.cc-codigo FORMAT "x(20)":U
      tt-centro-custo.descricao FORMAT "x(50)":U WIDTH 55.57
      tt-centro-custo.cod-estabel FORMAT "x(5)":U WIDTH 7.72
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-cc-codigo-ini AT ROW 1.25 COL 14.14 COLON-ALIGNED
     fi-cc-codigo-fin AT ROW 1.25 COL 43.86 COLON-ALIGNED NO-LABEL
     btCheck AT ROW 1.25 COL 83 RIGHT-ALIGNED
     brTable1 AT ROW 2.33 COL 2
     btImplant1 AT ROW 13 COL 2
     IMAGE-1 AT ROW 1.25 COL 38.14
     IMAGE-2 AT ROW 1.25 COL 42.29
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-centro-custo T "?" NO-UNDO mgcad centro-custo
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-centro-custo"
     _Options          = "NO-LOCK"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "Temp-Tables.tt-centro-custo.cc-codigo >= fi-cc-codigo-ini
 AND Temp-Tables.tt-centro-custo.cc-codigo <= fi-cc-codigo-fin"
     _FldNameList[1]   = Temp-Tables.tt-centro-custo.cc-codigo
     _FldNameList[2]   > Temp-Tables.tt-centro-custo.descricao
"tt-centro-custo.descricao" ? "x(50)" "character" ? ? ? ? ? ? no ? no no "55.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-centro-custo.cod-estabel
"tt-centro-custo.cod-estabel" ? ? "character" ? ? ? ? ? ? no ? no no "7.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1 fi-cc-codigo-ini fi-cc-codigo-fin.

  ASSIGN cc-codigo-ini = fi-cc-codigo-ini
         cc-codigo-fin = fi-cc-codigo-fin.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-cc-codigo-ini fi-cc-codigo-fin WITH FRAME fPage1.
    ENABLE fi-cc-codigo-ini fi-cc-codigo-fin btCheck WITH FRAME fPage1.
    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-cc-codigo-ini IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes916.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes009.p YES}
        {btb/btb008za.i2 esbo/boes009.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintcentro-custo IN {&hDBOTable1} (INPUT "",
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="centro-custo"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintcentro-custo IN {&hDBOTable1} (INPUT cc-codigo-ini,
                                                            INPUT cc-codigo-fin).
