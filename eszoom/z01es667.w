...
*******************************************************************************/
{include/i-prgvrs.i Z01ES667 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES667
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Procedimento

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.aq-procedimento.nr-seq-procedimento,mgesp.aq-procedimento.des-procedimento
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO

&GLOBAL-DEFINE ttTable1          tt-aq-procedimento
&GLOBAL-DEFINE hDBOTable1        h-boes667
&GLOBAL-DEFINE DBOTable1         aq-procedimento
...
define variable i-nr-seq-setor-ini as int initial 1 no-undo.
define variable i-nr-seq-setor-fim as int initial 999999 no-undo.
...
&Scoped-define INTERNAL-TABLES tt-aq-procedimento

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-aq-procedimento.des-procedimento ~
tt-aq-procedimento.nr-seq-procedimento tt-aq-procedimento.nr-seq-setor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-aq-procedimento NO-LOCK ~
    BY tt-aq-procedimento.des-procedimento
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-aq-procedimento NO-LOCK ~
    BY tt-aq-procedimento.des-procedimento.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-aq-procedimento
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-aq-procedimento
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-aq-procedimento.des-procedimento FORMAT "x(50)":U
      tt-aq-procedimento.nr-seq-procedimento FORMAT ">>>>>9":U
      tt-aq-procedimento.nr-seq-setor FORMAT ">>>>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     btCheck1 AT ROW 2.25 COL 75 WIDGET-ID 2
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-aq-procedimento"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-aq-procedimento.des-procedimento|yes"
     _FldNameList[1]   = Temp-Tables.tt-aq-procedimento.des-procedimento
     _FldNameList[2]   = Temp-Tables.tt-aq-procedimento.nr-seq-procedimento
     _FldNameList[3]   = Temp-Tables.tt-aq-procedimento.nr-seq-setor
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp\aqp\esaqp007.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes667.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes667.p YES}
        {btb/btb008za.i2 esbo/boes667.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "nr-seq-procedimento":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-seq-procedimento).
            WHEN "des-procedimento" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.des-procedimento).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setaFiltro wZoom 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    define input parameter p-nr-seq-setor as int no-undo.

    assign i-nr-seq-setor-ini = p-nr-seq-setor
           i-nr-seq-setor-fim = p-nr-seq-setor.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintproc IN {&hDBOTable1} (INPUT FnIniRangeIntPage(1,1),
                                                    INPUT FnEndRangeIntPage(1,1),
                                                    INPUT FnIniRangeCharPage(1,2),
                                                    INPUT FnEndRangeCharPage(1,2),
                                                    input i-nr-seq-setor-ini,
                                                    input i-nr-seq-setor-fim).
