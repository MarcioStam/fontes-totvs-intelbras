...
DEFINE TEMP-TABLE ttacao NO-UNDO LIKE acao
...
*******************************************************************************/
{include/i-prgvrs.i z01es002 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es002
...
&GLOBAL-DEFINE ttTable1          ttacao
&GLOBAL-DEFINE hDBOTable1        hboes002
&GLOBAL-DEFINE DBOTable1         ttacao
...
DEF VAR ini-cod-acao LIKE acao.cod-acao.
DEF VAR fim-cod-acao LIKE acao.cod-acao INIT 999.
...
&Scoped-define INTERNAL-TABLES ttacao

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttacao.cod-acao ttacao.Descricao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttacao ~
      WHERE ttacao.cod-acao >= fi-ini-cod-acao ~
and ttacao.cod-acao <= fi-fim-cod-acao NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttacao ~
      WHERE ttacao.cod-acao >= fi-ini-cod-acao ~
and ttacao.cod-acao <= fi-fim-cod-acao NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 ttacao
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttacao
...
DEFINE VARIABLE fi-fim-cod-acao AS INTEGER FORMAT ">>9" INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 16.29 BY .88.

DEFINE VARIABLE fi-ini-cod-acao AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "A‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 16.29 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttacao.cod-acao FORMAT ">>9":U
      ttacao.Descricao FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-cod-acao AT ROW 1.25 COL 12 COLON-ALIGNED
     fi-fim-cod-acao AT ROW 1.25 COL 52 COLON-ALIGNED NO-LABEL
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: ttacao T "?" NO-UNDO mgesp acao
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttacao"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "Temp-Tables.ttacao.cod-acao >= fi-ini-cod-acao
and Temp-Tables.ttacao.cod-acao <= fi-fim-cod-acao"
     _FldNameList[1]   = Temp-Tables.ttacao.cod-acao
     _FldNameList[2]   = Temp-Tables.ttacao.Descricao
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1
         fi-fim-cod-acao fi-ini-cod-acao.
  ASSIGN ini-cod-acao = fi-ini-cod-acao
         fim-cod-acao = fi-fim-cod-acao.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-fim-cod-acao fi-ini-cod-acao WITH FRAME fPage1.
    ENABLE fi-fim-cod-acao fi-ini-cod-acao btCheck WITH FRAME fPage1.
    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-ini-cod-acao IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes002.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes002.p YES}
        {btb/btb008za.i2 esbo/boes002.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="FaixaAcao"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintFaixaAcao IN {&hDBOTable1} (INPUT ini-cod-acao,
                                                         INPUT fim-cod-acao).
