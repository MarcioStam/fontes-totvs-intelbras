...
*******************************************************************************/
{include/i-prgvrs.i Z01ES571 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           Z01ES571
...
&GLOBAL-DEFINE FieldsRangePage1  mgesp.impressora-padrao.cod-estabel,mgesp.impressora-padrao.serie
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO,NO,NO

&GLOBAL-DEFINE ttTable1          tt-impressora-padrao
&GLOBAL-DEFINE hDBOTable1        h-boes571
&GLOBAL-DEFINE DBOTable1         impressora-padrao
...
&Scoped-define INTERNAL-TABLES tt-impressora-padrao

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-impressora-padrao.cod-estabel ~
tt-impressora-padrao.serie tt-impressora-padrao.impressora 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-impressora-padrao NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-impressora-padrao NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-impressora-padrao
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-impressora-padrao
...
DEFINE QUERY brTable1 FOR 
      tt-impressora-padrao SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-impressora-padrao.cod-estabel FORMAT "x(3)":U WIDTH 12.43
      tt-impressora-padrao.serie FORMAT "x(3)":U WIDTH 6.43
      tt-impressora-padrao.impressora FORMAT "x(100)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-impressora-padrao"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   > Temp-Tables.tt-impressora-padrao.cod-estabel
"cod-estabel" ? ? "character" ? ? ? ? ? ? no ? no no "12.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-impressora-padrao.serie
"serie" ? ? "character" ? ? ? ? ? ? no ? no no "6.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = Temp-Tables.tt-impressora-padrao.impressora
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/ftp/esftp081.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes571.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes571.p YES}
        {btb/btb008za.i2 esbo/boes571.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintRangeEstabSerie IN {&hDBOTable1} (INPUT "",
                                                       INPUT "ZZZ",
                                                       INPUT "",
                                                       INPUT "ZZZ") NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/openqueries.i &Query="RangeEstabSerie"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "impressora":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.impressora).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintRangeEstabSerie IN {&hDBOTable1} (INPUT fnIniRangeCharPage(1,1),
