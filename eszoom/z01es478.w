...
DEFINE TEMP-TABLE ttassistente NO-UNDO LIKE assistente
...
{include/i-prgvrs.i z01es478 1.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es478
&GLOBAL-DEFINE Version           1.00.00.001

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Assistente
...
&GLOBAL-DEFINE ttTable1          ttassistente
&GLOBAL-DEFINE hDBOTable1        HDBOttassistente
&GLOBAL-DEFINE DBOTable1         assistente
...
&Scoped-define INTERNAL-TABLES ttassistente

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttassistente.cod-assistente ~
ttassistente.nome ttassistente.matricula ttassistente.e-mail 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttassistente NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttassistente NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 ttassistente
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttassistente
...
DEFINE VARIABLE fi-fim-cod-assistente AS INTEGER FORMAT ">>9" INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88.

DEFINE VARIABLE fi-ini-cod-assistente AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "C¢d. Assistente":R17 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttassistente.cod-assistente FORMAT ">>9":U
      ttassistente.nome FORMAT "x(40)":U
      ttassistente.matricula FORMAT "x(10)":U
      ttassistente.e-mail FORMAT "x(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     ...
     fi-ini-cod-assistente AT ROW 2.75 COL 23 COLON-ALIGNED
     fi-fim-cod-assistente AT ROW 2.75 COL 45 COLON-ALIGNED NO-LABEL
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: ttassistente T "?" NO-UNDO mgesp assistente
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* SETTINGS FOR FRAME fPage1
                                                                        */
/* BROWSE-TAB brTable1 fi-fim-cod-assistente fPage1 */
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttassistente"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.ttassistente.cod-assistente
     _FldNameList[2]   = Temp-Tables.ttassistente.nome
     _FldNameList[3]   = Temp-Tables.ttassistente.matricula
     _FldNameList[4]   = Temp-Tables.ttassistente.e-mail
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ...
  ASSIGN INPUT FRAME fpage1
         fi-ini-cod-assistente
         fi-fim-cod-assistente.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-ini-cod-assistente
         fi-fim-cod-assistente
         WITH FRAME fPage1.

    ENABLE fi-ini-cod-assistente
           fi-fim-cod-assistente
           btCheck
           WITH FRAME fPage1.
    ...
    APPLY "entry" TO fi-ini-cod-assistente IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes478.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes478.p YES}
        {btb/btb008za.i2 esbo/boes478.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintByCod IN {&hDBOTable1} 
                (INPUT fi-ini-cod-assistente,
                 INPUT fi-fim-cod-assistente).
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-assistente":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-assistente).
        END CASE.
    END.
