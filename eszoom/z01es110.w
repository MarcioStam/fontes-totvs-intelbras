...
*******************************************************************************/
{include/i-prgvrs.i z01es110 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es110
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Fabricantes

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          ttitem-fabric
&GLOBAL-DEFINE hDBOTable1        hDBOttitem-fabric
&GLOBAL-DEFINE DBOTable1         ttitem-fabric
...
DEF VAR ini-it-codigo LIKE item-fabric.it-codigo.
DEF VAR fim-it-codigo LIKE item-fabric.it-codigo INIT "ZZZZZZZZZZZZZZZZ".
...
&Scoped-define INTERNAL-TABLES ttitem-fabric fabricante

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttitem-fabric.cod-fabric ~
fabricante.nome-abrev ttitem-fabric.it-fabric 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttitem-fabric NO-LOCK, ~
      FIRST fabricante WHERE fabricante.cod-fabric = ttitem-fabric.cod-fabric NO-LOCK ~
    BY ttitem-fabric.cod-fabric INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttitem-fabric NO-LOCK, ~
      FIRST fabricante WHERE fabricante.cod-fabric = ttitem-fabric.cod-fabric NO-LOCK ~
    BY ttitem-fabric.cod-fabric INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 ttitem-fabric fabricante
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttitem-fabric
&Scoped-define SECOND-TABLE-IN-QUERY-brTable1 fabricante
...
DEFINE QUERY brTable1 FOR 
      ttitem-fabric, 
      fabricante
    FIELDS(fabricante.nome-abrev) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttitem-fabric.cod-fabric COLUMN-LABEL "C¢digo" FORMAT ">>>,>>9":U
      fabricante.nome-abrev COLUMN-LABEL "Nome Abreviado" FORMAT "x(20)":U
      ttitem-fabric.it-fabric COLUMN-LABEL "Item Fabricante" FORMAT "x(36)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-it-codigo AT ROW 1.25 COL 12 COLON-ALIGNED HELP
          "C¢digo do Item"
     fi-fim-it-codigo AT ROW 1.25 COL 52 COLON-ALIGNED HELP
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttitem-fabric,mgesp.fabricante WHERE Temp-Tables.ttitem-fabric ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST USED"
     _OrdList          = "Temp-Tables.ttitem-fabric.cod-fabric|yes"
     _JoinCode[2]      = "mgesp.fabricante.cod-fabric = Temp-Tables.ttitem-fabric.cod-fabric"
     _FldNameList[1]   > Temp-Tables.ttitem-fabric.cod-fabric
"ttitem-fabric.cod-fabric" "C¢digo" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > mgesp.fabricante.nome-abrev
"fabricante.nome-abrev" "Nome Abreviado" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.ttitem-fabric.it-fabric
"ttitem-fabric.it-fabric" "Item Fabricante" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1
         fi-fim-it-codigo fi-ini-it-codigo.
  ASSIGN ini-it-codigo = fi-ini-it-codigo
         fim-it-codigo = fi-fim-it-codigo.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-fim-it-codigo fi-ini-it-codigo WITH FRAME fPage1.
    ENABLE fi-fim-it-codigo fi-ini-it-codigo btCheck WITH FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes110.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes110.p YES}
        {btb/btb008za.i2 esbo/boes110.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="FaixaItem"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintFaixaItem IN {&hDBOTable1} (INPUT ini-it-codigo,
                                                         INPUT fim-it-codigo).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setInitials wZoom 
PROCEDURE setInitials :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEF INPUT PARAM p-ini-it-codigo AS CHAR NO-UNDO.
    DEF INPUT PARAM p-fim-it-codigo AS CHAR NO-UNDO.

    ASSIGN fi-fim-it-codigo = p-fim-it-codigo
           fi-ini-it-codigo = p-ini-it-codigo
           ini-it-codigo    = p-ini-it-codigo
