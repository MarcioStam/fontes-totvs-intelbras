...
*******************************************************************************/
{include/i-prgvrs.i z03es513 2.06.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z03es513
&GLOBAL-DEFINE Version           2.06.00.001

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Familia

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          tt-fam-com-item
&GLOBAL-DEFINE hDBOTable1        hboes513
&GLOBAL-DEFINE DBOTable1         tt-fam-com-item
...
&Scoped-define INTERNAL-TABLES tt-fam-com-item

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-fam-com-item.fm-cod-com ~
tt-fam-com-item.descricao tt-fam-com-item.unidade tt-fam-com-item.segmento ~
tt-fam-com-item.familia1 tt-fam-com-item.familia2 tt-fam-com-item.origem 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-fam-com-item NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-fam-com-item NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-fam-com-item
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-fam-com-item
...
DEFINE VARIABLE fi-familia1 AS CHARACTER FORMAT "x(1)" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88.

DEFINE VARIABLE fi-familia2 AS CHARACTER FORMAT "x(2)" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88.

DEFINE VARIABLE fi-origem AS CHARACTER FORMAT "x(1)" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88.

DEFINE VARIABLE fi-segmento AS CHARACTER FORMAT "x(2)" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88.

DEFINE VARIABLE fi-unidade AS CHARACTER FORMAT "x(2)" 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-fam-com-item.fm-cod-com FORMAT "x(8)":U WIDTH 11.43
      tt-fam-com-item.descricao FORMAT "x(60)":U WIDTH 40.86
      tt-fam-com-item.unidade COLUMN-LABEL "Unid" FORMAT "x(2)":U
            WIDTH 4.57
      tt-fam-com-item.segmento COLUMN-LABEL "Segm" FORMAT "x(2)":U
            WIDTH 4.57
      tt-fam-com-item.familia1 COLUMN-LABEL "Fam" FORMAT "x(1)":U
            WIDTH 4.57
      tt-fam-com-item.familia2 COLUMN-LABEL "Sub" FORMAT "x(2)":U
            WIDTH 4.57
      tt-fam-com-item.origem COLUMN-LABEL "Orig" FORMAT "x(1)":U
            WIDTH 4.57
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     btCheck AT ROW 1.21 COL 83 RIGHT-ALIGNED
     fi-unidade AT ROW 1.25 COL 33.72 COLON-ALIGNED HELP
          "Unidade" WIDGET-ID 16
     fi-segmento AT ROW 1.25 COL 38.14 COLON-ALIGNED HELP
          "Segmento" NO-LABEL WIDGET-ID 12
     fi-familia1 AT ROW 1.25 COL 42.57 COLON-ALIGNED HELP
          "Familia1" NO-LABEL WIDGET-ID 4
     fi-familia2 AT ROW 1.25 COL 47 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     fi-origem AT ROW 1.25 COL 51.43 COLON-ALIGNED NO-LABEL WIDGET-ID 8
...
/* SETTINGS FOR FRAME fPage1
                                                                        */
/* BROWSE-TAB brTable1 fi-origem fPage1 */
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-fam-com-item"
     _Options          = "NO-LOCK"
     _TblOptList       = ", FIRST USED"
     _FldNameList[1]   > Temp-Tables.tt-fam-com-item.fm-cod-com
"fm-cod-com" ? ? "character" ? ? ? ? ? ? no ? no no "11.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-fam-com-item.descricao
"descricao" ? ? "character" ? ? ? ? ? ? no ? no no "40.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-fam-com-item.unidade
"unidade" "Unid" "x(2)" "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-fam-com-item.segmento
"segmento" "Segm" "x(2)" "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-fam-com-item.familia1
"familia1" "Fam" ? "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.tt-fam-com-item.familia2
"familia2" "Sub" "x(2)" "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Temp-Tables.tt-fam-com-item.origem
"origem" "Orig" ? "character" ? ? ? ? ? ? no ? no no "4.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  /*ASSIGN INPUT FRAME fpage1 -selecao fi-valor.*/
...
------------------------------------------------------------------------------*/
    ...
    RUN setConstraintZoom3 IN {&hDBOTable1} (INPUT fi-unidade,
                                             INPUT fi-segmento,
                                             INPUT fi-familia1,
                                             INPUT fi-familia2,
                                             INPUT fi-origem).
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="Zoom3"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "unidade":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.unidade).
            WHEN "segmento":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.segmento).
            WHEN "familia1":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.familia1).
            WHEN "familia2":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.familia2).
            WHEN "origem":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.origem).
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN fi-unidade      = p-unidade    
           fi-segmento     = p-segmento  
           fi-familia1     = p-familia1  
           fi-familia2     = p-familia2
           fi-origem       = p-origem.
