...
*******************************************************************************/
{include/i-prgvrs.i Z03ESIN403 2.00.00.004}  /*** 010004 ***/

&IF "{&EMSFND_VERSION}" >= "1.00"
&THEN
{include/i-license-manager.i Z03ESIN403 MUT}
&ENDIF
...
DEFINE VARIABLE h_b29esin403 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b29esin403.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b29esin403 ).
       RUN set-position IN h_b29esin403 ( 3.29 , 3.14 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 84.57 ) */

       /* Links to SmartBrowser h_b29esin403. */
       RUN add-link IN adm-broker-hdl ( h_b29esin403 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b29esin403 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  /* Dispatch standard ADM method.                             */
{utp/ut9000.i "Z03ESIN403" "2.00.00.004"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-seta-inicial w-pesquisa 
...
RUN pi-seta-inicial IN h_b29esin403 (INPUT p-it-codigo-ini,
...
-------------------------------------------------------------*/
  /*RUN esbrw\b26esin403.w.
  RUN inbrw\b27in403.w.
  RUN clbrw/b02cl007.w.
  RUN inbrw\b28in403.w.
  RUN inbrw\b30in403.w.*/
  RUN adm\objects\folder.w.
  RUN esbrw\b29esin403.w.
