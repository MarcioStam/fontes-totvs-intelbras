...
*******************************************************************************/
{include/i-prgvrs.i Z03ES154 2.00.00.000}  /*** 010000 ***/
/*------------------------------------------------------------------------
...
DEFINE VARIABLE h_b06di154 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 15.92 COL 1.86
     bt-ok AT ROW 17.21 COL 2
     bt-cancela AT ROW 17.21 COL 13
     bt-ajuda AT ROW 17.21 COL 78
     rt-button AT ROW 17 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Itens do Pedido"
         HEIGHT             = 17.54
         WIDTH              = 90
         MAX-HEIGHT         = 17.54
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Itens do Pedido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Itens do Pedido */
DO:
...
DO:
  {include/i-implan.i "pdp\pd0508.w" "h_b06di154"}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Item Pedido' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 14.58 , 88.57 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es539.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b06di154 ).
       RUN set-position IN h_b06di154 ( 3.29 , 3.57 ) NO-ERROR.
       /* Size in UIB:  ( 11.96 , 84.72 ) */

       /* Links to SmartBrowser h_b06di154. */
       RUN add-link IN adm-broker-hdl ( h_b06di154 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b06di154 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  /* Dispatch standard ADM method.                             */
  {utp/ut9000.i "Z03ES154" "2.00.00.000"}
