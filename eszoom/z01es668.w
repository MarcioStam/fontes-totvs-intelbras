...
*******************************************************************************/
{include/i-prgvrs.i Z01ES668 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES668
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      NÆo Conformidade

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.aq-nao-confor.nr-seq-nconfor,mgesp.aq-nao-confor.des-nao-confor
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO

&GLOBAL-DEFINE ttTable1          tt-aq-nao-confor
&GLOBAL-DEFINE hDBOTable1        h-boes668
&GLOBAL-DEFINE DBOTable1         aq-nao-confor
...
&Scoped-define INTERNAL-TABLES tt-aq-nao-confor

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-aq-nao-confor.des-nao-confor ~
tt-aq-nao-confor.nr-seq-nconfor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-aq-nao-confor ~
      WHERE tt-aq-nao-confor.log-inativo = no NO-LOCK ~
    BY tt-aq-nao-confor.des-nao-confor
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-aq-nao-confor ~
      WHERE tt-aq-nao-confor.log-inativo = no NO-LOCK ~
    BY tt-aq-nao-confor.des-nao-confor.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-aq-nao-confor
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-aq-nao-confor
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-aq-nao-confor.des-nao-confor FORMAT "x(50)":U
      tt-aq-nao-confor.nr-seq-nconfor FORMAT ">>>>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-aq-nao-confor"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-aq-nao-confor.des-nao-confor|yes"
     _Where[1]         = "tt-aq-nao-confor.log-inativo = no"
     _FldNameList[1]   = Temp-Tables.tt-aq-nao-confor.des-nao-confor
     _FldNameList[2]   = Temp-Tables.tt-aq-nao-confor.nr-seq-nconfor
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp\aqp\esaqp008.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes668.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes668.p YES}
        {btb/btb008za.i2 esbo/boes668.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "nr-seq-nconfor":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-seq-nconfor).
            WHEN "des-nao-confor" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.des-nao-confor).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintNC IN {&hDBOTable1} (INPUT FnIniRangeIntPage(1,1),
