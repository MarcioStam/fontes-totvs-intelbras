...
*******************************************************************************/
{include/i-prgvrs.i Z01ES664 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES664
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Problema

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.aq-problema.nr-seq-problema,mgesp.aq-problema.des-problema
&GLOBAL-DEFINE FieldsAnyKeyPage1 no,no

&GLOBAL-DEFINE ttTable1          tt-aq-problema
&GLOBAL-DEFINE hDBOTable1        h-boes664
&GLOBAL-DEFINE DBOTable1         aq-problema
...
&Scoped-define INTERNAL-TABLES tt-aq-problema

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-aq-problema.des-problema ~
tt-aq-problema.nr-seq-problema 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-aq-problema ~
      WHERE tt-aq-problema.log-inativo = no NO-LOCK ~
    BY tt-aq-problema.des-problema
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-aq-problema ~
      WHERE tt-aq-problema.log-inativo = no NO-LOCK ~
    BY tt-aq-problema.des-problema.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-aq-problema
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-aq-problema
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-aq-problema.des-problema FORMAT "x(50)":U
      tt-aq-problema.nr-seq-problema FORMAT ">>>>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-aq-problema"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-aq-problema.des-problema|yes"
     _Where[1]         = "tt-aq-problema.log-inativo = no"
     _FldNameList[1]   = Temp-Tables.tt-aq-problema.des-problema
     _FldNameList[2]   = Temp-Tables.tt-aq-problema.nr-seq-problema
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
ON OFF-END OF brTable1 IN FRAME fPage1
DO:
  return no-apply.
END.
...
DO:
    {zoom/implant.i &ProgramImplant="esp\aqp\esaqp004.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes664.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes664.p YES}
        {btb/btb008za.i2 esbo/boes664.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "nr-seq-problema":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-seq-problema).
            WHEN "des-problema" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.des-problema).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintProb IN {&hDBOTable1} (INPUT FnIniRangeIntPage(1,1),
