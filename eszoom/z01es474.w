...
*******************************************************************************/
{include/i-prgvrs.i z01es474 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es474
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Tipos

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          tt-juridico-tp-despesa
&GLOBAL-DEFINE hDBOTable1        hboes474
&GLOBAL-DEFINE DBOTable1         tt-juridico-tp-despesa
...
&Scoped-define INTERNAL-TABLES tt-juridico-tp-despesa

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-juridico-tp-despesa.codigo ~
tt-juridico-tp-despesa.descricao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-juridico-tp-despesa NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-juridico-tp-despesa NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-juridico-tp-despesa
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-juridico-tp-despesa
...
DEFINE VARIABLE fi-descricao-fim AS CHARACTER FORMAT "X(12)" INITIAL "ZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 28 BY .88.

DEFINE VARIABLE fi-descricao-ini AS CHARACTER FORMAT "X(12)" 
     LABEL "Descri‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 28 BY .88.
...
DEFINE QUERY brTable1 FOR 
      tt-juridico-tp-despesa SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-juridico-tp-despesa.codigo FORMAT ">>>>>>9":U WIDTH 9.43
      tt-juridico-tp-despesa.descricao FORMAT "x(40)":U WIDTH 68.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-descricao-ini AT ROW 1.25 COL 8.72 COLON-ALIGNED
     fi-descricao-fim AT ROW 1.25 COL 47.29 COLON-ALIGNED NO-LABEL
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-juridico-tp-despesa"
     _Options          = "NO-LOCK"
     _TblOptList       = ", FIRST USED"
     _FldNameList[1]   > Temp-Tables.tt-juridico-tp-despesa.codigo
"tt-juridico-tp-despesa.codigo" ? ? "integer" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.tt-juridico-tp-despesa.descricao
"tt-juridico-tp-despesa.descricao" ? ? "character" ? ? ? ? ? ? no ? no no "68.86" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1
         fi-descricao-ini fi-descricao-fim.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-descricao-ini fi-descricao-fim WITH FRAME fPage1.
    ENABLE fi-descricao-ini fi-descricao-fim btCheck WITH FRAME fPage1.
    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-descricao-ini IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes474.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes474.p YES}
        {btb/btb008za.i2 esbo/boes474.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintZoomDescricao IN {&hDBOTable1} (INPUT "",
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="ZoomDescricao"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "codigo":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.codigo).
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN DO:
            RUN setConstraintZoomDescricao IN {&hDBOTable1} (INPUT fi-descricao-ini,
                                                             INPUT fi-descricao-fim).
