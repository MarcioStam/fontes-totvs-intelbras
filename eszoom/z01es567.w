...
*******************************************************************************/
{include/i-prgvrs.i Z01ES567 2.00.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es567
&GLOBAL-DEFINE Version           2.00.00.001

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Atendente

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.crm-atendente.cod-estabel,mgesp.crm-atendente.cod-rep,mgesp.crm-atendente.cd-unid-negoc,mgesp.crm-atendente.cd-categoria,mgesp.crm-atendente.cd-atend,mgesp.crm-atendente.cod-gr-cli
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO,NO,NO,NO,NO

&GLOBAL-DEFINE ttTable1          tt-crm-atendente
&GLOBAL-DEFINE hDBOTable1        h-boes567
&GLOBAL-DEFINE DBOTable1         crm-atendente
...
&Scoped-define INTERNAL-TABLES tt-crm-atendente

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-crm-atendente.cod-estabel ~
tt-crm-atendente.cod-rep tt-crm-atendente.cd-unid-negoc ~
tt-crm-atendente.cd-categoria tt-crm-atendente.cd-atend ~
tt-crm-atendente.cod-gr-cli 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-crm-atendente NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-crm-atendente NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-crm-atendente
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-crm-atendente
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-crm-atendente.cod-estabel FORMAT "x(3)":U WIDTH 8.43
      tt-crm-atendente.cod-rep FORMAT ">>>>9":U WIDTH 10.43
      tt-crm-atendente.cd-unid-negoc FORMAT "x(3)":U WIDTH 10.43
      tt-crm-atendente.cd-categoria FORMAT ">>9":U WIDTH 9.43
      tt-crm-atendente.cd-atend FORMAT ">9":U WIDTH 10.43
      tt-crm-atendente.cod-gr-cli FORMAT ">9":U WIDTH 11.29
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 5.75
         FONT 2.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-crm-atendente"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-crm-atendente.cod-estabel
"cod-estabel" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-crm-atendente.cod-rep
"cod-rep" ? ? "integer" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-crm-atendente.cd-unid-negoc
"cd-unid-negoc" ? ? "character" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-crm-atendente.cd-categoria
"cd-categoria" ? ? "integer" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-crm-atendente.cd-atend
"cd-atend" ? ? "integer" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > Temp-Tables.tt-crm-atendente.cod-gr-cli
"cod-gr-cli" ? ? "integer" ? ? ? ? ? ? no ? no no "11.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/cdp/escdp037.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes567.p":U THEN DO:
        {btb/btb008za.i1 esbo/boes567.p YES}
        {btb/btb008za.i2 esbo/boes567.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "cd-atend" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cd-atend).
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintzoom1 IN {&hDBOTable1} (INPUT fnIniRangeCharPage(1,1),
                                                     INPUT fnEndRangeCharPage(1,1),
                                                     INPUT fnIniRangeIntPage(1,2),
                                                     INPUT fnEndRangeIntPage(1,2),
                                                     INPUT fnIniRangeCharPage(1,3),
                                                     INPUT fnEndRangeCharPage(1,3),
                                                     INPUT fnIniRangeIntPage(1,4),
                                                     INPUT fnEndRangeIntPage(1,4),
                                                     INPUT fnIniRangeIntPage(1,5),
                                                     INPUT fnEndRangeIntPage(1,5),
                                                     INPUT fnIniRangeIntPage(1,6),
                                                     INPUT fnEndRangeIntPage(1,6)).
    END CASE.
