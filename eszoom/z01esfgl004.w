...
*******************************************************************************/
{include/i-prgvrs.i z01esflg004 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01esflg004
...
&GLOBAL-DEFINE ttTable1          ttint-rat-desp
&GLOBAL-DEFINE hDBOTable1        hboes916
&GLOBAL-DEFINE DBOTable1         int-rat-desp
...
DEF VAR ini-tipo-rateio LIKE int-rat-desp.tipo-rateio.
DEF VAR fim-tipo-rateio LIKE int-rat-desp.tipo-rateio INIT 999999.
...
&Scoped-define INTERNAL-TABLES ttint-rat-desp

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttint-rat-desp.tipo-rateio ~
ttint-rat-desp.Descricao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttint-rat-desp ~
      WHERE ttint-rat-desp.tipo-rateio >= fi-ini-tipo-rateio ~
and ttint-rat-desp.tipo-rateio <= fi-fim-tipo-rateio NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttint-rat-desp ~
      WHERE ttint-rat-desp.tipo-rateio >= fi-ini-tipo-rateio ~
and ttint-rat-desp.tipo-rateio <= fi-fim-tipo-rateio NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 ttint-rat-desp
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttint-rat-desp
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttint-rat-desp.tipo-rateio FORMAT ">>>>>>9":U WIDTH 12
      ttint-rat-desp.Descricao FORMAT "x(100)":U WIDTH 60
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-tipo-rateio AT ROW 1.25 COL 19.14 COLON-ALIGNED
     fi-fim-tipo-rateio AT ROW 1.25 COL 43.86 COLON-ALIGNED NO-LABEL
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttint-rat-desp"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "Temp-Tables.ttint-rat-desp.tipo-rateio >= fi-ini-tipo-rateio
and Temp-Tables.ttint-rat-desp.tipo-rateio <= fi-fim-tipo-rateio"
     _FldNameList[1]   > Temp-Tables.ttint-rat-desp.tipo-rateio
"ttint-rat-desp.tipo-rateio" ? ? "integer" ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.ttint-rat-desp.Descricao
"ttint-rat-desp.Descricao" ? ? "character" ? ? ? ? ? ? no ? no no "60" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-fim-tipo-rateio fi-ini-tipo-rateio WITH FRAME fPage1.
    ENABLE fi-fim-tipo-rateio fi-ini-tipo-rateio btCheck WITH FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes916.p":U THEN DO:
        ...
        {btb/btb008za.i2 esbo/boes916.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintInt-Rat-Desp IN {&hDBOTable1}  (INPUT 0, INPUT 9999999) NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="int-rat-desp"
                        &PageNumber="1"}
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "tipo-rateio":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.tipo-rateio).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintInt-Rat-Desp IN {&hDBOTable1} (INPUT ini-tipo-rateio,
