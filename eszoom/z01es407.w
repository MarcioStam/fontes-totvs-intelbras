...
*******************************************************************************/
{include/i-prgvrs.i Z01ES407 2.04.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           Z01ES407
...
&GLOBAL-DEFINE FieldsRangePage1  mgesp.equipamentos.cod-estabel,mgesp.equipamentos.equipamento
&GLOBAL-DEFINE FieldsAnyKeyPage1 no,no

&GLOBAL-DEFINE ttTable1          tt-equipamentos
&GLOBAL-DEFINE hDBOTable1        h-boes407
&GLOBAL-DEFINE DBOTable1         equipamentos
...
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-equipamentos.cod-estabel ~
tt-equipamentos.equipamento tt-equipamentos.descricao tt-equipamentos.tipo ~
tt-equipamentos.val-limite 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-equipamentos NO-LOCK ~
    BY tt-equipamentos.cod-estabel ~
       BY tt-equipamentos.equipamento
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-equipamentos NO-LOCK ~
    BY tt-equipamentos.cod-estabel ~
       BY tt-equipamentos.equipamento.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-equipamentos
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-equipamentos
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-equipamentos.cod-estabel FORMAT "x(03)":U WIDTH 3.43
      tt-equipamentos.equipamento FORMAT "x(20)":U WIDTH 13.43
      tt-equipamentos.descricao FORMAT "x(60)":U WIDTH 44.43
      tt-equipamentos.tipo FORMAT ">>9":U WIDTH 4.29
      tt-equipamentos.val-limite FORMAT "->>>,>>9.99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-equipamentos"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-equipamentos.cod-estabel|yes,Temp-Tables.tt-equipamentos.equipamento|yes"
     _FldNameList[1]   > Temp-Tables.tt-equipamentos.cod-estabel
"cod-estabel" ? ? "character" ? ? ? ? ? ? no ? no no "3.43" yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.tt-equipamentos.equipamento
"equipamento" ? ? "character" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.tt-equipamentos.descricao
"descricao" ? ? "character" ? ? ? ? ? ? no ? no no "44.43" yes no no "U" "" ""
     _FldNameList[4]   > Temp-Tables.tt-equipamentos.tipo
"tipo" ? ? "integer" ? ? ? ? ? ? no ? no no "4.29" yes no no "U" "" ""
     _FldNameList[5]   = Temp-Tables.tt-equipamentos.val-limite
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes407.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes407.p YES}
        {btb/btb008za.i2 esbo/boes407.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "val-limite":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.val-limite).
        END CASE.
    END.
