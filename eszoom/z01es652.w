...
*******************************************************************************/
{include/i-prgvrs.i Z01ES652 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i Z01ES652 ESP}
&ENDIF
      */
...
&GLOBAL-DEFINE Program           Z01ES651
&GLOBAL-DEFINE Version           1.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Item MQA

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.item-mqa.cod-prod,mgesp.item-mqa.descricao,mgesp.item-mqa.cod-estabel
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO,NO

&GLOBAL-DEFINE ttTable1          tt-item-mqa
&GLOBAL-DEFINE hDBOTable1        h-boes652
&GLOBAL-DEFINE DBOTable1         item-mqa
...
&Scoped-define INTERNAL-TABLES tt-item-mqa

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-item-mqa.cod-prod ~
tt-item-mqa.cod-estabel tt-item-mqa.descricao tt-item-mqa.it-ref 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-item-mqa NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-item-mqa NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-item-mqa
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-item-mqa
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-item-mqa.cod-prod FORMAT ">>>,>>>,>>9":U
      tt-item-mqa.cod-estabel FORMAT "x(5)":U
      tt-item-mqa.descricao FORMAT "x(60)":U WIDTH 44.86
      tt-item-mqa.it-ref FORMAT "x(16)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-item-mqa"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.tt-item-mqa.cod-prod
     _FldNameList[2]   = Temp-Tables.tt-item-mqa.cod-estabel
     _FldNameList[3]   > Temp-Tables.tt-item-mqa.descricao
"tt-item-mqa.descricao" ? ? "character" ? ? ? ? ? ? no ? no no "44.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = Temp-Tables.tt-item-mqa.it-ref
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/cpp/escpp076.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes652.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes652.p YES}
        {btb/btb008za.i2 esbo/boes652.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "it-ref":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.it-ref).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintProduto IN {&hDBOTable1} (INPUT FnIniRangeCharPage(1,1),
