...
*******************************************************************************/
{include/i-prgvrs.i Z01ES614 2.00.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           Z01ES614
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      DUN-14

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.item-dun.it-codigo,mgesp.item-dun.qtd-emb
&GLOBAL-DEFINE FieldsAnyKeyPage1 YES,YES

&GLOBAL-DEFINE ttTable1          tt-item-dun
&GLOBAL-DEFINE hDBOTable1        h-boes614
&GLOBAL-DEFINE DBOTable1         item-dun
...
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-item-dun.it-codigo ~
tt-item-dun.qtd-emb tt-item-dun.digito tt-item-dun.cod-dun ~
tt-item-dun.impresso 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-item-dun NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-item-dun NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-item-dun
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-item-dun
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-item-dun.it-codigo FORMAT "x(7)":U WIDTH 8.43
      tt-item-dun.qtd-emb FORMAT "->,>>>,>>9":U WIDTH 14.43
      tt-item-dun.digito FORMAT "9":U WIDTH 19.86
      tt-item-dun.cod-dun FORMAT "x(14)":U WIDTH 15
      tt-item-dun.impresso FORMAT "Sim/NÆo":U WIDTH 8.86
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-item-dun"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-item-dun.it-codigo
"it-codigo" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-item-dun.qtd-emb
"qtd-emb" ? ? "integer" ? ? ? ? ? ? no ? no no "14.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-item-dun.digito
"digito" ? ? "integer" ? ? ? ? ? ? no ? no no "19.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-item-dun.cod-dun
"cod-dun" ? ? "character" ? ? ? ? ? ? no ? no no "15" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > Temp-Tables.tt-item-dun.impresso
"impresso" ? ? "logical" ? ? ? ? ? ? no ? no no "8.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/cpp/escpp067.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes614.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes614.p YES}
        {btb/btb008za.i2 esbo/boes614.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/openqueries.i &Query="ItQtd"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "qtd-emb":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.qtd-emb).
            WHEN "digito":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.digito).
            WHEN "cod-dun":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-dun).
            WHEN "impresso":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.impresso).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintItQtd IN {&hDBOTable1} (INPUT fnIniRangeCharPage(INPUT 1, INPUT 1),
