...
DEFINE TEMP-TABLE ttsituacao NO-UNDO LIKE situacao
...
*******************************************************************************/
{include/i-prgvrs.i z01es173 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es173
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Situa‡äes

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          ttsituacao
&GLOBAL-DEFINE hDBOTable1        hboes173
&GLOBAL-DEFINE DBOTable1         ttsituacao
...
DEF VAR ini-cod-situacao LIKE situacao.cod-situacao.
DEF VAR fim-cod-situacao LIKE situacao.cod-situacao INIT 9999999.
...
&Scoped-define INTERNAL-TABLES ttsituacao

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttsituacao.cod-situacao ~
ttsituacao.Descricao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttsituacao ~
      WHERE ttsituacao.cod-situacao >= ini-cod-situacao ~
 AND ttsituacao.cod-situacao <= fim-cod-situacao NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttsituacao ~
      WHERE ttsituacao.cod-situacao >= ini-cod-situacao ~
 AND ttsituacao.cod-situacao <= fim-cod-situacao NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 ttsituacao
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttsituacao
...
DEFINE VARIABLE fi-fim-cod-situacao AS INTEGER FORMAT ">>9" INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 16.29 BY .88.

DEFINE VARIABLE fi-ini-cod-situacao AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "Situa‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 16.29 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttsituacao.cod-situacao FORMAT ">>9":U
      ttsituacao.Descricao FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-cod-situacao AT ROW 1.25 COL 12 COLON-ALIGNED
     fi-fim-cod-situacao AT ROW 1.25 COL 52 COLON-ALIGNED NO-LABEL
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: ttsituacao T "?" NO-UNDO mgesp situacao
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttsituacao"
     _Options          = "NO-LOCK"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "Temp-Tables.ttsituacao.cod-situacao >= ini-cod-situacao
 AND Temp-Tables.ttsituacao.cod-situacao <= fim-cod-situacao"
     _FldNameList[1]   = Temp-Tables.ttsituacao.cod-situacao
     _FldNameList[2]   = Temp-Tables.ttsituacao.Descricao
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1
         fi-fim-cod-situacao fi-ini-cod-situacao.
  ASSIGN ini-cod-situacao = fi-ini-cod-situacao
         fim-cod-situacao = fi-fim-cod-situacao.
...
DO:
    {zoom/Implant.i &ProgramImplant="esp/ccp/esccp030.w"
                    &PageNumber="1"}                 
END.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-fim-cod-situacao fi-ini-cod-situacao WITH FRAME fPage1.
    ENABLE fi-fim-cod-situacao fi-ini-cod-situacao btCheck WITH FRAME fPage1.
    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-ini-cod-situacao IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes173.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes173.p YES}
        {btb/btb008za.i2 esbo/boes173.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="FaixaSituacao"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-situacao":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-situacao).
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintFaixaSituacao IN {&hDBOTable1} (INPUT ini-cod-situacao,
                                                             INPUT fim-cod-situacao).
