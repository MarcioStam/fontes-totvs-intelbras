...
*******************************************************************************/
{include/i-prgvrs.i Z01ES678 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES678
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Lote

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.aq-tipo-lote.nr-seq-tipo-lote,mgesp.aq-tipo-lote.des-lote
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO

&GLOBAL-DEFINE ttTable1          tt-aq-tipo-lote
&GLOBAL-DEFINE hDBOTable1        h-boes678
&GLOBAL-DEFINE DBOTable1         aq-tipo-lote
...
&Scoped-define INTERNAL-TABLES tt-aq-tipo-lote

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-aq-tipo-lote.des-lote ~
tt-aq-tipo-lote.nr-seq-tipo-lote 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-aq-tipo-lote NO-LOCK ~
    BY tt-aq-tipo-lote.des-lote
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-aq-tipo-lote NO-LOCK ~
    BY tt-aq-tipo-lote.des-lote.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-aq-tipo-lote
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-aq-tipo-lote
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-aq-tipo-lote.des-lote FORMAT "x(50)":U
      tt-aq-tipo-lote.nr-seq-tipo-lote FORMAT ">>>>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-aq-tipo-lote"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-aq-tipo-lote.des-lote|yes"
     _FldNameList[1]   = Temp-Tables.tt-aq-tipo-lote.des-lote
     _FldNameList[2]   = Temp-Tables.tt-aq-tipo-lote.nr-seq-tipo-lote
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp\aqp\esaqp002.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes678.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes678.p YES}
        {btb/btb008za.i2 esbo/boes678.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "nr-seq-tipo-lote":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-seq-tipo-lote).
            WHEN "des-lote" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.des-lote).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintLote IN {&hDBOTable1} (INPUT FnIniRangeIntPage(1,1),
