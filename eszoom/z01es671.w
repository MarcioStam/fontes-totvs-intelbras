...
*******************************************************************************/
{include/i-prgvrs.i Z01ES671 2.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES671
&GLOBAL-DEFINE Version           2.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Auditoria Geral

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.auditoria-geral.cod-estabel,mgesp.auditoria-geral.nr-seq-auditoria,mgesp.auditoria-geral.it-codigo
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO,no

&GLOBAL-DEFINE ttTable1          tt-auditoria-geral
&GLOBAL-DEFINE hDBOTable1        h-boes671
&GLOBAL-DEFINE DBOTable1         auditoria-geral
...
&Scoped-define INTERNAL-TABLES tt-auditoria-geral

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-auditoria-geral.cod-estabel ~
tt-auditoria-geral.nr-seq-auditoria tt-auditoria-geral.it-codigo 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-auditoria-geral NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-auditoria-geral NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-auditoria-geral
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-auditoria-geral
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-auditoria-geral.cod-estabel FORMAT "x(5)":U
      tt-auditoria-geral.nr-seq-auditoria COLUMN-LABEL "Amostragem" FORMAT ">>>>>9":U
      tt-auditoria-geral.it-codigo FORMAT "x(16)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     btCheck1 AT ROW 3 COL 74 WIDGET-ID 2
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-auditoria-geral"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.tt-auditoria-geral.cod-estabel
     _FldNameList[2]   > Temp-Tables.tt-auditoria-geral.nr-seq-auditoria
"tt-auditoria-geral.nr-seq-auditoria" "Amostragem" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = Temp-Tables.tt-auditoria-geral.it-codigo
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp\aqp\esaqp010.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes671.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes671.p YES}
        {btb/btb008za.i2 esbo/boes671.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "nr-seq-auditoria" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-seq-auditoria).
            WHEN "it-codigo" THEN
