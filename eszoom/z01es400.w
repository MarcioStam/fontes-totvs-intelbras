...
*******************************************************************************/
{include/i-prgvrs.i z01es400 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es400
...
&GLOBAL-DEFINE ttTable1          ttcat
&GLOBAL-DEFINE hDBOTable1        HDBOcat
&GLOBAL-DEFINE DBOTable1         cat
...
&Scoped-define INTERNAL-TABLES ttcat

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttcat.nr-cat ttcat.sequencia ~
ttcat.cod-emitente ttcat.ind-situacao ttcat.dt-cat ttcat.nro-docto 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttcat NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttcat NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 ttcat
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttcat
...
DEFINE VARIABLE i-cat-fim AS INTEGER FORMAT "->,>>>,>>9" INITIAL 999999 
     VIEW-AS FILL-IN 
     SIZE 8 BY .88.

DEFINE VARIABLE i-cat-ini AS INTEGER FORMAT "->,>>>,>>9" INITIAL 0 
     LABEL "CAT" 
     VIEW-AS FILL-IN 
     SIZE 9 BY .88.

DEFINE VARIABLE i-sequencia-fim AS INTEGER FORMAT "->,>>>,>>9" INITIAL 999999 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .88.

DEFINE VARIABLE i-sequencia-ini AS INTEGER FORMAT "->,>>>,>>9" INITIAL 0 
     LABEL "Sequencia" 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttcat.nr-cat FORMAT ">>>,>>>,>>9":U
      ttcat.sequencia FORMAT ">,>>>,>>9":U
      ttcat.cod-emitente FORMAT ">>>>,>>9":U WIDTH 12.29
      ttcat.ind-situacao FORMAT "9":U
      ttcat.dt-cat FORMAT "99/99/9999":U
      ttcat.nro-docto FORMAT "X(16)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 74 BY 8.5
         FONT 2.
...
DEFINE FRAME fPage1
     i-cat-ini AT ROW 2 COL 27 COLON-ALIGNED HELP
          "Cat Inicial"
     i-cat-fim AT ROW 2 COL 46 COLON-ALIGNED HELP
          "Cat Final" NO-LABEL
     i-sequencia-ini AT ROW 3 COL 27 COLON-ALIGNED HELP
          "Serie da Nota Fiscal" WIDGET-ID 4
     i-sequencia-fim AT ROW 3 COL 46 COLON-ALIGNED HELP
          "Nome do Programa" NO-LABEL WIDGET-ID 2
     btCheck-1 AT ROW 3 COL 61
     brTable1 AT ROW 4.5 COL 6
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttcat"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.ttcat.nr-cat
     _FldNameList[2]   = Temp-Tables.ttcat.sequencia
     _FldNameList[3]   > Temp-Tables.ttcat.cod-emitente
"cod-emitente" ? ? "integer" ? ? ? ? ? ? no ? no no "12.29" yes no no "U" "" ""
     _FldNameList[4]   = Temp-Tables.ttcat.ind-situacao
     _FldNameList[5]   = Temp-Tables.ttcat.dt-cat
     _FldNameList[6]   = Temp-Tables.ttcat.nro-docto
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    ENABLE
        i-cat-ini i-cat-fim i-sequencia-ini i-sequencia-fim
        btCheck-1
       WITH FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes400.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes400.p YES}
        {btb/btb008za.i2 esbo/boes400.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintRangecat IN {&hDBOTable1} (input v_cod_estab_usuar,
                                                input v_cod_estab_usuar,
                                                INPUT 0,
                                                INPUT 999999,
                                                INPUT 0,
                                                INPUT 999999) NO-ERROR.
...
/*:T------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    {zoom/OpenQueries.i &Query="Rangecat"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "nr-cat":U          THEN ASSIGN pcFieldValue = STRING({&ttTable1}.nr-cat).
            WHEN "sequencia":U    THEN ASSIGN pcFieldValue = STRING({&ttTable1}.sequencia).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintRangecat IN {&hDBOTable1} (c-cod-estabel-ini,
                                                        c-cod-estabel-fim,
                                                       INPUT FRAME fPage1 i-cat-ini,
                                                       INPUT FRAME fPage1 i-cat-fim,                                                                                                              
                                                       INPUT FRAME fPage1 i-sequencia-ini,
                                                       INPUT FRAME fPage1 i-sequencia-fim).
    END CASE.
