...
DEFINE TEMP-TABLE tt-cep-1 NO-UNDO LIKE cep
...
*******************************************************************************/
{include/i-prgvrs.i z02es539 9.99.99.999}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z02es539
&GLOBAL-DEFINE Version           <ProgramVersion>

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      CEP

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.cep.cep
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO

&GLOBAL-DEFINE ttTable1          tt-cep-1
&GLOBAL-DEFINE hDBOTable1        hdbocep1
&GLOBAL-DEFINE DBOTable1         cep
...
&Scoped-define INTERNAL-TABLES tt-cep-1

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-cep-1.cep tt-cep-1.localidade ~
tt-cep-1.uf 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-cep-1 NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-cep-1 NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-cep-1
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-cep-1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-cep-1.cep FORMAT ">>>>>>>9":U WIDTH 10.43
      tt-cep-1.localidade FORMAT "x(72)":U WIDTH 61.43
      tt-cep-1.uf FORMAT "x(2)":U WIDTH 5
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-cep-1 T "?" NO-UNDO mgesp cep
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-cep-1"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-cep-1.cep
"tt-cep-1.cep" ? ? "integer" ? ? ? ? ? ? no ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-cep-1.localidade
"tt-cep-1.localidade" ? ? "character" ? ? ? ? ? ? no ? no no "61.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-cep-1.uf
"tt-cep-1.uf" ? ? "character" ? ? ? ? ? ? no ? no no "5" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/cdp/escdp057.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes539.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes539.p YES}
        {btb/btb008za.i2 esbo/boes539.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintcep IN {&hDBOTable1} (INPUT 0, INPUT 99999999) NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cep":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cep).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintcep IN {&hDBOTable1} (INPUT fniniRangeIntPage(1,1),
