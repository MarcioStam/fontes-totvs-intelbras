...
*******************************************************************************/
{include/i-prgvrs.i z01es098 2.04.00.001}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es098
&GLOBAL-DEFINE Version           2.04.00.001

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Öndice CGC

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          ttindice-cgc
&GLOBAL-DEFINE hDBOTable1        hboes098
&GLOBAL-DEFINE DBOTable1         ttindice-cgc
...
DEF VAR ini-cgc  LIKE indice-cgc.cgc.
DEF VAR fim-cgc  LIKE indice-cgc.cgc.
DEF VAR ini-unid LIKE indice-cgc.unid-neg.
DEF VAR fim-unid LIKE indice-cgc.unid-neg.
...
&Scoped-define INTERNAL-TABLES ttindice-cgc

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttindice-cgc.cgc ~
ttindice-cgc.unid-neg ttindice-cgc.indice 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttindice-cgc NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttindice-cgc NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 ttindice-cgc
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttindice-cgc
...
DEFINE VARIABLE fi-fim-cgc AS CHARACTER FORMAT "X(10)":U INITIAL "ZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 11 BY .79 NO-UNDO.

DEFINE VARIABLE fi-fim-unid AS CHARACTER FORMAT "X(3)":U INITIAL "ZZZ" 
     VIEW-AS FILL-IN 
     SIZE 11 BY .79 NO-UNDO.

DEFINE VARIABLE fi-ini-cgc AS CHARACTER FORMAT "X(10)":U 
     LABEL "CGC" 
     VIEW-AS FILL-IN 
     SIZE 10.86 BY .79 NO-UNDO.

DEFINE VARIABLE fi-ini-unid AS CHARACTER FORMAT "X(3)":U 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttindice-cgc.cgc FORMAT "X(10)":U
      ttindice-cgc.unid-neg FORMAT "x(3)":U
      ttindice-cgc.indice FORMAT ">>9.9999":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     ...
     fi-ini-cgc AT ROW 1.5 COL 19 COLON-ALIGNED
     fi-fim-cgc AT ROW 1.5 COL 51 COLON-ALIGNED NO-LABEL
     fi-ini-unid AT ROW 2.5 COL 19 COLON-ALIGNED WIDGET-ID 2
     fi-fim-unid AT ROW 2.5 COL 51 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     brTable1 AT ROW 3.75 COL 2
     btImplant1 AT ROW 13 COL 2
     IMAGE-1 AT ROW 1.5 COL 32
     IMAGE-2 AT ROW 1.5 COL 50
     IMAGE-3 AT ROW 2.5 COL 32 WIDGET-ID 4
     IMAGE-4 AT ROW 2.5 COL 50 WIDGET-ID 8
...
/* SETTINGS FOR FRAME fPage1
                                                                        */
/* BROWSE-TAB brTable1 fi-fim-unid fPage1 */
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttindice-cgc"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.ttindice-cgc.cgc
     _FldNameList[2]   = Temp-Tables.ttindice-cgc.unid-neg
     _FldNameList[3]   = Temp-Tables.ttindice-cgc.indice
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    ...
    ASSIGN INPUT FRAME fpage1
           fi-fim-cgc fi-ini-cgc
           fi-ini-unid fi-fim-unid.
    ASSIGN ini-cgc  = fi-ini-cgc
           fim-cgc  = fi-fim-cgc
           ini-unid = fi-ini-unid
           fim-unid = fi-fim-unid.
...
------------------------------------------------------------------------------*/
    ...
    APPLY "entry" TO fi-ini-cgc IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes098.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes098.p YES}
        {btb/btb008za.i2 esbo/boes098.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="ByCGC"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cgc":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cgc).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintByCGC IN {&hDBOTable1} (INPUT fi-ini-cgc,
                                                     INPUT fi-fim-cgc,
                                                     INPUT fi-ini-unid,
                                                     INPUT fi-fim-unid).
