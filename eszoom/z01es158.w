...
*******************************************************************************/
{include/i-prgvrs.i Z01ES158 9.99.99.999}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es158
&GLOBAL-DEFINE Version           2.0.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Contratos

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  mgesp.int-recorrencia-contratos.id-recorrencia,mgesp.int-recorrencia-contratos.nr-contrato,mgesp.int-recorrencia-contratos.nr-parcela,mgesp.int-recorrencia-contratos.nr-transacao
&GLOBAL-DEFINE FieldsAnyKeyPage1 no,no,NO,NO

&GLOBAL-DEFINE ttTable1          tt-int-recorrencia-contratos
&GLOBAL-DEFINE hDBOTable1        h-boes158
&GLOBAL-DEFINE DBOTable1         int-recorrencia-contratos
...
&Scoped-define INTERNAL-TABLES tt-int-recorrencia-contratos

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ~
tt-int-recorrencia-contratos.id-recorrencia ~
...
tt-int-recorrencia-contratos.valor tt-int-recorrencia-contratos.observacao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-int-recorrencia-contratos NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-int-recorrencia-contratos NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-int-recorrencia-contratos
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-int-recorrencia-contratos
...
DEFINE QUERY brTable1 FOR 
      tt-int-recorrencia-contratos SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-int-recorrencia-contratos.id-recorrencia FORMAT "x(20)":U
            WIDTH 8.43
      tt-int-recorrencia-contratos.nr-contrato FORMAT "x(40)":U
            WIDTH 8.43
      tt-int-recorrencia-contratos.nr-parcela FORMAT "x(20)":U
            WIDTH 7.43
      tt-int-recorrencia-contratos.nr-transacao FORMAT "x(40)":U
            WIDTH 9.43
      tt-int-recorrencia-contratos.dt-venc FORMAT "99/99/9999":U
      tt-int-recorrencia-contratos.cod-emitente FORMAT ">>>>>>>>9":U
      tt-int-recorrencia-contratos.cod-sit-trans FORMAT "x(80)":U
      tt-int-recorrencia-contratos.valor FORMAT "->>>,>>>,>>9.99":U
      tt-int-recorrencia-contratos.observacao FORMAT "x(200)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-int-recorrencia-contratos"
     _Options          = "NO-LOCK"
     _FldNameList[1]   > Temp-Tables.tt-int-recorrencia-contratos.id-recorrencia
"tt-int-recorrencia-contratos.id-recorrencia" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > Temp-Tables.tt-int-recorrencia-contratos.nr-contrato
"tt-int-recorrencia-contratos.nr-contrato" ? ? "character" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.tt-int-recorrencia-contratos.nr-parcela
"tt-int-recorrencia-contratos.nr-parcela" ? ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Temp-Tables.tt-int-recorrencia-contratos.nr-transacao
"tt-int-recorrencia-contratos.nr-transacao" ? ? "character" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   = Temp-Tables.tt-int-recorrencia-contratos.dt-venc
     _FldNameList[6]   = Temp-Tables.tt-int-recorrencia-contratos.cod-emitente
     _FldNameList[7]   = Temp-Tables.tt-int-recorrencia-contratos.cod-sit-trans
     _FldNameList[8]   = Temp-Tables.tt-int-recorrencia-contratos.valor
     _FldNameList[9]   = Temp-Tables.tt-int-recorrencia-contratos.observacao
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
   ...
   IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes158.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes158.p YES}
        {btb/btb008za.i2 esbo/boes158.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "id-recorrencia":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.id-recorrencia).
            WHEN "nr-contrato":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-contrato).
            WHEN "nr-parcela":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-parcela).
            WHEN "nr-transacao":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.nr-transacao).
        END CASE.
    END.
