...
*******************************************************************************/
{include/i-prgvrs.i Z01es728 1.00.00.000}
...
&GLOBAL-DEFINE Program           Z01ES728
&GLOBAL-DEFINE Version           1.00.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Emails X Movimentos

&GLOBAL-DEFINE Range             YES

&GLOBAL-DEFINE FieldsRangePage1  vazia.emails-movto.cod-estabel,vazia.emails-movto.cod-depos,vazia.emails-movto.cod-localiz
&GLOBAL-DEFINE FieldsAnyKeyPage1 NO,NO,NO

&GLOBAL-DEFINE ttTable1          tt-emails-movto
&GLOBAL-DEFINE hDBOTable1        h-boes728
&GLOBAL-DEFINE DBOTable1         emails-movto
...
&Scoped-define INTERNAL-TABLES tt-emails-movto

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-emails-movto.cod-estabel ~
tt-emails-movto.cod-depos tt-emails-movto.cod-localiz 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-emails-movto NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-emails-movto NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-emails-movto
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-emails-movto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-emails-movto.cod-estabel FORMAT "x(5)":U
      tt-emails-movto.cod-depos FORMAT "x(3)":U WIDTH 5
      tt-emails-movto.cod-localiz FORMAT "x(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-emails-movto T "?" NO-UNDO vazia emails-movto
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-emails-movto"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   = Temp-Tables.tt-emails-movto.cod-estabel
     _FldNameList[2]   > Temp-Tables.tt-emails-movto.cod-depos
"tt-emails-movto.cod-depos" ? ? "character" ? ? ? ? ? ? no ? no no "5" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = Temp-Tables.tt-emails-movto.cod-localiz
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
    {zoom/implant.i &ProgramImplant="esp/cpp/escpp096.w"
                    &PageNumber="1"}
END.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes728.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes728.p YES}
        {btb/btb008za.i2 esbo/boes728.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/openqueries.i &Query="Chave"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            ...
            WHEN "cod-localiz":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-localiz).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintChave IN {&hDBOTable1} (INPUT FnIniRangeCharPage(1,1),
