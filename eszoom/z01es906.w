...
*******************************************************************************/
{include/i-prgvrs.i z01es906 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es906
...
&GLOBAL-DEFINE ttTable1          ttint-vtex-estab
&GLOBAL-DEFINE hDBOTable1        HDBOttint-vtex-estab
&GLOBAL-DEFINE DBOTable1         int-vtex-estab
...
DEF VAR c-tipo-1 AS CHAR LABEL "Descri‡Æo" NO-UNDO.
...
&Scoped-define INTERNAL-TABLES ttint-vtex-estab

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttint-vtex-estab.cod-estabel ~
fn-tipo-1(ttint-vtex-estab.cod-estab) @ c-tipo-1 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttint-vtex-estab NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttint-vtex-estab NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 ttint-vtex-estab
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttint-vtex-estab
...
FUNCTION fn-tipo-1 RETURNS CHARACTER
  ( p-cod-estab AS CHAR /* parameter-definitions */ )  FORWARD.
...
DEFINE VARIABLE c-cod-estab-fim AS CHARACTER FORMAT "x(5)" INITIAL "ZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .88.

DEFINE VARIABLE c-cod-estab-ini AS CHARACTER FORMAT "x(5)" 
     LABEL "Estab" 
     VIEW-AS FILL-IN 
     SIZE 9.72 BY .88.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttint-vtex-estab.cod-estabel FORMAT "x(5)":U
      fn-tipo-1(ttint-vtex-estab.cod-estab) @ c-tipo-1 COLUMN-LABEL "Nome"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     c-cod-estab-ini AT ROW 1 COL 21 COLON-ALIGNED HELP
          "Estab"
     c-cod-estab-fim AT ROW 1 COL 45.86 COLON-ALIGNED HELP
          "Estab" NO-LABEL WIDGET-ID 2
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttint-vtex-estab"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.ttint-vtex-estab.cod-estabel
     _FldNameList[2]   > "_<CALC>"
"fn-tipo-1(ttint-vtex-estab.cod-estab) @ c-tipo-1" "Nome" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    DISP ""           @ c-cod-estab-fim
         "ZZZZZ"      @ c-cod-estab-fim
         WITH FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes924.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes924.p YES}
        {btb/btb008za.i2 esbo/boes924.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintRangeEstabelec IN {&hDBOTable1} (INPUT "",
...
/*:T------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    {zoom/OpenQueries.i &Query="RangeEstabelec"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-estab":U  THEN ASSIGN pcFieldValue = STRING({&ttTable1}.cod-estab).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintRangeEstabelec IN {&hDBOTable1} (INPUT FRAME fPage1 c-cod-estab-ini,
                                                              INPUT FRAME fPage1 c-cod-estab-fim).
    END CASE.
...
FUNCTION fn-tipo-1 RETURNS CHARACTER
  ( p-cod-estab AS CHAR /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  ...
  FIND FIRST estabelec NO-LOCK
      WHERE estabelec.cod-estab = p-cod-estab NO-ERROR.

  IF  AVAIL estabelec THEN
      ASSIGN c-tipo-1 = estabelec.nome.
