...
DEFINE TEMP-TABLE tttipo-lote NO-UNDO LIKE tipo-lote
...
*******************************************************************************/
{include/i-prgvrs.i z01es188 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es188
&GLOBAL-DEFINE Version           2.04.00.000

&GLOBAL-DEFINE InitialPage       1
&GLOBAL-DEFINE FolderLabels      Tipos de Lote

&GLOBAL-DEFINE Range             NO
&GLOBAL-DEFINE ttTable1          tttipo-lote
&GLOBAL-DEFINE hDBOTable1        hboes188
&GLOBAL-DEFINE DBOTable1         tttipo-lote
...
DEF VAR ini-cod-lote LIKE tipo-lote.cod-lote.
DEF VAR fim-cod-lote LIKE tipo-lote.cod-lote INIT 999.
...
&Scoped-define INTERNAL-TABLES tttipo-lote

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tttipo-lote.cod-lote ~
tttipo-lote.descricao tttipo-lote.tipo-antecessor 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tttipo-lote ~
      WHERE tttipo-lote.cod-lote >= fi-ini-cod-lote ~
and tttipo-lote.cod-lote <= fi-fim-cod-lote NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tttipo-lote ~
      WHERE tttipo-lote.cod-lote >= fi-ini-cod-lote ~
and tttipo-lote.cod-lote <= fi-fim-cod-lote NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brTable1 tttipo-lote
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tttipo-lote
...
DEFINE VARIABLE fi-fim-cod-lote AS INTEGER FORMAT ">9" INITIAL 99 
     VIEW-AS FILL-IN 
     SIZE 16.29 BY .88.

DEFINE VARIABLE fi-ini-cod-lote AS INTEGER FORMAT ">9" INITIAL 0 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tttipo-lote.cod-lote FORMAT ">9":U
      tttipo-lote.descricao FORMAT "X(20)":U
      tttipo-lote.tipo-antecessor FORMAT ">9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     fi-ini-cod-lote AT ROW 1.25 COL 12 COLON-ALIGNED
     fi-fim-cod-lote AT ROW 1.25 COL 52 COLON-ALIGNED NO-LABEL
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tttipo-lote T "?" NO-UNDO mgesp tipo-lote
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tttipo-lote"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST USED"
     _Where[1]         = "Temp-Tables.tttipo-lote.cod-lote >= fi-ini-cod-lote
and Temp-Tables.tttipo-lote.cod-lote <= fi-fim-cod-lote"
     _FldNameList[1]   = Temp-Tables.tttipo-lote.cod-lote
     _FldNameList[2]   = Temp-Tables.tttipo-lote.descricao
     _FldNameList[3]   = Temp-Tables.tttipo-lote.tipo-antecessor
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
DO:
  ASSIGN INPUT FRAME fpage1
         fi-fim-cod-lote fi-ini-cod-lote.
  ASSIGN ini-cod-lote = fi-ini-cod-lote
         fim-cod-lote = fi-fim-cod-lote.
...
------------------------------------------------------------------------------*/
    ...
    DISP fi-fim-cod-lote fi-ini-cod-lote WITH FRAME fPage1.
    ENABLE fi-fim-cod-lote fi-ini-cod-lote btCheck WITH FRAME fPage1.
    APPLY "choose" TO btCheck IN FRAME fPage1.
    APPLY "entry" TO fi-ini-cod-lote IN FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes188.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes188.p YES}
        {btb/btb008za.i2 esbo/boes188.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    {zoom/OpenQueries.i &Query="FaixaCodigo"
                        &PageNumber="1"}
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-lote":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-lote).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintFaixaCodigo IN {&hDBOTable1} (INPUT ini-cod-lote,
                                                           INPUT fim-cod-lote).
