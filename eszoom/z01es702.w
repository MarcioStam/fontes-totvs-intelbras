...
*******************************************************************************/
{include/i-prgvrs.i z01es702 1.00.00.000}
...
&GLOBAL-DEFINE Program           z01es702
...
&GLOBAL-DEFINE FieldsRangePage1  mgesp.int-projeto.cod-projeto,mgesp.int-projeto.desc-projeto
&GLOBAL-DEFINE FieldsAnyKeyPage1 no,no

&GLOBAL-DEFINE ttTable1          tt-int-projeto
&GLOBAL-DEFINE hDBOTable1        h-boes702
&GLOBAL-DEFINE DBOTable1         int-projeto
...
&Scoped-define INTERNAL-TABLES tt-int-projeto

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 tt-int-projeto.cod-projeto ~
tt-int-projeto.desc-projeto 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH tt-int-projeto NO-LOCK ~
    BY tt-int-projeto.cod-projeto
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH tt-int-projeto NO-LOCK ~
    BY tt-int-projeto.cod-projeto.
&Scoped-define TABLES-IN-QUERY-brTable1 tt-int-projeto
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 tt-int-projeto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      tt-int-projeto.cod-projeto FORMAT "x(10)":U
      tt-int-projeto.desc-projeto FORMAT "X(100)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-int-projeto"
     _Options          = "NO-LOCK"
     _OrdList          = "Temp-Tables.tt-int-projeto.cod-projeto|yes"
     _FldNameList[1]   = Temp-Tables.tt-int-projeto.cod-projeto
     _FldNameList[2]   > Temp-Tables.tt-int-projeto.desc-projeto
"desc-projeto" ? "X(100)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes702.p":U THEN DO:
       
        {btb/btb008za.i1 esbo/boes702.p YES}
        {btb/btb008za.i2 esbo/boes702.p '' {&hDBOTable1}} 
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE {&ttTable1} THEN DO:
        CASE pcField:
            WHEN "cod-projeto":U THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.cod-projeto).
            WHEN "desc-projeto" THEN
                ASSIGN pcFieldValue = STRING({&ttTable1}.desc-projeto).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintProj IN {&hDBOTable1} (INPUT FnIniRangeCharPage(1,1),
