...
*******************************************************************************/
{include/i-prgvrs.i Z03ESIN124 2.00.00.011}  /*** 010011 ***/
...
def var l-pend          as logical INIT YES no-undo.
def var l-em-an         as logical init yes no-undo.
def var l-pe-ret        as logical init yes no-undo.
def var l-term          as logical INIT YES no-undo.
def var l-canc          as logical INIT YES no-undo.
...
&Scoped-Define ENABLED-OBJECTS bt-implantar bt-ok bt-cancela ~
bt-ajuda rt-button 
...
DEFINE VARIABLE h_b03esin124 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 15.04 COL 1.57     
     bt-ok AT ROW 16.54 COL 2.72
     bt-cancela AT ROW 16.54 COL 14
     bt-ajuda AT ROW 16.54 COL 79
     rt-button AT ROW 16.29 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Roteiro de Inspe‡Æo"
         HEIGHT             = 16.96
         WIDTH              = 89.72
         MAX-HEIGHT         = 20.54
         MAX-WIDTH          = 106
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Roteiro de Inspe‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Roteiro de Inspe‡Æo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Roteiro' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 13.88 , 89.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03esin124.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03esin124 ).
       RUN set-position IN h_b03esin124 ( 3.00 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.54 , 85.72 ) */

       /* Links to SmartBrowser h_b03esin124. */
       RUN add-link IN adm-broker-hdl ( h_b03esin124 , 'State':U , THIS-PROCEDURE ).
...
------------------------------------------------------------------------------*/
  ENABLE bt-implantar bt-ok bt-cancela bt-ajuda rt-button 
...
{utp/ut9000.i "Z03ESIN124" "2.00.00.001"}
...
/*
  ...
  repeat while program-name(level) <> ?.
      if  program-name(level) matches "*Q04IN124*":U then
          ASSIGN l-pend   = NO
                 l-em-an  = YES
                 l-pe-ret = YES
                 l-term   = NO
...
*/
  ...
  {include/win-size.i}
  /*
  run pi-rec-param in h_b03esin124 (input l-pend, input l-em-an, input l-pe-ret,
                                  input l-term, input l-canc).
*/
  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
  /* Code placed here will execute AFTER standard behavior.    */

  /* NÇO EXISTE CADASTRO DE FICHA DO CQ POR ISSO NÇO HABILITA. */
  assign l-implanta = no
         bt-implantar:sensitive in frame {&FRAME-NAME} = no.
...
------------------------------------------------------------------------------*/
    ...
    run pi-seta-inicial in h_b03esin124 (input p-ini-it-codigo).   
