...
*******************************************************************************/
{include/i-prgvrs.i z01es904 2.04.00.000}

CREATE WIDGET-POOL.

/* Preprocessors Definitions ---                                      */
&GLOBAL-DEFINE Program           z01es904
...
&GLOBAL-DEFINE ttTable1          ttint-cond-pag-cli
&GLOBAL-DEFINE hDBOTable1        HDBOttint-cond-pag-cli
&GLOBAL-DEFINE DBOTable1         int-cond-pag-cli
...
&Scoped-define INTERNAL-TABLES ttint-cond-pag-cli

/* Definitions for BROWSE brTable1                                      */
&Scoped-define FIELDS-IN-QUERY-brTable1 ttint-cond-pag-cli.cod-emitente ~
fn-tipo-1(ttint-cond-pag-cli.cod-emitente) @ c-tipo-1 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brTable1 
&Scoped-define QUERY-STRING-brTable1 FOR EACH ttint-cond-pag-cli NO-LOCK
&Scoped-define OPEN-QUERY-brTable1 OPEN QUERY brTable1 FOR EACH ttint-cond-pag-cli NO-LOCK.
&Scoped-define TABLES-IN-QUERY-brTable1 ttint-cond-pag-cli
&Scoped-define FIRST-TABLE-IN-QUERY-brTable1 ttint-cond-pag-cli
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wZoom _STRUCTURED
  QUERY brTable1 NO-LOCK DISPLAY
      ttint-cond-pag-cli.cod-emitente FORMAT ">>>>>>>>9":U
      fn-tipo-1(ttint-cond-pag-cli.cod-emitente) @ c-tipo-1 COLUMN-LABEL "Nome"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME fPage1
     i-cod-emitente-ini AT ROW 1 COL 21 COLON-ALIGNED HELP
          "Cliente"
     i-cod-emitente-fim AT ROW 1 COL 45.86 COLON-ALIGNED HELP
          "Cliente" WIDGET-ID 2
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.ttint-cond-pag-cli"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.ttint-cond-pag-cli.cod-emitente
     _FldNameList[2]   > "_<CALC>"
"fn-tipo-1(ttint-cond-pag-cli.cod-emitente) @ c-tipo-1" "Nome" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brTable1 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
    ...
    DISP 0              @ i-cod-emitente-fim
         999999999      @ i-cod-emitente-fim
         WITH FRAME fPage1.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE({&hDBOTable1}) OR
       ...
       {&hDBOTable1}:FILE-NAME <> "esbo/boes350.p":U THEN DO:
        ...
        {btb/btb008za.i2 esbo/boes918.p '' {&hDBOTable1}} 
    END.
    
    RUN setConstraintRangeEmitente IN {&hDBOTable1} (INPUT 0,
...
/*:T------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    {zoom/OpenQueries.i &Query="RangeEmitente"
                        &PageNumber="1"}
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintRangeEmitente IN {&hDBOTable1} (INPUT FRAME fPage1 i-cod-emitente-ini,
                                                             INPUT FRAME fPage1 i-cod-emitente-fim).
    END CASE.
...
------------------------------------------------------------------------------*/
  ...
  IF  AVAIL emitente THEN
      ASSIGN c-tipo-1 = emitente.nome-emit.
