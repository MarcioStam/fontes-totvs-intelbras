...
*******************************************************************************/
{include/i-prgvrs.i BC9018H 2.00.00.026 } /*** 010026 ***/
...
DEFINE TEMP-TABLE ttwm-etiqueta LIKE wm-etiqueta.

/* login */ {utp/utapi009.i} 
DEFINE INPUT  PARAMETER vRowid      AS ROWID NO-UNDO.
DEFINE OUTPUT PARAMETER p-completo  AS LOG   NO-UNDO.
...
&global-define Frame02Defs   'Picking WMS'                                     At Row 01 Col 01                               ~
                             ...
                             ttWork.cod-livre-4                                At Row 06 Col 06 NO-LABEL FORMAT "x(11)"       ~
                             ttWork.num-serial                                 At Row 07 Col 05 No-label                      ~
                             'Embalagens (777777)'                             At Row 08 Col 01                               ~
...
&global-define Frame05Defs   'Picking WMS'                                     At Row 01 Col 01                          ~
                             ...
                             c-endereco-entrega                                At Row 04 Col 02 No-label                 ~
                             'Area Packing:'                                   At Row 05 Col 01                          ~
                             de-id-packing                                     At Row 05 Col 12 No-label FORMAT '>>>>>9' ~
...
Procedure InicializaCamposFrame02:
    ...
    IF NOT l-acao-usuario THEN DO:
        ...
        RUN bcp/bc9018fr.p (INPUT ttWm-box-movto-idx-picking.cod-estabel, 
                            INPUT ttWm-box-movto-idx-picking.cod-local,   
...
Procedure GravaCamposFrame02:
    ...
    /* Se nao for embalagem, valida a etiqueta */
    If ttWork.num-serial <> 777777 Then do: 
    ...
    End.
        ...
        assign vNumItensSerial   = 0
               ttWork.num-serial = 0.

        IF CAN-FIND(FIRST wm-item WHERE
                    wm-item.cod-item = ttWm-box-movto-idx-picking.cod-item  /*AND
                    wm-item.cod-barras <> ""*/                                NO-LOCK) THEN DO:
            Hide All No-pause.
            UPDATE c-cod-barras WITH FRAME fEan.

            IF  TRIM(INPUT FRAME fEan c-cod-barras) = "" 
            AND NOT CAN-FIND(FIRST wm-item WHERE
                            wm-item.cod-item   = ttWm-box-movto-idx-picking.cod-item  AND
                            wm-item.cod-barras = INPUT FRAME fEan c-cod-barras) THEN DO:
                Assign vLogErro = Yes.
                {bcp/bc9105.i "600" "Item possui EAN. Informar EAN/DUM"}
            ...
            IF NOT CAN-FIND(FIRST wm-item WHERE
                ...
                IF NOT CAN-FIND(FIRST wm-item-embalagem-etiq WHERE
                                wm-item-embalagem-etiq.cod-item     =  ttWm-box-movto-idx-picking.cod-item  AND
                                wm-item-embalagem-etiq.cod-barras   =  INPUT FRAME fEan c-cod-barras        NO-LOCK) /* AND 
                   ttWm-box-movto-idx-picking.cod-item <> INPUT FRAME fEan c-cod-barras */ THEN DO:
                    Assign vLogErro = Yes.
    ...
    If  (v-log-abre-embalagem and ttWork.num-serial <> 0) or (ttWork.num-serial = 0 and ttWm-box-movto-idx-picking.log-picking = no) Then Do: 
        ...
        Hide All No-pause.
        Run bcp/bc9018a.p (Input Rowid(ttwm-box),
    ...
    End.
    ELSE DO: 
        /* Transaá∆o sem serial em Box de Picking */
        If ttWork.num-serial = 0 and ttWm-box-movto-idx-picking.log-picking Then do: 
            Hide All No-pause.

            RUN bcp/bc9018i.p (INPUT ROWID(ttWm-box-movto-idx-picking), 
                               INPUT-OUTPUT TABLE tt-picking-wms-table). /* Chamando o Zoom do Box de Picking */
    ...
    If Available tt-picking-wms-table and ttWork.num-serial <> 0 Then Do:
             ...
             ELSE DO:
                 ...
                 DO:
                     ...
                     IF (ttWm-box-movto-idx-picking.qtd-item * ttWm-box-movto-idx-picking.qti-embalagem) - ttWm-box-movto-idx-picking.qtd-item-picking = 0  THEN DO:
                         ...
                         Hide All No-pause.
                        /* Cria Tabela wms-etiq-packing - INTELBRAS*/
...
PROCEDURE pi-valida-doca:
    ...
    IF wm-docto.num-docto MATCHES "*-FRAC" THEN DO:
        REPEAT ON ENDKEY UNDO, RETRY:
            UPDATE de-id-packing WITH FRAME {&Frame05Name}.
    ...
    ELSE DO:    
        ...
        REPEAT ON ENDKEY UNDO, RETRY:            
            IF es-wm-bloco.pallet-fechado = 1 THEN DO:
                ...
                DISP c-endereco-entrega WITH FRAME {&Frame05Name}.
...
PROCEDURE pi-cria-etiq-unitizadora:
    ...
    IF AVAIL wm-tarefa-docto-itens THEN DO:
        ...
        IF NOT AVAIL es-wm-box-movto-etiq THEN DO:
            ...
            IF NOT AVAIL wms-etiq-packing OR wms-etiq-packing.nr-pedcli = "" THEN DO:
...
PROCEDURE pi-enderecos-entrega:
    ...
    RUN pi-destroy-dbo IN wgbc9018f.
END.
...
PROCEDURE pi-getIndTipoVisualiza:
    ...
    /* Caso o parametro seja informado incorretamente, informa ao usuario */
    IF iIndTipoVisualiza <> 2 AND iIndTipoVisualiza <> 3 AND iIndTipoVisualiza <> 4 THEN DO:
        {bcp/bc9105.i "104" "Valor do parametro incorreto para o item/familia/transacao (DC)"}
        ASSIGN iIndTipoVisualiza = 2.
    END.
