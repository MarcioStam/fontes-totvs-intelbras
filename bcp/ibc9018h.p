...
*******************************************************************************/
{include/i-prgvrs.i BC9018H 2.00.00.039 } /*** 010039 ***/
...
Define Temp-table tt-locais-filhos-ckd NO-UNDO
...
DEFINE INPUT PARAM TABLE FOR tt-locais-filhos-ckd.
...
Procedure GravaCamposFrame02:
    ...
    DEFINE VARIABLE d-etiqueta-pai LIKE in-agrup-etiqueta.id-etiqueta-pai NO-UNDO.
    ...
    ASSIGN d-etiqueta-pai = ttWork.num-serial.

    item-ckd:
    do trans on error undo, RETURN ERROR:
    
        FOR EACH in-agrup-etiqueta NO-LOCK
           WHERE in-agrup-etiqueta.id-etiqueta-pai  = d-etiqueta-pai:
            ...
            If  Can-find( First rowErrors) Then Do:
                Assign vLogErro = Yes.
                UNDO item-ckd, Return ERROR.
            End.
            
            FIND FIRST ttwm-etiqueta NO-ERROR.
    
            FIND FIRST tt-locais-filhos-ckd
                 WHERE tt-locais-filhos-ckd.it-codigo = ttwm-etiqueta.cod-item NO-ERROR.
            
            Find ttWm-box-movto-idx-picking
            Where Rowid(ttWm-box-movto-idx-picking) = tt-locais-filhos-ckd.rowid-box-movto NO-ERROR.
            ...
            If  Return-value <> 'OK':U OR NOT VALID-HANDLE(wgbc9018f) Then Do:
                 Hide All No-pause.
                 RUN bcp/bc9115.p (104,"Problemas na inicializa‡Æo (DC)",8,20,3).
                 Hide All No-pause.
            End.*/
            ...
            If  v-log-abre-embalagem Then Do: 
                ...
                Hide All No-pause.
                Run bcp/bc9018g.p (Input Rowid(ttwm-box),        
            ...
            If Available tt-picking-wms-table Then Do:
                ...
                if  avail tt-erro THEN DO: 
                    FOR EACH tt-erro:
                        ...
                        Assign vLogSai = Yes.
                    END.
                    If vLogErro = Yes Then UNDO item-ckd, Return ERROR.
                END.
        ...
        IF NOT CAN-FIND(FIRST ttWm-box-movto-idx-picking
                        WHERE ttWm-box-movto-idx-picking.ind-tipo-movto = 2) THEN DO:
     ...
     ELSE DO:
         ...
         ELSE DO:
             ...
             DO:
                 ...
                 /* Isto tamb‚m indica que o picking foi conclu¡do.                                         */
