...
DEFINE TEMP-TABLE tt-dep-tarefa NO-UNDO 
...
Define Query qry-wm-dep-movto For tt-dep-tarefa.

DEFINE BROWSE brw-wm-dep-movto QUERY qry-wm-dep-movto NO-LOCK NO-WAIT
      Display tt-dep-tarefa.cod-depos FORMAT "x(06)"
              tt-dep-tarefa.hora      FORMAT "x(10)"
              With No-box No-labels Size 20 By 2.5 NO-SCROLLBAR-VERTICAL.
...
&global-define TriggerAfterFrame02  Run GravaCamposFrame02. Empty Temp-table tt-mostra-locais.
&global-define TriggerAfterFrame03  Run GravaCamposFrame03. Empty Temp-table tt-dep-tarefa. 
...
Procedure InicializaCamposFrame03:
    ...
    Empty Temp-table tt-dep-tarefa.

    DO vNumSeq = 1 TO 2:     

        CREATE tt-dep-tarefa.
        ASSIGN tt-dep-tarefa.id-tarefa   = vNumSeq
               tt-dep-tarefa.cod-depos   = "Dep-" + STRING(vNumSeq)
               tt-dep-tarefa.hora        = STRING(vNumSeq,"99") + ":" + "00".
    END.
    ...
    */
    ...
    FIND FIRST tt-dep-tarefa NO-LOCK NO-ERROR.
    IF RECID(tt-dep-tarefa) = ? THEN DO:
        CREATE tt-dep-tarefa.
        ASSIGN tt-dep-tarefa.cod-depos   = "Tarefas"
               tt-dep-tarefa.hora        = "Inexistentes (WMS)".
    END. 

    Open Query qry-wm-dep-movto For Each tt-dep-tarefa By tt-dep-tarefa.cod-depos By tt-dep-tarefa.hora.
