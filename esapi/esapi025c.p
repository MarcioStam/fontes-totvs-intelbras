...
DEF OUTPUT PARAM TABLE FOR tt-fci.
...
ASSIGN de-mes-base = DATE("01/" + string(MONTH(TODAY), "99") + "/" + string(YEAR(TODAY), "9999")).
...
ASSIGN da-corte = TODAY .

IF OPSYS = "unix":U THEN
   ASSIGN c-arquivo-log1 = '/mnt/spool/is055792/esapi025-C-PROD.TXT'.
ELSE
   ASSIGN c-arquivo-log1 = 'c:/temp/esapi025-C.TXT'.

//OUTPUT TO VALUE(c-arquivo-log1).
...
RUN pi-gerar-dados-extrato (">> Entrou ESAPI025C").
...
IF CAN-FIND(FIRST tt-prog-ponto NO-LOCK) THEN DO:
    FOR EACH tt-prog-ponto:
        ...
        RUN pi-gerar-dados-extrato (">> NATUREZAS " + tt-naturezas.nat-operacao).
    END.
END.
...
FOR EACH b-item-selecao  NO-LOCK
    WHERE b-item-selecao.it-codigo = p-item:

    ASSIGN c-item-pai = b-item-selecao.it-codigo.

    RUN pi-zera-variaveis.

    RUN pi-gerar-dados-extrato (">> DEPOIS pi-zera-variaveis").

    RUN piImprimeRelat.

    RUN pi-gerar-dados-extrato (">> DEPOIS piImprimeRelat").
END.
...
PROCEDURE pi-retorna-saida-ponderada:
    ...
    FOR EACH nota-fiscal USE-INDEX nfftrm-20 NO-LOCK
        WHERE nota-fiscal.dt-emis-nota >= p-data-ini
          ...
          AND nota-fiscal.cod-estabel   = b-item-selecao.cod-estabel
        ...
        IF NOT CAN-FIND(FIRST tt-desp NO-LOCK
            ...
            FOR EACH item-doc-est NO-LOCK
               WHERE item-doc-est.it-codigo = tt-item.it-codigo
                 AND item-doc-est.data <= TODAY,
               FIRST docum-est OF item-doc-est NO-LOCK
                   WHERE docum-est.cod-estabel = b-item-selecao.cod-estabel
...
/*             DELETE tt-desp.                                                          */
    ...
    FOR EACH tt-desp
        WHERE tt-desp.it-codigo = p-item
        BREAK BY tt-desp.pai-filho:

        IF  tt-desp.pai-filho = 0 THEN DO:

            CREATE tt-fci.
            ASSIGN tt-fci.it-codigo        = tt-desp.it-codigo 
                   tt-fci.tot-parcela      = TRUNCATE(de-tot-parcela,2)                  
                   tt-fci.valor-medio      = TRUNCATE(de-valor-medio,2)                  
                   tt-fci.contr-importacao = TRUNCATE(de-tot-parcela / de-valor-medio,2). 
