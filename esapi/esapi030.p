...
IF AVAIL ord-prod THEN DO:
   ...
   ASSIGN de-total-qty  = ord-prod.qt-ordem
          i-estado      = ord-prod.estado
          c-item        = ord-prod.it-codigo
          i-num-op      = ord-prod.nr-ord-prod
...
objOP:ADD("cod_estabel"      ,   ord-prod.cod-estabel).
objOP:ADD("data_inicio"      ,   ord-prod.dt-inicio).
objOP:ADD("qtde"             ,   INT(de-total-qty)).
objOP:ADD("qtde_refugo"      ,   0).   
objOP:ADD("qtde_produzida"   ,   0).
objOP:ADD("data_criacao"     ,   TODAY).
objOP:ADD("data_fim"         ,   ord-prod.dt-termino).
objOP:ADD("sequencia"        ,   1).
objOP:ADD("nr_linha"         ,   ord-prod.nr-linha).
objOP:ADD("it_codigo"        ,   c-item).
objOP:ADD("nr_ordem"         ,   i-num-op).

CASE ord-prod.estado:
        WHEN 1 THEN ASSIGN c-situacao = 'Nao Iniciada'.    
        WHEN 2 THEN ASSIGN c-situacao = 'Liberada'.
        WHEN 3 THEN ASSIGN c-situacao = 'Reservada'.
        WHEN 4 THEN ASSIGN c-situacao = 'Separada'.
        WHEN 5 THEN ASSIGN c-situacao = 'Requisitada'.
        WHEN 6 THEN ASSIGN c-situacao = 'Iniciada'.
        WHEN 7 THEN ASSIGN c-situacao = 'Finalizada'.  
        WHEN 8 THEN ASSIGN c-situacao = 'Terminada'.
    END CASE.

IF p-acao = 'add' OR p-acao = 'update' THEN DO:
  objOP:ADD("status"  , c-situacao     ).  
END.
...
FIND FIRST es-api-URI WHERE es-api-URI.id-URI = 'ordemINTELMES' NO-LOCK NO-ERROR.
...
IF es-api-log.cod-retorno = '201' THEN DO:
   ...
   IF p-acao <> 'delete' THEN DO:
     RUN esapi/esapi031.p (INPUT c-num-op,
                           OUTPUT c-oper,
                           OUTPUT c-result).
   END.
