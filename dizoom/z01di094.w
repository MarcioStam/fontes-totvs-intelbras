...
*******************************************************************************/
{include/i-prgvrs.i Z01DI094 2.00.00.004}  /*** 010004 ***/

&IF "{&EMSFND_VERSION}" >= "1.00"
&THEN
{include/i-license-manager.i Z01DI094 MUT}
&ENDIF
...
DEFINE VARIABLE h_B01DI094 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02di094 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esb01di094 AS HANDLE NO-UNDO.
...
DO:
  {include/i-implan.i "ftp/ft0305.w" "h_B01DI094"}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Sigla|Item|Desc Item' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dibrw/B01DI094.W':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_B01DI094 ).
       RUN set-position IN h_B01DI094 ( 3.29 , 3.29 ) NO-ERROR.
       /* Size in UIB:  ( 10.13 , 84.57 ) */

       /* Links to SmartBrowser h_B01DI094. */
       RUN add-link IN adm-broker-hdl ( h_B01DI094 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_B01DI094 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dibrw/b02di094.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02di094 ).
       RUN set-position IN h_b02di094 ( 3.29 , 3.29 ) NO-ERROR.
       /* Size in UIB:  ( 10.13 , 84.57 ) */

       /* Links to SmartBrowser h_b02di094. */
       RUN add-link IN adm-broker-hdl ( h_b02di094 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02di094 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/esb01di094.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esb01di094 ).
       RUN set-position IN h_esb01di094 ( 2.75 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.13 , 84.57 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esb01di094 ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
...
{utp/ut9000.i "Z01DI094" "2.00.00.004"}
...
-------------------------------------------------------------*/
  RUN dibrw\b02di094.w.
  RUN adm\objects\folder.w.
  RUN dibrw\B01DI094.W.
