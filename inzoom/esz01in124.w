...
/********************************************************************************
*******************************************************************************/
{include/i-prgvrs.i ESZ01IN124 2.00.00.001 } /*** 010001 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
{include/i-license-manager.i ESZ01in124 MUT}
&ENDIF
...
DEFINE VARIABLE h_esb01in124 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-ok AT ROW 19 COL 3
     bt-cancela AT ROW 19 COL 14
     bt-ajuda AT ROW 19 COL 79
     rt-button AT ROW 18.75 COL 2
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 17.58 , 89.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'inbrw/esb01in124.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esb01in124 ).
       RUN set-position IN h_esb01in124 ( 2.50 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 15.50 , 85.72 ) */

       /* Links to SmartBrowser h_esb01in124. */
       RUN add-link IN adm-broker-hdl ( h_esb01in124 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esb01in124 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
{utp/ut9000.i "ESZ01IN124" "2.00.00.012"}
...
------------------------------------------------------------------------------*/
  ...
  run pi-rec-param in h_esb01in124 (input l-pend, input l-em-an, input l-pe-ret,
...
-------------------------------------------------------------*/
  RUN inbrw/esb01in124.w.
  RUN inbrw/b21in124.w.
