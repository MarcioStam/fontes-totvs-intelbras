...
** Data Geracao..........: 06/08/2014 - 08:28:07
** Criado por............: Ganzen
** Criado em.............: 16/11/1995 14:28:28
** Alterado por..........: fut1147
** Alterado em...........: 05/08/2014 17:37:21
** Gerado por............: fut1147
*****************************************************************************/

/*-- Filtro Multi-idioma Aplicado --*/

def var c-versao-prg as char initial " 1.00.01.064":U no-undo.
def var c-versao-rcode as char initial "[[[1.00.01.064[[[":U no-undo. /* Controle de Versao R-CODE - Nao retirar do Fonte */
...
run pi_messages (input "show",
                 input 5009,
                 input substitute ("&1~&2~&3~&4~&5~&6~&7~&8~&9", 
                                    "FNC_BORD_AP_IMPRIMIR","~~EMSFIN", "~~{~&emsfin_version}", "~~5.01")) /*msg_5009*/.
&else
...
def temp-table tt_cabec_bordero no-undo
    ...
    field ttv_num_bord_ap                  as integer format ">>>>>9" initial 0 label "N£mero Borderì" column-label "Borderì"
...
def temp-table tt_item_bord_ap_imprimir no-undo like item_bord_ap
    ...
    field ttv_cod_barra                    as character format "x(44)" label "C¢d. Barras"
...
def Input param p_log_impr_histor_pef
    ...
    format "Sim/N∆o"
    no-undo.
def Input param p_log_impr_cabec_bco
    as logical
    format "Sim/N∆o"
    no-undo.
...
def var v_cod_forma_pagto_altern
    ...
    no-undo.
def var v_cod_forma_pagto_subst_aux
    as character
    format "x(3)":U
    label "Forma Pagto"
    column-label "Forma Pagto"
    no-undo.
def var v_cod_forma_pagto_troca
    as character
    format "x(3)":U
    label "Forma Pagamento"
    column-label "F Pagto Alt"
...
def new global shared var v_ind_classif_bord
    ...
    view-as combo-box
    &if "{&FNC_MULTI_IDIOMA}" = "YES" &then
    list-item-pairs "Por Fornecedor","Por Fornecedor","Por Data Vencimento","Por Data Vencimento","Por Forma de Pagto/Fornecedor","Por Forma de Pagto/Fornecedor","Por Estabelecimento/Fornecedor","Por Estabelecimento/Fornecedor","Por Estabelecimento/Data Vencto","Por Estabelecimento/Data Vencto","Por Estabelecimento/Forma Pagto","Por Estabelecimento/Forma Pagto","Por Fornecedor/Documento","Por Fornecedor/Documento"
    &else
...
def var v_ind_origin_tit_ap
    ...
    view-as combo-box
    &if "{&FNC_MULTI_IDIOMA}" = "YES" &then
    list-item-pairs "PED","PED","REC","REC","EXP","EXP","APB","APB","ACR","ACR","EEC","EEC"
    &else
...
def var v_log_favorec_cheq_adm
    ...
    initial no
    no-undo.
def var v_log_forma_pagto_subst
    as logical
    format "Sim/N∆o"
    initial no
    no-undo.
...
def var v_val_juros_pag_bord_2
    ...
    decimals 2
    no-undo.
def var v_val_lim_forma_pagto_aux
    as decimal
    format "->>,>>>,>>>,>>9.99":U
    decimals 2
    label "Limite Pagto"
...
def new shared var v_rpt_s_1_columns as integer initial 118.
...
assign v_rpt_s_1_name = v_rpt_s_1_name.
def frame f_rpt_s_1_Grp_pag_bord_Lay_pag_bord header
    "Pag: " at 105
    v_num_pag to 113 format ">>>9" skip (1)
    with no-box no-labels width 118 page-bottom stream-io.
def frame f_rpt_s_1_grp_cabec_Lay_cabecalho header
    ...
    with no-box no-labels width 118 page-top stream-io.
...
def frame f_rpt_s_1_Grp_corpo_bord_Lay_corpo_bord header
    ...
    "Vl Multa" to 89
    "Vl Juros" to 102
    "Corr Monet" to 118 skip
    "Vencto" at 51
    "Impto Retido" to 76
    "Vl Descto" to 89
    "Vl Abat" to 102
    "Valor L°quido" to 118 skip
    "----------------------------------------------------------------------------------------------------------------------" at 1 skip
    with no-box no-labels width 118 page-top stream-io.
...
def frame f_rpt_s_1_Grp_tot_bord_Lay_tot_bord header
    ...
    v_val_multa_bord to 89 format "->>>>,>>9.99" view-as text
    v_val_juros_bord to 102 format "->>>>,>>9.99" view-as text
    v_val_var_mon_bord to 118 format "->>>,>>>,>>9.99" view-as text skip
    "Total de Titulos:" at 1
    v_qtd_tot_tit_bord to 35 format ">>>9" view-as text
    v_val_tot_impto_tot to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_desc_bord to 89 format "->>>>,>>9.99" view-as text
    v_val_abat_bord to 102 format "->>>>,>>9.99" view-as text
    v_val_liq_bord to 118 format "->>>,>>>,>>9.99" view-as text skip
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip (1)
    with no-box no-labels width 118 page-top stream-io.
...
def frame f_rpt_s_1_Grp_tot_bord_Lay_tot_forma header
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip
    v_val_pag_bord_2 to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_multa_pag_bord_2 to 89 format "->>>>,>>9.99" view-as text
    v_val_juros_pag_bord_2 to 102 format "->>>>,>>9.99" view-as text
    v_val_var_mon_pag_bord_2 to 118 format "->>>,>>>,>>9.99" view-as text skip
    "Total de T°tulos por Forma de Pagamento" at 1
    v_qtd_tot_tit_pag_bord_2 to 44 format ">>>9" view-as text
    v_val_pag_tot_impto_2 to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_desc_pag_bord_2 to 89 format "->>>>,>>9.99" view-as text
    v_val_abat_pag_bord_2 to 102 format "->>>>,>>9.99" view-as text
    v_val_liq_pag_bord_2 to 118 format "->>>,>>>,>>9.99" view-as text skip
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip
    with no-box no-labels width 118 page-top stream-io.
def frame f_rpt_s_1_Grp_tot_bord_Lay_tot_forneced header
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip
    v_val_pag_bord_1 to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_multa_pag_bord_1 to 89 format "->>>>,>>9.99" view-as text
    v_val_juros_pag_bord_1 to 102 format "->>>>,>>9.99" view-as text
    v_val_var_mon_pag_bord_1 to 118 format "->>>,>>>,>>9.99" view-as text skip
    "Total de T°tulos por Fornecedor" at 1
    v_qtd_tot_tit_pag_bord_1 to 44 format ">>>9" view-as text
    v_val_pag_tot_impto_1 to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_desc_pag_bord_1 to 89 format "->>>>,>>9.99" view-as text
    v_val_abat_pag_bord_1 to 102 format "->>>>,>>9.99" view-as text
    v_val_liq_pag_bord_1 to 118 format "->>>,>>>,>>9.99" view-as text skip
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip
    with no-box no-labels width 118 page-top stream-io.
def frame f_rpt_s_1_Grp_tot_pag_Lay_tot_pag header
    ...
    v_val_multa_pag_bord to 89 format "->>>>,>>9.99" view-as text
    v_val_juros_pag_bord to 102 format "->>>>,>>9.99" view-as text
    v_val_var_mon_pag_bord to 118 format "->>>,>>>,>>9.99" view-as text skip
    "Total de Titulos:" at 1
    v_qtd_tot_tit_pag_bord to 34 format ">>9" view-as text
    v_val_pag_tot_impto to 76 format "->>>,>>>,>>9.99" view-as text
    v_val_desc_pag_bord to 89 format "->>>>,>>9.99" view-as text
    v_val_abat_pag_bord to 102 format "->>>>,>>9.99" view-as text
    v_val_liq_pag_bord to 118 format "->>>,>>>,>>9.99" view-as text skip
    "---------------------------------------------------------" at 1
    "-------------------------------------------------------------" at 58 skip (1)
    with no-box no-labels width 118 page-top stream-io.
...
{include/i_trdfrm.i f_rpt_s_1_Grp_cabec_escrit_Lay_cabec_ender f_rpt_s_1_Grp_cabec_escrit_Lay_cabec_escrit f_rpt_s_1_grp_cabec_Lay_cabecalho f_rpt_s_1_grp_cabec_Lay_Cabec_1 f_rpt_s_1_grp_cabec_Lay_cabec_end f_rpt_s_1_grp_cabec_Lay_Cabec_end_1 f_rpt_s_1_grp_cabec_Lay_cab_msg1 f_rpt_s_1_grp_cabec_Lay_linha f_rpt_s_1_Grp_Cheq_Adm_Lay_Cheq_Adm f_rpt_s_1_Grp_Cheq_Cancel_Lay_Cheq_Cancel f_rpt_s_1_Grp_cheq_favor_Lay_cheq_favor f_rpt_s_1_Grp_cont_corp_Lay_branco f_rpt_s_1_Grp_cont_corp_Lay_cont_bord f_rpt_s_1_Grp_corpo_bord_Lay_corpo_bord f_rpt_s_1_Grp_erro_bord_Lay_erro_bord f_rpt_s_1_Grp_pag_bord_Lay_pag_bord f_rpt_s_1_Grp_tot_bord_Lay_msg_fim f_rpt_s_1_Grp_tot_bord_Lay_msg_fim2 f_rpt_s_1_Grp_tot_bord_Lay_msg_fim3 f_rpt_s_1_Grp_tot_bord_Lay_Tot_ADM f_rpt_s_1_Grp_tot_bord_Lay_tot_bord f_rpt_s_1_Grp_tot_bord_Lay_tot_ext_bord f_rpt_s_1_Grp_tot_bord_Lay_tot_forma f_rpt_s_1_Grp_tot_bord_Lay_tot_forneced f_rpt_s_1_Grp_tot_pag_Lay_tot_pag f_rpt_s_1_Grp_tot_pag_Lay_Tot_pag_cheq }
...
then do:
    run pi_version_extract ('fnc_bord_ap_imprimir':U, 'prgfin/apb/apb742zb.py':U, '1.00.01.064':U, 'pro':U).
end /* if */.
...
assign v_log_cta_fornec = yes
       /*v_log_cta_fornec = &if defined(BF_FIN_CONTAS_CORRENTES_FORNEC) &then yes &else GetDefinedFunction('SPP_CONTAS_CORRENTES_FORNEC':U)  &endif*/
...
** Alterado por..........: corp45566
** Alterado em...........: 12/04/2011 11:58:18
*****************************************************************************/
PROCEDURE pi_rpt_bord_ap_imprimir:
    ...
    do:
       ...
       /* verifica se existem itens com forma de pagamento = "Ordem de Pagamento" e se
                   ...
                   for each tt_item_bord_ap_imprimir exclusive-lock
                       ...
                       if  avail b_fornec_financ then do:     
                           find first b_item_bord_ap no-lock
                               where b_item_bord_ap.cod_estab_bord = tt_item_bord_ap_imprimir.cod_estab_bord
                                 and b_item_bord_ap.cod_portador   = tt_item_bord_ap_imprimir.cod_portador
                                 and b_item_bord_ap.num_bord_ap    = tt_item_bord_ap_imprimir.num_bord_ap
                                 and b_item_bord_ap.num_seq_bord   = tt_item_bord_ap_imprimir.num_seq_bord no-error.
                           if  avail b_item_bord_ap then do:

                               if  b_item_bord_ap.cod_bco_pagto = "" /* l_*/  then
                                   assign tt_item_bord_ap_imprimir.tta_cod_banco = b_fornec_financ.cod_banco.
                               else
                                   assign tt_item_bord_ap_imprimir.tta_cod_banco = b_item_bord_ap.cod_bco_pagto.

                               if  b_item_bord_ap.cod_agenc_bcia_pagto = "" /* l_*/  then
                                   assign tt_item_bord_ap_imprimir.tta_cod_agenc_bcia        = b_fornec_financ.cod_agenc_bcia
                                          tt_item_bord_ap_imprimir.tta_cod_digito_agenc_bcia = b_fornec_financ.cod_digito_agenc_bcia.
                               else
                                   assign tt_item_bord_ap_imprimir.tta_cod_agenc_bcia        = b_item_bord_ap.cod_agenc_bcia_pagto
                                          tt_item_bord_ap_imprimir.tta_cod_digito_agenc_bcia = b_item_bord_ap.cod_digito_agenc_bcia_pagto.
        ...
        run pi_validacao_de_erros. /* pi_traducao_economico */  
        ...
        if not can-find (first tt_log_erros_atualiz) then do:
            ...
            if avail agenc_bcia and avail emscad.banco then do:
    ...
    put stream s_1 unformatted 
                ...
                if  v_qtd_tot_tit_pag_bord <> 0 then do:
                    ...
                    put stream s_1 unformatted 
                        ...
                        v_val_multa_pag_bord to 89 format "->>>>,>>9.99"
                        v_val_juros_pag_bord to 102 format "->>>>,>>9.99"
                        v_val_var_mon_pag_bord to 118 format "->>>,>>>,>>9.99" skip.
    put stream s_1 unformatted 
                        "Total de Titulos:" at 1
                        v_qtd_tot_tit_pag_bord to 34 format ">>9"
                        v_val_pag_tot_impto to 76 format "->>>,>>>,>>9.99"
                        v_val_desc_pag_bord to 89 format "->>>>,>>9.99"
                        v_val_abat_pag_bord to 102 format "->>>>,>>9.99"
                        v_val_liq_pag_bord to 118 format "->>>,>>>,>>9.99" skip
                        "---------------------------------------------------------" at 1
                        "-------------------------------------------------------------" at 58 skip (1).
                end.
                ...
                put stream s_1 unformatted 
                    ...
                    v_val_multa_bord to 89 format "->>>>,>>9.99"
                    v_val_juros_bord to 102 format "->>>>,>>9.99"
                    v_val_var_mon_bord to 118 format "->>>,>>>,>>9.99" skip.
    put stream s_1 unformatted 
                    "Total de Titulos:" at 1
                    v_qtd_tot_tit_bord to 35 format ">>>9"
                    v_val_tot_impto_tot to 76 format "->>>,>>>,>>9.99"
                    v_val_desc_bord to 89 format "->>>>,>>9.99"
                    v_val_abat_bord to 102 format "->>>>,>>9.99"
                    v_val_liq_bord to 118 format "->>>,>>>,>>9.99" skip
                    "---------------------------------------------------------" at 1
                    "-------------------------------------------------------------" at 58 skip (1).
    ...
    &ENDIF
    &IF "{&emsfin_version}" >= "5.07A" AND "{&emsfin_version}" < "9.99" &THEN
                    tt_log_erros_atualiz.tta_cod_estab at 1 format "x(5)"
    &ENDIF
                    tt_log_erros_atualiz.tta_num_seq_refer to 8 format ">>>9"
                    tt_log_erros_atualiz.ttv_num_mensagem to 17 format ">>>>,>>9".
    put stream s_1 unformatted 
                    entry(1, v_des_return_value_editor, chr(255)) at 19 format "x(60)" skip
                    entry(2, v_des_return_value_editor, chr(255)) at 19 format "x(90)" skip.
...
** Alterado por..........: fut1147
** Alterado em...........: 06/08/2014 08:23:37
*****************************************************************************/
PROCEDURE pi_tratar_tt_item_bord_ap_imprimir:
    ...
    assign v_log_erro_impr = no  v_val_liq_item_bord = 0 v_cod_forma_pagto_troca = "" .
    for each item_bord_ap exclusive-lock
        ...
        if first-of (item_bord_ap.dat_vencto_tit_ap) then
           assign v_cod_forma_pagto_troca = "".
        ...
        if v_nom_prog_upc <> '' or v_nom_prog_appc <> '' or v_nom_prog_dpc <> '' then do:
           run pi_i_exec_program_epc_custom_apb742zb (input 'antes_create_temp_table_impressao':U).
        ...
        if v_log_forma_pagto_subst = no then do:

            if avail b_forma_pagto then do:
                &if '{&emsfin_version}' >= '5.06' &then
                    assign v_val_lim_forma_pagto_aux   = b_forma_pagto.val_lim_pagto
                           v_cod_forma_pagto_subst_aux = b_forma_pagto.cod_forma_pagto_subst.
                &else
                    assign v_val_lim_forma_pagto_aux   = b_forma_pagto.val_livre_1
                           v_cod_forma_pagto_subst_aux = GetEntryField(2,b_forma_pagto.cod_livre_1,chr(24)).
                &endif
            end.
            find first b_forma_pagto_alt no-lock
                 where b_forma_pagto_alt.cod_forma_pagto = v_cod_forma_pagto_subst_aux no-error.
        end.
        ...
        then do:
           ...
           if v_log_dat_pagto_bord = yes then do:
               ...
               if item_bord_ap.dat_pagto_tit_ap <> v_dat_pagto_aux then 
                  assign v_cod_forma_pagto_troca = "".

               assign v_dat_pagto_aux = item_bord_ap.dat_pagto_tit_ap.

               if v_cod_forma_pagto_troca = "" or v_log_forma_pagto_subst then do:
                  ...
                  find first tt_item_bord_ap_imprimir no-lock
                       ...
                       and   tt_item_bord_ap_imprimir.num_bord_ap  = item_bord_ap.num_bord_ap
                       and   tt_item_bord_ap_imprimir.cdn_fornecedor = item_bord_ap.cdn_fornecedor
                       and   tt_item_bord_ap_imprimir.cod_forma_pagto =  v_cod_forma_pagto_troca
                       and   tt_item_bord_ap_imprimir.dat_pagto_tit_ap = item_bord_ap.dat_pagto_tit_ap
                       and   tt_item_bord_ap_imprimir.ttv_cod_barra = "" no-error.
               end.        
           end.     
           ...
           then do:
              ...
              /* Begin_Include: i_create_tt_item_bord_ap_imprimir_001 */
              create tt_item_bord_ap_imprimir.
              assign v_num_count = v_num_count + 1.
              assign tt_item_bord_ap_imprimir.cod_estab_bord = item_bord_ap.cod_estab_bord
              ...
              assign tt_item_bord_ap_imprimir.cod_empresa                 = item_bord_ap.cod_empresa
              ...
              &endif
              &if '{&emsfin_version}' >= "5.06" &then
                     tt_item_bord_ap_imprimir.num_id_proces_pagto         = item_bord_ap.num_id_proces_pagto
              &endif
              &if '{&emsfin_version}' >= "5.07" &then
                     tt_item_bord_ap_imprimir.log_pagto_sem_desemb        = item_bord_ap.log_pagto_sem_desemb
              &endif.
              ...
              assign tt_item_bord_ap_imprimir.cod_forma_pagto = if tt_item_bord_ap_imprimir.val_pagto >= v_val_lim_forma_pagto_aux and v_val_lim_forma_pagto_aux <> 0 and v_log_forma_pagto_subst = no 
                                                                then v_cod_forma_pagto_subst_aux
                                                                else v_cod_forma_pagto_altern.
              assign tt_item_bord_ap_imprimir.tta_des_forma_pagto = if tt_item_bord_ap_imprimir.val_pagto >=  v_val_lim_forma_pagto_aux and  v_val_lim_forma_pagto_aux <> 0
                                                                    and v_log_forma_pagto_subst = no and avail b_forma_pagto_alt 
                                                                    then b_forma_pagto_alt.des_forma_pagto 
                                                                    else b_forma_pagto.des_forma_pagto.

              if tt_item_bord_ap_imprimir.val_pagto >= v_val_lim_forma_pagto_aux and v_val_lim_forma_pagto_aux <> 0 and v_log_forma_pagto_subst = no then
                  assign v_cod_forma_pagto_troca =  tt_item_bord_ap_imprimir.cod_forma_pagto. 

              /* End_Include: i_create_tt_item_bord_ap_imprimir_001 */
              ...
              if v_log_dat_pagto_bord = yes then do:       
               ...
               then do:
                   ...
                   then do:
                       if  item_bord_ap.cod_bco_pagto = "" /*l_*/  then
                           ...
                           assign tt_item_bord_ap_imprimir.tta_cod_banco = item_bord_ap.cod_bco_pagto.

                       if  item_bord_ap.cod_agenc_bcia_pagto = "" /*l_*/  then
                           assign tt_item_bord_ap_imprimir.tta_cod_agenc_bcia        = fornec_financ.cod_agenc_bcia
                                  tt_item_bord_ap_imprimir.tta_cod_digito_agenc_bcia = fornec_financ.cod_digito_agenc_bcia.
                       else
                           assign tt_item_bord_ap_imprimir.tta_cod_agenc_bcia        = item_bord_ap.cod_agenc_bcia_pagto
                                  tt_item_bord_ap_imprimir.tta_cod_digito_agenc_bcia = item_bord_ap.cod_digito_agenc_bcia_pagto.

                       if  item_bord_ap.cod_cta_corren_bco_pagto = "" /*l_*/  then
                           assign tt_item_bord_ap_imprimir.tta_cod_cta_corren        = fornec_financ.cod_cta_corren_bco
                                  tt_item_bord_ap_imprimir.tta_cod_digito_cta_corren = fornec_financ.cod_digito_cta_corren.
                       else
                           assign tt_item_bord_ap_imprimir.tta_cod_cta_corren        = item_bord_ap.cod_cta_corren_bco_pagto
                                  tt_item_bord_ap_imprimir.tta_cod_digito_cta_corren = item_bord_ap.cod_digito_cta_corren_pagto.
               ...
               /* Begin_Include: i_create_tt_item_bord_ap_imprimir_002 */
               ...
               if tt_item_bord_ap_imprimir.val_pagto >= v_val_lim_forma_pagto_aux and v_val_lim_forma_pagto_aux <> 0 and v_log_forma_pagto_subst = no then
                  assign v_cod_forma_pagto_troca =   tt_item_bord_ap_imprimir.cod_forma_pagto.        
               /* End_Include: i_create_tt_item_bord_ap_imprimir_002 */
    ...
    &endif.
        ...
        if v_nom_prog_upc <> '' or v_nom_prog_appc <> '' or v_nom_prog_dpc <> '' then
           run pi_i_exec_program_epc_custom_apb742zb (input 'apos_create_temp_table_impressao':U).
...
** Alterado por..........: fut1147
** Alterado em...........: 05/08/2014 17:36:34
*****************************************************************************/
PROCEDURE pi_corpo_bord_ap_imprimir:
    ...
    put stream s_1 unformatted 
        ...
        tt_item_bord_ap_imprimir.ttv_des_documento at 13 format "x(35)"
        tt_item_bord_ap_imprimir.tta_dat_emis_docto at 51 format "99/99/9999"
        tt_item_bord_ap_imprimir.val_pagto to 76 format "->>>,>>>,>>9.99"
        tt_item_bord_ap_imprimir.val_multa_tit_ap to 89 format "->>>>,>>9.99"
        tt_item_bord_ap_imprimir.val_juros to 102 format "->>>>,>>9.99"
        tt_item_bord_ap_imprimir.val_cm_tit_ap to 118 format "->>>>,>>9.99" skip.
    put stream s_1 unformatted 
        v_cod_label to 12
        v_cod_cgc_cpf  at 14 format v_cod_format
        tt_item_bord_ap_imprimir.dat_vencto_tit_ap at 51 format "99/99/9999"
        tt_item_bord_ap_imprimir.ttv_val_tot_impto_retid to 76 format "->>>>,>>9.99"
        tt_item_bord_ap_imprimir.val_desc_tit_ap to 89 format "->>>>,>>9.99"
        tt_item_bord_ap_imprimir.val_abat_tit_ap to 102 format "->>>>,>>9.99"
        tt_item_bord_ap_imprimir.ttv_val_liq_item_bord to 118 format "->>>,>>>,>>9.99" skip.
    ...
    if v_nom_prog_upc <> '' or v_nom_prog_appc <> '' or v_nom_prog_dpc <> '' then
       run pi_i_exec_program_epc_custom_apb742zb (input 'apos_impressao_dados_bancarios':U).
...
** Alterado por..........: corp45566
** Alterado em...........: 12/04/2011 09:57:20
*****************************************************************************/
...
** Alterado por..........: corp45566
** Alterado em...........: 12/04/2011 11:40:49
*****************************************************************************/
...
** Alterado em...........: 14/03/2012 14:23:19
*****************************************************************************/
            ...
            for each tt_item_bord_ap_imprimir no-lock
                ...
                if last-of(tt_item_bord_ap_imprimir.cdn_fornecedor) then do:
                    ...
                    put stream s_1 unformatted 
                        "---------------------------------------------------------" at 1
                        "-------------------------------------------------------------" at 58 skip
                        v_val_pag_bord_1 to 76 format "->>>,>>>,>>9.99"
                        v_val_multa_pag_bord_1 to 89 format "->>>>,>>9.99"
                        v_val_juros_pag_bord_1 to 102 format "->>>>,>>9.99"
                        v_val_var_mon_pag_bord_1 to 118 format "->>>,>>>,>>9.99" skip
                        "Total de T°tulos por Fornecedor" at 1.
    put stream s_1 unformatted 
                        v_qtd_tot_tit_pag_bord_1 to 44 format ">>>9"
                        v_val_pag_tot_impto_1 to 76 format "->>>,>>>,>>9.99"
                        v_val_desc_pag_bord_1 to 89 format "->>>>,>>9.99"
                        v_val_abat_pag_bord_1 to 102 format "->>>>,>>9.99"
                        v_val_liq_pag_bord_1 to 118 format "->>>,>>>,>>9.99" skip
                        "---------------------------------------------------------" at 1
                        "-------------------------------------------------------------" at 58 skip.
                ...
                if last-of(tt_item_bord_ap_imprimir.cod_forma_pagto) then do:
                    ...
                    put stream s_1 unformatted 
                        "---------------------------------------------------------" at 1
                        "-------------------------------------------------------------" at 58 skip
                        v_val_pag_bord_2 to 76 format "->>>,>>>,>>9.99"
                        v_val_multa_pag_bord_2 to 89 format "->>>>,>>9.99"
                        v_val_juros_pag_bord_2 to 102 format "->>>>,>>9.99"
                        v_val_var_mon_pag_bord_2 to 118 format "->>>,>>>,>>9.99" skip
                        "Total de T°tulos por Forma de Pagamento" at 1.
    put stream s_1 unformatted 
                        v_qtd_tot_tit_pag_bord_2 to 44 format ">>>9"
                        v_val_pag_tot_impto_2 to 76 format "->>>,>>>,>>9.99"
                        v_val_desc_pag_bord_2 to 89 format "->>>>,>>9.99"
                        v_val_abat_pag_bord_2 to 102 format "->>>>,>>9.99"
                        v_val_liq_pag_bord_2 to 118 format "->>>,>>>,>>9.99" skip
                        "---------------------------------------------------------" at 1
                        "-------------------------------------------------------------" at 58 skip.
        ...
        when "Por Estabelecimento/Fornecedor" /*l_por_estabelecimentofornecedor*/  then
            for each tt_item_bord_ap_imprimir no-lock
                by tt_item_bord_ap_imprimir.cod_estab
                by tt_item_bord_ap_imprimir.cdn_fornecedor:
                run pi_corpo_bord_ap_imprimir /*pi_corpo_bord_ap_imprimir*/.
            end.
...
** Alterado em...........: 06/03/2012 17:10:43
*****************************************************************************/
PROCEDURE pi_tratar_tt_item_bord_ap_imprimir_001:
    ...
    then do:
        ...
        assign v_log_forma_pagto_subst = no.
            ...
            if avail item_bord_ap_agrup then do:
                if item_bord_ap_agrup.val_tot_agrup_item_bord_ap >= v_val_lim_forma_pagto then 
                    assign v_cod_forma_pagto_altern = v_cod_forma_pagto_subst
                           v_log_forma_pagto_subst  = yes.
            end.
            else do:
                if item_bord_ap.val_pagto >= v_val_lim_forma_pagto then do:
                    assign v_cod_forma_pagto_altern = v_cod_forma_pagto_subst
                           v_log_forma_pagto_subst  = yes.
                end.    
            end.
...
/*****************************************************************************
** Procedure Interna.....: pi_validacao_de_erros
** Descricao.............: pi_validacao_de_erros
** Criado por............: fut43120
** Criado em.............: 02/12/2008 14:06:38
** Alterado por..........: fut43120
** Alterado em...........: 02/12/2008 16:06:07
*****************************************************************************/
PROCEDURE pi_validacao_de_erros:
    ...
    /* ** Tratamento para erro na traduá∆o do valor por extenso do borderì ***/
        ...
        if not avail agenc_bcia or not avail banco then do:
            if avail cta_corren then do:
     ...
     RETURN "OK" /*l_ok*/  /* Ok */.
END PROCEDURE. /* pi_validacao_de_erros */
/*****************************************************************************
** Procedure Interna.....: pi_i_exec_program_epc_custom_apb742zb
** Descricao.............: pi_i_exec_program_epc_custom_apb742zb
** Criado por............: fut1147
** Criado em.............: 05/08/2014 17:32:52
** Alterado por..........: fut1147
** Alterado em...........: 05/08/2014 17:46:13
*****************************************************************************/
PROCEDURE pi_i_exec_program_epc_custom_apb742zb:
    ...
    /* *****************************************************************************************************************************************************************/
    /* Objetivo..............: Substituir o c¢digo gerado pela include i_exec_program_epc_custom, muitas vezes repetido, com o intuito de evitar estouro de segmento. */
    /* Utilizaá∆o............: A utilizaá∆o desta procedure funciona exatamente como a include anteriormente utilizada para este fim, bastando apenas passar como     */
    /* par†metro o Evento que serˇ executado, conforme exemplo: RUN pi_i_exec_program_epc_custom_apb742zb(INPUT 'INITIALIZE').                */
    /* *****************************************************************************************************************************************************************/
    ...
    case p_cod_event:
        when 'antes_create_temp_table_impressao':U or 
        when 'apos_create_temp_table_impressao':U  then do:
            create tt_epc.
            assign tt_epc.cod_event     = p_cod_event
                   tt_epc.cod_parameter = 'cod_estab_bord':U
                   tt_epc.val_parameter = item_bord_ap.cod_estab_bord.

            create tt_epc.
            assign tt_epc.cod_event     = p_cod_event
                   tt_epc.cod_parameter = 'cod_portador':U
                   tt_epc.val_parameter = item_bord_ap.cod_portador.

            create tt_epc.
            assign tt_epc.cod_event     = p_cod_event
                   tt_epc.cod_parameter = 'num_bord_ap':U
                   tt_epc.val_parameter = string(item_bord_ap.num_bord_ap).

            create tt_epc.
            assign tt_epc.cod_event     = p_cod_event
                   tt_epc.cod_parameter = 'num_seq_bord':U
                   tt_epc.val_parameter = string(item_bord_ap.num_seq_bord).

            if p_cod_event = 'apos_create_temp_table_impressao':U then do:
                create tt_epc.
                assign tt_epc.cod_event     = p_cod_event
                       tt_epc.cod_parameter = 'tt_item_bord_ap_imprimir_buffer_handle':U
                       tt_epc.val_parameter = string(temp-table tt_item_bord_ap_imprimir:default-buffer-handle).
            end.
        end.
        when 'apos_impressao_dados_bancarios':U then do:
            create tt_epc.
            assign tt_epc.cod_event     = p_cod_event
                   tt_epc.cod_parameter = 'tt_item_bord_ap_imprimir_buffer_handle':U
                   tt_epc.val_parameter = string(temp-table tt_item_bord_ap_imprimir:default-buffer-handle).
    ...
    if p_cod_event = 'antes_create_temp_table_impressao':U then do:
       find first tt_epc
            where tt_epc.cod_event     = 'antes_create_temp_table_impressao':U
              and tt_epc.cod_parameter = 'ignora_item_bord_ap':U
              and tt_epc.val_parameter = "yes" /*l_yes*/  no-error.
...
END PROCEDURE. /* pi_i_exec_program_epc_custom_apb742zb */
