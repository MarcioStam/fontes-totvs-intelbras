...
*******************************************************************************/
{include/i-prgvrs.i CD0420 2.00.01.022}  /*** 010122 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i cd0420 MCD}
&ENDIF
...
def var p-ci-estab-ini   as char    no-undo format "x(5)"  init "101".
def var p-ci-estab-fim   as char    no-undo format "x(5)"  init "101".
&ELSE
def var p-ci-estab-ini   as char    no-undo format "x(3)"  init "101".
def var p-ci-estab-fim   as char    no-undo format "x(3)"  init "101".
&ENDIF

def var p-ci-linha-ini   as int     no-undo format ">>9"   init 0.
def var p-ci-linha-fim   as int     no-undo format ">>9"   init 999.
def var p-ci-item-ini    as char    no-undo format "x(16)" init "1010000".
def var p-ci-item-fim    as char    no-undo format "x(16)" init "1999999".
...
/* ASSIGN p-ci-comp-ini = c-seg-usuario  */
/*        p-ci-comp-fim = c-seg-usuario. */
...
DEFINE VARIABLE i-cod-plano AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "Plano":R7 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE l-cred-aprov AS LOGICAL INITIAL no 
...
DEFINE VARIABLE l-ped-crt AS LOGICAL INITIAL no 
...
DEFINE VARIABLE l-res-pla AS LOGICAL INITIAL no 
...
DEFINE VARIABLE c-item-fim AS CHARACTER FORMAT "X(16)":U INITIAL "1999999" 
     VIEW-AS FILL-IN 
     SIZE 22 BY .88 NO-UNDO.

DEFINE VARIABLE c-item-ini AS CHARACTER FORMAT "X(16)":U INITIAL "1010000" 
     LABEL "Item" 
     VIEW-AS FILL-IN 
     SIZE 22 BY .88 NO-UNDO.
...
/* SETTINGS FOR FRAME f-pg-par
   Custom                                                               */
/* SETTINGS FOR FILL-IN i-cod-plano IN FRAME f-pg-par
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       i-cod-plano:HIDDEN IN FRAME f-pg-par           = TRUE.

/* SETTINGS FOR TOGGLE-BOX l-planejada IN FRAME f-pg-par
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       l-planejada:HIDDEN IN FRAME f-pg-par           = TRUE.

/* SETTINGS FOR TOGGLE-BOX l-res-pla IN FRAME f-pg-par
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       l-res-pla:HIDDEN IN FRAME f-pg-par           = TRUE.

/* SETTINGS FOR RECTANGLE RECT-42 IN FRAME f-pg-par
   NO-ENABLE                                                            */
ASSIGN 
       RECT-42:HIDDEN IN FRAME f-pg-par           = TRUE.
...
DO:
  ...
  run cdp/cd0420b1.w (input-output p-cod-obsoleto,
...
DO:
  run cdp/cd0420a1.w (input-output p-ci-estab-ini,
...
DO:
    assign 
/*            l-planejada:sensitive  in frame f-pg-par = param-global.modulo-pl */
...
{utp/ut9000.i "CD0420" "2.00.01.022"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY l-comprado l-fabricado l-ord-com l-ord-prod l-res-com l-sld-est 
          l-sld-ter l-ped-crt l-it-sem-mov l-componente l-deposito l-cred-aprov 
          rs-beneficio rs-tipo l-comp-fabr l-comp-comp 
      WITH FRAME f-pg-par IN WINDOW C-Win.
  ENABLE l-comprado l-fabricado l-ord-com l-ord-prod l-res-com l-sld-est 
         l-sld-ter l-ped-crt l-it-sem-mov l-componente l-deposito l-cred-aprov 
         rs-beneficio rs-tipo l-comp-fabr l-comp-comp bt-parametro RECT-19 
         rt-beneficio rt-lista 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
/*   ASSIGN c-comp-ini:SCREEN-VALUE IN FRAME f-pg-sel = c-seg-usuario  */
/*          c-comp-fim:SCREEN-VALUE IN FRAME f-pg-sel = c-seg-usuario. */
...
/*            tt-param.i-cod-plano    = input frame f-pg-par i-cod-plano */
    ...
    {include/i-rprun.i cdp/cd0420rp.p}
