...
&scoped-define FONTES-WINDOWS \\totvs12\erp\intelbras\fontes\
&scoped-define FONTES-LINUX /opt/totvs/programas/intelbras/fontes/
&scoped-define DESTINO-TESTE \\totvs12\erp\intelbras\teste\
&scoped-define DESTINO-TESTE-LINUX /opt/totvs/programas/intelbras/teste/
&scoped-define DESTINO-LIBERACOES \\totvs12\erp\intelbras\liberacoes\totvs11\
&scoped-define DESTINO-LIBERACOES-LINUX /opt/totvs/programas/intelbras/liberacoes/totvs11/
&scoped-define APPSERVER-TESTE -AppService compilar -H totvs12.intelbras.com.br
...
DEFINE VARIABLE fiFontes AS CHARACTER FORMAT "X(256)":U INITIAL "c:/fontes11/" 
     LABEL "&Base dos fontes" 
     VIEW-AS FILL-IN 
     SIZE 33 BY .79 NO-UNDO.

DEFINE VARIABLE tg32 AS LOGICAL INITIAL no 
     LABEL "32 bit" 
     VIEW-AS TOGGLE-BOX
...
do:
   ...
   do i = 1 to num-entries(input frame fFrame fiProgramas:screen-value):
      ...
      if input frame fOS tg32 then do:
         run pi-acompanhar in h-acomp (input 'Compilando em 32 bit').
      ...
      if input frame fOS tg64 then do:
         if c-programa matches '*.w' then do:
            assign input frame fFrame edLog:screen-value = 'Programa ' + c-programa + ' n∆o compila em character 64 bit!~r~n' + input frame fFrame edLog:screen-value.
            next.
         end.
         ...
         if (return-value = 'NOK') then do:
            assign input frame fFrame edLog:screen-value = 'Erro compilando ' + c-programa + ' em 64 bit: ' + c-erro + '~r~n' + input frame fFrame edLog:screen-value.
            next.
         end.
         ...
         assign c-destino = (if input frame fDestino rsDestino = 1 then "{&DESTINO-TESTE}" else "{&DESTINO-LIBERACOES}")
                c-mkdir = c-destino + (if input frame fDestino tgPasta and input frame fDestino fiPasta:screen-value <> '' then
                                                  ...
                                                  '')
                                             + 'char64\' + replace(c-programa, input frame fFrame fiFontes:screen-value, '')
                c-mkdir = replace(c-mkdir, '/', '~\')
                c-mkdir = substring(c-mkdir, 1, r-index(c-mkdir, '~\') - 1).
         ...
         if hProc:connected() then do:
            ...
            if (return-value = 'NOK') then
               ...
               assign input frame fFrame edLog:screen-value = 'Programa ' + c-programa + ' compilado em 64 bit com sucesso!~r~n' + input frame fFrame edLog:screen-value.
         end.
         else do:
         ...
         for each tt-include no-lock:
             assign c-mkdir = "{&DESTINO-TESTE}" + 'char64\' + replace(tt-include.arquivo, input frame fFrame fiFontes:screen-value, '')
                    c-mkdir = replace(c-mkdir, '/', '~\').
...
------------------------------------------------------------------------------*/
   ...
   assign c-xref = substring(pFonte, 1, length(pFonte) - 2) + '.xref'.
