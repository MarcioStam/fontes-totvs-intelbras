...
{include/i-freeac.i}    
    ...
    FUNCTION replaceEspecialChars RETURN CHARACTER (INPUT pcTexto AS CHAR):
            ASSIGN pcTexto_2 =  "".
            DO i_cont = 1 TO LENGTH(pcTexto):
                IF  substring(pcTexto,i_cont,1) = "A" OR
                    substring(pcTexto,i_cont,1) = "B" OR     
                    substring(pcTexto,i_cont,1) = "C" OR     
                    substring(pcTexto,i_cont,1) = "D" OR     
                    substring(pcTexto,i_cont,1) = "E" OR     
                    substring(pcTexto,i_cont,1) = "F" OR     
                    substring(pcTexto,i_cont,1) = "G" OR     
                    substring(pcTexto,i_cont,1) = "H" OR     
                    substring(pcTexto,i_cont,1) = "I" OR     
                    substring(pcTexto,i_cont,1) = "J" OR     
                    substring(pcTexto,i_cont,1) = "K" OR     
                    substring(pcTexto,i_cont,1) = "L" OR     
                    substring(pcTexto,i_cont,1) = "M" OR     
                    substring(pcTexto,i_cont,1) = "N" OR     
                    substring(pcTexto,i_cont,1) = "O" OR     
                    substring(pcTexto,i_cont,1) = "P" OR     
                    substring(pcTexto,i_cont,1) = "Q" OR     
                    substring(pcTexto,i_cont,1) = "R" OR     
                    substring(pcTexto,i_cont,1) = "S" OR     
                    substring(pcTexto,i_cont,1) = "T" OR     
                    substring(pcTexto,i_cont,1) = "U" OR     
                    substring(pcTexto,i_cont,1) = "V" OR     
                    substring(pcTexto,i_cont,1) = "W" OR     
                    substring(pcTexto,i_cont,1) = "X" OR     
                    substring(pcTexto,i_cont,1) = "Y" OR     
                    substring(pcTexto,i_cont,1) = "Z" OR     
                    
                    substring(pcTexto,i_cont,1) = "1" OR     
                    substring(pcTexto,i_cont,1) = "2" OR     
                    substring(pcTexto,i_cont,1) = "3" OR     
                    substring(pcTexto,i_cont,1) = "4" OR     
                    substring(pcTexto,i_cont,1) = "5" OR     
                    substring(pcTexto,i_cont,1) = "6" OR     
                    substring(pcTexto,i_cont,1) = "7" OR     
                    substring(pcTexto,i_cont,1) = "8" OR     
                    substring(pcTexto,i_cont,1) = "9" OR     
                    substring(pcTexto,i_cont,1) = "0" OR
                    substring(pcTexto,i_cont,1) = " " OR
                    ...
                    substring(pcTexto,i_cont,1) = "-" OR     
                    substring(pcTexto,i_cont,1) = ")" OR     
                    substring(pcTexto,i_cont,1) = "(" THEN

                ASSIGN pcTexto_2 = pcTexto_2 + substring(pcTexto,i_cont,1).
            END.
            ASSIGN pcTexto = pcTexto_2.                                    
...
input stream s-import from value("C:\temp\Faltando_2.csv") CONVERT SOURCE SESSION:CHARSET.

OUTPUT STREAM s-export TO c:\temp\Faltando_2.lst. /*CONVERT TARGET "iso8859-1".*/
...
repeat on error undo, leave
    ...
    import stream s-import delimiter ";" 
        c-sequencia c-tipo c-nome c-cli-contribuinte c-ins-estadual c-insc-municipal c-cgc c-suframa c-endereco c-nro-end  c-fone-1 c-fone-2 
        c-comple-endereco c-cep c-bairro c-cidade c-uf c-pais c-email .

    ASSIGN c-cgc = get_cgc()
           c-identific = "".

    
    ASSIGN c-nome            = upper(replaceEspecialChars(c-nome)).
    ASSIGN c-cidade          = upper(replaceEspecialChars(c-cidade)).
    ASSIGN c-endereco        = upper(replaceEspecialChars(c-endereco)).
    ASSIGN c-comple-endereco = upper(replaceEspecialChars(c-comple-endereco))
           c-pais            = UPPER(c-pais)
           c-bairro            = UPPER(c-bairro).
...
/*    DISPLAY STREAM s-export c-sequencia c-nome c-cgc c-ie c-cidade c-uf WITH WIDTH 200.*/
    ...
    IF AVAIL emitente THEN NEXT. 
    
    IF AVAIL emitente THEN DO:
        IF c-tipo = "1" THEN
            IF  emitente.identific = 2 OR
                emitente.identific = 3 then
                ASSIGN c-identific = "3".
            ELSE
                ASSIGN c-identific = "1".
        ELSE
            IF c-tipo = "2" THEN
                IF  emitente.identific = 1 OR
                    emitente.identific = 3 then
                    ASSIGN c-identific = "3".
                ELSE
                    ASSIGN c-identific = "2".
...
/*
    ...
    FIND emitente WHERE emitente.nome-abrev = trim(substring(c-nome,1,7) + SUBSTRING(c-cgc,10,5)) NO-LOCK NO-ERROR.
    IF NOT AVAIL emitente THEN
        ASSIGN c-nome-abrev = trim(substring(c-nome,1,7) + SUBSTRING(c-cgc,10,5)).
    ELSE DO :
        FIND emitente WHERE emitente.nome-abrev = trim(substring(c-nome,1,6) + SUBSTRING(c-cgc,9,6)) NO-LOCK NO-ERROR.
        IF NOT AVAIL emitente THEN DO:
        
            ASSIGN c-nome-abrev = trim(substring(c-nome,1,6) + SUBSTRING(c-cgc,9,6)).
        END.
        ELSE DO:
            FIND emitente WHERE emitente.nome-abrev = trim(substring(c-nome,1,10) + "-B") NO-LOCK NO-ERROR.
            IF NOT AVAIL emitente THEN
                ASSIGN c-nome-abrev = trim(substring(c-nome,1,10) + "-B").
            ELSE DO :
                FIND emitente WHERE emitente.nome-abrev = trim(substring(c-nome,1,10) + "-C") NO-LOCK NO-ERROR.
                IF NOT AVAIL emitente THEN
                    ASSIGN c-nome-abrev = trim(substring(c-nome,1,10) + "-C").
            END.
...
/*    DISPLAY STREAM s-export c-sequencia c-nome c-cgc c-ie c-cidade c-uf c-identific  NO-LABEL WITH WIDTH 200.*/
    ...
    put STREAM s-export
        ...
        c-nome-abrev FORMAT "X(12)"                       /* 2 - Nome abreviado do Cliente/Fornecedor */
        c-cgc FORMAT "X(19)"                                        /* 3 - C.G.C.M.F./C.I.C. */
        c-identific format "x(01)".       /* 4 - Identificaá∆o */
    ...
    case length(trim(c-cgc)):                                       /* 5 - Natureza */
         ...
         otherwise put STREAM s-export "3".
    end case.    
    IF c-pais <> "Brasil" THEN 
       ASSIGN c-uf = "EX".
    ...
    IF c-pais <> "Brasil" THEN DO:
        put STREAM s-export
                    "90". /*         IF c-identific = "2" OR c-identific = "3" THEN "03" ELSE "  "                       /* 19 - C¢digo do Grupo do Fornecedor */ */
    END.
    ELSE  put STREAM s-export
                    "01". /*         IF c-identific = "2" OR c-identific = "3" THEN "03" ELSE "  "                       /* 19 - C¢digo do Grupo do Fornecedor */ */
     ...
     put STREAM s-export
        ...
        (if c-cli-contribuinte = "true" then "1" else "2") format "x(1)"        /* 27 - Contribuinte ICMS */
    ...
    IF c-identific = "1" OR c-identific = "3" THEN 
        PUT STREAM s-export "95".
    ELSE PUT STREAM s-export "00".                                                        /* 34 - Grupo de cliente = 95 - A Definir */ 


   PUT STREAM s-export 
        fill("00000000100", 11) format "x(11)"                                /* 35 - Limite de CrÇdito */
        ...
        "Importado da Automatiza " FORMAT "x(150)"                              /* 44 - Observaá∆o 1 */
        fill("0000 ", 4) format "x(4)"                                  /* 45 - Percentual Minimo Por Faturamento Parcial */
        "1"                                                         /* 46 - Meio Para Emiss∆o de Pedido de Compra */
        c-nome-abrev FORMAT "X(12)"                       /* 47 - Nome Fantasia da Matriz do Cliente */
...
/*        fill(" ", 25) format "x(25)"                                /* 81 - Cidade Cobranáa */*/
        ...
        "0" format "x(3)"                                  /* 86 - Banco do Cliente/Fornecedor */
...
/*        fill(" ", 20) format "x(20)"                                /* 113 - Pa°s de Cobranáa */*/
        ...
        fill("S", 1) format "x(1)"                                  /* 122 - Retem Pagto */   
