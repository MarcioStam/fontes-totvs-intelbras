...
*******************************************************************************/
{include/i-prgvrs.i BOES002 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES002
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  acao
&GLOBAL-DEFINE TableLabel  acao                            
...
define variable v-ini-cod-acao as integer no-undo.
define variable v-fim-cod-acao as integer no-undo.
...
DEFINE INPUT PARAMETER p-cod-acao AS integer NO-UNDO.                                                                           
    ...
    FIND bf{&TableName} NO-LOCK
        WHERE bf{&TableName}.cod-acao = p-cod-acao                    
        NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryAcao DBOProgram 
PROCEDURE openQueryAcao :
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryFaixaAcao DBOProgram 
PROCEDURE openQueryFaixaAcao :
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-acao >= v-ini-cod-acao                      
        AND   {&TableName}.cod-acao <= v-fim-cod-acao                      
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintAcao DBOProgram 
PROCEDURE setConstraintAcao :
DEFINE INPUT PARAMETER p-cod-acao AS integer NO-UNDO.                                                                           
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintFaixaAcao DBOProgram 
PROCEDURE setConstraintFaixaAcao :
DEFINE INPUT PARAMETER p-ini-cod-acao AS integer NO-UNDO.                                                                           
    DEFINE INPUT PARAMETER p-fim-cod-acao AS integer NO-UNDO.                                                                           

    ASSIGN 
    v-ini-cod-acao = p-ini-cod-acao                                           
        v-fim-cod-acao = p-fim-cod-acao                                           
    .
    RETURN "OK":U.
END PROCEDURE.
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-acao = RowObject.cod-acao) THEN DO:
