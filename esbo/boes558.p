...
*******************************************************************************/
{include/i-prgvrs.i BOES558 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES558
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  categoria-b2c
&GLOBAL-DEFINE TableLabel  Categoria Ikeda               
...
define variable v-cod-categoria as character no-undo.
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-categoria = RowObject.cod-categoria) THEN DO:
    ...
    if (pType = 'Create') and (RowObject.cod-categoria-pai <> '') and not can-find(first bf{&TableName}
                                                                                   where bf{&TableName}.cod-categoria = RowObject.cod-categoria-pai) then do:
       {method/svc/errors/inserr.i
           &ErrorNumber="17006"
           &ErrorType="EMS"
           &ErrorSubType="ERROR"
           &ErrorParameters="'Categoria pai informada inexistente'"
       }
    end.

    if ((pType = 'Create') or (pType = 'Update')) and (RowObject.nome = '') then do:
       {method/svc/errors/inserr.i
           &ErrorNumber="17006"
           &ErrorType="EMS"
           &ErrorSubType="ERROR"
           &ErrorParameters="'Descri‡Æo da categoria nÆo pode ficar em branco'"
       }
    end.
...
PROCEDURE beforeDeleteRecord :                                                                                                            
   ...
   if can-find (first categoria-item-b2c
                where categoria-item-b2c.cod-categoria = RowObject.cod-categoria) then do
      transaction on error undo, return error 'nok':
      
      for each categoria-item-b2c exclusive-lock
         where categoria-item-b2c.cod-categoria = RowObject.cod-categoria
         on error undo, return error 'nok':
         delete categoria-item-b2c.
      end.
   end.
END PROCEDURE.
...
PROCEDURE getCharField :                                                                                                            
    ...
    CASE pFieldName:
        WHEN "cod-categoria":U THEN ASSIGN pFieldValue = RowObject.cod-categoria.
        WHEN "cod-categoria-pai":U THEN ASSIGN pFieldValue = RowObject.cod-categoria-pai.
...
PROCEDURE setConstraintCh-pri :                                                                                                     
    DEFINE INPUT PARAMETER p-cod-categoria AS character NO-UNDO.                                                                    

    ASSIGN 
    v-cod-categoria = p-cod-categoria                                 
    .
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE openQueryCh-pri :                                                                                                         
    ...
    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-categoria = v-cod-categoria            
    .
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE getKey :
/*------------------------------------------------------------------------------
  Purpose:     Retorna valores dos campos do ¡ndice cod-seq
  Parameters:  
               retorna valor do campo cod-programa
               retorna valor do campo sequencia
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER p-cod-categoria LIKE categoria-b2c.cod-categoria NO-UNDO.
    ...
    ASSIGN p-cod-categoria = RowObject.cod-categoria
    .
