...
&GLOBAL-DEFINE TableName num-serie
...
DEFINE VARIABLE c-n-serie-ini AS CHARACTER   NO-UNDO.
DEFINE VARIABLE c-n-serie-fim AS CHARACTER   NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        ...
        WHEN "motiv-re":U THEN ASSIGN pFieldValue = RowObject.motiv-re.
        WHEN "n-serie":U THEN ASSIGN pFieldValue = RowObject.n-serie.
        WHEN "ns-keycode":U THEN ASSIGN pFieldValue = RowObject.ns-keycode.
        WHEN "us-ult-re":U THEN ASSIGN pFieldValue = RowObject.us-ult-re.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pn-serie LIKE num-serie.n-serie NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pn-serie LIKE num-serie.n-serie NO-UNDO.

    FIND FIRST bfnum-serie WHERE 
        bfnum-serie.n-serie = pn-serie NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfnum-serie THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfnum-serie)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryNumSerieItCodigo DBOProgram 
PROCEDURE openQueryNumSerieItCodigo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK 
    WHERE {&TableName}.n-serie >= c-n-serie-ini
    AND   {&TableName}.n-serie <= c-n-serie-fim
    AND   {&TableName}.it-codigo >= c-it-codigo-ini
    AND   {&TableName}.it-codigo <= c-it-codigo-fim
    BY {&TableName}.n-serie
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintNumSerieItCodigo DBOProgram 
PROCEDURE setConstraintNumSerieItCodigo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER p-n-serie-ini    AS CHAR     NO-UNDO.
    DEFINE INPUT PARAMETER p-n-serie-fim    AS CHAR     NO-UNDO.
    DEFINE INPUT PARAMETER p-it-codigo-ini    AS CHAR     NO-UNDO.
    DEFINE INPUT PARAMETER p-it-codigo-fim    AS CHAR     NO-UNDO.

    ASSIGN c-n-serie-ini = p-n-serie-ini
           c-n-serie-fim = p-n-serie-fim
