...
*******************************************************************************/
{include/i-prgvrs.i BOES128 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES128
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  monta-estrutura
&GLOBAL-DEFINE TableLabel  monta-estrutura               
...
define variable v-es-codigo as character no-undo.
define variable v-montadora as integer no-undo.
...
DEFINE INPUT PARAMETER pFieldName AS CHARACTER NO-UNDO.
    ...
    CASE pFieldName:
        ...
        WHEN "posicao":U THEN ASSIGN pFieldValue = RowObject.posicao.
...
DEFINE INPUT PARAMETER pFieldName AS CHARACTER NO-UNDO.
    ...
    CASE pFieldName:
        WHEN "montadora":U THEN ASSIGN pFieldValue = RowObject.montadora.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
...
DEFINE INPUT PARAMETER iAbertura AS INTEGER NO-UNDO.
    ...
    CASE iAbertura:
        ...
        WHEN 2 THEN           
            RUN openQueryStatic ("It-codigo":U).                      
    END CASE.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryByCodigo DBOProgram 
PROCEDURE openQueryByCodigo :
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.it-codigo = v-it-codigo                    
        AND {&TableName}.es-codigo = v-es-codigo                      
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintByCodigo DBOProgram 
PROCEDURE setConstraintByCodigo :
DEFINE INPUT PARAMETER p-it-codigo AS character NO-UNDO.                                                                        
    DEFINE INPUT PARAMETER p-es-codigo AS character NO-UNDO.                                                                        

    ASSIGN 
    v-it-codigo = p-it-codigo                                         
    v-es-codigo = p-es-codigo                                         
    .
    RETURN "OK":U.
END PROCEDURE.
