...
&GLOBAL-DEFINE DBOName BOES790
&GLOBAL-DEFINE DBOVersion 1.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName grupo-canais
&GLOBAL-DEFINE TableLabel 
&GLOBAL-DEFINE QueryName qrgrupo-canais 
...
DEFINE BUFFER bfgrupo-canais FOR {&TableName}.
DEF VAR v-ini-cod-gr-canais      LIKE grupo-canais.cod-gr-canais      NO-UNDO.
DEF VAR v-fim-cod-gr-canais      LIKE grupo-canais.cod-gr-canais      NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-gr-canais LIKE grupo-canais.cod-gr-canais NO-UNDO.    
    ...
    ASSIGN pcod-gr-canais = RowObject.cod-gr-canais.
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST bfgrupo-canais WHERE 
        bfgrupo-canais.cod-gr-canais = pcod-gr-canais NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro será retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfgrupo-canais THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query através de rowid e verifica a ocorrência de erros, caso
          existam erros será retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfgrupo-canais)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQuerybyGrup DBOProgram 
PROCEDURE openQuerybyGrup:
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK WHERE
     {&TableName}.cod-gr-canais >= v-ini-cod-gr-canais AND
     {&TableName}.cod-gr-canais <= v-fim-cod-gr-canais.
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAM p-ini-cod-gr-canais    LIKE grupo-canais.cod-gr-canais NO-UNDO.
    DEF INPUT PARAM p-fim-cod-gr-canais    LIKE grupo-canais.cod-gr-canais NO-UNDO.

    ASSIGN v-ini-cod-gr-canais = p-ini-cod-gr-canais
           v-fim-cod-gr-canais = p-fim-cod-gr-canais.
