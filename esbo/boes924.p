...
*******************************************************************************/
{include/i-prgvrs.i BOES924  2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES924
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  int-vtex-estab
&GLOBAL-DEFINE TableLabel  int-vtex-estab                 
...
/*:T--- Este include deve ser copiado para o diret¢rio do DBO e, ainda, seu nome
      ...
      extensÆo .i ---*/
/*{esbo/BOES924.i RowObject}*/

DEFINE TEMP-TABLE RowObject NO-UNDO LIKE int-vtex-estab
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH int-vtex-estab-depos EXCLUSIVE-LOCK
       WHERE int-vtex-estab-depos.cod-estabel = RowObject.cod-estabel:
        DELETE int-vtex-estab-depos.
    END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER p-cod-emitente LIKE {&TableName}.cod-estabel NO-UNDO.
    ...
    ASSIGN p-cod-emitente = RowObject.cod-estabel.
...
PROCEDURE openQueryRangeEstabelec:
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    OPEN QUERY qr{&TableName} FOR EACH {&TableName}
                                  ...
                                  AND   {&TableName}.cod-estabel <= c-cod-estabel-fim NO-LOCK.
...
PROCEDURE setConstraintRangeEstabelec :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    ASSIGN c-cod-estabel-ini = p-estab-ini
           c-cod-estabel-fim = p-estab-fim.
...
DEFINE INPUT PARAMETER p-cod-estab AS CHAR NO-UNDO.                                                                          
    ...
    FIND bf{&TableName} NO-LOCK
        WHERE bf{&TableName}.cod-estabel = p-cod-estab                  
        NO-ERROR.
...
OPEN QUERY {&QueryName} 
        FOR EACH {&TableName} NO-LOCK
           WHERE {&TableName}.cod-estab >= c-cod-estabel-ini
             AND {&TableName}.cod-estab <= c-cod-estabel-fim.
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE setConstraintch-chave :
    ...
    ASSIGN v-cod-estabel  = p-cod-estab.
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-estab = RowObject.cod-estab) THEN DO:
             ...
             IF  NOT CAN-FIND(estabelec
                                WHERE estabelec.cod-estab = RowObject.cod-estab) THEN DO:
                 {method/svc/errors/inserr.i
                     &ERRORNUMBER = "2"
                     &ERRORTYPE = "EMS"
                     &ErrorSubType="ERROR"
