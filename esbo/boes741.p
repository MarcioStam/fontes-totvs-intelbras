...
&GLOBAL-DEFINE DBOName BOES741
&GLOBAL-DEFINE DBOVersion 1.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName natureza-ped-fiscal
&GLOBAL-DEFINE TableLabel 
&GLOBAL-DEFINE QueryName qrnatureza-ped-fiscal 
...
DEFINE BUFFER bfnatureza-ped-fiscal FOR {&TableName}.
DEF VAR v-ini-natureza      LIKE natureza-ped-fiscal.natureza      NO-UNDO.
DEF VAR v-fim-natureza      LIKE natureza-ped-fiscal.natureza      NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pnatureza LIKE natureza-ped-fiscal.natureza NO-UNDO.    
    ...
    ASSIGN pnatureza = RowObject.natureza.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "lib-auto":U           THEN ASSIGN pFieldValue = RowObject.lib-auto.
        WHEN "lib-monitor":U        THEN ASSIGN pFieldValue = RowObject.lib-monitor.
        WHEN "informa-receptor":U   THEN ASSIGN pFieldValue = RowObject.informa-receptor.
        WHEN "informa-finalidade":U THEN ASSIGN pFieldValue = RowObject.informa-finalidade.
        OTHERWISE RETURN "NOK":U.
    END CASE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pnatureza LIKE natureza-ped-fiscal.natureza  NO-UNDO.    

    FIND FIRST bfnatureza-ped-fiscal WHERE 
        bfnatureza-ped-fiscal.natureza = pnatureza NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro será retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfnatureza-ped-fiscal THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query através de rowid e verifica a ocorrência de erros, caso
          existam erros será retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfnatureza-ped-fiscal)).
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK WHERE
     {&TableName}.natureza >= v-ini-natureza AND
     {&TableName}.natureza <= v-fim-natureza.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintByNat DBOProgram 
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAM p-ini-natureza    LIKE natureza-ped-fiscal.natureza NO-UNDO.
    DEF INPUT PARAM p-fim-natureza    LIKE natureza-ped-fiscal.natureza NO-UNDO.

    ASSIGN v-ini-natureza = p-ini-natureza
           v-fim-natureza = p-fim-natureza.
