...
*******************************************************************************/
{include/i-prgvrs.i BOES466 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES466
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  acordo-crescimento
&GLOBAL-DEFINE TableLabel  acordo-crescimento              
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.nr-acordo = RowObject.nr-acordo
    ...
    IF pType = "Create" OR pType = "Update" THEN DO:
        FIND FIRST bf{&TableName} NO-LOCK
             WHERE bf{&TableName}.nr-acordo      = RowObject.nr-acordo
               AND bf{&TableName}.per-cresc-ini <= RowObject.per-cresc-ini
               AND bf{&TableName}.per-cresc-fim >= RowObject.per-cresc-ini NO-ERROR.
        IF NOT AVAIL bf{&TableName} THEN
            FIND FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.nr-acordo      = RowObject.nr-acordo
                   AND bf{&TableName}.per-cresc-ini <= RowObject.per-cresc-fim
                   AND bf{&TableName}.per-cresc-fim >= RowObject.per-cresc-fim NO-ERROR.
        IF AVAIL bf{&TableName} THEN DO:
            {method/svc/errors/inserr.i
                &ErrorNumber="17006"
                &ErrorType="EMS"
                &ErrorSubType="ERROR"
                &ErrorParameters="'Faixa Percentual inv lida.~~~~Faixa Percentual nÆo pode estar no intervalo de outra faixa j  cadastrada.'"}
        END.
    END.
...
PROCEDURE getDecField :                                                                                                             
    ...
    CASE pFieldName:
        WHEN "per-acordo":U THEN ASSIGN pFieldValue = RowObject.per-acordo.
        WHEN "per-cresc-fim":U THEN ASSIGN pFieldValue = RowObject.per-cresc-fim.
        WHEN "per-cresc-ini":U THEN ASSIGN pFieldValue = RowObject.per-cresc-ini.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE linkToContrato DBOProgram 
PROCEDURE linkToContrato :
DEFINE INPUT PARAMETER pHandle AS HANDLE NO-UNDO.
   
   RUN getKey IN pHandle (OUTPUT v-nr-acordo).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryCrescimento DBOProgram
PROCEDURE openQueryCrescimento:
