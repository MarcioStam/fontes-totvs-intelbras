...
*******************************************************************************/
{include/i-prgvrs.i BOES188 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES188
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  tipo-lote
&GLOBAL-DEFINE TableLabel  tipo-lote                       
...
define variable v-cod-lote as integer no-undo.
define variable v-ini-cod-lote as integer no-undo.
define variable v-fim-cod-lote as integer no-undo.
...
DEFINE INPUT PARAMETER pFieldName AS CHARACTER NO-UNDO.
    ...
    CASE pFieldName:
        ...
        WHEN "tipo-antecessor":U THEN ASSIGN pFieldValue = RowObject.tipo-antecessor.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE goToKey :
DEFINE INPUT PARAMETER p-cod-lote AS integer NO-UNDO.                                                                           

    FIND bf{&TableName} NO-LOCK
        WHERE bf{&TableName}.cod-lote = p-cod-lote                    
        NO-ERROR.
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-lote = v-cod-lote                      
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryFaixaCodigo DBOProgram 
PROCEDURE openQueryFaixaCodigo :
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-lote >= v-ini-cod-lote                      
        AND   {&TableName}.cod-lote <= v-fim-cod-lote                      
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintFaixaCodigo DBOProgram 
PROCEDURE setConstraintFaixaCodigo :
DEFINE INPUT PARAMETER p-ini-cod-lote AS integer NO-UNDO.                                                                           
    DEFINE INPUT PARAMETER p-fim-cod-lote AS integer NO-UNDO.                                                                           

    ASSIGN 
    v-ini-cod-lote = p-ini-cod-lote                                           
        v-fim-cod-lote = p-fim-cod-lote                                           
    .
    RETURN "OK":U.
END PROCEDURE.
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-lote = RowObject.cod-lote) THEN DO:
