...
&GLOBAL-DEFINE TableName wm-aloca-saldo
&GLOBAL-DEFINE TableLabel wm-aloca-saldo
...
DEF VAR icod-estab LIKE wm-aloca-saldo.cod-estab.
DEF VAR icod-local LIKE wm-aloca-saldo.cod-local.
DEF VAR icod-item  LIKE wm-aloca-saldo.cod-item.

DEF VAR ini-cod-estab LIKE wm-aloca-saldo.cod-estab.
DEF VAR fim-cod-estab LIKE wm-aloca-saldo.cod-estab.
DEF VAR ini-cod-local LIKE wm-aloca-saldo.cod-local.
DEF VAR fim-cod-local LIKE wm-aloca-saldo.cod-local.
DEF VAR ini-it-codigo LIKE wm-aloca-saldo.cod-item.
DEF VAR fim-it-codigo LIKE wm-aloca-saldo.cod-item.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "dt-transacao":U THEN ASSIGN pFieldValue = RowObject.dt-transacao.
        OTHERWISE RETURN "NOK":U.
    END CASE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-estabel LIKE wm-aloca-saldo.cod-estabel NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-local   LIKE wm-aloca-saldo.cod-local   NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-item    LIKE wm-aloca-saldo.cod-item    NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pcod-estabel LIKE wm-aloca-saldo.cod-estabel NO-UNDO.
    DEFINE INPUT PARAMETER pcod-local   LIKE wm-aloca-saldo.cod-local   NO-UNDO.
    DEFINE INPUT PARAMETER pcod-item    LIKE wm-aloca-saldo.cod-item    NO-UNDO.

    FIND FIRST bfwm-aloca-saldo WHERE 
               bfwm-aloca-saldo.cod-estabel = pcod-estabel AND 
               bfwm-aloca-saldo.cod-local   = pcod-local   AND
               bfwm-aloca-saldo.cod-item    = pcod-item
               NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfwm-aloca-saldo THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfwm-aloca-saldo)).
