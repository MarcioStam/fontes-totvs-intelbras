...
*******************************************************************************/
{include/i-prgvrs.i boes923 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE TableName  int-vtex-estab-depos
&GLOBAL-DEFINE TableLabel  int-vtex-estab-depos                     
...
/*:T--- Este include deve ser copiado para o diret¢rio do DBO e, ainda, seu nome
 ...
 DEFINE TEMP-TABLE RowObject NO-UNDO LIKE int-vtex-estab-depos
...
define variable v-cod-estabel as CHAR no-undo.
define variable v-cod-depos as CHAR no-undo.
...
------------------------------------------------------------------------------*/
    ...
    THEN DO:
        {method/svc/errors/inserr.i                                                      
            ...
            &ErrorType="EMS"                                                                 
            &ErrorSubType="ERROR"                                                            
            &ErrorParameters="'Dep¢sito inexistente.~~NÆo encontrado dep¢sito no cadastro.'"}       
    END.
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-estab = RowObject.cod-estab
                   AND bf{&TableName}.cod-depos = RowObject.cod-depos) THEN DO:
                 {method/svc/errors/inserr.i
                     &ErrorNumber="1"
                     &ErrorType="EMS"
                     &ErrorSubType="ERROR"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintCh-Cod-deposito DBOProgram                                                        
PROCEDURE setConstraintCh-Cod-deposito :                                                                                                  
    DEFINE INPUT PARAMETER p-cod-emitente AS CHAR NO-UNDO.                                                                          
    DEFINE INPUT PARAMETER p-cod-deposito AS CHAR NO-UNDO.                                                                        

    ASSIGN 
    v-cod-estabel = p-cod-emitente                                         
    v-cod-depos   = p-cod-deposito                                     
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryCh-Cod-deposito DBOProgram                                                            
PROCEDURE openQueryCh-Cod-deposito :                                                                                                      

    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-estab = v-cod-estabel                    
...
PROCEDURE openQuery :
    ...
    CASE iAbertura:
        ...
        WHEN 2 THEN           
            RUN openQueryStatic ("Ch-Cod-deposito":U).                      
    END CASE.
END PROCEDURE.
...
PROCEDURE goToKey :
    DEFINE INPUT PARAMETER p-cod-estabelec  AS CHAR NO-UNDO.                                                                          
    DEFINE INPUT PARAMETER p-cod-depos AS CHAR NO-UNDO.                                                                        

    FIND bf{&TableName} NO-LOCK
        WHERE bf{&TableName}.cod-estab = p-cod-estabelec                  
