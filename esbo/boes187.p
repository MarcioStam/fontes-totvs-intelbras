...
&GLOBAL-DEFINE DBOName BOES187
&GLOBAL-DEFINE DBOVersion 1.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName tipo-local
&GLOBAL-DEFINE TableLabel tipo-local 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    define output parameter p-cod-estabel like tipo-local.cod-estabel no-undo.
    DEFINE OUTPUT PARAMETER p-cod-tipo LIKE tipo-local.cod-tipo NO-UNDO.
    ...
    ASSIGN p-cod-tipo = RowObject.cod-tipo
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "unico":U THEN ASSIGN pFieldValue = RowObject.unico.
       /* WHEN "entreposto":U THEN ASSIGN pFieldValue = RowObject.entreposto. */
        OTHERWISE RETURN "NOK":U.
    END CASE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEF INPUT PARAM p-cod-estabel LIKE tipo-local.cod-estabel NO-UNDO.
    DEF INPUT PARAM p-cod-tipo LIKE tipo-local.cod-tipo NO-UNDO.

    FIND FIRST bftipo-local 
         WHERE bftipo-local.cod-estabel = p-cod-estabel
           and bftipo-local.cod-tipo = p-cod-tipo NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bftipo-local THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bftipo-local)).
...
------------------------------------------------------------------------------*/
    DEF INPUT PARAM p-cod-estabel LIKE tipo-local.cod-estabel NO-UNDO.
    DEF INPUT PARAM p-ini-cod-tipo LIKE tipo-local.cod-tipo NO-UNDO.
    DEF INPUT PARAM p-fim-cod-tipo LIKE tipo-local.cod-tipo NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    IF pType = "Create" THEN DO:
        IF CAN-FIND(bf{&TableName}
              WHERE bf{&TableName}.cod-estabel = RowObject.cod-estabel
                and bf{&TableName}.cod-tipo = RowObject.cod-tipo) THEN DO:
            {method/svc/errors/inserr.i
            &ErrorNumber="2"
            &ErrorType="outros"
            &ErrorSubType="ERROR"
            &ErrorDescription="Tipo j† cadastrado no sistema."}
        END.
        IF rowObject.cod-tipo = 0 OR RowObject.descricao = "" THEN DO:
            {method/svc/errors/inserr.i
            &ErrorNumber="2"
            &ErrorType="outros"
            &ErrorSubType="ERROR"
            &ErrorDescription="ê obrigat¢rio preencher os campos C¢digo tipo e Descriá∆o."}
        END.
    ...
    IF pType = "Delete" THEN DO:
        IF rowObject.cod-tipo = 0 THEN DO:
            {method/svc/errors/inserr.i
                &ErrorNumber="2"
                &ErrorType="outros"
                &ErrorSubType="ERROR"
                &ErrorDescription="Este tipo n∆o pode ser exclu°do"}
        END.
    END.
