...
*******************************************************************************/
{include/i-prgvrs.i BOES469 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES469
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  juridico-varas
&GLOBAL-DEFINE TableLabel  Varas Jur¡dicas               
...
define variable v-descricao as character no-undo.
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.codigo = RowObject.codigo) THEN DO:
    ...
    IF pType = "Create" OR pType = "Update" THEN DO:
        IF RowObject.codigo = ? OR RowObject.codigo <= 0 THEN DO:
            {method/svc/errors/inserr.i
                &ErrorNumber="17006"
                &ErrorType="EMS"
                &ErrorSubType="ERROR"
                &ErrorParameters="'C¢digo inv lido.~~~~C¢digo inv lido.'"}
        END.
        ...
        IF RowObject.descricao = "" THEN DO:
            {method/svc/errors/inserr.i
                ...
                &ErrorType="EMS"
                &ErrorSubType="ERROR"
                &ErrorParameters="'Descri‡Æo inv lida.~~~~Descri‡Æo deve ser informada.'"}
        END.

        IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                    WHERE bf{&TableName}.descricao  = RowObject.descricao
                      AND bf{&TableName}.codigo    <> RowObject.codigo) THEN DO:
            {method/svc/errors/inserr.i
                &ErrorNumber="17006"
                &ErrorType="EMS"
                &ErrorSubType="ERROR"
                &ErrorParameters="'Descri‡Æo j  cadastrada em outro c¢digo.~~~~A descri‡Æo informada j  existe em outro c¢digo.'"}
        END.
    END.
...
PROCEDURE getCharField :                                                                                                            
    ...
    CASE pFieldName:
        ...
        WHEN "telefone":U THEN ASSIGN pFieldValue = RowObject.telefone.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintCh-descricao DBOProgram                                                     
PROCEDURE setConstraintCh-descricao :                                                                                               
    DEFINE INPUT PARAMETER p-descricao AS character NO-UNDO.                                                                        

    ASSIGN 
    v-descricao = p-descricao                                         
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryCh-descricao DBOProgram                                                         
PROCEDURE openQueryCh-descricao :                                                                                                   

    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.descricao = v-descricao                    
    .
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE openQuery :
    ...
    CASE iAbertura:
        ...
        WHEN 2 THEN           
            RUN openQueryStatic ("Ch-descricao":U).                   
        WHEN 3 THEN           
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintZoomDescricao DBOProgram 
PROCEDURE setConstraintZoomDescricao :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER p-descricao-ini AS character NO-UNDO.
    DEFINE INPUT PARAMETER p-descricao-fim AS character NO-UNDO.

    ASSIGN v-descricao-ini = p-descricao-ini
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryZoomDescricao DBOProgram 
PROCEDURE openQueryZoomDescricao :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
         WHERE {&TableName}.descricao >= v-descricao-ini
