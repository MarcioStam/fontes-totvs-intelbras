...
*******************************************************************************/
{include/i-prgvrs.i BOES174 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES174
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  sub-familia-item
&GLOBAL-DEFINE TableLabel  Sub-fam¡lia    
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    for each car-familia-item exclusive-lock
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL RowObject THEN DO:
        ...
        ASSIGN c-fam-fim = c-fam-ini + "ZZZ".
        ...
        for each familia EXCLUSIVE-LOCK
            ...
            ASSIGN SUBSTRING(familia.descricao, 1, LENGTH(RowObject.descricao)) = p-desc-nova.
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 ...
                 AND bf{&TableName}.cod-sub-familia = RowObject.cod-sub-familia) THEN DO:
             ...
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 ...
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe subfam¡lia com esta abreviatura",
                                        INPUT "Existe subfam¡lia com esta abreviatura",
                                        INPUT "").                    
             END.
         ...
         WHEN "Delete" THEN DO:
             ...
             IF CAN-FIND(FIRST car-familia-item no-lock
                         WHERE car-familia-item.cod-familia     = RowObject.cod-familia AND
                               car-familia-item.cod-sub-familia = RowObject.cod-sub-familia)
                         THEN DO:
                
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe Caracteristica cadastrada para Sub-Fam¡lia.",
                                        INPUT "Sub-Fam¡lia nÆo pode ser excluida.",
                                        INPUT "").
