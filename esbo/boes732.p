...
{include/i-prgvrs.i BOES732 2.00.00.000}  /*** 010003 ***/

    /*--- Diretrizes de definiá∆o ---*/
&GLOBAL-DEFINE DBOName BOES732
&GLOBAL-DEFINE DBOVersion 2.00.00.000 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName ncm-origem
...
DEFINE VARIABLE c-cod-ncm-ini     AS CHARACTER NO-UNDO.
DEFINE VARIABLE i-codigo-orig-ini AS INTEGER   NO-UNDO.
DEFINE VARIABLE c-cod-ncm-fim     AS CHARACTER NO-UNDO.
DEFINE VARIABLE i-codigo-orig-fim AS INTEGER   NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER p-c-cod-ncm       LIKE ncm-origem.cod-ncm     NO-UNDO.
    DEFINE OUTPUT PARAMETER p-c-codigo-orig   LIKE ncm-origem.codigo-orig NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER p-cod-ncm     LIKE ncm-origem.cod-ncm     NO-UNDO.
    DEFINE INPUT PARAMETER p-codigo-orig LIKE ncm-origem.codigo-orig NO-UNDO.
    
    FIND FIRST bfncm-origem 
        WHERE bfncm-origem.cod-ncm     = p-cod-ncm 
          AND bfncm-origem.codigo-orig = p-codigo-orig
          NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfncm-origem THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfncm-origem)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryncm-origem DBOProgram 
PROCEDURE openQueryncm-origem :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK WHERE
     {&TableName}.cod-ncm     >= c-cod-ncm-ini     AND
     {&TableName}.cod-ncm     <= c-cod-ncm-fim     AND
     {&TableName}.codigo-orig >= i-codigo-orig-ini AND
     {&TableName}.codigo-orig <= i-codigo-orig-fim 
     BY ncm-origem.cod-ncm
     INDEXED-REPOSITION.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT  PARAMETER p-cod-ncm-ini      AS CHARACTER NO-UNDO.
DEFINE INPUT  PARAMETER p-cod-ncm-fim      AS CHARACTER NO-UNDO.
DEFINE INPUT  PARAMETER p-codigo-orig-ini  AS INTEGER   NO-UNDO.
DEFINE INPUT  PARAMETER p-codigo-orig-fim  AS INTEGER   NO-UNDO.
 
 ASSIGN  c-cod-ncm-ini      = p-cod-ncm-ini
         i-codigo-orig-ini  = p-codigo-orig-ini
         c-cod-ncm-fim      = p-cod-ncm-fim
         i-codigo-orig-fim  = p-codigo-orig-fim.
