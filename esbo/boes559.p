...
*******************************************************************************/
{include/i-prgvrs.i BOES559 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES559
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  categoria-item-b2c
&GLOBAL-DEFINE TableLabel  Categoria x Item Ikeda        
...
define variable c-cod-categoria as character no-undo.
...
------------------------------------------------------------------------------*/
    ...
    if (pType = 'Create') and not can-find (first item-estab-b2c
                                            where item-estab-b2c.it-codigo         = RowObject.it-codigo
                                              and item-estab-b2c.ind-aceita-saldao = RowObject.ind-aceita-saldao) then do:
       {method/svc/errors/inserr.i
           &ErrorNumber="17006"
           &ErrorType="EMS"
           &ErrorSubType="ERROR"
           &ErrorParameters="'Item nÆo relacionado com nenhum estabelecimento para a Ikeda. Verifique no ESPDP047.'"
       }
    end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryCategoria DBOProgram                                                              
PROCEDURE openQueryCategoria :                                                                                                        

    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.cod-categoria = c-cod-categoria.
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE linkToCategoriaB2C DBOProgram
PROCEDURE linkToCategoriaB2C:
/*---------------------------------------------------------------
...
----------------------------------------------------------------*/
   ...
   RUN getKey IN pHandle (OUTPUT c-cod-categoria).
   
   RUN setConstraintCategoria IN THIS-PROCEDURE (INPUT c-cod-categoria).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintCh-pri DBOProgram                                                           
PROCEDURE setConstraintCategoria :                                                                                                     
    DEFINE INPUT PARAMETER p-cod-categoria AS character NO-UNDO.                                                                    
