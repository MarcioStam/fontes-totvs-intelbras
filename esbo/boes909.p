...
{include/i-prgvrs.i boes909 2.00.00.003}  /*** 010003 ***/

    /*--- Diretrizes de definiá∆o ---*/
&GLOBAL-DEFINE DBOName boes909
&GLOBAL-DEFINE DBOVersion 2.00.00.000 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName int-rateio-fatur
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        ...
        WHEN "vl-origem":U      THEN ASSIGN pFieldValue = RowObject.vl-origem.
        WHEN "vl-deducao":U     THEN ASSIGN pFieldValue = RowObject.vl-deducao.
        WHEN "vl-base-rateio":U THEN ASSIGN pFieldValue = RowObject.vl-base-rateio.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER ptipo-rateio LIKE int-rateio-fatur.tipo-rateio NO-UNDO.
    DEFINE OUTPUT PARAMETER pCompetencia LIKE int-rateio-fatur.competencia NO-UNDO.
    DEFINE OUTPUT PARAMETER pCod-estabel LIKE int-rateio-fatur.cod-estabel NO-UNDO.
    DEFINE OUTPUT PARAMETER pNr-fatura   LIKE int-rateio-fatur.nr-fatura   NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER ptipo-rateio  LIKE int-rateio-fatur.tipo-rateio NO-UNDO.
    DEFINE INPUT PARAMETER pcompetencia  LIKE int-rateio-fatur.competencia NO-UNDO.
    DEFINE INPUT PARAMETER pCod-estabel  LIKE int-rateio-fatur.cod-estabel NO-UNDO.
    DEFINE INPUT PARAMETER pNr-fatura    LIKE int-rateio-fatur.nr-fatura   NO-UNDO.

    FIND FIRST bfint-rateio-fatur
        WHERE bfint-rateio-fatur.tipo-rateio = ptipo-rateio
          AND bfint-rateio-fatur.competencia = pcompetencia 
          AND bfint-rateio-fatur.cod-estabel = pCod-estabel
          AND bfint-rateio-fatur.nr-fatura   = pNr-fatura NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-rateio-fatur THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-rateio-fatur)).
...
PROCEDURE openQueryint-rateio-fatur :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
    ...
    IF pType = "create" THEN DO:
       ...
       IF CAN-FIND (FIRST int-rateio-fatur WHERE
                    int-rateio-fatur.Tipo-rateio = RowObject.Tipo-rateio  AND
                    int-rateio-fatur.competencia = RowObject.competencia  AND
                    int-rateio-fatur.cod-estab   = RowObject.cod-estab    AND
                    int-rateio-fatur.nr-fatura   = RowObject.nr-fatura)       
       THEN DO:
          {method/svc/errors/inserr.i
              &ERRORNUMBER = "7"
              &ERRORTYPE = "EMS"
              &ERRORParameters = "'Rateio Faturas'"}
       END.
