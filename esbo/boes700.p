...
&GLOBAL-DEFINE DBOName BOES700
&GLOBAL-DEFINE DBOVersion 2.0
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName int-kit
...
/*:T--- Este include deve ser copiado para o diret¢rio do DBO e, ainda, seu nome
      ...
      extens∆o .i ---*/
{esbo/boes700.i RowObject}

DEFINE VARIABLE c-it-codigo AS CHARACTER   NO-UNDO.

DEFINE BUFFER b-int-kit FOR int-kit.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pit-codigo LIKE int-kit.it-codigo NO-UNDO.
    DEFINE OUTPUT PARAMETER pes-codigo LIKE int-kit.es-codigo NO-UNDO.
    DEFINE OUTPUT PARAMETER psequencia LIKE int-kit.sequencia NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pit-codigo LIKE int-kit.it-codigo NO-UNDO.
    DEFINE INPUT PARAMETER pes-codigo LIKE int-kit.es-codigo NO-UNDO.
    DEFINE INPUT PARAMETER psequencia LIKE int-kit.sequencia NO-UNDO.

    FIND FIRST bfint-kit WHERE 
        bfint-kit.it-codigo = pit-codigo AND 
        bfint-kit.es-codigo = pes-codigo AND 
        bfint-kit.sequencia = psequencia NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-kit THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-kit)).
...
------------------------------------------------------------------------------*/
    ...
    IF  pType = "Create":U OR
        ...
        IF CAN-FIND(FIRST b-int-kit
                    WHERE b-int-kit.it-codigo = RowObject.it-codigo
                    AND   b-int-kit.sequencia = RowObject.sequencia) THEN DO:
    
            {method/svc/errors/inserr.i &ErrorNumber     = 7
                                        &ErrorType       = "EMS"
                                        &ErrorParameters = "'Sequencia do Comp Kit'"}
        ...
        IF RowObject.sequencia = 0 THEN DO:
            ...
            {method/svc/errors/inserr.i &ErrorNumber     = 17006
                                        &ErrorType       = "EMS"
                                        &ErrorParameters = "'Sequencia do Comp Kit n∆o pode ser zero.'"}
