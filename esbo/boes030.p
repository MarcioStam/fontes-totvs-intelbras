...
/*------------------------------------------------------------------------*/
{include/i-prgvrs.i BOES030 2.00.00.001}

/* ***************************  Definitions  **************************** */

/*:T--- Diretrizes de definiá∆o ---*/
&GLOBAL-DEFINE DBOName              BOES030
&GLOBAL-DEFINE DBOVersion           2.00.00.001
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName            zona-separa-equip
&GLOBAL-DEFINE TableLabel           Equipto Zona Separaá∆o
&GLOBAL-DEFINE QueryName            qr{&TableName} 

{esbo/boes030.i RowObject}
{method/dboqry.i}
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "cod-prioridade":U THEN ASSIGN pFieldValue = RowObject.cod-prioridade.
...
PROCEDURE getKey :
/*------------------------------------------------------------------------------
  Purpose:     Retorna valores dos campos do °ndice znsepeq-01
  Parameters:  
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-estabel LIKE zona-separa-equip.cod-estabel NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-local LIKE zona-separa-equip.cod-local NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-zona LIKE zona-separa-equip.cod-zona NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-equipamento LIKE zona-separa-equip.cod-equipamento NO-UNDO.
    ...
    ASSIGN pcod-estabel = RowObject.cod-estabel
           ...
           pcod-equipamento = RowObject.cod-equipamento.
...
PROCEDURE goToKey :
/*------------------------------------------------------------------------------
  Purpose:     Reposiciona registro com base no °ndice znsepeq-01
  Parameters:  
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pcod-estabel LIKE zona-separa-equip.cod-estabel NO-UNDO.
    DEFINE INPUT PARAMETER pcod-local LIKE zona-separa-equip.cod-local NO-UNDO.
    DEFINE INPUT PARAMETER pcod-zona LIKE zona-separa-equip.cod-zona NO-UNDO.
    DEFINE INPUT PARAMETER pcod-equipamento LIKE zona-separa-equip.cod-equipamento NO-UNDO.

    FIND FIRST bfzona-separa-equip WHERE 
        bfzona-separa-equip.cod-estabel = pcod-estabel AND 
        bfzona-separa-equip.cod-local = pcod-local AND 
        bfzona-separa-equip.cod-zona = pcod-zona AND 
        bfzona-separa-equip.cod-equipamento = pcod-equipamento NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfzona-separa-equip THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfzona-separa-equip)).
    ...
    IF pType = "CREATE":U THEN DO:
        FIND FIRST bf{&TableName} 
             WHERE bf{&TableName}.cod-estabel     = RowObject.cod-estabel     
               ...
               AND bf{&tablename}.cod-equipamento = RowObject.cod-equipamento NO-LOCK NO-ERROR.
        IF AVAIL bf{&TableName} THEN DO:
            {method/svc/errors/inserr.i
                  &ErrorNumber=4242
                  &ErrorType="EMS"
                  &ErrorSubType="ERROR" 
                  &ErrorParameters="'Equipamento Zona Separaá∆o'"} 
        END.
        ...
        FOR FIRST wm-equipamento NO-LOCK
            WHERE wm-equipamento.cod-equipamento = RowObject.cod-equipamento:
        END.
        ...
        END.
        ELSE DO:
            FOR FIRST wm-tipo-equipamento NO-LOCK
                WHERE wm-tipo-equipamento.cdn-tipo-equipamento = wm-equipamento.cdn-tipo-equipamento:
            END.

            IF NOT AVAIL wm-tipo-equipamento
            OR wm-tipo-equipamento.ind-status-equipamento <> 2 THEN DO:
                {method/svc/errors/inserr.i
                      &ErrorNumber=17006
                      &ErrorType="EMS"
                      &ErrorSubType="ERROR" 
                      &ErrorParameters="'Equipamento n∆o Ç de transporte'"} 
            END.
