...
&GLOBAL-DEFINE DBOName BOES005
&GLOBAL-DEFINE DBOVersion 1.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName ae-baixa
&GLOBAL-DEFINE TableLabel ae-baixa 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER p-cod-estabel LIKE ae-baixa.cod-estabel NO-UNDO.
    DEFINE OUTPUT PARAMETER p-nr-ae LIKE ae-baixa.nr-ae NO-UNDO.
    DEF OUTPUT PARAM p-sequencia LIKE ae-baixa.sequencia NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAM p-nr-ae LIKE ae-baixa.nr-ae NO-UNDO.
    DEF INPUT PARAM p-sequencia LIKE ae-baixa.sequencia NO-UNDO.

    FIND FIRST bfae-baixa 
        WHERE bfae-baixa.cod-estabel = pCodEstabel
        and   bfae-baixa.nr-ae     = p-nr-ae
        AND   bfae-baixa.sequencia = p-sequencia NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfae-baixa THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfae-baixa)).
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
        WHEN "Create" THEN DO:
            ...
            IF CAN-FIND(FIRST ae-baixa NO-LOCK
                        WHERE ae-baixa.cod-estabel = RowObject.cod-estabel
                        and   ae-baixa.nr-ae = RowObject.nr-ae
                        AND   ae-baixa.sequencia = RowObject.sequencia) THEN DO:
