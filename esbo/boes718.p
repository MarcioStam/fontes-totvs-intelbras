...
&GLOBAL-DEFINE DBOName BOES718
&GLOBAL-DEFINE DBOVersion 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName altern-astec
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        ...
        WHEN "it-altern":U THEN ASSIGN pFieldValue = RowObject.it-altern.
...
/*------------------------------------------------------------------------------
  ...
  Parameters:  
               ...
               retorna valor do campo it-altern
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pit-codigo LIKE altern-astec.it-codigo NO-UNDO.
    DEFINE OUTPUT PARAMETER pit-altern LIKE altern-astec.it-altern NO-UNDO.
    ...
    ASSIGN pit-codigo = RowObject.it-codigo
           pit-altern = RowObject.it-altern.
...
/*------------------------------------------------------------------------------
  ...
  Parameters:  
               ...
               recebe valor do campo it-altern
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pit-codigo LIKE altern-astec.it-codigo NO-UNDO.
    DEFINE INPUT PARAMETER pit-altern LIKE altern-astec.it-altern NO-UNDO.

    FIND FIRST bfaltern-astec WHERE 
        bfaltern-astec.it-codigo = pit-codigo AND 
        bfaltern-astec.it-altern = pit-altern NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfaltern-astec THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfaltern-astec)).
...
----------------------------------------------------------------*/
   ...
   RUN getKey IN pHandle (OUTPUT c-chave).
   
   RUN setConstraintItem IN THIS-PROCEDURE (INPUT c-chave).
...
------------------------------------------------------------------------------*/
    ...
    IF pType = 'CREATE' THEN DO:
        ...
        IF CAN-FIND(FIRST altern-astec 
                    WHERE altern-astec.it-codigo = RowObject.it-codigo
                    AND   altern-astec.it-altern = RowObject.it-altern) THEN DO:

            {method/svc/errors/inserr.i
                         &ErrorNumber="8"
                         &ErrorType="EMS" &ErrorSubType="ERROR"
                         &ErrorParameters="'ITEM Alt'"}

        END.

        IF NOT CAN-FIND (FIRST ITEM
                     WHERE ITEM.it-codigo = RowObject.it-altern) THEN DO:
