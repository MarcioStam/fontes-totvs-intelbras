...
*******************************************************************************/
{include/i-prgvrs.i BOES452 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES452
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  tipo-acordo
&GLOBAL-DEFINE TableLabel  Tipo Acordo                   
...
define variable v-codigo-ini as integer no-undo.
define variable v-codigo-fim as integer no-undo.
...
DEFINE INPUT PARAMETER iAbertura AS INTEGER NO-UNDO.
    ...
    CASE iAbertura:
        ...
        WHEN 2 THEN           
            RUN openQueryStatic ("Ch-codigo":U).                      
    END CASE.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryCh-codigo DBOProgram 
PROCEDURE openQueryCh-codigo :
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryZoomCodigo DBOProgram 
PROCEDURE openQueryZoomCodigo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
         WHERE {&TableName}.codigo >= v-codigo-ini
           AND {&TableName}.codigo <= v-codigo-fim.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintCh-codigo DBOProgram 
PROCEDURE setConstraintCh-codigo :
DEFINE INPUT PARAMETER p-codigo AS integer NO-UNDO.                                                                             
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintZoomCodigo DBOProgram 
PROCEDURE setConstraintZoomCodigo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER p-codigo-ini AS integer NO-UNDO.
    DEFINE INPUT PARAMETER p-codigo-fim AS integer NO-UNDO.

    ASSIGN v-codigo-ini = p-codigo-ini
           v-codigo-fim = p-codigo-fim.
