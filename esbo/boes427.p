...
*******************************************************************************/
{include/i-prgvrs.i BOES427 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES427
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  int-estabelec
&GLOBAL-DEFINE TableLabel  Int Estabelecimento           
...
------------------------------------------------------------------------------*/
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-estabel = RowObject.cod-estabel) THEN DO:
         ...
         WHEN "Delete" THEN DO:
             ...
             IF RowObject.cod-estabel = "101" THEN DO:
                 {method/svc/errors/inserr.i
                     &ErrorNumber="17006"
                     &ErrorType="EMS"
                     &ErrorSubType="ERROR"
                     &ErrorParameters="'NÆo ‚ poss¡vel excluir estabelecimento matriz. Entre em contato com setor de inform tica!'"}
             END.
         END.
    END CASE.
    ...
    IF pType = "Create" OR pType = "Update" THEN DO:
        ...
        IF RowObject.cod-estabel = "101" AND NOT RowObject.ativa-ean13 THEN DO:
            {method/svc/errors/inserr.i
                &ErrorNumber="17006"
                &ErrorType="EMS"
                &ErrorSubType="ERROR"
                &ErrorParameters="'Parƒmetro EAN-13 deve estar ativo para estabelecimento(101) pois ‚ o matriz.'"}
        END.

        IF RowObject.ativa-ean13 THEN DO:
            IF LENGTH(TRIM(STRING(RowObject.prefixo-ean13))) < 7 OR  LENGTH(TRIM(STRING(RowObject.prefixo-ean13))) > 10 THEN DO:
                {method/svc/errors/inserr.i
                    &ErrorNumber="17006"
                    &ErrorType="EMS"
                    &ErrorSubType="ERROR"
                    &ErrorParameters="'Prefixo EAN-13 inv lido.~~~~Prefixo deve conter de 7 a 10 caracteres.'"}
            END.

            IF RowObject.numeros-ean13 <> "99"    AND
               RowObject.numeros-ean13 <> "999"   AND
               RowObject.numeros-ean13 <> "9999"  AND
               RowObject.numeros-ean13 <> "99999" THEN DO:
                {method/svc/errors/inserr.i
                    &ErrorNumber="17006"
                    &ErrorType="EMS"
                    &ErrorSubType="ERROR"
                    &ErrorParameters="'N£meros EAN-13 inv lidos.~~~~N£meros deve conter de 2 a 5 caracteres(9).'"}
            END.

            IF LENGTH(TRIM(STRING(RowObject.prefixo-ean13)) + TRIM(RowObject.numeros-ean13)) <> 12 THEN DO:
                {method/svc/errors/inserr.i
                    &ErrorNumber="17006"
                    &ErrorType="EMS"
                    &ErrorSubType="ERROR"
                    &ErrorParameters="'Prefixo + N£meros (EAN-13) com tamanho inv lido.~~~~Prefixo + N£meros (EAN-13) devem conter 12 caracteres.'"}
            END.

            /*Verifica se existe algum outro estabelecimento com prefixo inicial igual ao informado*/
            FIND FIRST bf{&TableName} NO-LOCK 
                 WHERE bf{&TableName}.cod-estabel <> RowObject.cod-estabel 
                   AND SUBSTRING(STRING(bf{&TableName}.prefixo-ean13),1,7) = SUBSTRING(STRING(RowObject.prefixo-ean13),1,7)
                   AND bf{&TableName}.ativa-ean13  NO-ERROR.
            IF AVAILABLE bf{&TableName} THEN DO:
                {method/svc/errors/inserr.i
                    &ErrorNumber="17006"
                    &ErrorType="EMS"
                    &ErrorSubType="ERROR"
                    &ErrorParameters="'Prefixo j  cadastrado em outro estabelecimento ativo.~~~~Os 7 primeiros caracteres do prefixo j  foram utilizados em outro estabelecimento'"}
            END.
        END.
    END.
...
PROCEDURE getCharField :                                                                                                            
    ...
    CASE pFieldName:
        ...
        WHEN "numeros-ean13":U THEN ASSIGN pFieldValue = RowObject.numeros-ean13.
...
PROCEDURE getIntField :                                                                                                             
    ...
    CASE pFieldName:
        WHEN "seq-atual-ean13":U THEN ASSIGN pFieldValue = RowObject.seq-atual-ean13.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
...
PROCEDURE getDecField :                                                                                                             
    DEFINE INPUT PARAMETER pFieldName AS CHARACTER NO-UNDO.
    DEFINE OUTPUT PARAMETER pFieldValue AS DECIMAL DECIMALS 0 NO-UNDO.                                                                         

    IF NOT AVAILABLE RowObject THEN 
        RETURN "NOK":U.

    CASE pFieldName:
        WHEN "prefixo-ean13":U THEN ASSIGN pFieldValue = RowObject.prefixo-ean13.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
