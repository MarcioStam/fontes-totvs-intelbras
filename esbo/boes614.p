...
&GLOBAL-DEFINE DBOName BOES614
&GLOBAL-DEFINE DBOVersion 2.0
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName item-dun
...
DEFINE VARIABLE i-qtd-emb-ini AS INTEGER     NO-UNDO.
DEFINE VARIABLE i-qtd-emb-fim AS INTEGER     NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "cod-dun":U THEN ASSIGN pFieldValue = RowObject.cod-dun.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "digito":U THEN ASSIGN pFieldValue = RowObject.digito.
        WHEN "qtd-emb":U THEN ASSIGN pFieldValue = RowObject.qtd-emb.
        OTHERWISE RETURN "NOK":U.
    END CASE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pit-codigo LIKE item-dun.it-codigo NO-UNDO.
    DEFINE OUTPUT PARAMETER pqtd-emb LIKE item-dun.qtd-emb NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pit-codigo LIKE item-dun.it-codigo NO-UNDO.
    DEFINE INPUT PARAMETER pqtd-emb LIKE item-dun.qtd-emb NO-UNDO.

    FIND FIRST bfitem-dun WHERE 
        bfitem-dun.it-codigo = pit-codigo AND 
        bfitem-dun.qtd-emb = pqtd-emb NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfitem-dun THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfitem-dun)).
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK 
    ...
    AND   {&TableName}.qtd-emb >= i-qtd-emb-ini
    AND   {&TableName}.qtd-emb <= i-qtd-emb-fim
    INDEXED-REPOSITION.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER p-qtd-emb-ini    AS INTEGER      NO-UNDO.
    DEFINE INPUT PARAMETER p-qtd-emb-fim    AS INTEGER      NO-UNDO.

    ASSIGN c-it-codigo-ini = p-it-codigo-ini
           c-it-codigo-fim = p-it-codigo-fim
           i-qtd-emb-ini   = p-qtd-emb-ini
           i-qtd-emb-fim   = p-qtd-emb-fim.
