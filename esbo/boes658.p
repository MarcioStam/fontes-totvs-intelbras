...
&GLOBAL-DEFINE DBOName BOES658
&GLOBAL-DEFINE DBOVersion 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName indice-qualid
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "absol-atencao":U THEN ASSIGN pFieldValue = RowObject.absol-atencao.
        WHEN "absol-problema":U THEN ASSIGN pFieldValue = RowObject.absol-problema.
        WHEN "dec-1":U THEN ASSIGN pFieldValue = RowObject.dec-1.
        WHEN "dec-2":U THEN ASSIGN pFieldValue = RowObject.dec-2.
        WHEN "relat-atencao":U THEN ASSIGN pFieldValue = RowObject.relat-atencao.
        WHEN "relat-problema":U THEN ASSIGN pFieldValue = RowObject.relat-problema.
        OTHERWISE RETURN "NOK":U.
    END CASE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-estabel LIKE indice-qualid.cod-estabel NO-UNDO.
    DEFINE OUTPUT PARAMETER pit-codigo LIKE indice-qualid.it-codigo NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pcod-estabel LIKE indice-qualid.cod-estabel NO-UNDO.
    DEFINE INPUT PARAMETER pit-codigo LIKE indice-qualid.it-codigo NO-UNDO.

    FIND FIRST bfindice-qualid WHERE 
        bfindice-qualid.cod-estabel = pcod-estabel AND 
        bfindice-qualid.it-codigo = pit-codigo NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfindice-qualid THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfindice-qualid)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE OpenQueryEstabItem DBOProgram 
PROCEDURE OpenQueryEstabItem :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintEstabItem DBOProgram 
PROCEDURE setConstraintEstabItem :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
