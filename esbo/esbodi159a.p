...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER pOrig-mercad   AS INTEGER                     NO-UNDO. 
    DEFINE INPUT PARAMETER pParcMinima    AS LOGICAL                     NO-UNDO.
    DEFINE INPUT PARAMETER pPedAlocado    AS LOGICAL                     NO-UNDO.
    ...
    DEFINE VARIABLE lParcMaior AS LOG NO-UNDO.
...
/*         VIEW-AS ALERT-BOX INFO BUTTONS OK. */
   RUN esp/es0018p.p (INPUT "espdp090",                       
    ...
    IF pPedido-ini <> "" OR pEstabel-ini = "" THEN DO:
        FOR EACH ped-venda NO-LOCK  
          ...
          IF pOrig-mercad <> 0 THEN DO:
             IF pOrig-mercad = 1 THEN DO: //Estabelec Intelbras
          ...
          FOR FIRST int-cond-pagto NO-LOCK
              ...
              IF pPedAlocado THEN DO: //filtrar somente pedido com item alocado
                    IF NOT CAN-FIND(FIRST ped-item
                                    WHERE  ped-item.nome-abrev    = ped-venda.nome-abrev  
                                      AND  ped-item.nr-pedcli     = ped-venda.nr-pedcli   
                                      AND (ped-item.cod-sit-item  = 1
                                       OR  ped-item.cod-sit-item  = 2)
                                      AND ped-item.qt-log-aloc > 0) THEN NEXT.
              END.
              ...
              IF pParcMinima THEN DO: //marcado somente abaixo da parcela minima
                 /* RUN pi-valida-parc-minima(INPUT ped-venda.nr-pedcli,
                                            INPUT ped-venda.cod-estabel,
                                            INPUT ped-venda.tp-pedido,
                                            OUTPUT lParcMinima). */

                  RUN pi-valida-parc-minima(OUTPUT lParcMaior).
                  IF lParcMaior THEN
                      NEXT.
              END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-valida-parc-minima DBOProgram 
PROCEDURE pi-valida-parc-minima :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
//DEFINE INPUT PARAM pEstab       LIKE ped-venda.cod-estabel.
//DEFINE INPUT PARAM pAtentendte  LIKE ped-venda.tp-pedido.
DEFINE OUTPUT PARAM pRetorno AS LOG NO-UNDO. 

DEFINE BUFFER b-item-minimo FOR ITEM.
DEFINE BUFFER bPed-item     FOR ped-item.

DEFINE VAR i-parc2       AS INT NO-UNDO.
DEFINE VAR de-valor2     AS DEC NO-UNDO.
DEFINE VAR lMaior        AS LOG NO-UNDO.
DEFINE VAR d-st-alocado  AS DEC NO-UNDO.
DEFINE VAR d-ipi-alocado AS DEC NO-UNDO.
DEFINE VAR d-ipi-trib2   AS INT NO-UNDO.
DEFINE VAR d-vl-total2   AS DEC NO-UNDO.

    FIND FIRST atendente
         WHERE atendente.cd-oper = INT(ped-venda.tp-pedido) NO-LOCK NO-ERROR.
    IF AVAIL atendente THEN DO: //se nao tem cadastro de minimo nao le o bloco inteiro
        FIND FIRST minimos-faturamento
             WHERE minimos-faturamento.cod-estabel = ped-venda.cod-estabel
               AND minimos-faturamento.oper-mestre = atendente.oper-mestre NO-LOCK NO-ERROR.
        IF NOT AVAIL minimos-faturamento THEN DO:
            ASSIGN pRetorno = NO.
           NEXT.
        END.
    END.

    FIND FIRST natur-oper OF ped-venda NO-LOCK.
       ...
       IF natur-oper.emite-duplic THEN DO:
           ...
           IF AVAIL pd-vendor THEN DO:
               FIND FIRST cond-pagto 
                    WHERE cond-pagto.cod-cond-pag = pd-vendor.cod-cond-cli NO-LOCK NO-ERROR.
               IF AVAIL cond-pagto THEN
                   ASSIGN i-parc2 = cond-pagto.num-parcelas.
           END.
           ELSE DO:
               FIND FIRST cond-pagto 
           ...
           FOR EACH bPed-item OF ped-venda NO-LOCK
              WHERE bPed-item.cod-sit-item < 3 
                AND bped-item.qt-log-aloc  > 0,
               FIRST natur-oper NO-LOCK
               WHERE natur-oper.nat-operacao = bPed-item.nat-operacao,
               FIRST b-item-minimo
               WHERE b-item-minimo.it-codigo = bPed-item.it-codigo NO-LOCK: // somente alocacao  
   
               assign d-ipi-trib2 = if  natur-oper.cd-trib-ipi = 1
                                        then if  b-item-minimo.cd-trib-ipi = 1
                                             or  b-item-minimo.cd-trib-ipi = 4
                                             then 1
                                             else b-item-minimo.cd-trib-ipi
                                        else if  natur-oper.cd-trib-ipi = 2
                                             or  natur-oper.cd-trib-ipi = 3
                                             then natur-oper.cd-trib-ipi
                                             else b-item-minimo.cd-trib-ipi.
            
                 ASSIGN d-vl-total2 = d-vl-total2 + (bPed-item.qt-log-aloc * bPed-item.vl-preuni).
            
                 IF d-ipi-trib2 = 1 THEN
                     ASSIGN d-ipi-alocado = d-ipi-alocado + ROUND((bPed-item.qt-log-aloc * bPed-item.vl-preuni * bPed-item.aliquota-ipi / 100),2).
            
                IF  natur-oper.subs-trib THEN
                    ASSIGN d-st-alocado = d-st-alocado + ROUND(((bPed-item.vl-tot-it - bPed-item.vl-liq-it -
                           (bPed-item.qt-pedida * bPed-item.vl-preuni) * (bPed-item.aliquota-ipi / 100)) / bPed-item.qt-pedida) * bPed-item.qt-log-aloc,2).
           END.
 
           ASSIGN de-valor2 = d-vl-total2 + d-ipi-alocado + d-st-alocado.
   
           IF de-valor2 = 0 THEN DO: // nao tem alocacao
               FOR EACH bPed-item OF ped-venda NO-LOCK
                  WHERE bPed-item.cod-sit-item < 3 
                    AND bped-item.qt-log-aloc  = 0: // sem  alocacao  
   
                ASSIGN de-valor2 = de-valor2 + bped-item.vl-tot-it.
               END.
           END. 

           IF AVAIL minimos-faturamento THEN DO:

               IF (de-valor2 / i-parc2) < minimos-faturamento.vl-parc-minima THEN DO:
                   IF de-valor2 < minimos-faturamento.vl-fatur-minimo THEN DO:
                       ASSIGN lMaior = NO.
                   END.
                   ELSE DO:
                       ASSIGN lMaior = NO.
