...
&GLOBAL-DEFINE DBOName BOES175
&GLOBAL-DEFINE DBOVersion 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName int-logistica-ecommerce
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcod-transp     LIKE int-logistica-ecommerce.cod-transp     NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-estabel    LIKE int-logistica-ecommerce.cod-estabel    NO-UNDO.
    DEFINE OUTPUT PARAMETER pcep-inicial    LIKE int-logistica-ecommerce.cep-inicial    NO-UNDO.
    DEFINE OUTPUT PARAMETER pcep-final      LIKE int-logistica-ecommerce.cep-final      NO-UNDO.
    DEFINE OUTPUT PARAMETER p-estado        LIKE int-logistica-ecommerce.estado         NO-UNDO.
    DEFINE OUTPUT PARAMETER pcidade         LIKE int-logistica-ecommerce.cidade         NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-emitente   LIKE int-logistica-ecommerce.cod-emitente   NO-UNDO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pcod-transp     LIKE int-logistica-ecommerce.cod-transp     NO-UNDO.
    DEFINE INPUT PARAMETER pcod-estabel    LIKE int-logistica-ecommerce.cod-estabel    NO-UNDO.
    DEFINE INPUT PARAMETER pcep-inicial    LIKE int-logistica-ecommerce.cep-inicial    NO-UNDO.
    DEFINE INPUT PARAMETER pcep-final      LIKE int-logistica-ecommerce.cep-final      NO-UNDO.
    DEFINE INPUT PARAMETER p-estado        LIKE int-logistica-ecommerce.estado         NO-UNDO.
    DEFINE INPUT PARAMETER pcidade         LIKE int-logistica-ecommerce.cidade         NO-UNDO.
    DEFINE INPUT PARAMETER pcod-emitente   LIKE int-logistica-ecommerce.cod-emitente   NO-UNDO.
    

    FIND FIRST bfint-logistica-ecommerce 
         WHERE bfint-logistica-ecommerce.cod-transp     = pcod-transp
           AND bfint-logistica-ecommerce.cod-estabel    = pcod-estabel 
           AND bfint-logistica-ecommerce.cep-inicial    = pcep-inicial 
           AND bfint-logistica-ecommerce.cep-final      = pcep-final
           AND bfint-logistica-ecommerce.estado         = p-estado 
           AND bfint-logistica-ecommerce.cidade         = pcidade 
           AND bfint-logistica-ecommerce.cod-emitente   = pcod-emitente
            NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-logistica-ecommerce THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-logistica-ecommerce)).
...
------------------------------------------------------------------------------*/
    ...
    IF pType = "Create" THEN DO:
        IF CAN-FIND(FIRST int-logistica-ecommerce 
                       WHERE int-logistica-ecommerce.cod-transp     = RowObject.cod-transp
                         AND int-logistica-ecommerce.cod-estabel    = RowObject.cod-estabel 
                         AND int-logistica-ecommerce.cep-inicial    = RowObject.cep-inicial
                         AND int-logistica-ecommerce.cep-final      = RowObject.cep-final
                         AND int-logistica-ecommerce.estado         = RowObject.estado      
                         AND int-logistica-ecommerce.cidade         = RowObject.cidade   
                         AND int-logistica-ecommerce.cod-emitente   = RowObject.cod-emitente   ) THEN DO:
