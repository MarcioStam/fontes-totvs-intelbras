...
*******************************************************************************/
{include/i-prgvrs.i BOES041 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES041
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  contenedor
&GLOBAL-DEFINE TableLabel  contenedor                      
...
define variable v-nr-contenedor as integer no-undo.
...
PROCEDURE getCharField :                                                                                                            
    ...
    CASE pFieldName:
        ...
        WHEN "local":U THEN ASSIGN pFieldValue = RowObject.local.
...
PROCEDURE getIntField :                                                                                                             
    ...
    CASE pFieldName:
        WHEN "nr-contenedor":U THEN ASSIGN pFieldValue = RowObject.nr-contenedor.
        OTHERWISE RETURN "NOK":U.
    END CASE.
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setConstraintIt-codigo DBOProgram                                                        
PROCEDURE setConstraintIt-codigo :                                                                                                  
    DEFINE INPUT PARAMETER p-it-codigo AS character NO-UNDO.                                                                        
    DEFINE INPUT PARAMETER p-nr-contenedor AS integer NO-UNDO.                                                                      

    ASSIGN 
    v-it-codigo = p-it-codigo                                         
    v-nr-contenedor = p-nr-contenedor                                 
    .
    RETURN "OK":U.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE openQueryIt-codigo DBOProgram                                                            
PROCEDURE openQueryIt-codigo :                                                                                                      

    OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.it-codigo = v-it-codigo                    
        AND {&TableName}.nr-contenedor = v-nr-contenedor              
    .
    RETURN "OK":U.
END PROCEDURE.
