...
*******************************************************************************/
{include/i-prgvrs.i BOES080 2.00.00.000}                               
/*--------------------------------------------------------------------------
    File       : 
...
&GLOBAL-DEFINE DBOName  BOES080
&GLOBAL-DEFINE DBOVersion  2.00.00.000
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName  familia-item
&GLOBAL-DEFINE TableLabel  Familia item                  
...
------------------------------------------------------------------------------*/
    ...
    for each sub-familia-item exclusive-lock
        where sub-familia-item.cod-familia = rowObject.cod-familia:
        
        for each car-familia-item of sub-familia-item exclusive-lock:
        
            for each comp-familia-item of car-familia-item:

                c-familia-id = string(comp-familia-item.cod-familia,"999") +
                               string(comp-familia-item.cod-sub-familia,"99") +
                               string(comp-familia-item.cod-car-familia,"99") +
                               string(comp-familia-item.cod-comp-familia,"9").
   
                for each fam-man-estab exclusive-lock
                    where fam-man-estab.fm-codigo = c-familia-id :
                    
                    delete fam-man-estab.
                end.     
            
                for each fam-mat-estab exclusive-lock
                    where fam-mat-estab.fm-codigo = c-familia-id :
                    
                    delete fam-mat-estab.
                end.     

                delete comp-familia-item.
            end.
        
            delete car-familia-item.
        end.
        
        delete sub-familia-item.
   end.     
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE output PARAMETER p-cod-familia AS integer NO-UNDO.                                                                        

   IF AVAILABLE {&TableName} THEN
      ASSIGN p-cod-familia = {&TableName}.cod-familia.
   ELSE
      ASSIGN p-cod-familia = ?.
...
DEFINE INPUT PARAMETER iAbertura AS INTEGER NO-UNDO.
    ...
    CASE iAbertura:
        ...
        WHEN 2 THEN           
            RUN openQueryStatic ("Descricao":U).                      
        WHEN 3 THEN           
...
OPEN QUERY {&QueryName} FOR EACH {&TableName} NO-LOCK
        WHERE {&TableName}.descricao >= v-ini-descricao                
    ...
    CASE pType:
         WHEN "Create" THEN DO:
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.cod-familia = RowObject.cod-familia) THEN DO:
             ...
             END.
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 WHERE bf{&TableName}.abreviatura = RowObject.abreviatura
                 or    bf{&TableName}.descricao = RowObject.descricao) THEN DO:
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe fam¡lia com esta abreviatura e/u descri‡Æo",
                                        INPUT "Existe fam¡lia com esta abreviatura e/u descri‡Æo",
                                        INPUT "").                    
             END.
         END.
         WHEN "Update" THEN DO:
             ...
             IF CAN-FIND(FIRST bf{&TableName} NO-LOCK
                 ...
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe fam¡lia com esta abreviatura",
                                        INPUT "Existe fam¡lia com esta abreviatura",
                                        INPUT "").                    
             END.
         ...
         WHEN "Delete" THEN DO:
             ...
             IF CAN-FIND(FIRST sub-familia-item no-lock
                         WHERE sub-familia-item.cod-familia     = RowObject.cod-familia)
                         THEN DO:
                 ...
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe Sub-Fam¡lia cadastrada para Fam¡lia.",
                                        INPUT "Fam¡lia nÆo pode ser excluida.",
                                        INPUT "").
             ...
             if can-find(first familia no-lock
                where familia.fm-codigo begins c-familia-id) then do:
                 RUN _insertErrorManual(INPUT 0,
                                        INPUT "EMS",
                                        INPUT "ERROR",
                                        INPUT "Existe fam¡la EMS para esta fam¡lia",
                                        INPUT "Existe fam¡la EMS para esta fam¡lia",
                                        INPUT "").                    
