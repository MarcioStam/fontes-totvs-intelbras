...
&GLOBAL-DEFINE DBOName BOES925
&GLOBAL-DEFINE DBOVersion 
&GLOBAL-DEFINE DBOCustomFunctions 
&GLOBAL-DEFINE TableName int-bloq-requis
...
DEFINE TEMP-TABLE Rowobject NO-UNDO LIKE int-bloq-requis
...
DEFINE VARIABLE i-codigo-ini      AS INTEGER     NO-UNDO.
DEFINE VARIABLE i-codigo-fim      AS INTEGER     NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    CASE pFieldName:
        WHEN "aviso":U THEN ASSIGN pFieldValue = RowObject.aviso.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER pcodigo      LIKE int-bloq-requis.codigo      NO-UNDO.
    DEFINE OUTPUT PARAMETER pcod-estabel LIKE int-bloq-requis.cod-estabel NO-UNDO.
    ...
    ASSIGN pcodigo      = RowObject.codigo
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER pcodigo      LIKE int-bloq-requis.codigo      NO-UNDO.
    DEFINE INPUT PARAMETER pcod-estabel LIKE int-bloq-requis.cod-estabel NO-UNDO.

    FIND FIRST bfint-bloq-requis WHERE 
               bfint-bloq-requis.codigo      = pcodigo AND
               bfint-bloq-requis.cod-estabel = pcod-estabel
               NO-LOCK NO-ERROR.

    /*--- Verifica se registro foi encontrado, em caso de erro ser† retornada flag "NOK":U ---*/
    IF NOT AVAILABLE bfint-bloq-requis THEN 
        RETURN "NOK":U.

    /*--- Reposiciona query atravÇs de rowid e verifica a ocorrància de erros, caso
          existam erros ser† retornada flag "NOK":U ---*/
    RUN repositionRecord IN THIS-PROCEDURE (INPUT ROWID(bfint-bloq-requis)).
...
------------------------------------------------------------------------------*/
    ...
    OPEN QUERY {&QueryName} FOR EACH  {&TableName} NO-LOCK
                                WHERE {&TableName}.codigo      >= i-codigo-ini
                                AND   {&TableName}.codigo      <= i-codigo-fim 
...
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT  PARAMETER pcodigo-ini         AS INTEGER     NO-UNDO.
    DEFINE INPUT  PARAMETER pcodigo-fim         AS INTEGER     NO-UNDO.
    DEFINE INPUT  PARAMETER pcod-estabel-ini    AS CHARACTER   NO-UNDO.
    DEFINE INPUT  PARAMETER pcod-estabel-fim    AS CHARACTER   NO-UNDO.

    ASSIGN i-codigo-ini      = pcodigo-ini
           i-codigo-fim      = pcodigo-fim
           c-cod-estabel-ini = pcod-estabel-ini
    ...
    IF  pType = "Create":U THEN DO:
        IF  rowObject.cod-estabel = "" THEN DO:
            {method/svc/errors/inserr.i &ErrorNumber     = 17006
                                        &ErrorType       = "EMS"
                                        &ErrorParameters = "'Informe um estabelecimento.'"}
        END.
    END.

    IF  pType = "Delete":U THEN DO:
        IF CAN-FIND (FIRST int-item-bloq-requis NO-LOCK WHERE
                           int-item-bloq-requis.codigo      = rowObject.codigo AND
                           int-item-bloq-requis.cod-estabel = rowObject.cod-estabel) 
        THEN DO:
            {method/svc/errors/inserr.i &ErrorNumber     = 17006
                                        &ErrorType       = "EMS"
                                        &ErrorParameters = "'N∆o pode ser deletado, existem itens vinculados.'"}
        END.
    END.
