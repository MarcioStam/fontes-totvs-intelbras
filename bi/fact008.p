...
&scoped-define TEMP_TABLE  tt-vpc
&scoped-define ARQUIVO_TXT FactVPC
&scoped-define EXTRATOR    fact008
...
log-manager:write-message('Indo executar. RPC? ' + string(l-rpc)).
...
if (l-rpc) then do:
   ...
   if (not error-status:error) and (hAppServer:connected()) then do:
      log-manager:write-message('RPC: connected').
      run bi/{&EXTRATOR}rp.p on server hAppServer (input table tt-param, output table {&TEMP_TABLE}) no-error.
      ...
      log-manager:write-message('RPC: not connected').
      run bi/{&EXTRATOR}rp.p (input table tt-param, output table {&TEMP_TABLE}) no-error.
   end.
end.
/** Caso online... **/
else do:
   log-manager:write-message('Execuá∆o online').

   run bi/{&EXTRATOR}rp.p (input table tt-param, output table {&TEMP_TABLE}) no-error.
end.

log-manager:write-message('Criando arquivo txt').
