 ...
 * Fato: Extrator da receita realizada de vendas
 *
...
define output parameter table for ttReceitaRealizada.
...
EMPTY TEMP-TABLE ttReceitaRealizada.
...
/*     END.                                                                                                                                */
    ...
    FIND FIRST ttReceitaRealizada
        WHERE  ttReceitaRealizada.CD_MasterExterno    = "1"
          AND  ttReceitaRealizada.NM_AnoExtracao      = i-ano-extracao
          AND  ttReceitaRealizada.NM_MesExtracao      = i-mes-extracao
          AND  ttReceitaRealizada.CD_Empresa          = i-ep-codigo-usuario
          AND  ttReceitaRealizada.CD_Estabelecimeto   = tt-calcula.cod-estabel  
          AND  ttReceitaRealizada.CD_Mercado          = SUBSTR(c-mercado,1,50)    
          AND  ttReceitaRealizada.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem))     
          AND  ttReceitaRealizada.CD_Vertical         = tt-calcula.c-vertical   
          AND  ttReceitaRealizada.CD_UnidNegoc        = TRIM(fn-free-accent(tt-calcula.descricao-un-nota))
          AND  ttReceitaRealizada.CD_Segmento         = SUBSTR(c-segmento,1,50) 
          AND  ttReceitaRealizada.CD_FamiliaComercial = c-familia-coml   
          AND  ttReceitaRealizada.CD_ClassifFiscal    = tt-calcula.ncm          
          AND  ttReceitaRealizada.CD_Item             = tt-calcula.it-codigo NO-ERROR.  


    IF  NOT AVAIL(ttReceitaRealizada)
    THEN DO:  
        CREATE ttReceitaRealizada.
        ASSIGN ttReceitaRealizada.CD_MasterExterno    = "1"                         
               ttReceitaRealizada.NM_AnoExtracao      = i-ano-extracao          
               ttReceitaRealizada.NM_MesExtracao      = i-mes-extracao          
               ttReceitaRealizada.CD_Empresa          = i-ep-codigo-usuario         
               ttReceitaRealizada.CD_Estabelecimeto   = tt-calcula.cod-estabel      
               ttReceitaRealizada.CD_Mercado          = SUBSTR(c-mercado,1,50)                   
               ttReceitaRealizada.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem))         
               ttReceitaRealizada.CD_Vertical         = tt-calcula.c-vertical                    
               ttReceitaRealizada.CD_UnidNegoc        = TRIM(fn-free-accent(tt-calcula.descricao-un-nota))
               ttReceitaRealizada.CD_Segmento         = SUBSTR(c-segmento,1,50)                  
               ttReceitaRealizada.CD_FamiliaComercial = c-familia-coml       
               ttReceitaRealizada.CD_ClassifFiscal    = tt-calcula.ncm              
               ttReceitaRealizada.CD_Item             = tt-calcula.it-codigo.
    END.

    IF  tt-calcula.tipo = 1 
    THEN
        ASSIGN ttReceitaRealizada.NM_ReceitaRealizada = ttReceitaRealizada.NM_ReceitaRealizada + tt-calcula.rec-sem-ipi.
    ELSE
        ASSIGN ttReceitaRealizada.NM_ReceitaRealizada = ttReceitaRealizada.NM_ReceitaRealizada - tt-calcula.rec-sem-ipi.
