 ...
 * Fato: Extrator dedu‡äes vendas realizado
 *
...
define output parameter table for ttDeducoesVendas.
...
DEFINE VARIABLE i-seq-deducao     AS INTEGER    NO-UNDO.
DEFINE VARIABLE c-deducoes        AS CHARACTER  NO-UNDO.
...
EMPTY TEMP-TABLE ttDeducoesVendas.
...
ASSIGN c-deducoes = "IPI;IPI Estorno;ICMS;PIS;COFINS;ISS;ICMS MCT;ICMS IMP;CPRB".
...
FOR EACH  tt-calcula
    ...
    ASSIGN c-unid-neg-item = TRIM(fn-free-accent(tt-calcula.descricao-un-nota)).
    ...
    FOR FIRST ITEM NO-LOCK
       ...
       THEN DO:
           FOR FIRST unid_negoc NO-LOCK
               ...
               ASSIGN c-unid-neg-item = TRIM(fn-free-accent(unid_negoc.des_unid_negoc)).
           END.
       END.
    END.
    ...
    FIND int-classif-fisc NO-LOCK
         WHERE int-classif-fisc.class-fiscal = tt-calcula.ncm NO-ERROR.
...
/*     END.                                                                                                                                */
    ...
    DO i-seq-deducao = 1 TO 9:
        FIND FIRST ttDeducoesVendas
            WHERE  ttDeducoesVendas.CD_MasterExterno    = "1"
              AND  ttDeducoesVendas.NM_AnoExtracao      = i-ano-extracao
              AND  ttDeducoesVendas.NM_MesExtracao      = i-mes-extracao
              AND  ttDeducoesVendas.CD_Empresa          = i-ep-codigo-usuario
              AND  ttDeducoesVendas.CD_Estabelecimeto   = tt-calcula.cod-estabel  
              AND  ttDeducoesVendas.CD_Mercado          = SUBSTR(c-mercado,1,50)    
              AND  ttDeducoesVendas.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem))     
              AND  ttDeducoesVendas.CD_Vertical         = tt-calcula.c-vertical   
              AND  ttDeducoesVendas.CD_UnidNegoc        = c-unid-neg-item
              AND  ttDeducoesVendas.CD_Segmento         = SUBSTR(c-segmento,1,50) 
              AND  ttDeducoesVendas.CD_FamiliaComercial = c-familia-coml   
              AND  ttDeducoesVendas.CD_ClassifFiscal    = tt-calcula.ncm          
              AND  ttDeducoesVendas.CD_Item             = tt-calcula.it-codigo 
              AND  ttDeducoesVendas.CD_Deducao          = ENTRY(i-seq-deducao,c-deducoes,";") NO-ERROR.
    
        IF  NOT AVAIL(ttDeducoesVendas)
        THEN DO:  
            CREATE ttDeducoesVendas.
            ASSIGN ttDeducoesVendas.CD_MasterExterno    = "1"                         
                   ttDeducoesVendas.NM_AnoExtracao      = i-ano-extracao
                   ttDeducoesVendas.NM_MesExtracao      = i-mes-extracao
                   ttDeducoesVendas.CD_Empresa          = i-ep-codigo-usuario         
                   ttDeducoesVendas.CD_Estabelecimeto   = tt-calcula.cod-estabel      
                   ttDeducoesVendas.CD_Mercado          = SUBSTR(c-mercado,1,50)                   
                   ttDeducoesVendas.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem)) 
                   ttDeducoesVendas.CD_Vertical         = tt-calcula.c-vertical                    
                   ttDeducoesVendas.CD_UnidNegoc        = c-unid-neg-item
                   ttDeducoesVendas.CD_Segmento         = SUBSTR(c-segmento,1,50)                  
                   ttDeducoesVendas.CD_FamiliaComercial = c-familia-coml       
                   ttDeducoesVendas.CD_ClassifFiscal    = tt-calcula.ncm              
                   ttDeducoesVendas.CD_Item             = tt-calcula.it-codigo
                   ttDeducoesVendas.CD_Deducao          = ENTRY(i-seq-deducao,c-deducoes,";").
        END. /* IF  NOT AVAIL(ttDeducoesVendas) */

        CASE i-seq-deducao:
            WHEN(1) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.ipi         * i-fator).
            WHEN(2) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.ipi         * i-fator). /* Estorno IPI */  
            WHEN(3) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-icms     * i-fator).                
            WHEN(4) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-pis      * i-fator).                 
            WHEN(5) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-cofins   * i-fator).              
            WHEN(6) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-iss      * i-fator).                 
            WHEN(7) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-icms-cp  * i-fator).             
            WHEN(8) THEN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + (tt-calcula.vl-icms-cpi * i-fator).            
            WHEN(9) 
                THEN DO:
                    IF AVAIL int-classif-fisc AND
                             int-classif-fisc.inss-faturamento = YES 
                    THEN 
                        ASSIGN ttDeducoesVendas.NM_ValorDeducao = ttDeducoesVendas.NM_ValorDeducao + ((tt-calcula.rec-sem-ip * 1 / 100)  * i-fator). /* CPRB */ 
                END.
        END CASE.

        ASSIGN i-seq-deducao = i-seq-deducao + 1.

    END. /* DO i-seq-desp-dedu = 1 TO 9: */
