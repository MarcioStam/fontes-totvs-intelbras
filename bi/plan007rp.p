 ...
 * Fato: Extrator despesas vendas realizado
 *
...
define output parameter table for ttDespesasVendas.
...
DEFINE VARIABLE i-seq-despesa     AS INTEGER    NO-UNDO.
DEFINE VARIABLE c-despesas        AS CHARACTER  NO-UNDO.
...
EMPTY TEMP-TABLE ttDespesasVendas.
...
ASSIGN c-despesas = "Fretes;ComissÆo;Acordo;Canais;Lei Inform tica;Lei Estorno".
...
/*     END.                                                                                                                                */
    ...
    DO i-seq-despesa = 1 TO 6:
        FIND FIRST ttDespesasVendas
            WHERE  ttDespesasVendas.CD_MasterExterno    = "1"
              AND  ttDespesasVendas.NM_AnoExtracao      = i-ano-extracao
              AND  ttDespesasVendas.NM_MesExtracao      = i-mes-extracao
              AND  ttDespesasVendas.CD_Empresa          = i-ep-codigo-usuario
              AND  ttDespesasVendas.CD_Estabelecimeto   = tt-calcula.cod-estabel  
              AND  ttDespesasVendas.CD_Mercado          = SUBSTR(c-mercado,1,50)    
              AND  ttDespesasVendas.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem))     
              AND  ttDespesasVendas.CD_Vertical         = tt-calcula.c-vertical   
              AND  ttDespesasVendas.CD_UnidNegoc        = c-unid-neg-item
              AND  ttDespesasVendas.CD_Segmento         = SUBSTR(c-segmento,1,50) 
              AND  ttDespesasVendas.CD_FamiliaComercial = c-familia-coml   
              AND  ttDespesasVendas.CD_ClassifFiscal    = tt-calcula.ncm          
              AND  ttDespesasVendas.CD_Item             = tt-calcula.it-codigo 
              AND  ttDespesasVendas.CD_Despesa          = ENTRY(i-seq-despesa,c-despesas,";") NO-ERROR.
    
        IF  NOT AVAIL(ttDespesasVendas)
        THEN DO:  
            CREATE ttDespesasVendas.
            ASSIGN ttDespesasVendas.CD_MasterExterno    = "1"                         
                   ttDespesasVendas.NM_AnoExtracao      = i-ano-extracao
                   ttDespesasVendas.NM_MesExtracao      = i-mes-extracao
                   ttDespesasVendas.CD_Empresa          = i-ep-codigo-usuario         
                   ttDespesasVendas.CD_Estabelecimeto   = tt-calcula.cod-estabel      
                   ttDespesasVendas.CD_Mercado          = SUBSTR(c-mercado,1,50)                   
                   ttDespesasVendas.CD_Origem           = TRIM(fn-free-accent(tt-calcula.c-origem))         
                   ttDespesasVendas.CD_Vertical         = tt-calcula.c-vertical                    
                   ttDespesasVendas.CD_UnidNegoc        = c-unid-neg-item
                   ttDespesasVendas.CD_Segmento         = SUBSTR(c-segmento,1,50)                  
                   ttDespesasVendas.CD_FamiliaComercial = c-familia-coml       
                   ttDespesasVendas.CD_ClassifFiscal    = tt-calcula.ncm              
                   ttDespesasVendas.CD_Item             = tt-calcula.it-codigo
                   ttDespesasVendas.CD_Despesa          = ENTRY(i-seq-despesa,c-despesas,";").
        END. /* IF  NOT AVAIL(ttDespesasVendas) */

        CASE i-seq-despesa:
            WHEN(1) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (tt-calcula.frete             * i-fator).
            WHEN(2) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (tt-calcula.comissao-distrato * i-fator).  
            WHEN(3) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (tt-calcula.vl-acordo         * i-fator).                
            WHEN(4) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (0                            * i-fator). /* Canais, verificar regra */                
            WHEN(5) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (tt-calcula.vpc               * i-fator). /* Lei inform tica         */              
            WHEN(6) THEN ttDespesasVendas.NM_ValorDespesa = ttDespesasVendas.NM_ValorDespesa + (tt-calcula.vpc               * i-fator). /* Estorno lei inform tica */
        END CASE.
