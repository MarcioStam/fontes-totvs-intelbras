...
*******************************************************************************/
{include/i-prgvrs.i G01PRM4103 1.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button RECT-1 c_cod-proj-int bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod-proj-int 
...
DEFINE VARIABLE c_cod-proj-int AS CHARACTER FORMAT "x(10)":U 
...
DEFINE FRAME d-vapara
     c_cod-proj-int AT ROW 2.21 COL 28.14 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     rt-button AT ROW 4.25 COL 1
     RECT-1 AT ROW 1.33 COL 2.14 WIDGET-ID 4
     SPACE(0.70) SKIP(1.74)
...
DO:
FIND prm-projeto-integrador NO-LOCK
    WHERE prm-projeto-integrador.cod-proj-int EQ INPUT FRAME {&FRAME-NAME} c_cod-proj-int NO-ERROR.

IF NOT AVAILABLE prm-projeto-integrador THEN DO:
    {utp/ut-table.i mgdes prm-projeto-integrador 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(prm-projeto-integrador).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod-proj-int 
      WITH FRAME d-vapara.
  ENABLE rt-button RECT-1 c_cod-proj-int bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01PRM4103" "1.00.00.000"}
